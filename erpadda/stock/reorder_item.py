...
from vmraid.utils import add_days, cint, flt, nowdate
...
def reorder_item():
	...
	if not (vmraid.db.a_row_exists("Company") and vmraid.db.a_row_exists("Fiscal Year")):
		return

	if cint(vmraid.db.get_value("Stock Settings", None, "auto_indent")):
		return _reorder_item()


def _reorder_item():
	material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
	warehouse_company = vmraid._dict(
		vmraid.db.sql(
	...
	default_company = (
		erpadda.get_default_company() or vmraid.db.sql("""select name from tabCompany limit 1""")[0][0]
	)

	items_to_consider = vmraid.db.sql_list(
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in vmraid.db.sql(
		...
		warehouse_doc = vmraid.get_doc("Warehouse", warehouse)
		...
		while warehouse_doc.parent_warehouse:
			...
			warehouse_doc = vmraid.get_doc("Warehouse", warehouse_doc.parent_warehouse)
...
def create_material_request(material_requests):
	...
	def _log_exception():
		if vmraid.local.message_log:
			exceptions_list.extend(vmraid.local.message_log)
			vmraid.local.message_log = []
		else:
			exceptions_list.append(vmraid.get_traceback())
	...
	for request_type in material_requests:
		for company in material_requests[request_type]:
			try:
				...
				for d in items:
					d = vmraid._dict(d)
					item = vmraid.get_doc("Item", d.item_code)
					uom = item.stock_uom
					...
					must_be_whole_number = vmraid.db.get_value("UOM", uom, "must_be_whole_number", cache=True)
	...
	if mr_list:
		if getattr(vmraid.local, "reorder_email_notify", None) is None:
			vmraid.local.reorder_email_notify = cint(
				vmraid.db.get_value("Stock Settings", None, "reorder_email_notify")
			)

		if vmraid.local.reorder_email_notify:
...
def send_email_notification(mr_list):
	...
	msg = vmraid.render_template("templates/emails/reorder_item.html", {"mr_list": mr_list})

	vmraid.sendmail(recipients=email_list, subject=_("Auto Material Requests Generated"), message=msg)


def notify_errors(exceptions_list):
	subject = _("[Important] [ERPAdda] Auto Reorder Errors")
	content = (
