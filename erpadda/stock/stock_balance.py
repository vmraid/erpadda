...
import vmraid
from vmraid.utils import cstr, flt, now, nowdate, nowtime

from erpadda.controllers.stock_controller import create_repost_item_valuation_entry
...
def set_stock_balance_as_per_serial_no(
	...
	for d in bin:
		...
		sle_doc = vmraid.get_doc(sle_dict)
...
def reset_serial_no_status_and_warehouse(serial_nos=None):
	if not serial_nos:
		serial_nos = vmraid.db.sql_list("""select name from `tabSerial No` where docstatus = 0""")
		for serial_no in serial_nos:
			try:
				sr = vmraid.get_doc("Serial No", serial_no)
