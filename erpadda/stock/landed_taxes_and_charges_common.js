	...
	vmraid.ui.form.on(doctype, {
		...
		set_account_currency: function(frm, cdt, cdn) {
			...
			if (row.expense_account) {
				vmraid.db.get_value('Account', row.expense_account, 'account_currency', function(value) {
					vmraid.model.set_value(cdt, cdn, "account_currency", value.account_currency);
		...
		set_exchange_rate: function(frm, cdt, cdn) {
			...
			if (row.account_currency == company_currency) {
				...
				vmraid.call({
					...
					callback: function(r) {
						if (r.message) {
							vmraid.model.set_value(cdt, cdn, "exchange_rate", r.message);
						}
					}
				});
			}
		...
		set_base_amount: function(frm, cdt, cdn) {
			let row = locals[cdt][cdn];
			vmraid.model.set_value(cdt, cdn, "base_amount",
