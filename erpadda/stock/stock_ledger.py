...
from vmraid.utils import cint, cstr, flt, get_link_to_form, getdate, now, nowdate
from pypika import CustomFunction

import erpadda
from erpadda.stock.doctype.bin.bin import update_qty as update_bin_qty
from erpadda.stock.utils import (
	get_incoming_outgoing_rate_for_cancel,
	get_or_make_bin,
	get_valuation_method,
)
from erpadda.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero


class NegativeStockError(vmraid.ValidationError):
	pass


class SerialNoExistsInFutureTransaction(vmraid.ValidationError):
	pass
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from erpadda.controllers.stock_controller import future_sle_exists
	...
	if sl_entries:
		...
		for sle in sl_entries:
			...
			is_stock_item = vmraid.get_cached_value("Item", args.get("item_code"), "is_stock_item")
			if is_stock_item:
...
def validate_serial_no(sle):
	...
	for sn in get_serial_nos(sle.serial_no):
		...
		for row in get_stock_ledger_entries(args, ">"):
			voucher_type = vmraid.bold(row.voucher_type)
			voucher_no = vmraid.bold(get_link_to_form(row.voucher_type, row.voucher_no))
			vouchers.append(f"{voucher_type} {voucher_no}")

		if vouchers:
			serial_no = vmraid.bold(sn)
			msg = (
			...
			vmraid.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransaction)


def validate_cancellation(args):
	if args[0].get("is_cancelled"):
		repost_entry = vmraid.db.get_value(
		...
		if repost_entry:
			...
			if repost_entry.status == "Queued":
				doc = vmraid.get_doc("Repost Item Valuation", repost_entry.name)
				doc.status = "Skipped"
...
def validate_item_warehouse(args):
	for field in ["item_code", "warehouse", "posting_date", "posting_time"]:
		if not args.get(field):
			validation_msg = f"The field {vmraid.unscrub(args.get(field))} is required for the reposting"
			vmraid.throw(_(validation_msg))
...
def get_distinct_item_warehouse(args=None, doc=None):
	...
	if doc and doc.distinct_item_and_warehouse:
		...
		distinct_item_warehouses = {
			vmraid.safe_eval(k): vmraid._dict(v) for k, v in distinct_item_warehouses.items()
		}
	else:
		for i, d in enumerate(args):
			distinct_item_warehouses.setdefault(
				(d.item_code, d.warehouse), vmraid._dict({"reposting_status": False, "sle": d, "args_idx": i})
			)
...
class update_entries_after(object):
	...
	def __init__(
		...
		self.args = vmraid._dict(args)
		if self.args.sle_id:
			self.args["name"] = self.args.sle_id

		self.company = vmraid.get_cached_value("Warehouse", self.args.warehouse, "company")
		self.get_precision()
		self.valuation_method = get_valuation_method(self.item_code)

		self.new_items_found = False
		self.distinct_item_warehouses = args.get("distinct_item_warehouses", vmraid._dict())
	...
	def get_precision(self):
		company_base_currency = vmraid.get_cached_value("Company", self.company, "default_currency")
		self.precision = get_field_precision(
			vmraid.get_meta("Stock Ledger Entry").get_field("stock_value"), currency=company_base_currency
		)
	...
	def initialize_previous_data(self, args):
		...
		"""
		self.data.setdefault(args.warehouse, vmraid._dict())
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle or vmraid._dict(
	...
	def update_distinct_item_warehouses(self, dependant_sle):
		...
		val = vmraid._dict({"sle": dependant_sle})
	...
	def append_future_sle_for_dependant(self, dependant_sle, entries_to_fix):
		...
		args = self.data[dependant_sle.warehouse].previous_sle or vmraid._dict(
	...
	def process_sle(self, sle):
		...
		elif sle.batch_no and vmraid.db.get_value(
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		if sle.voucher_type == "Stock Entry":
			...
			rate = vmraid.db.get_value("Stock Entry Detail", sle.voucher_detail_no, "valuation_rate")
		# Sales and Purchase Return
			...
			if vmraid.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from erpadda.controllers.sales_and_purchase_return import (
				...
				# check in item table
				item_code, incoming_rate = vmraid.db.get_value(
	...
	def update_rate_on_stock_entry(self, sle, outgoing_rate):
		vmraid.db.set_value("Stock Entry Detail", sle.voucher_detail_no, "basic_rate", outgoing_rate)
	...
	def recalculate_amounts_in_stock_entry(self, voucher_no):
		stock_entry = vmraid.get_doc("Stock Entry", voucher_no, for_update=True)
	...
	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		...
		item_code = vmraid.db.get_value(sle.voucher_type + " Item", sle.voucher_detail_no, "item_code")
		if item_code == sle.item_code:
			vmraid.db.set_value(
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if vmraid.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			vmraid.db.set_value(
		...
		if vmraid.get_cached_value(sle.voucher_type, sle.voucher_no, "is_subcontracted"):
			doc = vmraid.get_doc(sle.voucher_type, sle.voucher_no)
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		if ref_item_dt:
			return vmraid.db.get_value(ref_item_dt, voucher_detail_no, "allow_zero_valuation_rate")
		else:
			return 0
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=erpadda.get_company_currency(sle.company),
			company=sle.company,
			batch_no=sle.batch_no,
		)
	...
	def get_sle_before_datetime(self, args):
		...
		sle = sle[0] if sle else vmraid._dict()
		return sle
	...
	def raise_exceptions(self):
		...
		for warehouse, exceptions in self.exceptions.items():
			...
			) in vmraid.local.flags.currently_saving:

				msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
					abs(deficiency),
					vmraid.get_desk_link("Item", exceptions[0]["item_code"]),
					vmraid.get_desk_link("Warehouse", warehouse),
				)
			else:
				msg = _(
					...
					vmraid.get_desk_link(exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]),
				)
		...
		if msg_list:
			message = "\n\n".join(msg_list)
			if self.verbose:
				vmraid.throw(message, NegativeStockError, title=_("Insufficient Stock"))
			else:
	...
	def update_bin(self):
		...
		for warehouse, data in self.data.items():
			...
			vmraid.db.set_value("Bin", bin_name, updated_values)
...
def get_previous_sle_of_current_voucher(args, exclude_current_voucher=False):
	...
	return sle[0] if sle else vmraid._dict()
...
def get_stock_ledger_entries(
	...
	if check_serial_no and previous_sle.get("serial_no"):
		# conditions += " and serial_no like {}".format(vmraid.db.escape('%{0}%'.format(previous_sle.get("serial_no"))))
...
def get_batch_incoming_rate(
	...
	sle = vmraid.qb.DocType("Stock Ledger Entry")
...
def get_valuation_rate(
	...
	if not company:
		company = vmraid.get_cached_value("Warehouse", warehouse, "company")

	last_valuation_rate = None

	# Get moving average rate of a specific batch number
	if warehouse and batch_no and vmraid.db.get_value("Batch", batch_no, "use_batchwise_valuation"):
	...
	valuation_rate = vmraid.db.get_value("Item", item_code, "valuation_rate")

	if not valuation_rate:
		# try Item Standard rate
		valuation_rate = vmraid.db.get_value("Item", item_code, "standard_rate")

		if not valuation_rate:
			# try in price list
			valuation_rate = vmraid.db.get_value(
	...
	if (
		...
		and cint(erpadda.is_perpetual_inventory_enabled(company))
	):
		...
		vmraid.throw(msg=msg, title=_("Valuation Rate Missing"))
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if neg_sle:
		message = _(
			...
			vmraid.get_desk_link("Item", args.item_code),
			vmraid.get_desk_link("Warehouse", args.warehouse),
			neg_sle[0]["posting_date"],
			neg_sle[0]["posting_time"],
			vmraid.get_desk_link(neg_sle[0]["voucher_type"], neg_sle[0]["voucher_no"]),
		)
	...
	if neg_batch_sle:
		message = _(
			...
			vmraid.get_desk_link("Batch", args.batch_no),
			vmraid.get_desk_link("Warehouse", args.warehouse),
			neg_batch_sle[0]["posting_date"],
			neg_batch_sle[0]["posting_time"],
			vmraid.get_desk_link(neg_batch_sle[0]["voucher_type"], neg_batch_sle[0]["voucher_no"]),
		)
		vmraid.throw(message, NegativeStockError, title=_("Insufficient Stock for Batch"))
...
def is_negative_stock_allowed(*, item_code: Optional[str] = None) -> bool:
	if cint(vmraid.db.get_single_value("Stock Settings", "allow_negative_stock", cache=True)):
		return True
	if item_code and cint(vmraid.db.get_value("Item", item_code, "allow_negative_stock", cache=True)):
		return True
	return False
