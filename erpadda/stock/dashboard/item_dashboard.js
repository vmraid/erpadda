...
erpadda.stock.ItemDashboard = class ItemDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	make() {
		...
		this.content = $(vmraid.render_template('item_dashboard')).appendTo(this.parent);
		...
		this.content.on('click', '.btn-edit', function () {
			...
			vmraid.db.get_value('Putaway Rule', {
				'item_code': item,
				'warehouse': warehouse,
				'company': company
			}, 'name', (r) => {
				vmraid.set_route("Form", "Putaway Rule", r.name);
			});
		});
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					erpadda.stock.move_item(item, null, warehouse, actual_qty, rate, function () {
						me.refresh();
					});
				} else {
					erpadda.stock.move_item(item, warehouse, null, actual_qty, null, function () {
						me.refresh();
					});
				}
			}
		...
		function open_stock_entry(item, warehouse, entry_type) {
			vmraid.model.with_doctype('Stock Entry', function () {
				var doc = vmraid.model.get_new_doc('Stock Entry');
				if (entry_type) doc.stock_entry_type = entry_type;

				var row = vmraid.model.add_child(doc, 'items');
				row.item_code = item;
	...
	render(data) {
		...
		if (context.data.length > 0) {
			...
			$(vmraid.render_template(this.template, context)).appendTo(this.result);
		} else {
	...
	get_item_dashboard_data(data, max_count, show_item) {
		...
		let can_write = 0;
		if (vmraid.boot.user.can_write.indexOf("Stock Entry") >= 0) {
			can_write = 1;
		}
	...
	get_capacity_dashboard_data(data) {
		...
		let can_write = 0;
		if (vmraid.boot.user.can_write.indexOf("Putaway Rule") >= 0) {
			can_write = 1;
		}
...
erpadda.stock.move_item = function (item, source, target, actual_qty, rate, callback) {
