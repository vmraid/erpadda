...
# ERPAdda - web based ERP (http://erpadda.com)
# For license information, please see license.txt


import vmraid
from vmraid.tests.utils import VMRaidTestCase
from vmraid.utils import flt, today

from erpadda.stock.doctype.item.test_item import create_item
from erpadda.stock.doctype.material_request.material_request import (
	make_purchase_order,
	make_stock_entry,
...
class TestMaterialRequest(VMRaidTestCase):
	def test_make_purchase_order(self):
		mr = vmraid.copy_doc(test_records[0]).insert()

		self.assertRaises(vmraid.ValidationError, make_purchase_order, mr.name)

		mr = vmraid.get_doc("Material Request", mr.name)
		mr.submit()
	...
	def test_make_supplier_quotation(self):
		...
		self.assertRaises(vmraid.ValidationError, make_supplier_quotation, mr.name)
	...
	def test_make_stock_entry(self):
		...
		self.assertRaises(vmraid.ValidationError, make_stock_entry, mr.name)
	...
	def test_cannot_stop_cancelled_material_request(self):
		mr = vmraid.copy_doc(test_records[0])
		mr.insert()
		mr.submit()

		mr.load_from_db()
		mr.cancel()
		self.assertRaises(vmraid.ValidationError, mr.update_status, "Stopped")
	...
	def test_cannot_submit_cancelled_mr(self):
		...
		mr.load_from_db()
		mr.cancel()
		self.assertRaises(vmraid.ValidationError, mr.submit)
	...
	def test_cannot_change_cancelled_mr(self):
		...
		self.assertRaises(vmraid.InvalidStatusError, mr.update_status, "Draft")
		self.assertRaises(vmraid.InvalidStatusError, mr.update_status, "Stopped")
		self.assertRaises(vmraid.InvalidStatusError, mr.update_status, "Ordered")
		self.assertRaises(vmraid.InvalidStatusError, mr.update_status, "Issued")
		self.assertRaises(vmraid.InvalidStatusError, mr.update_status, "Transferred")
		self.assertRaises(vmraid.InvalidStatusError, mr.update_status, "Pending")
	...
	def test_cannot_delete_submitted_mr(self):
		...
		self.assertRaises(vmraid.ValidationError, mr.delete)
	...
	def test_completed_qty_for_purchase(self):
		...
		po = vmraid.copy_doc(po_doc)
		po.insert()
		po.load_from_db()
		mr.update_status("Stopped")
		self.assertRaises(vmraid.InvalidStatusError, po.submit)
		vmraid.db.set(po, "docstatus", 1)
		self.assertRaises(vmraid.InvalidStatusError, po.cancel)
	...
	def test_completed_qty_for_transfer(self):
		...
		# check for stopped status of Material Request
		se = vmraid.copy_doc(se_doc)
		se.insert()
		mr.update_status("Stopped")
		self.assertRaises(vmraid.InvalidStatusError, se.submit)
		...
		se.submit()
		mr.update_status("Stopped")
		self.assertRaises(vmraid.InvalidStatusError, se.cancel)
	...
	def test_over_transfer_qty_allowance(self):
		mr = vmraid.new_doc("Material Request")
		...
		vmraid.db.set_value("Stock Settings", None, "mr_qty_allowance", 20)
		...
		# make available the qty in _Test Warehouse 1 before transfer
		sr = vmraid.new_doc("Stock Reconciliation")
		...
		se = vmraid.copy_doc(se_doc)
		se.insert()
		self.assertRaises(vmraid.ValidationError)
		se.items[0].qty = 12
		se.submit()
	...
	def test_incorrect_mapping_of_stock_entry(self):
		...
		# check for stopped status of Material Request
		se = vmraid.copy_doc(se_doc)
		self.assertRaises(vmraid.MappingMismatchError, se.insert)
	...
	def test_material_request_type_manufacture(self):
		mr = vmraid.copy_doc(test_records[1]).insert()
		mr = vmraid.get_doc("Material Request", mr.name)
		mr.submit()
		completed_qty = mr.items[0].ordered_qty
		requested_qty = vmraid.db.sql(
		...
		po = vmraid.get_doc("Work Order", prod_order[0])
		...
		new_requested_qty = vmraid.db.sql(
			"""select indented_qty from `tabBin` where \
			item_code= %s and warehouse= %s """,
			(mr.items[0].item_code, mr.items[0].warehouse),
		)[0][0]
	...
	def test_requested_qty_multi_uom(self):
		...
		wo = vmraid.get_doc("Work Order", work_order[0])
		wo.qty = 50
	...
	def test_multi_uom_for_purchase(self):
		...
		if not vmraid.db.get_value("UOM Conversion Detail", {"parent": item.item_code, "uom": "Kg"}):
...
test_records = vmraid.get_test_records("Material Request")
