...
class ItemAttributeIncrementError(vmraid.ValidationError):
	pass
	...
	def validate_numeric(self):
		if self.numeric_values:
			...
			if self.from_range is None or self.to_range is None:
				vmraid.throw(_("Please specify from/to range"))

			elif flt(self.from_range) >= flt(self.to_range):
				vmraid.throw(_("From Range has to be less than To Range"))

			if not self.increment:
				vmraid.throw(_("Increment cannot be 0"), ItemAttributeIncrementError)
		else:
	...
	def validate_duplication(self):
		...
		for d in self.item_attribute_values:
			...
			if d.attribute_value in values:
				vmraid.throw(_("{0} must appear only once").format(d.attribute_value))
			values.append(d.attribute_value)

			if d.abbr in abbrs:
				vmraid.throw(_("{0} must appear only once").format(d.abbr))
			abbrs.append(d.abbr)
