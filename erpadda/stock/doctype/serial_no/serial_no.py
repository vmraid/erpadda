...
import vmraid
from vmraid import ValidationError, _
...
from erpadda.stock.get_item_details import get_reserved_qty_for_so
...
class SerialNo(StockController):
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			item_code, warehouse = vmraid.db.get_value("Serial No", self.name, ["item_code", "warehouse"])
			if not self.via_stock_ledger and item_code != self.item_code:
				vmraid.throw(_("Item Code cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				vmraid.throw(_("Warehouse cannot be changed for Serial No."), SerialNoCannotCannotChangeError)

	def validate_item(self):
		"""
		Validate whether serial no is required for this item
		"""
		item = vmraid.get_cached_doc("Item", self.item_code)
	...
	def set_purchase_details(self, purchase_sle):
		if purchase_sle:
			...
			if purchase_sle.voucher_type in ("Purchase Receipt", "Purchase Invoice"):
				self.supplier, self.supplier_name = vmraid.db.get_value(
	...
	def set_sales_details(self, delivery_sle):
		if delivery_sle:
			...
			if delivery_sle.voucher_type in ("Delivery Note", "Sales Invoice"):
				self.customer, self.customer_name = vmraid.db.get_value(
	...
	if item_det.has_serial_no == 0:
		...
		if serial_nos:
			...
			for serial_no in serial_nos:
				if vmraid.db.exists("Serial No", serial_no):
					...
					if cint(sle.actual_qty) > 0 and has_serial_no_exists(sr, sle):
						doc_name = vmraid.bold(get_link_to_form(sr.purchase_document_type, sr.purchase_document_no))
						vmraid.throw(
							_("Serial No {0} has already been received in the {1} #{2}").format(
								vmraid.bold(serial_no), sr.purchase_document_type, doc_name
							),
					...
					if (
						...
						return_against = vmraid.db.get_value(sle.voucher_type, sle.voucher_no, "return_against")
						if return_against and return_against != sr.delivery_document_no:
							vmraid.throw(_("Serial no {0} has been already returned").format(sr.name))
					...
					if cint(sle.actual_qty) < 0:
						...
						if not sr.purchase_document_no:
							vmraid.throw(_("Serial No {0} not in stock").format(serial_no), SerialNoNotExistsError)
						...
						if sle.voucher_type in ("Delivery Note", "Sales Invoice"):
							...
							if sr.sales_order:
								if sle.voucher_type == "Sales Invoice":
									...
									if not vmraid.db.exists(
										...
										if not invoice or vmraid.db.exists(
											"Sales Invoice Item",
							...
							if sle.voucher_type == "Sales Invoice":
								sales_order = vmraid.db.get_value(
									"Sales Invoice Item",
									{"parent": sle.voucher_no, "item_code": sle.item_code},
									"sales_order",
								)
								...
								if sales_order and get_reserved_qty_for_so(sales_order, sle.item_code):
									...
									sales_invoice = vmraid.get_value(
										"Delivery Note Item",
										{"parent": sle.voucher_no, "item_code": sle.item_code},
										"against_sales_invoice",
									)
...
def check_serial_no_validity_on_cancel(serial_no, sle):
	...
	sr_link = vmraid.utils.get_link_to_form("Serial No", serial_no)
	doc_link = vmraid.utils.get_link_to_form(sle.voucher_type, sle.voucher_no)
	...
	if sr and (actual_qty < 0 or is_stock_reco) and (sr.warehouse and sr.warehouse != sle.warehouse):
		...
		msg = _("Cannot cancel {0} {1} as Serial No {2} does not belong to the warehouse {3}").format(
			sle.voucher_type, doc_link, sr_link, vmraid.bold(sle.warehouse)
		)
		...
		if sr.warehouse:
			...
			msg = _("Cannot cancel {0} {1} as Serial No {2} is active in warehouse {3}").format(
				sle.voucher_type, doc_link, sr_link, vmraid.bold(sr.warehouse)
			)
			...
			msg = _("Cannot cancel {0} {1} as Serial No {2} does not belong to the company {3}").format(
				sle.voucher_type, doc_link, sr_link, vmraid.bold(sle.company)
			)

	if msg:
		vmraid.throw(msg, title=_("Cannot cancel"))
...
def validate_material_transfer_entry(sle_doc):
	...
	if (
		...
		and vmraid.get_cached_value("Stock Entry", sle_doc.voucher_no, "purpose") == "Material Transfer"
	):
...
def validate_so_serial_no(sr, sales_order):
	if not sr.sales_order or sr.sales_order != sales_order:
		...
		vmraid.throw(_("""{0} Serial No {1} cannot be delivered""").format(msg, sr.name))
...
def allow_serial_nos_with_different_item(sle_serial_no, sle):
	...
	if sle.voucher_type == "Stock Entry" and cint(sle.actual_qty) > 0:
		stock_entry = vmraid.get_cached_doc("Stock Entry", sle.voucher_no)
...
def get_new_serial_number(series):
	...
	if vmraid.db.exists("Serial No", sr_no):
...
def auto_make_serial_nos(args):
	...
	for serial_no in serial_nos:
		is_new = False
		if vmraid.db.exists("Serial No", serial_no):
			sr = vmraid.get_cached_doc("Serial No", serial_no)
		elif args.get("actual_qty", 0) > 0:
			sr = vmraid.new_doc("Serial No")
			is_new = True
	...
	if len(form_links) == 1:
		vmraid.msgprint(_("Serial No {0} Created").format(form_links[0]), singular_title)
		...
		vmraid.msgprint(message, multiple_title)
		...
		for sle in stock_ledger_entries:
			if sle.voucher_detail_no == d.name:
				if (
					...
					vmraid.db.set_value(d.doctype, d.name, "serial_no", sle.serial_no)
					...
					vmraid.db.set_value(d.doctype, d.name, "rejected_serial_no", sle.serial_no)
...
def update_maintenance_status():
	...
	for serial_no in serial_nos:
		doc = vmraid.get_doc("Serial No", serial_no[0])
		doc.set_maintenance_status()
		vmraid.db.set_value("Serial No", doc.name, "maintenance_status", doc.maintenance_status)


def get_delivery_note_serial_no(item_code, qty, delivery_note):
	serial_nos = ""
	dn_serial_nos = vmraid.db.sql_list(
...
def auto_fetch_serial_number(
	...
	filters = vmraid._dict({"item_code": item_code, "warehouse": warehouse})
...
def get_delivered_serial_nos(serial_nos):
	...
	SerialNo = vmraid.qb.DocType("Serial No")
	serial_nos = get_serial_nos(serial_nos)
	query = (
		vmraid.qb.select(SerialNo.name)
		.from_(SerialNo)
...
def get_pos_reserved_serial_nos(filters):
	...
	POSInvoiceItem = vmraid.qb.DocType("POS Invoice Item")
	query = (
		vmraid.qb.from_(POSInvoice)
...
def fetch_serial_numbers(filters, qty, do_not_include=None):
	...
	serial_no = vmraid.qb.DocType("Serial No")

	query = (
		vmraid.qb.from_(serial_no)
