...
test_records = vmraid.get_test_records("Serial No")


class TestSerialNo(VMRaidTestCase):
	def tearDown(self):
		vmraid.db.rollback()

	def test_cannot_create_direct(self):
		vmraid.delete_doc_if_exists("Serial No", "_TCSER0001")
	...
	def test_inter_company_transfer(self):
		...
		serial_no = vmraid.get_doc("Serial No", serial_nos[0])
	...
	def test_inter_company_transfer_intermediate_cancellation(self):
		...
		sn_doc = vmraid.get_doc("Serial No", serial_nos[0])
		...
		self.assertRaises(vmraid.ValidationError, se.cancel)
		...
		self.assertRaises(vmraid.ValidationError, dn.cancel)
		...
		self.assertRaises(vmraid.ValidationError, se.cancel)
		self.assertRaises(vmraid.ValidationError, dn.cancel)
		self.assertRaises(vmraid.ValidationError, pr.cancel)
	...
	def test_auto_fetch(self):
		...
		# expiry date
		vmraid.db.set_value("Batch", batch1, "expiry_date", "1980-01-01")
