...
from erpadda.stock.doctype.batch.test_batch import make_new_batch
...
class TestPutawayRule(VMRaidTestCase):
	def setUp(self):
		if not vmraid.db.exists("Item", "_Rice"):
			make_item(
				"_Rice", {"is_stock_item": 1, "has_batch_no": 1, "create_new_batch": 1, "stock_uom": "Kg"}
			)

		if not vmraid.db.exists("Warehouse", {"warehouse_name": "Rack 1"}):
			create_warehouse("Rack 1")
		if not vmraid.db.exists("Warehouse", {"warehouse_name": "Rack 2"}):
			create_warehouse("Rack 2")

		self.warehouse_1 = vmraid.db.get_value("Warehouse", {"warehouse_name": "Rack 1"})
		self.warehouse_2 = vmraid.db.get_value("Warehouse", {"warehouse_name": "Rack 2"})

		if not vmraid.db.exists("UOM", "Bag"):
	...
	def test_putaway_rules_multi_uom(self):
		...
		item = vmraid.get_doc("Item", "_Rice")
		if not vmraid.db.get_value("UOM Conversion Detail", {"parent": "_Rice", "uom": "Bag"}):
	...
	def test_putaway_rules_multi_uom_whole_uom(self):
		...
		vmraid.db.set_value("UOM", "Bag", "must_be_whole_number", 1)
	...
	def test_validate_over_receipt_in_warehouse(self):
		...
		self.assertRaises(vmraid.ValidationError, pr.save)
	...
	def test_putaway_rule_on_stock_entry_material_transfer_batch_serial_item(self):
		...
		if not vmraid.db.exists("Item", "Water Bottle"):
			make_item(
				"Water Bottle",
				{
					"is_stock_item": 1,
	...
	def test_warehouse_capacity_dashbord(self):
		from erpadda.stock.dashboard.warehouse_capacity_dashboard import get_data
...
def create_putaway_rule(**args):
	...
	putaway = vmraid.new_doc("Putaway Rule")
	...
	putaway.stock_uom = vmraid.db.get_value("Item", putaway.item_code, "stock_uom")
