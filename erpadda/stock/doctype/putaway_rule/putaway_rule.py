...
from vmraid.utils import cint, cstr, floor, flt, nowdate
...
class PutawayRule(Document):
	...
	def validate_duplicate_rule(self):
		existing_rule = vmraid.db.exists(
		...
		if existing_rule and existing_rule != self.name:
			vmraid.throw(
				_("Putaway Rule already exists for Item {0} in Warehouse {1}.").format(
					vmraid.bold(self.item_code), vmraid.bold(self.warehouse)
				),
				title=_("Duplicate"),
			)
	...
	def validate_priority(self):
		if self.priority < 1:
			vmraid.throw(_("Priority cannot be lesser than 1."), title=_("Invalid Priority"))

	def validate_warehouse_and_company(self):
		company = vmraid.db.get_value("Warehouse", self.warehouse, "company")
		if company != self.company:
			vmraid.throw(
				_("Warehouse {0} does not belong to Company {1}.").format(
					vmraid.bold(self.warehouse), vmraid.bold(self.company)
				),
				title=_("Invalid Warehouse"),
			)

	def validate_capacity(self):
		stock_uom = vmraid.db.get_value("Item", self.item_code, "stock_uom")
		...
		if flt(self.stock_capacity) < flt(balance_qty):
			vmraid.throw(
				...
				).format(self.item_code, vmraid.bold(balance_qty), stock_uom),
				title=_("Insufficient Capacity"),
			)

		if not self.capacity:
			vmraid.throw(_("Capacity must be greater than 0"), title=_("Invalid"))
...
def get_available_putaway_capacity(rule):
	stock_capacity, item_code, warehouse = vmraid.db.get_value(
...
def apply_putaway_rule(doctype, items, company, sync=None, purpose=None):
	...
	for item in items:
		...
		uom_must_be_whole_number = vmraid.db.get_value("UOM", item.uom, "must_be_whole_number")
	...
	if updated_table and _items_changed(items, updated_table, doctype):
		items[:] = updated_table
		vmraid.msgprint(_("Applied putaway rules."), alert=True)
...
def _items_changed(old, new, doctype: str) -> bool:
	...
	old = [vmraid._dict(item) if isinstance(item, dict) else item for item in old]
...
def show_unassigned_items_message(items_not_accomodated):
	...
	for entry in items_not_accomodated:
		item_link = vmraid.utils.get_link_to_form("Item", entry[0])
		formatted_item_rows += """
			<td>{0}</td>
			<td>{1}</td>
		</tr>""".format(
			item_link, vmraid.bold(entry[1])
		)
	...
	vmraid.msgprint(msg, title=_("Insufficient Capacity"), is_minimizable=True, wide=True)
