...
class Bin(Document):
	def before_save(self):
		if self.get("__islocal") or not self.stock_uom:
			self.stock_uom = vmraid.get_cached_value("Item", self.item_code, "stock_uom")
	...
	def update_reserved_qty_for_production(self):
		...
		in open work orders"""
		from erpadda.manufacturing.doctype.work_order.work_order import get_reserved_qty_for_production
	...
	def update_reserved_qty_for_sub_contracting(self):
		...
		po = vmraid.qb.DocType("Purchase Order")
		supplied_item = vmraid.qb.DocType("Purchase Order Item Supplied")
		...
		se = vmraid.qb.DocType("Stock Entry")
		se_item = vmraid.qb.DocType("Stock Entry Detail")
...
def on_doctype_update():
	vmraid.db.add_unique("Bin", ["item_code", "warehouse"], constraint_name="unique_item_warehouse")
