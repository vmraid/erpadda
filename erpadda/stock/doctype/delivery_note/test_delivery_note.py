...
from vmraid.utils import cstr, flt, nowdate, nowtime
...
from erpadda.stock.doctype.delivery_note.delivery_note import (
	make_delivery_trip,
	make_sales_invoice,
)
from erpadda.stock.doctype.item.test_item import make_item
from erpadda.stock.doctype.purchase_receipt.test_purchase_receipt import get_gl_entries
from erpadda.stock.doctype.serial_no.serial_no import SerialNoWarehouseError, get_serial_nos
...
class TestDeliveryNote(VMRaidTestCase):
	def test_over_billing_against_dn(self):
		vmraid.db.set_value("Stock Settings", None, "allow_negative_stock", 1)

		dn = create_delivery_note(do_not_submit=True)
		self.assertRaises(vmraid.ValidationError, make_sales_invoice, dn.name)
		...
		self.assertRaises(vmraid.ValidationError, vmraid.get_doc(si).insert)

	def test_delivery_note_no_gl_entry(self):
		company = vmraid.db.get_value("Warehouse", "_Test Warehouse - _TC", "company")
	...
	# 	stock_value_difference = abs(vmraid.db.get_value("Stock Ledger Entry",
	...
	def test_sales_return_for_non_bundled_items_partial(self):
		...
		returned = vmraid.get_doc("Delivery Note", dn1.name)
	...
	def test_delivery_of_bundled_items_to_target_warehouse(self):
		...
		stock_value_difference1 = vmraid.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Delivery Note",
				"voucher_no": dn.name,
	...
	def test_closed_delivery_note(self):
		from erpadda.stock.doctype.delivery_note.delivery_note import update_delivery_note_status
		...
		self.assertEqual(vmraid.db.get_value("Delivery Note", dn.name, "Status"), "Closed")
	...
	def test_dn_billing_status_case3(self):
		...
		from erpadda.selling.doctype.sales_order.sales_order import make_delivery_note
		from erpadda.selling.doctype.sales_order.sales_order import (
	...
	def test_dn_billing_status_case4(self):
		# SO -> SI -> DN
		from erpadda.accounts.doctype.sales_invoice.sales_invoice import make_delivery_note
...
def create_delivery_note(**args):
	dn = vmraid.new_doc("Delivery Note")
