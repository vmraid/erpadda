...
from erpadda.stock.doctype.serial_no.serial_no import get_delivery_note_serial_no
...
class DeliveryNote(SellingController):
	...
	def before_print(self, settings=None):
		...
		item_meta = vmraid.get_meta("Delivery Note Item")
		print_hide_fields = {
	...
	def so_required(self):
		...
		if vmraid.db.get_value("Selling Settings", None, "so_required") == "Yes":
			for d in self.get("items"):
				if not d.against_sales_order:
					vmraid.throw(_("Sales Order required for Item {0}").format(d.item_code))
	...
	def check_next_docstatus(self):
		...
		if submit_rv:
			vmraid.throw(_("Sales Invoice {0} has already been submitted").format(submit_rv[0][0]))

		submit_in = vmraid.db.sql(
			"""select t1.name
		...
		if submit_in:
			vmraid.throw(_("Installation Note {0} has already been submitted").format(submit_in[0][0]))
	...
	def cancel_packing_slips(self):
		...
		if res:
			for r in res:
				ps = vmraid.get_doc("Packing Slip", r[0])
				ps.cancel()
			vmraid.msgprint(_("Packing Slip(s) cancelled"))
	...
	def update_billing_status(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			dn_doc = self if (dn == self.name) else vmraid.get_doc("Delivery Note", dn)
	...
	def make_return_invoice(self):
		try:
			...
			credit_note_link = vmraid.utils.get_link_to_form("Sales Invoice", return_invoice.name)

			vmraid.msgprint(_("Credit Note {0} has been created automatically").format(credit_note_link))
		except Exception:
			vmraid.throw(
				_(
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
	...
	si_item = vmraid.qb.DocType("Sales Invoice Item").as_("si_item")
	sum_amount = Sum(si_item.amount).as_("amount")

	billed_against_so = (
		vmraid.qb.from_(si_item)
		.select(sum_amount)
		.where(
	...
	dn = vmraid.qb.DocType("Delivery Note").as_("dn")
	dn_item = vmraid.qb.DocType("Delivery Note Item").as_("dn_item")
	...
	for dnd in dn_details:
		...
		if dnd.si_detail:
			...
			billed_amt_agianst_dn = vmraid.db.sql(
...
def get_invoiced_qty_map(delivery_note):
	...
	for dn_detail, qty in vmraid.db.sql(
...
def make_sales_invoice(source_name, target_doc=None):
	doc = vmraid.get_doc("Delivery Note", source_name)
	...
	def set_missing_values(source, target):
		...
		if len(target.get("items")) == 0:
			vmraid.throw(_("All these items have already been Invoiced/Returned"))
...
def make_shipment(source_name, target_doc=None):
	def postprocess(source, target):
		user = vmraid.db.get_value(
			"User", vmraid.session.user, ["email", "full_name", "phone", "mobile_no"], as_dict=1
		)
		...
		target.pickup_contact_person = vmraid.session.user

		contact = vmraid.db.get_value(
...
def update_delivery_note_status(docname, status):
	dn = vmraid.get_doc("Delivery Note", docname)
...
def on_doctype_update():
	vmraid.db.add_index("Delivery Note", ["customer", "is_return", "return_against"])
