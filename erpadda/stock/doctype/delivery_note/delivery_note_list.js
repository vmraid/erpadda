vmraid.listview_settings['Delivery Note'] = {
	...
	onload: function (listview) {
		const action = () => {
			...
			if (selected_docs.length > 0) {
				for (let doc of selected_docs) {
					if (!doc.docstatus) {
						vmraid.throw(__("Cannot create a Delivery Trip from Draft documents."));
					}
				};

				vmraid.new_doc("Delivery Trip")
					.then(() => {
		...
		listview.page.add_action_item(__("Sales Invoice"), ()=>{
			erpadda.bulk_transaction_processing.create(listview, "Delivery Note", "Sales Invoice");
		});

		listview.page.add_action_item(__("Packaging Slip From Delivery Note"), ()=>{
			erpadda.bulk_transaction_processing.create(listview, "Delivery Note", "Packing Slip");
		});
	}
};
