...
vmraid.provide("erpadda.stock.delivery_note");
...
vmraid.ui.form.on("Delivery Note", {
	...
	print_without_amount: function(frm) {
		erpadda.stock.delivery_note.set_print_hide(frm.doc);
	},
		...
		if (frm.doc.docstatus == 1 && !frm.doc.inter_company_reference) {
			...
			if (internal) {
				...
				me.frm.add_custom_button(button_label, function() {
					vmraid.model.open_mapped_doc({
						method: 'erpadda.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt',
						frm: frm,
					});
				}, __('Create'));
			}
...
vmraid.ui.form.on("Delivery Note Item", {
...
erpadda.stock.DeliveryNoteController = class DeliveryNoteController extends erpadda.selling.SellingController {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
		...
		if (!doc.is_return && doc.status!="Closed") {
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				this.frm.add_custom_button(__('Packing Slip'), function() {
					vmraid.model.open_mapped_doc({
						method: "erpadda.stock.doctype.delivery_note.delivery_note.make_packing_slip",
						frm: me.frm
					}) }, __('Create'));
			}
		...
		erpadda.stock.delivery_note.set_print_hide(doc, dt, dn);
	...
	make_shipment() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm
		})
	}
	...
	make_sales_return() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm
		})
	}
	...
	update_status(status) {
		...
		vmraid.call({
			method:"erpadda.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new erpadda.stock.DeliveryNoteController({frm: cur_frm}));
...
vmraid.ui.form.on('Delivery Note', {
	...
	unhide_account_head: function(frm) {
		...
		var aii_enabled = erpadda.is_perpetual_inventory_enabled(frm.doc.company)
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	}
})


erpadda.stock.delivery_note.set_print_hide = function(doc, cdt, cdn){
	var dn_fields = vmraid.meta.docfield_map['Delivery Note'];
	var dn_item_fields = vmraid.meta.docfield_map['Delivery Note Item'];
