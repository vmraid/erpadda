...
vmraid.ui.form.on('Pick List', {
	setup: (frm) => {
		...
		frm.set_query('work_order', () => {
			return {
				query: 'erpadda.stock.doctype.pick_list.pick_list.get_pending_work_orders',
				filters: {
					'company': frm.doc.company
				}
			};
	...
	set_item_locations:(frm, save) => {
		if (!(frm.doc.locations && frm.doc.locations.length)) {
			vmraid.msgprint(__('Add items in the Item Locations table'));
		} else {
	...
	refresh: (frm) => {
		...
		if (frm.doc.docstatus === 1) {
			vmraid.xcall('erpadda.stock.doctype.pick_list.pick_list.target_document_exists', {
	...
	work_order: (frm) => {
		vmraid.db.get_value('Work Order',
			frm.doc.work_order,
			...
			vmraid.prompt({
				...
				erpadda.utils.map_current_doc({
					method: 'erpadda.manufacturing.doctype.work_order.work_order.create_pick_list',
					target: frm,
	...
	create_delivery_note: (frm) => {
		vmraid.model.open_mapped_doc({
			method: 'erpadda.stock.doctype.pick_list.pick_list.create_delivery_note',
			frm: frm
		});

	},
	create_stock_entry: (frm) => {
		vmraid.xcall('erpadda.stock.doctype.pick_list.pick_list.create_stock_entry', {
			'pick_list': frm.doc,
		}).then(stock_entry => {
			vmraid.model.sync(stock_entry);
			vmraid.set_route("Form", 'Stock Entry', stock_entry.name);
		});
	},
...
vmraid.ui.form.on('Pick List Item', {
	item_code: (frm, cdt, cdn) => {
		let row = vmraid.get_doc(cdt, cdn);
		if (row.item_code) {
			get_item_details(row.item_code).then(data => {
				vmraid.model.set_value(cdt, cdn, 'uom', data.stock_uom);
				vmraid.model.set_value(cdt, cdn, 'stock_uom', data.stock_uom);
				vmraid.model.set_value(cdt, cdn, 'conversion_factor', 1);
			});
		}
	},
	uom: (frm, cdt, cdn) => {
		...
		if (row.uom) {
			get_item_details(row.item_code, row.uom).then(data => {
				vmraid.model.set_value(cdt, cdn, 'conversion_factor', data.conversion_factor);
			});
		}
	},
	qty: (frm, cdt, cdn) => {
		let row = vmraid.get_doc(cdt, cdn);
		vmraid.model.set_value(cdt, cdn, 'stock_qty', row.qty * row.conversion_factor);
	},
...
function get_item_details(item_code, uom=null) {
	if (item_code) {
		return vmraid.xcall('erpadda.stock.doctype.pick_list.pick_list.get_item_details', {
			item_code,
			uom
		});
	}
