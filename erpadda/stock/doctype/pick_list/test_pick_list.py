...
from erpadda.stock.doctype.pick_list.pick_list import create_delivery_note
...
class TestPickList(VMRaidTestCase):
	...
	def test_pick_list_shows_serial_no_for_serialized_item(self):
		...
		stock_reconciliation = vmraid.get_doc(
			{
				"doctype": "Stock Reconciliation",
	...
	def test_pick_list_shows_batch_no_for_batched_item(self):
		...
		item = vmraid.db.exists("Item", {"item_name": "Batched Item"})
		if not item:
			...
			item = vmraid.get_doc("Item", {"item_name": "Batched Item"})
	...
	def test_pick_list_for_batched_and_serialised_item(self):
		...
		item = vmraid.db.exists("Item", {"item_name": "Batched and Serialised Item"})
		if not item:
			...
			item = vmraid.get_doc("Item", {"item_name": "Batched and Serialised Item"})
				...
				"items": [
					{
						"item_code": item_code,
						"qty": 10,
						"delivery_date": vmraid.utils.today(),
						"warehouse": "_Test Warehouse - _TC",
					}
				],
			}
	...
	def test_multiple_dn_creation(self):
		...
		for dn in vmraid.get_all(
			"Delivery Note",
			filters={"pick_list": pick_list.name, "customer": "_Test Customer"},
			fields={"name"},
		):
			for dn_item in vmraid.get_doc("Delivery Note", dn.name).get("items"):
