...
from vmraid.model.mapper import map_child_doc
from vmraid.utils import cint, floor, flt, today
...
class PickList(Document):
	...
	def before_save(self):
		...
		for location in self.get("locations"):
			if (
				...
				and vmraid.db.get_value("Sales Order", location.sales_order, "per_picked") == 100
			):
				vmraid.throw(
	...
	def before_submit(self):
		for item in self.locations:
			...
			if not item.serial_no:
				vmraid.throw(
					_("Row #{0}: {1} does not have any available serial numbers in {2}").format(
						vmraid.bold(item.idx), vmraid.bold(item.item_code), vmraid.bold(item.warehouse)
					),
			...
			vmraid.throw(
				...
				).format(vmraid.bold(item.item_code), vmraid.bold(item.idx)),
	...
	def update_so(self, so_item, picked_qty, item_code):
		so_doc = vmraid.get_doc(
			"Sales Order", vmraid.db.get_value("Sales Order Item", so_item, "parent")
		)
		already_picked, actual_qty = vmraid.db.get_value(
			"Sales Order Item", so_item, ["picked_qty", "qty"]
		)

		if self.docstatus == 1:
			if (((already_picked + picked_qty) / actual_qty) * 100) > (
				100 + flt(vmraid.db.get_single_value("Stock Settings", "over_delivery_receipt_allowance"))
			):
				vmraid.throw(
					_(
		...
		vmraid.db.set_value("Sales Order Item", so_item, "picked_qty", already_picked + picked_qty)
	...
	def set_item_locations(self, save=False):
		...
		self.item_location_map = vmraid._dict()

		from_warehouses = None
		if self.parent_warehouse:
			from_warehouses = vmraid.db.get_descendants("Warehouse", self.parent_warehouse)
	...
	def aggregate_item_qty(self):
		...
		for item in locations:
			if not item.item_code:
				vmraid.throw("Row #{0}: Item Code is Mandatory".format(item.idx))
	...
	def validate_for_qty(self):
		if self.purpose == "Material Transfer for Manufacture" and (
			...
			vmraid.throw(_("Qty of Finished Goods Item should be greater than 0."))
...
def validate_item_locations(pick_list):
	if not pick_list.locations:
		vmraid.throw(_("Add items in the Item Locations table"))
...
def get_items_with_location_and_quantity(item_doc, item_location_map, docstatus):
	...
	while remaining_stock_qty > 0 and available_locations:
		...
		item_location = vmraid._dict(item_location)
		...
		uom_must_be_whole_number = vmraid.db.get_value("UOM", item_doc.uom, "must_be_whole_number")
...
def get_available_item_locations(
	...
	locations = []
	has_serial_no = vmraid.get_cached_value("Item", item_code, "has_serial_no")
	has_batch_no = vmraid.get_cached_value("Item", item_code, "has_batch_no")
	...
	if remaining_qty > 0 and not ignore_validation:
		vmraid.msgprint(
			_("{0} units of Item {1} is not available.").format(
				remaining_qty, vmraid.get_desk_link("Item", item_code)
			),
...
def get_available_item_locations_for_serialized_item(
	...
	filters = vmraid._dict({"item_code": item_code, "company": company, "warehouse": ["!=", ""]})
	...
	warehouse_serial_nos_map = vmraid._dict()
...
def get_available_item_locations_for_other_item(item_code, from_warehouses, required_qty, company):
	...
	warehouses = [x.get("name") for x in vmraid.get_list("Warehouse", {"company": company}, "name")]
...
def create_delivery_note(source_name, target_doc=None):
	pick_list = vmraid.get_doc("Pick List", source_name)
	...
	for location in pick_list.locations:
		if location.sales_order:
			sales_orders.append(
				[vmraid.db.get_value("Sales Order", location.sales_order, "customer"), location.sales_order]
			)
	...
	vmraid.msgprint(_("Delivery Note(s) created for the Pick List"))
	return delivery_note


def create_dn_wo_so(pick_list):
	delivery_note = vmraid.new_doc("Delivery Note")
...
def map_pl_locations(pick_list, item_mapper, delivery_note, sales_order=None):
	...
	for location in pick_list.locations:
		if location.sales_order == sales_order:
			if location.sales_order_item:
				sales_order_item = vmraid.get_cached_doc(
	...
	delivery_note.customer = vmraid.get_value("Sales Order", sales_order, "customer")


@vmraid.whitelist()
def create_stock_entry(pick_list):
	pick_list = vmraid.get_doc(json.loads(pick_list))
	validate_item_locations(pick_list)

	if stock_entry_exists(pick_list.get("name")):
		return vmraid.msgprint(_("Stock Entry has been already created against this Pick List"))
...
def get_pending_work_orders(doctype, txt, searchfield, start, page_length, filters, as_dict):
	return vmraid.db.sql(
			...
			%(start)s, %(page_length)s""",
		{
			...
			"_txt": txt.replace("%", ""),
			"start": start,
			"page_length": vmraid.utils.cint(page_length),
...
def target_document_exists(pick_list_name, purpose):
	if purpose == "Delivery":
		return vmraid.db.exists("Delivery Note", {"pick_list": pick_list_name})
...
def get_item_details(item_code, uom=None):
	details = vmraid.db.get_value("Item", item_code, ["stock_uom", "name"], as_dict=1)
...
def update_delivery_note_item(source, target, delivery_note):
	cost_center = vmraid.db.get_value("Project", delivery_note.project, "cost_center")
	if not cost_center:
...
def stock_entry_exists(pick_list_name):
	return vmraid.db.exists("Stock Entry", {"pick_list": pick_list_name})


def update_stock_entry_based_on_work_order(pick_list, stock_entry):
	work_order = vmraid.get_doc("Work Order", pick_list.get("work_order"))
	...
	is_wip_warehouse_group = vmraid.db.get_value("Warehouse", work_order.wip_warehouse, "is_group")
...
def update_stock_entry_based_on_material_request(pick_list, stock_entry):
	for location in pick_list.locations:
		...
		if location.material_request_item:
			target_warehouse = vmraid.get_value(
