...
from vmraid.utils import add_to_date, flt, now
...
from erpadda.accounts.utils import update_gl_entries_after
from erpadda.assets.doctype.asset.test_asset import create_asset_category, create_fixed_asset_item
...
class TestLandedCostVoucher(VMRaidTestCase):
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi_lc_value = vmraid.db.get_value(
	...
	def test_landed_cost_voucher_for_serialized_item(self):
		...
		serial_no_rate = vmraid.db.get_value("Serial No", "SN001", "purchase_rate")

		create_landed_cost_voucher("Purchase Receipt", pr.name, pr.company)

		serial_no = vmraid.db.get_value("Serial No", "SN001", ["warehouse", "purchase_rate"], as_dict=1)
	...
	def test_serialized_lcv_delivered(self):
		...
		serial_no_rate = vmraid.db.get_value("Serial No", serial_no, "purchase_rate")
	...
	def test_asset_lcv(self):
		...
		self.assertEqual(vmraid.db.get_value("Asset", assets[0].name, "gross_purchase_amount"), 50080)
...
def make_landed_cost_voucher(**args):
	...
	ref_doc = vmraid.get_doc(args.receipt_document_type, args.receipt_document)

	lcv = vmraid.new_doc("Landed Cost Voucher")
...
def create_landed_cost_voucher(receipt_document_type, receipt_document, company, charges=50):
	ref_doc = vmraid.get_doc(receipt_document_type, receipt_document)
...
test_records = vmraid.get_test_records("Landed Cost Voucher")
