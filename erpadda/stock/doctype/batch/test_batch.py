...
from vmraid.utils.data import add_to_date, getdate

from erpadda.accounts.doctype.purchase_invoice.test_purchase_invoice import make_purchase_invoice
from erpadda.stock.doctype.batch.batch import UnableToSelectBatchError, get_batch_no, get_batch_qty
...
from erpadda.stock.stock_ledger import get_valuation_rate


class TestBatch(VMRaidTestCase):
	def test_item_has_batch_enabled(self):
		self.assertRaises(
			ValidationError,
			vmraid.get_doc({"doctype": "Batch", "name": "_test Batch", "item": "_Test Item"}).save,
		)
	...
	def test_batch_split(self):
		...
		from erpadda.stock.doctype.batch.batch import split_batch
	...
	def test_total_batch_qty(self):
		...
		existing_batch_qty = flt(vmraid.db.get_value("Batch", "B100", "batch_qty"))
		stock_entry = self.make_new_batch_and_entry("ITEM-BATCH-3", "B100", "_Test Warehouse - _TC")

		current_batch_qty = flt(vmraid.db.get_value("Batch", "B100", "batch_qty"))
	...
	def make_new_batch_and_entry(cls, item_name, batch_name, warehouse):
		...
		if not vmraid.db.exists("Batch", batch_name):
			batch = vmraid.get_doc(dict(doctype="Batch", item=item_name, batch_id=batch_name)).insert(
	...
	def test_batch_name_with_naming_series(self):
		stock_settings = vmraid.get_single("Stock Settings")
		use_naming_series = cint(stock_settings.use_naming_series)

		if not use_naming_series:
			vmraid.set_value("Stock Settings", "Stock Settings", "use_naming_series", 1)
		...
		if not use_naming_series:
			vmraid.set_value("Stock Settings", "Stock Settings", "use_naming_series", 0)

	def make_new_batch(self, item_name, batch_id=None, do_not_insert=0):
		batch = vmraid.new_doc("Batch")
	...
	def test_batch_wise_item_price(self):
		if not vmraid.db.get_value("Item", "_Test Batch Price Item"):
			vmraid.get_doc(
				{
					"doctype": "Item",
					"is_stock_item": 1,
	...
	def test_basic_batch_wise_valuation(self, batch_qty=100):
		...
		for batch, qty in consumption_plan:
			...
			sle = vmraid.get_last_doc("Stock Ledger Entry", {"is_cancelled": 0, "voucher_no": se.name})
	...
	def test_update_batch_properties(self):
		...
		batch_no = se.items[0].batch_no
		batch = vmraid.get_doc("Batch", batch_no)
...
def create_batch(item_code, rate, create_item_price_for_batch):
	...
	batch = vmraid.db.get_value("Batch", {"item": item_code, "reference_name": pi.name})
...
def make_new_batch(**args):
	...
	if vmraid.db.exists("Batch", args.batch_id):
		batch = vmraid.get_doc("Batch", args.batch_id)
	else:
