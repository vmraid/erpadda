...
vmraid.ui.form.on('Batch', {
	...
	item: (frm) => {
		// vmraid.db.get_value('Item', {name: frm.doc.item}, 'has_expiry_date', (r) => {
		// 	frm.toggle_reqd('expiry_date', r.has_expiry_date);
		// });
		vmraid.db.get_value('Item', {name: frm.doc.item}, ['shelf_life_in_days', 'has_expiry_date'], (r) => {
			if (r.has_expiry_date && r.shelf_life_in_days) {
				// Calculate expiry date based on shelf_life_in_days
				frm.set_value('expiry_date', vmraid.datetime.add_days(frm.doc.manufacturing_date, r.shelf_life_in_days));
				...
				callback: (r) => {
					...
					rows.find('.btn-move').on('click', function() {
						...
						vmraid.prompt(
							...
							(data) => {
								vmraid.call({
									method: 'erpadda.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry',
									args: {
									...
									callback: (r) => {
										vmraid.show_alert(__('Stock Entry {0} created',
					...
					rows.find('.btn-split').on('click', function() {
						...
						(data) => {
							vmraid.call({
								method: 'erpadda.stock.doctype.batch.batch.split_batch',
								args: {
									item_code: frm.doc.item,
									batch_no: frm.doc.name,
									qty: data.qty,
...
vmraid.ui.form.on('Batch', 'manufacturing_date', function (frm){
	vmraid.db.get_value('Item', {name: frm.doc.item}, ['shelf_life_in_days', 'has_expiry_date'], (r) => {
		if (r.has_expiry_date && r.shelf_life_in_days) {
			// Calculate expiry date based on shelf_life_in_days
			frm.set_value('expiry_date', vmraid.datetime.add_days(frm.doc.manufacturing_date, r.shelf_life_in_days));
