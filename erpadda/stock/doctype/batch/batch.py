...
from vmraid.model.naming import make_autoname, revert_series_if_last
from vmraid.utils import cint, flt, get_link_to_form
from vmraid.utils.data import add_days
from vmraid.utils.jinja import render_template


class UnableToSelectBatchError(vmraid.ValidationError):
	pass
...
def get_name_from_hash():
	...
	temp = None
	while not temp:
		temp = vmraid.generate_hash()[:7].upper()
		if vmraid.db.exists("Batch", temp):
			temp = None
...
def batch_uses_naming_series():
	...
	:return: bool
	"""
	use_naming_series = cint(vmraid.db.get_single_value("Stock Settings", "use_naming_series"))
...
def _get_batch_prefix():
	...
	"""
	naming_series_prefix = vmraid.db.get_single_value("Stock Settings", "naming_series_prefix")
...
class Batch(Document):
	def autoname(self):
		...
		if not self.batch_id:
			create_new_batch, batch_number_series = vmraid.db.get_value(
			...
			if create_new_batch:
				...
				vmraid.throw(_("Batch ID is mandatory"), vmraid.MandatoryError)

		self.name = self.batch_id

	def onload(self):
		self.image = vmraid.db.get_value("Item", self.item, "image")
	...
	def item_has_batch_enabled(self):
		if vmraid.db.get_value("Item", self.item, "has_batch_no") == 0:
			vmraid.throw(_("The selected item cannot have Batch"))

	def set_batchwise_valuation(self):
		from erpadda.stock.stock_ledger import get_valuation_method

		if self.is_new() and get_valuation_method(self.item) != "Moving Average":
			self.use_batchwise_valuation = 1

	def before_save(self):
		has_expiry_date, shelf_life_in_days = vmraid.db.get_value(
		...
		if has_expiry_date and not self.expiry_date:
			vmraid.throw(
				msg=_("Please set {0} for Batched Item {1}, which is used to set {2} on Submit.").format(
					vmraid.bold("Shelf Life in Days"),
					get_link_to_form("Item", self.item),
					vmraid.bold("Batch Expiry Date"),
				),
...
def get_batches_by_oldest(item_code, warehouse):
	...
	batches_dates = [
		[batch, vmraid.get_value("Batch", batch.batch_no, "expiry_date")] for batch in batches
	]
...
def split_batch(batch_no, item_code, warehouse, qty, new_batch_id=None):
	...
	batch = vmraid.get_doc(dict(doctype="Batch", item=item_code, batch_id=new_batch_id)).insert()

	company = vmraid.db.get_value(
		"Stock Ledger Entry",
		dict(item_code=item_code, batch_no=batch_no, warehouse=warehouse),
		["company"],
	)

	stock_entry = vmraid.get_doc(
		dict(
			doctype="Stock Entry",
			purpose="Repack",
			company=company,
...
def set_batch_nos(doc, warehouse_field, throw=False, child_table="items"):
	...
	for d in doc.get(child_table):
		...
		if warehouse and qty > 0 and vmraid.db.get_value("Item", d.item_code, "has_batch_no"):
			if not d.batch_no:
...
def get_batches(item_code, warehouse, qty=1, throw=False, serial_no=None):
	...
	cond = ""
	if serial_no and vmraid.get_cached_value("Item", item_code, "has_batch_no"):
		serial_nos = get_serial_nos(serial_no)
		batch = vmraid.get_all(
			"Serial No",
		...
		cond = " and `tabBatch`.name = %s" % (vmraid.db.escape(batch[0].batch_no))
...
def validate_serial_no_with_batch(serial_nos, item_code):
	if vmraid.get_cached_value("Serial No", serial_nos[0], "item_code") != item_code:
		vmraid.throw(
	...
	vmraid.throw(_("There is no batch found against the {0}: {1}").format(message, serial_no_link))


def make_batch(args):
	if vmraid.db.get_value("Item", args.item, "has_batch_no"):
		args.doctype = "Batch"
		vmraid.get_doc(args).insert().name
...
def get_pos_reserved_batch_qty(filters):
	...
	p = vmraid.qb.DocType("POS Invoice").as_("p")
	item = vmraid.qb.DocType("POS Invoice Item").as_("item")
	sum_qty = vmraid.query_builder.functions.Sum(item.qty).as_("qty")
