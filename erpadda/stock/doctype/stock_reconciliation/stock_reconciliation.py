...
class OpeningEntryAccountError(vmraid.ValidationError):
	pass


class EmptyStockReconciliationItemsError(vmraid.ValidationError):
	pass
...
class StockReconciliation(StockController):
	...
	def validate(self):
		if not self.expense_account:
			self.expense_account = vmraid.get_cached_value(
				"Company", self.company, "stock_adjustment_account"
			)
		if not self.cost_center:
			self.cost_center = vmraid.get_cached_value("Company", self.company, "cost_center")
	...
	def remove_items_with_no_change(self):
		...
		if not items:
			...
			for i, item in enumerate(self.items):
				item.idx = i + 1
			vmraid.msgprint(_("Removed items with no change in quantity or value."))
	...
	def validate_data(self):
		...
		for row_num, row in enumerate(self.items):
			...
			# validate warehouse
			if not vmraid.db.get_value("Warehouse", row.warehouse):
			...
			if row.qty and row.valuation_rate in ["", None]:
				...
				if not row.valuation_rate:
					...
					buying_rate = vmraid.db.get_value(
						"Item Price",
					...
					if buying_rate:
						...
						row.valuation_rate = vmraid.get_value("Item", row.item_code, "valuation_rate")
		...
		if self.validation_messages:
			...
			raise vmraid.ValidationError(self.validation_messages)

	def validate_item(self, item_code, row):
		from erpadda.stock.doctype.item.item import (
		...
		try:
			...
			if item.has_batch_no and not row.batch_no and not item.create_new_batch:
				raise vmraid.ValidationError(_("Batch no is required for batched item {0}").format(item_code))
	...
	def update_stock_ledger(self):
		...
		for row in self.items:
			item = vmraid.get_doc("Item", row.item_code)
			...
			if item.has_serial_no or item.has_batch_no:
				...
				if row.serial_no or row.batch_no:
					vmraid.throw(
						...
						).format(row.idx, vmraid.bold(row.item_code))
					)
	...
	def update_valuation_rate_for_serial_nos(self, row, serial_nos):
		...
		for d in serial_nos:
			vmraid.db.set_value("Serial No", d, "purchase_rate", valuation_rate)
		...
		data = vmraid._dict(
			{
				...
				"stock_uom": vmraid.db.get_value("Item", row.item_code, "stock_uom"),
	...
	def make_sle_on_cancel(self):
		...
		if sl_entries:
			...
			allow_negative_stock = cint(
				vmraid.db.get_single_value("Stock Settings", "allow_negative_stock")
			)
			self.make_sl_entries(sl_entries, allow_negative_stock=allow_negative_stock)
	...
	def validate_expense_account(self):
		...
		if not self.expense_account:
			vmraid.throw(_("Please enter Expense Account"))
		elif self.purpose == "Opening Stock" or not vmraid.db.sql(
			"""select name from `tabStock Ledger Entry` limit 1"""
		):
			if vmraid.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
				vmraid.throw(
					_(
	...
	def set_zero_value_for_customer_provided_items(self):
		...
		for d in self.get("items"):
			is_customer_item = vmraid.db.get_value("Item", d.item_code, "is_customer_provided_item")
...
def get_items(
	...
	items = [vmraid._dict({"item_code": item_code, "warehouse": warehouse})]
...
def get_itemwise_batch(warehouse, posting_date, company, item_code=None):
	from erpadda.stock.report.batch_wise_balance_history.batch_wise_balance_history import execute
...
def get_stock_balance_for(
	...
	vmraid.has_permission("Stock Reconciliation", "write", throw=True)

	item_dict = vmraid.get_cached_value(
	...
	if not item_dict:
		...
		vmraid.throw(msg, title=_("Missing"))
