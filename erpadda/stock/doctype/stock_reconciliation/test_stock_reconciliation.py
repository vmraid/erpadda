...
from vmraid.utils import add_days, cstr, flt, nowdate, nowtime, random_string

from erpadda.accounts.utils import get_stock_and_account_balance
...
from erpadda.stock.stock_ledger import get_previous_sle, update_entries_after
from erpadda.stock.utils import get_incoming_rate, get_stock_value_on, get_valuation_method


class TestStockReconciliation(VMRaidTestCase):
	@classmethod
	def setUpClass(cls):
	...
	def tearDown(self):
		vmraid.flags.dont_execute_stock_reposts = None
	...
	def test_stock_reco_for_serialized_item(self):
		...
		for d in to_delete_records:
			stock_doc = vmraid.get_doc("Stock Reconciliation", d)
			stock_doc.cancel()
	...
	def test_backdated_stock_reco_future_negative_stock(self):
		...
		dn2_balance = vmraid.db.get_value(
	...
	def test_backdated_stock_reco_cancellation_future_negative_stock(self):
		...
		dn_balance = vmraid.db.get_value(
		...
		repost_exists = bool(vmraid.db.exists("Repost Item Valuation", {"voucher_no": sr.name}))
	...
	def test_intermediate_sr_bin_update(self):
		...
		vmraid.flags.dont_execute_stock_reposts = True
		vmraid.db.rollback()
		...
		old_bin_qty = vmraid.db.get_value(
		...
		new_bin_qty = vmraid.db.get_value(
			"Bin", {"item_code": item_code, "warehouse": warehouse}, "actual_qty"
		)
	...
	def test_valid_batch(self):
		...
		self.assertRaises(vmraid.ValidationError, sr.submit)
...
def create_batch_item_with_batch(item_name, batch_id):
	...
	if not vmraid.db.exists("Batch", batch_id):
		b = vmraid.new_doc("Batch")
		b.item = item_name
		b.batch_id = batch_id
		b.save()
...
def create_stock_reconciliation(**args):
	...
	sr.expense_account = args.expense_account or (
		"Stock Adjustment - _TC" if vmraid.get_all("Stock Ledger Entry") else "Temporary Opening - _TC"
	)
	sr.cost_center = (
		args.cost_center
		or vmraid.get_cached_value("Company", sr.company, "cost_center")
...
def set_valuation_method(item_code, valuation_method):
	...
	vmraid.db.set_value("Item", item_code, "valuation_method", valuation_method)

	for warehouse in vmraid.get_all(
