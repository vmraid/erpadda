...
from erpadda.stock.doctype.quality_inspection_template.quality_inspection_template import (
...
class QualityInspection(Document):
	def validate(self):
		...
		if self.inspection_type == "In Process" and self.reference_type == "Job Card":
			item_qi_template = vmraid.db.get_value("Item", self.item_code, "quality_inspection_template")
	...
	def get_item_specification_details(self):
		if not self.quality_inspection_template:
			self.quality_inspection_template = vmraid.db.get_value(
	...
	def get_quality_inspection_template(self):
		template = ""
		if self.bom_no:
			template = vmraid.db.get_value("BOM", self.bom_no, "quality_inspection_template")

		if not template:
			template = vmraid.db.get_value("BOM", self.item_code, "quality_inspection_template")
	...
	def set_status_based_on_acceptance_formula(self, reading):
		...
		try:
			result = vmraid.safe_eval(condition, None, data)
			reading.status = "Accepted" if result else "Rejected"
		except NameError as e:
			field = vmraid.bold(e.args[0].split()[1])
			vmraid.throw(
...
def make_quality_inspection(source_name, target_doc=None):
	def postprocess(source, doc):
		doc.inspected_by = vmraid.session.user
