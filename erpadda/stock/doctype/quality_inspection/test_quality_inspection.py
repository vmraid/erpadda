...
from erpadda.controllers.stock_controller import (
...
# test_records = vmraid.get_test_records('Quality Inspection')


class TestQualityInspection(VMRaidTestCase):
	def setUp(self):
		super().setUp()
		create_item("_Test Item with QA")
		vmraid.db.set_value("Item", "_Test Item with QA", "inspection_required_before_delivery", 1)
	...
	def test_qa_for_delivery(self):
		...
		vmraid.db.set_value("Quality Inspection", qa.name, "status", "Accepted")
		dn.reload()
		dn.submit()
	...
	def test_make_quality_inspections_from_linked_document(self):
		...
		for qi in quality_inspections:
			vmraid.delete_doc("Quality Inspection", qi)
		dn.delete()
	...
	def test_rejected_qi_validation(self):
		...
		vmraid.db.set_value("Stock Settings", None, "action_if_quality_inspection_is_rejected", "Stop")
		se.reload()
		self.assertRaises(
			QualityInspectionRejectedError, se.submit
		)  # when blocked in Stock settings, block rejected QI

		vmraid.db.set_value("Stock Settings", None, "action_if_quality_inspection_is_rejected", "Warn")
		se.reload()
...
def create_quality_inspection(**args):
	...
	qa = vmraid.new_doc("Quality Inspection")
	...
	qa.sample_size = 1
	qa.inspected_by = vmraid.session.user
