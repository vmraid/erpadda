...
class ItemPriceDuplicateItem(vmraid.ValidationError):
	pass
...
class ItemPrice(Document):
	...
	def validate_item(self):
		if not vmraid.db.exists("Item", self.item_code):
			vmraid.throw(_("Item {0} not found.").format(self.item_code))

	def validate_dates(self):
		if self.valid_from and self.valid_upto:
			if getdate(self.valid_from) > getdate(self.valid_upto):
				vmraid.throw(_("Valid From Date must be lesser than Valid Upto Date."))

	def update_price_list_details(self):
		if self.price_list:
			price_list_details = vmraid.db.get_value(
				"Price List", {"name": self.price_list, "enabled": 1}, ["buying", "selling", "currency"]
			)

			if not price_list_details:
				link = vmraid.utils.get_link_to_form("Price List", self.price_list)
				vmraid.throw("The price list {0} does not exist or is disabled".format(link))

			self.buying, self.selling, self.currency = price_list_details

	def update_item_details(self):
		if self.item_code:
			self.item_name, self.item_description = vmraid.db.get_value(
