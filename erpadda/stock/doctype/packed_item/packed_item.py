...
from erpadda.stock.get_item_details import get_item_details, get_price_list_rate
...
def make_packing_list(doc):
	...
	set_price_from_children = vmraid.db.get_single_value(
	...
	for item_row in doc.get("items"):
		if vmraid.db.exists("Product Bundle", {"new_item_code": item_row.item_code}):
...
def get_product_bundle_items(item_code):
	product_bundle = vmraid.qb.DocType("Product Bundle")
	product_bundle_item = vmraid.qb.DocType("Product Bundle Item")

	query = (
		vmraid.qb.from_(product_bundle_item)
		.join(product_bundle)
...
def get_packed_item_details(item_code, company):
	...
	item_default = vmraid.qb.DocType("Item Default")
	query = (
		vmraid.qb.from_(item)
...
def update_packed_item_price_data(pi_row, item_data, doc):
	...
	item_doc = vmraid.get_cached_doc("Item", pi_row.item_code)
...
def on_doctype_update():
	vmraid.db.add_index("Packed Item", ["item_code", "warehouse"])
