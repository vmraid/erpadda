...
from vmraid.utils import cint, flt, getdate, nowdate
...
from erpadda.stock.doctype.delivery_note.delivery_note import make_inter_company_transaction
...
class PurchaseReceipt(BuyingController):
		...
		if vmraid.db.get_value("Buying Settings", None, "po_required") == "Yes":
			for d in self.get("items"):
				if not d.purchase_order:
					vmraid.throw(_("Purchase Order number required for Item {0}").format(d.item_code))
	...
	def get_po_qty_and_warehouse(self, po_detail):
		po_qty, po_warehouse = vmraid.db.get_value(
	...
	def check_next_docstatus(self):
		...
		if submit_rv:
			vmraid.throw(_("Purchase Invoice {0} is already submitted").format(self.submit_rv[0][0]))
	...
	def on_cancel(self):
		...
		if submitted:
			vmraid.throw(_("Purchase Invoice {0} is already submitted").format(submitted[0][0]))
	...
	def make_item_gl_entries(self, gl_entries, warehouse_account=None):
		...
		if erpadda.is_perpetual_inventory_enabled(self.company):
		...
		for d in self.get("items"):
			if d.item_code in stock_items and flt(d.valuation_rate) and flt(d.qty):
				if warehouse_account.get(d.warehouse):
					stock_value_diff = vmraid.db.get_value(
						"Stock Ledger Entry",
						{
					...
					if divisional_loss:
						...
						cost_center = d.cost_center or vmraid.get_cached_value(
	...
	def add_provisional_gl_entry(
		...
		if reverse:
			multiplication_factor = -1
			expense_account = vmraid.db.get_value(
	...
	def make_tax_gl_entries(self, gl_entries):
		...
		if negative_expense_to_be_booked and valuation_tax:
			...
			negative_expense_booked_in_pi = vmraid.db.sql(
	...
	def update_assets(self, item, valuation_rate):
		...
		for asset in assets:
			vmraid.db.set_value("Asset", asset.name, "gross_purchase_amount", flt(valuation_rate))
			vmraid.db.set_value("Asset", asset.name, "purchase_receipt_amount", flt(valuation_rate))
	...
	def update_billing_status(self, update_modified=True):
		...
		for pr in set(updated_pr):
			pr_doc = self if (pr == self.name) else vmraid.get_doc("Purchase Receipt", pr)
...
def update_billed_amount_based_on_po(po_detail, update_modified=True):
	...
	billed_against_po = vmraid.db.sql(
	...
	for pr_item in pr_details:
		...
		billed_amt_agianst_pr = vmraid.db.sql(
			"""select sum(amount) from `tabPurchase Invoice Item`
...
def update_billing_percentage(pr_doc, update_modified=True):
	...
	for item in pr_doc.items:
		return_data = vmraid.db.get_list(
			"Purchase Receipt",
...
def make_purchase_invoice(source_name, target_doc=None):
	from erpadda.accounts.party import get_payment_terms_template

	doc = vmraid.get_doc("Purchase Receipt", source_name)
	returned_qty_map = get_returned_qty_map(source_name)
	invoiced_qty_map = get_invoiced_qty_map(source_name)

	def set_missing_values(source, target):
		if len(target.get("items")) == 0:
			vmraid.throw(_("All items have already been Invoiced/Returned"))

		doc = vmraid.get_doc(target)
...
def get_invoiced_qty_map(purchase_receipt):
	...
	for pr_detail, qty in vmraid.db.sql(
...
def update_purchase_receipt_status(docname, status):
	pr = vmraid.get_doc("Purchase Receipt", docname)
...
def get_item_account_wise_additional_cost(purchase_document):
	landed_cost_vouchers = vmraid.get_all(
	...
	for lcv in landed_cost_vouchers:
		landed_cost_voucher_doc = vmraid.get_doc("Landed Cost Voucher", lcv.parent)

		# Use amount field for total item cost for manually cost distributed LCVs
		if landed_cost_voucher_doc.distribute_charges_based_on == "Distribute Manually":
			based_on_field = "amount"
		else:
			based_on_field = vmraid.scrub(landed_cost_voucher_doc.distribute_charges_based_on)
...
def on_doctype_update():
	vmraid.db.add_index("Purchase Receipt", ["supplier", "is_return", "return_against"])
