...
from vmraid.utils import add_days, cint, cstr, flt, today

import erpadda
from erpadda.accounts.doctype.account.test_account import get_inventory_account
from erpadda.controllers.buying_controller import QtyMismatchError
from erpadda.stock.doctype.item.test_item import create_item, make_item
from erpadda.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_invoice
from erpadda.stock.doctype.serial_no.serial_no import SerialNoDuplicateError, get_serial_nos
from erpadda.stock.doctype.warehouse.test_warehouse import create_warehouse
from erpadda.stock.stock_ledger import SerialNoExistsInFutureTransaction


class TestPurchaseReceipt(VMRaidTestCase):
	def setUp(self):
		vmraid.db.set_value("Buying Settings", None, "allow_multiple_items", 1)
	...
	def test_reverse_purchase_receipt_sle(self):
		...
		sl_entry_cancelled = vmraid.db.get_all(
			"Stock Ledger Entry",
			{"voucher_type": "Purchase Receipt", "voucher_no": pr.name},
			["actual_qty"],
			order_by="creation",
	...
	def test_make_purchase_invoice(self):
		...
		template = vmraid.db.get_value(
			"Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice"
		)
		old_template_in_supplier = vmraid.db.get_value("Supplier", "_Test Supplier", "payment_terms")
		vmraid.db.set_value("Supplier", "_Test Supplier", "payment_terms", template)

		pr = make_purchase_receipt(do_not_save=True)
		self.assertRaises(vmraid.ValidationError, make_purchase_invoice, pr.name)
		pr.submit()
		...
		self.assertRaises(vmraid.ValidationError, vmraid.get_doc(pi).submit)
		...
		pi.delete()  # draft PI
		pr.cancel()
		vmraid.db.set_value("Supplier", "_Test Supplier", "payment_terms", old_template_in_supplier)
		vmraid.get_doc(
			"Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice"
		).delete()
	...
	def test_purchase_receipt_no_gl_entry(self):
		...
		existing_bin_qty, existing_bin_stock_value = vmraid.db.get_value(
			"Bin",
		...
		stock_value_difference = vmraid.db.get_value(
			"Stock Ledger Entry",
			{
		...
		current_bin_stock_value = vmraid.db.get_value(
	...
	def test_batched_serial_no_purchase(self):
		item = vmraid.db.exists("Item", {"item_name": "Batched Serialized Item"})
		if not item:
			...
			item = vmraid.get_doc("Item", {"item_name": "Batched Serialized Item"})

		pr = make_purchase_receipt(item_code=item.name, qty=5, rate=500)

		self.assertTrue(vmraid.db.get_value("Batch", {"item": item.name, "reference_name": pr.name}))

		pr.load_from_db()
		batch_no = pr.items[0].batch_no
		pr.cancel()

		self.assertFalse(vmraid.db.get_value("Batch", {"item": item.name, "reference_name": pr.name}))
		self.assertFalse(vmraid.db.get_all("Serial No", {"batch_no": batch_no}))

	def test_duplicate_serial_nos(self):
		from erpadda.stock.doctype.delivery_note.test_delivery_note import create_delivery_note

		item = vmraid.db.exists("Item", {"item_name": "Test Serialized Item 123"})
		if not item:
			...
			item = vmraid.get_doc("Item", {"item_name": "Test Serialized Item 123"})

		# First make purchase receipt
		pr = make_purchase_receipt(item_code=item.name, qty=2, rate=500)
		pr.load_from_db()

		serial_nos = vmraid.db.get_value(
			"Stock Ledger Entry",
	...
	def test_purchase_receipt_gl_entry(self):
		...
		self.assertEqual(cint(erpadda.is_perpetual_inventory_enabled(pr.company)), 1)
	...
	def test_subcontracting_over_receipt(self):
		...
		"""
		from erpadda.buying.doctype.purchase_order.purchase_order import make_purchase_receipt
		from erpadda.buying.doctype.purchase_order.purchase_order import (
			make_rm_stock_entry as make_subcontract_transfer_entry,
		)
		from erpadda.buying.doctype.purchase_order.test_purchase_order import (
		...
		se = vmraid.get_doc(make_subcontract_transfer_entry(po.name, rm_item_string))
		...
		pr1.submit()
		self.assertRaises(vmraid.ValidationError, pr2.submit)
		vmraid.db.rollback()
	...
	def test_serial_no_supplier(self):
		...
		self.assertEqual(vmraid.db.get_value("Serial No", pr_row_1_serial_no, "supplier"), pr.supplier)

		pr.cancel()
		self.assertFalse(vmraid.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"))

	def test_rejected_serial_no(self):
		pr = vmraid.copy_doc(test_records[0])
		...
		for serial_no in accepted_serial_nos:
			self.assertEqual(
				vmraid.db.get_value("Serial No", serial_no, "warehouse"), pr.get("items")[0].warehouse
			)
		...
		for serial_no in rejected_serial_nos:
			self.assertEqual(
				vmraid.db.get_value("Serial No", serial_no, "warehouse"), pr.get("items")[0].rejected_warehouse
			)
	...
	def test_purchase_return_partial(self):
		...
		# check sle
		outgoing_rate = vmraid.db.get_value(
			"Stock Ledger Entry",
		...
		returned = vmraid.get_doc("Purchase Receipt", return_pr.name)
	...
	def test_purchase_return_for_rejected_qty(self):
		from erpadda.stock.doctype.warehouse.test_warehouse import get_warehouse

		rejected_warehouse = "_Test Rejected Warehouse - TCP1"
		if not vmraid.db.exists("Warehouse", rejected_warehouse):
			get_warehouse(
				company="_Test Company with perpetual inventory",
				abbr=" - TCP1",
		...
		actual_qty = vmraid.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
	...
	def test_purchase_return_for_serialized_items(self):
		def _check_serial_no_values(serial_no, field_values):
			serial_no = vmraid.get_doc("Serial No", serial_no)
	...
	def test_closed_purchase_receipt(self):
		...
		self.assertEqual(vmraid.db.get_value("Purchase Receipt", pr.name, "status"), "Closed")
	...
	def test_auto_asset_creation(self):
		...
		if not vmraid.db.exists("Item", asset_item):
			asset_category = vmraid.get_all("Asset Category")
		...
		assets = vmraid.db.get_all("Asset", filters={"purchase_receipt": pr.name})

		self.assertEqual(len(assets), 3)

		location = vmraid.db.get_value("Asset", assets[0].name, "location")
		self.assertEqual(location, "Test Location")

		pr.cancel()

	def test_purchase_return_with_submitted_asset(self):
		from erpadda.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_return

		pr = make_purchase_receipt(item_code="Test Asset Item", qty=1)

		asset = vmraid.get_doc("Asset", {"purchase_receipt": pr.name})
		asset.available_for_use_date = vmraid.utils.nowdate()
		...
		self.assertRaises(vmraid.exceptions.ValidationError, pr_return.submit)
	...
	def test_purchase_receipt_cost_center(self):
		...
		if not vmraid.db.exists("Location", "Test Location"):
			vmraid.get_doc({"doctype": "Location", "location_name": "Test Location"}).insert()
	...
	def test_subcontracted_pr_for_multi_transfer_batches(self):
		...
		rm_item_string = json.dumps(rm_items)
		se = vmraid.get_doc(make_rm_stock_entry(po.name, rm_item_string))
		self.assertEqual(len(se.items), 2)
		se.items[0].batch_no = ste1.items[0].batch_no
		se.items[1].batch_no = ste2.items[0].batch_no
		se.submit()

		supplied_qty = vmraid.db.get_value(
	...
	def test_po_to_pi_and_po_to_pr_worflow_full(self):
		...
		"""
		from erpadda.buying.doctype.purchase_order import purchase_order, test_purchase_order
	...
	def test_purchase_receipt_with_exchange_rate_difference(self):
		from erpadda.accounts.doctype.purchase_invoice.purchase_invoice import (
		...
		exchange_gain_loss_account = vmraid.db.get_value(
			"Company", pr.company, "exchange_gain_loss_account"
		)

		# fetching the latest GL Entry with exchange gain and loss account account
		amount = vmraid.db.get_value(
	...
	def test_payment_terms_are_fetched_when_creating_purchase_invoice(self):
		...
		from erpadda.selling.doctype.sales_order.test_sales_order import (
...
def make_purchase_receipt(**args):
	...
	vmraid.db.set_value("Buying Settings", None, "allow_multiple_items", 1)
	pr = vmraid.new_doc("Purchase Receipt")
	args = vmraid._dict(args)
	...
	uom = args.uom or vmraid.db.get_value("Item", item_code, "stock_uom") or "_Test UOM"
	pr.append(
		"items",
		{
			"item_code": item_code,
			...
			or vmraid.get_cached_value("Company", pr.company, "cost_center"),
	...
	if args.get_multiple_items:
		...
		company_cost_center = vmraid.get_cached_value("Company", pr.company, "cost_center")
...
def create_subcontracted_item(**args):
	...
	if not args.raw_materials:
		if not vmraid.db.exists("Item", "Test Extra Item 1"):
			make_item(
				"Test Extra Item 1",
				{
					"is_stock_item": 1,
		...
		if not vmraid.db.exists("Item", "Test Extra Item 2"):
			make_item(
				"Test Extra Item 2",
				{
					"is_stock_item": 1,
	...
	if not vmraid.db.get_value("BOM", {"item": args.item_code}, "name"):
		make_bom(item=args.item_code, raw_materials=args.get("raw_materials"))


test_dependencies = ["BOM", "Item Price", "Location"]
test_records = vmraid.get_test_records("Purchase Receipt")
