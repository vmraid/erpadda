...
from vmraid.utils import add_days, flt, now_datetime, nowdate

import erpadda
from erpadda.stock.doctype.delivery_trip.delivery_trip import (
...
class TestDeliveryTrip(VMRaidTestCase):
	def setUp(self):
		super().setUp()
	...
	def tearDown(self):
		vmraid.db.sql("delete from `tabDriver`")
		vmraid.db.sql("delete from `tabVehicle`")
		vmraid.db.sql("delete from `tabEmail Template`")
		vmraid.db.sql("delete from `tabDelivery Trip`")
...
def create_address(driver):
	if not vmraid.db.exists("Address", {"address_title": "_Test Address for Driver"}):
		...
		vmraid.db.set_value("Driver", driver.name, "address", address.name)

		return address

	return vmraid.get_doc("Address", {"address_title": "_Test Address for Driver"})


def create_driver():
	if not vmraid.db.exists("Driver", {"full_name": "Newton Scmander"}):
		driver = vmraid.get_doc(
			{
				"doctype": "Driver",
	...
	return vmraid.get_doc("Driver", {"full_name": "Newton Scmander"})


def create_delivery_notification():
	if not vmraid.db.exists("Email Template", "Delivery Notification"):
		dispatch_template = vmraid.get_doc(
			{
	...
	delivery_settings = vmraid.get_single("Delivery Settings")
	delivery_settings.dispatch_template = "Delivery Notification"
	delivery_settings.save()


def create_vehicle():
	if not vmraid.db.exists("Vehicle", "JB 007"):
