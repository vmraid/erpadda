...
from vmraid.utils import flt, nowdate, nowtime

from erpadda.accounts.doctype.account.test_account import get_inventory_account
from erpadda.stock.doctype.item.test_item import (
	create_item,
	make_item_variant,
	set_item_variant_settings,
)
from erpadda.stock.doctype.serial_no.serial_no import *  # noqa
from erpadda.stock.doctype.stock_entry.stock_entry import (
	FinishedGoodError,
	move_sample_to_retention_warehouse,
)
from erpadda.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
from erpadda.stock.doctype.stock_ledger_entry.stock_ledger_entry import StockFreezeError
from erpadda.stock.doctype.stock_reconciliation.stock_reconciliation import (
	OpeningEntryAccountError,
)
from erpadda.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
	create_stock_reconciliation,
)
from erpadda.stock.stock_ledger import NegativeStockError, get_previous_sle
...
class TestStockEntry(VMRaidTestCase):
	def tearDown(self):
		vmraid.db.rollback()
	...
	def test_fifo(self):
		...
		self.assertEqual([[1, 10]], vmraid.safe_eval(sle.stock_queue))
		...
		self.assertEqual([[-2, 10]], vmraid.safe_eval(sle.stock_queue))

		# move stock to positive
		make_stock_entry(item_code=item_code, target=warehouse, qty=3, basic_rate=20)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]
		self.assertEqual([[1, 20]], vmraid.safe_eval(sle.stock_queue))

		# incoming entry with diff rate
		make_stock_entry(item_code=item_code, target=warehouse, qty=1, basic_rate=30)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]

		self.assertEqual([[1, 20], [1, 30]], vmraid.safe_eval(sle.stock_queue))

		vmraid.db.set_default("allow_negative_stock", 0)
	...
	def _test_auto_material_request(
		...
		variant = vmraid.get_doc("Item", item_code)

		projected_qty, actual_qty = vmraid.db.get_value(
		...
		projected_qty = (
			vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "projected_qty")
			or 0
		)

		vmraid.db.set_value("Stock Settings", None, "auto_indent", 1)
		...
		from erpadda.stock.reorder_item import reorder_item

		mr_list = reorder_item()

		vmraid.db.set_value("Stock Settings", None, "auto_indent", 0)
	...
	def test_material_receipt_gl_entry(self):
		company = vmraid.db.get_value("Warehouse", "Stores - TCP1", "company")
	...
	def test_repack_multiple_fg(self):
		...
		repack = vmraid.copy_doc(test_records[3])
	...
	def test_repack_with_additional_costs(self):
		...
		expenses_included_in_valuation = vmraid.get_value(
	...
	def test_serial_no_not_reqd(self):
		se = vmraid.copy_doc(test_records[0])
	...
	def test_serial_no_transfer_in(self):
		...
		self.assertTrue(vmraid.db.exists("Serial No", "ABCD"))
		self.assertTrue(vmraid.db.exists("Serial No", "EFGH"))

		se.cancel()
		self.assertFalse(vmraid.db.get_value("Serial No", "ABCD", "warehouse"))

	def test_serial_no_not_exists(self):
		vmraid.db.sql("delete from `tabSerial No` where name in ('ABCD', 'EFGH')")
	...
	def test_serial_by_series(self):
		...
		self.assertTrue(vmraid.db.exists("Serial No", serial_nos[0]))
		self.assertTrue(vmraid.db.exists("Serial No", serial_nos[1]))

		return se, serial_nos

	def test_serial_item_error(self):
		se, serial_nos = self.test_serial_by_series()
		if not vmraid.db.exists("Serial No", "ABCD"):
	...
	def test_serial_move(self):
		...
		se.submit()
		self.assertTrue(
			vmraid.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse 1 - _TC"
		)

		se.cancel()
		self.assertTrue(
			vmraid.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse - _TC"
		)
	...
	def test_serial_cancel(self):
		...
		serial_no = get_serial_nos(se.get("items")[0].serial_no)[0]
		self.assertFalse(vmraid.db.get_value("Serial No", serial_no, "warehouse"))

	def test_warehouse_company_validation(self):
		company = vmraid.db.get_value("Warehouse", "_Test Warehouse 2 - _TC1", "company")
		vmraid.get_doc("User", "test2@example.com").add_roles(
			"Sales User", "Sales Manager", "Stock User", "Stock Manager"
		)
		vmraid.set_user("test2@example.com")

		from erpadda.stock.utils import InvalidWarehouseCompany

		st1 = vmraid.copy_doc(test_records[0])
	...
	def test_warehouse_user(self):
		...
		test_user = vmraid.get_doc("User", "test@example.com")
		...
		st1.get("items")[0].t_warehouse = "_Test Warehouse 2 - _TC1"
		self.assertRaises(vmraid.PermissionError, st1.insert)
	...
	def test_freeze_stocks(self):
		vmraid.db.set_value("Stock Settings", None, "stock_auth_role", "")

		# test freeze_stocks_upto
		vmraid.db.set_value("Stock Settings", None, "stock_frozen_upto", add_days(nowdate(), 5))
		se = vmraid.copy_doc(test_records[0]).insert()
		self.assertRaises(StockFreezeError, se.submit)

		vmraid.db.set_value("Stock Settings", None, "stock_frozen_upto", "")

		# test freeze_stocks_upto_days
		vmraid.db.set_value("Stock Settings", None, "stock_frozen_upto_days", -1)
		se = vmraid.copy_doc(test_records[0])
		...
		se.insert()
		self.assertRaises(StockFreezeError, se.submit)
		vmraid.db.set_value("Stock Settings", None, "stock_frozen_upto_days", 0)
	...
	def test_work_order(self):
		...
		bom_no, bom_operation_cost = vmraid.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "is_default": 1, "docstatus": 1}, ["name", "operating_cost"]
		)

		work_order = vmraid.new_doc("Work Order")
		work_order.update(
			{
	...
	def test_work_order_manufacture_with_material_consumption(self):
		...
		bom_no = vmraid.db.get_value("BOM", {"item": "_Test FG Item", "is_default": 1, "docstatus": 1})
		...
		s = vmraid.get_doc(_make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		...
		s = vmraid.get_doc(_make_stock_entry(work_order.name, "Manufacture", 1))
		s.save()
		rm_cost = 0
		for d in s.get("items"):
			if d.s_warehouse:
	...
	def test_variant_work_order(self):
		...
		stock_entry = vmraid.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		stock_entry.insert()
	...
	def test_retain_sample(self):
		from erpadda.stock.doctype.batch.batch import get_batch_qty
		...
		if not vmraid.db.exists("Item", test_item_code):
		...
		receipt_entry = vmraid.new_doc("Stock Entry")
		...
		retention_entry = vmraid.new_doc("Stock Entry")
	...
	def test_quality_check(self):
		...
		repack.insert()
		self.assertRaises(vmraid.ValidationError, repack.submit)
	...
	def test_zero_incoming_rate(self):
		...
		value_diff = vmraid.db.get_value(
			"Stock Ledger Entry",
	...
	def test_gle_for_opening_stock_entry(self):
		...
		is_opening = vmraid.db.get_value(
			"GL Entry",
	...
	def test_conversion_factor_change(self):
		vmraid.db.set_value("Stock Settings", None, "allow_negative_stock", 1)
		repack_entry = vmraid.copy_doc(test_records[3])
	...
	def test_additional_cost_distribution_manufacture(self):
		se = vmraid.get_doc(
			doctype="Stock Entry",
			purpose="Manufacture",
			additional_costs=[vmraid._dict(base_amount=100)],
			items=[
				vmraid._dict(item_code="RM", basic_amount=10),
				vmraid._dict(item_code="FG", basic_amount=20, t_warehouse="X", is_finished_item=1),
				vmraid._dict(item_code="scrap", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_additional_cost_distribution_non_manufacture(self):
		se = vmraid.get_doc(
			...
			additional_costs=[vmraid._dict(base_amount=100)],
			items=[
				vmraid._dict(item_code="RECEIVED_1", basic_amount=20, t_warehouse="X"),
				vmraid._dict(item_code="RECEIVED_2", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_independent_manufacture_entry(self):
		...
		se = vmraid.get_doc(
			...
			items=[
				...
				vmraid._dict(item_code="_Test FG Item", qty=4, t_warehouse="_Test Warehouse 1 - _TC"),
			],
		)
	...
	def test_future_negative_sle_batch(self):
		from erpadda.stock.doctype.batch.test_batch import TestBatch
	...
	def test_multi_batch_value_diff(self):
		...
		receipt.append(
			"items", vmraid.copy_doc(receipt.items[0], ignore_no_copy=False).update({"basic_rate": 20})
		)
		receipt.save()
		receipt.submit()
		...
		issue.append("items", vmraid.copy_doc(issue.items[0], ignore_no_copy=False))
	...
	def test_transfer_qty_validation(self):
		...
		self.assertRaises(vmraid.ValidationError, se.save)
...
test_records = vmraid.get_test_records("Stock Entry")


def initialize_records_for_future_negative_sle_test(
	item_code, batch_no, warehouses, opening_qty, posting_date
):
	from erpadda.stock.doctype.batch.test_batch import TestBatch, make_new_batch
	from erpadda.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
		create_stock_reconciliation,
	)
	from erpadda.stock.doctype.warehouse.test_warehouse import create_warehouse
