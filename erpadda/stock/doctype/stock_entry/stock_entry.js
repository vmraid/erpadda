// Copyright (c) 2015, VMRaid and Contributors // License: GNU General Public License v3. See license.txt

vmraid.provide("erpadda.stock");
vmraid.provide("erpadda.accounts.dimensions");

{% include 'erpadda/stock/landed_taxes_and_charges_common.js' %};

vmraid.ui.form.on('Stock Entry', {
	setup: function(frm) {
		...
		vmraid.db.get_value('Stock Settings', {name: 'Stock Settings'}, 'sample_retention_warehouse', (r) => {
		...
		frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
			...
			if(!item.item_code) {
				vmraid.throw(__("Please enter Item Code to get Batch Number"));
			} else {
				if (in_list(["Material Transfer for Manufacture", "Manufacture", "Repack", "Send to Subcontractor"], doc.purpose)) {
					var filters = {
						'item_code': item.item_code,
						'posting_date': frm.doc.posting_date || vmraid.datetime.nowdate()
					}
				} else {
					var filters = {
						'item_code': item.item_code
		...
		vmraid.db.get_single_value('Stock Settings', 'disable_serial_no_and_batch_selector')
		.then((value) => {
			if (value) {
	...
	setup_quality_inspection: function(frm) {
		...
		frm.set_query("quality_inspection", "items", function(doc, cdt, cdn) {
			...
			return {
				query:"erpadda.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function(frm) {
		if(!frm.doc.docstatus) {
			...
			frm.add_custom_button(__('Material Request'), function() {
				vmraid.model.with_doctype('Material Request', function() {
					var mr = vmraid.model.get_new_doc('Material Request');
					var items = frm.get_field('items').grid.get_selected_children();
					if(!items.length) {
						items = frm.doc.items;
					}
					items.forEach(function(item) {
						var mr_item = vmraid.model.add_child(mr, 'items');
						...
						mr_item.required_date = vmraid.datetime.nowdate();
					});
					vmraid.set_route('Form', 'Material Request', mr.name);
				});
			}, __("Create"));
		}
		...
		if(frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Item'), () => {
					erpadda.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
		...
		if (frm.doc.docstatus === 1) {
			if (frm.doc.add_to_transit && frm.doc.purpose=='Material Transfer' && frm.doc.per_transferred < 100) {
				frm.add_custom_button(__('End Transit'), function() {
					vmraid.model.open_mapped_doc({
						method: "erpadda.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm
					})
				});
			}
			...
			if (frm.doc.per_transferred > 0) {
				frm.add_custom_button(__('Received Stock Entries'), function() {
					...
					vmraid.set_route('List', 'Stock Entry');
				}, __("View"));
			}
		}
		...
		if (frm.doc.docstatus===0) {
			frm.add_custom_button(__('Purchase Invoice'), function() {
				erpadda.utils.map_current_doc({
					method: "erpadda.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(__('Material Request'), function() {
				...
				const d = erpadda.utils.map_current_doc({
		...
		if (frm.doc.docstatus===0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(__('Expired Batches'), function() {
				vmraid.call({
					method: "erpadda.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
					callback: function(r) {
						if (!r.exc && r.message) {
							frm.set_value("items", []);
	...
	validate_purpose_consumption: function(frm) {
		vmraid.call({
			method: "erpadda.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
		}).then(r => {
			if (cint(r.message) == 0
				&& frm.doc.purpose=="Material Consumption for Manufacture") {
				frm.set_value("purpose", 'Manufacture');
				vmraid.throw(__('Material Consumption is not set in Manufacturing Settings.'));
			}
		});
	},
	...
	company: function(frm) {
		if(frm.doc.company) {
			var company_doc = vmraid.get_doc(":Company", frm.doc.company);
	...
	set_serial_no: function(frm, cdt, cdn, callback) {
		...
		vmraid.call({
			method: "erpadda.stock.get_item_details.get_serial_no",
			args: {"args": args},
			callback: function(r) {
				if (!r.exe && r.message){
					vmraid.model.set_value(cdt, cdn, "serial_no", r.message);
				}
				if (callback) {
					callback();
				}
		...
		vmraid.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					vmraid.msgprint(__("Retention Stock Entry already created or Sample Quantity not provided"));
				}
			}
		});
	},
	...
	set_basic_rate: function(frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			vmraid.call({
				...
				callback: function(r) {
					vmraid.model.set_value(cdt, cdn, 'basic_rate', (r.message || 0.0));
	...
	get_warehouse_details: function(frm, cdt, cdn) {
		...
		if(!child.bom_no) {
			vmraid.call({
				method: "erpadda.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					"args": {
				...
				callback: function(r) {
					if (!r.message) {
						...
						$.each(r.message, function(i, item) {
							let d = vmraid.model.add_child(cur_frm.doc, "Stock Entry Detail", "items");
	...
	calculate_total_additional_costs: function(frm) {
		const total_additional_costs = vmraid.utils.sum(
	...
	source_warehouse_address: function(frm) {
		erpadda.utils.get_address_display(frm, 'source_warehouse_address', 'source_address_display', false);
	},

	target_warehouse_address: function(frm) {
		erpadda.utils.get_address_display(frm, 'target_warehouse_address', 'target_address_display', false);
	},
	...
	set_transit_warehouse: function(frm) {
		if(frm.doc.add_to_transit && frm.doc.purpose == 'Material Transfer' && !frm.doc.to_warehouse
			...
			vmraid.db.get_value(dt, dn, 'default_in_transit_warehouse', (r) => {
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) erpadda.apply_putaway_rule(frm, frm.doc.purpose);
	}
});

vmraid.ui.form.on('Stock Entry Detail', {
	...
	s_warehouse: function(frm, cdt, cdn) {
		...
		if (item.s_warehouse) {
			vmraid.model.set_value(cdt, cdn, "allow_zero_valuation_rate", 0);
		}
	},
	...
	uom: function(doc, cdt, cdn) {
		...
		if(d.uom && d.item_code){
			return vmraid.call({
				method: "erpadda.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty
				...
				callback: function(r) {
					if(r.message) {
						...
						$.each(r.message, function(k, v) {
							if (v) {
								vmraid.model.set_value(cdt, cdn, k, v); // qty and it's subsequent fields weren't triggered
							}
						});
						refresh_field("items");
						...
						if (no_batch_serial_number_value && !vmraid.flags.hide_serial_batch_dialog) {
							erpadda.stock.select_batch_and_serial_no(frm, d);
						}
					}
				}
			});
	...
	expense_account: function(frm, cdt, cdn) {
		erpadda.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},
	cost_center: function(frm, cdt, cdn) {
		erpadda.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
...
vmraid.ui.form.on('Landed Cost Taxes and Charges', {
...
erpadda.stock.StockEntry = class StockEntry extends erpadda.stock.StockController {
	setup() {
		var me = this;
		...
		this.frm.fields_dict.items.grid.get_field('item_code').get_query = function() {
			return erpadda.queries.item({is_stock_item: 1});
		};
		...
		vmraid.dynamic_link = { doc: this.frm.doc, fieldname: 'supplier', doctype: 'Supplier' }
		this.frm.set_query("supplier_address", erpadda.queries.address_query)
	}
	...
	refresh() {
		...
		if (this.frm.doc.docstatus===1 && erpadda.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		erpadda.hide_company();
		erpadda.utils.add_item(this.frm);
	}
	...
	set_default_account(callback) {
		...
		if(this.frm.doc.company && erpadda.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				method: "erpadda.accounts.utils.get_company_default",
				args: {
	...
	clean_up() {
		...
		if(this.frm.doc.work_order &&
			...
			vmraid.model.remove_from_locals("Work Order",
	...
	get_items() {
		...
		if(!this.frm.doc.fg_completed_qty || !this.frm.doc.bom_no)
			vmraid.throw(__("BOM and Manufacturing Quantity are required"));
	...
	work_order() {
		...
		return vmraid.call({
			method: "erpadda.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	add_excise_button() {
		if(vmraid.boot.sysdefaults.country === "India")
			this.frm.add_custom_button(__("Excise Invoice"), function() {
				var excise = vmraid.model.make_new_doc_and_get_name('Journal Entry');
				excise = locals['Journal Entry'][excise];
				excise.voucher_type = 'Excise Entry';
				vmraid.set_route('Form', 'Journal Entry', excise.name);
			}, __('Create'));
	}
	...
	set_warehouse_in_children(child_table, warehouse_field, warehouse) {
		let transaction_controller = new erpadda.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	}

	items_on_form_rendered(doc, grid_row) {
		erpadda.setup_serial_or_batch_no();
	}
	...
	supplier(doc) {
		erpadda.utils.get_party_details(this.frm, null, null, null);
	}
};

erpadda.stock.select_batch_and_serial_no = (frm, item) => {
...
function attach_bom_items(bom_no) {
	...
	vmraid.db.get_doc("BOM",bom_no).then(bom => {
		const {name, items} = bom
		erpadda.stock.bom = {name, items:{}}
		items.forEach(item => {
			erpadda.stock.bom.items[item.item_code] = item;
		});
	});
}
...
function check_should_not_attach_bom_items(bom_no) {
  return (
    bom_no === undefined ||
    (erpadda.stock.bom && erpadda.stock.bom.name === bom_no)
  );
}

extend_cscript(cur_frm.cscript, new erpadda.stock.StockEntry({frm: cur_frm}));
