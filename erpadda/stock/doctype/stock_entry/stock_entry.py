...
from vmraid.query_builder.functions import Sum
from vmraid.utils import cint, comma_or, cstr, flt, format_time, formatdate, getdate, nowdate

import erpadda
from erpadda.accounts.general_ledger import process_gl_map
from erpadda.controllers.taxes_and_totals import init_landed_taxes_and_totals
from erpadda.manufacturing.doctype.bom.bom import add_additional_cost, validate_bom_no
from erpadda.setup.doctype.brand.brand import get_brand_defaults
from erpadda.setup.doctype.item_group.item_group import get_item_group_defaults
from erpadda.stock.doctype.batch.batch import get_batch_no, get_batch_qty, set_batch_nos
from erpadda.stock.doctype.item.item import get_item_defaults
from erpadda.stock.doctype.serial_no.serial_no import (
	get_serial_nos,
	update_serial_nos_after_submit,
)
from erpadda.stock.doctype.stock_reconciliation.stock_reconciliation import (
	OpeningEntryAccountError,
)
from erpadda.stock.get_item_details import (
	get_bin_details,
	get_conversion_factor,
	get_default_cost_center,
	get_reserved_qty_for_so,
)
from erpadda.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from erpadda.stock.utils import get_bin, get_incoming_rate


class FinishedGoodError(vmraid.ValidationError):
	pass


class IncorrectValuationRateError(vmraid.ValidationError):
	pass


class DuplicateEntryForWorkOrderError(vmraid.ValidationError):
	pass


class OperationsNotCompleteError(vmraid.ValidationError):
	pass


class MaxSampleAlreadyRetainedError(vmraid.ValidationError):
	pass


from erpadda.controllers.stock_controller import StockController
...
class StockEntry(StockController):
	...
	def before_validate(self):
		from erpadda.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate(self):
		self.pro_doc = vmraid._dict()
		if self.work_order:
			self.pro_doc = vmraid.get_doc("Work Order", self.work_order)
	...
	def validate_work_order_status(self):
		pro_doc = vmraid.get_doc("Work Order", self.work_order)
		if pro_doc.status == "Completed":
			vmraid.throw(_("Cannot cancel transaction for Completed Work Order."))
	...
	def validate_purpose(self):
		...
		if self.purpose not in valid_purposes:
			vmraid.throw(_("Purpose must be one of {0}").format(comma_or(valid_purposes)))
	...
	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in vmraid.get_all(
				...
				vmraid.delete_doc("Stock Entry", d.name)

	def set_transfer_qty(self):
		for item in self.get("items"):
			if not flt(item.qty):
				vmraid.throw(_("Row {0}: Qty is mandatory").format(item.idx), title=_("Zero quantity"))
			if not flt(item.conversion_factor):
				vmraid.throw(_("Row {0}: UOM Conversion Factor is mandatory").format(item.idx))
	...
	def update_cost_in_project(self):
		...
		if self.project:
			...
			additional_costs = vmraid.db.sql(
			...
			vmraid.db.set_value("Project", self.project, "total_consumed_material_cost", amount)
			...
			if flt(item.qty) and flt(item.qty) < 0:
				vmraid.throw(
					_("Row {0}: The item {1}, quantity must be positive number").format(
						item.idx, vmraid.bold(item.item_code)
					)
				)

			if item.item_code not in stock_items:
				vmraid.throw(_("{0} is not a stock Item").format(item.item_code))
	...
	def validate_qty(self):
		...
		if self.purpose in manufacture_purpose and self.work_order:
			if not vmraid.get_value("Work Order", self.work_order, "skip_transfer"):
				item_code = []
				for item in self.items:
					if cstr(item.t_warehouse) == "":
						req_items = vmraid.get_all(
							"Work Order Item",
							filters={"parent": self.work_order, "item_code": item.item_code},
							fields=["item_code"],
						)

						transferred_materials = vmraid.db.sql(
							"""
									select
										sum(qty) as qty
	...
	def validate_fg_completed_qty(self):
		...
		for item_code, qty_list in item_wise_qty.items():
			total = flt(sum(qty_list), vmraid.get_precision("Stock Entry Detail", "qty"))
			if self.fg_completed_qty != total:
				vmraid.throw(
					_("The finished product {0} quantity {1} and For Quantity {2} cannot be different").format(
						vmraid.bold(item_code), vmraid.bold(total), vmraid.bold(self.fg_completed_qty)
					)
				)

	def validate_difference_account(self):
		if not cint(erpadda.is_perpetual_inventory_enabled(self.company)):
			return
		...
		for d in self.get("items"):
			if not d.expense_account:
				vmraid.throw(
					...
					).format(vmraid.bold(self.company))
				)

			elif (
				self.is_opening == "Yes"
				and vmraid.db.get_value("Account", d.expense_account, "report_type") == "Profit and Loss"
			):
				vmraid.throw(
					_(
		...
		for d in self.get("items"):
			...
			if self.purpose in source_mandatory and not d.s_warehouse:
				if self.from_warehouse:
					...
					vmraid.throw(_("Source warehouse is mandatory for row {0}").format(d.idx))

			if self.purpose in target_mandatory and not d.t_warehouse:
				if self.to_warehouse:
					d.t_warehouse = self.to_warehouse
				else:
					vmraid.throw(_("Target warehouse is mandatory for row {0}").format(d.idx))
			...
			if (
				...
				vmraid.throw(_("Source and target warehouse cannot be same for row {0}").format(d.idx))

			if not (d.s_warehouse or d.t_warehouse):
				vmraid.throw(_("Atleast one warehouse is mandatory"))
		...
		if self.purpose in (
			...
			if (
				...
				if not self.fg_completed_qty:
					vmraid.throw(_("For Quantity (Manufactured Qty) is mandatory"))
	...
	def check_if_operations_completed(self):
		...
		prod_order = vmraid.get_doc("Work Order", self.work_order)
		allowance_percentage = flt(
			vmraid.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order")
		)
		...
		for d in prod_order.get("operations"):
			...
			if total_completed_qty > flt(completed_qty):
				job_card = vmraid.db.get_value("Job Card", {"operation_id": d.name}, "name")
				if not job_card:
					vmraid.throw(
				...
				work_order_link = vmraid.utils.get_link_to_form("Work Order", self.work_order)
				job_card_link = vmraid.utils.get_link_to_form("Job Card", job_card)
				vmraid.throw(
					_(
						...
						d.idx,
						vmraid.bold(d.operation),
						vmraid.bold(total_completed_qty),
						work_order_link,
						job_card_link,
					),
	...
	def check_duplicate_entry_for_work_order(self):
		...
		if other_ste:
			production_item, qty = vmraid.db.get_value(
				"Work Order", self.work_order, ["production_item", "qty"]
			)
			args = other_ste + [production_item]
			fg_qty_already_entered = vmraid.db.sql(
	...
	def set_actual_qty(self):
		from erpadda.stock.stock_ledger import is_negative_stock_allowed
			...
			if (
				...
				vmraid.throw(
					_(
						...
						d.idx,
						vmraid.bold(d.s_warehouse),
						formatdate(self.posting_date),
						format_time(self.posting_time),
						vmraid.bold(d.item_code),
					)
					+ "<br><br>"
					+ _("Available quantity is {0}, you need {1}").format(
						vmraid.bold(d.actual_qty), vmraid.bold(d.transfer_qty)
					),
					NegativeStockError,
	...
	def set_serial_nos(self, work_order):
		previous_se = vmraid.db.get_value(
			"Stock Entry",
		...
		for d in self.get("items"):
			transferred_serial_no = vmraid.db.get_value(
	...
	def get_basic_rate_for_manufactured_item(self, finished_item_qty, outgoing_items_cost=0):
		...
		if not outgoing_items_cost and vmraid.db.get_single_value(
	...
	def set_stock_entry_type(self):
		if self.purpose:
			self.stock_entry_type = vmraid.get_cached_value(
	...
	def set_purpose_for_stock_entry(self):
		if self.stock_entry_type and not self.purpose:
			self.purpose = vmraid.get_cached_value("Stock Entry Type", self.stock_entry_type, "purpose")
	...
	def validate_purchase_order(self):
		...
		backflush_raw_materials_based_on = vmraid.db.get_single_value(
			"Buying Settings", "backflush_raw_materials_of_subcontract_based_on"
		)

		qty_allowance = flt(vmraid.db.get_single_value("Buying Settings", "over_transfer_allowance"))

		if not (self.purpose == "Send to Subcontractor" and self.purchase_order):
			return

		if backflush_raw_materials_based_on == "BOM":
			purchase_order = vmraid.get_doc("Purchase Order", self.purchase_order)
			for se_item in self.items:
				...
				if not required_qty:
					...
					if se_item.allow_alternative_item:
						original_item_code = vmraid.get_value(
				...
				if not row.subcontracted_item:
					vmraid.throw(
						_("Row {0}: Subcontracted Item is mandatory for the raw material {1}").format(
							row.idx, vmraid.bold(row.item_code)
						)
					)
					...
					po_detail = vmraid.db.get_value("Purchase Order Item Supplied", filters, "name")
					if po_detail:
	...
	def get_finished_item(self):
		finished_item = None
		if self.work_order:
			finished_item = vmraid.db.get_value("Work Order", self.work_order, "production_item")
		elif self.bom_no:
			finished_item = vmraid.db.get_value("BOM", self.bom_no, "item")
	...
	def validate_finished_goods(self):
		...
		wo_details = vmraid.db.get_value("Work Order", self.work_order, ["production_item", "qty"])
		if wo_details:
	...
	def update_work_order(self):
		def _validate_work_order(pro_doc):
			if flt(pro_doc.docstatus) != 1:
				vmraid.throw(_("Work Order {0} must be submitted").format(self.work_order))
		...
		if self.job_card:
			job_doc = vmraid.get_doc("Job Card", self.job_card)
	...
	def get_item_details(self, args=None, for_update=False):
		...
		if self.purpose == "Material Issue":
			ret["expense_account"] = (
				...
				or vmraid.get_cached_value("Company", self.company, "default_expense_account")
			)
		...
		for company_field, field in {
			...
			if not ret.get(field):
				ret[field] = vmraid.get_cached_value("Company", self.company, company_field)
		...
		if (
			...
			subcontract_items = vmraid.get_all(
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == "Material Transfer":
			doc = vmraid.get_doc("Stock Entry", self.outgoing_stock_entry)

			if doc.per_transferred == 100:
				vmraid.throw(_("Goods are already received against the outward entry {0}").format(doc.name))
	...
	def get_items(self):
		...
		if not self.posting_date or not self.posting_time:
			vmraid.throw(_("Posting date and posting time is mandatory"))

		self.set_work_order_details()
		self.flags.backflush_based_on = vmraid.db.get_single_value(
			"Manufacturing Settings", "backflush_raw_materials_based_on"
		)

		if self.bom_no:

			backflush_based_on = vmraid.db.get_single_value(
				"Manufacturing Settings", "backflush_raw_materials_based_on"
			)
			...
			if self.purpose in [
				...
				if self.work_order and self.purpose == "Material Transfer for Manufacture":
					...
					and vmraid.db.get_single_value("Manufacturing Settings", "material_consumption") == 1
				):
					self.get_unconsumed_raw_materials()

				else:
					if not self.fg_completed_qty:
						vmraid.throw(_("Manufacturing Quantity is mandatory"))
			...
			if self.work_order and self.purpose == "Manufacture":
				...
				work_order = vmraid.get_doc("Work Order", self.work_order)
	...
	def load_items_from_bom(self):
		if self.work_order:
			...
			item_code = vmraid.db.get_value("BOM", self.bom_no, "item")
	...
	def set_batchwise_finished_goods(self, args, item):
		...
		data = vmraid.get_all("Batch", filters=filters, fields=fields, order_by="creation asc")
	...
	def get_bom_raw_materials(self, qty):
		from erpadda.manufacturing.doctype.bom.bom import get_bom_items_as_dict
		...
		for item in item_dict.values():
			...
			if item["allow_alternative_item"]:
				item["allow_alternative_item"] = vmraid.db.get_value(
	...
	def get_scrap_items_from_job_card(self):
		...
		job_card_scrap_item = vmraid.qb.DocType("Job Card Scrap Item")

		scrap_items = (
			vmraid.qb.from_(job_card)
			.select(
		...
		for row in scrap_items:
			...
			if cint(vmraid.get_cached_value("UOM", row.stock_uom, "must_be_whole_number")):
				row.stock_qty = vmraid.utils.ceil(row.stock_qty)
	...
	def get_unconsumed_raw_materials(self):
		wo = vmraid.get_doc("Work Order", self.work_order)
		wo_items = vmraid.get_all(
			"Work Order Item",
	...
	def get_transfered_raw_materials(self):
		...
		materials_already_backflushed = vmraid.db.sql(
			"""
			select
		...
		for item in transferred_materials:
			...
			if trans_qty and manufacturing_qty > (produced_qty + flt(self.fg_completed_qty)):
				...
				precision = vmraid.get_precision("Stock Entry Detail", "qty")
			...
			if cint(vmraid.get_cached_value("UOM", item.stock_uom, "must_be_whole_number")):
				qty = vmraid.utils.ceil(qty)
	...
	def get_pending_raw_materials(self, backflush_based_on=None):
		...
		if not len(item_dict):
			vmraid.msgprint(_("""All items have already been transferred for this Work Order."""))
	...
	def get_pro_order_required_items(self, backflush_based_on=None):
		...
		"""
		item_dict, job_card_items = vmraid._dict(), []
		work_order = vmraid.get_doc("Work Order", self.work_order)
		...
		if not vmraid.db.get_value("Warehouse", work_order.wip_warehouse, "is_group"):
		...
		for d in work_order.get("required_items"):
			...
			if d.include_item_in_manufacturing:
				...
				if consider_job_card:
					job_card_item = vmraid.db.get_value(
						"Job Card Item", {"item_code": d.item_code, "parent": self.get("job_card")}
					)
					item_row["job_card_item"] = job_card_item or None

				if d.source_warehouse and not vmraid.db.get_value("Warehouse", d.source_warehouse, "is_group"):
	...
	def add_to_stock_entry_detail(self, item_dict, bom_no=None):
		for d in item_dict:
			...
			stock_uom = item_row.get("stock_uom") or vmraid.db.get_value("Item", d, "stock_uom")
	...
	def validate_batch(self):
		if self.purpose in [
			...
			for item in self.get("items"):
				if item.batch_no:
					disabled = vmraid.db.get_value("Batch", item.batch_no, "disabled")
					if disabled == 0:
						expiry_date = vmraid.db.get_value("Batch", item.batch_no, "expiry_date")
						if expiry_date:
							if getdate(self.posting_date) > getdate(expiry_date):
								vmraid.throw(_("Batch {0} of Item {1} has expired.").format(item.batch_no, item.item_code))
					else:
						vmraid.throw(_("Batch {0} of Item {1} is disabled.").format(item.batch_no, item.item_code))
	...
	def update_purchase_order_supplied_items(self):
		if self.purchase_order and (
			...
			for name, item in supplied_items.items():
				vmraid.db.set_value("Purchase Order Item Supplied", name, item)
	...
	def update_so_in_serial_number(self):
		so_name, item_code = vmraid.db.get_value(
		...
		if so_name and item_code:
			...
			if qty_to_reserve:
				...
				if qty_to_reserve > 0:
					for item in self.items:
						has_serial_no = vmraid.get_cached_value("Item", item.item_code, "has_serial_no")
						if item.item_code == item_code and has_serial_no:
							serial_nos = (item.serial_no).split("\n")
							for serial_no in serial_nos:
								if qty_to_reserve > 0:
									vmraid.db.set_value("Serial No", serial_no, "sales_order", so_name)
									qty_to_reserve -= 1
		...
		for item in self.items:
			if item.s_warehouse and not item.t_warehouse and item.serial_no:
				for sr in get_serial_nos(item.serial_no):
					sales_order = vmraid.db.get_value("Serial No", sr, "sales_order")
					if sales_order:
						msg = _(
							"(Serial No: {0}) cannot be consumed as it's reserverd to fullfill Sales Order {1}."
						).format(sr, sales_order)

						vmraid.throw(_("Item {0} {1}").format(item.item_code, msg))
	...
	def update_transferred_qty(self):
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			...
			for d in self.items:
				...
				transferred_qty = vmraid.get_all(
					"Stock Entry Detail",
			...
			for data, transferred_qty in stock_entries.items():
				cond += """ WHEN (parent = %s and name = %s) THEN %s
					""" % (
					vmraid.db.escape(data[0]),
					vmraid.db.escape(data[1]),
					transferred_qty,
				)
	...
	def set_material_request_transfer_status(self, status):
		...
		if self.outgoing_stock_entry:
			parent_se = vmraid.get_value("Stock Entry", self.outgoing_stock_entry, "add_to_transit")
		...
		for item in self.items:
			...
			if self.purpose == "Material Transfer" and material_request not in material_requests:
				if self.outgoing_stock_entry and parent_se:
					material_request = vmraid.get_value("Stock Entry Detail", item.ste_detail, "material_request")

			if material_request and material_request not in material_requests:
				material_requests.append(material_request)
				vmraid.db.set_value("Material Request", material_request, "transfer_status", status)
	...
	def update_items_for_process_loss(self):
		...
		for d in self.get("items"):
			...
			scrap_warehouse = vmraid.db.get_single_value(
	...
	def get_serial_nos_for_fg(self, args):
		...
		stock_entries = vmraid.get_all("Stock Entry", fields=fields, filters=filters)
...
def move_sample_to_retention_warehouse(company, items):
	...
	retention_warehouse = vmraid.db.get_single_value("Stock Settings", "sample_retention_warehouse")
...
def make_stock_in_entry(source_name, target_doc=None):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = vmraid.db.get_value("Stock Entry", source_name, "add_to_transit")
			if add_to_transit:
				warehouse = vmraid.get_value(
...
def get_work_order_details(work_order, company):
	work_order = vmraid.get_doc("Work Order", work_order)
...
def get_operating_cost_per_unit(work_order=None, bom_no=None):
	...
	if not operating_cost_per_unit and bom_no:
		bom = vmraid.db.get_value("BOM", bom_no, ["operating_cost", "quantity"], as_dict=1)
		if bom.quantity:
...
def get_valuation_rate_for_finished_good_entry(work_order):
	work_order_qty = flt(
		vmraid.get_cached_value("Work Order", work_order, "material_transferred_for_manufacturing")
	)

	field = "(SUM(total_outgoing_value) / %s) as valuation_rate" % (work_order_qty)

	stock_data = vmraid.get_all(
		"Stock Entry",
		fields=field,
		filters={
			"docstatus": 1,
...
def validate_sample_quantity(item_code, sample_quantity, qty, batch_no=None):
	...
	max_retain_qty = vmraid.get_value("Item", item_code, "sample_quantity")
...
def get_supplied_items(purchase_order):
	...
	for row in vmraid.get_all("Stock Entry", fields=fields, filters=filters):
		...
		if key not in supplied_item_details:
			supplied_item_details.setdefault(
				key, vmraid._dict({"supplied_qty": 0, "returned_qty": 0, "total_supplied_qty": 0})
			)
