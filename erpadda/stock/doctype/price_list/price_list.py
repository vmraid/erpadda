...
class PriceList(Document):
	...
	def set_default_if_missing(self):
		if cint(self.selling):
			if not vmraid.db.get_value("Selling Settings", None, "selling_price_list"):
				vmraid.set_value("Selling Settings", "Selling Settings", "selling_price_list", self.name)

		elif cint(self.buying):
			if not vmraid.db.get_value("Buying Settings", None, "buying_price_list"):
				vmraid.set_value("Buying Settings", "Buying Settings", "buying_price_list", self.name)
	...
	def check_impact_on_shopping_cart(self):
		...
		affects_cart = self.name == vmraid.get_cached_value("E Commerce Settings", None, "price_list")
	...
	def on_trash(self):
		...
		def _update_default_price_list(module):
			b = vmraid.get_doc(module + " Settings")
	...
	def delete_price_list_details_key(self):
		vmraid.cache().hdel("price_list_details", self.name)


def get_price_list_details(price_list):
	price_list_details = vmraid.cache().hget("price_list_details", price_list)

	if not price_list_details:
		price_list_details = vmraid.get_cached_value(
		...
		vmraid.cache().hset("price_list_details", price_list, price_list_details)
