...
from vmraid.utils import cint, get_link_to_form, get_weekday, now, nowtime
from vmraid.utils.user import get_users_with_role
from rq.timeouts import JobTimeoutException

import erpadda
from erpadda.accounts.utils import update_gl_entries_after
from erpadda.stock.stock_ledger import get_items_to_be_repost, repost_future_sle
...
class RepostItemValuation(Document):
	...
	def on_submit(self):
		...
		Exceptions:
		        ...
		        2. global flag vmraid.flags.dont_execute_stock_reposts is set
		...
		if not vmraid.flags.in_test:
			return
		if self.flags.dont_run_in_test or vmraid.flags.dont_execute_stock_reposts:
			return
	...
	def check_pending_repost_against_cancelled_transaction(self):
		...
		transaction_status = vmraid.db.get_value(self.voucher_type, self.voucher_no, "docstatus")
		if transaction_status == 2:
			msg = _("Cannot cancel as processing of cancelled documents is  pending.")
			msg += "<br>" + _("Please try again in an hour.")
			vmraid.throw(msg, title=_("Pending processing"))
...
def on_doctype_update():
	vmraid.db.add_index("Repost Item Valuation", ["warehouse", "item_code"], "item_warehouse")
...
def repost_entries():
	...
	for row in riv_entries:
		doc = vmraid.get_doc("Repost Item Valuation", row.name)
...
def in_configured_timeslot(repost_settings=None, current_time=None):
	...
	if repost_settings is None:
		repost_settings = vmraid.get_cached_doc("Stock Reposting Settings")
