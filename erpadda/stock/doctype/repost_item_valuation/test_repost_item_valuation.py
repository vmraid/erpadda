...
from erpadda.controllers.stock_controller import create_item_wise_repost_entries
...
from erpadda.stock.doctype.repost_item_valuation.repost_item_valuation import (
	in_configured_timeslot,
)
from erpadda.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
from erpadda.stock.utils import PendingRepostingError


class TestRepostItemValuation(VMRaidTestCase):
	def tearDown(self):
		vmraid.flags.dont_execute_stock_reposts = False

	def test_repost_time_slot(self):
		repost_settings = vmraid.get_doc("Stock Reposting Settings")
	...
	def test_deduplication(self):
		...
		riv2 = vmraid.get_doc(riv_args.update({"posting_date": "2021-01-03"}))
		...
		riv3 = vmraid.get_doc(riv_args.update({"posting_date": "2021-01-01"}))
		...
		riv4 = vmraid.get_doc(riv_args.update({"warehouse": "Stores - _TC"}))
	...
	def test_prevention_of_cancelled_transaction_riv(self):
		...
		riv = vmraid.get_last_doc(
			"Repost Item Valuation", {"voucher_type": old.doctype, "voucher_no": old.name}
		)
		self.assertRaises(vmraid.ValidationError, riv.cancel)
