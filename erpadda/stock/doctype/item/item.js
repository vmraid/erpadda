...
vmraid.provide("erpadda.item");

vmraid.ui.form.on("Item", {
	setup: function(frm) {
	...
	onload: function(frm) {
		erpadda.item.setup_queries(frm);
	...
	refresh: function(frm) {
		if (frm.doc.is_stock_item) {
			...
			frm.add_custom_button(__("Stock Projected Qty"), function() {
				...
				vmraid.set_route("query-report", "Stock Projected Qty");
			}, __("View"));
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.add_custom_button(__("Show Variants"), function() {
				vmraid.set_route("List", "Item", {"variant_of": frm.doc.name});
			}, __("View"));

			frm.add_custom_button(__("Item Variant Settings"), function() {
				vmraid.set_route("Form", "Item Variant Settings");
			}, __("View"));

			frm.add_custom_button(__("Variant Details Report"), function() {
				vmraid.set_route("query-report", "Item Variant Details", {"item": frm.doc.name});
			}, __("View"));

			if(frm.doc.variant_based_on==="Item Attribute") {
				frm.add_custom_button(__("Single Variant"), function() {
					erpadda.item.show_single_variant_dialog(frm);
				}, __('Create'));
				frm.add_custom_button(__("Multiple Variants"), function() {
					erpadda.item.show_multiple_variants_dialog(frm);
				}, __('Create'));
			} else {
				frm.add_custom_button(__("Variant"), function() {
					erpadda.item.show_modal_for_manufacturers(frm);
				}, __('Create'));
			}
		...
		if (vmraid.defaults.get_default("item_naming_by")!="Naming Series" || frm.doc.variant_of) {
		...
		if (!frm.doc.published_in_website) {
			frm.add_custom_button(__("Publish in Website"), function() {
				vmraid.call({
					method: "erpadda.e_commerce.doctype.website_item.website_item.make_website_item",
					args: {doc: frm.doc},
					freeze: true,
			...
			frm.add_custom_button(__("Website Item"), function() {
				vmraid.db.get_value("Website Item", {item_code: frm.doc.name}, "name", (d) => {
					if (!d.name) vmraid.throw(__("Website Item not found"));
					vmraid.set_route("Form", "Website Item", d.name);
				});
			}, __("View"));
		}

		erpadda.item.edit_prices_button(frm);
		erpadda.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			erpadda.item.make_dashboard(frm);
		}

		frm.add_custom_button(__('Duplicate'), function() {
			var new_item = vmraid.model.copy_doc(frm.doc);
			...
			vmraid.set_route('Form', 'Item', new_item.name);
		});
	...
	validate: function(frm){
		erpadda.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function(frm) {
		...
		vmraid.call({
			method: "erpadda.stock.doctype.item.item.get_asset_naming_series",
			callback: function(r) {
	...
	page_name: vmraid.utils.warn_page_name_change,
...
vmraid.ui.form.on('Item Reorder', {
...
vmraid.ui.form.on('Item Customer Detail', {
	customer_items_add: function(frm, cdt, cdn) {
		vmraid.model.set_value(cdt, cdn, 'customer_group', "");
	},
	...
	customer_group: function(frm, cdt, cdn) {
		if(set_customer_group(frm, cdt, cdn)){
			vmraid.msgprint(__("Changing Customer Group for the selected Customer is not allowed."));
		}
	}
});
...
var set_customer_group = function(frm, cdt, cdn) {
	...
	vmraid.model.with_doc("Customer", row.customer_name, function() {
		var customer = vmraid.model.get_doc("Customer", row.customer_name);
...
$.extend(erpadda.item, {
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.is_stock_item) {
			vmraid.require('item-dashboard.bundle.js', function() {
				const section = frm.dashboard.add_section('', __("Stock Levels"));
				erpadda.item.item_dashboard = new erpadda.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: 'erpadda.stock.dashboard.item_dashboard.get_data',
					template: 'item_dashboard_list'
				});
				erpadda.item.item_dashboard.refresh();
			});
		}
	},
	...
	edit_prices_button: function(frm) {
		frm.add_custom_button(__("Add / Edit Prices"), function() {
			vmraid.set_route("List", "Item Price", {"item_code": frm.doc.name});
		}, __("Actions"));
	},
	...
	show_modal_for_manufacturers: function(frm) {
		...
		dialog.set_primary_action(__('Create'), function() {
			...
			vmraid.call({
				method: "erpadda.controllers.item_variant.get_variant",
				args: data,
				callback: function(r) {
	...
	show_multiple_variants_dialog: function(frm) {
		...
		function make_and_show_dialog(fields) {
			me.multiple_variant_dialog = new vmraid.ui.Dialog({
			...
			me.multiple_variant_dialog.set_primary_action(__('Create Variants'), () => {
				...
				vmraid.call({
					method: "erpadda.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						"item": frm.doc.name,
			...
			$($(me.multiple_variant_dialog.$wrapper.find('.form-column'))
				.find('.vmraid-control')).css('margin-bottom', '0px');
			...
			vmraid.call({
				...
				callback: function(r) {
					...
					if (r.message) {
						...
						vmraid.msgprint_dialog = vmraid.msgprint(__("Item Variant {0} already exists with same attributes",
							[repl('<a href="/app/item/%(item_encoded)s" class="strong variant-click">%(item)s</a>', {
								item_encoded: encodeURIComponent(variant),
								item: variant
							})]
						));
						vmraid.msgprint_dialog.hide_on_page_refresh = true;
						vmraid.msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							d.hide();
						});
					} else {
						d.hide();
						vmraid.call({
							method: "erpadda.controllers.item_variant.create_variant",
							args: {
								"item": frm.doc.name,
								"args": d.get_values()
							},
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on('input', function(e) {
					...
					vmraid.call({
						method: "erpadda.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
vmraid.ui.form.on("UOM Conversion Detail", {
	uom: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.uom) {
			vmraid.call({
				method: "erpadda.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					"uom": row.uom,
				...
				callback: function(r) {
					if (!r.exc && r.message) {
						vmraid.model.set_value(cdt, cdn, "conversion_factor", r.message);
					}
				}
			});
		}
...
vmraid.tour['Item'] = [
	...
	},
	{
		...
		description: __("If you are maintaining stock of this Item in your Inventory, ERPAdda will make a stock ledger entry for each transaction of this item.")
	},
	{
