...
def make_item(item_code=None, properties=None):
	if not item_code:
		item_code = vmraid.generate_hash(length=16)
...
class TestItem(VMRaidTestCase):
	...
	def test_get_item_details(self):
		...
		currency = vmraid.get_cached_value("Company", company, "default_currency")
	...
	def test_item_default_validations(self):
		...
		with self.assertRaises(vmraid.ValidationError) as ve:
			make_item(
				"Bad Item defaults",
				{
					"item_group": "_Test Item Group",
	...
	def test_make_item_variant_with_numeric_values(self):
		# cleanup
		for d in vmraid.db.get_all("Item", filters={"variant_of": "_Test Numeric Template Item"}):
	...
	def test_item_merging(self):
		old = create_item(vmraid.generate_hash(length=20)).name
		new = create_item(vmraid.generate_hash(length=20)).name
		...
		vmraid.rename_doc("Item", old, new, merge=True)

		self.assertFalse(vmraid.db.exists("Item", old))

		self.assertTrue(
			vmraid.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse - _TC"})
		)
		self.assertTrue(
			vmraid.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse 1 - _TC"})
		)
	...
	def test_item_merging_with_product_bundle(self):
		...
		with self.assertRaises(DataValidationError):
			vmraid.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		bundle1.delete()
		vmraid.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		self.assertFalse(vmraid.db.exists("Item", "Test Item Bundle Item 1"))
	...
	def test_item_variant_by_manufacturer(self):
		...
		manufacturer = vmraid.get_doc(dict(doctype="Manufacturer", short_name="MSG1")).insert()
	...
	def test_index_creation(self):
		...
		indices = vmraid.db.sql("show index from tabItem", as_dict=1)
	...
	def test_check_stock_uom_with_bin(self):
		...
		item = vmraid.get_doc("Item", "_Test Item")
		item.stock_uom = "Gram"
		self.assertRaises(vmraid.ValidationError, item.save)

	def test_check_stock_uom_with_bin_no_sle(self):
		from erpadda.stock.stock_balance import update_bin_qty
	...
	def test_erasure_of_old_conversions(self):
		...
		item.stock_uom = "Gram"
		item.append("uoms", vmraid._dict(uom="Box", conversion_factor=2))
		item.save()
		item.reload()
		item.stock_uom = "Nos"
		item.save()
	...
	def test_validate_stock_item(self):
		self.assertRaises(vmraid.ValidationError, validate_is_stock_item, "_Test Non Stock Item")
	...
	def test_item_dashboard(self):
		from erpadda.stock.dashboard.item_dashboard import get_data
