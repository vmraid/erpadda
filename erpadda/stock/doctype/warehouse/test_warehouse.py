...
from erpadda.stock.doctype.warehouse.warehouse import convert_to_group_or_ledger, get_children

test_records = vmraid.get_test_records("Warehouse")


class TestWarehouse(VMRaidTestCase):
	def setUp(self):
		super().setUp()
	...
	def test_warehouse_hierarchy(self):
		...
		child_warehouses = vmraid.db.sql(
	...
	def test_naming(self):
		...
		wh = vmraid.get_doc(doctype="Warehouse", warehouse_name=warehouse_name, company=company).insert()
	...
	def test_unlinking_warehouse_from_item_defaults(self):
		...
		for warehouse in warehouse_ids:
			vmraid.delete_doc("Warehouse", warehouse)

		# Check Item existance
		for item in item_names:
			self.assertTrue(bool(vmraid.db.exists("Item", item)), f"{item} doesn't exist")
	...
	def test_group_non_group_conversion(self):
		...
		warehouse = vmraid.get_doc("Warehouse", create_warehouse("TestGroupConversion"))
		...
		# chid exists
		self.assertRaises(vmraid.ValidationError, convert_to_group_or_ledger, warehouse.name)
		vmraid.delete_doc("Warehouse", child)
...
def get_group_stock_account(company, company_abbr=None):
	group_stock_account = vmraid.db.get_value(
