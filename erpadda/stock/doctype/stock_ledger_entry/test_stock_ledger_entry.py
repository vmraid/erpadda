...
import vmraid
from vmraid.core.page.permission_manager.permission_manager import reset
...
from erpadda.accounts.doctype.gl_entry.gl_entry import rename_gle_sle_docs
...
from erpadda.stock.doctype.landed_cost_voucher.test_landed_cost_voucher import (
...
from erpadda.stock.doctype.stock_ledger_entry.stock_ledger_entry import BackDatedStockTransaction
...
class TestStockLedgerEntry(VMRaidTestCase):
	def setUp(self):
		items = create_items()
		reset("Stock Entry")
	...
	def test_item_cost_reposting(self):
		...
		create_stock_reconciliation(
			...
			if vmraid.get_all("Stock Ledger Entry")
		...
		target_wh_sle = vmraid.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": "_Test Item for Reposting",
				"warehouse": "Finished Goods - _TC",
		...
		finished_item_sle = vmraid.db.get_value(
			"Stock Ledger Entry",
			{
	...
	def test_purchase_return_valuation_reposting(self):
		...
		# check sle
		outgoing_rate, stock_value_difference = vmraid.db.get_value(
			"Stock Ledger Entry",
	...
	def test_fifo_dependent_consumption(self):
		...
		for rate in rates[1:]:
			row = vmraid.copy_doc(receipt.items[0], ignore_no_copy=False)
			row.basic_rate = rate
		...
		for rate in rates[1:]:
			row = vmraid.copy_doc(transfer.items[0], ignore_no_copy=False)
	...
	def test_fifo_multi_item_repack_consumption(self):
		...
		for rate in rates[1:]:
			row = vmraid.copy_doc(repack.items[0], ignore_no_copy=False)
...
def create_repack_entry(**args):
	...
	repack = vmraid.new_doc("Stock Entry")
...
def create_product_bundle_item(new_item_code, packed_items):
	if not vmraid.db.exists("Product Bundle", new_item_code):
		item = vmraid.new_doc("Product Bundle")
...
class TestDeferredNaming(VMRaidTestCase):
	@classmethod
	def setUpClass(cls) -> None:
		super().setUpClass()
		cls.gle_autoname = vmraid.get_meta("GL Entry").autoname
		cls.sle_autoname = vmraid.get_meta("Stock Ledger Entry").autoname
	...
	def get_gle_sles(se):
		...
		gle = set(vmraid.get_list("GL Entry", filters, pluck="name"))
		sle = set(vmraid.get_list("Stock Ledger Entry", filters, pluck="name"))
		return gle, sle
