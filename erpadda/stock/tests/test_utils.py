...
class TestStockUtilities(VMRaidTestCase):
	def test_barcode_scanning(self):
		...
		batch = vmraid.get_doc(doctype="Batch", item=batch_item.name).insert()
		...
		serial = vmraid.get_doc(
			doctype="Serial No", item_code=serial_item.name, serial_no=vmraid.generate_hash()
		).insert()
