...
from vmraid.utils import add_days, add_months, cint, cstr, flt, getdate

from erpadda import get_company_currency
from erpadda.accounts.doctype.pricing_rule.pricing_rule import (
...
from erpadda.stock.doctype.batch.batch import get_batch_no
from erpadda.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from erpadda.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from erpadda.stock.doctype.price_list.price_list import get_price_list_details
...
def get_item_details(args, doc=None, for_validate=False, overwrite_warehouse=True):
	...
	item = vmraid.get_cached_doc("Item", args.item_code)
...
def set_valuation_rate(out, args):
	if vmraid.db.exists("Product Bundle", args.item_code, cache=True):
		valuation_rate = 0.0
		bundled_items = vmraid.get_doc("Product Bundle", args.item_code)
...
def get_item_code(barcode=None, serial_no=None):
	if barcode:
		item_code = vmraid.db.get_value("Item Barcode", {"barcode": barcode}, fieldname=["parent"])
		if not item_code:
			vmraid.throw(_("No Item with Barcode {0}").format(barcode))
	elif serial_no:
		item_code = vmraid.db.get_value("Serial No", serial_no, "item_code")
		if not item_code:
			vmraid.throw(_("No Item with Serial No {0}").format(serial_no))
...
def validate_item_details(args, item):
	...
	from erpadda.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(args, item, overwrite_warehouse=True):
	...
	if not item:
		item = vmraid.get_doc("Item", args.get("item_code"))
	...
	if args.get("doctype") == "Material Request" and not args.get("material_request_type"):
		args["material_request_type"] = vmraid.db.get_value(
	...
	if args.get("batch_no") and item.name != vmraid.get_cached_value(
	...
	if args.get("doctype") in purchase_doctypes:
		from erpadda.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
	...
	for d in [
		...
		if not out[d[1]]:
			out[d[1]] = vmraid.get_cached_value("Company", args.company, d[2]) if d[2] else None
	...
	meta = vmraid.get_meta(child_doctype)
...
def get_item_warehouse(item, args, overwrite_warehouse, defaults=None):
	...
	if overwrite_warehouse or not args.warehouse:
		...
		if not warehouse:
			...
			warehouse_exists = vmraid.db.exists(
	...
	if not warehouse:
		default_warehouse = vmraid.db.get_single_value("Stock Settings", "default_warehouse")
		if vmraid.db.get_value("Warehouse", default_warehouse, "company") == args.company:
...
def get_item_tax_info(company, tax_category, item_codes, item_rates=None, item_tax_templates=None):
	...
	for item_code in item_codes:
		...
		out[item_code[1]] = {}
		item = vmraid.get_cached_doc("Item", item_code[0])
		args = {
			"company": company,
...
def get_item_tax_template(args, item, out):
	...
	if not item_tax_template:
		...
		while item_group and not item_tax_template:
			item_group_doc = vmraid.get_cached_doc("Item Group", item_group)
...
def _get_item_tax_template(args, taxes, out=None, for_validate=False):
	...
	for tax in taxes:
		tax_company = vmraid.get_cached_value("Item Tax Template", tax.item_tax_template, "company")
...
def get_item_tax_map(company, item_tax_template, as_json=True):
	...
	if item_tax_template:
		template = vmraid.get_cached_doc("Item Tax Template", item_tax_template)
		for d in template.taxes:
			if vmraid.get_cached_value("Account", d.tax_type, "company") == company:
...
def get_default_deferred_account(args, item, fieldname=None):
	if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
		return (
			...
			or vmraid.get_cached_value("Company", args.company, "default_" + fieldname)
		)
	else:
		return None
...
def get_default_cost_center(args, item=None, item_group=None, brand=None, company=None):
	...
	if args.get("project"):
		cost_center = vmraid.db.get_value("Project", args.get("project"), "cost_center", cache=True)
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = "erpadda.stock.get_item_details.{0}".format(method)
			data = vmraid.get_attr(path)(args.get("item_code"), company)
	...
	if (
		company
		and cost_center
		and vmraid.get_cached_value("Cost Center", cost_center, "company") != company
	):
		return None

	if not cost_center and company:
		cost_center = vmraid.get_cached_value("Company", company, "cost_center")
...
def get_price_list_rate(args, item_doc, out=None):
	...
	meta = vmraid.get_meta(args.parenttype or args.doctype)
	...
	if meta.get_field("currency") or args.get("currency"):
		...
		if not out.price_list_rate and args.transaction_type == "buying":
			from erpadda.stock.doctype.item.item import get_last_purchase_details
...
def insert_item_price(args):
	...
	if vmraid.db.get_value(
		...
		vmraid.db.get_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing")
	):
		if vmraid.has_permission("Item Price", "write"):
			price_list_rate = (
			...
			if item_price and item_price.name:
				if item_price.price_list_rate != price_list_rate and vmraid.db.get_single_value(
					"Stock Settings", "update_existing_price_list_rate"
				):
					vmraid.db.set_value("Item Price", item_price.name, "price_list_rate", price_list_rate)
					vmraid.msgprint(
...
def get_item_price(args, item_code, ignore_party=False):
	...
	:param args: dict (or vmraid._dict) with mandatory fields price_list, uom
...
def check_packing_list(price_list_rate_name, desired_qty, item_code):
	...
	flag = True
	item_price = vmraid.get_doc("Item Price", price_list_rate_name)
...
def validate_conversion_rate(args, meta):
	from erpadda.controllers.accounts_controller import validate_conversion_rate

	company_currency = vmraid.get_cached_value("Company", args.company, "default_currency")
	...
	args.conversion_rate = flt(
		args.conversion_rate,
		get_field_precision(meta.get_field("conversion_rate"), vmraid._dict({"fields": args})),
	)

	if args.price_list:
		if not args.plc_conversion_rate and args.price_list_currency == vmraid.db.get_value(
			"Price List", args.price_list, "currency", cache=True
		):
		...
		if not args.get("price_list_currency"):
			...
			if meta.get_field("plc_conversion_rate"):
				args.plc_conversion_rate = flt(
					args.plc_conversion_rate,
					get_field_precision(meta.get_field("plc_conversion_rate"), vmraid._dict({"fields": args})),
				)
...
def get_party_item_code(args, item_doc, out):
	if args.transaction_type == "selling" and args.customer:
		...
		if customer_item_code:
			...
			customer_group = vmraid.get_cached_value("Customer", args.customer, "customer_group")
...
def get_pos_profile_item_details(company, args, pos_profile=None, update_data=False):
	...
	if not vmraid.flags.pos_profile and not pos_profile:
		pos_profile = vmraid.flags.pos_profile = get_pos_profile(company, args.get("pos_profile"))
...
def get_pos_profile(company, pos_profile=None, user=None):
	if pos_profile:
		return vmraid.get_cached_doc("POS Profile", pos_profile)
...
def get_serial_nos_by_fifo(args, sales_order=None):
	if vmraid.db.get_single_value("Stock Settings", "automatically_set_serial_nos_based_on_fifo"):
		return "\n".join(
			vmraid.db.sql_list(
...
def get_conversion_factor(item_code, uom):
	variant_of = vmraid.db.get_value("Item", item_code, "variant_of", cache=True)
	filters = {"parent": item_code, "uom": uom}
	if variant_of:
		filters["parent"] = ("in", (item_code, variant_of))
	conversion_factor = vmraid.db.get_value("UOM Conversion Detail", filters, "conversion_factor")
	if not conversion_factor:
		stock_uom = vmraid.db.get_value("Item", item_code, "stock_uom")
...
def get_projected_qty(item_code, warehouse):
	return {
		"projected_qty": vmraid.db.get_value(
...
def get_bin_details(item_code, warehouse, company=None):
	bin_details = vmraid.db.get_value(
		"Bin",
...
def get_bin_details_and_serial_nos(
	...
	if flt(stock_qty) > 0:
		if has_batch_no:
			args = vmraid._dict({"item_code": item_code, "warehouse": warehouse, "stock_qty": stock_qty})
			serial_no = get_serial_no(args)
...
def get_batch_qty(batch_no, warehouse, item_code):
	from erpadda.stock.doctype.batch import batch
...
def apply_price_list(args, as_doc=False):
	...
	if "items" in args:
		...
		for item in item_list:
			args_copy = vmraid._dict(args.copy())
...
def apply_price_list_on_item(args):
	item_doc = vmraid.db.get_value("Item", args.item_code, ["name", "variant_of"], as_dict=1)
...
def get_serial_no(args, serial_nos=None, sales_order=None):
	...
	if args.get("warehouse") and args.get("stock_qty") and args.get("item_code"):
		has_serial_no = vmraid.get_value("Item", {"item_code": args.item_code}, "has_serial_no")
...
def get_blanket_order_details(args):
	...
	if args.item_code:
		...
		blanket_order_details = vmraid.db.sql(
			"""
