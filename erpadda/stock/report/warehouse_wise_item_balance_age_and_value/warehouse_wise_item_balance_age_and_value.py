...
from erpadda.stock.report.stock_balance.stock_balance import (
	get_item_details,
	get_item_warehouse_map,
	get_items,
	get_stock_ledger_entries,
)
from erpadda.stock.utils import is_reposting_item_valuation_in_progress
...
def validate_filters(filters):
	if not (filters.get("item_code") or filters.get("warehouse")):
		sle_count = flt(vmraid.db.sql("""select count(name) from `tabStock Ledger Entry`""")[0][0])
		if sle_count > 500000:
			vmraid.throw(_("Please set filter based on Item or Warehouse"))
	if not filters.get("company"):
		filters["company"] = vmraid.defaults.get_user_default("Company")
