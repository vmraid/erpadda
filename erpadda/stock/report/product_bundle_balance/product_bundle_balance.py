...
from erpadda.stock.report.stock_ledger.stock_ledger import get_item_group_condition
...
def execute(filters=None):
	...
	for parent_item in parent_items:
		...
		for child_item in required_items:
			child_item_balance = stock_balance.get(child_item.item_code, vmraid._dict())
		...
		for warehouse, company in warehouse_company_map.items():
			...
			for child_item_detail in required_items:
				child_item_balance = stock_balance.get(child_item_detail.item_code, vmraid._dict()).get(
					warehouse, vmraid._dict()
				)
				child_row = {
					"indent": 1,
...
def get_items(filters):
	...
	if parent_items:
		child_item_details = vmraid.db.sql(
			"""
			select
...
def get_stock_balance(filters, items):
	...
	stock_balance = vmraid._dict()
	for d in sle:
		stock_balance.setdefault(d.item_code, vmraid._dict())[d.warehouse] = d
...
def get_stock_ledger_entries(filters, items):
	...
	item_conditions_sql = " and sle.item_code in ({})".format(
		", ".join(vmraid.db.escape(i) for i in items)
	)
...
def get_sle_conditions(filters):
	...
	if not filters.get("date"):
		vmraid.throw(_("'Date' is required"))

	conditions += " and sle.posting_date <= %s" % vmraid.db.escape(filters.get("date"))

	if filters.get("warehouse"):
		warehouse_details = vmraid.db.get_value(
