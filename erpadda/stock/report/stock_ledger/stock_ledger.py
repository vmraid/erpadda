...
from erpadda.stock.doctype.stock_reconciliation.stock_reconciliation import get_stock_balance_for
...
def execute(filters=None):
	...
	precision = cint(vmraid.db.get_single_value("System Settings", "float_precision"))
...
def get_opening_balance(filters, columns, sl_entries):
	...
	for sle in sl_entries:
		if (
			...
			and vmraid.db.get_value("Stock Reconciliation", sle.voucher_no, "purpose") == "Opening Stock"
		):
			last_entry = sle
...
def get_warehouse_condition(warehouse):
	warehouse_details = vmraid.db.get_value("Warehouse", warehouse, ["lft", "rgt"], as_dict=1)
...
def get_item_group_condition(item_group):
	item_group_details = vmraid.db.get_value("Item Group", item_group, ["lft", "rgt"], as_dict=1)
