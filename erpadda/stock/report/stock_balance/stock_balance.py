...
from vmraid.query_builder.functions import CombineDatetime
from vmraid.utils import cint, date_diff, flt, getdate
from vmraid.utils.nestedset import get_descendants_of
from pypika.terms import ExistsCriterion

import erpadda
from erpadda.stock.report.stock_ageing.stock_ageing import FIFOSlots, get_average_age
from erpadda.stock.utils import add_additional_uom_columns, is_reposting_item_valuation_in_progress
...
def execute(filters: Optional[StockBalanceFilter] = None):
	...
	if filters.get("company"):
		...
		company_currency = vmraid.db.get_single_value("Global Defaults", "default_currency")
...
def apply_conditions(query, filters):
	...
	warehouse_table = vmraid.qb.DocType("Warehouse")

	if not filters.get("from_date"):
		vmraid.throw(_("'From Date' is required"))

	if to_date := filters.get("to_date"):
		query = query.where(sle.posting_date <= to_date)
	else:
		vmraid.throw(_("'To Date' is required"))
	...
	if warehouse := filters.get("warehouse"):
		...
		chilren_subquery = (
			vmraid.qb.from_(warehouse_table)
...
def get_item_warehouse_map(filters: StockBalanceFilter, sle: List[SLEntry]):
	...
	float_precision = cint(vmraid.db.get_default("float_precision")) or 3

	for d in sle:
		key = (d.company, d.item_code, d.warehouse)
		if key not in iwb_map:
			iwb_map[key] = vmraid._dict(
				{
					"opening_qty": 0.0,
					"opening_val": 0.0,
					"in_qty": 0.0,
		...
		if d.posting_date < from_date or (
			...
			and vmraid.db.get_value("Stock Reconciliation", d.voucher_no, "purpose") == "Opening Stock"
		):
...
def get_items(filters: StockBalanceFilter) -> List[str]:
	...
	if item_code := filters.get("item_code"):
		...
		return vmraid.get_all("Item", filters=item_filters, pluck="name", order_by=None)
...
def get_item_details(items: List[str], sle: List[SLEntry], filters: StockBalanceFilter):
	...
	item_table = vmraid.qb.DocType("Item")

	query = (
		vmraid.qb.from_(item_table)
		.select(
			item_table.name,
	...
	if uom := filters.get("include_uom"):
		uom_conv_detail = vmraid.qb.DocType("UOM Conversion Detail")
		query = (
...
def get_item_reorder_details(items):
	item_reorder_details = vmraid._dict()

	if items:
		item_reorder_details = vmraid.get_all(
			"Item Reorder",
...
def get_variants_attributes() -> List[str]:
	...
	return vmraid.get_all("Item Attribute", pluck="name")
