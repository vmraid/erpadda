...
from vmraid.utils import cint, date_diff, flt
...
def format_report_data(filters: Filters, item_details: Dict, to_date: str) -> List[Dict]:
	...
	precision = cint(vmraid.db.get_single_value("System Settings", "float_precision", cache=True))
...
class FIFOSlots:
		...
		for key, row in wh_wise_data.items():
			...
			if not item_aggregated_data.get(item):
				item_aggregated_data.setdefault(
					item,
					{"details": vmraid._dict(), "fifo_queue": [], "qty_after_transaction": 0.0, "total_qty": 0.0},
				)
	...
	def __get_item_query(self) -> str:
		...
		item = vmraid.qb.from_("Item").select(
	...
	def __get_warehouse_conditions(self, sle, sle_query) -> str:
		warehouse = vmraid.qb.DocType("Warehouse")
		lft, rgt = vmraid.db.get_value("Warehouse", self.filters.get("warehouse"), ["lft", "rgt"])

		warehouse_results = (
			vmraid.qb.from_(warehouse)
			.select("name")
