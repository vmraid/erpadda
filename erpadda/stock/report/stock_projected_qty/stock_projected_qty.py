...
from erpadda.accounts.doctype.pos_invoice.pos_invoice import get_pos_reserved_qty
...
def execute(filters=None):
	...
	for bin in bin_list:
		...
		company = warehouse_company.setdefault(
			bin.warehouse, vmraid.db.get_value("Warehouse", bin.warehouse, "company")
		)
...
def get_item_map(item_code, include_uom):
	...
	condition = ""
	if item_code:
		condition = "and item_code = {0}".format(vmraid.db.escape(item_code, percent=False))
	...
	condition = ""
	if item_code:
		condition = "where parent={0}".format(vmraid.db.escape(item_code, percent=False))

	reorder_levels = vmraid._dict()
	for ir in vmraid.db.sql(
