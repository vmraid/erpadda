...
class DelayedItemReport(object):
	...
	def get_data(self, consolidated=False):
		...
		if self.filters.get("item_group"):
			conditions += " and `tab%s`.item_group = %s" % (
				child_doc,
				vmraid.db.escape(self.filters.get("item_group")),
			)
		...
		for field in ["customer", "customer_group", "company"]:
			if self.filters.get(field):
				conditions += " and `tab%s`.%s = %s" % (
					doctype,
					field,
					vmraid.db.escape(self.filters.get(field)),
				)
		...
		self.transactions = vmraid.db.sql(
	...
	def filter_transactions_data(self, consolidated=False):
		...
		so_data = {}
		for d in vmraid.get_all(doctype, filters=filters, fields=["delivery_date", "parent", "name"]):
