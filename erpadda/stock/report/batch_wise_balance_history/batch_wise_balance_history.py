...
def get_conditions(filters):
	...
	for field in ["item_code", "warehouse", "batch_no", "company"]:
		if filters.get(field):
			conditions += " and {0} = {1}".format(field, vmraid.db.escape(filters.get(field)))
...
def get_item_warehouse_batch_map(filters, float_precision):
	...
	for d in sle:
		iwb_map.setdefault(d.item_code, {}).setdefault(d.warehouse, {}).setdefault(
			d.batch_no, vmraid._dict({"opening_qty": 0.0, "in_qty": 0.0, "out_qty": 0.0, "bal_qty": 0.0})
		)
...
def get_item_details(filters):
	item_map = {}
	for d in vmraid.db.sql("select name, item_name, description, stock_uom from tabItem", as_dict=1):
