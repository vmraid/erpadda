...
def get_incorrect_serial_nos(serial_nos_data):
	...
	total_value = vmraid._dict(
		{"qty": 0, "valuation_rate": 0, "serial_no": vmraid.bold(_("Balance"))}
	)

	for serial_no, data in serial_nos_data.items():
		total_dict = vmraid._dict({"qty": 0, "valuation_rate": 0, "serial_no": vmraid.bold(_("Total"))})
