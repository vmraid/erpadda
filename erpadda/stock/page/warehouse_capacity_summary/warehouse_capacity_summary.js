vmraid.pages['warehouse-capacity-summary'].on_page_load = function(wrapper) {
	...
	page.sort_selector = new vmraid.ui.SortSelector({
	...
	vmraid.require('item-dashboard.bundle.js', function() {
		$(vmraid.render_template('warehouse_capacity_summary_header')).appendTo(page.main);

		page.capacity_dashboard = new erpadda.stock.ItemDashboard({
			page_name: "warehouse-capacity-summary",
			page_length: 10,
			parent: page.main,
			sort_by: 'stock_capacity',
			sort_order: 'desc',
			method: 'erpadda.stock.dashboard.warehouse_capacity_dashboard.get_data',
		...
		let setup_click = function(doctype) {
			page.main.on('click', 'a[data-type="'+ doctype.toLowerCase() +'"]', function() {
				...
				if (field.get_value()===name) {
					vmraid.set_route('Form', doctype, name);
				} else {
