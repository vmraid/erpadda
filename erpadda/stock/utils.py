...
from vmraid.utils import cstr, flt, get_link_to_form, nowdate, nowtime

import erpadda
from erpadda.stock.valuation import FIFOValuation, LIFOValuation


class InvalidWarehouseCompany(vmraid.ValidationError):
	pass


class PendingRepostingError(vmraid.ValidationError):
	pass
...
def get_stock_value_from_bin(warehouse=None, item_code=None):
	...
	stock_value = vmraid.db.sql(query, values)
...
def get_stock_value_on(warehouse=None, posting_date=None, item_code=None):
	...
	if warehouse:
		...
		lft, rgt, is_group = vmraid.db.get_value("Warehouse", warehouse, ["lft", "rgt", "is_group"])
...
def get_bin(item_code, warehouse):
	bin = vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
	if not bin:
		bin_obj = _create_bin(item_code, warehouse)
	else:
		bin_obj = vmraid.get_doc("Bin", bin, for_update=True)
	bin_obj.flags.ignore_permissions = True
	return bin_obj


def get_or_make_bin(item_code: str, warehouse: str) -> str:
	bin_record = vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
...
def _create_bin(item_code, warehouse):
	...
	try:
		vmraid.db.savepoint(bin_creation_savepoint)
		bin_obj = vmraid.get_doc(doctype="Bin", item_code=item_code, warehouse=warehouse)
		bin_obj.flags.ignore_permissions = 1
		bin_obj.insert()
	except vmraid.UniqueValidationError:
		vmraid.db.rollback(save_point=bin_creation_savepoint)  # preserve transaction in postgres
		bin_obj = vmraid.get_last_doc("Bin", {"item_code": item_code, "warehouse": warehouse})
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
	...
	from erpadda.stock.stock_ledger import (
	...
	elif args.get("batch_no") and vmraid.db.get_value(
	...
	if in_rate is None:
		in_rate = get_valuation_rate(
			...
			currency=erpadda.get_company_currency(args.get("company")),
...
def get_valuation_method(item_code):
	...
	val_method = vmraid.db.get_value("Item", item_code, "valuation_method", cache=True)
	if not val_method:
		val_method = (
			vmraid.db.get_value("Stock Settings", None, "valuation_method", cache=True) or "FIFO"
		)
	return val_method
	...
	for val in serial_nos:
		if val:
			...
			if val in valid_serial_nos:
				vmraid.throw(_("Serial number {0} entered more than once").format(val))
			else:
				valid_serial_nos.append(val)

	if qty and len(valid_serial_nos) != abs(qty):
		vmraid.throw(_("{0} valid serial nos for Item {1}").format(abs(qty), item_code))

	return valid_serial_nos


def validate_warehouse_company(warehouse, company):
	warehouse_company = vmraid.db.get_value("Warehouse", warehouse, "company", cache=True)
...
def is_group_warehouse(warehouse):
	if vmraid.db.get_value("Warehouse", warehouse, "is_group", cache=True):
		vmraid.throw(_("Group node warehouse is not allowed to select for transactions"))


def validate_disabled_warehouse(warehouse):
	if vmraid.db.get_value("Warehouse", warehouse, "disabled", cache=True):
		vmraid.throw(
		...
		if d.get("convertible"):
			...
			columns.insert(
				idx + 1,
				{
					...
					"fieldname": "{0}_{1}".format(d.get("fieldname"), vmraid.scrub(include_uom)),
	...
	for row_idx, row in enumerate(result):
		...
		for key, value in data:
			...
			if not is_dict_obj:
				...
				new_key = "{0}_{1}".format(key, vmraid.scrub(include_uom))
...
def add_additional_uom_columns(columns, result, include_uom, conversion_factors):
	...
	for col_idx in list(reversed(range(0, len(columns)))):
		...
		if isinstance(col, dict) and col.get("convertible") in ["rate", "qty"]:
			...
			convertible_column_map[col.get("fieldname")] = vmraid._dict(
...
def is_reposting_item_valuation_in_progress():
	reposting_in_progress = vmraid.db.exists(
...
def check_pending_reposting(posting_date: str, throw_error: bool = True) -> bool:
	...
	reposting_pending = vmraid.db.exists("Repost Item Valuation", filters)
	if reposting_pending and throw_error:
		...
		vmraid.msgprint(
			...
			primary_action={
				...
				"client_action": "erpadda.route_to_pending_reposts",
				"args": filters,
			},
		)
...
def scan_barcode(search_value: str) -> Dict[str, Optional[str]]:
	...
	# search barcode no
	barcode_data = vmraid.db.get_value(
		"Item Barcode",
	...
	# search serial no
	serial_no_data = vmraid.db.get_value(
		"Serial No",
		search_value,
	...
	# search batch no
	batch_no_data = vmraid.db.get_value(
		"Batch",
		search_value,
...
def _update_item_info(scan_result: Dict[str, Optional[str]]) -> Dict[str, Optional[str]]:
	if item_code := scan_result.get("item_code"):
		if item_info := vmraid.get_cached_value(
			"Item",
			item_code,
