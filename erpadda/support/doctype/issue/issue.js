vmraid.ui.form.on("Issue", {
	onload: function(frm) {
		...
		vmraid.db.get_value("Support Settings", {name: "Support Settings"},
		...
		if (frm.doc.status !== "Closed") {
			...
			frm.add_custom_button(__("Task"), function() {
				vmraid.model.open_mapped_doc({
					method: "erpadda.support.doctype.issue.issue.make_task",
					frm: frm
				});
			}, __("Create"));
	...
	reset_service_level_agreement: function(frm) {
		let reset_sla = new vmraid.ui.Dialog({
			...
			primary_action: (values) => {
				...
				vmraid.call("erpadda.support.doctype.service_level_agreement.service_level_agreement.reset_service_level_agreement", {
					reason: values.reason,
					user: vmraid.session.user_email
				}, () => {
					reset_sla.enable_primary_action();
					frm.refresh();
					vmraid.msgprint(__("Service Level Agreement was reset."));
				});
			}
		});
	...
	timeline_refresh: function(frm) {
					...
					var dialog = new vmraid.ui.Dialog({
						...
						primary_action: () => {
							frm.call("split_issue", {
								...
								vmraid.msgprint(`New issue created: <a href="/app/issue/${r.message}">${r.message}</a>`);
								frm.reload_doc();
								dialog.hide();
							});
						}
		...
		// if (vmraid.model.can_create("Help Article")) {
		...
		// 			${vmraid.utils.icon('solid-info', 'sm')}
		// 		</a>
		// 	`);
		...
		// 			const doc = vmraid.model.get_new_doc("Help Article");
		// 			doc.title = frm.doc.subject;
		// 			doc.content = content;
		// 			vmraid.set_route("Form", "Help Article", doc.name);
		// 		});
		// 	}
