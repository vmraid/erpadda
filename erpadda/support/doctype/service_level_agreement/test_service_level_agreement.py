...
from erpadda.support.doctype.issue_priority.test_issue_priority import make_priorities
from erpadda.support.doctype.service_level_agreement.service_level_agreement import (
...
class TestServiceLevelAgreement(unittest.TestCase):
	def setUp(self):
		...
		vmraid.db.sql("delete from `tabLead`")
	...
	def test_custom_field_creation_for_sla_on_standard_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				vmraid.db.exists("Custom Field", {"dt": doctype, "fieldname": field.get("fieldname")})
			)
	...
	def test_docfield_creation_for_sla_on_custom_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				vmraid.db.exists("DocField", {"fieldname": field.get("fieldname"), "parent": doctype.name})
			)
	...
	def test_sla_application(self):
		...
		vmraid.flags.current_time = datetime.datetime(2019, 3, 4, 15, 0)
		lead.reload()
	...
	def test_hold_time(self):
		...
		vmraid.flags.current_time = datetime.datetime(2020, 3, 4, 4, 15)
		lead.reload()
		lead.status = "Replied"
		lead.save()

		lead.reload()
		self.assertEqual(lead.on_hold_since, vmraid.flags.current_time)

		vmraid.flags.current_time = datetime.datetime(2020, 3, 4, 5, 5)
		lead.reload()
		lead.status = "Converted"
		lead.save()
	...
	def test_failed_sla_for_response_only(self):
		...
		vmraid.flags.current_time = datetime.datetime(2019, 3, 4, 16, 5)
		lead.reload()
		lead.status = "Replied"
		lead.save()
	...
	def test_fulfilled_sla_for_response_only(self):
		...
		vmraid.flags.current_time = datetime.datetime(2019, 3, 4, 15, 30)
		lead.reload()
		lead.status = "Replied"
		lead.save()
	...
	def test_service_level_agreement_filters(self):
		...
		applied_sla = vmraid.db.get_value("Lead", lead.name, "service_level_agreement")
		self.assertFalse(applied_sla)

		source = vmraid.get_doc(doctype="Lead Source", source_name="Test Source")
	...
	def tearDown(self):
		for d in vmraid.get_all("Service Level Agreement"):
			vmraid.delete_doc("Service Level Agreement", d.name, force=1)
...
def get_service_level_agreement(
	...
	service_level_agreement = vmraid.get_doc("Service Level Agreement", filters)
...
def create_service_level_agreement(
	...
	service_level_agreement = vmraid._dict(
		{
			...
			"entity": entity,
			"start_date": vmraid.utils.getdate(),
			"end_date": vmraid.utils.add_to_date(vmraid.utils.getdate(), days=100),
	...
	sla = vmraid.db.exists("Service Level Agreement", filters)
	if sla:
		vmraid.delete_doc("Service Level Agreement", sla, force=1)

	return vmraid.get_doc(service_level_agreement).insert(
...
def create_customer():
	...
	if not vmraid.db.exists("Customer", "_Test Customer"):
		customer.insert(ignore_permissions=True)
		return customer.name
	else:
		return vmraid.db.exists("Customer", "_Test Customer")


def create_customer_group():
	customer_group = vmraid.get_doc(
		{"doctype": "Customer Group", "customer_group_name": "_Test SLA Customer Group"}
	)

	if not vmraid.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"}):
		customer_group.insert()
		return customer_group.name
	else:
		return vmraid.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"})


def create_territory():
	territory = vmraid.get_doc(
		{
			"doctype": "Territory",
	...
	if not vmraid.db.exists("Territory", {"territory_name": "_Test SLA Territory"}):
		territory.insert()
		return territory.name
	else:
		return vmraid.db.exists("Territory", {"territory_name": "_Test SLA Territory"})
...
def make_holiday_list():
	holiday_list = vmraid.db.exists("Holiday List", "__Test Holiday List")
	if not holiday_list:
...
def create_custom_doctype():
	if not vmraid.db.exists("DocType", "Test SLA on Custom Dt"):
		doc = vmraid.get_doc(
			{
				"doctype": "DocType",
				"module": "Support",
		...
		return vmraid.get_doc("DocType", "Test SLA on Custom Dt")
