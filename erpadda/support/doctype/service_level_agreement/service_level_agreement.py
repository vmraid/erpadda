...
from vmraid.utils.nestedset import get_ancestors_of
...
class ServiceLevelAgreement(Document):
	...
	def check_support_and_resolution(self):
		...
		if not len(set(support_days)) == len(support_days):
			...
			vmraid.throw(_("Workday {0} has been repeated.").format(repeated_days))

	def validate_doc(self):
		if (
			self.enabled
			and self.document_type == "Issue"
			and not vmraid.db.get_single_value("Support Settings", "track_service_level_agreement")
		):
			vmraid.throw(
				_("{0} is not enabled in {1}").format(
					vmraid.bold("Track Service Level Agreement"),
					get_link_to_form("Support Settings", "Support Settings"),
				)
			)

		if self.default_service_level_agreement and vmraid.db.exists(
		...
		if (
			...
			vmraid.throw(
				_("Service Level Agreement for {0} {1} already exists.").format(
					vmraid.bold(self.entity_type), vmraid.bold(self.entity)
				)
			)

	def validate_selected_doctype(self):
		invalid_doctypes = list(vmraid.model.core_doctypes_list)
		invalid_doctypes.extend(["Cost Center", "Company"])
		valid_document_types = vmraid.get_all(
			"DocType",
			{
				"issingle": 0,
				"istable": 0,
		...
		if self.document_type not in valid_document_types:
			vmraid.throw(msg=_("Please select valid document type."), title=_("Invalid Document Type"))
	...
	def validate_status_field(self):
		...
		if not meta.get_field("status"):
			vmraid.throw(
				...
				).format(vmraid.bold(self.document_type))
			)
	...
	def validate_condition(self):
		...
		if self.condition:
			try:
				vmraid.safe_eval(self.condition, None, get_context(temp_doc))
			except Exception:
				vmraid.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def get_service_level_agreement_priority(self, priority):
		priority = vmraid.get_doc("Service Level Priority", {"priority": priority, "parent": self.name})
	...
	def before_insert(self):
		...
		meta = vmraid.get_meta(self.document_type, cached=False)
	...
	def create_docfields(self, meta, service_level_agreement_fields):
		...
		vmraid.get_doc("DocType", self.document_type).save(ignore_permissions=True)
	...
	def reset_field_properties(self, field, field_dt, sla_field):
		field = vmraid.get_doc(field_dt, {"fieldname": field.fieldname})
...
def check_agreement_status():
	service_level_agreements = vmraid.get_all(
		"Service Level Agreement",
	...
	for service_level_agreement in service_level_agreements:
		...
		if doc.end_date and getdate(doc.end_date) < getdate(vmraid.utils.getdate()):
			vmraid.db.set_value("Service Level Agreement", service_level_agreement.name, "enabled", 0)


def get_active_service_level_agreement_for(doc):
	if not vmraid.db.get_single_value("Support Settings", "track_service_level_agreement"):
		return
	...
	for agreement in agreements:
		...
		if not condition or (condition and vmraid.safe_eval(condition, None, get_context(doc))):
...
def get_customer_group(customer):
	customer_groups = []
	customer_group = vmraid.db.get_value("Customer", customer, "customer_group") if customer else None
	if customer_group:
...
def get_customer_territory(customer):
	...
	customer_territory = vmraid.db.get_value("Customer", customer, "territory") if customer else None
...
def get_service_level_agreement_filters(doctype, name, customer=None):
	...
	return {
		"priority": [
			priority.priority
			for priority in vmraid.get_all(
				"Service Level Priority", filters={"parent": name}, fields=["priority"]
			)
		],
		"service_level_agreements": [
			d.name
			for d in vmraid.get_all("Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
...
def get_documents_with_active_service_level_agreement():
	sla_doctypes = vmraid.cache().hget("service_level_agreement", "active")
...
def set_documents_with_active_service_level_agreement():
	...
	vmraid.cache().hset("service_level_agreement", "active", active)
	return active
...
def handle_status_change(doc, apply_sla_for_resolution):
	now_time = vmraid.flags.current_time or now_datetime(doc.get("owner"))
	prev_status = vmraid.db.get_value(doc.doctype, doc.name, "status")
...
def get_fulfillment_statuses(service_level_agreement):
	return [
		entry.status
		for entry in vmraid.db.get_all(
...
def get_allotted_seconds(parameter, service_level):
	...
	if parameter == "response":
		...
		vmraid.throw(_("{0} parameter is invalid").format(parameter))
...
def get_support_days(service_level):
	...
	for service in service_level.get("support_and_resolution"):
		support_days[service.workday] = vmraid._dict(
			{
...
def change_service_level_agreement_and_priority(self):
	if (
		...
		and vmraid.db.exists("Issue", self.name)
		and vmraid.db.get_single_value("Support Settings", "track_service_level_agreement")
	):

		if not self.priority == vmraid.db.get_value("Issue", self.name, "priority"):
			self.set_response_and_resolution_time(
				priority=self.priority, service_level_agreement=self.service_level_agreement
			)
			vmraid.msgprint(_("Priority has been changed to {0}.").format(self.priority))

		if not self.service_level_agreement == vmraid.db.get_value(
...
def get_response_and_resolution_duration(doc):
	sla = vmraid.get_doc("Service Level Agreement", doc.service_level_agreement)
...
def reset_service_level_agreement(doc, reason, user):
	if not vmraid.db.get_single_value("Support Settings", "allow_resetting_service_level_agreement"):
		vmraid.throw(_("Allow Resetting Service Level Agreement from Support Settings."))
...
def on_communication_update(doc, status):
	...
	for_resolution = vmraid.db.get_value(
...
def get_tz(user):
	return vmraid.db.get_value("User", user, "time_zone") or get_time_zone()
...
def get_sla_doctypes():
	doctypes = []
	data = vmraid.get_all("Service Level Agreement", {"enabled": 1}, ["document_type"], distinct=1)
