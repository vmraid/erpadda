	...
	def innerfn(fn):
		...
		if vmraid.request and settings and settings.get(secret_key) and not vmraid.flags.in_test:
			sig = base64.b64encode(
				hmac.new(settings.get(secret_key).encode("utf8"), vmraid.request.data, hashlib.sha256).digest()
			)

			if vmraid.request.data and not sig == bytes(vmraid.get_request_header(hmac_key).encode()):
...
def create_mode_of_payment(gateway, payment_type="General"):
	...
	mode_of_payment = vmraid.db.exists("Mode of Payment", gateway)
	if not mode_of_payment and payment_gateway_account:
		...
		return vmraid.get_doc("Mode of Payment", mode_of_payment)
