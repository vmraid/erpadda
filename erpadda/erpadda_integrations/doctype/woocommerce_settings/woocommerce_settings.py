...
class WoocommerceSettings(Document):
	...
	def create_delete_custom_fields(self):
		if self.enable_sync:
			...
			if not vmraid.get_value("Item Group", {"name": _("WooCommerce Products")}):
				item_group = vmraid.new_doc("Item Group")
	...
	def validate_settings(self):
		if self.enable_sync:
			if not self.secret:
				self.set("secret", vmraid.generate_hash())

			if not self.woocommerce_server_url:
				vmraid.throw(_("Please enter Woocommerce Server URL"))

			if not self.api_consumer_key:
				vmraid.throw(_("Please enter API Consumer Key"))

			if not self.api_consumer_secret:
				vmraid.throw(_("Please enter API Consumer Secret"))

	def create_webhook_url(self):
		endpoint = "/api/method/erpadda.erpadda_integrations.connectors.woocommerce_connection.order"
...
def generate_secret():
	...
	woocommerce_settings.secret = vmraid.generate_hash()
...
def get_series():
	return {
		"sales_order_series": vmraid.get_meta("Sales Order").get_options("naming_series") or "SO-WOO-",
	}
