...
def callback(*args, **kwargs):
	migrator = vmraid.get_doc("QuickBooks Migrator")
	...
	vmraid.respond_as_web_page("Quickbooks Authentication", html="<script>window.close()</script>")
...
class QuickBooksMigrator(Document):
	...
	def on_update(self):
		if self.company:
			# We need a Cost Center corresponding to the selected erpadda Company
			self.default_cost_center = vmraid.db.get_value("Company", self.company, "cost_center")
			company_warehouses = vmraid.get_all(
	...
	@vmraid.whitelist()
	def migrate(self):
		vmraid.enqueue_doc("QuickBooks Migrator", "QuickBooks Migrator", "_migrate", queue="long")
	...
	def _migrate(self):
		try:
			...
			# QuickBooks data can have transactions that do not fall in existing fiscal years in ERPAdda
	...
	def _make_custom_quickbooks_id_field(self, doctype):
		if not vmraid.get_meta(doctype).has_field("quickbooks_id"):
			vmraid.get_doc(
				{
	...
	def _make_custom_company_field(self, doctype):
		if not vmraid.get_meta(doctype).has_field("company"):
			vmraid.get_doc(
				{
					"doctype": "Custom Field",
					"label": "Company",
	...
	def _create_fiscal_years(self):
		try:
			...
			oldest_fiscal_year = vmraid.get_all(
			...
			while smallest_ledger_entry_date < oldest_fiscal_year.year_start_date:
				new_fiscal_year = vmraid.get_doc({"doctype": "Fiscal Year"})
				...
				entries.append(
					{
						...
						"type": data[1]["value"],
						"id": data[1].get("id"),
						"credit": vmraid.utils.flt(data[2]["value"]),
						"debit": vmraid.utils.flt(data[3]["value"]),
					}
				)
			if row["type"] == "Section":
	...
	def _save_account(self, account):
		...
		# Map Quickbooks Account Types to ERPAdda root_accunts and and root_type
		try:
			if not vmraid.db.exists(
	...
	def _save_customer(self, customer):
		try:
			if not vmraid.db.exists(
				...
				try:
					receivable_account = vmraid.get_all(
						"Account",
						filters={
		...
		try:
			if not vmraid.db.exists(
				...
				if item["Type"] in ("Service", "Inventory"):
					...
					vmraid.get_doc(item_dict).insert()
		except Exception as e:
			self._log_error(e, item)

	def _allow_fraction_in_unit(self):
		vmraid.db.set_value("UOM", "Unit", "must_be_whole_number", 0)
	...
	def _save_sales_invoice(self, invoice, quickbooks_id, is_return=False, is_pos=False):
		try:
			if not vmraid.db.exists(
				...
				invoice_doc = vmraid.get_doc(invoice_dict)
				invoice_doc.insert()
				invoice_doc.submit()
		except Exception as e:
	...
	def _save_invoice_as_journal_entry(self, invoice, quickbooks_id):
		try:
			...
			for line in self.general_ledger["Invoice"][invoice["Id"]]["lines"]:
				...
				if vmraid.db.get_value("Account", line["account"], "account_type") == "Receivable":
					account_line["party_type"] = "Customer"
					account_line["party"] = vmraid.get_all(
						"Customer",
			...
			for line in bill_payment["Line"]:
				...
				if linked_transaction["TxnType"] == "Bill":
					...
					if vmraid.db.exists(
						...
						purchase_invoice = vmraid.get_all(
							"Purchase Invoice",
							filters={
	...
	def _create_address(self, entity, doctype, address, address_type):
		try:
			if not vmraid.db.exists({"doctype": "Address", "quickbooks_id": address["Id"]}):
				vmraid.get_doc(
					{
						"doctype": "Address",
	...
	def _publish(self, *args, **kwargs):
		vmraid.publish_realtime("quickbooks_progress_update", *args, **kwargs)
