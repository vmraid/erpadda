...
from vmraid.utils import call_hook_method, fmt_money, get_request_site_address

from erpadda.erpadda_integrations.doctype.mpesa_settings.mpesa_connector import MpesaConnector
from erpadda.erpadda_integrations.doctype.mpesa_settings.mpesa_custom_fields import (
	create_custom_pos_fields,
)
from erpadda.erpadda_integrations.utils import create_mode_of_payment
...
class MpesaSettings(Document):
	...
	def request_for_payment(self, **kwargs):
		...
		for i, amount in enumerate(request_amounts):
			...
			if vmraid.flags.in_test:
				from erpadda.erpadda_integrations.doctype.mpesa_settings.test_mpesa_settings import (
					get_payment_request_response_payload,
				)

				response = vmraid._dict(get_payment_request_response_payload(amount))
			else:
				response = vmraid._dict(generate_stk_push(**args))
	...
	def split_request_amount_according_to_transaction_limit(self, args):
		...
		if request_amount > self.transaction_limit:
			...
			request_amounts = []
			requests_to_be_made = vmraid.utils.ceil(
	...
	def get_account_balance_info(self):
		...
		if vmraid.flags.in_test:
			...
			response = vmraid._dict(get_test_account_balance_response())
		else:
			response = vmraid._dict(get_account_balance(payload))
	...
	def handle_api_response(self, global_id, request_dict, response):
		...
		if not vmraid.db.exists("Integration Request", req_name):
			create_request_log(request_dict, "Host", "Mpesa", req_name, error)

		if error:
			vmraid.throw(_(getattr(response, "errorMessage")), title=_("Transaction Error"))
...
def generate_stk_push(**kwargs):
	...
	try:
		callback_url = (
			...
			+ "/api/method/erpadda.erpadda_integrations.doctype.mpesa_settings.mpesa_settings.verify_transaction"
		)

		mpesa_settings = vmraid.get_doc("Mpesa Settings", args.payment_gateway[6:])
		...
		vmraid.log_error(title=_("Mpesa Express Transaction Error"))
		vmraid.throw(
...
def verify_transaction(**kwargs):
	...
	transaction_response = vmraid._dict(kwargs["Body"]["stkCallback"])

	checkout_id = getattr(transaction_response, "CheckoutRequestID", "")
	if not isinstance(checkout_id, str):
		vmraid.throw(_("Invalid Checkout Request ID"))

	integration_request = vmraid.get_doc("Integration Request", checkout_id)
	transaction_data = vmraid._dict(loads(integration_request.data))
	...
	if transaction_response["ResultCode"] == 0:
		if integration_request.reference_doctype and integration_request.reference_docname:
			try:
				...
				vmraid.db.set_value("POS Invoice", pr.reference_name, "mpesa_receipt_number", mpesa_receipts)
...
def get_completed_integration_requests_info(reference_doctype, reference_docname, checkout_id):
	output_of_other_completed_requests = vmraid.get_all(
		"Integration Request",
		filters={
			"name": ["!=", checkout_id],
...
def get_account_balance(request_payload):
	...
	try:
		mpesa_settings = vmraid.get_doc("Mpesa Settings", request_payload.get("reference_docname"))
		env = "production" if not mpesa_settings.sandbox else "sandbox"
		connector = MpesaConnector(
			env=env,
			app_key=mpesa_settings.consumer_key,
		...
		callback_url = (
			get_request_site_address(True)
			+ "/api/method/erpadda.erpadda_integrations.doctype.mpesa_settings.mpesa_settings.process_balance_info"
		)
		...
		vmraid.log_error(title=_("Account Balance Processing Error"))
		vmraid.throw(_("Please check your configuration and try again"), title=_("Error"))
...
def process_balance_info(**kwargs):
	...
	account_balance_response = vmraid._dict(kwargs["Result"])

	conversation_id = getattr(account_balance_response, "ConversationID", "")
	if not isinstance(conversation_id, str):
		vmraid.throw(_("Invalid Conversation ID"))

	request = vmraid.get_doc("Integration Request", conversation_id)

	if request.status == "Completed":
		return

	transaction_data = vmraid._dict(loads(request.data))
	...
	if account_balance_response["ResultCode"] == 0:
		try:
			...
			ref_doc = vmraid.get_doc(transaction_data.reference_doctype, transaction_data.reference_docname)
