...
vmraid.provide("erpadda.tally_migration");

vmraid.ui.form.on("Tally Migration", {
	onload: function (frm) {
		let reload_status = true;
		vmraid.realtime.on("tally_migration_progress_update", function (data) {
			if (reload_status) {
				vmraid.model.with_doc(frm.doc.doctype, frm.doc.name, () => {
					frm.refresh_header();
				});
				reload_status = false;
			}
	...
	refresh: function (frm) {
		...
		erpadda.tally_migration.failed_import_log = JSON.parse(frm.doc.failed_import_log);
		erpadda.tally_migration.fixed_errors_log = JSON.parse(frm.doc.fixed_errors_log);
	...
	erpadda_company: function (frm) {
		vmraid.db.exists("Company", frm.doc.erpadda_company).then(exists => {
			if (exists) {
				vmraid.msgprint(
					__("Company {0} already exists. Continuing will overwrite the Company and Chart of Accounts", [frm.doc.erpadda_company]),
				);
			}
		});
	},
	...
	render_html_table(frm, shown_logs, hidden_logs, field) {
		...
		let rows = erpadda.tally_migration.get_html_rows(shown_logs, field);
	...
	show_error_summary(frm) {
		let summary = erpadda.tally_migration.failed_import_log.reduce((summary, row) => {
			if (row.doc) {
	...
	show_errored_import_log(frm) {
		let import_log = erpadda.tally_migration.failed_import_log;
	...
	show_fixed_errors_log(frm) {
		let completed_log = erpadda.tally_migration.fixed_errors_log;
...
erpadda.tally_migration.getError = (traceback) => {
...
erpadda.tally_migration.cleanDoc = (obj) => {
	...
	$.each(temp, function(key, value){
		if (value === "" || value === null){
			...
			erpadda.tally_migration.cleanDoc(value);
		} else if ($.isArray(value)) {
			$.each(value, function (k,v) { erpadda.tally_migration.cleanDoc(v); });
		}
	});
	return temp;
}

erpadda.tally_migration.unresolve = (document) => {
	/* Mark document migration as unresolved ie. move to failed error log */
	let frm = cur_frm;
	let failed_log = erpadda.tally_migration.failed_import_log;
	let fixed_log = erpadda.tally_migration.fixed_errors_log;

	let modified_fixed_log = fixed_log.filter(row => {
		if (!vmraid.utils.deep_equal(erpadda.tally_migration.cleanDoc(row.doc), document)) {
			return row
		}
	});
...
erpadda.tally_migration.resolve = (document) => {
...
erpadda.tally_migration.create_new_doc = (document) => {
	/* Mark as resolved and create new document */
	erpadda.tally_migration.resolve(document);
	return vmraid.call({
		type: "POST",
		method: 'erpadda.erpadda_integrations.doctype.tally_migration.tally_migration.new_doc',
		args: {
			document
		},
		freeze: true,
		callback: function(r) {
			if(!r.exc) {
				...
				vmraid.get_doc(r.message.doctype, r.message.name).__run_link_triggers = true;
...
erpadda.tally_migration.get_html_rows = (logs, field) => {
	let index = 0;
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let error_message = erpadda.tally_migration.getError(traceback);
			index++;
			...
			let show_doc = `
				...
				<div class="collapse margin-top" id="${id}-doc">
					<div class="well">
						<pre style="font-size: smaller;">${JSON.stringify(erpadda.tally_migration.cleanDoc(doc), null, 1)}</pre>
					</div>
				</div>`;

			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='erpadda.tally_migration.create_new_doc(${JSON.stringify(doc)})'>
					${__("Create Document")}
				</button>`

			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='erpadda.tally_migration.unresolve(${JSON.stringify(doc)})'>
