...
from erpadda import get_default_company, get_region
...
def get_client():
	taxjar_settings = vmraid.get_single("TaxJar Settings")
...
def create_transaction(doc, method):
	TAXJAR_CREATE_TRANSACTIONS = vmraid.db.get_single_value(
	...
	TAX_ACCOUNT_HEAD = vmraid.db.get_single_value("TaxJar Settings", "tax_account_head")
	...
	tax_dict["transaction_date"] = vmraid.utils.today()
	...
	try:
		...
		vmraid.throw(_(sanitize_error_response(err)))
...
def get_tax_data(doc):
	SHIP_ACCOUNT_HEAD = vmraid.db.get_single_value("TaxJar Settings", "shipping_account_head")

	from_address = get_company_address_details(doc)
	from_shipping_state = from_address.get("state")
	from_country_code = vmraid.db.get_value("Country", from_address.country, "code")
	...
	to_country_code = vmraid.db.get_value("Country", to_address.country, "code")
...
def get_state_code(address, location):
	if address is not None:
		...
		if state_code not in SUPPORTED_STATE_CODES:
			vmraid.throw(_("Please enter a valid State in the {0} Address").format(location))
	else:
		vmraid.throw(_("Please enter a valid State in the {0} Address").format(location))
...
def set_sales_tax(doc, method):
	TAX_ACCOUNT_HEAD = vmraid.db.get_single_value("TaxJar Settings", "tax_account_head")
	TAXJAR_CALCULATE_TAX = vmraid.db.get_single_value("TaxJar Settings", "taxjar_calculate_tax")
...
def check_for_nexus(doc, tax_dict):
	TAX_ACCOUNT_HEAD = vmraid.db.get_single_value("TaxJar Settings", "tax_account_head")
	if not vmraid.db.get_value("TaxJar Nexus", {"region_code": tax_dict["to_state"]}):
...
def check_sales_tax_exemption(doc):
	...
	sales_tax_exempted = (
		...
		or vmraid.db.has_column("Customer", "exempt_from_sales_tax")
		and vmraid.db.get_value("Customer", doc.customer, "exempt_from_sales_tax")
	)
...
def get_company_address_details(doc):
	...
	if not company_address:
		vmraid.throw(_("Please set a default company address"))

	company_address = vmraid.get_doc("Address", company_address)
...
def get_shipping_address_details(doc):
	...
	if doc.shipping_address_name:
		...
		shipping_address = vmraid.get_doc("Address", doc.customer_address)
	else:
...
def get_iso_3166_2_state_code(address):
	...
	country_code = vmraid.db.get_value("Country", address.get("country"), "code")
