...
import vmraid
from vmraid.core.doctype.report.report import get_report_module_dotted_path
...
def execute_script_report(
	...
	test_filters = []
	report_execute_fn = vmraid.get_attr(
		get_report_module_dotted_path(module, report_name) + ".execute"
	)
	report_filters = vmraid._dict(default_filters).copy().update(filters)
