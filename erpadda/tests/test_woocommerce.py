...
from erpadda.erpadda_integrations.connectors.woocommerce_connection import order


class TestWoocommerce(unittest.TestCase):
	def setUp(self):

		woo_settings = vmraid.get_doc("Woocommerce Settings")
	...
	def test_sales_order_for_woocommerce(self):
		vmraid.flags.woocomm_test_order_data = {
			"id": 75,
			"parent_id": 0,
			"number": "74",
		...
		self.assertTrue(vmraid.get_value("Customer", {"woocommerce_email": "tony@gmail.com"}))
		self.assertTrue(vmraid.get_value("Item", {"woocommerce_id": 56}))
		self.assertTrue(vmraid.get_value("Sales Order", {"woocommerce_id": 75}))
		vmraid.flags.woocomm_test_order_data = {}
...
def emulate_request():
	...
	# Build URL
	port = vmraid.get_site_config().webserver_port or "8000"

	if os.environ.get("CI"):
		host = "localhost"
	else:
		host = vmraid.local.site

	url = "http://{site}:{port}/api/method/erpadda.erpadda_integrations.connectors.woocommerce_connection.order".format(
		site=host, port=port
	)
