...
QUnit.test('Make fixtures', assert => {
	...
	let tasks = [];
	Object.keys(vmraid.test_data).forEach(function(doctype) {
		tasks.push(function() {
			return vmraid.tests.setup_doctype(doctype, vmraid.test_data[doctype]);
		});
	});
	vmraid.run_serially(tasks).then(() => done());
});
