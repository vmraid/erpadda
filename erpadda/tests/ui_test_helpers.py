...
def create_employee_records():
	...
	vmraid.db.sql("DELETE FROM tabEmployee WHERE company='Test Org Chart'")
...
def create_company():
	company = vmraid.db.exists("Company", "Test Org Chart")
	if not company:
		company = (
			vmraid.get_doc(
				{
...
def create_employee(first_name, designation, reports_to=None):
	employee = vmraid.db.exists("Employee", {"first_name": first_name, "designation": designation})
	if not employee:
		employee = (
			vmraid.get_doc(
				{
...
def create_missing_designation():
	if not vmraid.db.exists("Designation", "CTO"):
		vmraid.get_doc({"doctype": "Designation", "designation_name": "CTO"}).insert()
