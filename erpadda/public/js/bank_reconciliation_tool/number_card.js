...
erpadda.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new erpadda.accounts.NumberCard(summary);
...
erpadda.accounts.NumberCard = class NumberCard {
	constructor(options) {
		this.$card = vmraid.utils.build_summary_item(options);
	}
