vmraid.provide("erpadda.e_commerce.wishlist");
var wishlist = erpadda.e_commerce.wishlist;
...
$.extend(wishlist, {
	set_wishlist_count: function(animate=false) {
		...
		var wish_count = vmraid.get_cookie("wish_count");
	...
	bind_remove_action: function() {
		...
		$('.page_content').on("click", ".remove-wish", (e) => {
			...
			let success_action = function() {
				...
				$card_wrapper.addClass("wish-removed");
				if (vmraid.get_cookie("wish_count") == 0) {
					$(".page_content").empty();
	...
	wishlist_action(btn) {
		...
		let success_action = function() {
			erpadda.e_commerce.wishlist.set_wishlist_count(true);
		};
	...
	add_remove_from_wishlist(action, args, success_action, failure_action, async=false) {
		...
		if (vmraid.session.user==="Guest") {
			...
			let method = "erpadda.e_commerce.doctype.wishlist.wishlist.add_to_wishlist";
			if (action === "remove") {
				method = "erpadda.e_commerce.doctype.wishlist.wishlist.remove_from_wishlist";
			}
	...
	render_empty_state() {
		$(".page_content").append(`
			<div class="cart-empty vmraid-card">
				<div class="cart-empty-state">
					<img src="/assets/erpadda/images/ui-states/cart-empty-state.png" alt="Empty Cart">
				</div>
