class Quiz {
	...
	get_quiz() {
		vmraid.call('erpadda.education.utils.get_quiz', {
			quiz_name: this.name,
			course: this.course
		}).then(res => {
	...
	submit() {
		...
		this.disable()
		vmraid.call('erpadda.education.utils.evaluate_quiz', {
			quiz_name: this.name,
			...
			if (!res.message) {
				vmraid.throw(__("Something went wrong while evaluating the quiz."))
			}
