vmraid.provide("erpadda.timesheet");

erpadda.timesheet.timer = function(frm, row, timestamp=0) {
	...
	erpadda.timesheet.control_timer(frm, dialog, row, timestamp);
	dialog.show();
};

erpadda.timesheet.control_timer = function(frm, dialog, row, timestamp=0) {
	...
	$btn_start.click(function(e) {
		if (!initialised) {
			...
			row = vmraid.model.add_child(frm.doc, "Timesheet Detail", "time_logs");
			row.activity_type = args.activity_type;
			row.from_time = vmraid.datetime.get_datetime_as_string();
			...
			if(row.expected_hours) {
				...
				row.to_time = d.format(vmraid.defaultDatetimeFormat);
			}
	...
	$btn_complete.click(function() {
		...
		grid_row.doc.to_time = vmraid.datetime.now_datetime();
		grid_row.refresh();
		frm.save();
		reset();
		dialog.hide();
	...
	function updateStopwatch(increment) {
		...
		if(cur_dialog && cur_dialog.get_value('expected_hours') > 0) {
			if(flag && (currentIncrement >= (cur_dialog.get_value('expected_hours') * 3600))) {
				vmraid.utils.play_sound("alert");
				vmraid.msgprint(__("Timer exceeded the given hours."));
				flag = false;
			}
		}
