erpadda.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		this.frm = opts.frm;
		...
		// }
		this.scan_api = opts.scan_api || "erpadda.stock.utils.scan_barcode";
	}
	...
	update_table(data) {
		...
		let batch_no_scan =
			Boolean(batch_no) && vmraid.meta.has_field(cur_grid.doctype, this.batch_no_field);
		...
		if (!row) {
			...
			row = vmraid.model.add_child(this.frm.doc, cur_grid.doctype, this.items_table_name);
	...
	set_item(row, item_code) {
		...
		vmraid.model.set_value(row.doctype, row.name, item_data);
	}

	set_serial_no(row, serial_no) {
		if (serial_no && vmraid.meta.has_field(row.doctype, this.serial_no_field)) {
			...
			vmraid.model.set_value(row.doctype, row.name, this.serial_no_field, new_serial_nos);
		}
	}

	set_batch_no(row, batch_no) {
		if (batch_no && vmraid.meta.has_field(row.doctype, this.batch_no_field)) {
			vmraid.model.set_value(row.doctype, row.name, this.batch_no_field, batch_no);
		}
	}

	set_barcode(row, barcode) {
		if (barcode && vmraid.meta.has_field(row.doctype, this.barcode_field)) {
			vmraid.model.set_value(row.doctype, row.name, this.barcode_field, barcode);
		}
	}
