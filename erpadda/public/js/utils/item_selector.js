erpadda.ItemSelector = class ItemSelector {
	constructor(opts) {
		$.extend(this, opts);
		...
		if (!this.item_query) {
			this.item_query = erpadda.queries.item().query;
		}
	...
	add_item(item_code) {
		...
		$.each(this.frm.doc.items || [], (i, d) => {
			if(d[this.item_field]===item_code) {
				vmraid.model.set_value(d.doctype, d.name, 'qty', d.qty + 1);
				vmraid.show_alert({message: __("Added {0} ({1})", [item_code, d.qty]), indicator: 'green'});
				added = true;
				return false;
			}
		});
		...
		if(!added) {
			...
			vmraid.run_serially([
				...
				() => vmraid.model.set_value(d.doctype, d.name, this.item_field, item_code),
				() => vmraid.timeout(0.1),
				() => {
					vmraid.model.set_value(d.doctype, d.name, 'qty', 1);
					vmraid.show_alert({message: __("Added {0} ({1})", [item_code, 1]), indicator: 'green'});
				}
			]);
		}
	...
	render_items() {
		...
		var me = this;
		vmraid.link_search("Item", args, function(r) {
			$.each(r.values, function(i, d) {
				if(!d.image) {
					d.abbr = vmraid.get_abbr(d.item_name);
					d.color = vmraid.get_palette(d.item_name);
				}
			});
			me.dialog.results.html(vmraid.render_template('item_selector', {'data':r.values}));
		});
	}
};
