...
erpadda.HierarchyChart = class {
	...
	*/
	...
	make_node_element(node) {
		let node_card = vmraid.render_template('node_card', {
			id: node.id,
			name: node.name,
			title: node.title,
			image: node.image,
		...
		let company = this.page.add_field({
			...
			change: () => {
				...
				if (company.get_value()) {
					...
					vmraid.throw(__('Please select a company first.'));
				}
			}
		});
	...
	export_chart() {
		vmraid.dom.freeze(__('Exporting...'));
		this.page.main.css({
			'min-height': '',
			'max-height': '',
			'overflow': 'visible',
	...
	get_all_nodes() {
		...
		return new Promise(resolve => {
			vmraid.call({
				method: 'erpadda.utilities.hierarchy_chart.get_all_nodes',
				args: {
					method: me.method,
					company: me.company
				},
	...
	setup_edit_node_action(node) {
		...
		node_element.find('.btn-edit-node').click(function() {
			vmraid.set_route('Form', me.doctype, node.id);
		});
	}
