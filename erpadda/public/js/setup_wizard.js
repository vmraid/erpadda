vmraid.provide("erpadda.setup");

vmraid.pages['setup-wizard'].on_page_load = function(wrapper) {
	if(vmraid.sys_defaults.company) {
		vmraid.set_route("desk");
		return;
	}
};

vmraid.setup.on("before_load", function () {
	erpadda.setup.slides_settings.map(vmraid.setup.add_slide);
});

erpadda.setup.slides_settings = [
	{
		// Domain
		name: 'domain',
		...
		validate: function () {
			if (this.values.domains.length === 0) {
				vmraid.msgprint(__("Please select at least one domain."));
				return false;
			}
			vmraid.setup.domains = this.values.domains;
			return true;
		},
	},

	{
		...
		fields: [
			...
			},
			{
				...
				label: vmraid.setup.domains.includes('Education') ?
		...
		bind_events: function (slide) {
			...
			}).val(vmraid.boot.sysdefaults.company_name || "").trigger("change");

			slide.get_input("company_abbr").on("change", function () {
				if (slide.get_input("company_abbr").val().length > 10) {
					vmraid.msgprint(__("Company Abbreviation cannot have more than 5 characters"));
		...
		validate: function() {
			if ((this.values.company_name || "").toLowerCase() == "company") {
				vmraid.msgprint(__("Company Name cannot be Company"));
				return false;
			}
	...
	},
	{
		...
		icon: "fa fa-building",
		// help: vmraid.setup.domains.includes('Education') ?
		...
		fields: [
			{
				...
				fieldtype: 'Data',
				placeholder: vmraid.setup.domains.includes('Education') ?
		...
		validate: function () {
			...
			if(me.values.bank_account) {
				vmraid.call({
					async: false,
					method: "erpadda.accounts.doctype.account.chart_of_accounts.chart_of_accounts.validate_bank_account",
					args: {
		...
		validate_fy_dates: function() {
			...
			if (invalid || start_greater_than_end) {
				vmraid.msgprint(__("Please enter valid Financial Year Start and End Dates"));
				return false;
			}
		...
		set_fy_dates: function (slide) {
			var country = vmraid.wizard.values.country;

			if (country) {
				var fy = erpadda.setup.fiscal_years[country];
				...
				if (year_start_date > vmraid.datetime.get_today()) {
					next_year = current_year;
					current_year -= 1;
				}
		...
		bind_events: function (slide) {
			...
			slide.get_input("fy_start_date").on("change", function () {
				...
				var year_end_date =
					vmraid.datetime.add_days(vmraid.datetime.add_months(start_date, 12), -1);
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new vmraid.ui.Tree({
				parent: $(dialog.body),
				label: parent,
				expandable: true,
				method: 'erpadda.accounts.utils.get_coa',
				args: {
			...
			const buttons = $(form_container).find('.vmraid-control');
