...
{% include 'erpadda/accounts/doctype/purchase_taxes_and_charges_template/purchase_taxes_and_charges_template.js' %}

cur_frm.email_field = "contact_email";

erpadda.buying.BuyingController = class BuyingController extends erpadda.TransactionController {
	setup() {
		super.setup();
	}
	...
	onload(doc, cdt, cdn) {
		...
		if (this.frm.doc.__islocal
			&& vmraid.meta.has_field(this.frm.doc.doctype, "disable_rounded_total")) {

				var df = vmraid.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total");
				var disable = cint(df.default) || cint(vmraid.sys_defaults.disable_rounded_total);
		...
		if(this.frm.get_field('shipping_address')) {
			this.frm.set_query("shipping_address", function() {
				if(me.frm.doc.customer) {
					...
					return erpadda.queries.company_address_query(me.frm.doc)
			});
		}
		/* eslint-enable */
	}
	...
	setup_queries(doc, cdt, cdn) {
		...
		me.frm.set_query('supplier', erpadda.queries.supplier);
		me.frm.set_query('contact_person', erpadda.queries.contact_query);
		me.frm.set_query('supplier_address', erpadda.queries.address_query);

		me.frm.set_query('billing_address', erpadda.queries.company_address_query);
		...
		this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
			...
			return {
				query: "erpadda.controllers.queries.item_manufacturer_query",
	...
	supplier() {
		var me = this;
		erpadda.utils.get_party_details(this.frm, null, null, function(){
			me.apply_price_list();
		});
	}

	supplier_address() {
		erpadda.utils.get_address_display(this.frm);
		erpadda.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
	}
	...
	calculate_received_qty(doc, cdt, cdn){
		var item = vmraid.get_doc(cdt, cdn);
		vmraid.model.round_floats_in(item, ["qty", "rejected_qty"]);
		...
		vmraid.model.set_value(cdt, cdn, "received_qty", received_qty);
		vmraid.model.set_value(cdt, cdn, "received_stock_qty", received_stock_qty);
	}
	...
	validate_negative_quantity(cdt, cdn, item, fieldnames){
		...
		for(var i = 0; i<fieldnames.length; i++) {
			if(item[fieldnames[i]] < 0){
				vmraid.msgprint(__("Row #{0}: {1} can not be negative for item {2}", [item.idx,__(vmraid.meta.get_label(cdt, fieldnames[i], cdn)), item.item_code]));
	...
	warehouse(doc, cdt, cdn) {
		...
		if(item.item_code && item.warehouse) {
			return this.frm.call({
				method: "erpadda.stock.get_item_details.get_bin_details",
				child: item,
				args: {
	...
	set_from_product_bundle() {
		...
		this.frm.add_custom_button(__("Product Bundle"), function() {
			erpadda.buying.get_items_from_product_bundle(me.frm);
		}, __("Get Items From"));
	}

	shipping_address(){
		var me = this;
		erpadda.utils.get_address_display(this.frm, "shipping_address",
			"shipping_address_display", true);
	}

	billing_address() {
		erpadda.utils.get_address_display(this.frm, "billing_address",
	...
	update_auto_repeat_reference(doc) {
		if (doc.auto_repeat) {
			vmraid.call({
				method:"vmraid.automation.doctype.auto_repeat.auto_repeat.update_reference",
				args:{
				...
				callback: function(r){
					if (r.message=="success") {
						vmraid.show_alert({message:__("Auto repeat document updated"), indicator:'green'});
					} else {
						vmraid.show_alert({message:__("An error occurred during the update process"), indicator:'red'});
					}
				}
			})
		}
	...
	manufacturer(doc, cdt, cdn) {
		...
		if(row.manufacturer) {
			vmraid.call({
				method: "erpadda.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
				args: {
				...
				callback: function(r) {
					if (r.message) {
						vmraid.model.set_value(cdt, cdn, 'manufacturer_part_no', r.message);
					}
				}
			});
		}
	...
	manufacturer_part_no(doc, cdt, cdn) {
		...
		if (row.manufacturer_part_no) {
			vmraid.model.get_value('Item Manufacturer',
				{
					'item_code': row.item_code,
					'manufacturer': row.manufacturer,
...
erpadda.buying.link_to_mrs = function(frm) {
	vmraid.call({
		method: "erpadda.buying.utils.get_linked_material_requests",
		args:{
			...
			for (let item of frm.doc.items) {
				...
				(r.message[0] || []).forEach(function(d) {
					if (d.qty > 0 && qty > 0 && item.item_code == d.item_code && !item.material_request_item)
					{
						...
						vmraid.msgprint("Assigning " + d.mr_name + " to " + d.item_code + " (row " + item.idx + ")");
						if (qty > 0)
						{
							vmraid.msgprint("Splitting " + qty + " units of " + d.item_code);
							var newrow = vmraid.model.add_child(frm.doc, item.doctype, "items");
							item_length++;
...
erpadda.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return vmraid.call({
				type: "GET",
				method: "erpadda.stock.get_item_details.get_default_bom",
				args: {
					"item_code": d.item_code,
				},
				callback: function(r) {
					if(r) {
						vmraid.model.set_value(d.doctype, d.name, "bom", r.message);
					}
				}
			})
		}
...
erpadda.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new vmraid.ui.Dialog({
		...
		primary_action(args){
			...
			return vmraid.call({
				type: "GET",
				method: "erpadda.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
				...
				callback: function(r) {
					...
					if(!r.exc && r.message) {
						...
						for (var i=0; i< r.message.length; i++) {
							...
							if(vmraid.meta.get_docfield(d.doctype, "price_list_rate", d.name)) {
