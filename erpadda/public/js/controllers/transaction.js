...
erpadda.TransactionController = class TransactionController extends erpadda.taxes_and_totals {
	setup() {
		super.setup();
		let me = this;
		vmraid.flags.hide_serial_batch_dialog = true;
		vmraid.ui.form.on(this.frm.doctype + " Item", "rate", function(frm, cdt, cdn) {
			var item = vmraid.get_doc(cdt, cdn);
			var has_margin_field = vmraid.meta.has_field(cdt, 'margin_type');

			vmraid.model.round_floats_in(item, ["rate", "price_list_rate"]);
		...
		vmraid.ui.form.on(this.frm.cscript.tax_table, "rate", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		vmraid.ui.form.on(this.frm.cscript.tax_table, "tax_amount", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		vmraid.ui.form.on(this.frm.cscript.tax_table, "row_id", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		vmraid.ui.form.on(this.frm.cscript.tax_table, "included_in_print_rate", function(frm, cdt, cdn) {
			cur_frm.cscript.set_dynamic_labels();
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		vmraid.ui.form.on(this.frm.doctype, "apply_discount_on", function(frm) {
		...
		vmraid.ui.form.on(this.frm.doctype, "additional_discount_percentage", function(frm) {
			if(!frm.doc.apply_discount_on) {
				vmraid.msgprint(__("Please set 'Apply Additional Discount On'"));
				return;
			}
			...
			var total = flt(frm.doc[vmraid.model.scrub(frm.doc.apply_discount_on)]);
		...
		vmraid.ui.form.on(this.frm.doctype, "discount_amount", function(frm) {
		...
		vmraid.ui.form.on(this.frm.doctype + " Item", {
			items_add: function(frm, cdt, cdn) {
				...
				erpadda.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, 'items');
			}
		});
		...
		if(vmraid.meta.get_docfield(this.frm.doc.doctype, "pricing_rules")) {
		...
		if (this.frm.fields_dict["items"].grid.get_field('blanket_order')) {
			this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
				...
				return {
					query: "erpadda.controllers.queries.get_blanket_orders",
					filters: {
						"company": doc.company,
	...
	onload() {
		...
		if(this.frm.doc.__islocal) {
			var currency = vmraid.defaults.get_user_default("currency");
	...
	onload_post_render() {
		if(this.frm.doc.__islocal && !(this.frm.doc.taxes || []).length
			...
			vmraid.after_ajax(() => this.apply_default_taxes());
		} else if(this.frm.doc.__islocal && this.frm.doc.company && this.frm.doc["items"]
			&& !this.frm.doc.is_pos) {
			vmraid.after_ajax(() => this.calculate_taxes_and_totals());
		}
		if(vmraid.meta.get_docfield(this.frm.doc.doctype + " Item", "item_code")) {
	...
	scan_barcode() {
		const barcode_scanner = new erpadda.utils.BarcodeScanner({frm:this.frm});
		barcode_scanner.process_scan();
	}

	apply_default_taxes() {
		var me = this;
		var taxes_and_charges_field = vmraid.meta.get_docfield(me.frm.doc.doctype, "taxes_and_charges",
			me.frm.doc.name);
		...
		if (taxes_and_charges_field) {
			return vmraid.call({
				method: "erpadda.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
	...
	send_sms() {
		var sms_man = new erpadda.SMSManager(this.frm.doc);
	}
							...
							vmraid.run_serially([
								...
								() => {
									if (show_batch_dialog)
										return vmraid.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"])
											.then((r) => {
												if (r.message &&
												(r.message.has_batch_no || r.message.has_serial_no)) {
													vmraid.flags.hide_serial_batch_dialog = false;
												}
											});
								},
								() => {
									// check if batch serial selector is disabled or not
									if (show_batch_dialog && !vmraid.flags.hide_serial_batch_dialog)
										return vmraid.db.get_single_value('Stock Settings', 'disable_serial_no_and_batch_selector')
											.then((value) => {
												if (value) {
													vmraid.flags.hide_serial_batch_dialog = true;
												}
								...
								() => {
									if(show_batch_dialog && !vmraid.flags.hide_serial_batch_dialog) {
										...
										erpadda.show_serial_batch_selector(me.frm, d, (item) => {
											me.frm.script_manager.trigger('qty', item.doctype, item.name);
											if (!me.frm.doc.set_warehouse)
												me.frm.script_manager.trigger('warehouse', item.doctype, item.name);
											me.apply_price_list(item, true);
										}, undefined, !vmraid.flags.hide_serial_batch_dialog);
									}
								},
	...
	price_list_rate(doc, cdt, cdn) {
		var item = vmraid.get_doc(cdt, cdn);
		vmraid.model.round_floats_in(item, ["price_list_rate", "discount_percentage"]);
	...
	margin_rate_or_amount(doc, cdt, cdn) {
		...
		let item = vmraid.get_doc(cdt, cdn);
		this.apply_pricing_rule_on_item(item);
		this.calculate_taxes_and_totals();
	...
	margin_type(doc, cdt, cdn) {
		...
		let item = vmraid.get_doc(cdt, cdn);
		if (!item.margin_type) {
			vmraid.model.set_value(cdt, cdn, "margin_rate_or_amount", 0);
		} else {
	...
	get_incoming_rate(item, posting_date, posting_time, voucher_type, company) {
		...
		vmraid.call({
			method: 'erpadda.stock.utils.get_incoming_rate',
			args: {
				args: item_args
			},
			callback: function(r) {
				vmraid.model.set_value(item.doctype, item.name, 'rate', r.message * item.conversion_factor);
			}
		});
	}
	...
	add_taxes_from_item_tax_template(item_tax_map) {
		...
		if(item_tax_map && cint(vmraid.defaults.get_default("add_taxes_from_item_tax_template"))) {
			...
			$.each(item_tax_map, function(tax, rate) {
				...
				if(!found) {
					let child = vmraid.model.add_child(me.frm.doc, "taxes");
	...
	serial_no(doc, cdt, cdn) {
		...
		if (item && item.serial_no) {
			if (!item.item_code) {
				...
				if (!doc.is_return && cint(vmraid.user_defaults.set_qty_in_transactions_based_on_serial_no_input)) {
					setTimeout(() => {
	...
	update_qty(cdt, cdn) {
		...
		vmraid.model.set_value(item.doctype, item.name,
			"qty", valid_serial_nos.length / item.conversion_factor);
		vmraid.model.set_value(item.doctype, item.name, "stock_qty", valid_serial_nos.length);
	}
	...
	set_default_internal_warehouse() {
		...
		if ((this.frm.doc.doctype === 'Sales Invoice' && me.frm.doc.update_stock)
			...
			if (this.frm.doc.is_internal_customer && this.frm.doc.company === this.frm.doc.represents_company) {
				vmraid.db.get_value('Company', this.frm.doc.company, 'default_in_transit_warehouse', function(value) {
	...
	company() {
		...
		var set_pricing = function() {
			if(me.frm.doc.company && me.frm.fields_dict.currency) {
				var company_currency = me.get_company_currency();
				var company_doc = vmraid.get_doc(":Company", me.frm.doc.company);
				...
				if (company_doc.default_selling_terms && vmraid.meta.has_field(me.frm.doc.doctype, "tc_name") &&
				...
				if (company_doc.default_buying_terms && vmraid.meta.has_field(me.frm.doc.doctype, "tc_name") &&
		...
		var set_party_account = function(set_pricing) {
			if (in_list(["Sales Invoice", "Purchase Invoice"], me.frm.doc.doctype)) {
				...
				var party = me.frm.doc[vmraid.model.scrub(party_type)];
				if(party && me.frm.doc.company) {
					return vmraid.call({
						method: "erpadda.accounts.party.get_party_account",
						args: {
							company: me.frm.doc.company,
		...
		if (vmraid.meta.get_docfield(this.frm.doctype, "shipping_address") &&
			in_list(['Purchase Order', 'Purchase Receipt', 'Purchase Invoice'], this.frm.doctype)) {
			erpadda.utils.get_shipping_address(this.frm, function() {
				set_party_account(set_pricing);
			});

			// Get default company billing address in Purchase Invoice, Order and Receipt
			if (this.frm.doc.company && vmraid.meta.get_docfield(this.frm.doctype, "billing_address")) {
				vmraid.call({
					method: "erpadda.setup.doctype.company.company.get_default_company_address",
					...
					callback: function(r) {
						if (r.message) {
							...
							if (vmraid.meta.get_docfield(me.frm.doctype, 'company_address')) {
		...
		if(this.frm.doc.company) {
			erpadda.last_selected_company = this.frm.doc.company;
		}
	}
	...
	transaction_date() {
		if (this.frm.doc.transaction_date) {
			...
			vmraid.ui.form.trigger(this.frm.doc.doctype, "currency");
		}
	}
	...
	posting_date() {
		...
		if (this.frm.doc.posting_date) {
			...
			if ((this.frm.doc.doctype == "Sales Invoice" && this.frm.doc.customer) ||
				...
				return vmraid.call({
					method: "erpadda.accounts.party.get_due_date",
					args: {
					...
					callback: function(r, rt) {
						if(r.message) {
							...
							vmraid.ui.form.trigger(me.frm.doc.doctype, "currency");
	...
	get_company_currency() {
		return erpadda.get_currency(this.frm.doc.company);
	}

	contact_person() {
		erpadda.utils.get_contact_details(this.frm);
	}
	...
	conversion_rate() {
		...
		this.frm.set_df_property("conversion_rate", "read_only", erpadda.stale_rate_allowed() ? 0 : 1);
	}
		...
		if (in_list(["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "Purchase Invoice", "Purchase Order", "Purchase Receipt"]), this.frm.doc.doctype) {
			...
			$.each(this.frm.doc.items || [], function(i, d) {
				if(d.margin_type == "Amount") {
					vmraid.model.set_value(d.doctype, d.name, "margin_rate_or_amount",
	...
	set_actual_charges_based_on_currency(exchange_rate) {
		...
		$.each(this.frm.doc.taxes || [], function(i, d) {
			if(d.charge_type == "Actual") {
				vmraid.model.set_value(d.doctype, d.name, "tax_amount",
	...
	uom(doc, cdt, cdn) {
		...
		if(item.item_code && item.uom) {
			return this.frm.call({
				method: "erpadda.stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom
				},
				callback: function(r) {
					if(!r.exc) {
						vmraid.model.set_value(cdt, cdn, 'conversion_factor', r.message.conversion_factor);
					}
				}
			});
		}
	...
	conversion_factor(doc, cdt, cdn, dont_fetch_price_list_rate) {
		if(vmraid.meta.get_docfield(cdt, "stock_qty", cdn)) {
			var item = vmraid.get_doc(cdt, cdn);
			vmraid.model.round_floats_in(item, ["qty", "conversion_factor"]);
			...
			if(vmraid.flags.dont_fetch_price_list_rate) {
				return
			}

			if (!dont_fetch_price_list_rate &&
				vmraid.meta.has_field(doc.doctype, "price_list_currency")) {
	...
	toggle_conversion_factor(item) {
		...
		if(this.frm.get_field('items').grid.fields_map.conversion_factor) {
			this.frm.fields_dict.items.grid.toggle_enable("conversion_factor",
				((item.uom != item.stock_uom) && !vmraid.meta.get_docfield(cur_frm.fields_dict.items.grid.doctype, "conversion_factor").read_only)? true: false);
		}
	...
	service_stop_date(frm, cdt, cdn) {
		...
		if(child.service_stop_date) {
			...
			if(stop_date < start_date) {
				vmraid.model.set_value(cdt, cdn, "service_stop_date", "");
				vmraid.throw(__("Service Stop Date cannot be before Service Start Date"));
			} else if (stop_date > end_date) {
				vmraid.model.set_value(cdt, cdn, "service_stop_date", "");
				vmraid.throw(__("Service Stop Date cannot be after Service End Date"));
			}
		}
	}
	...
	service_start_date(frm, cdt, cdn) {
		...
		if(child.service_start_date) {
			vmraid.call({
				"method": "erpadda.stock.get_item_details.calculate_service_end_date",
				args: {"args": child},
				callback: function(r) {
					vmraid.model.set_value(cdt, cdn, "service_end_date", r.message.service_end_date);
				}
			})
		}
	}
	...
	change_form_labels(company_currency) {
		...
		if(vmraid.meta.get_docfield(cur_frm.doctype, "net_total"))
			cur_frm.toggle_display("net_total", show);

		if(vmraid.meta.get_docfield(cur_frm.doctype, "base_net_total"))
	...
	change_grid_labels(company_currency) {
		...
		if (this.frm.doc.operations && this.frm.doc.operations.length > 0) {
			...
			$.each(["base_operating_cost", "base_hour_rate"], function(i, fname) {
				if(vmraid.meta.get_docfield(item_grid.doctype, fname))
	...
	update_payment_schedule_grid_labels(company_currency) {
		...
		if (this.frm.doc.payment_schedule && this.frm.doc.payment_schedule.length > 0) {
			...
			$.each(["base_payment_amount", "base_outstanding", "base_paid_amount"], function(i, fname) {
				if (vmraid.meta.get_docfield(schedule_grid.doctype, fname))
	...
	ignore_pricing_rule() {
		if(this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "erpadda.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	apply_pricing_rule(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "erpadda.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	_set_values_for_item_list(children) {
		...
		for(var i=0, l=children.length; i<l; i++) {
			var d = children[i] ;
			let item_row = vmraid.get_doc(d.doctype, d.name);
			var existing_pricing_rule = vmraid.model.get_value(d.doctype, d.name, "pricing_rules");
			for(var k in d) {
				var v = d[k];
			...
			vmraid.model.round_floats_in(item_row, ["price_list_rate", "discount_percentage"]);

			// if pricing rule set as blank from an existing value, apply price_list
			if(!me.frm.doc.ignore_pricing_rule && existing_pricing_rule && !d.pricing_rules) {
				me.apply_price_list(vmraid.get_doc(d.doctype, d.name));
			} else if(!d.pricing_rules) {
				me.remove_pricing_rule(vmraid.get_doc(d.doctype, d.name));
			}
					...
					if (in_list(data.apply_rule_on_other_items, d[data.apply_rule_on])) {
						for(var k in data) {
							if (in_list(fields, k) && data[k] && (data.price_or_product_discount === 'price' || k === 'pricing_rules')) {
								vmraid.model.set_value(d.doctype, d.name, k, data[k]);
							}
						}
					}
				});
	...
	apply_product_discount(args) {
		...
		args.free_item_data.forEach(pr_row => {
			...
			if (!items || !in_list(exist_items, (pr_row.item_code, pr_row.pricing_rules))) {
				...
				row_to_modify = vmraid.model.add_child(this.frm.doc,
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		// `erpadda.stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm.call({
			method: "erpadda.stock.get_item_details.apply_price_list",
			args: {	args: args },
			callback: function(r) {
				if (!r.exc) {
					vmraid.run_serially([
	...
	validate_company_and_party() {
		...
		$.each(["company", "customer"], function(i, fieldname) {
			if(vmraid.meta.has_field(me.frm.doc.doctype, fieldname) && me.frm.doc.doctype != "Purchase Order") {
				if (!me.frm.doc[fieldname]) {
					vmraid.msgprint(__("Please specify") + ": " +
						vmraid.meta.get_label(me.frm.doc.doctype, fieldname, me.frm.doc.name) +
	...
	get_terms() {
		...
		erpadda.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function(r) {
			if(!r.exc) {
	...
	taxes_and_charges() {
		...
		if(this.frm.doc.taxes_and_charges) {
			return this.frm.call({
				method: "erpadda.controllers.accounts_controller.get_taxes_and_charges",
				args: {
					"master_doctype": vmraid.meta.get_docfield(this.frm.doc.doctype, "taxes_and_charges",
	...
	tax_category() {
		...
		vmraid.run_serially([
			...
			() => erpadda.utils.set_taxes(this.frm, "tax_category"),
		]);
	}
	...
	update_item_tax_map() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "erpadda.stock.get_item_details.get_item_tax_info",
				args: {
					company: me.frm.doc.company,
	...
	item_tax_template(doc, cdt, cdn) {
		...
		if(item.item_tax_template) {
			return this.frm.call({
				method: "erpadda.stock.get_item_details.get_item_tax_map",
				args: {
					company: me.frm.doc.company,
					item_tax_template: item.item_tax_template,
					as_json: true
	...
	is_recurring() {
		...
		if(this.frm.doc.is_recurring && this.frm.doc.__islocal) {
			vmraid.msgprint(__("Please set recurring after saving"));
		...
		if(this.frm.doc.is_recurring) {
			...
			var owner_email = this.frm.doc.owner=="Administrator"
				? vmraid.user_info("Administrator").email
				: this.frm.doc.owner;
			...
			this.frm.doc.repeat_on_day_of_month = vmraid.datetime.str_to_obj(this.frm.doc.posting_date).getDate();
		}
	...
	from_date() {
		...
		if(this.frm.doc.from_date) {
			...
			if(months) {
				var to_date = vmraid.datetime.add_months(this.frm.doc.from_date,
					months);
				this.frm.doc.to_date = vmraid.datetime.add_days(to_date, -1);
	...
	setup_item_selector() {
		...
		// 	this.item_selector = new erpadda.ItemSelector({frm: this.frm});
		// }
	}
	...
	make_quality_inspection() {
		...
		const dialog = new vmraid.ui.Dialog({
			...
			primary_action: function () {
				...
				vmraid.call({
					method: "erpadda.controllers.stock_controller.make_quality_inspections",
					args: {
						doctype: me.frm.doc.doctype,
					...
					callback: function (r) {
						if (r.message.length > 0) {
							if (r.message.length === 1) {
								vmraid.set_route("Form", "Quality Inspection", r.message[0]);
							} else {
								vmraid.route_options = {
									"reference_type": me.frm.doc.doctype,
									"reference_name": me.frm.doc.name
								};
								vmraid.set_route("List", "Quality Inspection");
							}
						}
						dialog.hide();
					}
		...
		if (!data.length) {
			vmraid.msgprint(__("All items in this document already have a linked Quality Inspection."));
		} else {
			dialog.show();
		}
	}
	...
	get_method_for_payment() {
		var method = "erpadda.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
		if(cur_frm.doc.__onload && cur_frm.doc.__onload.make_payment_via_journal_entry){
			if(in_list(['Sales Invoice', 'Purchase Invoice'],  cur_frm.doc.doctype)){
				method = "erpadda.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			}else {
				method= "erpadda.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_batch(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			vmraid.throw(__("Please enter Item Code to get batch no"));
			...
			let filters = {
				'item_code': item.item_code,
				'posting_date': me.frm.doc.posting_date || vmraid.datetime.nowdate(),
			}
			...
			return {
				query : "erpadda.controllers.queries.get_batch_no",
				filters: filters
			}
		}
	}
	...
	set_query_for_item_tax_template(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			...
			return {
				query: "erpadda.controllers.queries.get_tax_template",
				filters: filters
			}
		}
	}
	...
	payment_terms_template() {
		...
		if(doc.payment_terms_template && doc.doctype !== 'Delivery Note') {
			...
			vmraid.call({
				method: "erpadda.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term(doc, cdt, cdn) {
		...
		if(row.payment_term) {
			vmraid.call({
				method: "erpadda.controllers.accounts_controller.get_payment_term_details",
				args: {
				...
				callback: function(r) {
					if(r.message && !r.exc) {
						for (var d in r.message) {
							vmraid.model.set_value(cdt, cdn, d, r.message[d]);
							const company_currency = me.get_company_currency();
							me.update_payment_schedule_grid_labels(company_currency);
						}
					}
	...
	against_blanket_order(doc, cdt, cdn) {
		...
		if(!item.against_blanket_order) {
			vmraid.model.set_value(this.frm.doctype + " Item", item.name, "blanket_order", null);
			vmraid.model.set_value(this.frm.doctype + " Item", item.name, "blanket_order_rate", 0.00);
		}
	}
	...
	blanket_order(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype=="Sales Order" || item.parenttype=="Purchase Order")) {
			vmraid.call({
				method: "erpadda.stock.get_item_details.get_blanket_order_details",
				args: {
					args:{
						item_code: item.item_code,
				...
				callback: function(r) {
					if (!r.message) {
						vmraid.throw(__("Invalid Blanket Order for the selected Customer and Item"));
					} else {
						vmraid.run_serially([
							() => vmraid.model.set_value(cdt, cdn, "blanket_order_rate", r.message.blanket_order_rate),
							() => me.frm.script_manager.trigger("price_list_rate", cdt, cdn)
						]);
					}
				}
	...
	autofill_warehouse(child_table, warehouse_field, warehouse) {
		if (warehouse && child_table && child_table.length) {
			...
			$.each(child_table || [], function(i, item) {
				vmraid.model.set_value(doctype, item.name, warehouse_field, warehouse);
			});
		}
	}
...
erpadda.show_serial_batch_selector = function (frm, d, callback, on_close, show_dialog) {
	...
	vmraid.require("assets/erpadda/js/utils/serial_no_batch_selector.js", function() {
		new erpadda.SerialNoBatchSelector({
			frm: frm,
			item: d,
			warehouse_details: {
				type: "Warehouse",
...
erpadda.apply_putaway_rule = (frm, purpose=null) => {
	if (!frm.doc.company) {
		vmraid.throw({message: __("Please select a Company first."), title: __("Mandatory")});
	}
	if (!frm.doc.items.length) return;

	vmraid.call({
		method: "erpadda.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
