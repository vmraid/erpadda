...
erpadda.taxes_and_totals = class TaxesAndTotals extends erpadda.payments {
	setup() {
	...
	apply_pricing_rule_on_item(item) {
		...
		vmraid.model.set_value(item.doctype, item.name, "rate", item_rate);
	}
	...
	calculate_discount_amount() {
		if (vmraid.meta.get_docfield(this.frm.doc.doctype, "discount_amount")) {
	...
	validate_conversion_rate() {
		...
		var conversion_rate_label = vmraid.meta.get_label(this.frm.doc.doctype, "conversion_rate",
			this.frm.doc.name);
	...
	calculate_item_values() {
		...
		if (!this.discount_amount_applied) {
			$.each(this.frm.doc["items"] || [], function(i, item) {
				vmraid.model.round_floats_in(item);
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			vmraid.model.round_floats_in(tax);
		});
	}

	fetch_round_off_accounts() {
		let me = this;
		vmraid.flags.round_off_applicable_accounts = [];

		if (me.frm.doc.company) {
			return vmraid.call({
				"method": "erpadda.controllers.taxes_and_totals.get_round_off_applicable_accounts",
				"args": {
					"company": me.frm.doc.company,
					"account_list": vmraid.flags.round_off_applicable_accounts
				},
				callback(r) {
					if (r.message) {
						vmraid.flags.round_off_applicable_accounts.push(...r.message);
					}
				}
			});
		}
	...
	calculate_shipping_charges() {
		...
		vmraid.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
		if (vmraid.meta.get_docfield(this.frm.doc.doctype, "shipping_rule", this.frm.doc.name)) {
	...
	round_off_totals(tax) {
		if (vmraid.flags.round_off_applicable_accounts.includes(tax.account_head)) {
	...
	manipulate_grand_total_for_inclusive_tax() {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				var non_inclusive_tax_amount = vmraid.utils.sum($.map(this.frm.doc.taxes || [],
					function(d) {
	...
	calculate_totals() {
		...
		if(in_list(["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "POS Invoice"], this.frm.doc.doctype)) {
			...
			if(tax_count) {
				...
				vmraid.model.round_floats_in(this.frm.doc,
		...
		vmraid.model.round_floats_in(this.frm.doc, ["grand_total", "base_grand_total"]);
	...
	set_rounded_total() {
		...
		if(vmraid.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total", this.frm.doc.name)) {
			disable_rounded_total = this.frm.doc.disable_rounded_total;
		} else if (vmraid.sys_defaults.disable_rounded_total) {
			disable_rounded_total = vmraid.sys_defaults.disable_rounded_total;
		}
		...
		if(vmraid.meta.get_docfield(this.frm.doc.doctype, "rounded_total", this.frm.doc.name)) {
	...
	_cleanup() {
		...
		if(this.frm.doc["items"] && this.frm.doc["items"].length) {
			if(!vmraid.meta.get_docfield(this.frm.doc["items"][0].doctype, "item_tax_amount", this.frm.doctype)) {
				$.each(this.frm.doc["items"] || [], function(i, item) {
		...
		if(this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if(!vmraid.meta.get_docfield(this.frm.doc["taxes"][0].doctype, "tax_amount_after_discount_amount", this.frm.doctype)) {
	...
	set_discount_amount() {
		if(this.frm.doc.additional_discount_percentage) {
			this.frm.doc.discount_amount = flt(flt(this.frm.doc[vmraid.scrub(this.frm.doc.apply_discount_on)])
	...
	apply_discount_amount() {
		...
		if (this.frm.doc.discount_amount) {
			if(!this.frm.doc.apply_discount_on)
				vmraid.throw(__("Please select Apply Discount On"));
	...
	calculate_total_advance(update_paid_amount) {
		var total_allocated_amount = vmraid.utils.sum($.map(this.frm.doc["advances"] || [], function(adv) {
	...
	calculate_outstanding_amount(update_paid_amount) {
		...
		vmraid.model.round_floats_in(this.frm.doc, ["grand_total", "total_advance", "write_off_amount"]);
		...
		if(in_list(["Sales Invoice", "POS Invoice", "Purchase Invoice"], this.frm.doc.doctype)) {
			...
			vmraid.model.round_floats_in(this.frm.doc, ["paid_amount"]);
		...
		if(this.frm.doc.is_pos && (update_paid_amount===undefined || update_paid_amount)) {
			$.each(this.frm.doc['payments'] || [], function(index, data) {
				if(data.default && payment_status && total_amount_to_pay > 0) {
					...
					vmraid.model.set_value(data.doctype, data.name, "base_amount", base_amount);
					let amount = flt(total_amount_to_pay / me.frm.doc.conversion_rate, precision("amount", data));
					vmraid.model.set_value(data.doctype, data.name, "amount", amount);
					payment_status = false;
				} else if(me.frm.doc.paid_amount) {
					vmraid.model.set_value(data.doctype, data.name, "amount", 0.0);
				}
			});
		}
	}
