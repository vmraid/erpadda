vmraid.ui.form.on("Communication", {
	...
	setup_custom_buttons: (frm) => {
		...
		if(frm.doc.reference_doctype !== "Issue") {
			frm.add_custom_button(__("Issue"), () => {
				vmraid.confirm(__(confirm_msg, [__("Issue")]), () => {
		...
		if(!in_list(["Lead", "Opportunity"], frm.doc.reference_doctype)) {
			frm.add_custom_button(__("Lead"), () => {
				vmraid.confirm(__(confirm_msg, [__("Lead")]), () => {
					frm.trigger('make_lead_from_communication');
				})
			}, __('Create'));

			frm.add_custom_button(__("Opportunity"), () => {
				vmraid.confirm(__(confirm_msg, [__("Opportunity")]), () => {
	...
	make_lead_from_communication: (frm) => {
		return vmraid.call({
			method: "erpadda.crm.doctype.lead.lead.make_lead_from_communication",
			args: {
	...
	make_issue_from_communication: (frm) => {
		return vmraid.call({
			method: "erpadda.support.doctype.issue.issue.make_issue_from_communication",
			args: {
				communication: frm.doc.name
			},
			freeze: true,
	...
	make_opportunity_from_communication: (frm) => {
		...
		vmraid.prompt(fields, data => {
			vmraid.call({
				method: "erpadda.crm.doctype.opportunity.opportunity.make_opportunity_from_communication",
				args: {
					communication: frm.doc.name,
