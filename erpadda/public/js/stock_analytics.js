...
erpadda.StockAnalytics = class StockAnalytics extends erpadda.StockGridReport {
		...
		var args = {
			...
			tree_grid: {
				...
				formatter: function(item) {
					if(!item.is_group) {
						return repl("<a \
							onclick='vmraid.cur_grid_report.show_stock_ledger(\"%(value)s\")'>\
							%(value)s</a>", {
								value: item.name,
							});
					} else {
	...
	prepare_balances() {
		var me = this;
		var from_date = vmraid.datetime.str_to_obj(this.from_date);
		var to_date = vmraid.datetime.str_to_obj(this.to_date);
		var data = vmraid.report_dump.data["Stock Ledger Entry"];
		...
		for(var i=0, j=data.length; i<j; i++) {
			...
			var posting_datetime = vmraid.datetime.str_to_obj(sl.posting_datetime);
			...
			if(me.is_default("warehouse") ? true : me.warehouse == sl.warehouse) {
				...
				if(me.value_or_qty!="Quantity") {
					...
					var valuation_method = item.valuation_method ?
						item.valuation_method : vmraid.sys_defaults.valuation_method;
