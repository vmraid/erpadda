...
erpadda.payments = class payments extends erpadda.stock.StockController {
	make_payment() {
		var me = this;
	...
	make_keyboard(){
		var me = this;
		$(this.$body).empty();
		$(this.$body).html(vmraid.render_template('pos_payment', this.frm.doc))
	...
	make_multimode_payment(){
		...
		this.payments = vmraid.model.add_child(this.frm.doc, 'Multi Mode Payment', "payments");
	...
	show_payment_details(){
		...
		if (this.frm.doc.payments.length) {
			$.each(this.frm.doc.payments, function(index, data) {
				$(vmraid.render_template('payment_details', {
	...
	show_amounts(){
		...
		$(this.$body).find('.outstanding_amount').text(format_currency(this.frm.doc.outstanding_amount, vmraid.get_doc(":Company", this.frm.doc.company).default_currency));
		this.update_invoice();
	}
}
