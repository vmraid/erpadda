...
$.extend(erpadda, {
	get_currency: function(company) {
		...
		if(company)
			return vmraid.get_doc(":Company", company).default_currency || vmraid.boot.sysdefaults.currency;
		else
			return vmraid.boot.sysdefaults.currency;
	},

	get_presentation_currency_list: () => {
		const docs = vmraid.boot.docs;
	...
	hide_company: function() {
		if(cur_frm.fields_dict.company) {
			...
			} else if(erpadda.last_selected_company) {
				if(!cur_frm.doc.company) cur_frm.set_value("company", erpadda.last_selected_company);
			}
		}
	},
	...
	is_perpetual_inventory_enabled: function(company) {
		if(company) {
			return vmraid.get_doc(":Company", company).enable_perpetual_inventory
		}
	},

	stale_rate_allowed: () => {
		return cint(vmraid.boot.sysdefaults.allow_stale);
	},
	...
	setup_serial_or_batch_no: function() {
		...
		vmraid.model.get_value('Item', {'name': grid_row.doc.item_code},
	...
	route_to_adjustment_jv: (args) => {
		vmraid.model.with_doctype('Journal Entry', () => {
			// route to adjustment Journal Entry to handle Account Balance and Stock Value mismatch
			let journal_entry = vmraid.model.get_new_doc('Journal Entry');

			args.accounts.forEach((je_account) => {
				let child_row = vmraid.model.add_child(journal_entry, "accounts");
			...
			vmraid.set_route('Form','Journal Entry', journal_entry.name);
		});
	},

	route_to_pending_reposts: (args) => {
		vmraid.set_route('List', 'Repost Item Valuation', args);
	},
	...
	proceed_save_with_reminders_frequency_change: () => {
		...
		vmraid.call({
			method: 'erpadda.hr.doctype.hr_settings.hr_settings.set_proceed_with_frequency_change',
			callback: () => {
				cur_frm.save();
			}
		});
...
$.extend(erpadda.utils, {
	set_party_dashboard_indicators: function(frm) {
		if(frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if(company_wise_info.length > 1) {
				company_wise_info.forEach(function(info) {
					erpadda.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function(tc_name, doc, callback) {
		if(tc_name) {
			return vmraid.call({
				method: 'erpadda.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions',
				args: {
	...
	make_bank_account: function(doctype, docname) {
		vmraid.call({
			method: "erpadda.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	add_dimensions: function(report_name, index) {
		...
		vmraid.call({
			method: "erpadda.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function(r) {
	...
	make_pricing_rule: function(doctype, docname) {
		vmraid.call({
			method: "erpadda.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	**/
	...
	get_tree_options: function(option) {
		...
		let unscrub_option = vmraid.model.unscrub(option);
		let user_permission = vmraid.defaults.get_user_permissions();
		let options;
	...
	get_tree_default: function(option) {
		...
		if(options.includes(vmraid.defaults.get_default(option))) {
			return vmraid.defaults.get_default(option);
		} else {
			return options[0];
		}
	},
	overrides_parent_value_in_all_rows: function(doc, dt, dn, table_fieldname, fieldname, parent_fieldname) {
		if (doc[parent_fieldname]) {
			...
			vmraid.refresh_field(table_fieldname);
		}
	},
...
erpadda.utils.select_alternate_items = function(opts) {
	const frm = opts.frm;
	...
	const dialog = new vmraid.ui.Dialog({
				...
				fields: [{
					...
					onchange: function() {
						...
						if (item_code && warehouse) {
							vmraid.call({
								method: "erpadda.stock.utils.get_latest_stock_qty",
								args: {
									item_code: item_code,
									warehouse: warehouse
								},
					...
					get_query: (e) => {
						return {
							query: "erpadda.stock.doctype.item_alternative.item_alternative.get_alternative_items",
							filters: {
		...
		primary_action: function() {
			...
			alternative_items.forEach(d => {
				let row = vmraid.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				...
				vmraid.model.set_value(row.doctype, row.name, 'qty', qty);
				vmraid.model.set_value(row.doctype, row.name, opts.original_item_field, d.item_code);
...
erpadda.utils.update_child_items = function(opts) {
	const frm = opts.frm;
	const cannot_add_row = (typeof opts.cannot_add_row === 'undefined') ? true : opts.cannot_add_row;
	const child_docname = (typeof opts.cannot_add_row === 'undefined') ? "items" : opts.child_docname;
	const child_meta = vmraid.get_meta(`${frm.doc.doctype} Item`);
	...
	const dialog = new vmraid.ui.Dialog({
		...
		primary_action: function() {
			...
			vmraid.call({
				method: 'erpadda.controllers.accounts_controller.update_child_qty_rate',
				freeze: true,
				args: {
...
erpadda.utils.map_current_doc = function(opts) {
	function _map() {
		if($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			var items_doctype = vmraid.meta.get_docfield(cur_frm.doctype, 'items').options;

			// find the link fieldname from items table for the given
			// source_doctype
			var link_fieldname = null;
			vmraid.get_meta(items_doctype).fields.forEach(function(d) {
			...
			if(already_set) {
				opts.source_name.forEach(function(src) {
					vmraid.model.with_doc(opts.source_doctype, src, function(r) {
						var source_doc = vmraid.model.get_doc(opts.source_doctype, src);
					...
					if(already_set) {
						vmraid.msgprint(__("You have already selected items from {0} {1}",
		...
		return vmraid.call({
			...
			type: "POST",
			method: 'vmraid.model.mapper.map_docs',
			args: {
				"method": opts.method,
	...
	if (opts.source_doctype) {
		const d = new vmraid.ui.form.MultiSelectDialog({
			...
			action: function(selections, args) {
				...
				if (values.length === 0) {
					vmraid.msgprint(__("Please select {0}", [opts.source_doctype]))
					return;
				}
...
vmraid.form.link_formatters['Item'] = function(value, doc) {
...
vmraid.form.link_formatters['Employee'] = function(value, doc) {
...
vmraid.form.link_formatters['Project'] = function(value, doc) {
...
$(document).on('app_ready', function() {
	if(!vmraid.datetime.is_timezone_same()) {
		$.each(["Stock Reconciliation", "Stock Entry", "Stock Ledger Entry",
			...
			vmraid.ui.form.on(d, "onload", function(frm) {
				cur_frm.set_df_property("posting_time", "description",
					vmraid.sys_defaults.time_zone);
			});
		});
	}
});
...
$(document).on('app_ready', function() {
	vmraid.call({
		method: 'erpadda.support.doctype.service_level_agreement.service_level_agreement.get_sla_doctypes',
		callback: function(r) {
			if (!r.message)
				return;
			...
			$.each(r.message, function(_i, d) {
				vmraid.ui.form.on(d, {
					onload: function(frm) {
						...
						vmraid.call({
							method: 'erpadda.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters',
							args: {
