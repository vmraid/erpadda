...
def get_context(context):
	...
	settings = vmraid.get_cached_doc("E Commerce Settings")
...
def get_slideshow(slideshow):
	...
	slideshow = vmraid.get_cached_doc("Website Slideshow", slideshow)
...
def get_tabs(categories):
	...
	for index, tab in enumerate(categorical_data, start=1):
		tab_values[f"tab_{index + 1}_title"] = vmraid.unscrub(tab)
		# pre-render cards for each tab
		tab_values[f"tab_{index + 1}_content"] = vmraid.render_template(
			"erpadda/www/shop-by-category/category_card_section.html",
...
def get_category_records(categories):
	...
	for category in categories:
		if category == "item_group":
			categorical_data["item_group"] = vmraid.db.get_all(
				"Item Group",
				filters={"parent_item_group": "All Item Groups", "show_in_website": 1},
				fields=["name", "parent_item_group", "is_group", "image", "route"],
			)
		else:
			doctype = vmraid.unscrub(category)
			fields = ["name"]
			if vmraid.get_meta(doctype, cached=True).get_field("image"):
				fields += ["image"]

			categorical_data[category] = vmraid.db.get_all(doctype, fields=fields)
