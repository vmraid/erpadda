...
logger = vmraid.logger("patch", allow_site=True, file_count=50)


def execute():
	affected_purchase_receipts = vmraid.db.sql(
	...
	vmraid.reload_doc("stock", "doctype", "Purchase Receipt")
	vmraid.reload_doc("stock", "doctype", "Purchase Receipt Item")

	for pr in affected_purchase_receipts:
		pr_name = pr[0]
		logger.info("purchase_receipt_status: patching PR - {}".format(pr_name))

		pr_doc = vmraid.get_doc("Purchase Receipt", pr_name)
