...
def execute():
	if "tax_type" not in vmraid.db.get_table_columns("Item Tax"):
		return
	old_item_taxes = {}
	item_tax_templates = {}

	vmraid.reload_doc("accounts", "doctype", "item_tax_template_detail", force=1)
	vmraid.reload_doc("accounts", "doctype", "item_tax_template", force=1)
	existing_templates = vmraid.db.sql(
	...
	vmraid.reload_doc("stock", "doctype", "item", force=1)
	vmraid.reload_doc("stock", "doctype", "item_tax", force=1)
	vmraid.reload_doc("selling", "doctype", "quotation_item", force=1)
	vmraid.reload_doc("selling", "doctype", "sales_order_item", force=1)
	vmraid.reload_doc("stock", "doctype", "delivery_note_item", force=1)
	vmraid.reload_doc("accounts", "doctype", "sales_invoice_item", force=1)
	vmraid.reload_doc("buying", "doctype", "supplier_quotation_item", force=1)
	vmraid.reload_doc("buying", "doctype", "purchase_order_item", force=1)
	vmraid.reload_doc("stock", "doctype", "purchase_receipt_item", force=1)
	vmraid.reload_doc("accounts", "doctype", "purchase_invoice_item", force=1)
	vmraid.reload_doc("accounts", "doctype", "accounts_settings", force=1)
	...
	for item_code in old_item_taxes.keys():
		...
		vmraid.db.sql("delete from `tabItem Tax` where parent=%s and parenttype='Item'", item_code)
	...
	for dt in doctypes:
		for d in vmraid.db.sql(
			...
			vmraid.db.set_value(dt + " Item", d.name, "item_tax_template", item_tax_template_name)
...
def get_item_tax_template(
	...
	item_tax_template = vmraid.new_doc("Item Tax Template")
	...
	for tax_type, tax_rate in item_tax_map.items():
		...
		if account_details:
			...
			if account_details.account_type not in (
				...
				vmraid.db.set_value("Account", account_details.name, "account_type", "Chargeable")
		else:
			...
			if not account_name:
				...
				tax_type = vmraid.db.get_value("Account", filters)
				if not tax_type:
					account = vmraid.new_doc("Account")
					account.update(filters)
					try:
						account.insert()
						tax_type = account.name
					except vmraid.DuplicateEntryError:
						tax_type = vmraid.db.get_value(
							"Account", {"account_name": account_name, "company": company}, "name"
						)

		account_type = vmraid.get_cached_value("Account", tax_type, "account_type")
...
def get_company(company_abbr, parenttype=None, parent=None):
	if parenttype and parent:
		company = vmraid.get_cached_value(parenttype, parent, "company")
	else:
		company = vmraid.db.get_value("Company", filters={"abbr": company_abbr})
