...
def execute():
	if not vmraid.db.has_column("Work Order", "has_batch_no"):
		return
	...
	work_orders = [d.name for d in vmraid.get_all("Work Order", filters=filters, fields=fields)]
	...
	stock_entries = vmraid.db.sql_list(
		"""
		SELECT
			se.name
		FROM
	...
	for stock_entry in stock_entries:
		doc = vmraid.get_doc("Stock Entry", stock_entry)
...
def set_expense_account(doc):
	for row in doc.items:
		if row.is_finished_item and not row.expense_account:
			row.expense_account = vmraid.get_cached_value(
