...
def execute():
	...
	vmraid.db.MAX_WRITES_PER_TRANSACTION += len(broken_sles)
	...
	broken_sr_no_records = vmraid.get_list(
		"Serial No",
		filters={
			"status": "Active",
	...
	vmraid.db.MAX_WRITES_PER_TRANSACTION += len(broken_sr_no_records)

	patch_savepoint = "serial_no_patch"
	for serial_no in broken_sr_no_records:
		try:
			vmraid.db.savepoint(patch_savepoint)
			sn = vmraid.get_doc("Serial No", serial_no)
			sn.update_serial_no_reference()
			sn.db_update()
		except Exception:
			vmraid.db.rollback(save_point=patch_savepoint)
