...
def execute():
	...
	if vmraid.db.exists("DocType", "Service Level Agreement"):
		sla_details = vmraid.db.get_all("Service Level Agreement", fields=["name", "service_level"])
		priorities = vmraid.db.get_all(
		...
		vmraid.reload_doc("support", "doctype", "pause_sla_on_status")
		vmraid.reload_doc("support", "doctype", "service_level_priority")
		vmraid.reload_doc("support", "doctype", "service_day")

		for entry in sla_details:
			values = vmraid.db.get_value(
		...
		if vmraid.db.exists("DocType", "Service Level"):
			service_levels = vmraid.db.get_all(
				"Service Level", filters={"service_level": ("not in", sl)}, fields=["*"]
			)
			for entry in service_levels:
				sla = vmraid.new_doc("Service Level Agreement")
				...
				if priority_list:
					sla = vmraid.get_doc("Service Level Agreement", sla.name)
	...
	vmraid.delete_doc_if_exists("DocType", "Service Level")
