...
def execute():
	...
	vmraid.reload_doc("Accounts", "doctype", "Salary Component Account")
	if vmraid.db.has_column("Salary Component Account", "default_account"):
	...
	for item in doctype_list:
		vmraid.reload_doc(item["module"], "doctype", item["doctype"])
	...
	vmraid.db.sql("update `tabEmployee Advance` set exchange_rate=1")

	# get all companies and it's currency
	all_companies = vmraid.db.get_all(
	...
	for d in all_companies:
		...
		if not default_payroll_payable_account:
			default_payroll_payable_account = vmraid.db.get_value(
				"Account",
				{
