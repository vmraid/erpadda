...
def execute():
	vmraid.reload_doc("e_commerce", "doctype", "website_item")
	vmraid.reload_doc("e_commerce", "doctype", "website_item_tabbed_section")
	vmraid.reload_doc("e_commerce", "doctype", "website_offer")
	vmraid.reload_doc("e_commerce", "doctype", "recommended_items")
	vmraid.reload_doc("e_commerce", "doctype", "e_commerce_settings")
	...
	item_table_fields = vmraid.db.sql("desc `tabItem`", as_dict=1)  # nosemgrep
	...
	items = vmraid.db.get_all("Item", fields=item_fields, or_filters=or_filters)
	total_count = len(items)

	for count, item in enumerate(items, start=1):
		if vmraid.db.exists("Website Item", {"item_code": item.item_code}):
			continue
		...
		vmraid.utils.update_progress_bar("Creating Website Items", count, total_count)
