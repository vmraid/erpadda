...
def execute():
	vmraid.reload_doc("hr", "doctype", "leave_policy_assignment")
	vmraid.reload_doc("hr", "doctype", "employee_grade")
	employee_with_assignment = []
	leave_policy = []

	if "leave_policy" in vmraid.db.get_table_columns("Employee"):
		employees_with_leave_policy = vmraid.db.sql(
	...
	if "default_leave_policy" in vmraid.db.get_table_columns("Employee Grade"):
		employee_grade_with_leave_policy = vmraid.db.sql(
	...
	leave_allocations = vmraid.db.sql(
...
def create_assignment(employee, leave_policy, leave_period=None, allocation_exists=False):
	if vmraid.db.get_value("Leave Policy", leave_policy, "docstatus") == 2:
		return
	...
	if not vmraid.db.exists("Leave Policy Assignment", filters):
		lpa = vmraid.new_doc("Leave Policy Assignment")
		...
		if allocation_exists:
			...
			vmraid.db.sql("Update `tabLeave Allocation` set leave_policy_assignment = %s", lpa.name)


def get_employee_with_grade(grade):
	return vmraid.get_list("Employee", filters={"grade": grade})
