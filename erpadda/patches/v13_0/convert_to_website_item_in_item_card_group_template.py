...
def execute():
	...
	"""
	vmraid.reload_doc("e_commerce", "web_template", "item_card_group")

	blocks = vmraid.db.get_all(
		"Web Page Block",
	...
	for block in blocks:
		...
		for field in fields:
			...
			if vmraid.db.exists("Website Item", {"item_code": item}):
				website_item = vmraid.db.get_value("Website Item", {"item_code": item})
			else:
