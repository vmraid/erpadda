...
def execute():
	...
	"""
	if not vmraid.db.has_column("Item", "default_warehouse"):
		return
	...
	if len(companies) == 1 and not vmraid.get_all("Item Default", limit=1):
		try:
			vmraid.db.sql(
				"""
		...
		for item_data in item_details:
			for d in [
				...
				if item_data.get(d[0]):
					company = vmraid.get_value(d[1], item_data.get(d[0]), "company", cache=True)
