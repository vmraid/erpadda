...
def get_existing_cost_center_allocations():
	if not vmraid.db.exists("DocType", "Distributed Cost Center"):
		return

	par = vmraid.qb.DocType("Cost Center")
	child = vmraid.qb.DocType("Distributed Cost Center")
	...
	cc_allocations = vmraid._dict()
	for d in records:
		cc_allocations.setdefault(d.name, vmraid._dict()).setdefault(
