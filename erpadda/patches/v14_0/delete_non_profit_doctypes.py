...
def execute():
	vmraid.delete_doc("Module Def", "Non Profit", ignore_missing=True, force=True)

	vmraid.delete_doc("Workspace", "Non Profit", ignore_missing=True, force=True)
	...
	reports = vmraid.get_all("Report", {"module": "Non Profit", "is_standard": "Yes"}, pluck="name")
	for report in reports:
		vmraid.delete_doc("Report", report, ignore_missing=True, force=True)

	dashboards = vmraid.get_all("Dashboard", {"module": "Non Profit", "is_standard": 1}, pluck="name")
	for dashboard in dashboards:
		vmraid.delete_doc("Dashboard", dashboard, ignore_missing=True, force=True)

	doctypes = vmraid.get_all("DocType", {"module": "Non Profit", "custom": 0}, pluck="name")
	...
	for record in custom_records:
		try:
			vmraid.delete_doc(record["doctype"], record["name"], ignore_missing=True)
