...
class QualityProcedure(NestedSet):
	...
	def after_insert(self):
		...
		if self.parent_quality_procedure:
			parent = vmraid.get_doc("Quality Procedure", self.parent_quality_procedure)
	...
	def set_parent(self):
		for process in self.processes:
			...
			has_parent = vmraid.db.get_value(
				"Quality Procedure", process.procedure, "parent_quality_procedure"
			)
			if not has_parent and process.procedure:
				vmraid.db.set_value(self.doctype, process.procedure, "parent_quality_procedure", self.name)
	...
	def check_for_incorrect_child(self):
		for process in self.processes:
			if process.procedure:
				...
				parent_quality_procedure = vmraid.db.get_value(
					"Quality Procedure", process.procedure, "parent_quality_procedure"
				)
				if parent_quality_procedure and parent_quality_procedure != self.name:
					vmraid.throw(
						_("{0} already has a Parent Procedure {1}.").format(
							vmraid.bold(process.procedure), vmraid.bold(parent_quality_procedure)
						),
...
def get_children(doctype, parent=None, parent_quality_procedure=None, is_root=False):
	...
	if parent:
		parent_procedure = vmraid.get_doc("Quality Procedure", parent)
		# return the list in order
		return [
			dict(
				value=d.procedure, expandable=vmraid.db.get_value("Quality Procedure", d.procedure, "is_group")
			)
...
def add_node():
	...
	return vmraid.get_doc(args).insert()
