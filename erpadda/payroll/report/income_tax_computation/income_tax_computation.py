...
from vmraid.utils import add_days, flt, getdate, rounded

from erpadda.payroll.doctype.payroll_entry.payroll_entry import get_start_end_dates
from erpadda.payroll.doctype.salary_slip.salary_slip import calculate_tax_by_tax_slab
...
class IncomeTaxComputationReport(object):
	def __init__(self, filters=None):
		...
		self.columns = []
		self.data = []
		self.employees = vmraid._dict()
		self.payroll_period_start_date = None
		self.payroll_period_end_date = None
		if self.filters.payroll_period:
			self.payroll_period_start_date, self.payroll_period_end_date = vmraid.db.get_value(
	...
	def get_employee_details(self):
		...
		employees = vmraid.get_all("Employee", filters=filters, or_filters=or_filters, fields=fields)
		...
		if not self.employees:
			vmraid.throw(_("No employees found with selected filters and active salary structure"))
	...
	def get_future_salary_slips(self):
		self.future_salary_slips = vmraid._dict()
	...
	def get_last_salary_slip(self, employee):
		last_salary_slip = vmraid.db.get_value(
			"Salary Slip",
			{
				"employee": employee,
				"docstatus": 1,
	...
	def get_tax_exempted_earnings_and_deductions(self):
		...
		ss = vmraid.qb.DocType("Salary Slip")
		ss_comps = vmraid.qb.DocType("Salary Detail")
		...
		existing_ss_exemptions = vmraid._dict()
		for d in records:
	...
	def get_employee_tax_exemptions(self):
		# add columns
		exemption_categories = vmraid.get_all("Employee Tax Exemption Category", {"is_active": 1})
	...
	def get_tax_exemptions(self, source):
		...
		par = vmraid.qb.DocType(source)
		child = vmraid.qb.DocType(child_doctype)
	...
	def get_hra(self):
		if not vmraid.get_meta("Employee Tax Exemption Declaration").has_field("monthly_house_rent"):
			return
	...
	def get_applicable_tax(self):
		...
		is_tax_rounded = vmraid.db.get_value(
			"Salary Component",
		...
		for emp, emp_details in self.employees.items():
			...
			if tax_slab:
				tax_slab = vmraid.get_cached_doc("Income Tax Slab", tax_slab)
				employee_dict = vmraid.get_doc("Employee", emp).as_dict()
	...
	def get_total_deducted_tax(self):
		...
		ss = vmraid.qb.DocType("Salary Slip")
		ss_ded = vmraid.qb.DocType("Salary Detail")
