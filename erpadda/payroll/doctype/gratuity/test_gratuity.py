...
import vmraid
from vmraid.utils import add_days, flt, get_datetime, getdate

from erpadda.hr.doctype.employee.test_employee import make_employee
from erpadda.hr.doctype.expense_claim.test_expense_claim import get_payable_account
from erpadda.payroll.doctype.gratuity.gratuity import get_last_salary_slip
...
class TestGratuity(unittest.TestCase):
	def setUp(self):
		vmraid.db.delete("Gratuity")
		vmraid.db.delete("Additional Salary", {"ref_doctype": "Gratuity"})
	...
	def test_check_gratuity_amount_based_on_current_slab_and_additional_salary_creation(self):
		...
		# amount Calculation
		component_amount = vmraid.get_all(
			"Salary Detail",
			filters={
				"docstatus": 1,
				"parent": sal_slip,
		...
		self.assertTrue(vmraid.db.exists("Additional Salary", {"ref_docname": gratuity.name}))
...
def get_gratuity_rule(name):
	rule = vmraid.db.exists("Gratuity Rule", name)
	if not rule:
		create_gratuity_rule()
	rule = vmraid.get_doc("Gratuity Rule", name)
...
def create_gratuity(**args):
	...
	gratuity = vmraid.new_doc("Gratuity")
...
def set_mode_of_payment_account():
	...
	mode_of_payment = vmraid.get_doc("Mode of Payment", "Cash")
...
def create_employee_and_get_last_salary_slip():
	...
	vmraid.db.set_value("Employee", employee, "date_of_joining", add_days(getdate(), -(6 * 365)))
	if not vmraid.db.exists("Salary Slip", {"employee": employee}):
	...
	if not vmraid.db.get_value("Employee", "test_employee@salary.com", "holiday_list"):
