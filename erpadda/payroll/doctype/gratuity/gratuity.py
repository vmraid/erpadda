...
from vmraid.utils import flt, get_datetime, get_link_to_form
...
class Gratuity(AccountsController):
	...
	def get_gl_entries(self):
		...
		if self.amount:
			...
			vmraid.throw(_("Total Amount can not be zero"))
	...
	def set_total_advance_paid(self):
		...
		if flt(paid_amount) > self.amount:
			vmraid.throw(_("Row {0}# Paid Amount cannot be greater than Total amount"))
...
def calculate_work_experience(employee, gratuity_rule):
	...
	total_working_days_per_year, minimum_year_for_gratuity = vmraid.db.get_value(
...
def calculate_employee_total_workings_days(employee, date_of_joining, relieving_date):
	...
	payroll_based_on = vmraid.db.get_value("Payroll Settings", None, "payroll_based_on") or "Leave"
...
def get_non_working_days(employee, relieving_date, status):
	...
	if status == "On Leave":
		lwp_leave_types = vmraid.get_list("Leave Type", filters={"is_lwp": 1})
		lwp_leave_types = [leave_type.name for leave_type in lwp_leave_types]
		filters["leave_type"] = ("IN", lwp_leave_types)

	record = vmraid.get_all("Attendance", filters=filters, fields=["COUNT(name) as total_lwp"])
...
def calculate_gratuity_amount(employee, gratuity_rule, experience):
	...
	calculate_gratuity_amount_based_on = vmraid.db.get_value(
...
def get_applicable_components(gratuity_rule):
	applicable_earnings_component = vmraid.get_all(
...
def get_total_applicable_component_amount(employee, applicable_earnings_component, gratuity_rule):
	...
	if not sal_slip:
		vmraid.throw(_("No Salary Slip is found for Employee: {0}").format(bold(employee)))
	component_and_amounts = vmraid.get_all(
		"Salary Detail",
		filters={
			"docstatus": 1,
			"parent": sal_slip,
	...
	if not len(component_and_amounts):
		vmraid.throw(_("No Applicable Component is present in last month salary slip"))
