...
vmraid.ui.form.on("Salary Slip", {
	...
	set_dynamic_labels: function(frm) {
		var company_currency = frm.doc.company? erpadda.get_currency(frm.doc.company): vmraid.defaults.get_default("currency");
			...
			return vmraid.call({
				...
				callback: function(r) {
					if (r.message[1] !== "Leave" && r.message[0]) {
						frm.fields_dict.absent_days.set_description(__("Unmarked Days is treated as {0}. You can can change this in {1}", [r.message, vmraid.utils.get_form_link("Payroll Settings", "Payroll Settings", true)]));
					}
					frm.refresh();
				}
			});
...
vmraid.ui.form.on('Salary Slip Timesheet', {
