...
from vmraid.tests.utils import change_settings
...
from erpadda.hr.doctype.leave_allocation.test_leave_allocation import create_leave_allocation
from erpadda.hr.doctype.leave_type.test_leave_type import create_leave_type
...
class TestSalarySlip(unittest.TestCase):
	def setUp(self):
		setup_test()
		vmraid.flags.pop("via_payroll_entry", None)
	...
	def test_payment_days_based_on_attendance(self):
		...
		vmraid.db.set_value("Employee", emp_id, {"relieving_date": None, "status": "Active"})

		vmraid.db.set_value("Leave Type", "Leave Without Pay", "include_holiday", 0)

		month_start_date = get_first_day(nowdate())
		month_end_date = get_last_day(nowdate())

		first_sunday = vmraid.db.sql(
			"""
	...
	def test_payment_days_in_salary_slip_based_on_timesheet(self):
		...
		from erpadda.projects.doctype.timesheet.timesheet import (
		...
		vmraid.db.set_value("Employee", emp, {"relieving_date": None, "status": "Active"})
	...
	def test_payment_days(self):
		...
		if date_of_joining.day > 1:
			self.assertRaises(vmraid.ValidationError, ss.save)
		...
		vmraid.db.set_value(
			"Employee",
			vmraid.get_value("Employee", {"employee_name": "test_payment_days@salary.com"}, "name"),
			"relieving_date",
			None,
		)
		vmraid.db.set_value(
	...
	def test_employee_salary_slip_read_permission(self):
		...
		vmraid.set_user("test_employee_salary_slip_read_permission@salary.com")
	...
	def test_email_salary_slip(self):
		...
		email_queue = vmraid.db.a_row_exists("Email Queue")
		self.assertTrue(email_queue)

	def test_loan_repayment_salary_slip(self):
		from erpadda.loan_management.doctype.loan.test_loan import (
			create_loan,
	...
	def test_payroll_frequency(self):
		fiscal_year = get_fiscal_year(nowdate(), company=erpadda.get_default_company())[0]
	...
	def test_component_wise_year_to_date_computation(self):
		...
		# clear salary slip for this employee
		vmraid.db.sql("DELETE FROM `tabSalary Slip` where employee_name = '%s'" % employee_name)
		...
		year_to_date = dict()
		for slip in salary_slips:
			doc = vmraid.get_doc("Salary Slip", slip.name)
	...
	def test_tax_for_payroll_period(self):
		...
		vmraid.db.sql("""delete from `tabPayroll Period`""")
		vmraid.db.sql("""delete from `tabSalary Component`""")
		...
		for doc in delete_docs:
			vmraid.db.sql("delete from `tab%s` where employee='%s'" % (doc, employee))
		...
		try:
			...
			raise
		vmraid.db.sql("""delete from `tabSalary Slip` where employee=%s""", (employee))
		...
		try:
			...
			print("\nSalary Slip - Tax calculation failed on following case\n", data, "\n")
			raise
		vmraid.db.sql("""delete from `tabAdditional Salary` where employee=%s""", (employee))
	...
	def make_activity_for_employee(self):
		activity_type = vmraid.get_doc("Activity Type", "_Test Activity Type")
...
def make_employee_salary_slip(user, payroll_frequency, salary_structure=None):
	...
	salary_slip_name = vmraid.db.get_value(
		"Salary Slip", {"employee": vmraid.db.get_value("Employee", {"user_id": user})}
	)
	...
	if not salary_slip_name:
		...
		salary_slip = vmraid.get_doc("Salary Slip", salary_slip_name)
...
def make_salary_component(salary_components, test_tax, company_list=None):
	for salary_component in salary_components:
		if vmraid.db.exists("Salary Component", salary_component["salary_component"]):
			continue
		...
		doc = vmraid.new_doc("Salary Component")
...
def get_salary_component_account(sal_comp, company_list=None):
	...
	if not isinstance(sal_comp, Document):
		sal_comp = vmraid.get_doc("Salary Component", sal_comp)

	if not sal_comp.get("accounts"):
		for d in company_list:
			company_abbr = vmraid.get_cached_value("Company", d, "abbr")
...
def create_account(account_name, company, parent_account, account_type=None):
	...
	account = vmraid.db.get_value("Account", account_name + " - " + company_abbr)
	if not account:
		vmraid.get_doc(
			{
				"doctype": "Account",
...
def create_proof_submission(employee, payroll_period, amount):
	...
	proof_submission = vmraid.get_doc(
		{
...
def create_tax_slab(
	...
	if company:
		currency = erpadda.get_company_currency(company)
	...
	income_tax_slab_name = vmraid.db.get_value("Income Tax Slab", {"currency": currency})
...
def make_leave_application(
	...
	leave_application = vmraid.get_doc(
		dict(
			...
			company=company or erpadda.get_default_company() or "_Test Company",
			status="Approved",
...
def setup_test():
	...
	vmraid.db.set_value(
		"Company", erpadda.get_default_company(), "default_holiday_list", "Salary Slip Test Holiday List"
	)
	vmraid.db.set_value("Payroll Settings", None, "email_salary_slip_to_employee", 0)
	vmraid.db.set_value("HR Settings", None, "leave_status_notification_template", None)
	vmraid.db.set_value("HR Settings", None, "leave_approval_notification_template", None)


def make_holiday_list(list_name=None, from_date=None, to_date=None):
	fiscal_year = get_fiscal_year(nowdate(), company=erpadda.get_default_company())
	name = list_name or "Salary Slip Test Holiday List"

	vmraid.delete_doc_if_exists("Holiday List", name, force=True)

	holiday_list = vmraid.get_doc(
		{
...
def make_salary_structure_for_payment_days_based_component_dependency():
	...
	if vmraid.db.exists("Salary Structure", salary_structure):
		vmraid.db.delete("Salary Structure", salary_structure)
	...
	salary_structure_doc = vmraid.get_doc(details)
...
def make_salary_slip_for_payment_days_dependency_test(employee, salary_structure):
	...
	salary_slip_name = vmraid.db.get_value("Salary Slip", {"employee": employee.name})
