...
from erpadda.hr.utils import get_holiday_dates_for_employee, validate_active_employee
from erpadda.loan_management.doctype.loan_repayment.loan_repayment import (
	calculate_amounts,
	create_repayment_entry,
)
from erpadda.payroll.doctype.additional_salary.additional_salary import get_additional_salaries
from erpadda.payroll.doctype.employee_benefit_application.employee_benefit_application import (
	get_benefit_component_amount,
)
from erpadda.payroll.doctype.employee_benefit_claim.employee_benefit_claim import (
	get_benefit_claim_amount,
	get_last_payroll_period_benefits,
)
from erpadda.payroll.doctype.payroll_entry.payroll_entry import get_start_end_dates
from erpadda.payroll.doctype.payroll_period.payroll_period import (
	get_payroll_period,
	get_period_factor,
)
...
class SalarySlip(TransactionBase):
	...
	def validate(self):
		...
		if vmraid.db.get_single_value("Payroll Settings", "max_working_hours_against_timesheet"):
			max_working_hours = vmraid.db.get_single_value(
	...
	def set_net_total_in_words(self):
		...
		company_currency = erpadda.get_company_currency(self.company)
	...
	def on_submit(self):
		if self.net_pay < 0:
			vmraid.throw(_("Net Pay cannot be less than 0"))
		else:
			self.set_status()
			self.update_status(self.name)
			self.make_loan_repayment_entry()
			if (
				vmraid.db.get_single_value("Payroll Settings", "email_salary_slip_to_employee")
			) and not vmraid.flags.via_payroll_entry:
				self.email_salary_slip()

		self.update_payment_status_for_gratuity()

	def update_payment_status_for_gratuity(self):
		add_salary = vmraid.db.get_all(
			"Additional Salary",
			filters={
		...
		if len(add_salary):
			...
			if add_salary[0].name in [data.additional_salary for data in self.earnings]:
				vmraid.db.set_value("Gratuity", add_salary.ref_docname, "status", status)
	...
	def validate_dates(self, joining_date=None, relieving_date=None):
		...
		if not joining_date:
			joining_date, relieving_date = vmraid.get_cached_value(
				"Employee", self.employee, ("date_of_joining", "relieving_date")
			)

		if date_diff(self.end_date, joining_date) < 0:
			vmraid.throw(_("Cannot create Salary Slip for Employee joining after Payroll Period"))

		if relieving_date and date_diff(relieving_date, self.start_date) < 0:
			vmraid.throw(_("Cannot create Salary Slip for Employee who has left before Payroll Period"))

	def is_rounding_total_disabled(self):
		return cint(vmraid.db.get_single_value("Payroll Settings", "disable_rounded_total"))
	...
	def check_existing(self):
		if not self.salary_slip_based_on_timesheet:
			...
			for data in self.timesheets:
				if vmraid.db.get_value("Timesheet", data.time_sheet, "status") == "Payrolled":
					vmraid.throw(
	...
	def get_working_days_details(
		...
		payroll_based_on = vmraid.db.get_value("Payroll Settings", None, "payroll_based_on")
		include_holidays_in_total_working_days = vmraid.db.get_single_value(
		...
		if not cint(include_holidays_in_total_working_days):
			...
			if working_days < 0:
				vmraid.throw(_("There are more holidays than working days this month."))

		if not payroll_based_on:
			vmraid.throw(_("Please set Payroll based on in Payroll settings"))
		...
		if flt(payment_days) > flt(lwp):
			...
			consider_unmarked_attendance_as = (
				vmraid.db.get_value("Payroll Settings", None, "consider_unmarked_attendance_as") or "Present"
			)
	...
	def get_unmarked_days_based_on_doj_or_relieving(
		...
		"""
		from erpadda.hr.doctype.employee.employee import is_holiday
	...
	def get_payment_days(self, joining_date, relieving_date, include_holidays_in_total_working_days):
		...
		if relieving_date:
			if getdate(self.start_date) <= relieving_date <= getdate(self.end_date):
				...
				vmraid.throw(_("Employee relieved on {0} must be set as 'Left'").format(relieving_date))
	...
	def calculate_lwp_or_ppl_based_on_leave_application(self, holidays, working_days):
		...
		daily_wages_fraction_for_half_day = (
			flt(vmraid.db.get_value("Payroll Settings", None, "daily_wages_fraction_for_half_day")) or 0.5
		)
	...
	def add_earning_for_hourly_wages(self, doc, salary_component, amount):
		...
		if not row_exists:
			wages_row = {
				...
				"abbr": vmraid.db.get_value("Salary Component", salary_component, "salary_component_abbr"),
	...
	def calculate_component_amounts(self, component_type):
		if not getattr(self, "_salary_structure_doc", None):
			self._salary_structure_doc = vmraid.get_doc("Salary Structure", self.salary_structure)
	...
	def get_data_for_eval(self):
		...
		data = vmraid._dict()
		employee = vmraid.get_doc("Employee", self.employee).as_dict()
		...
		salary_structure_assignment = vmraid.get_value(
		...
		if not salary_structure_assignment:
			vmraid.throw(
				_(
					...
					vmraid.bold(formatdate(date_to_validate)),
				)
			)
		...
		salary_components = vmraid.get_all("Salary Component", fields=["salary_component_abbr"])
	...
	def eval_condition_and_formula(self, d, data):
		try:
			...
			if condition:
				if not vmraid.safe_eval(condition, self.whitelisted_globals, data):
					return None
			amount = d.amount
			if d.amount_based_on_formula:
				formula = d.formula.strip().replace("\n", " ") if d.formula else None
				if formula:
					amount = flt(vmraid.safe_eval(formula, self.whitelisted_globals, data), d.precision("amount"))
			if amount:
				data[d.abbr] = amount
			...
			vmraid.throw(_("Syntax error in formula or condition: {0}").format(err))
		except Exception as e:
			vmraid.throw(_("Error in formula or condition: {0}").format(e))
			raise
			...
			if getdate(payroll_period.end_date) <= getdate(self.end_date):
				...
				if last_benefits:
					for last_benefit in last_benefits:
						last_benefit = vmraid._dict(last_benefit)
						amount = last_benefit.amount
						self.update_component_row(vmraid._dict(last_benefit.struct_row), amount, "earnings")
	...
	def add_tax_components(self, payroll_period):
		...
		if not tax_components:
			tax_components = [
				d.name
				for d in vmraid.get_all("Salary Component", filters={"variable_based_on_taxable_salary": 1})
	...
	def get_income_tax_slabs(self, payroll_period):
		income_tax_slab, ss_assignment_name = vmraid.db.get_value(
			"Salary Structure Assignment",
		...
		income_tax_slab_doc = vmraid.get_doc("Income Tax Slab", income_tax_slab)
		if income_tax_slab_doc.disabled:
			vmraid.throw(_("Income Tax Slab: {0} is disabled").format(income_tax_slab))
	...
	def get_taxable_earnings_for_prev_period(self, start_date, end_date, allow_tax_exemption=False):
		taxable_earnings = vmraid.db.sql(
			"""
	...
	def get_future_recurring_additional_amount(
		...
		to_date = vmraid.db.get_value("Additional Salary", additional_salary, "to_date")
	...
	def get_amount_based_on_payment_days(self, row, joining_date, relieving_date):
		...
		timesheet_component = vmraid.db.get_value(
	...
	def get_total_exemption_amount(self, payroll_period, tax_slab):
		...
		if tax_slab.allow_tax_exemption:
			if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
				exemption_proof = vmraid.db.get_value(
				...
				declaration = vmraid.db.get_value(
	...
	def get_joining_and_relieving_dates(self):
		...
		if not joining_date:
			vmraid.throw(
				_("Please set the Date Of Joining for employee {0}").format(vmraid.bold(self.employee_name))
			)
			...
			if payment.total_payment > total_amount:
				vmraid.throw(
					_(
						...
						payment.idx,
						vmraid.bold(payment.total_payment),
						vmraid.bold(total_amount),
						vmraid.bold(payment.loan),
					)
				)
	...
	def cancel_loan_repayment_entry(self):
		for loan in self.loans:
			if loan.loan_repayment_entry:
				repayment_entry = vmraid.get_doc("Loan Repayment", loan.loan_repayment_entry)
				repayment_entry.cancel()

	def email_salary_slip(self):
		receiver = vmraid.db.get_value("Employee", self.employee, "prefered_email")
		payroll_settings = vmraid.get_single("Payroll Settings")
		...
		if receiver:
			email_args = {
				...
				"attachments": [
					vmraid.attach_print(self.doctype, self.name, file_name=self.name, password=password)
				],
				"reference_doctype": self.doctype,
				"reference_name": self.name,
			}
			if not vmraid.flags.in_test:
				enqueue(method=vmraid.sendmail, queue="short", timeout=300, is_async=True, **email_args)
			else:
	...
	def update_status(self, salary_slip=None):
		for data in self.timesheets:
			if data.time_sheet:
				timesheet = vmraid.get_doc("Timesheet", data.time_sheet)
	...
	def calculate_total_for_salary_slip_based_on_timesheet(self):
		...
		self.base_hour_rate = flt(self.hour_rate) * flt(self.exchange_rate)
		salary_component = vmraid.db.get_value(
	...
	def compute_year_to_date(self):
		...
		salary_slip_sum = vmraid.get_list(
			"Salary Slip",
	...
	def add_leave_balances(self):
		...
		if vmraid.db.get_single_value("Payroll Settings", "show_leave_balances_in_salary_slip"):
			from erpadda.hr.doctype.leave_application.leave_application import get_leave_details
...
def unlink_ref_doc_from_salary_slip(ref_no):
	...
	if linked_ss:
		for ss in linked_ss:
			ss_doc = vmraid.get_doc("Salary Slip", ss)
			vmraid.db.set_value("Salary Slip", ss_doc.name, "journal_entry", "")


def generate_password_for_pdf(policy_template, employee):
	employee = vmraid.get_doc("Employee", employee)
...
def get_payroll_payable_account(company, payroll_entry):
	if payroll_entry:
		payroll_payable_account = vmraid.db.get_value(
...
def eval_tax_slab_condition(condition, eval_globals=None, eval_locals=None):
	...
	try:
		...
		if condition:
			return vmraid.safe_eval(condition, eval_globals, eval_locals)
	except NameError as err:
		vmraid.throw(
			_("{0} <br> This error can be due to missing or deleted field.").format(err),
			title=_("Name error"),
		...
		vmraid.throw(_("Syntax error in condition: {0} in Income Tax Slab").format(err))
	except Exception as e:
		vmraid.throw(_("Error in formula or condition: {0} in Income Tax Slab").format(e))
		raise
