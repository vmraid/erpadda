...
from erpadda.hr.utils import get_previous_claimed_amount, validate_active_employee
from erpadda.payroll.doctype.employee_benefit_application.employee_benefit_application import (
	get_max_benefits,
)
from erpadda.payroll.doctype.payroll_period.payroll_period import get_payroll_period
...
class EmployeeBenefitClaim(Document):
	def validate(self):
		...
		if not max_benefits or max_benefits <= 0:
			vmraid.throw(_("Employee {0} has no maximum benefit amount").format(self.employee))
		payroll_period = get_payroll_period(
			self.claim_date, self.claim_date, vmraid.db.get_value("Employee", self.employee, "company")
		)
		if not payroll_period:
			vmraid.throw(
				_("{0} is not in a valid Payroll Period").format(
					vmraid.format(self.claim_date, dict(fieldtype="Date"))
				)
			)
...
def get_benefit_pro_rata_ratio_amount(employee, on_date, sal_struct):
	...
	for sal_struct_row in sal_struct.get("earnings"):
		try:
			...
			vmraid.throw(_("Unable to find Salary Component {0}").format(sal_struct_row.salary_component))
...
def get_total_benefit_dispensed(employee, sal_struct, sal_slip_start_date, payroll_period):
	...
	if application:
		application_obj = vmraid.get_doc("Employee Benefit Application", application)
		pro_rata_amount = (
	...
	if remaining_benefit > 0:
		...
		for d in sal_struct.get("earnings"):
			if d.is_flexible_benefit == 1:
				salary_component = vmraid.get_doc("Salary Component", d.salary_component)
