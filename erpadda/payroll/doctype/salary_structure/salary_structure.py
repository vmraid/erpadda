...
class SalaryStructure(Document):
	...
	def set_missing_values(self):
		...
		for table in ["earnings", "deductions"]:
			for d in self.get(table):
				component_default_value = vmraid.db.get_value(
					"Salary Component",
	...
	def validate_amount(self):
		if flt(self.net_pay) < 0 and self.salary_slip_based_on_timesheet:
			vmraid.throw(_("Net pay cannot be negative"))
		...
		if self.earnings:
			...
			for earning_component in self.earnings:
				if earning_component.is_flexible_benefit == 1:
					have_a_flexi = True
					max_of_component = vmraid.db.get_value(
			...
			if have_a_flexi and flt(self.max_benefits) == 0:
				vmraid.throw(_("Max benefits should be greater than zero to dispense benefits"))
	...
	def assign_salary_structure(
		...
		if employees:
			...
			vmraid.msgprint(_("No Employee Found"))
...
def assign_salary_structure_for_employees(
	...
	if salary_structures_assignments:
		vmraid.msgprint(_("Structures have been assigned successfully"))
...
def create_salary_structures_assignment(
	...
	if not payroll_payable_account:
		...
		if not payroll_payable_account:
			vmraid.throw(_('Please set "Default Payroll Payable Account" in Company Defaults'))
	payroll_payable_account_currency = vmraid.db.get_value(
		"Account", payroll_payable_account, "account_currency"
	)
	company_curency = erpadda.get_company_currency(salary_structure.company)
	if (
	...
	assignment = vmraid.new_doc("Salary Structure Assignment")
...
def get_existing_assignments(employees, salary_structure, from_date):
	salary_structures_assignments = vmraid.db.sql_list(
		"""
...
def make_salary_slip(
	...
	def postprocess(source, target):
		if employee:
			employee_details = vmraid.db.get_value(
	...
	if cint(as_print):
		...
		return vmraid.get_print(doc.doctype, doc.name, doc=doc, print_format=print_format)
	else:
		return doc
