...
import vmraid
from vmraid.utils import add_years, date_diff, get_first_day, nowdate
...
class TestSalaryStructure(unittest.TestCase):
	...
	def make_holiday_list(self):
		if not vmraid.db.get_value("Holiday List", "Salary Structure Test Holiday List"):
	...
	def test_amount_totals(self):
		...
		sal_slip = vmraid.get_value("Salary Slip", {"employee_name": "test_employee_2@salary.com"})
		if not sal_slip:
	...
	def test_salary_structures_assignment(self):
		company_currency = erpadda.get_default_currency()
		...
		salary_structure_assignment = vmraid.get_doc(
...
def make_salary_structure(
	...
	if test_tax:
		vmraid.db.sql("""delete from `tabSalary Structure` where name=%s""", (salary_structure))
	...
	if (
		employee
		and not vmraid.db.get_value("Salary Structure Assignment", filters)
...
def create_salary_structure_assignment(
	...
	if vmraid.db.exists("Salary Structure Assignment", {"employee": employee}):
		vmraid.db.sql("""delete from `tabSalary Structure Assignment` where employee=%s""", (employee))

	if not payroll_period:
		payroll_period = create_payroll_period()

	income_tax_slab = vmraid.db.get_value("Income Tax Slab", {"currency": currency})

	if not income_tax_slab:
		income_tax_slab = create_tax_slab(payroll_period, allow_tax_exemption=True, currency=currency)

	salary_structure_assignment = vmraid.new_doc("Salary Structure Assignment")
	...
	if not from_date:
		...
		joining_date = vmraid.get_cached_value("Employee", employee, "date_of_joining")
	...
	salary_structure_assignment.company = company or erpadda.get_default_company()
...
def get_payable_account(company=None):
	...
	return vmraid.db.get_value("Company", company, "default_payroll_payable_account")
