...
vmraid.ui.form.on('Salary Structure', {
	onload: function(frm) {
		...
		let help_button = $(`<a class = 'control-label'>
			...
			let message_html = vmraid.render_template("condition_and_formula_help")
	...
	refresh: function(frm) {
		...
		if(frm.doc.docstatus==1) {
			frm.add_custom_button(__("Assign Salary Structure"), function() {
				var doc = vmraid.model.get_new_doc('Salary Structure Assignment');
				doc.salary_structure = frm.doc.name;
				doc.company = frm.doc.company;
				vmraid.set_route('Form', 'Salary Structure Assignment', doc.name);
			});
	...
	preview_salary_slip: function(frm) {
		vmraid.call({
			method: "erpadda.payroll.doctype.salary_structure.salary_structure.get_employees",
			args: {
	...
	open_salary_slip: function(frm, employee){
		...
		vmraid.call({
			method: "erpadda.payroll.doctype.salary_structure.salary_structure.make_salary_slip",
			args: {
...
var validate_date = function(frm, cdt, cdn) {
	...
	if(doc.to_date && doc.from_date) {
		var from_date = vmraid.datetime.str_to_obj(doc.from_date);
		var to_date = vmraid.datetime.str_to_obj(doc.to_date);

		if(to_date < from_date) {
			vmraid.model.set_value(cdt, cdn, "to_date", "");
			vmraid.throw(__("From Date cannot be greater than To Date"));
		}
	}
}
...
vmraid.ui.form.on('Salary Detail', {
	amount: function(frm) {
		calculate_totals(frm.doc);
	},
			...
			vmraid.call({
				...
				callback: function(data) {
					if(data.message){
						...
						vmraid.model.set_value(cdt, cdn, 'condition', result.condition);
						vmraid.model.set_value(cdt, cdn, 'amount_based_on_formula', result.amount_based_on_formula);
						if(result.amount_based_on_formula == 1){
							vmraid.model.set_value(cdt, cdn, 'formula', result.formula);
						}
						else{
							vmraid.model.set_value(cdt, cdn, 'amount', result.amount);
						}
						vmraid.model.set_value(cdt, cdn, 'statistical_component', result.statistical_component);
						vmraid.model.set_value(cdt, cdn, 'depends_on_payment_days', result.depends_on_payment_days);
						vmraid.model.set_value(cdt, cdn, 'do_not_include_in_total', result.do_not_include_in_total);
						vmraid.model.set_value(cdt, cdn, 'variable_based_on_taxable_salary', result.variable_based_on_taxable_salary);
						vmraid.model.set_value(cdt, cdn, 'is_tax_applicable', result.is_tax_applicable);
						vmraid.model.set_value(cdt, cdn, 'is_flexible_benefit', result.is_flexible_benefit);
	...
	amount_based_on_formula: function(frm, cdt, cdn) {
		...
		if(child.amount_based_on_formula == 1){
			vmraid.model.set_value(cdt, cdn, 'amount', null);
		}
		else{
			vmraid.model.set_value(cdt, cdn, 'formula', null);
		}
	}
})
