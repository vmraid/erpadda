...
from vmraid.utils import add_days, cint, cstr, date_diff, getdate, rounded
...
class EmployeeBenefitApplication(Document):
		...
		if self.employee_benefits:
			for benefit in self.employee_benefits:
				if benefit.pay_against_benefit_claim == 1:
					payroll_period = vmraid.get_doc("Payroll Period", self.payroll_period)
	...
	def validate_remaining_benefit_amount(self):
		...
		if salary_struct_name:
			...
			pro_rata_amount = 0
			salary_structure = vmraid.get_doc("Salary Structure", salary_struct_name)
			if salary_structure.earnings:
				for earnings in salary_structure.earnings:
					if earnings.is_flexible_benefit == 1 and earnings.salary_component not in benefit_components:
						pay_against_benefit_claim, max_benefit_amount = vmraid.db.get_value(
							"Salary Component",
	...
	def validate_max_benefit(self, earning_component_name):
		max_benefit_amount = vmraid.db.get_value(
		...
		prev_sal_slip_flexi_amount = get_sal_slip_total_benefit_given(
			self.employee, vmraid.get_doc("Payroll Period", self.payroll_period), earning_component_name
		)
...
def get_max_benefits(employee, on_date):
	...
	if sal_struct:
		max_benefits = vmraid.db.get_value("Salary Structure", sal_struct, "max_benefits")
		if max_benefits > 0:
			return max_benefits
	return False
...
def get_max_benefits_remaining(employee, on_date, payroll_period):
	...
	if max_benefits and max_benefits > 0:
		...
		payroll_period_obj = vmraid.get_doc("Payroll Period", payroll_period)
		...
		if prev_sal_slip_flexi_total > 0:
			...
			if sal_struct_name:
				sal_struct = vmraid.get_doc("Salary Structure", sal_struct_name)
				for sal_struct_row in sal_struct.get("earnings"):
					salary_component = vmraid.get_doc("Salary Component", sal_struct_row.salary_component)
					if (
...
def get_benefit_component_amount(
	...
	benefit_application = vmraid.db.sql(
		"""
		select name
	...
	component_max_benefit, depends_on_payment_days = vmraid.db.get_value(
		"Salary Component", salary_component, ["max_benefit_amount", "depends_on_payment_days"]
	)

	benefit_amount = 0
	if benefit_application:
		benefit_amount = vmraid.db.get_value(
