...
vmraid.ui.form.on('Employee Benefit Application', {
	employee: function(frm) {
		...
		if(frm.doc.employee && frm.doc.date && frm.doc.payroll_period){
			method = "erpadda.payroll.doctype.employee_benefit_application.employee_benefit_application.get_max_benefits_remaining";
			args = {
			...
			method = "erpadda.payroll.doctype.employee_benefit_application.employee_benefit_application.get_max_benefits";
			args = {
				employee: frm.doc.employee,
				on_date: frm.doc.date
			};
	...
	set_earning_component: function(frm) {
		...
		frm.set_query("earning_component", "employee_benefits", function() {
			return {
				query : "erpadda.payroll.doctype.employee_benefit_application.employee_benefit_application.get_earning_components",
...
vmraid.ui.form.on("Employee Benefit Application Detail",{
	amount:  function(frm) {
		calculate_all(frm.doc);
	},
