...
class DuplicateAssignment(vmraid.ValidationError):
	pass
...
class SalaryStructureAssignment(Document):
	...
	def validate_dates(self):
		...
		if self.from_date:
			if vmraid.db.exists(
				...
				vmraid.throw(_("Salary Structure Assignment for Employee already exists"), DuplicateAssignment)
	...
	def validate_income_tax_slab(self):
		...
		income_tax_slab_currency = vmraid.db.get_value(
			...
			if not payroll_payable_account:
				payroll_payable_account = vmraid.db.get_value(
					"Account",
					{
						...
						"account_currency": vmraid.db.get_value("Company", self.company, "default_currency"),
						"is_group": 0,
					},
				)
	...
	def get_payroll_cost_center(self):
		payroll_cost_center = vmraid.db.get_value("Employee", self.employee, "payroll_cost_center")
		if not payroll_cost_center and self.department:
			payroll_cost_center = vmraid.db.get_value("Department", self.department, "payroll_cost_center")
	...
	def validate_cost_center_distribution(self):
		if self.get("payroll_cost_centers"):
			...
			if total_percentage != 100:
				vmraid.throw(_("Total percentage against cost centers should be 100"))


def get_assigned_salary_structure(employee, on_date):
	if not employee or not on_date:
		return None
	salary_structure = vmraid.db.sql(
		"""
...
def get_employee_currency(employee):
	employee_currency = vmraid.db.get_value(
