...
from vmraid.utils import add_months

import erpadda
from erpadda.accounts.utils import get_fiscal_year, getdate, nowdate
...
from erpadda.payroll.doctype.payroll_entry.payroll_entry import get_end_date, get_start_end_dates
...
class TestPayrollEntry(unittest.TestCase):
	...
	def setUpClass(cls):
		vmraid.db.set_value(
			"Company", erpadda.get_default_company(), "default_holiday_list", "_Test Holiday List"
		)
	...
	def test_payroll_entry(self):  # pylint: disable=no-self-use
		...
		for data in vmraid.get_all("Salary Component", fields=["name"]):
			if not vmraid.db.get_value(
				"Salary Component Account", {"parent": data.name, "company": company}, "name"
			):
				get_salary_component_account(data.name)

		employee = vmraid.db.get_value("Employee", {"company": company})
		company_doc = vmraid.get_doc("Company", company)
		make_salary_structure(
	...
	def test_multi_currency_payroll_entry(self):  # pylint: disable=no-self-use
		...
		vmraid.db.sql(
			...
			(vmraid.db.get_value("Employee", {"user_id": "test_muti_currency_employee@payroll.com"})),
		)
		...
		if payroll_je:
			payroll_je_doc = vmraid.get_doc("Journal Entry", payroll_je)

			self.assertEqual(salary_slip.base_gross_pay, payroll_je_doc.total_debit)
			self.assertEqual(salary_slip.base_gross_pay, payroll_je_doc.total_credit)

		payment_entry = vmraid.db.sql(
			"""
	...
	def test_payroll_entry_with_employee_cost_center(self):  # pylint: disable=no-self-use
		...
		if not vmraid.db.exists("Department", "cc - _TC"):
			vmraid.get_doc(
				{"doctype": "Department", "department_name": "cc", "company": "_Test Company"}
			).insert()

		vmraid.db.sql("""delete from `tabEmployee` where employee_name='test_employee1@example.com' """)
		vmraid.db.sql("""delete from `tabEmployee` where employee_name='test_employee2@example.com' """)
		vmraid.db.sql("""delete from `tabSalary Structure` where name='_Test Salary Structure 1' """)
		vmraid.db.sql("""delete from `tabSalary Structure` where name='_Test Salary Structure 2' """)
		...
		if not vmraid.db.exists("Account", "_Test Payroll Payable - _TC"):
			create_account(
		...
		if (
			not vmraid.db.get_value("Company", "_Test Company", "default_payroll_payable_account")
			or vmraid.db.get_value("Company", "_Test Company", "default_payroll_payable_account")
			!= "_Test Payroll Payable - _TC"
		):
			vmraid.db.set_value(
				"Company", "_Test Company", "default_payroll_payable_account", "_Test Payroll Payable - _TC"
			)
		currency = vmraid.db.get_value("Company", "_Test Company", "default_currency")
		...
		ssa = vmraid.db.get_value(
			"Salary Structure Assignment",
			{"employee": employee2, "salary_structure": ss.name, "docstatus": 1},
			"name",
		)

		ssa_doc = vmraid.get_doc("Salary Structure Assignment", ssa)
		...
		if not vmraid.db.get_value(
			...
			je = vmraid.db.get_value("Salary Slip", {"payroll_entry": pe.name}, "journal_entry")
			je_entries = vmraid.db.sql(
				"""
	...
	def test_loan(self):
		...
		if not company_doc.default_payroll_payable_account:
			company_doc.default_payroll_payable_account = vmraid.db.get_value(
				"Account", {"company": company, "root_type": "Liability", "account_type": ""}, "name"
			)
			company_doc.save()

		if not vmraid.db.exists("Branch", branch):
			vmraid.get_doc({"doctype": "Branch", "branch": branch}).insert()

		employee_doc = vmraid.get_doc("Employee", applicant)
		...
		if not vmraid.db.exists("Loan Type", "Car Loan"):
			create_loan_accounts()
			create_loan_type(
				"Car Loan",
				500000,
		...
		salary_slip = vmraid.get_doc("Salary Slip", name)
		...
		if salary_slip.docstatus == 0:
			vmraid.delete_doc("Salary Slip", name)


def make_payroll_entry(**args):
	args = vmraid._dict(args)

	payroll_entry = vmraid.new_doc("Payroll Entry")
	payroll_entry.company = args.company or erpadda.get_default_company()
...
def get_payment_account():
	return vmraid.get_value(
		"Account",
		{"account_type": "Cash", "company": erpadda.get_default_company(), "is_group": 0},
		"name",
	)


def make_holiday(holiday_list_name):
	if not vmraid.db.exists("Holiday List", holiday_list_name):
