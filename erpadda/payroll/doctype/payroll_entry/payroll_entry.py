...
class PayrollEntry(Document):
	def onload(self):
		...
		entries = vmraid.db.count("Salary Slip", {"payroll_entry": self.name, "docstatus": 1}, ["name"])
	...
	def before_submit(self):
		...
		if self.validate_attendance:
			if self.validate_employee_attendance():
				vmraid.throw(_("Cannot Submit, Employees left to mark attendance"))
	...
	def validate_employee_details(self):
		...
		if len(emp_with_sal_slip):
			vmraid.throw(_("Salary Slip already exists for {0}").format(comma_and(emp_with_sal_slip)))
	...
	def fill_employee_details(self):
		...
		if not employees:
			error_msg = _(
				...
				vmraid.bold(self.currency),
				vmraid.bold(self.payroll_payable_account),
			)
			if self.branch:
				error_msg += "<br>" + _("Branch: {0}").format(vmraid.bold(self.branch))
			if self.department:
				error_msg += "<br>" + _("Department: {0}").format(vmraid.bold(self.department))
			if self.designation:
				error_msg += "<br>" + _("Designation: {0}").format(vmraid.bold(self.designation))
			if self.start_date:
				error_msg += "<br>" + _("Start date: {0}").format(vmraid.bold(self.start_date))
			if self.end_date:
				error_msg += "<br>" + _("End date: {0}").format(vmraid.bold(self.end_date))
			vmraid.throw(error_msg, title=_("No employees found"))
	...
	def check_mandatory(self):
		for fieldname in ["company", "start_date", "end_date"]:
			if not self.get(fieldname):
				vmraid.throw(_("Please set {0}").format(self.meta.get_label(fieldname)))
	...
	def create_salary_slips(self):
		...
		if employees:
			...
			if len(employees) > 30:
				vmraid.enqueue(create_salary_slips_for_employees, timeout=600, employees=employees, args=args)
			else:
	...
	def get_sal_slip_list(self, ss_status, as_dict=False):
		...
		ss = vmraid.qb.DocType("Salary Slip")
		ss_list = (
			vmraid.qb.from_(ss)
	...
	def email_salary_slip(self, submitted_ss):
		if vmraid.db.get_single_value("Payroll Settings", "email_salary_slip_to_employee"):
	...
	def get_salary_component_account(self, salary_component):
		...
		if not account:
			vmraid.throw(_("Please set account in Salary Component {0}").format(salary_component))
	...
	def get_salary_components(self, component_type):
		...
		if salary_slips:
			ss = vmraid.qb.DocType("Salary Slip")
			ssd = vmraid.qb.DocType("Salary Detail")
			salary_components = (
				vmraid.qb.from_(ss)
				.join(ssd)
		...
		if salary_components:
			...
			for item in salary_components:
				...
				if component_type == "earnings":
					is_flexible_benefit, only_tax_impact = vmraid.get_cached_value(
	...
	def get_payroll_cost_centers_for_employee(self, employee, salary_structure):
		if not self.employee_cost_centers.get(employee):
			ss_assignment_name = vmraid.db.get_value(
			...
			if ss_assignment_name:
				...
				if not cost_centers:
					default_cost_center, department = vmraid.get_cached_value(
						"Employee", employee, ["payroll_cost_center", "department"]
					)
					if not default_cost_center and department:
						default_cost_center = vmraid.get_cached_value(
	...
	def make_payment_entry(self):
		...
		salary_slip_name_list = vmraid.db.sql(
		...
		if salary_slip_name_list and len(salary_slip_name_list) > 0:
			...
			for salary_slip_name in salary_slip_name_list:
				salary_slip = vmraid.get_doc("Salary Slip", salary_slip_name[0])
				...
				for sal_detail in salary_slip.deductions:
					statistical_component = vmraid.db.get_value(
	...
	def update_salary_slip_status(self, jv_name=None):
		...
		for ss in ss_list:
			ss_obj = vmraid.get_doc("Salary Slip", ss[0])
			vmraid.db.set_value("Salary Slip", ss_obj.name, "journal_entry", jv_name)
	...
	def validate_employee_attendance(self):
		...
		for employee_detail in self.employees:
			employee_joining_date = vmraid.db.get_value(
...
def get_filter_condition(filters):
	...
	for f in ["company", "branch", "department", "designation"]:
		if filters.get(f):
			cond += " and t1." + f + " = " + vmraid.db.escape(filters.get(f))
...
def get_start_end_dates(payroll_frequency, start_date=None, company=None):
	...
	return vmraid._dict({"start_date": start_date, "end_date": end_date})
...
def get_month_details(year, month):
	ysd = vmraid.db.get_value("Fiscal Year", year, "year_start_date")
	if ysd:
		import calendar
		import datetime
		...
		vmraid.throw(_("Fiscal Year {0} not found").format(year))
...
def create_salary_slips_for_employees(employees, args, publish_progress=True):
	...
	payroll_entry = vmraid.get_doc("Payroll Entry", args.payroll_entry)
	...
	if salary_slips_not_created:
		vmraid.msgprint(
			...
			).format(vmraid.bold(", ".join([emp for emp in salary_slips_not_created]))),
			title=_("Message"),
			indicator="orange",
		)
...
def submit_salary_slips_for_employees(payroll_entry, salary_slips, publish_progress=True):
	submitted_ss = []
	not_submitted_ss = []
	vmraid.flags.via_payroll_entry = True
	...
	for ss in salary_slips:
		...
		count += 1
		if publish_progress:
			vmraid.publish_progress(count * 100 / len(salary_slips), title=_("Submitting Salary Slips..."))
	if submitted_ss:
	...
	if not_submitted_ss:
		vmraid.msgprint(_("Could not submit some Salary Slips"))

	vmraid.flags.via_payroll_entry = False
...
def employee_query(doctype, txt, searchfield, start, page_len, filters):
	filters = vmraid._dict(filters)
	conditions = []
	include_employees = []
	emp_cond = ""

	if not filters.payroll_frequency:
		vmraid.throw(_("Select Payroll Frequency."))
