...
from vmraid.utils import add_months, cint, date_diff, flt, formatdate, getdate, month_diff
...
def get_payroll_period_days(start_date, end_date, employee, company=None):
	...
	if len(payroll_period) > 0:
		...
		if not cint(
			vmraid.db.get_value("Payroll Settings", None, "include_holidays_in_total_working_days")
		):
