...
from vmraid.utils import add_days, add_months, nowdate

import erpadda
from erpadda.hr.doctype.employee.test_employee import make_employee
from erpadda.payroll.doctype.salary_component.test_salary_component import create_salary_component
...
class TestAdditionalSalary(VMRaidTestCase):
	def setUp(self):
		setup_test()
	...
	def test_recurring_additional_salary(self):
		...
		vmraid.db.set_value("Employee", emp_id, "relieving_date", add_days(nowdate(), 1800))
	...
	def test_non_recurring_additional_salary(self):
		...
		emp_id = make_employee("test_additional@salary.com")
		vmraid.db.set_value("Employee", emp_id, "relieving_date", add_days(date, 1800))
		salary_structure = make_salary_structure(
			"Test Salary Structure Additional Salary", "Monthly", employee=emp_id
		)
...
def get_additional_salary(emp_id, recurring=True, payroll_date=None):
	...
	add_sal = vmraid.new_doc("Additional Salary")
	...
	add_sal.amount = 5000
	add_sal.currency = erpadda.get_default_currency()
	add_sal.save()
	add_sal.submit()
