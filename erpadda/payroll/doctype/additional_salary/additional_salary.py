...
from vmraid.utils import comma_and, date_diff, formatdate, getdate
...
class AdditionalSalary(Document):
	...
	def validate(self):
		...
		if self.amount < 0:
			vmraid.throw(_("Amount should not be less than zero"))
	...
	def validate_recurring_additional_salary_overlap(self):
		if self.is_recurring:
			additional_salaries = vmraid.db.sql(
				"""
				SELECT
					name
	...
	def validate_dates(self):
		date_of_joining, relieving_date = vmraid.db.get_value(
			"Employee", self.employee, ["date_of_joining", "relieving_date"]
		)

		if getdate(self.from_date) > getdate(self.to_date):
			vmraid.throw(_("From Date can not be greater than To Date."))

		if date_of_joining:
			if self.payroll_date and getdate(self.payroll_date) < getdate(date_of_joining):
				vmraid.throw(_("Payroll date can not be less than employee's joining date."))
		...
		if relieving_date:
			if self.to_date and getdate(self.to_date) > getdate(relieving_date):
				vmraid.throw(_("To date can not be greater than employee's relieving date."))
			if self.payroll_date and getdate(self.payroll_date) > getdate(relieving_date):
				vmraid.throw(_("Payroll date can not be greater than employee's relieving date."))

	def validate_employee_referral(self):
		if self.ref_doctype == "Employee Referral":
			referral_details = vmraid.db.get_value(
				"Employee Referral",
				self.ref_docname,
			...
			if self.type == "Deduction":
				vmraid.throw(_("Earning Salary Component is required for Employee Referral Bonus."))
			...
			if referral_details.status != "Accepted":
				vmraid.throw(
					...
					).format(vmraid.bold("Accepted"))
				)

	def update_return_amount_in_employee_advance(self):
		if self.ref_doctype == "Employee Advance" and self.ref_docname:
			return_amount = vmraid.db.get_value("Employee Advance", self.ref_docname, "return_amount")
			...
			vmraid.db.set_value("Employee Advance", self.ref_docname, "return_amount", return_amount)
			advance = vmraid.get_doc("Employee Advance", self.ref_docname)
			advance.set_status(update=True)

	def update_employee_referral(self, cancel=False):
		if self.ref_doctype == "Employee Referral":
			status = "Unpaid" if cancel else "Paid"
			vmraid.db.set_value("Employee Referral", self.ref_docname, "referral_payment_status", status)
...
def get_additional_salaries(employee, start_date, end_date, component_type):
	...
	additional_sal = vmraid.qb.DocType("Additional Salary")
	...
	additional_salary_list = (
		vmraid.qb.from_(additional_sal)
		.select(
			additional_sal.name,
			component_field,
			additional_sal.type,
		...
		if d.overwrite:
			if d.component in components_to_overwrite:
				vmraid.throw(
					...
					).format(vmraid.bold(d.component), start_date, end_date),
					title=_("Error"),
				)
