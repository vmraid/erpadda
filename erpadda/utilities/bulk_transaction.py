...
def job(deserialized_data, from_doctype, to_doctype):
	...
	for d in deserialized_data:
		...
		try:
			i += 1
			doc_name = d.get("name")
			vmraid.db.savepoint("before_creation_state")
			task(doc_name, from_doctype, to_doctype)

		except Exception as e:
			vmraid.db.rollback(save_point="before_creation_state")
...
def task(doc_name, from_doctype, to_doctype):
	from erpadda.accounts.doctype.payment_entry import payment_entry
	from erpadda.accounts.doctype.purchase_invoice import purchase_invoice
	from erpadda.accounts.doctype.sales_invoice import sales_invoice
	from erpadda.buying.doctype.purchase_order import purchase_order
	from erpadda.buying.doctype.supplier_quotation import supplier_quotation
	from erpadda.selling.doctype.quotation import quotation
	from erpadda.selling.doctype.sales_order import sales_order
	from erpadda.stock.doctype.delivery_note import delivery_note
	from erpadda.stock.doctype.purchase_receipt import purchase_receipt
...
def check_logger_doc_exists(log_date):
	return vmraid.db.exists("Bulk Transaction Log", log_date)


def get_logger_doc(log_date):
	return vmraid.get_doc("Bulk Transaction Log", log_date)


def create_logger_doc():
	log_doc = vmraid.new_doc("Bulk Transaction Log")
