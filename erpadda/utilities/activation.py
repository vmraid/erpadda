...
def get_level():
	...
	for doctype, min_count in doctypes.items():
		count = vmraid.db.count(doctype)
		if count > min_count:
			activation_level += 1
		sales_data.append({doctype: count})

	if vmraid.db.get_single_value("System Settings", "setup_complete"):
		activation_level += 1

	communication_number = vmraid.db.count("Communication", dict(communication_medium="Email"))
...
def get_help_messages():
	...
	domain = vmraid.get_cached_value("Company", erpadda.get_default_company(), "domain")
	messages = []
	...
	for m in message_settings:
		if not m.domain or domain in m.domain:
			m.count = vmraid.db.count(m.doctype)
			if m.count < m.target:
				messages.append(m)
