...
from vmraid.utils import cint, cstr, flt, get_time, now_datetime

from erpadda.controllers.status_updater import StatusUpdater


class UOMMustBeIntegerError(vmraid.ValidationError):
	pass
...
class TransactionBase(StatusUpdater):
	def validate_posting_time(self):
		...
		if vmraid.flags.in_import and self.posting_date:
		...
		if not getattr(self, "set_posting_time", None):
			...
			try:
				...
				vmraid.throw(_("Invalid Posting Time"))
		...
		if participations:
			for participation in participations:
				...
				if len(total_participants) <= 1:
					vmraid.db.sql("delete from `tabEvent` where name='%s'" % participation.parent)

				vmraid.db.sql("delete from `tabEvent Participants` where name='%s'" % participation.name)

	def _add_calendar_event(self, opts):
		opts = vmraid._dict(opts)
		...
		if self.contact_date:
			...
			if vmraid.db.exists("User", self.contact_by):
				vmraid.share.add("Event", event.name, self.contact_by, flags={"ignore_share_permission": True})
				...
				if ref_dn:
					if is_child:
						...
						if ref_dn not in item_ref_dn:
							...
							vmraid.throw(_("Duplicate row {0} with same {1}").format(d.idx, key))
					elif ref_dn:
	...
	def compare_values(self, ref_doc, fields, doc=None):
		for reference_doctype, ref_dn_list in ref_doc.items():
			for reference_name in ref_dn_list:
				prevdoc_values = vmraid.db.get_value(
					reference_doctype, reference_name, [d[0] for d in fields], as_dict=1
				)

				if not prevdoc_values:
					vmraid.throw(_("Invalid reference {0} {1}").format(reference_doctype, reference_name))
	...
	def validate_rate_with_reference_doc(self, ref_details):
		...
		if self.doctype in buying_doctypes:
			action = vmraid.db.get_single_value("Buying Settings", "maintain_same_rate_action")
			settings_doc = "Buying Settings"
		else:
			action = vmraid.db.get_single_value("Selling Settings", "maintain_same_rate_action")
			settings_doc = "Selling Settings"

		for ref_dt, ref_dn_field, ref_link_field in ref_details:
			for d in self.get("items"):
				if d.get(ref_link_field):
					ref_rate = vmraid.db.get_value(ref_dt + " Item", d.get(ref_link_field), "rate")

					if abs(flt(d.rate - ref_rate, d.precision("rate"))) >= 0.01:
						if action == "Stop":
							role_allowed_to_override = vmraid.db.get_single_value(
								settings_doc, "role_to_override_stop_action"
							)

							if role_allowed_to_override not in vmraid.get_roles():
								vmraid.throw(
...
def delete_events(ref_type, ref_name):
	...
	if events:
		vmraid.delete_doc("Event", events, for_reload=True)
...
def validate_uom_is_integer(doc, uom_field, qty_fields, child_dt=None):
	...
	integer_uoms = list(
		filter(
			lambda uom: vmraid.db.get_value("UOM", uom, "must_be_whole_number", cache=True) or None,
			distinct_uoms,
		)
	)
					...
					if abs(cint(qty) - flt(qty)) > 0.0000001:
						vmraid.throw(
							_(
								...
								qty, d.idx, vmraid.bold(_("Must be Whole Number")), vmraid.bold(d.get(uom_field))
							),
