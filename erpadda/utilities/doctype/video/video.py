...
class Video(Document):
	...
	def set_youtube_statistics(self):
		api_key = vmraid.db.get_single_value("Video Settings", "api_key")
...
def is_tracking_enabled():
	return vmraid.db.get_single_value("Video Settings", "enable_youtube_tracking")


def get_frequency(value):
	# Return numeric value from frequency field, return 1 as fallback default value: 1 hour
	if value != "Daily":
		return vmraid.utils.cint(value[:2].strip())
	elif value:
		return 24
	return 1
...
def update_youtube_data():
	...
	enable_youtube_tracking, frequency = vmraid.db.get_value(
	...
	time = datetime.now()
	timezone = pytz.timezone(vmraid.utils.get_time_zone())
...
def get_id_from_url(url):
	...
	if not isinstance(url, str):
		vmraid.throw(_("URL can only be a string"), title=_("Invalid URL"))
...
def batch_update_youtube_data():
	...
	video_list = vmraid.get_all("Video", fields=["youtube_video_id"])
