...
def get_current_student():
	"""Returns current student from vmraid.session.user
...
def get_portal_programs():
	...
	"""
	published_programs = vmraid.get_all("Program", filters={"is_published": True})
	if not published_programs:
		return None

	program_list = [vmraid.get_doc("Program", program) for program in published_programs]
	portal_programs = [
...
def enroll_in_program(program_name, student=None):
	...
	if not student == None:
		student = vmraid.get_doc("Student", student)
	else:
		# Check if self enrollment in allowed
		program = vmraid.get_doc("Program", program_name)
		if not program.allow_self_enroll:
			return vmraid.throw(_("You are not allowed to enroll for this course"))
...
def has_super_access():
	...
	"""
	current_user = vmraid.get_doc("User", vmraid.session.user)
...
def add_activity(course, content_type, content, program):
	...
	if not student:
		return vmraid.throw(
			_("Student with email {0} does not exist").format(vmraid.session.user), vmraid.DoesNotExistError
		)
...
def evaluate_quiz(quiz_response, quiz_name, course, program, time_taken):
	...
	quiz = vmraid.get_doc("Quiz", quiz_name)
...
def get_quiz(quiz_name, course):
	try:
		...
		vmraid.throw(_("Quiz {0} does not exist").format(quiz_name), vmraid.DoesNotExistError)
		return None
...
def get_course_progress(course, program):
	...
	for course_topic in course.topics:
		topic = vmraid.get_doc("Topic", course_topic.topic)
...
def get_program_progress(program):
	...
	for program_course in program.courses:
		course = vmraid.get_doc("Course", program_course.course)
...
def get_program_completion(program):
	...
	progress = []
	for topic in topics:
		topic_doc = vmraid.get_doc("Topic", topic[0])
...
def get_or_create_course_enrollment(course, program):
	...
	if not course_enrollment:
		...
		if not program_enrollment:
			vmraid.throw(_("You are not enrolled in program {0}").format(program))
			return
		return student.enroll_in_course(
			course_name=course, program_enrollment=get_enrollment("program", program.name, student.name)
		)
	else:
		return vmraid.get_doc("Course Enrollment", course_enrollment)
