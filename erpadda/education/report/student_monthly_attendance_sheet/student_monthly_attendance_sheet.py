...
from vmraid.utils import add_days, cstr, date_diff, get_first_day, get_last_day, getdate
...
def execute(filters=None):
	...
	for stud in students:
		...
		student_status = vmraid.db.get_value("Student", stud.student, "enabled")
		date = from_date
		total_p = total_a = 0.0
...
def get_attendance_list(from_date, to_date, student_group, students_list):
	...
	for d in attendance_list:
		att_map.setdefault(d.student, vmraid._dict()).setdefault(d.date, "")
	...
	for dt in daterange(getdate(from_date), getdate(to_date)):
		if dt in holidays:
			for student in students_list:
				att_map.setdefault(student, vmraid._dict()).setdefault(dt, "Holiday")
