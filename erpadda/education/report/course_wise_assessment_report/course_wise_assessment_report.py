...
def execute(filters=None):
	...
	if args["assessment_group"] == "All Assessment Groups":
		vmraid.throw(_("Please select the assessment group other than 'All Assessment Groups'"))
	...
	for student in result_dict:
		...
		for criteria in assessment_criteria_dict:
			scrub_criteria = vmraid.scrub(criteria)
			if criteria in result_dict[student][args.course][args.assessment_group]:
				...
				student_row[vmraid.scrub(criteria)] = ""
				student_row[vmraid.scrub(criteria) + "_score"] = ""
...
def get_formatted_result(
	...
	assessment_result = vmraid.db.sql(
		"""
		SELECT
	...
	for result in assessment_result:
		...
		assessment_criteria_details = vmraid._dict(
			{
...
def get_column(assessment_criteria):
	...
	for d in assessment_criteria:
		columns.append({"fieldname": vmraid.scrub(d), "label": d, "fieldtype": "Data", "width": 110})
		columns.append(
			{
				"fieldname": vmraid.scrub(d) + "_score",
...
def get_chart_data(grades, criteria_list, kounter):
	...
	for grade in grades:
		tmp = vmraid._dict({"name": grade, "values": []})
...
def get_child_assessment_groups(assessment_group):
	assessment_groups = []
	group_type = vmraid.get_value("Assessment Group", assessment_group, "is_group")
	if group_type:
		from vmraid.desk.treeview import get_children
