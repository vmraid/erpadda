...
from vmraid.utils import add_years, date_diff, getdate, nowdate
...
class StudentApplicant(Document):
	def autoname(self):
		...
		if self.student_admission:
			...
			if self.program:
				...
				vmraid.throw(_("Select the program first"))
	...
	def on_update_after_submit(self):
		student = vmraid.get_list("Student", filters={"student_applicant": self.name})
		if student:
			vmraid.throw(
	...
	def validation_from_student_admission(self):
		...
		if (
			...
			vmraid.throw(_("Not eligible for the admission in this program as per Date Of Birth"))
...
def get_student_admission_data(student_admission, program):
	...
	student_admission = vmraid.db.sql(
