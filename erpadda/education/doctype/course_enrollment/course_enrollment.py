...
class CourseEnrollment(Document):
	...
	def get_progress(self, student):
		...
		"""
		course = vmraid.get_doc("Course", self.course)
			...
			try:
				if not quiz_response[key]:
					...
					item["selected_option"] = ", ".join(
						vmraid.get_value("Options", res, "option") for res in quiz_response[key]
					)
				else:
					item["selected_option"] = vmraid.get_value("Options", quiz_response[key], "option")
			except KeyError:
				item["selected_option"] = "Unattempted"
			result_data.append(item)

		quiz_activity = vmraid.get_doc(
			{
