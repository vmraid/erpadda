	...
	add_course_to_programs: function(frm) {
		get_programs_without_course(frm.doc.name).then(r => {
			if (r.message.length) {
				...
				function(data) {
					vmraid.call({
						method: 'erpadda.education.doctype.course.course.add_course_to_programs',
						args: {
							'course': frm.doc.name,
				...
				vmraid.msgprint(__('This course is already added to the existing programs'));
			}
		});
	}
});

vmraid.ui.form.on('Course Topic', {
...
let get_programs_without_course = function(course) {
	return vmraid.call({
		type: 'GET',
		method: 'erpadda.education.doctype.course.course.get_programs_without_course',
		args: {'course': course}
	});
}
