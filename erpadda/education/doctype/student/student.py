...
from erpadda.education.utils import check_content_completion, check_quiz_completion
...
class Student(Document):
	def validate(self):
		...
		if vmraid.get_value("Student", self.name, "title") != self.title:
	...
	def validate_dates(self):
		...
		if self.date_of_birth and getdate(self.date_of_birth) >= getdate(today()):
			vmraid.throw(_("Date of Birth cannot be greater than today."))

		if self.date_of_birth and getdate(self.date_of_birth) >= getdate(self.joining_date):
			vmraid.throw(_("Date of Birth cannot be greater than Joining Date."))
		...
		if (
			...
			vmraid.throw(_("Joining Date can not be greater than Leaving Date"))

	def update_student_name_in_linked_doctype(self):
		linked_doctypes = get_linked_doctypes("Student")
		for d in linked_doctypes:
			meta = vmraid.get_meta(d)
			if not meta.issingle:
				...
				if "child_doctype" in linked_doctypes[d].keys() and "student_name" in [
					f.fieldname for f in vmraid.get_meta(linked_doctypes[d]["child_doctype"]).fields
				]:
					vmraid.db.sql(
						"""UPDATE `tab{0}` set student_name = %s where {1} = %s""".format(
	...
	def after_insert(self):
		if not vmraid.get_single("Education Settings").get("user_creation_skip"):
			self.create_student_user()

	def create_student_user(self):
		"""Create a website user for student creation if not already exists"""
		if not vmraid.db.exists("User", self.student_email_id):
			student_user = vmraid.get_doc(
				{
					"doctype": "User",
	...
	def get_all_course_enrollments(self):
		...
		course_enrollments = vmraid.get_all(
	...
	def get_program_enrollments(self):
		"""Returns a list of course enrollments linked with the current student"""
		program_enrollments = vmraid.get_all(
	...
	def enroll_in_program(self, program_name):
		try:
			enrollment = vmraid.get_doc(
				{
					"doctype": "Program Enrollment",
					"student": self.name,
					"academic_year": vmraid.get_last_doc("Academic Year").name,
					"program": program_name,
					"enrollment_date": vmraid.utils.datetime.datetime.now(),
				}
			)
			enrollment.save(ignore_permissions=True)
		except vmraid.exceptions.ValidationError:
			enrollment_name = vmraid.get_list(
				"Program Enrollment", filters={"student": self.name, "Program": program_name}
			)[0].name
			return vmraid.get_doc("Program Enrollment", enrollment_name)
		else:
			enrollment.submit()
			return enrollment
	...
	def enroll_in_course(self, course_name, program_enrollment, enrollment_date=None):
		...
		try:
			...
			return vmraid.get_doc("Course Enrollment", enrollment_name)
		else:
			return enrollment
