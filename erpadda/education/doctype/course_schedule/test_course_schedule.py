...
import vmraid
from vmraid.utils import to_timedelta, today
from vmraid.utils.data import add_to_date

from erpadda.education.utils import OverlapError

# test_records = vmraid.get_test_records('Course Schedule')
...
class TestCourseSchedule(unittest.TestCase):
	def test_student_group_conflict(self):
		...
		cs2 = make_course_schedule_test_record(
			...
			room=vmraid.get_all("Room")[1].name,
			do_not_save=1,
		)
...
def make_course_schedule_test_record(**args):
	...
	course_schedule = vmraid.new_doc("Course Schedule")
	...
	course_schedule.room = args.room or vmraid.get_all("Room")[0].name
