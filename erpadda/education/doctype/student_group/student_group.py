...
from erpadda.education.utils import validate_duplicate_student
...
class StudentGroup(Document):
	...
	def validate_students(self):
		...
		for d in self.students:
			if not vmraid.db.get_value("Student", d.student, "enabled") and d.active and not self.disabled:
				vmraid.throw(_("{0} - {1} is inactive student").format(d.group_roll_number, d.student_name))

			if (
				(self.group_based_on == "Batch")
				and cint(vmraid.defaults.get_defaults().validate_batch)
			...
			if (
				...
				and cint(vmraid.defaults.get_defaults().validate_course)
				and (d.student not in students)
			):
				vmraid.throw(
	...
	def validate_and_set_child_table_fields(self):
		...
		for d in self.students:
			if not d.student_name:
				d.student_name = vmraid.db.get_value("Student", d.student, "title")
			if not d.group_roll_number:
				max_roll_no += 1
				d.group_roll_number = max_roll_no
			if d.group_roll_number in roll_no_list:
				vmraid.throw(_("Duplicate roll number for student {0}").format(d.student_name))
			else:
...
def get_students(
	...
	if enrolled_students:
		...
		for s in enrolled_students:
			if vmraid.db.get_value("Student", s.student, "enabled"):
				s.update({"active": 1})
			else:
				s.update({"active": 0})
		...
		vmraid.msgprint(_("No students found"))
		return []
...
def fetch_students(doctype, txt, searchfield, start, page_len, filters):
	if filters.get("group_based_on") != "Activity":
		...
		student_group_student = vmraid.db.sql_list(
