...
vmraid.ui.form.on('Student Group', {
	onload: function(frm) {
		...
		if (!frm.__islocal) {
			frm.set_query('student', 'students', function() {
				return{
					query: 'erpadda.education.doctype.student_group.student_group.fetch_students',
					filters: {
						'academic_year': frm.doc.academic_year,
	...
	refresh: function(frm) {
		if (!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Add Guardians to Email Group'), function() {
				vmraid.call({
					method: 'erpadda.education.api.update_email_group',
					args: {
			...
			frm.add_custom_button(__('Student Attendance Tool'), function() {
				...
				vmraid.set_route('Form', 'Student Attendance Tool', 'Student Attendance Tool');
			}, __('Tools'));
			...
			frm.add_custom_button(__('Course Scheduling Tool'), function() {
				...
				vmraid.set_route('Form', 'Course Scheduling Tool', 'Course Scheduling Tool');
			}, __('Tools'));
			...
			frm.add_custom_button(__('Newsletter'), function() {
				...
				vmraid.set_route('List', 'Newsletter');
			}, __('View'));
	...
	get_students: function(frm) {
		if (frm.doc.group_based_on == 'Batch' || frm.doc.group_based_on == 'Course') {
			...
			if (frm.doc.academic_year) {
				vmraid.call({
					method: 'erpadda.education.doctype.student_group.student_group.get_students',
					args: {
						'academic_year': frm.doc.academic_year,
						'academic_term': frm.doc.academic_term,
						'group_based_on': frm.doc.group_based_on,
					...
					callback: function(r) {
						if (r.message) {
							...
							vmraid.msgprint(__('Student Group is already updated.'))
						}
					}
				})
			}
		} else {
			vmraid.msgprint(__('Select students manually for the Activity based Group'));
		}
	}
});

vmraid.ui.form.on('Student Group Instructor', {
