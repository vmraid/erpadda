...
from vmraid.query_builder.functions import Min
from vmraid.utils import comma_and, get_link_to_form, getdate
...
class ProgramEnrollment(Document):
	def validate(self):
		...
		if not self.student_name:
			self.student_name = vmraid.db.get_value("Student", self.student, "title")
		if not self.courses:
	...
	def validate_duplication(self):
		...
		if enrollment:
			vmraid.throw(_("Student is already enrolled."))

	def update_student_joining_date(self):
		table = vmraid.qb.DocType("Program Enrollment")
		date = (
		...
		if date:
			vmraid.db.set_value("Student", self.student, "joining_date", date[0].enrollment_date)

	def make_fee_records(self):
		from erpadda.education.api import get_fee_components

		fee_list = []
		for d in self.fees:
			fee_components = get_fee_components(d.fee_structure)
			if fee_components:
				fees = vmraid.new_doc("Fees")
				fees.update(
					{
						"student": self.student,
						"academic_year": self.academic_year,
	...
	def create_course_enrollments(self):
		student = vmraid.get_doc("Student", self.student)
...
def get_program_courses(doctype, txt, searchfield, start, page_len, filters):
	if not filters.get("program"):
		vmraid.msgprint(_("Please select a Program first."))
		return []
...
def get_students(doctype, txt, searchfield, start, page_len, filters):
	if not filters.get("academic_term"):
		filters["academic_term"] = vmraid.defaults.get_defaults().academic_term

	if not filters.get("academic_year"):
		filters["academic_year"] = vmraid.defaults.get_defaults().academic_year

	enrolled_students = vmraid.get_list(
		"Program Enrollment",
		filters={
