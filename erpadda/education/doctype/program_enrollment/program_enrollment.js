...
vmraid.ui.form.on('Program Enrollment', {
	setup: function(frm) {
	...
	onload: function(frm) {
		...
		frm.set_query('student', function() {
			return{
				query: 'erpadda.education.doctype.program_enrollment.program_enrollment.get_students',
				filters: {
					'academic_year': frm.doc.academic_year,
	...
	program: function(frm) {
		...
		if (frm.doc.program) {
			vmraid.call({
				method: 'erpadda.education.api.get_fee_schedule',
				args: {
					'program': frm.doc.program,
	...
	student_category: function() {
		vmraid.ui.form.trigger('Program Enrollment', 'program');
	},
...
vmraid.ui.form.on('Program Enrollment Course', {
