	...
	add_article_to_topics: function(frm) {
		get_topics_without_article(frm.doc.name).then(r => {
			if (r.message.length) {
				...
				vmraid.msgprint(__('This article is already added to the existing topics'));
			}
		});
	}
});
...
let get_topics_without_article = function(article) {
	return vmraid.call({
		type: 'GET',
		method: 'erpadda.education.doctype.article.article.get_topics_without_article',
