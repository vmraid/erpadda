...
vmraid.ui.form.on('Assessment Result', {
	...
	assessment_plan: function(frm) {
		if (frm.doc.assessment_plan) {
			vmraid.call({
				...
				callback: function(r) {
					if (r.message) {
						vmraid.model.clear_table(frm.doc, 'details');
...
vmraid.ui.form.on('Assessment Result Detail', {
	score: function(frm, cdt, cdn) {
		var d  = locals[cdt][cdn];

		if (!d.maximum_score || !frm.doc.grading_scale) {
			d.score = '';
			vmraid.throw(__('Please fill in all the details to generate Assessment Result.'));
		}

		if (d.score > d.maximum_score) {
			vmraid.throw(__('Score cannot be greater than Maximum Score'));
		}
		else {
			vmraid.call({
				...
				callback: function(r) {
					if (r.message) {
						vmraid.model.set_value(cdt, cdn, 'grade', r.message);
					}
				}
			});
		}
