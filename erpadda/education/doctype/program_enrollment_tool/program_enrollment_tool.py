...
from erpadda.education.api import enroll_student


class ProgramEnrollmentTool(Document):
	def onload(self):
		academic_term_reqd = cint(vmraid.db.get_single_value("Education Settings", "academic_term_reqd"))
	...
	def get_students(self):
		...
		if not self.get_students_from:
			vmraid.throw(_("Mandatory field - Get Students From"))
		elif not self.program:
			vmraid.throw(_("Mandatory field - Program"))
		elif not self.academic_year:
			vmraid.throw(_("Mandatory field - Academic Year"))
		else:
			...
			if self.get_students_from == "Student Applicant":
				...
				if student_list:
					inactive_students = vmraid.db.sql(
						"""
		...
		if students:
			...
			vmraid.throw(_("No students Found"))
	...
	def enroll_students(self):
		...
		for i, stud in enumerate(self.students):
			vmraid.publish_realtime(
				"program_enrollment_tool", dict(progress=[i + 1, total]), user=vmraid.session.user
			)
			if stud.student:
				prog_enrollment = vmraid.new_doc("Program Enrollment")
				...
				prog_enrollment.save()
		vmraid.msgprint(_("{0} Students have been enrolled").format(total))
