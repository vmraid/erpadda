...
class CourseSchedulingTool(Document):
	...
	def validate_mandatory(self):
		...
		for d in fields:
			if not self.get(d):
				vmraid.throw(_("{0} is mandatory").format(self.meta.get_label(d)))

	def validate_date(self):
		"""Validates if Course Start Date is greater than Course End Date"""
		if self.course_start_date > self.course_end_date:
			vmraid.throw(_("Course Start Date cannot be greater than Course End Date."))

	def delete_course_schedule(self, rescheduled, reschedule_errors):
		"""Delete all course schedule within the Date range and specified filters"""

		schedules = vmraid.get_list(
			"Course Schedule",
		...
		for d in schedules:
			try:
				if self.day == calendar.day_name[getdate(d.schedule_date).weekday()]:
					vmraid.delete_doc("Course Schedule", d.name)
