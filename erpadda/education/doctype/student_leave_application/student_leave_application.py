...
from vmraid.utils import date_diff, flt, get_link_to_form, getdate
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			if attendance:
				...
				vmraid.db.set_value("Student Attendance", attendance, values)
			else:
				# make a new attendance record
				doc = vmraid.new_doc("Student Attendance")
	...
	def cancel_attendance(self):
		if self.docstatus == 2:
			...
			for name in attendance:
				vmraid.db.set_value("Student Attendance", name, "docstatus", 2)
