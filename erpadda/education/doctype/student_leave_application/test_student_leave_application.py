...
import vmraid
from vmraid.utils import add_days, add_months, getdate

from erpadda import get_default_company
from erpadda.education.doctype.student.test_student import create_student
from erpadda.education.doctype.student_group.test_student_group import get_random_group


class TestStudentLeaveApplication(unittest.TestCase):
	def setUp(self):
		vmraid.db.sql("""delete from `tabStudent Leave Application`""")
		create_holiday_list()
	...
	def test_attendance_record_updated(self):
		...
		self.assertEqual(vmraid.db.get_value("Student Attendance", attendance.name, "status"), "Absent")

	def test_attendance_record_cancellation(self):
		leave_application = create_leave_application()
		leave_application.cancel()
		attendance_status = vmraid.db.get_value(
	...
	def test_holiday(self):
		...
		vmraid.db.set_value("Company", company, "default_holiday_list", "")
		self.assertRaises(vmraid.ValidationError, leave_application.save)

		vmraid.db.set_value("Company", company, "default_holiday_list", "Test Holiday List for Student")
	...
	def tearDown(self):
		company = get_default_company() or vmraid.get_all("Company")[0].name
		vmraid.db.set_value("Company", company, "default_holiday_list", "_Test Holiday List")


def create_leave_application(from_date=None, to_date=None, mark_as_present=0, submit=1):
	student = get_student()

	leave_application = vmraid.new_doc("Student Leave Application")
...
def create_holiday_list():
	...
	company = get_default_company() or vmraid.get_all("Company")[0].name
	vmraid.db.set_value("Company", company, "default_holiday_list", holiday_list)
	return holiday_list
