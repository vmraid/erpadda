...
vmraid.ui.form.on('Student Attendance Tool', {
	setup: (frm) => {
	...
	refresh: function(frm) {
		if (vmraid.route_options) {
			frm.set_value("based_on", vmraid.route_options.based_on);
			frm.set_value("student_group", vmraid.route_options.student_group);
			frm.set_value("course_schedule", vmraid.route_options.course_schedule);
	...
	student_group: function(frm) {
		if ((frm.doc.student_group && frm.doc.date) || frm.doc.course_schedule) {
			...
			var method = "erpadda.education.doctype.student_attendance_tool.student_attendance_tool.get_student_attendance_records";
	...
	date: function(frm) {
		if (frm.doc.date > vmraid.datetime.get_today())
			vmraid.throw(__("Cannot mark attendance for future dates."));
	...
	make(frm, students) {
		...
		student_toolbar.find(".btn-mark-att")
			...
			.on("click", function() {
				...
				vmraid.confirm(__("Do you want to update attendance? <br> Present: {0} <br> Absent: {1}",
					[students_present.length, students_absent.length]),
					function() {	//ifyes
						if(!vmraid.request.ajax_count) {
							vmraid.call({
								method: "erpadda.education.api.mark_attendance",
								freeze: true,
