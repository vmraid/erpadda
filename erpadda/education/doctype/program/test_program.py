...
from erpadda.education.doctype.course.test_course import make_course, make_course_and_linked_topic
from erpadda.education.doctype.topic.test_topic import make_topic_and_linked_content
...
class TestProgram(unittest.TestCase):
	...
	def test_get_course_list(self):
		program = vmraid.get_doc("Program", "_Test Program 1")
	...
	def tearDown(self):
		for dt in ["Program", "Course", "Topic", "Article"]:
			for entry in vmraid.get_all(dt):
				vmraid.delete_doc(dt, entry.program)


def make_program(name):
	program = vmraid.get_doc(
		{
			"doctype": "Program",
			"program_name": name,
			"program_code": name,
