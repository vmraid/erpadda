...
vmraid.ui.form.on('Assessment Result Tool', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		if (vmraid.route_options) {
			...
			frm.set_value("assessment_plan", vmraid.route_options.assessment_plan);
	...
	assessment_plan: function(frm) {
		...
		if(frm.doc.assessment_plan) {
			if (!frm.doc.student_group)
				return
			vmraid.call({
				method: "erpadda.education.api.get_assessment_students",
				args: {
	...
	render_table: function(frm) {
		...
		vmraid.call({
			method: "erpadda.education.api.get_assessment_details",
			args: {
	...
	get_marks: function(frm, criteria_list) {
		...
		var result_table = $(vmraid.render_template('assessment_result_tool', {
			frm: frm,
		...
		result_table.on('change', 'input', function(e) {
			...
			if (Object.keys(student_scores["assessment_details"]).length === criteria_list.length) {
				...
				vmraid.call({
					method: "erpadda.education.api.mark_assessment_result",
					args: {
						"assessment_plan": frm.doc.assessment_plan,
	...
	submit_result: function(frm) {
		if (frm.doc.show_submit) {
			frm.page.set_primary_action(__("Submit"), function() {
				vmraid.call({
					method: "erpadda.education.api.submit_assessment_results",
					args: {
						"assessment_plan": frm.doc.assessment_plan,
						"student_group": frm.doc.student_group
					},
					callback: function(r) {
						if (r.message) {
							vmraid.msgprint(__("{0} Result submittted", [r.message]));
						} else {
							vmraid.msgprint(__("No Result to submit"));
						}
