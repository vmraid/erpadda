...
class Question(Document):
	...
	def check_at_least_one_option(self):
		if len(self.options) <= 1:
			vmraid.throw(_("A question must have more than one options"))
		else:
			pass
	...
	def check_minimum_one_correct_answer(self):
		...
		if bool(sum(correct_options)):
			...
			vmraid.throw(_("A qustion must have at least one correct options"))
	...
	def get_answer(self):
		...
		if len(answers) == 0:
			vmraid.throw(_("No correct answer is set for {0}").format(self.name))
			return None
		elif len(answers) == 1:
			return answers[0]
		else:
