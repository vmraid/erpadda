...
from erpadda.e_commerce.variant_selector.utils import get_next_attribute_and_values
from erpadda.stock.doctype.item.test_item import make_item

test_dependencies = ["Item"]


class TestVariantSelector(VMRaidTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_item_attributes(self):
		...
		"""
		from erpadda.e_commerce.variant_selector.utils import get_attributes_and_values
		...
		small_variant = vmraid.get_doc("Item", "Test-Tshirt-Temp-S-R")
	...
	def test_exact_match_with_price(self):
		...
		"""
		from erpadda.e_commerce.doctype.website_item.test_website_item import make_web_item_price
		...
		vmraid.local.shopping_cart_settings = None  # clear cached settings values
		next_values = get_next_attribute_and_values(
