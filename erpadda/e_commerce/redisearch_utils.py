...
def get_indexable_web_fields():
	...
	web_item_meta = vmraid.get_meta("Website Item", cached=True)
	valid_fields = filter(
...
def is_redisearch_enabled():
	...
	is_redisearch_enabled = vmraid.db.get_single_value("E Commerce Settings", "is_redisearch_enabled")
...
def create_website_items_index():
	...
	# CREATE index
	client = Client(make_key(WEBSITE_ITEM_INDEX), conn=vmraid.cache())
	...
	idx_fields = vmraid.db.get_single_value("E Commerce Settings", "search_index_fields")
...
def insert_to_name_ac(web_name, doc_name):
	ac = AutoCompleter(make_key(WEBSITE_ITEM_NAME_AUTOCOMPLETE), conn=vmraid.cache())
...
def create_item_groups_autocomplete_dict(autocompleter):
	...
	published_item_groups = vmraid.get_all(
	...
	for item_group in published_item_groups:
		...
		autocompleter.add_suggestions(
			Suggestion(
				...
				score=vmraid.utils.flt(item_group.weightage) or 1.0,
...
def reindex_all_web_items():
	items = vmraid.get_all("Website Item", fields=get_fields_indexed(), filters={"published": True})
...
def get_cache_key(name):
	name = vmraid.scrub(name)
	return f"{WEBSITE_ITEM_KEY_PREFIX}{name}"


def get_fields_indexed():
	fields_to_index = vmraid.db.get_single_value("E Commerce Settings", "search_index_fields")
...
def raise_redisearch_error():
	...
	log = vmraid.log_error(traceback, vmraid._("Redisearch Error"))
	log_link = vmraid.utils.get_link_to_form("Error Log", log.name)
