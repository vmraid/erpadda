...
class UnverifiedReviewer(vmraid.ValidationError):
	pass
...
def get_item_reviews(web_item, start=0, end=10, data=None):
	...
	if settings and settings.get("enable_reviews"):
		reviews_cache = vmraid.cache().hget("item_reviews", web_item)
...
def get_queried_reviews(web_item, start=0, end=10, data=None):
	...
	data.reviews = vmraid.db.get_all(
		"Item Review",
...
def set_reviews_in_cache(web_item, reviews_dict):
	vmraid.cache().hset("item_reviews", web_item, reviews_dict)
...
def add_item_review(web_item, title, rating, comment=None):
	...
	if vmraid.session.user == "Guest":
		...
		vmraid.throw(_("You are not verified to write a review yet."), exc=UnverifiedReviewer)

	if not vmraid.db.exists("Item Review", {"user": vmraid.session.user, "website_item": web_item}):
		doc = vmraid.get_doc(
			{
				"doctype": "Item Review",
				...
				"item": vmraid.db.get_value("Website Item", web_item, "item_code"),
				"review_title": title,
				"rating": rating,
				"comment": comment,
			}
...
def get_customer(silent=False):
	...
	if customer:
		return vmraid.db.get_value("Customer", customer)
	elif silent:
		return None
	else:
