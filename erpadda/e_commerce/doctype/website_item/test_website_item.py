...
from erpadda.stock.doctype.item.item import DataValidationError
...
class TestWebsiteItem(unittest.TestCase):
	...
	def test_index_creation(self):
		...
		from erpadda.e_commerce.doctype.website_item.website_item import on_doctype_update

		on_doctype_update()

		indices = vmraid.db.sql("show index from `tabWebsite Item`", as_dict=1)
	...
	def test_publish_variant_and_template(self):
		...
		self.assertIsNone(vmraid.db.exists("Website Item", {"item_code": variant.variant_of}))
		...
		try:
			template_web_item = vmraid.get_doc("Website Item", {"item_code": variant.variant_of})
	...
	def test_impact_on_merging_items(self):
		...
		with self.assertRaises(DataValidationError):
			vmraid.rename_doc("Item", "Test First Item", "Test Second Item", merge=True)
	...
	def test_website_item_breadcrumbs(self):
		...
		from erpadda.setup.doctype.item_group.item_group import get_parent_item_groups
		...
		if not vmraid.db.exists("Website Item", {"item_code": item_code}):
			...
			web_item = vmraid.get_cached_doc("Website Item", {"item_code": item_code})

		vmraid.db.set_value("Item Group", "_Test Item Group B - 1", "show_in_website", 1)
		vmraid.db.set_value("Item Group", "_Test Item Group B", "show_in_website", 1)
	...
	def test_website_item_price_for_logged_in_user(self):
		...
		vmraid.set_user("test_contact_customer@example.com")
	...
	def test_website_item_stock_when_in_stock(self):
		...
		# unset warehouse
		vmraid.db.set_value("Website Item", {"item_code": item_code}, "website_warehouse", "")
	...
	def test_recommended_item(self):
		...
		vmraid.get_cached_doc("Item", "Test Mobile Phone 1").delete()
...
def make_web_item_price(**kwargs):
	...
	if not vmraid.db.exists("Item Price", {"item_code": item_code}):
		...
		item_price = vmraid.get_cached_doc("Item Price", {"item_code": item_code})
...
def make_web_pricing_rule(**kwargs):
	...
	if not vmraid.db.exists("Pricing Rule", title):
		pricing_rule = vmraid.get_doc(
			{
		...
		pricing_rule = vmraid.get_doc("Pricing Rule", {"title": title})
...
def create_user_and_customer_if_not_exists(email, first_name=None):
	...
	contact = vmraid.get_last_doc("Contact", filters={"email_id": email})
