...
vmraid.ui.form.on("E Commerce Settings", {
	onload: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.enabled) {
			frm.get_field('store_page_docs').$wrapper.removeClass('hide-control').html(
				`<div>${__("Follow these steps to create a landing page for your store")}:
					<a href="https://docs.erpadda.com/docs/user/manual/en/website/store-landing-page"
		...
		vmraid.model.with_doctype("Item", () => {
			const web_item_meta = vmraid.get_meta('Website Item');
