...
from vmraid.utils import comma_and, flt, unique
...
class ShoppingCartSetupError(vmraid.ValidationError):
	pass
...
class ECommerceSettings(Document):
	...
	def validate(self):
		...
		vmraid.clear_document_cache("E Commerce Settings", "E Commerce Settings")

		self.is_redisearch_enabled_pre_save = vmraid.db.get_single_value(
	...
	def validate_search_index_fields(self):
		...
		if not (set(fields).issubset(allowed_indexable_fields)):
			...
			if num_invalid_fields > 1:
				vmraid.throw(
					_("{0} are not valid options for Search Index Field.").format(vmraid.bold(invalid_fields))
				)
			else:
				vmraid.throw(
					_("{0} is not a valid option for Search Index Field.").format(vmraid.bold(invalid_fields))
				)
	...
	def validate_price_list_exchange_rate(self):
		...
		price_list_currency = vmraid.db.get_value("Price List", self.price_list, "currency")

		if not company_currency:
			msg = f"Please specify currency in Company {self.company}"
			vmraid.throw(_(msg), title=_("Missing Currency"), exc=ShoppingCartSetupError)

		if not price_list_currency:
			msg = f"Please specify currency in Price List {vmraid.bold(self.price_list)}"
			vmraid.throw(_(msg), title=_("Missing Currency"), exc=ShoppingCartSetupError)
		...
		if price_list_currency != company_currency:
			...
			if not flt(exchange_rate):
				...
				vmraid.throw(_(msg), title=_("Missing"), exc=ShoppingCartSetupError)
...
def validate_cart_settings(doc=None, method=None):
	vmraid.get_doc("E Commerce Settings", "E Commerce Settings").run_method("validate")


def get_shopping_cart_settings():
	return vmraid.get_cached_doc("E Commerce Settings")
