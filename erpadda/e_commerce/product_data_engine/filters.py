...
import vmraid
from vmraid.utils import floor


class ProductFiltersBuilder:
	def __init__(self, item_group=None):
		if not item_group:
			self.doc = vmraid.get_doc("E Commerce Settings")
		else:
			self.doc = vmraid.get_doc("Item Group", item_group)
	...
	def get_field_filters(self):
		...
		item_meta = vmraid.get_meta("Item", cached=True)
		...
		for df in fields:
			...
			if df.fieldtype == "Link":
				if self.item_group:
					include_child = vmraid.db.get_value("Item Group", self.item_group, "include_descendants")
					if include_child:
	...
	def get_filtered_link_doctype_records(self, field):
		...
		meta = vmraid.get_meta(link_doctype, cached=True) if link_doctype else None
		if meta:
			filters = self.get_link_doctype_filters(meta)
			link_doctype_values = set(d.name for d in vmraid.get_all(link_doctype, filters))
	...
	def get_attribute_filters(self):
		...
		for name, values in attribute_value_map.items():
			out.append(vmraid._dict(name=name, item_attribute_values=values))
		return out
