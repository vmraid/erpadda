...
from vmraid.utils import add_months, cint, nowdate

from erpadda.accounts.doctype.tax_rule.tax_rule import ConflictingTaxRule
from erpadda.e_commerce.doctype.website_item.website_item import make_website_item
from erpadda.e_commerce.shopping_cart.cart import (
	_get_cart_quotation,
	get_cart_quotation,
	get_party,
...
class TestShoppingCart(unittest.TestCase):
	...
	def setUp(self):
		...
		if not vmraid.db.exists("Website Item", {"item_code": "_Test Item"}):
			make_website_item(vmraid.get_cached_doc("Item", "_Test Item"))

		if not vmraid.db.exists("Website Item", {"item_code": "_Test Item 2"}):
			make_website_item(vmraid.get_cached_doc("Item", "_Test Item 2"))
	...
	def test_get_cart_new_user(self):
		...
		self.assertEqual(
			...
			vmraid.db.get_value("Contact", dict(email_id="test_cart_user@example.com")),
		)
		self.assertEqual(quotation.contact_email, vmraid.session.user)
	...
	def test_add_item_variant_without_web_item_to_cart(self):
		...
		vmraid.render_template("templates/includes/cart/cart_items.html", cart)
	...
	def test_cart_without_checkout_and_draft_quotation(self):
		...
		quote_doctstatus = cint(vmraid.db.get_value("Quotation", quote_name, "docstatus"))

		self.assertEqual(quote_doctstatus, 0)

		vmraid.db.set_value("E Commerce Settings", None, "save_quotations_as_draft", 0)
		vmraid.local.shopping_cart_settings = None
		update_cart("_Test Item", 1)
		quote_name = request_for_quotation()  # Request for Quote
		quote_doctstatus = cint(vmraid.db.get_value("Quotation", quote_name, "docstatus"))
	...
	def create_tax_rule(self):
		tax_rule = vmraid.get_test_records("Tax Rule")[0]
		try:
			vmraid.get_doc(tax_rule).insert(ignore_if_duplicate=True)
		except (vmraid.DuplicateEntryError, ConflictingTaxRule):
			pass

	def create_quotation(self):
		quotation = vmraid.new_doc("Quotation")

		values = {
			"doctype": "Quotation",
			"quotation_to": "Customer",
			"order_type": "Shopping Cart",
			"party_name": get_party(vmraid.session.user).name,
			"docstatus": 0,
			...
			"taxes": vmraid.get_doc("Sales Taxes and Charges Template", "_Test Tax 1 - _TC").taxes,
			"company": "_Test Company",
		}
	...
	def enable_shopping_cart(self):
		settings = vmraid.get_doc("E Commerce Settings", "E Commerce Settings")
	...
	def login_as_new_user(self):
		...
		vmraid.set_user("test_cart_user@example.com")
	...
	def login_as_customer(
		...
		vmraid.set_user(email)

	def clear_existing_quotations(self):
		quotations = vmraid.get_all(
			"Quotation",
		...
		for quotation in quotations:
			vmraid.delete_doc("Quotation", quotation, ignore_permissions=True, force=True)
