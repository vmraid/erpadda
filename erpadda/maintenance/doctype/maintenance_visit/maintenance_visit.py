...
from vmraid.utils import format_date, get_datetime
...
class MaintenanceVisit(TransactionBase):
	...
	def validate_serial_no(self):
		for d in self.get("purposes"):
			if d.serial_no and not vmraid.db.exists("Serial No", d.serial_no):
				vmraid.throw(_("Serial No {0} does not exist").format(d.serial_no))

	def validate_purpose_table(self):
		if not self.purposes:
			vmraid.throw(_("Add Items in the Purpose Table"), title=_("Purposes Required"))

	def validate_maintenance_date(self):
		if self.maintenance_type == "Scheduled" and self.maintenance_schedule_detail:
			item_ref = vmraid.db.get_value(
		...
		if not self.maintenance_schedule:
			for d in self.get("purposes"):
				if d.prevdoc_docname and d.prevdoc_doctype == "Warranty Claim":
					...
					wc_doc = vmraid.get_doc("Warranty Claim", d.prevdoc_docname)
					wc_doc.update(
						{
