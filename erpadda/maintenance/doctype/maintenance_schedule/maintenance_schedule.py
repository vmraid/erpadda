...
from vmraid.utils import add_days, cint, cstr, date_diff, formatdate, getdate
...
from erpadda.stock.utils import get_valid_serial_nos
from erpadda.utilities.transaction_base import TransactionBase, delete_events
...
class MaintenanceSchedule(TransactionBase):
	...
	def on_submit(self):
		...
		for d in self.get("items"):
			...
			if d.sales_person not in email_map:
				sp = vmraid.get_doc("Sales Person", d.sales_person)
				try:
			...
			for key in scheduled_date:
				description = vmraid._("Reference: {0}, Item Code: {1} and Customer: {2}").format(
	...
	def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
		...
		employee = vmraid.db.get_value("Sales Person", sales_person, "employee")
		if employee:
			holiday_list = get_holiday_list_for_employee(employee)
		else:
			holiday_list = vmraid.get_cached_value("Company", self.company, "default_holiday_list")

		holidays = vmraid.db.sql_list(
	...
	def validate_serial_no(self, item_code, serial_nos, amc_start_date):
		for serial_no in serial_nos:
			...
			if not sr_details:
				vmraid.throw(_("Serial No {0} not found").format(serial_no))

			if sr_details.get("item_code") != item_code:
				vmraid.throw(
					_("Serial No {0} does not belong to Item {1}").format(
						vmraid.bold(serial_no), vmraid.bold(item_code)
					),
					title=_("Invalid"),
				)
