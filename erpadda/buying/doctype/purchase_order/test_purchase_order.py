...
from erpadda.stock.doctype.material_request.material_request import make_purchase_order
from erpadda.stock.doctype.material_request.test_material_request import make_material_request
...
class TestPurchaseOrder(VMRaidTestCase):
	def test_make_purchase_receipt(self):
		po = create_purchase_order(do_not_submit=True)
		self.assertRaises(vmraid.ValidationError, make_purchase_receipt, po.name)
		po.submit()
	...
	def test_ordered_qty_against_pi_with_update_stock(self):
		...
		vmraid.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 0)
		vmraid.db.set_value("Item", "_Test Item", "over_billing_allowance", 0)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(
			vmraid.ValidationError, update_child_qty_rate, "Purchase Order", trans_item, po.name
		)
	...
	def test_purchase_order_invoice_receipt_workflow(self):
		from erpadda.accounts.doctype.purchase_invoice.purchase_invoice import make_purchase_receipt
	...
	def test_make_purchase_invoice(self):
		...
		self.assertRaises(vmraid.ValidationError, make_pi_from_po, po.name)
	...
	def test_purchase_order_on_hold(self):
		...
		self.assertRaises(vmraid.ValidationError, pr.submit)
		self.assertRaises(vmraid.ValidationError, pi.submit)
	...
	def test_uom_integer_validation(self):
		from erpadda.utilities.transaction_base import UOMMustBeIntegerError
		...
		vmraid.get_doc(
			{
				...
				"currency": vmraid.get_cached_value("Company", "_Test Company", "default_currency"),
				"conversion_factor": 1,
	...
	def test_po_for_blocked_supplier_all(self):
		...
		self.assertRaises(vmraid.ValidationError, create_purchase_order)
	...
	def test_terms_are_not_copied_if_automatically_fetch_payment_terms_is_unchecked(self):
		...
		vmraid.db.set_value("Company", "_Test Company", "payment_terms", "_Test Payment Term Template 1")
		pi = make_pi_from_po(po.name)
		pi.save()

		self.assertEqual(pi.get("payment_terms_template"), "_Test Payment Term Template 1")
		vmraid.db.set_value("Company", "_Test Company", "payment_terms", "")
	...
	def test_exploded_items_in_subcontracted(self):
		...
		name = vmraid.db.get_value("BOM", {"item": item_code}, "name")
		bom = vmraid.get_doc("BOM", name)
	...
	def test_backflush_based_on_stock_entry(self):
		...
		transferred_rm_map = vmraid._dict()
	...
	def test_supplied_qty_against_subcontracted_po(self):
		...
		po_doc = vmraid.get_doc("Purchase Order", po.name)
	...
	def test_advance_payment_entry_unlink_against_purchase_order(self):
		...
		po_doc = vmraid.get_doc("Purchase Order", po_doc.name)
		po_doc.cancel()

		pe_doc = vmraid.get_doc("Payment Entry", pe.name)
		pe_doc.cancel()
	...
	def test_po_optional_blanket_order(self):
		...
		po_doc = vmraid.get_doc("Purchase Order", po.get("name"))
...
def update_backflush_based_on(based_on):
	doc = vmraid.get_doc("Buying Settings")
...
def create_purchase_order(**args):
	po = vmraid.new_doc("Purchase Order")
	args = vmraid._dict(args)
	...
	po.currency = args.currency or vmraid.get_cached_value("Company", po.company, "default_currency")
...
def get_ordered_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(
		vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "ordered_qty")
	)


def get_requested_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(
		vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "indented_qty")
	)


test_dependencies = ["BOM", "Item Price"]

test_records = vmraid.get_test_records("Purchase Order")
