...
vmraid.ui.form.on("Purchase Order", {
	setup: function(frm) {
	...
	get_materials_from_supplier: function(frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(__('Return of Components'), () => {
				frm.call({
					method: 'erpadda.buying.doctype.purchase_order.purchase_order.get_materials_from_supplier',
					freeze: true,
					freeze_message: __('Creating Stock Entry'),
					args: { purchase_order: frm.doc.name, po_details: po_details },
					callback: function(r) {
						if (r && r.message) {
							const doc = vmraid.model.sync(r.message);
...
vmraid.ui.form.on("Purchase Order Item", {
...
erpadda.buying.PurchaseOrderController = class PurchaseOrderController extends erpadda.buying.BuyingController {
	setup() {
	...
	get_items_from_open_material_requests() {
		erpadda.utils.map_current_doc({
			method: "erpadda.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method: "erpadda.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier"
		});
	}
			...
			if(me.values) {
				me.values.sub_con_rm_items.map((row,i) => {
					if (!row.item_code || !row.rm_item_code || !row.warehouse || !row.qty || row.qty === 0) {
						let row_id = i+1;
						vmraid.throw(__("Item Code, warehouse and quantity are required on row {0}", [row_id]));
					}
				})
	...
	_make_rm_stock_entry(rm_items) {
		vmraid.call({
			method:"erpadda.buying.doctype.purchase_order.purchase_order.make_rm_stock_entry",
			args: {
	...
	make_inter_company_order(frm) {
		vmraid.model.open_mapped_doc({
			method: "erpadda.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
			frm: frm
		});
	}
	...
	make_purchase_invoice() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: cur_frm
		})
	}
					...
					while (i < item_length) {
						...
						(r.message[0] || []).forEach(function(d) {
							if (d.qty > 0 && qty > 0 && me.frm.doc.items[i].item_code == d.item_code && !me.frm.doc.items[i].material_request_item)
							{
								...
								vmraid.msgprint("Assigning " + d.mr_name + " to " + d.item_code + " (row " + me.frm.doc.items[i].idx + ")");
								if (qty > 0) {
									vmraid.msgprint("Splitting " + qty + " units of " + d.item_code);
									var new_row = vmraid.model.add_child(me.frm.doc, me.frm.doc.items[i].doctype, "items");
									item_length++;
...
extend_cscript(cur_frm.cscript, new erpadda.buying.PurchaseOrderController({frm: cur_frm}));

cur_frm.cscript.update_status= function(label, status){
	vmraid.call({
		method: "erpadda.buying.doctype.purchase_order.purchase_order.update_status",
...
vmraid.ui.form.on("Purchase Order", "is_subcontracted", function(frm) {
