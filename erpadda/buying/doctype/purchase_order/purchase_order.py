...
from erpadda.buying.utils import check_on_hold_or_closed_status, validate_for_items
...
from erpadda.stock.doctype.item.item import get_item_defaults, get_last_purchase_details
from erpadda.stock.stock_balance import get_ordered_qty, update_bin_qty
...
class PurchaseOrder(BuyingController):
	...
	def validate_with_previous_doc(self):
		...
		if cint(vmraid.db.get_single_value("Buying Settings", "maintain_same_rate")):
	...
	def validate_supplier(self):
		prevent_po = vmraid.db.get_value("Supplier", self.supplier, "prevent_pos")
		if prevent_po:
			standing = vmraid.db.get_value("Supplier Scorecard", self.supplier, "status")
			if standing:
				vmraid.throw(
		...
		warn_po = vmraid.db.get_value("Supplier", self.supplier, "warn_pos")
		if warn_po:
			standing = vmraid.db.get_value("Supplier Scorecard", self.supplier, "status")
			vmraid.msgprint(
				_(
	...
	def validate_minimum_order_qty(self):
		...
		itemwise_min_order_qty = vmraid._dict(
			vmraid.db.sql(
	...
	def get_schedule_dates(self):
		for d in self.get("items"):
			if d.material_request_item and not d.schedule_date:
				d.schedule_date = vmraid.db.get_value(
		...
		for d in self.get("items"):
			if d.item_code:
				...
				if last_purchase_details:
					...
					item_last_purchase_rate = vmraid.get_cached_value("Item", d.item_code, "last_purchase_rate")
	...
	def update_requested_qty(self):
		...
		for mr, mr_item_rows in material_request_map.items():
			if mr and mr_item_rows:
				mr_obj = vmraid.get_doc("Material Request", mr)

				if mr_obj.status in ["Stopped", "Cancelled"]:
					vmraid.throw(
						_("Material Request {0} is cancelled or stopped").format(mr), vmraid.InvalidStatusError
					)
	...
	def check_modified_date(self):
		mod_db = vmraid.db.sql("select modified from `tabPurchase Order` where name = %s", self.name)
		date_diff = vmraid.db.sql("select '%s' - '%s' " % (mod_db[0][0], cstr(self.modified)))
	...
	def update_delivered_qty_in_sales_order(self):
		...
		for so_name in sales_orders_to_update:
			so = vmraid.get_doc("Sales Order", so_name)
...
def item_last_purchase_rate(name, conversion_rate, item_code, conversion_factor=1.0):
	...
	if last_purchase_details:
		...
		item_last_purchase_rate = vmraid.get_cached_value("Item", item_code, "last_purchase_rate")
		if item_last_purchase_rate:
...
def close_or_unclose_purchase_orders(names, status):
	if not vmraid.has_permission("Purchase Order", "write"):
		vmraid.throw(_("Not permitted"), vmraid.PermissionError)

	names = json.loads(names)
	for name in names:
		po = vmraid.get_doc("Purchase Order", name)
		if po.docstatus == 1:
			if status == "Closed":
...
def make_purchase_invoice_from_portal(purchase_order_name):
	...
	if doc.contact_email != vmraid.session.user:
		vmraid.throw(_("Not Permitted"), vmraid.PermissionError)
	doc.save()
	vmraid.db.commit()
	vmraid.response["type"] = "redirect"
	vmraid.response.location = "/purchase-invoices/" + doc.name
...
def get_mapped_purchase_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	def update_item(obj, target, source_parent):
		...
		target.cost_center = (
			obj.cost_center
			or vmraid.db.get_value("Project", obj.project, "cost_center")
...
def make_rm_stock_entry(purchase_order, rm_items):
	...
	if isinstance(rm_items, str):
		...
		vmraid.throw(_("No Items available for transfer"))

	if rm_items_list:
		fg_items = list(set(d["item_code"] for d in rm_items_list))
	else:
		vmraid.throw(_("No Items selected for transfer"))

	if purchase_order:
		purchase_order = vmraid.get_doc("Purchase Order", purchase_order)
...
def get_materials_from_supplier(purchase_order, po_details):
	...
	doc = vmraid.get_cached_doc("Purchase Order", purchase_order)
...
def make_return_stock_entry_for_subcontract(available_materials, po_doc, po_details):
	ste_doc = vmraid.new_doc("Stock Entry")
