...
class TestSupplierScorecard(VMRaidTestCase):
	...
	def test_criteria_weight(self):
		...
		for d in my_doc.criteria:
			d.weight = 0
		self.assertRaises(vmraid.ValidationError, my_doc.insert)


def make_supplier_scorecard():
	my_doc = vmraid.get_doc(valid_scorecard[0])

	# Make sure the criteria exist (making them)
	for d in valid_scorecard[0].get("criteria"):
		if not vmraid.db.exists("Supplier Scorecard Criteria", d.get("criteria_name")):
			d["doctype"] = "Supplier Scorecard Criteria"
			d["name"] = d.get("criteria_name")
			my_criteria = vmraid.get_doc(d)
			my_criteria.insert()
	return my_doc
...
def delete_test_scorecards():
	my_doc = make_supplier_scorecard()
	if vmraid.db.exists("Supplier Scorecard", my_doc.name):
		...
		vmraid.delete_doc(my_doc.doctype, my_doc.name)
