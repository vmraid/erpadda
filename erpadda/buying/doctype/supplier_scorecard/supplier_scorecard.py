...
from vmraid.utils import add_days, add_years, get_last_day, getdate, nowdate

from erpadda.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import (
...
class SupplierScorecard(Document):
	...
	def calculate_total_score(self):
		...
		for scp in scorecards:
			my_sc = vmraid.get_doc("Supplier Scorecard Period", scp.name)
		...
		for standing in self.standings:
			if (not standing.min_grade or (standing.min_grade <= self.supplier_score)) and (
				...
				for fieldname in ("prevent_pos", "prevent_rfqs", "warn_rfqs", "warn_pos"):
					...
					vmraid.db.set_value("Supplier", self.supplier, fieldname, self.get(fieldname))


@vmraid.whitelist()
def get_timeline_data(doctype, name):
	# Get a list of all the associated scorecards
	scs = vmraid.get_doc(doctype, name)
	out = {}
	timeline_data = {}
	scorecards = vmraid.db.sql(
		"""
	...
	for sc in scorecards:
		start_date, end_date, total_score = vmraid.db.get_value(
...
def refresh_scorecards():
	...
	for sc in scorecards:
		...
		if make_all_scorecards(sc.name) > 0:
			...
			vmraid.get_doc("Supplier Scorecard", sc.name).save()


@vmraid.whitelist()
def make_all_scorecards(docname):

	sc = vmraid.get_doc("Supplier Scorecard", docname)
	supplier = vmraid.get_doc("Supplier", sc.supplier)
