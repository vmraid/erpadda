...
class VariablePathNotFound(vmraid.ValidationError):
	pass
	...
	def validate_path_exists(self):
		if "." in self.path:
			try:
				...
				vmraid.throw(_("Could not find path for " + self.path), VariablePathNotFound)
...
def get_item_workdays(scorecard):
	"""Gets the number of days in this period"""
	supplier = vmraid.get_doc("Supplier", scorecard.supplier)
	total_item_days = vmraid.db.sql(
		"""
			SELECT
...
def get_cost_of_on_time_shipments(scorecard):
	...
	total_delivered_on_time_costs = vmraid.db.sql(
		"""
			SELECT
...
def get_total_days_late(scorecard):
	...
	supplier = vmraid.get_doc("Supplier", scorecard.supplier)
	total_delivered_late_days = vmraid.db.sql(
		"""
			SELECT
	...
	total_missed_late_days = vmraid.db.sql(
		"""
			SELECT
...
def get_on_time_shipments(scorecard):
	...
	# Look up all PO Items with delivery dates between our dates
	total_items_delivered_on_time = vmraid.db.sql(
		"""
			SELECT
				COUNT(pr_item.qty)
			FROM
