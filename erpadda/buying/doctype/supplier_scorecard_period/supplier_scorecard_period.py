...
import erpadda.buying.doctype.supplier_scorecard_variable.supplier_scorecard_variable as variable_functions
from erpadda.buying.doctype.supplier_scorecard_criteria.supplier_scorecard_criteria import (
	get_variables,
)
...
class SupplierScorecardPeriod(Document):
		...
		for crit in self.criteria:
			try:
				crit.score = min(
					crit.max_score,
					max(
						0, vmraid.safe_eval(self.get_eval_statement(crit.formula), None, {"max": max, "min": min})
					),
				)
			except Exception:
				vmraid.throw(
	...
	def calculate_weighted_score(self, weighing_function):
		try:
			weighed_score = vmraid.safe_eval(
...
def make_supplier_scorecard(source_name, target_doc=None):
	def update_criteria_fields(obj, target, source_parent):
		target.max_score, target.formula = vmraid.db.get_value(
