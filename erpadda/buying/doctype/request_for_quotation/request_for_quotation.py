...
from vmraid.utils.print_format import download_pdf
from vmraid.utils.user import get_user_fullname

from erpadda.accounts.party import get_party_account_currency, get_party_details
from erpadda.buying.utils import validate_for_items
from erpadda.controllers.buying_controller import BuyingController
from erpadda.stock.doctype.material_request.material_request import set_missing_values
...
class RequestforQuotation(BuyingController):
	def validate(self):
		...
		if self.docstatus < 1:
			...
			vmraid.db.set(self, "status", "Draft")

	def validate_duplicate_supplier(self):
		supplier_list = [d.supplier for d in self.suppliers]
		if len(supplier_list) != len(set(supplier_list)):
			vmraid.throw(_("Same supplier has been entered multiple times"))

	def validate_supplier_list(self):
		for d in self.suppliers:
			prevent_rfqs = vmraid.db.get_value("Supplier", d.supplier, "prevent_rfqs")
			if prevent_rfqs:
				standing = vmraid.db.get_value("Supplier Scorecard", d.supplier, "status")
				vmraid.throw(
					_("RFQs are not allowed for {0} due to a scorecard standing of {1}").format(
						d.supplier, standing
					)
				)
			warn_rfqs = vmraid.db.get_value("Supplier", d.supplier, "warn_rfqs")
			if warn_rfqs:
				standing = vmraid.db.get_value("Supplier Scorecard", d.supplier, "status")
				vmraid.msgprint(
					_(
	...
	def update_email_id(self):
		for rfq_supplier in self.suppliers:
			if not rfq_supplier.email_id:
				rfq_supplier.email_id = vmraid.db.get_value("Contact", rfq_supplier.contact, "email_id")

	def validate_email_id(self, args):
		if not args.email_id:
			vmraid.throw(
				_("Row {0}: For Supplier {1}, Email Address is Required to send an email").format(
					args.idx, vmraid.bold(args.supplier)
				)
			)
	...
	def update_supplier_part_no(self, supplier):
		...
		for item in self.items:
			item.supplier_part_no = vmraid.db.get_value(
	...
	def update_supplier_contact(self, rfq_supplier, link):
		...
		if vmraid.db.exists("User", rfq_supplier.email_id):
			user = vmraid.get_doc("User", rfq_supplier.email_id)
		else:
	...
	def link_supplier_contact(self, rfq_supplier, user):
		...
		if rfq_supplier.contact:
			contact = vmraid.get_doc("Contact", rfq_supplier.contact)
		else:
	...
	def supplier_rfq_mail(self, data, update_password_link, rfq_link, preview=False):
		full_name = get_user_fullname(vmraid.session["user"])
		if full_name == "Guest":
		...
		args = {
			...
			"message": vmraid.render_template(self.message_for_supplier, doc_args),
			"rfq_link": rfq_link,
		...
		sender = vmraid.session.user not in STANDARD_USERS and vmraid.session.user or None
		message = vmraid.get_template(template).render(args)
	...
	def send_email(self, data, sender, subject, message, attachments):
		...
		vmraid.msgprint(_("Email Sent to Supplier {0}").format(data.supplier))

	def get_attachments(self):
		attachments = [d.name for d in get_attachments(self.doctype, self.name)]
		attachments.append(vmraid.attach_print(self.doctype, self.name, doc=self))
		return attachments
...
def send_supplier_emails(rfq_name):
	...
	rfq = vmraid.get_doc("Request for Quotation", rfq_name)
	if rfq.docstatus == 1:
...
def make_supplier_quotation_from_rfq(source_name, target_doc=None, for_supplier=None):
	def postprocess(source, target_doc):
		if for_supplier:
			...
			target_doc.buying_price_list = args.buying_price_list or vmraid.db.get_value(
...
def create_supplier_quotation(doc):
	...
	try:
		sq_doc = vmraid.get_doc(
			{
				...
				or vmraid.db.get_value("Buying Settings", None, "buying_price_list"),
			}
		)
		...
		sq_doc.save()
		vmraid.msgprint(_("Supplier Quotation {0} Created").format(sq_doc.name))
		return sq_doc.name
	except Exception:
		return None
	...
	args.update(
		{
			...
			"supplier_part_no": vmraid.db.get_value(
...
def get_supplier_tag():
	...
	tags = list(
		set(tag.tag for tag in vmraid.get_all("Tag Link", filters=filters, fields=["tag"]) if tag)
	)
