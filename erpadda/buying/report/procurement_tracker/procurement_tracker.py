...
def get_conditions(filters):
	...
	if filters.get("company"):
		conditions += " AND parent.company=%s" % vmraid.db.escape(filters.get("company"))

	if filters.get("cost_center") or filters.get("project"):
		conditions += """
			AND (child.`cost_center`=%s OR child.`project`=%s)
			""" % (
			vmraid.db.escape(filters.get("cost_center")),
			vmraid.db.escape(filters.get("project")),
		)
...
def get_mapped_mr_details(conditions):
	...
	for record in mr_details:
		if record.per_ordered:
			mr_records.setdefault(record.name, []).append(vmraid._dict(record))
		else:
