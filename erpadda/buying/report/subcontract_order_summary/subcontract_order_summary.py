...
def get_subcontracted_orders(report_filters):
	...
	return vmraid.get_all("Purchase Order", fields=fields, filters=filters) or []
...
def get_supplied_items(orders, report_filters):
	...
	supplied_items = {}
	for row in vmraid.get_all("Purchase Order Item Supplied", fields=fields, filters=filters):
...
def prepare_subcontracted_data(orders, supplied_items):
	...
	for row in orders:
		...
		if key not in po_details:
			po_details.setdefault(key, vmraid._dict({"po_item": row, "supplied_items": []}))
