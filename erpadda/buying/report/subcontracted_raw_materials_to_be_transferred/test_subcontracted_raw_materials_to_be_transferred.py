...
# Embedded file name: /Users/anuragmishra/vmraid-develop/apps/erpadda/erpadda/buying/report/subcontracted_raw_materials_to_be_transferred/test_subcontracted_raw_materials_to_be_transferred.py
...
from erpadda.buying.doctype.purchase_order.purchase_order import make_rm_stock_entry
from erpadda.buying.doctype.purchase_order.test_purchase_order import create_purchase_order
from erpadda.buying.report.subcontracted_raw_materials_to_be_transferred.subcontracted_raw_materials_to_be_transferred import (
	execute,
)
from erpadda.stock.doctype.stock_entry.test_stock_entry import make_stock_entry


class TestSubcontractedItemToBeTransferred(VMRaidTestCase):
			...
			filters=vmraid._dict(
				{
					...
					"from_date": vmraid.utils.get_datetime(
						vmraid.utils.add_to_date(po.transaction_date, days=-10)
					),
					"to_date": vmraid.utils.get_datetime(vmraid.utils.add_to_date(po.transaction_date, days=10)),
				}
			)
		)
		po.reload()
