...
def get_data(filters, conditions):
	supplier_quotation_data = vmraid.db.sql(
		"""
		SELECT
...
def prepare_data(supplier_quotation_data, filters):
	...
	company_currency = vmraid.db.get_default("currency")
	...
	for data in supplier_quotation_data:
		...
		supplier_currency = vmraid.db.get_value(
...
def prepare_chart_data(suppliers, qty_list, supplier_qty_price_map):
	...
	dataset = []
	currency_symbol = vmraid.db.get_value("Currency", vmraid.db.get_default("currency"), "symbol")
	for qty in qty_list:
		datapoints = {
