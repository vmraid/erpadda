...
vmraid.query_reports["Supplier Quotation Comparison"] = {
	filters: [
		{
			fieldtype: "Link",
			label: __("Company"),
		...
		},
		{
			...
			fieldtype: "Link",
			get_query: () => {
				let quote = vmraid.query_report.get_filter_value('supplier_quotation');
				if (quote != "") {
					return {
						query: "erpadda.stock.doctype.quality_inspection.quality_inspection.item_query",
						filters: {
		...
		},
		{
			...
			get_data: function(txt) {
				return vmraid.db.get_link_options('Supplier', txt);
			}
		},
		{
			fieldtype: "MultiSelectList",
			...
			default: "",
			get_data: function(txt) {
				return vmraid.db.get_link_options('Supplier Quotation', txt, {'docstatus': ["<", 2]});
			}
		},
		{
			fieldtype: "Link",
	...
	formatter: (value, row, column, data, default_formatter) => {
		...
		if(column.fieldname === "valid_till" && data.valid_till){
			if(vmraid.datetime.get_diff(data.valid_till, vmraid.datetime.nowdate()) <= 1){
				value = `<div style="color:red">${value}</div>`;
			}
			else if (vmraid.datetime.get_diff(data.valid_till, vmraid.datetime.nowdate()) <= 7){
	...
	onload: (report) => {
		...
		report.page.add_inner_button(__("Select Default Supplier"), () => {
			let reporter = vmraid.query_reports["Quoted Item Comparison"];
		...
		dialog.set_primary_action("Set Default Supplier", () => {
			...
			if(values) {
				...
				vmraid.call({
					...
					freeze: true,
					callback: (r) => {
						vmraid.msgprint(__("Successfully Set Supplier"));
						dialog.hide();
					}
				});
			}
