...
vmraid.ready(function() {
	...
	vmraid.call({
		type: "POST",
		method: "erpadda.e_commerce.shopping_cart.product_info.get_product_info_for_website",
		args: {
	...
	$("#item-add-to-cart button").on("click", function() {
		vmraid.provide('erpadda.shopping_cart');

		erpadda.shopping_cart.update_cart({
			item_code: get_item_code(),
	...
	$("[itemscope] .item-view-attribute .form-control").on("change", function() {
		try {
			...
			if (!item_code) {
				vmraid.msgprint(__("Cannot find a matching Item. Please select some other value for {0}.", [attribute]))
				throw e;
			}
		}
