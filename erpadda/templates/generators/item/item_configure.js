class ItemConfigure {
	...
	show_range_input_if_applicable(fieldname) {
		...
		if (changed_value && changed_value.includes(' to ')) {
			...
			if (!numbers.some(n => isNaN(n))) {
				...
				const control = vmraid.ui.form.make_control({
					df: {
						fieldtype: 'Int',
	...
	get_next_attribute_and_values(selected_attributes) {
		return this.call('erpadda.e_commerce.variant_selector.utils.get_next_attribute_and_values', {
	...
	get_attributes_and_values() {
		return this.call('erpadda.e_commerce.variant_selector.utils.get_attributes_and_values', {
			item_code: this.item_code
		});
	}
	...
	call(method, args) {
		// promisified vmraid.call
		return new Promise((resolve, reject) => {
			vmraid.call(method, args)
