...
from vmraid.utils import flt, get_url
...
def check_mandate(data, reference_doctype, reference_docname):
	...
	payer = vmraid.get_doc("Customer", data["payer_name"])

	if payer.customer_type == "Individual" and payer.customer_primary_contact is not None:
		primary_contact = vmraid.get_doc("Contact", payer.customer_primary_contact)
		prefilled_customer = {
		...
		if primary_contact.email_id is not None:
			...
			prefilled_customer.update({"email": vmraid.session.user})

	else:
		prefilled_customer = {"company_name": payer.name, "email": vmraid.session.user}
	...
	try:
		redirect_flow = client.redirect_flows.create(
			params={
				...
				"session_token": vmraid.session.user,
