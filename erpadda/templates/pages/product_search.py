...
from erpadda.e_commerce.shopping_cart.product_info import set_product_info_for_website
from erpadda.setup.doctype.item_group.item_group import get_item_for_list_in_html
...
def get_product_data(search=None, start=0, limit=12):
	...
	return vmraid.db.sql(query, {"search": search}, as_dict=1)  # nosemgrep
...
def product_search(query, limit=10, fuzzy_search=True):
	...
	search_results["results"] = sorted(
		search_results["results"], key=lambda k: vmraid.utils.cint(k["ranking"]), reverse=True
	)
...
def get_category_suggestions(query):
	...
	ac = AutoCompleter(make_key(WEBSITE_ITEM_CATEGORY_AUTOCOMPLETE), conn=vmraid.cache())
