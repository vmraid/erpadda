...
def setup_taxes_and_charges(company_name: str, country: str):
	if not vmraid.db.exists("Company", company_name):
		vmraid.throw(_("Company {} does not exist yet. Taxes setup aborted.").format(company_name))
...
def from_detailed_data(company_name, data):
	...
	coa_name = vmraid.db.get_value("Company", company_name, "chart_of_accounts")
...
def update_regional_tax_settings(country, company):
	path = vmraid.get_app_path("erpadda", "regional", vmraid.scrub(country))
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = "erpadda.regional.{0}.setup.update_regional_tax_settings".format(
				vmraid.scrub(country)
			)
			vmraid.get_attr(module_name)(country, company)
...
def make_taxes_and_charges_template(company_name, doctype, template):
	...
	if vmraid.db.exists(doctype, {"title": template.get("title"), "company": company_name}):
		return
	...
	for tax_row in template.get("taxes"):
		...
		tax_row_defaults = {
			...
			"cost_center": vmraid.db.get_value("Company", company_name, "cost_center"),
		}
...
def get_or_create_account(company_name, account):
	...
	existing_accounts = vmraid.get_all(
		"Account",
	...
	if existing_accounts:
		return vmraid.get_doc("Account", existing_accounts[0].name)
...
def get_or_create_tax_group(company_name, root_type):
	...
	tax_group_name = vmraid.db.get_value(
		"Account",
	...
	tax_group_account = vmraid.get_doc(
		{
			"doctype": "Account",
			"company": company_name,
			"is_group": 1,
...
def make_tax_category(tax_category):
	...
	if not vmraid.db.exists(doctype, tax_category["title"]):
		doc = vmraid.get_doc(tax_category)
		doc.insert(ignore_permissions=True)
