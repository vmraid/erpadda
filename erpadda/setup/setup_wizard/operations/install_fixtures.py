...
from vmraid.desk.page.setup_wizard.setup_wizard import make_records
from vmraid.utils import cstr, getdate
from vmraid.utils.nestedset import rebuild_tree

from erpadda.accounts.doctype.account.account import RootNotEditable
...
def install(country=None):
	...
	from erpadda.setup.setup_wizard.data.industry_type import get_industry_types
...
def update_selling_defaults():
	selling_settings = vmraid.get_doc("Selling Settings")
...
def update_buying_defaults():
	buying_settings = vmraid.get_doc("Buying Settings")
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(
		open(vmraid.get_app_path("erpadda", "setup", "setup_wizard", "data", "uom_data.json")).read()
	)
	for d in uoms:
		if not vmraid.db.exists("UOM", _(d.get("uom_name"))):
			uom_doc = vmraid.get_doc(
				{
					"doctype": "UOM",
	...
	uom_conversions = json.loads(
		open(
			vmraid.get_app_path("erpadda", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
	for d in uom_conversions:
		if not vmraid.db.exists("UOM Category", _(d.get("category"))):
			vmraid.get_doc({"doctype": "UOM Category", "category_name": _(d.get("category"))}).db_insert()

		if not vmraid.db.exists(
			"UOM Conversion Factor", {"from_uom": _(d.get("from_uom")), "to_uom": _(d.get("to_uom"))}
		):
			uom_conversion = vmraid.get_doc(
				{
...
def add_sale_stages():
	...
	for sales_stage in records:
		vmraid.get_doc(sales_stage).db_insert()
...
def install_post_company_fixtures(args=None):
	...
	vmraid.local.flags.ignore_update_nsm = True
	make_records(records[1:])
	vmraid.local.flags.ignore_update_nsm = False
...
def install_defaults(args=None):
	...
	vmraid.db.set_value("Currency", args.get("currency"), "enabled", 1)
	vmraid.db.set_value("Stock Settings", None, "email_footer_address", args.get("company_name"))
...
def set_global_defaults(args):
	...
	current_fiscal_year = vmraid.get_all("Fiscal Year")[0]
...
def set_active_domains(args):
	vmraid.get_single("Domain Settings").set_active_domains(args.get("domains"))
...
def update_stock_settings():
	...
	stock_settings.default_warehouse = vmraid.db.get_value(
...
def create_bank_account(args):
	...
	if bank_account_group:
		...
		try:
			...
			vmraid.throw(_("Bank account cannot be named as {0}").format(args.get("bank_account")))
...
def update_shopping_cart_settings(args):  # nosemgrep
	shopping_cart = vmraid.get_doc("E Commerce Settings")
	shopping_cart.update(
		{
			"enabled": 1,
			"company": args.company_name,
			"price_list": vmraid.db.get_value("Price List", {"selling": 1}),
