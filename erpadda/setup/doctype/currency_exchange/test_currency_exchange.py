...
test_records = vmraid.get_test_records("Currency Exchange")
...
def save_new_records(test_records):
	for record in test_records:
		...
		try:
			vmraid.set_value(**kwargs)
		except vmraid.DoesNotExistError:
			curr_exchange = vmraid.new_doc(record.get("doctype"))
...
class TestCurrencyExchange(unittest.TestCase):
	...
	def tearDown(self):
		vmraid.db.set_value("Accounts Settings", None, "allow_stale", 1)
		self.clear_cache()
	...
	def test_exchange_rate_via_exchangerate_host(self, mock_get):
		...
		settings = vmraid.get_single("Currency Exchange Settings")
	...
	def test_exchange_rate_strict(self, mock_get):
		...
		vmraid.db.set_value("Accounts Settings", None, "allow_stale", 0)
		vmraid.db.set_value("Accounts Settings", None, "stale_days", 1)
