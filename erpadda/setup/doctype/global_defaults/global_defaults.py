...
class GlobalDefaults(Document):
	def on_update(self):
		"""update defaults"""
		for key in keydict:
			vmraid.db.set_default(key, self.get(keydict[key], ""))

		# update year start date and year end date from fiscal_year
		year_start_end_date = vmraid.db.sql(
		...
		if year_start_end_date:
			...
			if ysd and yed:
				vmraid.db.set_default("year_start_date", ysd.strftime("%Y-%m-%d"))
				vmraid.db.set_default("year_end_date", yed.strftime("%Y-%m-%d"))
	...
	def get_defaults(self):
		return vmraid.defaults.get_defaults()
