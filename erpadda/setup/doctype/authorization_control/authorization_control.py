...
import vmraid
from vmraid import _, session
from vmraid.utils import comma_or, cstr, flt, has_common
...
class AuthorizationControl(TransactionBase):
	def get_appr_user_role(self, det, doctype_name, total, based_on, condition, item, company):
		...
		if det:
			...
			if not has_common(appr_roles, vmraid.get_roles()) and not has_common(
				appr_users, [session["user"]]
			):
				vmraid.msgprint(_("Not authroized since {0} exceeds limits").format(_(based_on)))
				vmraid.throw(_("Can be approved by {0}").format(comma_or(appr_roles + appr_users)))

	def validate_auth_rule(self, doctype_name, total, based_on, cond, company, item=""):
		chk = 1
		add_cond1, add_cond2 = "", ""
		if based_on == "Itemwise Discount":
			add_cond1 += " and master_name = " + vmraid.db.escape(cstr(item))
			itemwise_exists = vmraid.db.sql(
	...
	def bifurcate_based_on_type(self, doctype_name, total, av_dis, based_on, doc_obj, val, company):
		...
		if val == 1:
			add_cond += " and system_user = {}".format(vmraid.db.escape(session["user"]))
		elif val == 2:
			add_cond += " and system_role IN %s" % ("('" + "','".join(vmraid.get_roles()) + "')")
		else:
		...
		if based_on == "Grand Total":
			...
			if doc_obj:
				...
				add_cond = " and master_name = {}".format(vmraid.db.escape(customer))
		if based_on == "Itemwise Discount":
			if doc_obj:
	...
	def validate_approving_authority(self, doctype_name, company, total, doc_obj=""):
		if not vmraid.db.count("Authorization Rule"):
			return
