...
from vmraid.permissions import get_doctypes_with_read
from vmraid.utils import cint, cstr


class NamingSeriesNotSetError(vmraid.ValidationError):
	pass
...
class NamingSeries(Document):
	...
	def get_transactions(self, arg=None):
		...
		for d in doctypes:
			...
			try:
				...
				vmraid.msgprint(_("Unable to find DocType {0}").format(d))
				# vmraid.pass_does_not_exist_error()
				continue
		...
		custom_prefixes = vmraid.get_all(
			"DocType",
			fields=["autoname"],
			filters={
	...
	def validate_series_set(self):
		if self.select_doc_for_series and not self.set_options:
			vmraid.throw(_("Please set the series to be used."))
	...
	def set_series_for(self, doctype, ol):
		...
		for prop in prop_dict:
			ps_exists = vmraid.db.get_value(
				"Property Setter", {"field_name": "naming_series", "doc_type": doctype, "property": prop}
			)

			if ps_exists:
				ps = vmraid.get_doc("Property Setter", ps_exists)
	...
	def check_duplicate(self):
		...
		sr = [[vmraid.get_meta(p).get_field("naming_series").options, p] for p in parent]
		dt = vmraid.get_doc("DocType", self.select_doc_for_series)
			...
			for i in sr:
				if i[0]:
					...
					if series.split(".")[0] in existing_series:
						vmraid.throw(_("Series {0} already used in {1}").format(series, i[1]))
	...
	def get_options(self, arg=None):
		if vmraid.get_meta(arg or self.select_doc_for_series).get_field("naming_series"):
			return vmraid.get_meta(arg or self.select_doc_for_series).get_field("naming_series").options
	...
	def get_current(self, arg=None):
		...
		if self.prefix:
			...
			self.current_value = vmraid.db.get_value("Series", prefix, "current", order_by="name")

	def insert_series(self, series):
		"""insert series if missing"""
		if vmraid.db.get_value("Series", series, "name", order_by="name") == None:
			vmraid.db.sql("insert into tabSeries (name, current) values (%s, 0)", (series))
