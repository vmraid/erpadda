...
from erpadda.e_commerce.product_data_engine.filters import ProductFiltersBuilder
...
class ItemGroup(NestedSet, WebsiteGenerator):
	...
	def validate(self):
		...
		if not self.parent_item_group and not vmraid.flags.in_test:
			if vmraid.db.exists("Item Group", _("All Item Groups")):
	...
	def make_route(self):
		...
		if not self.route:
			...
			if self.parent_item_group:
				parent_item_group = vmraid.get_doc("Item Group", self.parent_item_group)
	...
	def get_context(self, context):
		...
		context.page_length = (
			cint(vmraid.db.get_single_value("E Commerce Settings", "products_per_page")) or 6
		)
		...
		if self.slideshow:
			...
			slideshow = vmraid.get_doc("Website Slideshow", self.slideshow)
	...
	def delete_child_item_groups_key(self):
		vmraid.cache().hdel("child_item_groups", self.name)

	def validate_item_group_defaults(self):
		from erpadda.stock.doctype.item.item import validate_item_default_company_links
...
def get_child_groups_for_website(item_group_name, immediate=False, include_self=False):
	...
	item_group = vmraid.get_cached_value("Item Group", item_group_name, ["lft", "rgt"], as_dict=1)
	...
	return vmraid.get_all("Item Group", filters=filters, fields=["name", "route"], order_by="name")
...
def get_item_for_list_in_html(context):
	...
	context["show_availability_status"] = cint(
		vmraid.db.get_single_value("E Commerce Settings", "show_availability_status")
	)

	products_template = "templates/includes/products_as_list.html"

	return vmraid.get_template(products_template).render(context)


def get_parent_item_groups(item_group_name, from_item=False):
	base_nav_page = {"name": _("Shop by Category"), "route": "/shop-by-category"}

	if from_item and vmraid.request.environ.get("HTTP_REFERER"):
		# base page after 'Home' will vary on Item page
		last_page = vmraid.request.environ["HTTP_REFERER"].split("/")[-1]
	...
	item_group = vmraid.db.get_value("Item Group", item_group_name, ["lft", "rgt"], as_dict=1)
	parent_groups = vmraid.db.sql(
...
def invalidate_cache_for(doc, item_group=None):
	...
	for d in get_parent_item_groups(item_group):
		item_group_name = vmraid.db.get_value("Item Group", d.get("name"))
		if item_group_name:
			clear_cache(vmraid.db.get_value("Item Group", item_group_name, "route"))


def get_item_group_defaults(item, company):
	item = vmraid.get_cached_doc("Item", item)
	item_group = vmraid.get_cached_doc("Item Group", item.item_group)
