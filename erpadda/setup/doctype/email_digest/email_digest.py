...
import vmraid
import vmraid.desk.notifications
...
from erpadda.accounts.utils import get_balance_on, get_count_on, get_fiscal_year
...
class EmailDigest(Document):
	def __init__(self, *args, **kwargs):
		...
		self.set_dates()
		self._accounts = {}
		self.currency = vmraid.db.get_value("Company", self.company, "default_currency")

	@vmraid.whitelist()
	def get_users(self):
		"""get list of users"""
		user_list = vmraid.db.sql(
			"""
		...
		vmraid.response["user_list"] = user_list
	...
	def get_msg_html(self):
		...
		from erpadda.setup.doctype.email_digest.quotes import get_random_quote
		...
		if self.get("purchase_orders_items_overdue"):
			...
			if not context.purchase_order_list:
				vmraid.throw(_("No items to be received are overdue"))
		...
		return vmraid.render_template(
			"erpadda/setup/doctype/email_digest/templates/default.html", context, is_path=True
		)
	...
	def get_notifications(self):
		...
		notifications = vmraid.desk.notifications.get_notifications()
	...
	def get_todo_list(self, user_id=None):
		"""Get to-do list"""
		if not user_id:
			user_id = vmraid.session.user

		todo_list = vmraid.db.sql(
			"""select *
	...
	def get_issue_list(self, user_id=None):
		...
		meta = vmraid.get_meta("Issue")
		role_permissions = vmraid.permissions.get_role_permissions(meta, user_id)
		if not role_permissions.get("read"):
			return None

		issue_list = vmraid.db.sql(
			"""select *
	...
	def set_accounting_cards(self, context):
		...
		for key in (
			...
			if self.get(key):
				...
				if card:
					card = vmraid.safe_eval(card)

				else:
					card = vmraid._dict(getattr(self, "get_" + key)())
	...
	def get_income(self):
		...
		income_account = vmraid.db.get_all(
			"Account",
			fields=["name"],
	...
	def get_expenses_booked(self):
		...
		expense_account = vmraid.db.get_all(
			"Account",
			fields=["name"],
	...
	def get_summary_of_pending(self, doc_type, fieldname, getfield):
		...
		value, count, billed_value, delivered_value = vmraid.db.sql(
	...
	def get_purchase_orders_items_overdue_list(self):
		...
		purchase_order_list = vmraid.db.sql(sql_po, as_dict=True)
		purchase_order_items_overdue_list = vmraid.db.sql(sql_poi, as_dict=True)
...
def send():
	...
	for ed in vmraid.db.sql(
		"""select name from `tabEmail Digest`
			where enabled=1 and docstatus<2""",
		as_list=1,
	):
		ed_obj = vmraid.get_doc("Email Digest", ed[0])
...
def get_digest_msg(name):
	return vmraid.get_doc("Email Digest", name).get_msg_html()
