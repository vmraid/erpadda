...
vmraid.provide("erpadda.company");

vmraid.ui.form.on("Company", {
	onload: function(frm) {
		if (frm.doc.__islocal && frm.doc.parent_company) {
			vmraid.db.get_value('Company', frm.doc.parent_company, 'is_group', (r) => {
				if (!r.is_group) {
	...
	setup: function(frm) {
		erpadda.company.setup_queries(frm);
	...
	date_of_commencement: function(frm) {
		...
		{
			vmraid.throw(__("Date of Commencement should be greater than Date of Incorporation"));
		}
	...
	refresh: function(frm) {
		...
		if (!frm.is_new()) {
			...
			vmraid.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Company'}

			if (vmraid.perm.has_perm("Cost Center", 0, 'read')) {
				frm.add_custom_button(__('Cost Centers'), function() {
					vmraid.set_route('Tree', 'Cost Center', {'company': frm.doc.name});
				}, __("View"));
			}

			if (vmraid.perm.has_perm("Account", 0, 'read')) {
				frm.add_custom_button(__('Chart of Accounts'), function() {
					vmraid.set_route('Tree', 'Account', {'company': frm.doc.name});
				}, __("View"));
			}

			if (vmraid.perm.has_perm("Sales Taxes and Charges Template", 0, 'read')) {
				frm.add_custom_button(__('Sales Tax Template'), function() {
					vmraid.set_route('List', 'Sales Taxes and Charges Template', {'company': frm.doc.name});
				}, __("View"));
			}

			if (vmraid.perm.has_perm("Purchase Taxes and Charges Template", 0, 'read')) {
				frm.add_custom_button(__('Purchase Tax Template'), function() {
					vmraid.set_route('List', 'Purchase Taxes and Charges Template', {'company': frm.doc.name});
				}, __("View"));
			}
		...
		erpadda.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	make_default_tax_template: function(frm) {
		frm.call({
			...
			freeze: true,
			callback: function() {
				vmraid.msgprint(__("Default tax templates for sales, purchase and items are created."));
			}
		})
	},
		...
		vmraid.verify_password(function() {
			...
			function(data) {
				if(data.company_name !== frm.doc.name) {
					vmraid.msgprint(__("Company name not same"));
					return;
				}
				vmraid.call({
					method: "erpadda.setup.doctype.company.company.create_transaction_deletion_request",
					args: {
					...
					callback: function(r, rt) {
						if(!r.exc)
							vmraid.msgprint(__("Successfully deleted all transactions related to this company!"));
					},
					onerror: function() {
						vmraid.msgprint(__("Wrong Password"));
					}
				});
			},
...
erpadda.company.set_chart_of_accounts_options = function(doc) {
	var selected_value = doc.chart_of_accounts;
	if(doc.country) {
		return vmraid.call({
			method: "erpadda.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				"country": doc.country,
				"with_standard": true
			},
...
erpadda.company.setup_queries = function(frm) {
	$.each([
		...
		erpadda.company.set_custom_query(frm, v);
	});
...
erpadda.company.set_custom_query = function(frm, v) {
	var filters = {
