...
class TestCompany(unittest.TestCase):
	...
	def test_basic_tree(self, records=None):
		min_lft = 1
		max_rgt = vmraid.db.sql("select max(rgt) from `tabCompany`")[0][0]

		if not records:
			records = test_records[2:]

		for company in records:
			lft, rgt, parent_company = vmraid.db.get_value(
				"Company", company["company_name"], ["lft", "rgt", "parent_company"]
			)

			if parent_company:
				parent_lft, parent_rgt = vmraid.db.get_value("Company", parent_company, ["lft", "rgt"])
			else:
				# root
	...
	def test_change_parent_company(self):
		child_company = vmraid.get_doc("Company", "_Test Company 5")
