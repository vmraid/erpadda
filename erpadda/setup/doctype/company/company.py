...
from vmraid.cache_manager import clear_defaults_cache
...
from vmraid.utils import cint, formatdate, get_timestamp, today
from vmraid.utils.nestedset import NestedSet

from erpadda.accounts.doctype.account.account import get_account_currency
from erpadda.setup.setup_wizard.operations.taxes_setup import setup_taxes_and_charges
...
class Company(NestedSet):
	...
	def validate_abbr(self):
		...
		# 	vmraid.throw(_("Abbreviation cannot have more than 5 characters"))

		if not self.abbr.strip():
			vmraid.throw(_("Abbreviation is mandatory"))

		if vmraid.db.sql(
			"select abbr from tabCompany where name!=%s and abbr=%s", (self.name, self.abbr)
		):
			vmraid.throw(_("Abbreviation already used for another company"))
	...
	def validate_default_accounts(self):
		...
		for account in accounts:
			if self.get(account[1]):
				for_company = vmraid.db.get_value("Account", self.get(account[1]), "company")
				...
				if get_account_currency(self.get(account[1])) != self.default_currency:
					...
					).format(vmraid.bold(account[0]))
					vmraid.throw(error_message)

	def validate_currency(self):
		if self.is_new():
			return
		self.previous_default_currency = vmraid.get_cached_value(
	...
	def on_update(self):
		...
		if not vmraid.db.sql(
			...
			if not vmraid.local.flags.ignore_chart_of_accounts:
				vmraid.flags.country_change = True
				self.create_default_accounts()
				self.create_default_warehouses()

		if not vmraid.db.get_value("Cost Center", {"is_group": 0, "company": self.name}):
			self.create_default_cost_center()

		if vmraid.flags.country_change:
			install_country_fixtures(self.name, self.country)
			self.create_default_tax_template()

		if not vmraid.db.get_value("Department", {"company": self.name}):
			from erpadda.setup.setup_wizard.operations.install_fixtures import install_post_company_fixtures

			install_post_company_fixtures(vmraid._dict({"company_name": self.name}))
		...
		if self.default_currency:
			vmraid.db.set_value("Currency", self.default_currency, "enabled", 1)

		if (
			hasattr(vmraid.local, "enable_perpetual_inventory")
			and self.name in vmraid.local.enable_perpetual_inventory
		):
			vmraid.local.enable_perpetual_inventory[self.name] = self.enable_perpetual_inventory

		if vmraid.flags.parent_company_changed:
	...
	def create_default_accounts(self):
		from erpadda.accounts.doctype.account.chart_of_accounts.chart_of_accounts import create_charts

		vmraid.local.flags.ignore_root_company_validation = True
	...
	def validate_coa_input(self):
		if self.create_chart_of_accounts_based_on == "Existing Company":
			...
			if not self.existing_company:
				vmraid.throw(_("Please select Existing Company for creating Chart of Accounts"))
	...
	def validate_provisional_account_for_non_stock_items(self):
		if not self.get("__islocal"):
			if (
				...
				vmraid.throw(
					_("Set default {0} account for non stock items").format(vmraid.bold("Provisional Account"))
				)
	...
	def check_country_change(self):
		vmraid.flags.country_change = False

		if not self.is_new() and self.country != vmraid.get_cached_value(
	...
	def validate_parent_company(self):
		if self.parent_company:
			is_group = vmraid.get_value("Company", self.parent_company, "is_group")

			if not is_group:
				vmraid.throw(_("Parent Company must be a group company"))
	...
	def set_default_accounts(self):
		...
		if not self.default_income_account:
			income_account = vmraid.db.get_value(
		...
		if not self.default_employee_advance_account:
			employe_advance_account = vmraid.db.get_value(
		...
		if not self.write_off_account:
			write_off_acct = vmraid.db.get_value(
		...
		if not self.exchange_gain_loss_account:
			exchange_gain_loss_acct = vmraid.db.get_value(
		...
		if not self.disposal_account:
			disposal_acct = vmraid.db.get_value(
				"Account",
	...
	def set_mode_of_payment_account(self):
		cash = vmraid.db.get_value("Mode of Payment", {"type": "Cash"}, "name")
		if (
			cash
			and self.default_cash_account
			and not vmraid.db.get_value("Mode of Payment Account", {"company": self.name, "parent": cash})
		):
			mode_of_payment = vmraid.get_doc("Mode of Payment", cash, for_update=True)
	...
	def create_default_cost_center(self):
		...
		vmraid.db.set(self, "cost_center", _("Main") + " - " + self.abbr)
		vmraid.db.set(self, "round_off_cost_center", _("Main") + " - " + self.abbr)
		vmraid.db.set(self, "depreciation_cost_center", _("Main") + " - " + self.abbr)

	def after_rename(self, olddn, newdn, merge=False):
		vmraid.db.set(self, "company_name", newdn)
	...
	def on_trash(self):
		...
		rec = vmraid.db.sql("SELECT name from `tabGL Entry` where company = %s", self.name)
		if not rec:
			vmraid.db.sql(
			...
			for doctype in ["Account", "Cost Center", "Budget", "Party Account"]:
				vmraid.db.sql("delete from `tab{0}` where company = %s".format(doctype), self.name)

		if not vmraid.db.get_value("Stock Ledger Entry", {"company": self.name}):
			vmraid.db.sql("""delete from `tabWarehouse` where company=%s""", self.name)

		vmraid.defaults.clear_default("company", value=self.name)
		for doctype in ["Mode of Payment Account", "Item Default"]:
			vmraid.db.sql("delete from `tab{0}` where company = %s".format(doctype), self.name)

		# clear default accounts, warehouses from item
		warehouses = vmraid.db.sql_list("select name from tabWarehouse where company=%s", self.name)
		if warehouses:
			vmraid.db.sql(
		...
		# delete BOMs
		boms = vmraid.db.sql_list("select name from tabBOM where company=%s", self.name)
		if boms:
			vmraid.db.sql("delete from tabBOM where company=%s", self.name)
		...
		vmraid.db.sql("delete from tabEmployee where company=%s", self.name)
		vmraid.db.sql("delete from tabDepartment where company=%s", self.name)
		vmraid.db.sql("delete from `tabTax Withholding Account` where company=%s", self.name)
		vmraid.db.sql("delete from `tabTransaction Deletion Record` where company=%s", self.name)

		# delete tax templates
		vmraid.db.sql("delete from `tabSales Taxes and Charges Template` where company=%s", self.name)
		vmraid.db.sql("delete from `tabPurchase Taxes and Charges Template` where company=%s", self.name)
		vmraid.db.sql("delete from `tabItem Tax Template` where company=%s", self.name)

		# delete Process Deferred Accounts if no GL Entry found
		if not vmraid.db.get_value("GL Entry", {"company": self.name}):
			vmraid.db.sql("delete from `tabProcess Deferred Accounting` where company=%s", self.name)

	def check_parent_changed(self):
		vmraid.flags.parent_company_changed = False

		if not self.is_new() and self.parent_company != vmraid.db.get_value(
			"Company", self.name, "parent_company"
		):
			vmraid.flags.parent_company_changed = True
...
def install_country_fixtures(company, country):
	try:
		module_name = f"erpadda.regional.{vmraid.scrub(country)}.setup.setup"
		vmraid.get_attr(module_name)(company, False)
	except ImportError:
		pass
	except Exception:
		vmraid.log_error()
...
def update_company_current_month_sales(company):
	...
	results = vmraid.db.sql(
		...
		GROUP BY
			month_year
	""".format(
			current_month_year=current_month_year, company=vmraid.db.escape(company)
		),
		as_dict=True,
	)
	...
	vmraid.db.set_value("Company", company, "total_monthly_sales", monthly_total)
...
def update_company_monthly_sales(company):
	...
	from vmraid.utils.goal import get_monthly_results

	filter_str = "company = {0} and status != 'Draft' and docstatus=1".format(
		vmraid.db.escape(company)
	)
	month_to_value_dict = get_monthly_results(
		"Sales Invoice", "base_grand_total", "posting_date", filter_str, "sum"
	)

	vmraid.db.set_value("Company", company, "sales_monthly_history", json.dumps(month_to_value_dict))
...
def get_children(doctype, parent=None, company=None, is_root=False):
	...
	return vmraid.db.sql(
		...
		""".format(
			doctype=doctype, parent=vmraid.db.escape(parent)
		),
		as_dict=1,
	)
...
def get_timeline_data(doctype, name):
	...
	history = vmraid.get_cached_value("Company", name, "transactions_annual_history")
...
def create_transaction_deletion_request(company):
	tdr = vmraid.get_doc({"doctype": "Transaction Deletion Record", "company": company})
	tdr.insert()
	tdr.submit()
