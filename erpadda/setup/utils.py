...
from vmraid.utils import add_days, flt, get_datetime_str, nowdate
from vmraid.utils.data import now_datetime
from vmraid.utils.nestedset import get_ancestors_of, get_root_of  # noqa
...
def before_tests():
	...
	if not vmraid.db.a_row_exists("Company"):
...
def get_exchange_rate(from_currency, to_currency, transaction_date=None, args=None):
	...
	try:
		...
		if not value:
			...
			settings = vmraid.get_cached_doc("Currency Exchange Settings")
			req_params = {
...
def _enable_all_domains():
	domains = vmraid.get_all("Domain", pluck="name")
	if not domains:
		return
	vmraid.get_single("Domain Settings").set_active_domains(domains)


def _enable_all_roles_for_admin():
	from vmraid.desk.page.setup_wizard.setup_wizard import add_all_roles_to

	all_roles = set(vmraid.db.get_values("Role", pluck="name"))
	admin_roles = set(
		vmraid.db.get_values("Has Role", {"parent": "Administrator"}, fieldname="role", pluck="role")
	)
...
def set_defaults_for_tests():
	...
	vmraid.db.set_single_value("Selling Settings", defaults)
	for key, value in defaults.items():
		vmraid.db.set_default(key, value)
	vmraid.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 0)
