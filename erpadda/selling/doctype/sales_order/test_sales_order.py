...
from vmraid.utils import add_days, flt, getdate, nowdate, today

from erpadda.controllers.accounts_controller import update_child_qty_rate
from erpadda.maintenance.doctype.maintenance_schedule.test_maintenance_schedule import (
	make_maintenance_schedule,
)
from erpadda.maintenance.doctype.maintenance_visit.test_maintenance_visit import (
	make_maintenance_visit,
)
from erpadda.manufacturing.doctype.blanket_order.test_blanket_order import make_blanket_order
from erpadda.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
from erpadda.selling.doctype.sales_order.sales_order import (
	WarehouseRequired,
	make_delivery_note,
...
class TestSalesOrder(VMRaidTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_make_material_request(self):
		...
		self.assertRaises(vmraid.ValidationError, make_material_request, so.name)
	...
	def test_make_delivery_note(self):
		...
		self.assertRaises(vmraid.ValidationError, make_delivery_note, so.name)
	...
	def test_make_sales_invoice(self):
		...
		self.assertRaises(vmraid.ValidationError, make_sales_invoice, so.name)
	...
	def test_reserved_qty_for_over_delivery(self):
		...
		vmraid.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
	...
	def test_reserved_qty_for_over_delivery_via_sales_invoice(self):
		...
		# set over-delivery allowance
		vmraid.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
		vmraid.db.set_value("Item", "_Test Item", "over_billing_allowance", 20)
	...
	def test_sales_order_on_hold(self):
		...
		si = make_sales_invoice(so.name)
		self.assertRaises(vmraid.ValidationError, create_dn_against_so, so.name)
	...
	def test_reserved_qty_for_over_delivery_with_packing_list(self):
		...
		# set over-delivery allowance
		vmraid.db.set_value("Item", "_Test Product Bundle Item", "over_delivery_receipt_allowance", 50)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(
			vmraid.ValidationError, update_child_qty_rate, "Sales Order", trans_item, so.name
		)
	...
	def test_update_child_with_precision(self):
		...
		precision = get_field_precision(vmraid.get_meta("Sales Order Item").get_field("rate"))
	...
	def test_update_child_qty_rate_with_workflow(self):
		from vmraid.model.workflow import apply_workflow
		...
		test_user2 = vmraid.get_doc("User", user2)
		test_user2.add_roles("Sales User", "Test Approver")
		vmraid.set_user(user2)
	...
	def test_update_child_product_bundle(self):
		...
		if not vmraid.db.exists("Item", "_Product Bundle Item"):
	...
	def test_update_child_with_tax_template(self):
		...
		"""
		if not vmraid.db.exists("Item", "Test Item with Tax"):
			make_item(
				"Test Item with Tax",
				{
					"is_stock_item": 1,
		...
		if not vmraid.db.exists("Item Tax Template", {"title": "Test Update Items Template"}):
			vmraid.get_doc(
				{
		...
		new_item_with_tax = vmraid.get_doc("Item", "Test Item with Tax")
		...
		if not vmraid.db.exists("Item Tax", {"parent": item, "item_tax_template": tax_template}):
			item_doc = vmraid.get_doc("Item", item)
		...
		old_stock_settings_value = vmraid.db.get_single_value("Stock Settings", "default_warehouse")
		vmraid.db.set_value("Stock Settings", None, "default_warehouse", "_Test Warehouse - _TC")
		...
		vmraid.get_doc("Item Tax Template", "Test Update Items Template - _TC").delete()
		vmraid.db.set_value("Stock Settings", None, "default_warehouse", old_stock_settings_value)

	def test_warehouse_user(self):
		test_user = create_user("test_so_warehouse_user@example.com", "Sales User", "Stock User")

		test_user_2 = vmraid.get_doc("User", "test2@example.com")
		test_user_2.add_roles("Sales User", "Stock User")
		test_user_2.remove_roles("Sales Manager")

		vmraid.permissions.add_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		vmraid.permissions.add_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		vmraid.permissions.add_user_permission("Company", "_Test Company 1", test_user_2.name)
		...
		self.assertRaises(vmraid.PermissionError, so.insert)

		vmraid.set_user(test_user_2.name)
		so.insert()

		vmraid.set_user("Administrator")
		vmraid.permissions.remove_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		vmraid.permissions.remove_user_permission(
			"Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name
		)
		vmraid.permissions.remove_user_permission("Company", "_Test Company 1", test_user_2.name)
	...
	def test_block_delivery_note_against_cancelled_sales_order(self):
		...
		self.assertRaises(vmraid.CancelledLinkError, dn.submit)
	...
	def test_auto_insert_price(self):
		...
		vmraid.db.set_value("Stock Settings", None, "auto_insert_price_list_rate_if_missing", 1)

		item_price = vmraid.db.get_value(
			"Item Price", {"price_list": "_Test Price List", "item_code": "_Test Item for Auto Price List"}
		)
		if item_price:
			vmraid.delete_doc("Item Price", item_price)
	...
	def test_drop_shipping(self):
		from erpadda.buying.doctype.purchase_order.purchase_order import update_status
		from erpadda.selling.doctype.sales_order.sales_order import (
			make_purchase_order_for_default_supplier,
		)
		from erpadda.selling.doctype.sales_order.sales_order import update_status as so_update_status
		...
		if vmraid.db.get_value("Item", "_Test Regular Item", "is_stock_item") == 1:
		...
		self.assertEqual(flt(vmraid.db.get_value("Sales Order", so.name, "per_delivered"), 2), 100.00)
		po.load_from_db()
	...
	def test_drop_shipping_full_for_default_suppliers(self):
		...
		if not vmraid.db.exists("Item", "_Test Item for Drop Shipping 1"):
			make_item("_Test Item for Drop Shipping 1", {"is_stock_item": 1, "delivered_by_supplier": 1})

		if not vmraid.db.exists("Item", "_Test Item for Drop Shipping 2"):
	...
	def test_product_bundles_in_so_are_replaced_with_bundle_items_in_po(self):
		...
		"""
		from erpadda.selling.doctype.sales_order.sales_order import make_purchase_order
	...
	def test_serial_no_based_delivery(self):
		vmraid.set_value("Stock Settings", None, "automatically_set_serial_nos_based_on_fifo", 1)
		item = make_item(
		...
		vmraid.db.sql("""delete from `tabSerial No` where item_code=%s""", (item.item_code))
		make_item(
			"_Test Item A",
			{
				"maintain_stock": 1,
		...
		item_serial_no = vmraid.get_doc("Serial No", {"item_code": item.item_code})
		from erpadda.manufacturing.doctype.work_order.work_order import (
			make_stock_entry as make_production_stock_entry,
		)

		se = vmraid.get_doc(make_production_stock_entry(work_order.name, "Manufacture", 1))
		se.submit()
		reserved_serial_no = se.get("items")[2].serial_no
		serial_no_so = vmraid.get_value("Serial No", reserved_serial_no, "sales_order")
		...
		si.update_stock = 0
		si.submit()
		from erpadda.accounts.doctype.sales_invoice.sales_invoice import (
		...
		se.load_from_db()
		se.cancel()
		self.assertFalse(vmraid.db.exists("Serial No", {"sales_order": so.name}))
	...
	def test_advance_payment_entry_unlink_against_sales_order(self):
		...
		so_doc = vmraid.get_doc("Sales Order", so.name)

		self.assertRaises(vmraid.LinkExistsError, so_doc.cancel)
	...
	def test_cancel_sales_order_after_cancel_payment_entry(self):
		...
		po_doc = vmraid.get_doc("Payment Entry", pe.name)
		po_doc.cancel()
	...
	def test_request_for_raw_materials(self):
		...
		mr_doc = vmraid.get_doc("Material Request", mr.get("name"))
	...
	def test_so_optional_blanket_order(self):
		...
		so_doc = vmraid.get_doc("Sales Order", so.get("name"))
	...
	def test_so_cancellation_when_si_drafted(self):
		...
		self.assertRaises(vmraid.ValidationError, so.cancel)
	...
	def test_so_cancellation_after_si_submission(self):
		...
		so.load_from_db()
		self.assertRaises(vmraid.LinkExistsError, so.cancel)
	...
	def test_payment_terms_are_fetched_when_creating_sales_invoice(self):
		from erpadda.accounts.doctype.payment_entry.test_payment_entry import (
	...
	def test_so_back_updated_from_wo_via_mr(self):
		...
		from erpadda.stock.doctype.material_request.material_request import raise_work_orders
		...
		wo = vmraid.get_doc("Work Order", wo_name)
		...
		se = vmraid.get_doc(make_se_from_wo(wo.name, "Manufacture", 2))
		se.submit()  # Finish WO
...
def automatically_fetch_payment_terms(enable=1):
	accounts_settings = vmraid.get_doc("Accounts Settings")
...
def get_reserved_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(
		vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "reserved_qty")
	)
...
def make_sales_order_workflow():
	if vmraid.db.exists("Workflow", "SO Test Workflow"):
		doc = vmraid.get_doc("Workflow", "SO Test Workflow")
		doc.set("is_active", 1)
		doc.save()
		return doc

	vmraid.get_doc(dict(doctype="Role", role_name="Test Junior Approver")).insert(
		ignore_if_duplicate=True
	)
	vmraid.get_doc(dict(doctype="Role", role_name="Test Approver")).insert(ignore_if_duplicate=True)
	vmraid.cache().hdel("roles", vmraid.session.user)

	workflow = vmraid.get_doc(
		{
			"doctype": "Workflow",
