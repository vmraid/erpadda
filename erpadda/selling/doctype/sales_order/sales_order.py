...
from vmraid.contacts.doctype.address.address import get_company_address
...
from vmraid.utils import add_days, cint, cstr, flt, get_link_to_form, getdate, nowdate, strip_html
...
from erpadda.manufacturing.doctype.production_plan.production_plan import (
...
from erpadda.stock.stock_balance import get_reserved_qty, update_bin_qty

form_grid_templates = {"items": "templates/form_grid/item_grid.html"}


class WarehouseRequired(vmraid.ValidationError):
	pass
...
class SalesOrder(SellingController):
	...
	def validate_po(self):
		...
		if self.po_no and self.customer and not self.skip_delivery_note:
			...
			if (
				...
				and so[0][0]
				and not cint(
					vmraid.db.get_single_value("Selling Settings", "allow_against_multiple_purchase_orders")
				)
			):
				vmraid.msgprint(
	...
	def validate_sales_mntc_quotation(self):
		for d in self.get("items"):
			if d.prevdoc_docname:
				...
				if not res:
					vmraid.msgprint(_("Quotation {0} not of type {1}").format(d.prevdoc_docname, self.order_type))
	...
	def validate_delivery_date(self):
		if self.order_type == "Sales" and not self.skip_delivery_note:
			...
			if self.delivery_date:
				...
				vmraid.throw(_("Please enter Delivery Date"))
	...
	def validate_warehouse(self):
		...
		for d in self.get("items"):
			if (
				(
					vmraid.get_cached_value("Item", d.item_code, "is_stock_item") == 1
	...
	def update_enquiry_status(self, prevdoc, flag):
		...
		if enq:
			vmraid.db.sql("update `tabOpportunity` set status = %s where name=%s", (flag, enq[0][0]))

	def update_prevdoc_status(self, flag=None):
		for quotation in set(d.prevdoc_docname for d in self.get("items")):
			if quotation:
				doc = vmraid.get_doc("Quotation", quotation)
				if doc.docstatus == 2:
					vmraid.throw(_("Quotation {0} is cancelled").format(quotation))
	...
	def validate_drop_ship(self):
		for d in self.get("items"):
			if d.delivered_by_supplier and not d.supplier:
				vmraid.throw(_("Row #{0}: Set Supplier for item {1}").format(d.idx, d.item_code))
	...
	def on_submit(self):
		...
		if self.coupon_code:
			from erpadda.accounts.doctype.pricing_rule.utils import update_coupon_code_count
	...
	def on_cancel(self):
		...
		if self.status == "Closed":
			vmraid.throw(_("Closed order cannot be cancelled. Unclose to cancel."))
	...
	def check_nextdoc_docstatus(self):
		linked_invoices = vmraid.db.sql_list(
	...
	def check_modified_date(self):
		mod_db = vmraid.db.get_value("Sales Order", self.name, "modified")
		date_diff = vmraid.db.sql("select TIMEDIFF('%s', '%s')" % (mod_db, cstr(self.modified)))
		if date_diff and date_diff[0][0]:
			vmraid.throw(_("{0} {1} has been modified. Please refresh.").format(self.doctype, self.name))
	...
	def update_reserved_qty(self, so_item_rows=None):
		...
		def _valid_for_reserve(item_code, warehouse):
			if (
				...
				and vmraid.get_cached_value("Item", item_code, "is_stock_item")
			):
	...
	def update_delivery_status(self):
		...
		for item in self.items:
			if item.delivered_by_supplier:
				item_delivered_qty = vmraid.db.sql(
					"""select sum(qty)
	...
	def on_recurring(self, reference_doc, auto_repeat_doc):
		def _get_delivery_date(ref_doc_delivery_date, red_doc_transaction_date, transaction_date):
			...
			if delivery_date <= transaction_date:
				delivery_date_diff = vmraid.utils.date_diff(ref_doc_delivery_date, red_doc_transaction_date)
				delivery_date = vmraid.utils.add_days(transaction_date, delivery_date_diff)
		...
		for d in self.get("items"):
			reference_delivery_date = vmraid.db.get_value(
				"Sales Order Item",
		...
		for item in self.items:
			if item.ensure_delivery_based_on_produced_serial_no:
				...
				if item.item_code not in reserved_items:
					if not vmraid.get_cached_value("Item", item.item_code, "has_serial_no"):
						vmraid.throw(
							_(
								"Item {0} has no Serial No. Only serilialized items can have delivery based on Serial No"
							).format(item.item_code)
						)
					if not vmraid.db.exists("BOM", {"item": item.item_code, "is_active": 1}):
						vmraid.throw(
...
def close_or_unclose_sales_orders(names, status):
	if not vmraid.has_permission("Sales Order", "write"):
		vmraid.throw(_("Not permitted"), vmraid.PermissionError)

	names = json.loads(names)
	for name in names:
		so = vmraid.get_doc("Sales Order", name)
		if so.docstatus == 1:
			if status == "Closed":
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			...
			"Sales Order Item": {
				...
				"condition": lambda doc: not vmraid.db.exists("Product Bundle", doc.item_code)
...
def make_delivery_note(source_name, target_doc=None, skip_item_mapping=False):
	...
	def update_item(source, target, source_parent):
		...
		if item:
			target.cost_center = (
				vmraid.db.get_value("Project", source_parent.project, "cost_center")
	...
	if not skip_item_mapping:
		...
		def condition(doc):
			# make_mapped_doc sets js `args` into `vmraid.flags.args`
			if vmraid.flags.args and vmraid.flags.args.delivery_dates:
				if cstr(doc.delivery_date) not in vmraid.flags.args.delivery_dates:
					return False
...
def make_sales_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	def update_item(source, target, source_parent):
		...
		if source_parent.project:
			target.cost_center = vmraid.db.get_value("Project", source_parent.project, "cost_center")
		if target.item_code:
			item = get_item_defaults(target.item_code, source_parent.company)
			item_group = get_item_group_defaults(target.item_code, source_parent.company)
...
def make_purchase_order_for_default_supplier(source_name, selected_items=None, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		default_price_list = vmraid.get_value("Supplier", supplier, "default_price_list")
...
def make_purchase_order(source_name, selected_items=None, target_doc=None):
	...
	# po = vmraid.get_list("Purchase Order", filters={"sales_order":source_name, "supplier":supplier, "docstatus": ("<", "2")})
	doc = get_mapped_doc(
		"Sales Order",
		source_name,
		{
...
def is_product_bundle(item_code):
	return vmraid.db.exists("Product Bundle", item_code)
...
def make_work_orders(items, sales_order, company, project=None):
	...
	for i in items:
		if not i.get("bom"):
			vmraid.throw(_("Please select BOM against item {0}").format(i.get("item_code")))
		if not i.get("pending_qty"):
			vmraid.throw(_("Please select Qty against item {0}").format(i.get("item_code")))

		work_order = vmraid.get_doc(
			dict(
				doctype="Work Order",
...
def get_default_bom_item(item_code):
	bom = vmraid.get_all("BOM", dict(item=item_code, is_active=True), order_by="is_default desc")
...
def make_raw_material_request(items, company, sales_order, project=None):
	...
	if isinstance(items, str):
		items = vmraid._dict(json.loads(items))
	...
	material_request = vmraid.new_doc("Material Request")
	...
	for item in raw_materials:
		item_doc = vmraid.get_cached_doc("Item", item.get("item_code"))
...
def update_produced_qty_in_so_item(sales_order, sales_order_item):
	...
	linked_wo_with_so_item = vmraid.db.get_all(
		"Work Order",
		["produced_qty"],
	...
	if not total_produced_qty and vmraid.flags.in_patch:
		return

	vmraid.db.set_value("Sales Order Item", sales_order_item, "produced_qty", total_produced_qty)
