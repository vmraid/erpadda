vmraid.listview_settings['Sales Order'] = {
	...
	get_indicator: function (doc) {
		if (doc.status === "Closed") {
			...
			if (vmraid.datetime.get_diff(doc.delivery_date) < 0) {
	...
	onload: function(listview) {
		var method = "erpadda.selling.doctype.sales_order.sales_order.close_or_unclose_sales_orders";
		...
		listview.page.add_action_item(__("Sales Invoice"), ()=>{
			erpadda.bulk_transaction_processing.create(listview, "Sales Order", "Sales Invoice");
		});

		listview.page.add_action_item(__("Delivery Note"), ()=>{
			erpadda.bulk_transaction_processing.create(listview, "Sales Order", "Delivery Note");
		});

		listview.page.add_action_item(__("Advance Payment"), ()=>{
			erpadda.bulk_transaction_processing.create(listview, "Sales Order", "Advance Payment");
		});
