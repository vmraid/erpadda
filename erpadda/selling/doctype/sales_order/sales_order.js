...
vmraid.ui.form.on("Sales Order", {
	...
	refresh: function(frm) {
		if(frm.doc.docstatus === 1 && frm.doc.status !== 'Closed'
			...
			frm.add_custom_button(__('Update Items'), () => {
				erpadda.utils.update_child_items({
					frm: frm,
					child_docname: "items",
	...
	onload: function(frm) {
		...
		frm.set_query('project', function(doc, cdt, cdn) {
			return {
				query: "erpadda.controllers.queries.get_project_name",
				filters: {
		...
		erpadda.queries.setup_warehouse_query(frm);
...
vmraid.ui.form.on("Sales Order Item", {
	...
	delivery_date: function(frm, cdt, cdn) {
		if(!frm.doc.delivery_date) {
			erpadda.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	}
});

erpadda.selling.SalesOrderController = class SalesOrderController extends erpadda.selling.SellingController {
	onload(doc, dt, dn) {
			...
			this.frm.add_custom_button(__('Quotation'),
				function() {
					erpadda.utils.map_current_doc({
						method: "erpadda.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm
		})
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new vmraid.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					method: 'erpadda.selling.doctype.sales_order.sales_order.make_raw_material_request',
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
					...
					callback: function(r) {
						if(r.message) {
							vmraid.msgprint(__('Material Request {0} submitted.',
					...
					${delivery_dates.map(date => `
						<div class="list-item">
							<div class="list-item__content list-item__content--flex-2">
								...
								${vmraid.datetime.str_to_user(date)}
								</label>
							</div>
						</div>
					`).join("")}
	...
	make_delivery_note(delivery_dates) {
		vmraid.model.open_mapped_doc({
			method: "erpadda.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates
			}
	...
	make_project() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm
		})
	}
	...
	make_inter_company_order() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm
		});
	}
	...
	make_purchase_order(){
		...
		if(!pending_items){
			vmraid.throw({message: __("Purchase Order already created for all Sales Order items"), title: __("Note")});
		}
		...
		var dialog = new vmraid.ui.Dialog({
			...
			primary_action (args) {
				...
				if(selected_items.length == 0) {
					vmraid.throw({message: 'Please select Items from the Table', title: __('Items Required'), indicator:'blue'})
				}
				...
				return vmraid.call({
					method: "erpadda.selling.doctype.sales_order.sales_order." + method,
					freeze: true,
					...
					callback: function(r) {
						if(!r.exc) {
							if (!args.against_default_supplier) {
								...
								vmraid.set_route("List", "Purchase Order");
							}
						}
					}
				})
	...
	hold_sales_order(){
		...
		var d = new vmraid.ui.Dialog({
			...
			primary_action: function() {
				var data = d.get_values();
				vmraid.call({
					method: "vmraid.desk.form.utils.add_comment",
					args: {
	...
	update_status(label, status){
		...
		vmraid.call({
			method: "erpadda.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new erpadda.selling.SalesOrderController({frm: cur_frm}));
