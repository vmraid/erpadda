...
from vmraid.utils import flt, getdate, nowdate

from erpadda.controllers.selling_controller import SellingController
from erpadda.crm.utils import add_link_in_communication, copy_comments
...
class Quotation(SellingController):
	...
	def after_insert(self):
		if vmraid.db.get_single_value("CRM Settings", "carry_forward_communication_and_comments"):
			if self.opportunity:
	...
	def validate_valid_till(self):
		if self.valid_till and getdate(self.valid_till) < getdate(self.transaction_date):
			vmraid.throw(_("Valid till date cannot be before transaction date"))

	def has_sales_order(self):
		return vmraid.db.get_value("Sales Order Item", {"prevdoc_docname": self.name, "docstatus": 1})

	def update_lead(self):
		if self.quotation_to == "Lead" and self.party_name:
			vmraid.get_doc("Lead", self.party_name).set_status(update=True)

	def set_customer_name(self):
		if self.party_name and self.quotation_to == "Customer":
			self.customer_name = vmraid.db.get_value("Customer", self.party_name, "customer_name")
		elif self.party_name and self.quotation_to == "Lead":
			lead_name, company_name = vmraid.db.get_value(
	...
	def update_opportunity_status(self, status, opportunity=None):
		...
		opp = vmraid.get_doc("Opportunity", opportunity)
		opp.set_status(status=status, update=True)

	@vmraid.whitelist()
	def declare_enquiry_lost(self, lost_reasons_list, competitors, detailed_reason=None):
		if not self.has_sales_order():
			get_lost_reasons = vmraid.get_list("Quotation Lost Reason", fields=["name"])
			lost_reasons_lst = [reason.get("name") for reason in get_lost_reasons]
			vmraid.db.set(self, "status", "Lost")

			if detailed_reason:
				vmraid.db.set(self, "order_lost_reason", detailed_reason)
				...
				if reason.get("lost_reason") in lost_reasons_lst:
					...
					vmraid.throw(
						_("Invalid lost reason {0}, please create a new lost reason").format(
							vmraid.bold(reason.get("lost_reason"))
						)
					)
			...
			vmraid.throw(_("Cannot set as Lost as Sales Order is made."))
...
def make_sales_order(source_name, target_doc=None):
	quotation = vmraid.db.get_value(
		"Quotation", source_name, ["transaction_date", "valid_till"], as_dict=1
	)
	if quotation.valid_till and (
		quotation.valid_till < quotation.transaction_date or quotation.valid_till < getdate(nowdate())
	):
		vmraid.throw(_("Validity period of this quotation has ended."))
...
def _make_sales_order(source_name, target_doc=None, ignore_permissions=False):
	...
	def set_missing_values(source, target):
		...
		if source.referral_sales_partner:
			...
			target.commission_rate = vmraid.get_value(
...
def _make_customer(source_name, ignore_permissions=False):
	...
	if quotation and quotation.get("party_name"):
		if not vmraid.db.exists("Customer", quotation.get("party_name")):
			lead_name = quotation.get("party_name")
			customer_name = vmraid.db.get_value(
				"Customer", {"lead_name": lead_name}, ["name", "customer_name"], as_dict=True
			)
			if not customer_name:
				from erpadda.crm.doctype.lead.lead import _make_customer

				customer_doclist = _make_customer(lead_name, ignore_permissions=ignore_permissions)
				customer = vmraid.get_doc(customer_doclist)
				customer.flags.ignore_permissions = ignore_permissions
				if quotation.get("party_name") == "Shopping Cart":
					customer.customer_group = vmraid.db.get_value(
				...
				try:
					...
					if vmraid.defaults.get_global_default("cust_master_name") == "Customer Name":
						...
						raise
				except vmraid.MandatoryError as e:
					mandatory_fields = e.args[0].split(":")[1].split(",")
					mandatory_fields = [customer.meta.get_label(field.strip()) for field in mandatory_fields]

					vmraid.local.message_log = []
					lead_link = vmraid.utils.get_link_to_form("Lead", lead_name)
					message = (
					...
					vmraid.throw(message, title=_("Mandatory Missing"))
			else:
				return customer_name
		else:
			return vmraid.get_doc("Customer", quotation.get("party_name"))
