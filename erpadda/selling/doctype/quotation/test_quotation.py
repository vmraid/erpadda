...
from vmraid.utils import add_days, add_months, flt, getdate, nowdate

test_dependencies = ["Product Bundle"]


class TestQuotation(VMRaidTestCase):
	...
	def test_make_sales_order_terms_copied(self):
		from erpadda.selling.doctype.quotation.quotation import make_sales_order

		quotation = vmraid.copy_doc(test_records[0])
	...
	def test_make_sales_order(self):
		...
		self.assertRaises(vmraid.ValidationError, make_sales_order, quotation.name)
		quotation.submit()
	...
	def test_valid_till(self):
		...
		self.assertRaises(vmraid.ValidationError, quotation.validate)
	...
	def test_quotation_expiry(self):
		from erpadda.selling.doctype.quotation.quotation import set_expired_status
...
test_records = vmraid.get_test_records("Quotation")
