...
from erpadda.accounts.doctype.monthly_distribution.monthly_distribution import (
...
def get_data_column(filters, partner_doctype):
	...
	for key, value in rows.items():
		value.update({vmraid.scrub(partner_doctype): key[0], "item_group": key[1]})
...
def get_data(filters, period_list, partner_doctype):
	sales_field = vmraid.scrub(partner_doctype)
...
def get_columns(filters, period_list, partner_doctype):
	...
	columns = [
		{
			"fieldname": vmraid.scrub(partner_doctype),
