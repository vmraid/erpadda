...
from vmraid.utils import add_days, add_to_date, flt, getdate
...
class Analytics(object):
		...
		for d in reversed(self.group_entries):
			...
			for end_date in self.periodic_daterange:
				...
				if d.parent and (self.filters.tree_type != "Order Type" or d.parent == "Order Types"):
					self.entity_periodic_data.setdefault(d.parent, vmraid._dict()).setdefault(period, 0.0)
	...
	def get_periodic_data(self):
		self.entity_periodic_data = vmraid._dict()

		for d in self.entries:
			if self.filters.tree_type == "Supplier Group":
				d.entity = self.parent_child_map.get(d.entity)
			period = self.get_period(d.get(self.date_field))
			self.entity_periodic_data.setdefault(d.entity, vmraid._dict()).setdefault(period, 0.0)
	...
	def get_groups(self):
		...
		self.depth_map = vmraid._dict()

		self.group_entries = vmraid.db.sql(
	...
	def get_supplier_parent_child_map(self):
		self.parent_child_map = vmraid._dict(
			vmraid.db.sql(""" select name, supplier_group from `tabSupplier`""")
		)
