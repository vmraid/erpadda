...
import vmraid
from vmraid import _, qb, query_builder
from vmraid.query_builder import Criterion, functions
...
def get_conditions(filters):
	...
	"""
	filters = vmraid._dict(filters) if filters else vmraid._dict({})
	conditions = vmraid._dict({})

	conditions.company = filters.company or vmraid.defaults.get_user_default("company")
	conditions.end_date = filters.period_end_date or vmraid.utils.today()
	conditions.start_date = filters.period_start_date or vmraid.utils.add_months(
...
def set_payment_terms_statuses(sales_orders, invoices, filters):
	...
	for so in sales_orders:
		so.currency = vmraid.get_cached_value("Company", filters.get("company"), "default_currency")
		so.invoices = ""
