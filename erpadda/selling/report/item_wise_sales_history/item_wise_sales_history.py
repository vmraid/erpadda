...
def get_conditions(filters):
	...
	if filters.get("item_group"):
		conditions += "AND so_item.item_group = %s" % vmraid.db.escape(filters.item_group)
	...
	if filters.get("item_code"):
		conditions += "AND so_item.item_code = %s" % vmraid.db.escape(filters.item_code)

	if filters.get("customer"):
		conditions += "AND so.customer = %s" % vmraid.db.escape(filters.customer)

	return conditions


def get_customer_details():
	details = vmraid.get_all("Customer", fields=["name", "customer_name", "customer_group"])
	customer_details = {}
	for d in details:
		customer_details.setdefault(
			d.name, vmraid._dict({"customer_name": d.customer_name, "customer_group": d.customer_group})
		)
	return customer_details


def get_item_details():
	details = vmraid.db.get_all("Item", fields=["name", "item_name", "item_group"])
	item_details = {}
	for d in details:
		item_details.setdefault(
			d.name, vmraid._dict({"item_name": d.item_name, "item_group": d.item_group})
		)
	return item_details
