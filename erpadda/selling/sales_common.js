...
{% include 'erpadda/accounts/doctype/sales_taxes_and_charges_template/sales_taxes_and_charges_template.js' %}


cur_frm.email_field = "contact_email";

vmraid.provide("erpadda.selling");
erpadda.selling.SellingController = class SellingController extends erpadda.TransactionController {
	setup() {
		super.setup();
	...
	setup_queries() {
		...
		$.each([["customer", "customer"],
			...
			function(i, opts) {
				if(me.frm.fields_dict[opts[0]])
					me.frm.set_query(opts[0], erpadda.queries[opts[1]]);
			});
		...
		me.frm.set_query('shipping_address_name', erpadda.queries.address_query);
		me.frm.set_query('dispatch_address_name', erpadda.queries.dispatch_address_query);
	...
	customer_address() {
		erpadda.utils.get_address_display(this.frm, "customer_address");
		erpadda.utils.set_taxes_from_address(this.frm, "customer_address", "customer_address", "shipping_address_name");
	}

	shipping_address_name() {
		erpadda.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
		erpadda.utils.set_taxes_from_address(this.frm, "shipping_address_name", "customer_address", "shipping_address_name");
	}

	dispatch_address_name() {
		erpadda.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
	}
	...
	total_commission() {
		vmraid.model.round_floats_in(this.frm.doc, ["amount_eligible_for_commission", "total_commission"]);
	...
	allocated_percentage(doc, cdt, cdn) {
		var sales_person = vmraid.get_doc(cdt, cdn);
	...
	warehouse(doc, cdt, cdn) {
		...
		var has_batch_no;
		vmraid.db.get_value('Item', {'item_code': item.item_code}, 'has_batch_no', (r) => {
			has_batch_no = r && r.has_batch_no;
			if(item.item_code && item.warehouse) {
				return this.frm.call({
					method: "erpadda.stock.get_item_details.get_bin_details_and_serial_nos",
					child: item,
					args: {
	...
	toggle_editable_price_list_rate() {
		var df = vmraid.meta.get_docfield(this.frm.doc.doctype + " Item", "price_list_rate", this.frm.doc.name);
		var editable_price_list_rate = cint(vmraid.defaults.get_default("editable_price_list_rate"));

		if(df && editable_price_list_rate) {
			const parent_field = vmraid.meta.get_parentfield(this.frm.doc.doctype, this.frm.doc.doctype + " Item");
	...
	calculate_commission() {
		...
		if(this.frm.doc.commission_rate > 100) {
			...
			vmraid.throw(`${__(vmraid.meta.get_label(
	...
	calculate_contribution() {
		...
		$.each(this.frm.doc.doctype.sales_team || [], function(i, sales_person) {
			vmraid.model.round_floats_in(sales_person);
	...
	batch_no(doc, cdt, cdn) {
		...
		item.serial_no = null;
		var has_serial_no;
		vmraid.db.get_value('Item', {'item_code': item.item_code}, 'has_serial_no', (r) => {
			has_serial_no = r && r.has_serial_no;
			if(item.warehouse && item.item_code && item.batch_no) {
				return this.frm.call({
					method: "erpadda.stock.get_item_details.get_batch_qty_and_serial_no",
					child: item,
					args: {
	...
	set_product_bundle_help(doc) {
		...
		if ((doc.packed_items || []).length) {
			...
			if (in_list(['Delivery Note', 'Sales Invoice'], doc.doctype)) {
				...
				"</div>";
				vmraid.meta.get_docfield(doc.doctype, 'product_bundle_help', doc.name).options = help_msg;
			}
		} else {
			$(cur_frm.fields_dict.packing_list.row.wrapper).toggle(false);
			if (in_list(['Delivery Note', 'Sales Invoice'], doc.doctype)) {
				vmraid.meta.get_docfield(doc.doctype, 'product_bundle_help', doc.name).options = '';
			}
		}
	...
	*/
	set_batch_number(cdt, cdn) {
		const doc = vmraid.get_doc(cdt, cdn);
	...
	_set_batch_number(doc) {
		...
		return vmraid.call({
			method: 'erpadda.stock.doctype.batch.batch.get_batch_no',
			args: args,
			callback: function(r) {
				if(r.message) {
					vmraid.model.set_value(doc.doctype, doc.name, 'batch_no', r.message);
				} else {
				    vmraid.model.set_value(doc.doctype, doc.name, 'batch_no', r.message);
				}
			}
...
vmraid.ui.form.on(cur_frm.doctype,"project", function(frm) {
	if(in_list(["Delivery Note", "Sales Invoice"], frm.doc.doctype)) {
		if(frm.doc.project) {
			vmraid.call({
				method:'erpadda.projects.doctype.project.project.get_cost_center_name' ,
				args: {	project: frm.doc.project	},
				callback: function(r, rt) {
					if(!r.exc) {
						$.each(frm.doc["items"] || [], function(i, row) {
							if(r.message) {
								vmraid.model.set_value(row.doctype, row.name, "cost_center", r.message);
								vmraid.msgprint(__("Cost Center For Item with Item Code {0} has been Changed to {1}", [row.item_name, r.message]));
							}
						})
					}
				}
...
vmraid.ui.form.on(cur_frm.doctype, {
