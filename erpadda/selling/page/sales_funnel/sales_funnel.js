...
vmraid.pages['sales-funnel'].on_page_load = function(wrapper) {
	...
	wrapper.sales_funnel = new erpadda.SalesFunnel(wrapper);

	vmraid.breadcrumbs.add("Selling");
}

erpadda.SalesFunnel = class SalesFunnel {
	constructor(wrapper) {
		var me = this;
	...
	setup(wrapper) {
		...
		this.company_field = wrapper.page.add_field({"fieldtype": "Link", "fieldname": "company", "options": "Company",
			"label": __("Company"), "reqd": 1, "default": vmraid.defaults.get_user_default('company'),
			change: function() {
				me.company = this.value || vmraid.defaults.get_user_default('company');
				me.get_data();
			}
		}),
		...
		this.company = vmraid.defaults.get_user_default('company');
		this.options = {
			from_date: vmraid.datetime.add_months(vmraid.datetime.get_today(), -1),
			to_date: vmraid.datetime.get_today(),
			chart: 'sales_funnel'
		};
		...
		$.each(this.options, function(k, v) {
			if (['from_date', 'to_date'].includes(k)) {
				me.elements[k].val(vmraid.datetime.str_to_user(v));
			} else {
				me.elements[k].val(v);
			}
			...
			me.elements[k].on("change", function() {
				if (['from_date', 'to_date'].includes(k)) {
					me.options[k] = vmraid.datetime.user_to_str($(this).val()) != 'Invalid date' ? vmraid.datetime.user_to_str($(this).val()) : vmraid.datetime.get_today();
				} else {
	...
	get_data(btn) {
		var me = this;
		if (!this.company) {
			vmraid.throw(__("Please Select a Company."));
		}

		const method_map = {
			"sales_funnel": "erpadda.selling.page.sales_funnel.sales_funnel.get_funnel_data",
			"opp_by_lead_source": "erpadda.selling.page.sales_funnel.sales_funnel.get_opp_by_lead_source",
			"sales_pipeline": "erpadda.selling.page.sales_funnel.sales_funnel.get_pipeline_data"
		};
		vmraid.call({
