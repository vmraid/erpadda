erpadda.PointOfSale.ItemCart = class {
	...
	make_cart_numpad() {
		...
		this.number_pad = new erpadda.PointOfSale.NumberPad({
	...
	bind_events() {
		...
		vmraid.ui.form.on("POS Invoice", "paid_amount", frm => {
	...
	attach_shortcuts() {
		for (let row of this.number_pad.keys) {
			for (let btn of row) {
				...
				let shortcut_key = `ctrl+${vmraid.scrub(String(btn))[0]}`;
				...
				const fieldname = this.number_pad.fieldnames[btn] ? this.number_pad.fieldnames[btn] :
					typeof btn === 'string' ? vmraid.scrub(btn) : btn;

				let shortcut_label = shortcut_key.split('+').map(vmraid.utils.to_title_case).join('+');
				shortcut_label = vmraid.utils.is_mac() ? shortcut_label.replace('Ctrl', '⌘') : shortcut_label;
				this.$numpad_section.find(`.numpad-btn[data-button-value="${fieldname}"]`).attr("title", shortcut_label);

				vmraid.ui.keys.on(`${shortcut_key}`, () => {
		...
		const ctrl_label = vmraid.utils.is_mac() ? '⌘' : 'Ctrl';
	...
	make_customer_selector() {
		...
		`);
		const me = this;
		const query = { query: 'erpadda.controllers.queries.customer_query' };
		...
		this.customer_field = vmraid.ui.form.make_control({
			df: {
				label: __('Customer'),
				fieldtype: 'Link',
				options: 'Customer',
				...
				onchange: function() {
					if (this.value) {
						...
						vmraid.dom.freeze();
						vmraid.model.set_value(frm.doc.doctype, frm.doc.name, 'customer', this.value);
	...
	fetch_customer_details(customer) {
		if (customer) {
			return new Promise((resolve) => {
				vmraid.db.get_value('Customer', customer, ["email_id", "mobile_no", "image", "loyalty_program"]).then(({ message }) => {
					const { loyalty_program } = message;
					// if loyalty program then fetch loyalty points too
					if (loyalty_program) {
						vmraid.call({
							method: "erpadda.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details_with_points",
	...
	show_discount_control() {
		...
		this.discount_field = vmraid.ui.form.make_control({
			df: {
				label: __('Discount'),
				fieldtype: 'Data',
				...
				onchange: function() {
					if (flt(this.value) != 0) {
						vmraid.model.set_value(frm.doc.doctype, frm.doc.name, 'additional_discount_percentage', flt(this.value));
						me.hide_discount_control(this.value);
					} else {
						vmraid.model.set_value(frm.doc.doctype, frm.doc.name, 'additional_discount_percentage', 0);
	...
	get_customer_image() {
		...
		if (image) {
			...
			return `<div class="customer-image customer-abbr">${vmraid.get_abbr(customer)}</div>`;
		}
	}
	...
	update_totals_section(frm) {
		...
		const grand_total = cint(vmraid.sys_defaults.disable_rounded_total) ? frm.doc.grand_total : frm.doc.rounded_total;
	...
	render_cart_item(item_data, $item_to_update) {
		...
		function get_description_html() {
			if (item_data.description) {
				...
				item_data.description = vmraid.ellipsis(item_data.description, 45);
				...
				return `
					<div class="item-image">
						<img
							...
							src="${image}" alt="${vmraid.get_abbr(item_name)}"">
					</div>`;
			} else {
				return `<div class="item-image item-abbr">${vmraid.get_abbr(item_name)}</div>`;
			}
		}
	}
	...
	render_customer_fields() {
		...
		const me = this;
		dfs.forEach(df => {
			this[`customer_${df.fieldname}_field`] = vmraid.ui.form.make_control({
				df: { ...df,
		...
		function handle_customer_field_change() {
			...
			if (this.value && current_value != this.value && this.df.fieldname != 'loyalty_points') {
				vmraid.call({
					method: 'erpadda.selling.page.point_of_sale.point_of_sale.set_customer_info',
					args: {
	...
	fetch_customer_transactions() {
		vmraid.db.get_list('POS Invoice', {
