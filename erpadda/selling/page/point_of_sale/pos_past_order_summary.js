erpadda.PointOfSale.PastOrderSummary = class {
	...
	init_email_print_dialog() {
		const email_dialog = new vmraid.ui.Dialog({
			title: 'Email Receipt',
			fields: [
		...
		const print_dialog = new vmraid.ui.Dialog({
			title: 'Print Receipt',
			fields: [
	...
	print_receipt() {
		...
		vmraid.utils.print(
			...
			this.doc.language || vmraid.boot.lang
		);
	}
	...
	send_email() {
		...
		vmraid.call({
			...
			args: {
				...
				send_email: 1,
				print_format,
				sender_full_name: vmraid.user.full_name(),
				_lang: doc.language
			},
			callback: r => {
				if (!r.exc) {
	...
	attach_document_info(doc) {
		vmraid.db.get_value('Customer', this.doc.customer, 'email_id').then(({ message }) => {
