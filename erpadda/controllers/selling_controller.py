...
import vmraid
from vmraid import _, bold, throw
from vmraid.contacts.doctype.address.address import get_address_display
from vmraid.utils import cint, cstr, flt, get_link_to_form, nowtime
...
from erpadda.stock.doctype.item.item import set_item_default
from erpadda.stock.get_item_details import get_bin_details, get_conversion_factor
...
class SellingController(StockController):
	...
	def set_missing_lead_customer_details(self, for_validate=False):
		...
		if customer:
			from erpadda.accounts.party import _get_party_details

			fetch_payment_terms_template = False
			if self.get("__islocal") or self.company != vmraid.db.get_value(
			...
			from erpadda.crm.doctype.lead.lead import get_lead_details
	...
	def validate_max_discount(self):
		for d in self.get("items"):
			if d.item_code:
				discount = flt(vmraid.get_cached_value("Item", d.item_code, "max_discount"))

				if discount and flt(d.discount_percentage) > discount:
					vmraid.throw(_("Maximum discount for Item {0} is {1}%").format(d.item_code, discount))
	...
	def validate_selling_price(self):
		...
		if self.get("is_return") or not vmraid.db.get_single_value(
		...
		for item in self.items:
			...
			last_purchase_rate, is_stock_item = vmraid.get_cached_value(
		...
		or_conditions = (
			f"""(item_code = {vmraid.db.escape(valuation_rate[0])}
			and warehouse = {vmraid.db.escape(valuation_rate[1])})"""
			for valuation_rate in valuation_rate_map
		)

		valuation_rates = vmraid.db.sql(
			f"""
			select
	...
	def get_item_list(self):
		...
		for d in self.get("items"):
			if d.qty is None:
				vmraid.throw(_("Row {0}: Qty is mandatory").format(d.idx))
	...
	def get_already_delivered_qty(self, current_docname, so, so_detail):
		delivered_via_dn = vmraid.db.sql(
		...
		delivered_via_si = vmraid.db.sql(
	...
	def check_sales_order_on_hold_or_close(self, ref_fieldname):
		for d in self.get("items"):
			if d.get(ref_fieldname):
				status = vmraid.db.get_value("Sales Order", d.get(ref_fieldname), "status")
				if status in ("Closed", "On Hold"):
					vmraid.throw(_("Sales Order {0} is {1}").format(d.get(ref_fieldname), status))
	...
	def update_reserved_qty(self):
		...
		for so, so_item_rows in so_map.items():
			if so and so_item_rows:
				sales_order = vmraid.get_doc("Sales Order", so)

				if sales_order.status in ["Closed", "Cancelled"]:
					vmraid.throw(
						_("{0} {1} is cancelled or closed").format(_("Sales Order"), so), vmraid.InvalidStatusError
					)
	...
	def update_stock_ledger(self):
		...
		for d in self.get_item_list():
			if vmraid.get_cached_value("Item", d.item_code, "is_stock_item") == 1 and flt(d.qty):
	...
	def get_po_nos(self, ref_doctype, ref_fieldname, po_nos):
		...
		if doc_list:
			po_nos += [
				d.po_no
				for d in vmraid.get_all(ref_doctype, "po_no", filters={"name": ("in", doc_list)})
				if d.get("po_no")
			]
	...
	def validate_for_duplicate_items(self):
		...
		if cint(vmraid.db.get_single_value("Selling Settings", "allow_multiple_items")):
			return
		...
		for d in self.get("items"):
			...
			if vmraid.db.get_value("Item", d.item_code, "is_stock_item") == 1:
				duplicate_items_msg = _("Item {0} entered multiple times.").format(vmraid.bold(d.item_code))
				duplicate_items_msg += "<br><br>"
				duplicate_items_msg += _("Please enable {} in {} to allow same item in multiple rows").format(
					vmraid.bold("Allow Item to Be Added Multiple Times in a Transaction"),
					get_link_to_form("Selling Settings", "Selling Settings"),
				)
				if stock_items in check_list:
					vmraid.throw(duplicate_items_msg)
				else:
	...
	def validate_target_warehouse(self):
		...
		for d in items:
			if d.get("target_warehouse") and d.get("warehouse") == d.get("target_warehouse"):
				warehouse = vmraid.bold(d.get("target_warehouse"))
				vmraid.throw(
		...
		if not self.get("is_internal_customer") and any(d.get("target_warehouse") for d in items):
			...
			vmraid.msgprint(msg, title="Internal Transfer", alert=True)

	def validate_items(self):
		# validate items to see if they have is_sales_item enabled
		from erpadda.controllers.buying_controller import validate_item_type
