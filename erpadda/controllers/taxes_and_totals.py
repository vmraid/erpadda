...
from vmraid.utils import cint, flt, round_based_on_smallest_currency_fraction

import erpadda
from erpadda.accounts.doctype.journal_entry.journal_entry import get_exchange_rate
from erpadda.accounts.doctype.pricing_rule.utils import get_applied_pricing_rules
from erpadda.controllers.accounts_controller import (
	validate_conversion_rate,
	validate_inclusive_tax,
	validate_taxes_and_charges,
)
from erpadda.stock.get_item_details import _get_item_tax_template
...
class calculate_taxes_and_totals(object):
	def __init__(self, doc):
		...
		get_round_off_applicable_accounts(self.doc.company, vmraid.flags.round_off_applicable_accounts)
		self.calculate()
	...
	def validate_conversion_rate(self):
		...
		company_currency = erpadda.get_company_currency(self.doc.company)
	...
	def calculate_shipping_charges(self):
		...
		if hasattr(self.doc, "shipping_rule") and self.doc.shipping_rule:
			shipping_rule = vmraid.get_doc("Shipping Rule", self.doc.shipping_rule)
	...
	def round_off_totals(self, tax):
		if tax.account_head in vmraid.flags.round_off_applicable_accounts:
	...
	def apply_discount_amount(self):
		if self.doc.discount_amount:
			if not self.doc.apply_discount_on:
				vmraid.throw(_("Please select Apply Discount On"))
	...
	def set_total_amount_to_default_mop(self, total_amount_to_pay):
		default_mode_of_payment = vmraid.db.get_value(
			"POS Payment Method",
...
def get_itemised_tax_breakup_html(doc):
	if not doc.taxes:
		return
	vmraid.flags.company = doc.company
	...
	for tax in taxes:
		...
		if item_tax_map:
			for item_code, tax_data in item_tax_map.items():
				itemised_tax.setdefault(item_code, vmraid._dict())
				...
				itemised_tax[item_code][tax.description] = vmraid._dict(
...
def get_itemised_taxable_amount(items):
	itemised_taxable_amount = vmraid._dict()
	for item in items:
	...
	def set_account_currency(self):
		...
		for d in self.doc.get(self.tax_field):
			if not d.account_currency:
				account_currency = vmraid.db.get_value("Account", d.expense_account, "account_currency")
