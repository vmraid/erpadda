...
from vmraid.utils import cint, cstr, flt, get_link_to_form, getdate
...
from erpadda.stock.stock_ledger import get_items_to_be_repost


class QualityInspectionRequiredError(vmraid.ValidationError):
	pass


class QualityInspectionRejectedError(vmraid.ValidationError):
	pass


class QualityInspectionNotSubmittedError(vmraid.ValidationError):
	pass
...
class StockController(AccountsController):
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False):
		...
		if (
			cint(erpadda.is_perpetual_inventory_enabled(self.company))
	...
	def validate_serialized_batch(self):
		...
		for d in self.get("items"):
			...
			if flt(d.qty) > 0.0 and d.get("batch_no") and self.get("posting_date") and self.docstatus < 2:
				expiry_date = vmraid.get_cached_value("Batch", d.get("batch_no"), "expiry_date")
	...
	def clean_serial_nos(self):
		from erpadda.stock.doctype.serial_no.serial_no import clean_serial_no_string
	...
	def get_gl_entries(
		...
		if warehouse_with_no_account:
			for wh in warehouse_with_no_account:
				if vmraid.db.get_value("Warehouse", wh, "company"):
					vmraid.throw(
						_(
	...
	def get_debit_field_precision(self):
		if not vmraid.flags.debit_field_precision:
			vmraid.flags.debit_field_precision = vmraid.get_precision(
				"GL Entry", "debit_in_account_currency"
			)

		return vmraid.flags.debit_field_precision

	def get_voucher_details(self, default_expense_account, default_cost_center, sle_map):
		if self.doctype == "Stock Reconciliation":
			reconciliation_purpose = vmraid.db.get_value(self.doctype, self.name, "purpose")
	...
	def get_stock_ledger_details(self):
		stock_ledger = {}
		stock_ledger_entries = vmraid.db.sql(
			"""
			select
	...
	def make_batches(self, warehouse_field):
		...
		for d in self.items:
			if d.get(warehouse_field) and not d.batch_no:
				has_batch_no, create_new_batch = vmraid.db.get_value(
		...
		if not item.get("expense_account"):
			...
			vmraid.throw(
				_("Row #{0}: Expense Account not set for the Item {1}. {2}").format(
					item.idx, vmraid.bold(item.item_code), msg
				),
			...
			is_expense_account = (
				vmraid.get_cached_value("Account", item.get("expense_account"), "report_type")
				== "Profit and Loss"
			)
			if (
				self.doctype
	...
	def delete_auto_created_batches(self):
		...
		for data in vmraid.get_all(
			...
			vmraid.delete_doc("Batch", data.name)

	def get_sl_entries(self, d, args):
		sl_dict = vmraid._dict(
			{
	...
	def make_sl_entries(self, sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
		from erpadda.stock.stock_ledger import make_sl_entries
	...
	def get_serialized_items(self):
		...
		if item_codes:
			serialized_items = vmraid.db.sql_list(
	...
	def validate_inspection(self):
		...
		for row in self.get("items"):
			qi_required = False
			if inspection_required_fieldname and vmraid.db.get_value(
	...
	def validate_qi_presence(self, row):
		...
		if not row.quality_inspection:
			msg = f"Row #{row.idx}: Quality Inspection is required for Item {vmraid.bold(row.item_code)}"
			if self.docstatus == 1:
				vmraid.throw(_(msg), title=_("Inspection Required"), exc=QualityInspectionRequiredError)
			else:
				vmraid.msgprint(_(msg), title=_("Inspection Required"), indicator="blue")

	def validate_qi_submission(self, row):
		"""Check if QI is submitted on row level, during submission"""
		action = vmraid.db.get_single_value(
			"Stock Settings", "action_if_quality_inspection_is_not_submitted"
		)
		qa_docstatus = vmraid.db.get_value("Quality Inspection", row.quality_inspection, "docstatus")

		if not qa_docstatus == 1:
			link = vmraid.utils.get_link_to_form("Quality Inspection", row.quality_inspection)
			msg = (
				f"Row #{row.idx}: Quality Inspection {link} is not submitted for the item: {row.item_code}"
			)
			if action == "Stop":
				vmraid.throw(_(msg), title=_("Inspection Submission"), exc=QualityInspectionNotSubmittedError)
			else:
				vmraid.msgprint(_(msg), alert=True, indicator="orange")

	def validate_qi_rejection(self, row):
		"""Check if QI is rejected on row level, during submission"""
		action = vmraid.db.get_single_value("Stock Settings", "action_if_quality_inspection_is_rejected")
		qa_status = vmraid.db.get_value("Quality Inspection", row.quality_inspection, "status")

		if qa_status == "Rejected":
			link = vmraid.utils.get_link_to_form("Quality Inspection", row.quality_inspection)
			msg = f"Row #{row.idx}: Quality Inspection {link} was rejected for item {row.item_code}"
			if action == "Stop":
				vmraid.throw(_(msg), title=_("Inspection Rejected"), exc=QualityInspectionRejectedError)
			else:
				vmraid.msgprint(_(msg), alert=True, indicator="orange")
	...
	def update_blanket_order(self):
		...
		for blanket_order in blanket_orders:
			vmraid.get_doc("Blanket Order", blanket_order).update_ordered_qty()

	def validate_customer_provided_item(self):
		for d in self.get("items"):
			# Customer Provided parts will have zero valuation rate
			if vmraid.db.get_value("Item", d.item_code, "is_customer_provided_item"):
	...
	def validate_multi_currency(self):
		if self.currency != self.company_currency:
			vmraid.throw(_("Internal transfers can only be done in company's default currency"))

	def validate_packed_items(self):
		if self.doctype in ("Sales Invoice", "Delivery Note Item") and self.get("packed_items"):
			vmraid.throw(_("Packed Items cannot be transferred internally"))

	def validate_putaway_capacity(self):
		# if over receipt is attempted while 'apply putaway rule' is disabled
		# and if rule was applied on the transaction, validate it.
		from erpadda.stock.doctype.putaway_rule.putaway_rule import get_available_putaway_capacity
		...
		if valid_doctype:
			...
			for rule, values in rule_map.items():
				if flt(values["qty_put"]) > flt(values["capacity"]):
					...
					vmraid.throw(msg=message, title=_("Over Receipt"))

	def prepare_over_receipt_message(self, rule, values):
		message = _(
			"{0} qty of Item {1} is being received into Warehouse {2} with capacity {3}."
		).format(
			vmraid.bold(values["qty_put"]),
			vmraid.bold(values["item"]),
			vmraid.bold(values["warehouse"]),
			vmraid.bold(values["capacity"]),
		)
		message += "<br><br>"
		rule_link = vmraid.utils.get_link_to_form("Putaway Rule", rule)
	...
	def repost_future_sle_and_gle(self):
		...
		if future_sle_exists(args) or repost_required_for_queue(self):
			item_based_reposting = cint(
				vmraid.db.get_single_value("Stock Reposting Settings", "item_based_reposting")
			)
...
def repost_required_for_queue(doc: StockController) -> bool:
	...
	consuming_sles = vmraid.db.get_all(
		"Stock Ledger Entry",
		filters={
...
def make_quality_inspections(doctype, docname, items):
	...
	for item in items:
		...
		quality_inspection = vmraid.get_doc(
			{
				"doctype": "Quality Inspection",
				"inspection_type": "Incoming",
				"inspected_by": vmraid.session.user,
...
def future_sle_exists(args, sl_entries=None):
	...
	for d in data:
		vmraid.local.future_sle[key][(d.item_code, d.warehouse)] = d.total_row
...
def validate_future_sle_not_exists(args, key, sl_entries=None):
	...
	if not sl_entries and hasattr(vmraid.local, "future_sle"):
		if not vmraid.local.future_sle.get(key) or (
			item_key and item_key not in vmraid.local.future_sle.get(key)
		):
			return True


def get_cached_data(args, key):
	if not hasattr(vmraid.local, "future_sle"):
		vmraid.local.future_sle = {}

	if key not in vmraid.local.future_sle:
		vmraid.local.future_sle[key] = vmraid._dict({})

	if args.get("item_code"):
		item_key = (args.get("item_code"), args.get("warehouse"))
		count = vmraid.local.future_sle[key].get(item_key)

		return True if (count or count == 0) else False
	else:
		return vmraid.local.future_sle[key]
...
def get_conditions_to_validate_future_sle(sl_entries):
	...
	for warehouse, items in warehouse_items_map.items():
		or_conditions.append(
			f"""warehouse = {vmraid.db.escape(warehouse)}
				and item_code in ({', '.join(vmraid.db.escape(item) for item in items)})"""
		)
...
def create_repost_item_valuation_entry(args):
	...
	repost_entry = vmraid.new_doc("Repost Item Valuation")
