...
from vmraid.modules.utils import get_module_app
from vmraid.utils import flt, has_common
from vmraid.utils.user import is_website_user


def get_list_context(context=None):
	return {
		"global_number_format": vmraid.db.get_default("number_format") or "#,###.##",
...
def get_webform_list_context(module):
	if get_module_app(module) != "erpadda":
		return
	...
	if txt:
		if meta.get_field("items"):
			if meta.get_field("items").options:
				...
				for item in vmraid.get_all(child_doctype, {"item_name": ["like", "%" + txt + "%"]}):
					child = vmraid.get_doc(child_doctype, item.name)
					or_filters.append([doctype, "name", "=", child.parent])

	if or_filters:
		for r in vmraid.get_list(
			doctype,
			fields=fields,
			filters=filters,
...
def get_customers_suppliers(doctype, user):
	...
	if has_common(["Supplier", "Customer"], vmraid.get_roles(user)):
		contacts = vmraid.db.sql(
			"""
			select
				`tabContact`.email_id,
	...
	elif vmraid.has_permission(doctype, "read", user=user):
		customer_list = vmraid.get_list("Customer")
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	if customers:
		return vmraid.db.exists(doctype, get_customer_filter(doc, customers))
	elif suppliers:
		fieldname = "suppliers" if doctype == "Request for Quotation" else "supplier"
		return vmraid.db.exists(doctype, {"name": doc.name, fieldname: ["in", suppliers]})
	else:
		return False
