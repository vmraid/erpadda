...
def validate_filters(filters):
	for f in ["Fiscal Year", "Based On", "Period", "Company"]:
		if not filters.get(f.lower().replace(" ", "_")):
			vmraid.throw(_("{0} is mandatory").format(f))

	if not vmraid.db.exists("Fiscal Year", filters.get("fiscal_year")):
		vmraid.throw(_("Fiscal Year {0} Does Not Exist").format(filters.get("fiscal_year")))

	if filters.get("based_on") == filters.get("group_by"):
		vmraid.throw(_("'Based On' and 'Group By' can not be same"))
...
def based_wise_columns_query(based_on, trans):
	...
	if based_on == "Item":
		...
		if trans in ["Sales Invoice", "Delivery Note", "Sales Order"]:
			...
			vmraid.throw(_("Project-wise data is not available for Quotation"))
