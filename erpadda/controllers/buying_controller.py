...
import vmraid
from vmraid import ValidationError, _, msgprint
from vmraid.contacts.doctype.address.address import get_address_display
from vmraid.utils import cint, cstr, flt, getdate
...
from erpadda.buying.utils import update_last_purchase_rate, validate_for_items
from erpadda.controllers.sales_and_purchase_return import get_rate_for_return
from erpadda.controllers.stock_controller import StockController
from erpadda.controllers.subcontracting import Subcontracting
...
class BuyingController(StockController, Subcontracting):
	...
	def validate(self):
		...
		if getattr(self, "supplier", None) and not self.supplier_name:
			self.supplier_name = vmraid.db.get_value("Supplier", self.supplier, "supplier_name")
		...
		if self.meta.get_field("supplier") and not self.supplier:
			for d in self.get("items"):
				...
				if supplier:
					...
					item_group = vmraid.db.get_value("Item", d.item_code, "item_group")
					supplier = vmraid.db.get_value(
	...
	def validate_asset_return(self):
		...
		not_cancelled_asset = [
			d.name
			for d in vmraid.db.get_all("Asset", {purchase_doc_field: self.return_against, "docstatus": 1})
		]
	...
	def set_incoming_rate(self):
		...
		for d in items:
			if not cint(self.get("is_return")):
				...
				if not d.get(vmraid.scrub(ref_doctype)):
					...
					rate = vmraid.db.get_value(ref_doctype, d.get(vmraid.scrub(ref_doctype)), "rate")
	...
	def get_supplied_items_cost(self, item_row_id, reset_outgoing_rate=True):
		...
		for d in self.get("supplied_items"):
			if d.reference_name == item_row_id:
				if reset_outgoing_rate and vmraid.get_cached_value("Item", d.rm_item_code, "is_stock_item"):
	...
	def validate_for_subcontracting(self):
		if self.is_subcontracted:
			if self.doctype in ["Purchase Receipt", "Purchase Invoice"] and not self.supplier_warehouse:
				vmraid.throw(_("Supplier Warehouse mandatory for sub-contracted {0}").format(self.doctype))

			for item in self.get("items"):
				if item in self.sub_contracted_items and not item.bom:
					vmraid.throw(_("Please select BOM in BOM field for Item {0}").format(item.item_code))
			...
			for row in self.get("supplied_items"):
				if not row.reserve_warehouse:
					msg = f"Reserved Warehouse is mandatory for the Item {vmraid.bold(row.rm_item_code)} in Raw Materials supplied"
					vmraid.throw(_(msg))
		else:
			for item in self.get("items"):
				if item.bom:
		...
		for d in self.get("items"):
			if d.meta.get_field("stock_qty"):
				...
				if not d.conversion_factor and d.item_code:
					vmraid.throw(_("Row {0}: Conversion Factor is mandatory").format(d.idx))
	...
	def validate_purchase_return(self):
		for d in self.get("items"):
			if self.is_return and flt(d.rejected_qty) != 0:
				vmraid.throw(_("Row #{0}: Rejected Qty can not be entered in Purchase Return").format(d.idx))
	...
	def validate_accepted_rejected_qty(self):
		for d in self.get("items"):
			...
			if flt(val, d.precision("received_qty")) != flt(d.received_qty, d.precision("received_qty")):
				...
				vmraid.throw(msg=message, title=_("Mismatch"), exc=QtyMismatchError)
			...
			if flt(item_row[fieldname]) < 0:
				vmraid.throw(
					_("Row #{0}: {1} can not be negative for item {2}").format(
						item_row["idx"],
						vmraid.get_meta(item_row.doctype).get_label(fieldname),
						item_row["item_code"],
					)
				)
	...
	def check_for_on_hold_or_closed_status(self, ref_doctype, ref_fieldname):
		for d in self.get("items"):
			if d.get(ref_fieldname):
				status = vmraid.db.get_value(ref_doctype, d.get(ref_fieldname), "status")
				if status in ("Closed", "On Hold"):
					vmraid.throw(_("{0} {1} is {2}").format(ref_doctype, d.get(ref_fieldname), status))
	...
	def update_ordered_and_reserved_qty(self):
		...
		for po, po_item_rows in po_map.items():
			if po and po_item_rows:
				po_obj = vmraid.get_doc("Purchase Order", po)

				if po_obj.status in ["Closed", "Cancelled"]:
					vmraid.throw(
						_("{0} {1} is cancelled or closed").format(_("Purchase Order"), po),
						vmraid.InvalidStatusError,
					)
	...
	def auto_make_assets(self, asset_items):
				...
				if item_data.get("auto_create_assets"):
					...
					if item_data.get("asset_naming_series"):
						...
						if len(created_assets) > 5:
							...
							messages.append(
								_("{} Assets created for {}").format(len(created_assets), vmraid.bold(d.item_code))
							)
						else:
							assets_link = list(map(lambda d: vmraid.utils.get_link_to_form("Asset", d), created_assets))
							assets_link = vmraid.bold(",".join(assets_link))

							is_plural = "s" if len(created_assets) != 1 else ""
							messages.append(
								_("Asset{} {assets_link} created for {}").format(
									is_plural, vmraid.bold(d.item_code), assets_link=assets_link
								)
							)
					else:
						vmraid.throw(
							_("Row {}: Asset Naming Series is mandatory for the auto creation for item {}").format(
								d.idx, vmraid.bold(d.item_code)
							)
						)
				else:
					messages.append(
		...
		for message in messages:
			vmraid.msgprint(message, title="Success", indicator="green")

	def make_asset(self, row, is_grouped_asset=False):
		if not row.asset_location:
			vmraid.throw(_("Row {0}: Enter location for the asset item {1}").format(row.idx, row.item_code))

		item_data = vmraid.db.get_value(
	...
	def update_fixed_asset(self, field, delete_asset=False):
		for d in self.get("items"):
			if d.is_fixed_asset:
				is_auto_create_enabled = vmraid.db.get_value("Item", d.item_code, "auto_create_assets")
				assets = vmraid.db.get_all("Asset", filters={field: self.name, "item_code": d.item_code})
				...
				for asset in assets:
					...
					if delete_asset and is_auto_create_enabled:
						...
						for movement in movements:
							vmraid.delete_doc("Asset Movement", movement.name, force=1)
						vmraid.delete_doc("Asset", asset.name, force=1)
						continue
					...
					if self.docstatus in [0, 1] and not asset.get(field):
						...
						if asset.docstatus == 1 and delete_asset:
							vmraid.throw(
								...
								).format(vmraid.utils.get_link_to_form("Asset", asset.name))
							)
	...
	def delete_linked_asset(self):
		...
		vmraid.db.sql("delete from `tabAsset Movement` where reference_name=%s", self.name)
		...
		if self.schedule_date:
			for d in self.get("items"):
				...
				if (
					...
					vmraid.throw(_("Row #{0}: Reqd by Date cannot be before Transaction Date").format(d.idx))
		else:
			vmraid.throw(_("Please enter Reqd by Date"))
...
def validate_item_type(doc, fieldname, message):
	...
	item_list = ", ".join(["%s" % vmraid.db.escape(d) for d in items])
