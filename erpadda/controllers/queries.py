...
from vmraid.utils import nowdate, unique
...
def customer_query(doctype, txt, searchfield, start, page_len, filters):
	conditions = []
	cust_master_name = vmraid.defaults.get_user_default("cust_master_name")
	...
	searchfields = vmraid.get_meta("Customer").get_search_fields()
...
def supplier_query(doctype, txt, searchfield, start, page_len, filters):
	supp_master_name = vmraid.defaults.get_user_default("supp_master_name")
...
def tax_account_query(doctype, txt, searchfield, start, page_len, filters):
	company_currency = erpadda.get_company_currency(filters.get("company"))
...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	if filters and isinstance(filters, dict):
		if filters.get("customer") or filters.get("supplier"):
			...
			item_rules_list = vmraid.get_all(
	...
	description_cond = ""
	if vmraid.db.count("Item", cache=True) < 50000:
...
def get_project_name(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters and filters.get("customer"):
		cond = """(`tabProject`.customer = %s or
			...
			vmraid.db.escape(filters.get("customer"))
		)

	fields = get_fields("Project", ["name", "project_name"])
	searchfields = vmraid.get_meta("Project").get_search_fields()
	searchfields = " or ".join([field + " like %(txt)s" for field in searchfields])
...
def get_batch_no(doctype, txt, searchfield, start, page_len, filters):
	...
	meta = vmraid.get_meta("Batch", cached=True)
	searchfields = meta.get_search_fields()
...
def get_account_list(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.desk.reportview.execute(
		"Account",
		filters=filter_list,
...
def get_blanket_orders(doctype, txt, searchfield, start, page_len, filters):
	return vmraid.db.sql(
		...
		where
			...
			item_code=vmraid.db.escape(filters.get("item")),
			blanket_order_type=filters.get("blanket_order_type"),
			company=vmraid.db.escape(filters.get("company")),
		)
	)
...
def get_filtered_dimensions(doctype, txt, searchfield, start, page_len, filters):
	...
	searchfields = vmraid.get_meta(doctype).get_search_fields()
...
def warehouse_query(doctype, txt, searchfield, start, page_len, filters):
	...
	query = """select `tabWarehouse`.name,
		...
		start=start,
		page_len=page_len,
		txt=vmraid.db.escape("%{0}%".format(txt)),
	)

	return vmraid.db.sql(query)
...
def get_batch_numbers(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters and filters.get("item"):
		query += " and item = {item}".format(item=vmraid.db.escape(filters.get("item")))

	return vmraid.db.sql(query, filters)
...
def item_manufacturer_query(doctype, txt, searchfield, start, page_len, filters):
	...
	item_manufacturers = vmraid.get_all(
		"Item Manufacturer",
...
def get_purchase_receipts(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters and filters.get("item_code"):
		query += " and pritem.item_code = {item_code}".format(
			item_code=vmraid.db.escape(filters.get("item_code"))
		)
...
def get_tax_template(doctype, txt, searchfield, start, page_len, filters):
	...
	item_doc = vmraid.get_cached_doc("Item", filters.get("item_code"))
