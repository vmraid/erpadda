...
class ItemVariantExistsError(vmraid.ValidationError):
	pass


class InvalidItemAttributeValueError(vmraid.ValidationError):
	pass


class ItemTemplateCannotHaveStock(vmraid.ValidationError):
	pass
...
def get_variant(template, args=None, variant=None, manufacturer=None, manufacturer_part_no=None):
	...
	"""
	item_template = vmraid.get_doc("Item", template)
	...
	if item_template.variant_based_on == "Manufacturer" and manufacturer:
		...
		if not args:
			vmraid.throw(_("Please specify at least one attribute in the Attributes table"))
...
def validate_is_incremental(numeric_attribute, attribute, value, item):
	...
	if increment == 0:
		...
		vmraid.throw(_("Increment for Attribute {0} cannot be 0").format(attribute))
...
def validate_item_attribute_value(
	...
	allow_rename_attribute_value = vmraid.db.get_single_value(
		...
		if from_variant:
			vmraid.throw(
				_("{0} is not a valid Value for Attribute {1} of Item {2}.").format(
					vmraid.bold(attribute_value), vmraid.bold(attribute), vmraid.bold(item)
				),
				InvalidItemAttributeValueError,
			...
			msg = _("The value {0} is already assigned to an existing Item {1}.").format(
				vmraid.bold(attribute_value), vmraid.bold(item)
			)
			msg += "<br>" + _(
				"To still proceed with editing this Attribute Value, enable {0} in Item Variant Settings."
			).format(vmraid.bold("Allow Rename Attribute Value"))

			vmraid.throw(msg, InvalidItemAttributeValueError, title=_("Edit Not Allowed"))


def get_attribute_values(item):
	if not vmraid.flags.attribute_values:
		attribute_values = {}
		numeric_values = {}
		for t in vmraid.get_all("Item Attribute Value", fields=["parent", "attribute_value"]):
			attribute_values.setdefault(t.parent.lower(), []).append(t.attribute_value)

		for t in vmraid.get_all(
		...
		vmraid.flags.attribute_values = attribute_values
		vmraid.flags.numeric_values = numeric_values

	return vmraid.flags.attribute_values, vmraid.flags.numeric_values


def find_variant(template, args, variant_item_code=None):
	conditions = [
		"""(iv_attribute.attribute={0} and iv_attribute.attribute_value={1})""".format(
			vmraid.db.escape(key), vmraid.db.escape(cstr(value))
		)
	...
	from erpadda.e_commerce.variant_selector.utils import get_item_codes_by_attributes
	...
	for variant in possible_variants:
		variant = vmraid.get_doc("Item", variant)
...
def enqueue_multiple_variant_creation(item, args):
	...
	if total_variants >= 600:
		vmraid.throw(_("Please do not create more than 500 items at a time"))
		return
	if total_variants < 10:
		return create_multiple_variants(item, args)
	else:
		vmraid.enqueue(
			"erpadda.controllers.item_variant.create_multiple_variants",
			item=item,
			args=args,
...
def copy_attributes_to_variant(item, variant):
	...
	allow_fields = [d.field_name for d in vmraid.get_all("Variant Field", fields=["field_name"])]
...
def make_variant_item_code(template_item_code, template_item_name, variant):
	...
	for attr in variant.attributes:
		...
		if not item_attribute:
			continue
			# vmraid.throw(_('Invalid attribute {0} {1}').format(vmraid.bold(attr.attribute),
			# 	vmraid.bold(attr.attribute_value)), title=_('Invalid Attribute'),
...
def create_variant_doc_for_quick_entry(template, args):
	variant_based_on = vmraid.db.get_value("Item", template, "variant_based_on")
	args = json.loads(args)
