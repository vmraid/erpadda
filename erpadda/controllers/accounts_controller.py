...
from vmraid.model.workflow import get_workflow_name, is_transition_condition_satisfied
...
from erpadda.accounts.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_items,
	apply_pricing_rule_on_transaction,
	get_applied_pricing_rules,
)
from erpadda.accounts.party import (
	get_party_account,
	get_party_account_currency,
	validate_party_frozen_disabled,
)
from erpadda.accounts.utils import get_account_currency, get_fiscal_years, validate_fiscal_year
from erpadda.buying.utils import update_last_purchase_rate
from erpadda.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from erpadda.controllers.sales_and_purchase_return import validate_return
from erpadda.exceptions import InvalidCurrency
...
class AccountMissingError(vmraid.ValidationError):
	pass
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = erpadda.get_company_currency(self.company)
	...
	def onload(self):
		self.set_onload(
			...
			vmraid.db.get_single_value("Accounts Settings", "make_payment_via_journal_entry"),
		)
	...
	def ensure_supplier_is_not_blocked(self):
		...
		if is_buying_invoice or is_supplier_payment:
			...
			supplier = vmraid.get_doc("Supplier", supplier_name)
	...
	def on_trash(self):
		...
		if vmraid.db.get_single_value("Accounts Settings", "delete_linked_ledger_entries"):
			vmraid.db.sql(
		...
		for item in self.get("items"):
			if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
				if not item.get(field_map.get(self.doctype)):
					default_deferred_account = vmraid.db.get_value(
	...
	def calculate_taxes_and_totals(self):
		from erpadda.controllers.taxes_and_totals import calculate_taxes_and_totals
			...
			if item.get(item_field) == self.get(party_account_field):
				vmraid.throw(
					_("Row {0}: {1} {2} cannot be same as {3} (Party Account) {4}").format(
						item.idx,
						vmraid.bold(vmraid.unscrub(item_field)),
						item.get(item_field),
						vmraid.bold(vmraid.unscrub(party_account_field)),
	...
	def validate_inter_company_reference(self):
		...
		if self.is_internal_transfer():
			if not (
				...
				vmraid.throw(msg, title=_("Internal Sales Reference Missing"))

	def validate_due_date(self):
		if self.get("is_pos"):
			return

		from erpadda.accounts.party import validate_due_date

		if self.doctype == "Sales Invoice":
			if not self.due_date:
				vmraid.throw(_("Due Date is mandatory"))
	...
	def set_price_list_currency(self, buying_or_selling):
		...
		if self.meta.get_field("currency"):
			...
			if self.meta.get_field(fieldname) and self.get(fieldname):
				self.price_list_currency = vmraid.db.get_value("Price List", self.get(fieldname), "currency")
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center", self.get("cost_center") or erpadda.get_default_cost_center(self.company)
						)
	...
	def apply_pricing_rule_on_items(self, item, pricing_rule_args):
		if not pricing_rule_args.get("validate_applied_rule", 0):
			...
			for pricing_rule in get_applied_pricing_rules(item.get("pricing_rules")):
				pricing_rule_doc = vmraid.get_cached_doc("Pricing Rule", pricing_rule)
					...
					if item.get(field) < pricing_rule_doc.get(field):
						...
						vmraid.msgprint(
							_("Row {0}: user has not applied the rule {1} on the item {2}").format(
								item.idx, vmraid.bold(title), vmraid.bold(item.item_code)
							)
						)
	...
	def set_taxes(self):
		...
		if (self.is_new() or self.is_pos_profile_changed()) and not self.get("taxes"):
			if self.company and not self.get("taxes_and_charges"):
				...
				self.taxes_and_charges = vmraid.db.get_value(
	...
	def is_pos_profile_changed(self):
		if (
			...
			and self.is_pos
			and self.pos_profile != vmraid.db.get_value("Sales Invoice", self.name, "pos_profile")
		):
			return True
	...
	def validate_enabled_taxes_and_charges(self):
		...
		if vmraid.db.get_value(taxes_and_charges_doctype, self.taxes_and_charges, "disabled"):
			vmraid.throw(
	...
	def validate_tax_account_company(self):
		for d in self.get("taxes"):
			if d.account_head:
				tax_account_company = vmraid.db.get_value("Account", d.account_head, "company")
	...
	def validate_qty_is_not_zero(self):
		if self.doctype != "Purchase Receipt":
			for item in self.items:
				if not item.qty:
					vmraid.throw(_("Item quantity can not be zero"))
	...
	def apply_shipping_rule(self):
		if self.shipping_rule:
			shipping_rule = vmraid.get_doc("Shipping Rule", self.shipping_rule)
		...
		for fieldname in ("shipping_address_name", "shipping_address"):
			...
			if shipping_field and shipping_field.fieldtype == "Link":
				if self.get(fieldname):
					return vmraid.get_doc("Address", self.get(fieldname))
	...
	def is_inclusive_tax(self):
		is_inclusive = cint(
			vmraid.db.get_single_value("Accounts Settings", "show_inclusive_tax_in_print")
		)
		...
		if self.get("doctype") in ["Purchase Invoice", "Sales Invoice"]:
			for d in self.get("advances"):
				if d.exchange_gain_loss:
					...
					gain_loss_account = vmraid.db.get_value("Company", self.company, "exchange_gain_loss_account")
						...
						self.get_gl_dict(
							{
								...
								"cost_center": self.cost_center or erpadda.get_default_cost_center(self.company),
	...
	def update_against_document_in_jv(self):
				...
				args = vmraid._dict(
					{
						...
						"difference_account": vmraid.db.get_value(
		...
		if lst:
			from erpadda.accounts.utils import reconcile_against_document

			reconcile_against_document(lst)

	def on_cancel(self):
		from erpadda.accounts.utils import unlink_ref_doc_from_payment_entries

		if self.doctype in ["Sales Invoice", "Purchase Invoice"]:
			if vmraid.db.get_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice"):
				unlink_ref_doc_from_payment_entries(self)

		elif self.doctype in ["Sales Order", "Purchase Order"]:
			if vmraid.db.get_single_value(
	...
	def unlink_ref_doc_from_po(self):
		...
		if linked_po:
			...
			vmraid.msgprint(_("Purchase Orders {0} are un-linked").format("\n".join(linked_po)))
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on, parentfield):
		from erpadda.controllers.status_updater import get_allowance_for

		item_allowance = {}
		global_qty_allowance, global_amount_allowance = None, None

		role_allowed_to_over_bill = vmraid.db.get_single_value(
		...
		for item in self.get("items"):
			...
			ref_amt = flt(
				vmraid.db.get_value(ref_dt + " Item", item.get(item_ref_dn), based_on),
	...
	def get_billed_amount_for_item(self, item, item_ref_dn, based_on):
		...
		from vmraid.query_builder import Criterion
		from vmraid.query_builder.functions import Sum

		item_doctype = vmraid.qb.DocType(item.doctype)
		based_on_field = vmraid.qb.Field(based_on)
		join_field = vmraid.qb.Field(item_ref_dn)

		result = (
			vmraid.qb.from_(item_doctype)
	...
	def get_company_default(self, fieldname, ignore_validation=False):
		from erpadda.accounts.utils import get_company_default
	...
	def get_stock_items(self):
		...
		if item_codes:
			stock_items = vmraid.db.get_values(
	...
	def set_total_advance_paid(self):
		...
		if advance:
			...
			vmraid.db.set_value(self.doctype, self.name, "party_account_currency", advance.account_currency)
			...
			vmraid.db.set_value(self.doctype, self.name, "advance_paid", advance_paid)

	@property
	def company_abbr(self):
		if not hasattr(self, "_abbr"):
			self._abbr = vmraid.db.get_value("Company", self.company, "abbr")
	...
	def raise_missing_debit_credit_account_error(self, party_type, party):
		...
		db_or_cr = (
			vmraid.bold("Debit To") if self.doctype == "Sales Invoice" else vmraid.bold("Credit To")
		)
		rec_or_pay = "Receivable" if self.doctype == "Sales Invoice" else "Payable"

		link_to_party = vmraid.utils.get_link_to_form(party_type, party)
		link_to_company = vmraid.utils.get_link_to_form("Company", self.company)

		message = _("{0} Account not found against Customer {1}.").format(
			db_or_cr, vmraid.bold(party) or ""
		)
		...
		vmraid.throw(message, title=_("Account Missing"), exc=AccountMissingError)
	...
	def set_payment_schedule(self):
		...
		if self.doctype in ("Sales Invoice", "Purchase Invoice"):
			...
			automatically_fetch_payment_terms = cint(
				vmraid.db.get_single_value("Accounts Settings", "automatically_fetch_payment_terms")
			)
	...
	def linked_order_has_payment_terms_template(self, po_or_so, doctype):
		return vmraid.get_value(doctype, po_or_so, "payment_terms_template")

	def linked_order_has_payment_schedule(self, po_or_so):
		return vmraid.get_all("Payment Schedule", filters={"parent": po_or_so})

	def fetch_payment_terms_from_order(self, po_or_so, po_or_so_doctype):
		"""
		Fetch Payment Terms from Purchase/Sales Order on creating a new Purchase/Sales Invoice.
		"""
		po_or_so = vmraid.get_cached_doc(po_or_so_doctype, po_or_so)
	...
	def is_rounded_total_disabled(self):
		if self.meta.get_field("disable_rounded_total"):
			...
			return vmraid.db.get_single_value("Global Defaults", "disable_rounded_total")
	...
	def set_inter_company_account(self):
		...
		if self.is_internal_transfer() and not self.unrealized_profit_loss_account:
			unrealized_profit_loss_account = vmraid.db.get_value(
	...
	def process_common_party_accounting(self):
		...
		if vmraid.db.get_single_value("Accounts Settings", "enable_common_party_accounting"):
	...
	def create_advance_and_reconcile(self, party_link):
		...
		jv = vmraid.new_doc("Journal Entry")
		jv.voucher_type = "Journal Entry"
		jv.posting_date = self.posting_date
		jv.company = self.company
		jv.remark = "Adjustment for {} {}".format(self.doctype, self.name)

		reconcilation_entry = vmraid._dict()
		advance_entry = vmraid._dict()
...
def get_tax_rate(account_head):
	return vmraid.db.get_value("Account", account_head, ["tax_rate", "account_name"], as_dict=True)
...
def get_default_taxes_and_charges(master_doctype, tax_template=None, company=None):
	...
	if tax_template and company:
		tax_template_company = vmraid.db.get_value(master_doctype, tax_template, "company")
		if tax_template_company == company:
			return

	default_tax = vmraid.db.get_value(master_doctype, {"is_default": 1, "company": company})
...
def get_taxes_and_charges(master_doctype, master_name):
	...
	tax_master = vmraid.get_doc(master_doctype, master_name)
...
def validate_account_head(idx, account, company, context=""):
	account_company = vmraid.get_cached_value("Account", account, "company")

	if account_company != company:
		vmraid.throw(
			_("Row {0}: {3} Account {1} does not belong to Company {2}").format(
				idx, vmraid.bold(account), vmraid.bold(company), context
			),
...
def validate_cost_center(tax, doc):
	...
	company = vmraid.get_cached_value("Cost Center", tax.cost_center, "company")

	if company != doc.company:
		vmraid.throw(
			_("Row {0}: Cost Center {1} does not belong to Company {2}").format(
				tax.idx, vmraid.bold(tax.cost_center), vmraid.bold(doc.company)
			),
...
def validate_inclusive_tax(tax, doc):
	...
	if cint(getattr(tax, "included_in_print_rate", None)):
		if tax.charge_type == "Actual":
			...
			vmraid.throw(_("Valuation type charges can not be marked as Inclusive"))
...
def get_advance_payment_entries(
	...
	if order_list or against_all_orders:
		...
		payment_entries_against_order = vmraid.db.sql(
			"""
			select
	...
	if include_unallocated:
		unallocated_payment_entries = vmraid.db.sql(
			"""
...
def update_invoice_status():
	...
	for doctype in ("Sales Invoice", "Purchase Invoice"):
		...
		total_amount = vmraid.qb.terms.Case().when(consider_base_amount, base_total).else_(total)
		...
		vmraid.qb.update(invoice).set("status", status).where(conditions).run()
...
def get_payment_terms(
	...
	terms_doc = vmraid.get_doc("Payment Terms Template", terms_template)
...
def get_payment_term_details(
	...
	term_details = vmraid._dict()
	if isinstance(term, str):
		term = vmraid.get_doc("Payment Term", term)
	else:
...
def get_supplier_block_status(party_name):
	...
	a `Supplier`
	"""
	supplier = vmraid.get_doc("Supplier", party_name)
	info = {
...
def add_taxes_from_tax_template(child_item, parent_doc, db_insert=True):
	add_taxes_from_item_tax_template = vmraid.db.get_single_value(
...
def set_order_defaults(
	...
	"""
	p_doc = vmraid.get_doc(parent_doctype, parent_doctype_name)
	child_item = vmraid.new_doc(child_doctype, p_doc, child_docname)
	item = vmraid.get_doc("Item", trans_item.get("item_code"))
		...
		if not child_item.warehouse:
			vmraid.throw(
				_("Cannot find {} for item {}. Please set the same in Item Master or Stock Settings.").format(
					vmraid.bold("default warehouse"), vmraid.bold(item.item_code)
				)
			)
...
def update_bin_on_delete(row, doctype):
	...
	from erpadda.stock.stock_balance import (
		get_indented_qty,
		get_ordered_qty,
		get_reserved_qty,
		update_bin_qty,
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	def validate_workflow_conditions(doc):
		...
		workflow_doc = vmraid.get_doc("Workflow", workflow)
	...
	def validate_quantity(child_item, d):
		if parent_doctype == "Sales Order" and flt(d.get("qty")) < flt(child_item.delivered_qty):
			vmraid.throw(_("Cannot set quantity less than delivered quantity"))

		if parent_doctype == "Purchase Order" and flt(d.get("qty")) < flt(child_item.received_qty):
			vmraid.throw(_("Cannot set quantity less than received quantity"))

	data = json.loads(trans_items)

	sales_doctypes = ["Sales Order", "Sales Invoice", "Delivery Note", "Quotation"]
	parent = vmraid.get_doc(parent_doctype, parent_doctype_name)
	...
	for d in data:
		...
		if not d.get("docname"):
			...
			check_doc_permissions(parent, "write")
			child_item = vmraid.get_doc(parent_doctype + " Item", d.get("docname"))
