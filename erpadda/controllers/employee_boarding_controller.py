...
from vmraid.utils import add_days, flt, unique
...
class EmployeeBoardingController(Document):
		...
		if self.doctype == "Employee Separation":
			...
			if self.employee:
				...
				if not self.holiday_list:
					vmraid.throw(_("Please set the Holiday List."), vmraid.MandatoryError)
				else:
	...
	def on_cancel(self):
		# delete task project
		project = self.project
		for task in vmraid.get_all("Task", filters={"project": project}):
			vmraid.delete_doc("Task", task.name, force=1)
		vmraid.delete_doc("Project", project, force=1)
		self.db_set("project", "")
		for activity in self.activities:
...
def update_employee_boarding_status(project):
	employee_onboarding = vmraid.db.exists("Employee Onboarding", {"project": project.name})
	employee_separation = vmraid.db.exists("Employee Separation", {"project": project.name})
	...
	if employee_onboarding:
		vmraid.db.set_value("Employee Onboarding", employee_onboarding, "boarding_status", status)
	elif employee_separation:
		vmraid.db.set_value("Employee Separation", employee_separation, "boarding_status", status)
