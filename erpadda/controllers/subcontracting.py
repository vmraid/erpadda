...
class Subcontracting:
	...
	def initialized_fields(self):
		self.available_materials = vmraid._dict()
		self.__transferred_items = vmraid._dict()
		self.alternative_item_details = vmraid._dict()
		self.__get_backflush_based_on()

	def __get_backflush_based_on(self):
		self.backflush_based_on = vmraid.db.get_single_value(
	...
	def __get_transferred_items(self):
		...
		return vmraid.get_all("Stock Entry", fields=fields, filters=filters)
	...
	def __get_received_items(self, doctype):
		...
		return vmraid.get_all(f"{doctype}", fields=fields, filters=filters)
	...
	def __get_materials_from_bom(self, item_code, bom_no, exploded_item=0):
		...
		return (
			vmraid.get_all("BOM", fields=fields, filters=filters, order_by=f"`tab{doctype}`.`idx`") or []
		)
	...
	def __get_qty_based_on_material_transfer(self, item_row, transfer_item):
		...
		if self.qty_to_be_received:
			...
			if transfer_item.serial_no or vmraid.get_cached_value(
				"UOM", transfer_item.item_details.stock_uom, "must_be_whole_number"
			):
				return vmraid.utils.ceil(qty)
	...
	def __update_consumed_qty_in_po(self, itemwise_consumed_qty):
		...
		for row in vmraid.get_all(
			...
			vmraid.db.set_value("Purchase Order Item Supplied", row.name, "consumed_qty", consumed_qty)
	...
	def __validate_batch_no(self, row, key):
		if row.get("batch_no") and row.get("batch_no") not in self.__transferred_items.get(key).get(
			...
			msg = f'The Batch No {vmraid.bold(row.get("batch_no"))} has not supplied against the Purchase Order {link}'
			vmraid.throw(_(msg), title=_("Incorrect Batch Consumed"))
	...
	def __validate_serial_no(self, row, key):
		if row.get("serial_no"):
			...
			if incorrect_sn:
				...
				vmraid.throw(_(msg), title=_("Incorrect Serial Number Consumed"))
