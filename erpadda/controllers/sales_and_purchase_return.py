...
from vmraid.utils import flt, format_datetime, get_datetime

import erpadda
from erpadda.stock.utils import get_incoming_rate


class StockOverReturnError(vmraid.ValidationError):
	pass
...
def validate_return_against(doc):
	if not vmraid.db.exists(doc.doctype, doc.return_against):
		vmraid.throw(
			_("Invalid {0}: {1}").format(doc.meta.get_label("return_against"), doc.return_against)
		)
	else:
		ref_doc = vmraid.get_doc(doc.doctype, doc.return_against)
...
def validate_returned_items(doc):
	...
	for d in doc.get("items"):
		if d.item_code and (flt(d.qty) < 0 or flt(d.get("received_qty")) < 0):
			if d.item_code not in valid_items:
				...
				ref = valid_items.get(d.item_code, vmraid._dict())
				...
				if ref.rate and doc.doctype in ("Delivery Note", "Sales Invoice") and flt(d.rate) > ref.rate:
					...
					if not d.serial_no:
						vmraid.throw(_("Row # {0}: Serial No is mandatory").format(d.idx))
					else:
				...
				if (
					...
					and vmraid.db.get_value("Item", d.item_code, "is_stock_item")
				):
					vmraid.throw(_("Warehouse is mandatory"))
	...
	if not items_returned:
		vmraid.throw(_("Atleast one item should be entered with negative quantity in return document"))
...
def validate_quantity(doc, args, ref, valid_items, already_returned_items):
	...
	company_currency = erpadda.get_company_currency(doc.company)
	stock_qty_precision = get_field_precision(
		vmraid.get_meta(doc.doctype + " Item").get_field("stock_qty"), company_currency
	)
	...
	for column in fields:
		...
		if reference_qty:
			if flt(args.get(column)) > 0:
				vmraid.throw(_("{0} must be negative in return document").format(label))
...
def get_returned_qty_map_for_row(return_against, party, row_name, doctype):
	...
	reference_field = "dn_detail" if doctype == "Delivery Note" else vmraid.scrub(child_doctype)
...
def make_return_doc(doctype, source_name, target_doc=None):
	...
	company = vmraid.db.get_value("Delivery Note", source_name, "company")
	default_warehouse_for_sales_return = vmraid.db.get_value(
	...
	def set_missing_values(source, target):
		...
		if doctype == "Sales Invoice" or doctype == "POS Invoice":
			...
			if not doc.select_print_heading:
				doc.select_print_heading = vmraid.db.get_value("Print Heading", _("Credit Note"))

		elif doctype == "Purchase Invoice":
			# look for Print Heading "Debit Note"
			doc.select_print_heading = vmraid.db.get_value("Print Heading", _("Debit Note"))
...
def get_rate_for_return(
	...
	if not return_against:
		return_against = vmraid.get_cached_value(voucher_type, voucher_no, "return_against")
	...
	rate = flt(vmraid.db.get_value("Stock Ledger Entry", filters, select_field))
	if not (rate and return_against) and voucher_type in ["Sales Invoice", "Delivery Note"]:
		rate = vmraid.db.get_value(f"{voucher_type} Item", voucher_detail_no, "incoming_rate")
...
def get_filters(
	...
	if item_row:
		...
		reference_voucher_detail_no = vmraid.db.get_value(
...
def get_returned_serial_nos(child_doc, parent_doc):
	...
	return_ref_field = vmraid.scrub(child_doc.doctype)
	...
	for row in vmraid.get_all(parent_doc.doctype, fields=fields, filters=filters):
