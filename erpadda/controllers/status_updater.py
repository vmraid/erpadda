...
from vmraid.utils import comma_or, flt, getdate, now, nowdate


class OverAllowanceError(vmraid.ValidationError):
	pass


def validate_status(status, options):
	if status not in options:
		vmraid.throw(_("Status must be one of {0}").format(comma_or(options)))
...
class StatusUpdater(Document):
		...
		for args in self.status_updater:
			...
			for d in self.get_all_children():
				if hasattr(d, "qty") and d.qty < 0 and not self.get("is_return"):
					vmraid.throw(_("For an item {0}, quantity must be positive number").format(d.item_code))

				if hasattr(d, "qty") and d.qty > 0 and self.get("is_return"):
					vmraid.throw(_("For an item {0}, quantity must be negative number").format(d.item_code))
	...
	def check_overflow_with_allowance(self, item, args):
		...
		role_allowed_to_over_deliver_receive = vmraid.db.get_single_value(
		...
		if overflow_percent - allowance > 0.01:
			...
			if role not in vmraid.get_roles():
	...
	def limits_crossed_error(self, args, item, qty_or_amount):
		...
		vmraid.throw(
			_(
				...
				vmraid.bold(_(item["target_ref_field"].title())),
				vmraid.bold(item["reduce_by"]),
				vmraid.bold(_(args.get("target_dt"))),
				vmraid.bold(_(self.doctype)),
				vmraid.bold(item.get("item_code")),
			)
			+ "<br><br>"
			+ action_msg,
			OverAllowanceError,
	...
	def warn_about_bypassing_with_role(self, item, qty_or_amount, role):
		...
		vmraid.msgprint(msg, indicator="orange", alert=True)
	...
	def _update_children(self, args, update_modified):
		...
		for d in self.get_all_children():
			...
			if (
				...
				args["second_source_condition"] = vmraid.db.sql(
	...
	def _update_percent_field(self, args, update_modified=True):
		...
		if args.get("target_parent_field"):
			...
			if update_modified:
				target = vmraid.get_doc(args["target_parent_dt"], args["name"])
	...
	def _update_modified(self, args, update_modified):
		...
		args["update_modified"] = ", modified = {0}, modified_by = {1}".format(
			vmraid.db.escape(now()), vmraid.db.escape(vmraid.session.user)
		)

	def update_billing_status_for_zero_amount_refdoc(self, ref_dt):
		ref_fieldname = vmraid.scrub(ref_dt)
		...
		zero_amount_refdocs = vmraid.db.sql_list(
			"""
			SELECT
				name
			from
	...
	def update_billing_status(self, zero_amount_refdoc, ref_dt, ref_fieldname):
		for ref_dn in zero_amount_refdoc:
			...
			ref_doc = vmraid.get_doc(ref_dt, ref_dn)
...
def get_allowance_for(
	...
	if item_allowance is None:
		item_allowance = {}
	if qty_or_amount == "qty":
		if item_allowance.get(item_code, vmraid._dict()).get("qty"):
			return (
		...
		if item_allowance.get(item_code, vmraid._dict()).get("amount"):
			return (
	...
	qty_allowance, over_billing_allowance = vmraid.db.get_value(
	...
	if qty_or_amount == "qty" and not qty_allowance:
		if global_qty_allowance == None:
			global_qty_allowance = flt(
				vmraid.db.get_single_value("Stock Settings", "over_delivery_receipt_allowance")
			)
		qty_allowance = global_qty_allowance
	elif qty_or_amount == "amount" and not over_billing_allowance:
		if global_amount_allowance == None:
			global_amount_allowance = flt(
				vmraid.db.get_single_value("Accounts Settings", "over_billing_allowance")
			)
		over_billing_allowance = global_amount_allowance

	if qty_or_amount == "qty":
		allowance = qty_allowance
		item_allowance.setdefault(item_code, vmraid._dict()).setdefault("qty", qty_allowance)
	else:
		allowance = over_billing_allowance
		item_allowance.setdefault(item_code, vmraid._dict()).setdefault("amount", over_billing_allowance)
