...
from erpadda.e_commerce.shopping_cart.cart import get_debtors_account


def set_default_role(doc, method):
	"""Set customer, supplier, student, guardian based on email"""
	if vmraid.flags.setting_role or vmraid.flags.in_migrate:
		return

	roles = vmraid.get_roles(doc.name)

	contact_name = vmraid.get_value("Contact", dict(email_id=doc.email))
	if contact_name:
		contact = vmraid.get_doc("Contact", contact_name)
		for link in contact.links:
			vmraid.flags.setting_role = True
			if link.link_doctype == "Customer" and "Customer" not in roles:
				doc.add_roles("Customer")
			elif link.link_doctype == "Supplier" and "Supplier" not in roles:
				doc.add_roles("Supplier")
	elif vmraid.get_value("Student", dict(student_email_id=doc.email)) and "Student" not in roles:
		doc.add_roles("Student")
	elif vmraid.get_value("Guardian", dict(email_address=doc.email)) and "Guardian" not in roles:
...
def create_customer_or_supplier():
	...
	if vmraid.db.get_value("User", user, "user_type") != "Website User":
		return

	user_roles = vmraid.get_roles()
	portal_settings = vmraid.get_single("Portal Settings")
	...
	party = vmraid.new_doc(doctype)
	fullname = vmraid.utils.get_fullname(user)
...
def party_exists(doctype, user):
	...
	contact_name = vmraid.db.get_value("Contact", {"email_id": user})
	if contact_name:
		contact = vmraid.get_doc("Contact", contact_name)
