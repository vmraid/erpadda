...
from vmraid.utils import flt, get_datetime, getdate
...
class LoanSecurityUnpledge(Document):
			...
			if d.loan_security not in security_list:
				...
				vmraid.throw(
					_("Row {0}: Loan Security {1} added multiple times").format(
						d.idx, vmraid.bold(d.loan_security)
					)
				)
	...
	def validate_unpledge_qty(self):
		...
		loan_security_price_map = vmraid._dict(
			vmraid.get_all(
				"Loan Security Price",
		...
		for security in self.securities:
			...
			if security.qty > pledged_qty:
				msg = _("Row {0}: {1} {2} of {3} is pledged against Loan {4}.").format(
					...
					pledged_qty,
					security.uom,
					vmraid.bold(security.loan_security),
					vmraid.bold(self.loan),
				)
				msg += "<br>"
				msg += _("You are trying to unpledge more.")
				vmraid.throw(msg, title=_("Loan Security Unpledge Error"))
	...
	def _throw(self, security_value, pending_principal_amount, ltv_ratio):
		msg = _("Loan Security Value after unpledge is {0}").format(vmraid.bold(security_value))
		msg += "<br>"
		msg += _("Pending principal amount is {0}").format(vmraid.bold(flt(pending_principal_amount, 2)))
		msg += "<br>"
		msg += _("Loan To Security Value ratio must always be {0}").format(vmraid.bold(ltv_ratio))
		vmraid.throw(msg, title=_("Loan To Value ratio breach"))
	...
	def update_loan_status(self, cancel=0):
		if cancel:
			loan_status = vmraid.get_value("Loan", self.loan, "status")
			if loan_status == "Closed":
				vmraid.db.set_value("Loan", self.loan, "status", "Loan Closure Requested")
		else:
			pledged_qty = 0
			...
			if not pledged_qty:
				vmraid.db.set_value("Loan", self.loan, {"status": "Closed", "closure_date": getdate()})
