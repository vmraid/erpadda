...
vmraid.ui.form.on('Loan Application', {
	...
	add_toolbar_buttons: function(frm) {
		if (frm.doc.status == "Approved") {
			...
			if (frm.doc.is_secured_loan) {
				vmraid.db.get_value("Loan Security Pledge", {"loan_application": frm.doc.name, "docstatus": 1}, "name", (r) => {
			...
			vmraid.db.get_value("Loan", {"loan_application": frm.doc.name, "docstatus": 1}, "name", (r) => {
				if (Object.keys(r).length === 0) {
	...
	create_loan: function(frm) {
		if (frm.doc.status != "Approved") {
			vmraid.throw(__("Cannot create loan until application is approved"));
		}

		vmraid.model.open_mapped_doc({
			method: 'erpadda.loan_management.doctype.loan_application.loan_application.create_loan',
			frm: frm
		});
	},
	create_loan_security_pledge: function(frm) {
		...
		if(!frm.doc.is_secured_loan) {
			vmraid.throw(__("Loan Security Pledge can only be created for secured loans"));
		}

		vmraid.call({
			method: "erpadda.loan_management.doctype.loan_application.loan_application.create_pledge",
			args: {
				loan_application: frm.doc.name
			},
			callback: function(r) {
				vmraid.set_route("Form", "Loan Security Pledge", r.message);
			}
		})
	},
	...
	calculate_amounts: function(frm, cdt, cdn) {
		...
		if (row.qty) {
			vmraid.model.set_value(cdt, cdn, 'amount', row.qty * row.loan_security_price);
			vmraid.model.set_value(cdt, cdn, 'post_haircut_amount', cint(row.amount - (row.amount * row.haircut/100)));
		} else if (row.amount) {
			vmraid.model.set_value(cdt, cdn, 'qty', cint(row.amount / row.loan_security_price));
			vmraid.model.set_value(cdt, cdn, 'amount', row.qty * row.loan_security_price);
			vmraid.model.set_value(cdt, cdn, 'post_haircut_amount', cint(row.amount - (row.amount * row.haircut/100)));
		}
...
vmraid.ui.form.on("Proposed Pledge", {
	loan_security: function(frm, cdt, cdn) {
		let row = locals[cdt][cdn];

		if (row.loan_security) {
			vmraid.call({
				method: "erpadda.loan_management.doctype.loan_security_price.loan_security_price.get_loan_security_price",
				args: {
					loan_security: row.loan_security
				},
				callback: function(r) {
					vmraid.model.set_value(cdt, cdn, 'loan_security_price', r.message);
