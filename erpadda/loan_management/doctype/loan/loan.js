...
vmraid.ui.form.on('Loan', {
	...
	make_loan_disbursement: function (frm) {
		vmraid.call({
			...
			method: "erpadda.loan_management.doctype.loan.loan.make_loan_disbursement",
			callback: function (r) {
				if (r.message)
	...
	make_repayment_entry: function(frm) {
		vmraid.call({
			...
			method: "erpadda.loan_management.doctype.loan.loan.make_repayment_entry",
			callback: function (r) {
				if (r.message)
					var doc = vmraid.model.sync(r.message)[0];
				vmraid.set_route("Form", doc.doctype, doc.name);
	...
	make_loan_write_off_entry: function(frm) {
		vmraid.call({
			...
			method: "erpadda.loan_management.doctype.loan.loan.make_loan_write_off",
			callback: function (r) {
				if (r.message)
					var doc = vmraid.model.sync(r.message)[0];
				vmraid.set_route("Form", doc.doctype, doc.name);
	...
	make_loan_refund: function(frm) {
		vmraid.call({
			...
			method: "erpadda.loan_management.doctype.loan.loan.make_refund_jv",
			callback: function (r) {
				if (r.message) {
	...
	request_loan_closure: function(frm) {
		vmraid.confirm(__("Do you really want to close this loan"),
			function() {
				vmraid.call({
					args: {
						'loan': frm.doc.name
					},
					method: "erpadda.loan_management.doctype.loan.loan.request_loan_closure",
					callback: function() {
						frm.reload_doc();
					}
				});
	...
	create_loan_security_unpledge: function(frm) {
		vmraid.call({
			method: "erpadda.loan_management.doctype.loan.loan.unpledge_security",
			args : {
				"loan": frm.doc.name,
				"as_dict": 1
			},
	...
	loan_application: function (frm) {
		if(frm.doc.loan_application){
			return vmraid.call({
				method: "erpadda.loan_management.doctype.loan.loan.get_loan_application",
				args: {
