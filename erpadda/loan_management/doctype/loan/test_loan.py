...
import vmraid
from vmraid.utils import add_days, add_months, add_to_date, date_diff, flt, get_datetime, nowdate
...
from erpadda.loan_management.doctype.loan_application.loan_application import create_pledge
from erpadda.loan_management.doctype.loan_disbursement.loan_disbursement import (
	get_disbursal_amount,
)
from erpadda.loan_management.doctype.loan_interest_accrual.loan_interest_accrual import (
	days_in_year,
)
...
from erpadda.loan_management.doctype.process_loan_security_shortfall.process_loan_security_shortfall import (
...
from erpadda.selling.doctype.customer.test_customer import get_customer_dict
...
class TestLoan(unittest.TestCase):
	def setUp(self):
		...
		if not vmraid.db.exists("Customer", "_Test Loan Customer"):
			vmraid.get_doc(get_customer_dict("_Test Loan Customer")).insert(ignore_permissions=True)

		if not vmraid.db.exists("Customer", "_Test Loan Customer 1"):
			vmraid.get_doc(get_customer_dict("_Test Loan Customer 1")).insert(ignore_permissions=True)

		self.applicant2 = vmraid.db.get_value("Customer", {"name": "_Test Loan Customer"}, "name")
		self.applicant3 = vmraid.db.get_value("Customer", {"name": "_Test Loan Customer 1"}, "name")

		create_loan(self.applicant1, "Personal Loan", 280000, "Repay Over Number of Periods", 20)

	def test_loan(self):
		loan = vmraid.get_doc("Loan", {"applicant": self.applicant1})
	...
	def test_loan_disbursement(self):
		...
		loan = vmraid.get_doc("Loan", loan.name)
		gl_entries1 = vmraid.db.get_all(
			"GL Entry",
			fields=["name"],
			filters={"voucher_type": "Loan Disbursement", "voucher_no": loan_disbursement_entry1.name},
		)

		gl_entries2 = vmraid.db.get_all(
			"GL Entry",
			fields=["name"],
	...
	def test_sanctioned_amount_limit(self):
		...
		vmraid.db.sql("DELETE FROM `tabLoan` where applicant = '_Test Loan Customer 1'")
		vmraid.db.sql("DELETE FROM `tabLoan Application` where applicant = '_Test Loan Customer 1'")
		vmraid.db.sql("DELETE FROM `tabLoan Security Pledge` where applicant = '_Test Loan Customer 1'")
		...
		self.assertRaises(vmraid.ValidationError, loan_application.save)
	...
	def test_security_shortfall(self):
		...
		create_process_loan_security_shortfall()
		loan_security_shortfall = vmraid.get_doc("Loan Security Shortfall", {"loan": loan.name})
	...
	def test_penalty(self):
		...
		calculated_penalty_amount = vmraid.db.get_value(
...
def create_loan_accounts():
	if not vmraid.db.exists("Account", "Loans and Advances (Assets) - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
	...
	if not vmraid.db.exists("Account", "Loan Account - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not vmraid.db.exists("Account", "Payment Account - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not vmraid.db.exists("Account", "Disbursement Account - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not vmraid.db.exists("Account", "Interest Income Account - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not vmraid.db.exists("Account", "Penalty Income Account - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_loan_type(
	...
	if not vmraid.db.exists("Loan Type", loan_name):
		loan_type = vmraid.get_doc(
			{
				"doctype": "Loan Type",
				"company": "_Test Company",
				"loan_name": loan_name,
...
def create_loan_security_type():
	if not vmraid.db.exists("Loan Security Type", "Stock"):
		vmraid.get_doc(
			{
...
def create_loan_security():
	if not vmraid.db.exists("Loan Security", "Test Security 1"):
		vmraid.get_doc(
			{
	...
	if not vmraid.db.exists("Loan Security", "Test Security 2"):
		vmraid.get_doc(
			{
				"doctype": "Loan Security",
				"loan_security_type": "Stock",
...
def make_loan_disbursement_entry(loan, amount, disbursement_date=None):
	...
	loan_disbursement_entry = vmraid.get_doc(
		{
	...
	loan = vmraid.get_doc(
		{
			...
			"repayment_periods": repayment_periods,
			"repayment_start_date": repayment_start_date or nowdate(),
			"mode_of_payment": vmraid.db.get_value("Mode of Payment", {"type": "Cash"}, "name"),
			"payment_account": "Payment Account - _TC",
			"loan_account": "Loan Account - _TC",
			"interest_income_account": "Interest Income Account - _TC",
			"penalty_income_account": "Penalty Income Account - _TC",
