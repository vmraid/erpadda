...
from erpadda.regional.report.vat_audit_report.vat_audit_report import execute
...
class TestVATAuditReport(TestCase):
	...
	def tearDown(self):
		vmraid.db.sql("delete from `tabSales Invoice` where company='_Test Company SA VAT'")
		vmraid.db.sql("delete from `tabPurchase Invoice` where company='_Test Company SA VAT'")
...
def set_sa_vat_accounts():
	if not vmraid.db.exists("South Africa VAT Settings", "_Test Company SA VAT"):
...
def make_customer():
	if not vmraid.db.exists("Customer", "_Test SA Customer"):
		vmraid.get_doc(
			{
				"doctype": "Customer",
...
def make_supplier():
	if not vmraid.db.exists("Supplier", "_Test SA Supplier"):
		vmraid.get_doc(
			{
				"doctype": "Supplier",
