...
from vmraid.utils import formatdate, get_link_to_form
...
class VATAuditReport(object):
	...
	def get_sa_vat_accounts(self):
		self.sa_vat_accounts = vmraid.get_all(
			"South Africa VAT Account", filters={"parent": self.filters.company}, pluck="account"
		)
		if not self.sa_vat_accounts and not vmraid.flags.in_test and not vmraid.flags.in_migrate:
			link_to_settings = get_link_to_form(
				"South Africa VAT Settings", "", label="South Africa VAT Settings"
			)
			vmraid.throw(_("Please set VAT Accounts in {0}").format(link_to_settings))
	...
	def get_items_based_on_tax_rate(self, doctype):
		self.items_based_on_tax_rate = vmraid._dict()
	...
	def get_data(self, doctype):
		...
		for rate, section in consolidated_data.items():
			rate = int(rate)
			label = vmraid.bold(section_name + "- " + "Rate" + " " + str(rate) + "%")
			...
			total = {
				"posting_date": vmraid.bold(_("Total")),
