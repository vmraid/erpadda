...
from vmraid.utils import flt, formatdate, getdate
...
class Gstr1Report(object):
	...
	def get_invoice_data(self):
		self.invoices = vmraid._dict()
		conditions = self.get_conditions()

		invoice_data = vmraid.db.sql(
			"""
			select
				{select_columns}
			from `tab{doctype}`
	...
	def get_conditions(self):
		...
		if self.filters.get("type_of_business") in ("B2C Large", "B2C Small"):
			b2c_limit = vmraid.db.get_single_value("GST Settings", "b2c_limit")
			if not b2c_limit:
				vmraid.throw(_("Please set B2C Limit in GST Settings."))
	...
	def get_invoice_items(self):
		self.invoice_items = vmraid._dict()
		self.item_tax_rate = vmraid._dict()
	...
	def get_items_based_on_tax_rate(self):
		self.tax_details = vmraid.db.sql(
			"""
			select
		...
		self.invoice_cess = vmraid._dict()
		...
		for item in report_data[:-1]:
			if not item.get("place_of_supply"):
				vmraid.throw(
					...
					).format(vmraid.bold("Place Of Supply"))
				)
			...
			if not invoice[0]["place_of_supply"]:
				vmraid.throw(
					...
					Please update and try again"""
						...
						vmraid.bold("Place Of Supply"), vmraid.bold(invoice[0]["invoice_number"])
					)
				)
...
def get_company_gstin_number(company, address=None, all_gstins=False):
	gstin = ""
	if address:
		gstin = vmraid.db.get_value("Address", address, "gstin")
	...
	if not gstin:
		address = vmraid.bold(address) if address else ""
		vmraid.throw(
			_("Please set valid GSTIN No. in Company Address {} for company {}").format(
				address, vmraid.bold(company)
			)
		)
...
def download_json_file():
	...
	vmraid.response["filename"] = (
		vmraid.scrub("{0} {1}".format(data["report_name"], data["report_type"])) + ".json"
	)
...
def get_company_gstins(company):
	address = vmraid.qb.DocType("Address")
	links = vmraid.qb.DocType("Dynamic Link")

	addresses = (
		vmraid.qb.from_(address)
		.inner_join(links)
