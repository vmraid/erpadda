...
from vmraid.utils import flt, get_datetime_str, today
...
class ImportSupplierInvoice(Document):
	def validate(self):
		if not vmraid.db.get_value("Stock Settings", fieldname="stock_uom"):
			vmraid.throw(_("Please set default UOM in Stock Settings"))
	...
	def import_xml_data(self):
		...
		self.default_uom = vmraid.db.get_value("Stock Settings", fieldname="stock_uom")
	...
	def prepare_data_for_import(self, file_content, file_name, encoded_content):
		for line in file_content.find_all("DatiGeneraliDocumento"):
			...
			if not invoices_args.get("bill_no", ""):
				vmraid.throw(_("Numero has not set in the XML file"))
	...
	def process_file_data(self):
		...
		vmraid.enqueue_doc(self.doctype, self.name, "import_xml_data", queue="long", timeout=3600)
...
def get_file_content(file_name, zip_file_object):
	...
	try:
		...
		try:
			...
			vmraid.log_error(message=e, title="UTF-16 encoding error for File Name: " + file_name)
...
def get_payment_terms_from_file(file_content):
	...
	mop_options = vmraid.get_meta("Mode of Payment").fields[4].options
	mop_str = re.sub("\n", ",", mop_options)
...
def create_supplier(supplier_group, args):
	...
	existing_supplier_name = vmraid.db.get_value(
	...
	if existing_supplier_name:
		...
		if not vmraid.get_list("Contact", filters):
		...
		new_supplier = vmraid.new_doc("Supplier")
...
def create_address(supplier_name, args):
	...
	existing_address = vmraid.get_list("Address", filters)

	if args.address_line1:
		new_address_doc = vmraid.new_doc("Address")
		...
		for address in existing_address:
			address_doc = vmraid.get_doc("Address", address["name"])
			if (
...
def create_purchase_invoice(supplier_name, file_name, args, name):
	...
	pi = vmraid.get_doc(
		{
			...
			"currency": erpadda.get_company_currency(args.company),
	...
	try:
		...
		vmraid.db.set_value("Import Supplier Invoice", name, "status", "Error")
		vmraid.log_error(
...
def get_country(code):
	existing_country_name = vmraid.db.get_value("Country", filters={"code": code}, fieldname="name")
	if existing_country_name:
		return existing_country_name
	else:
		vmraid.throw(_("Country Code in File does not match with country code set up in the system"))


def create_uom(uom):
	existing_uom = vmraid.db.get_value("UOM", filters={"uom_name": uom}, fieldname="uom_name")
	if existing_uom:
		return existing_uom
	else:
		new_uom = vmraid.new_doc("UOM")
