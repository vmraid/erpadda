...
class TestGSTR3BReport(unittest.TestCase):
	def setUp(self):
		...
		vmraid.db.sql("delete from `tabSales Invoice` where company='_Test Company GST'")
		vmraid.db.sql("delete from `tabPurchase Invoice` where company='_Test Company GST'")
		vmraid.db.sql("delete from `tabGSTR 3B Report` where company='_Test Company GST'")
	...
	def test_gstr_3b_report(self):
		...
		if vmraid.db.exists("GSTR 3B Report", "GSTR3B-March-2019-_Test Address GST-Billing"):
			report = vmraid.get_doc("GSTR 3B Report", "GSTR3B-March-2019-_Test Address GST-Billing")
			report.save()
		else:
	...
	def test_gst_rounding(self):
		...
		current_country = vmraid.flags.country
		...
		vmraid.flags.country = current_country
		gst_settings.round_off_gst_values = 1
		gst_settings.save()

	def test_gst_category_auto_update(self):
		if not vmraid.db.exists("Customer", "_Test GST Customer With GSTIN"):
		...
		if not vmraid.db.exists("Address", "_Test GST Category-1-Billing"):
...
def make_suppliers():
	if not vmraid.db.exists("Supplier", "_Test Registered Supplier"):
		vmraid.get_doc(
			{
	...
	if not vmraid.db.exists("Supplier", "_Test Unregistered Supplier"):
		vmraid.get_doc(
			{
				"supplier_group": "_Test Supplier Group",
	...
	if not vmraid.db.exists("Address", "_Test Supplier GST-1-Billing"):
		address = vmraid.get_doc(
			{
				"address_line1": "_Test Address Line 1",
	...
	if not vmraid.db.exists("Address", "_Test Supplier GST-2-Billing"):
		address = vmraid.get_doc(
			{
				"address_line1": "_Test Address Line 1",
...
def make_customers():
	if not vmraid.db.exists("Customer", "_Test GST Customer"):
		vmraid.get_doc(
			{
				"customer_group": "_Test Customer Group",
	...
	if not vmraid.db.exists("Customer", "_Test GST SEZ Customer"):
		vmraid.get_doc(
			{
				"customer_group": "_Test Customer Group",
	...
	if not vmraid.db.exists("Customer", "_Test Unregistered Customer"):
		vmraid.get_doc(
			{
				"customer_group": "_Test Customer Group",
	...
	if not vmraid.db.exists("Address", "_Test GST-1-Billing"):
		address = vmraid.get_doc(
			{
				"address_line1": "_Test Address Line 1",
	...
	if not vmraid.db.exists("Address", "_Test GST-2-Billing"):
		address = vmraid.get_doc(
			{
				"address_line1": "_Test Address Line 1",
	...
	if not vmraid.db.exists("Address", "_Test GST-3-Billing"):
		address = vmraid.get_doc(
			{
				"address_line1": "_Test Address Line 1",
...
def make_company():
	if vmraid.db.exists("Company", "_Test Company GST"):
		return
	...
	if not vmraid.db.exists("Address", "_Test Address GST-Billing"):
		address = vmraid.get_doc(
			{
