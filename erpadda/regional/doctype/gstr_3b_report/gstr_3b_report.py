...
from erpadda.regional.india import state_numbers
...
class GSTR3BReport(Document):
	...
	def get_data(self):
		...
		self.json_output = vmraid.as_json(self.report_dict)
	...
	def get_itc_reversal_entries(self):
		reversal_entries = vmraid.db.sql(
			"""
	...
	def get_inward_nil_exempt(self, state):
		inward_nil_exempt = vmraid.db.sql(
			"""
...
def get_address_state_map():
	return vmraid._dict(vmraid.get_all("Address", fields=["name", "gst_state"], as_list=1))
...
def view_report(name):
	json_data = vmraid.get_value("GSTR 3B Report", name, "json_output")
...
def make_json(name):
	...
	vmraid.local.response.filename = file_name
	vmraid.local.response.filecontent = json_data
