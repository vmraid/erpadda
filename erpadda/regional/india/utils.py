...
from vmraid.utils import cint, cstr, date_diff, flt, getdate, nowdate

from erpadda.controllers.accounts_controller import get_taxes_and_charges
from erpadda.controllers.taxes_and_totals import get_itemised_tax, get_itemised_taxable_amount
from erpadda.hr.utils import get_salary_assignment
from erpadda.payroll.doctype.salary_structure.salary_structure import make_salary_slip
from erpadda.regional.india import number_state_mapping, state_numbers, states
...
def validate_gstin_for_india(doc, method):
	...
	if hasattr(doc, "gst_category"):
		if len(doc.links):
			...
			if link_doctype in ["Customer", "Supplier"]:
				gst_category = vmraid.db.get_value(link_doctype, {"name": link_name}, ["gst_category"])
	...
	if len(doc.gstin) != 15:
		vmraid.throw(_("A GSTIN must have 15 characters."), title=_("Invalid GSTIN"))
	...
	if gst_category and gst_category == "UIN Holders":
		...
		if not doc.gst_state:
			vmraid.throw(_("Please enter GST state"), title=_("Invalid State"))
...
def validate_pan_for_india(doc, method):
	...
	if not PAN_NUMBER_FORMAT.match(doc.pan):
		vmraid.throw(_("Invalid PAN No. The input you've entered doesn't match the format of PAN."))


def validate_tax_category(doc, method):
	if doc.get("gst_state") and vmraid.db.get_value(
		"Tax Category",
		{
...
def update_gst_category(doc, method):
	for link in doc.links:
		if link.link_doctype in ["Customer", "Supplier"]:
			meta = vmraid.get_meta(link.link_doctype)
...
def get_itemised_tax_breakup_header(item_doctype, tax_accounts):
	hsn_wise_in_gst_settings = vmraid.db.get_single_value("GST Settings", "hsn_wise_tax_breakup")
	if vmraid.get_meta(item_doctype).has_field("gst_hsn_code") and hsn_wise_in_gst_settings:
...
def get_itemised_tax_breakup_data(doc, account_wise=False, hsn_wise=False):
	...
	if not vmraid.get_meta(doc.doctype + " Item").has_field("gst_hsn_code"):
	...
	for item, taxes in itemised_tax.items():
		...
		hsn_tax.setdefault(item_or_hsn, vmraid._dict())
	...
	# set taxable amount
	hsn_taxable_amount = vmraid._dict()
...
def validate_document_name(doc, method=None):
	...
	country = vmraid.get_cached_value("Company", doc.company, "country")
...
def get_place_of_supply(party_details, doctype):
	if not vmraid.get_meta("Address").has_field("gst_state"):
		return
...
def get_regional_address_details(party_details, doctype, company):
	if isinstance(party_details, str):
		...
		party_details = vmraid._dict(party_details)
...
def get_tax_template_based_on_category(master_doctype, company, party_details):
	...
	default_tax = vmraid.db.get_value(
...
def calculate_hra_exemption(
	...
	exemptions = []
	frequency = vmraid.get_value("Salary Structure", salary_structure, "payroll_frequency")
...
def validate_house_rent_dates(doc):
	if not doc.rented_to_date or not doc.rented_from_date:
		vmraid.throw(_("House rented dates required for exemption calculation"))

	if date_diff(doc.rented_to_date, doc.rented_from_date) < 14:
		vmraid.throw(_("House rented dates should be atleast 15 days apart"))

	proofs = vmraid.db.sql(
		"""
		select name
	...
	if proofs:
		vmraid.throw(_("House rent paid days overlapping with {0}").format(proofs[0][0]))
...
def get_ewb_data(dt, dn):
	...
	ewaybills = []
	for doc_name in dn:
		doc = vmraid.get_doc(dt, doc_name)
		...
		if dt == "Delivery Note":
			...
			vmraid.throw(_("Unsupported GST Category for E-Way Bill JSON generation"))

		data.docType = "INV"
		data.docDate = vmraid.utils.formatdate(doc.posting_date, "dd/mm/yyyy")

		company_address = vmraid.get_doc("Address", doc.company_address)
		billing_address = vmraid.get_doc("Address", doc.customer_address)

		# added dispatch address
		dispatch_address = (
			vmraid.get_doc("Address", doc.dispatch_address_name)
			if doc.dispatch_address_name
			else company_address
		)
		shipping_address = vmraid.get_doc("Address", doc.shipping_address_name)
		...
		disable_rounded = vmraid.db.get_single_value("Global Defaults", "disable_rounded_total")
...
def download_ewb_json():
	data = json.loads(vmraid.local.form_dict.data)
	vmraid.local.response.filecontent = json.dumps(data, indent=4, sort_keys=True)
	vmraid.local.response.type = "download"

	filename_prefix = "Bulk"
	docname = vmraid.local.form_dict.docname
	if docname:
	...
	vmraid.local.response.filename = "{0}_e-WayBill_Data_{1}.json".format(
		filename_prefix, vmraid.utils.random_string(5)
	)
...
def get_item_list(data, doc, hsn_wise=False):
	...
	for item_or_hsn, taxable_amount in hsn_taxable_amount.items():
		item_data = vmraid._dict()
		if not item_or_hsn:
			vmraid.throw(_("GST HSN Code does not exist for one or more items"))
...
def validate_doc(doc):
	if doc.docstatus != 1:
		vmraid.throw(_("E-Way Bill JSON can only be generated from submitted document"))

	if doc.is_return:
		vmraid.throw(_("E-Way Bill JSON cannot be generated for Sales Return as of now"))

	if doc.ewaybill:
		vmraid.throw(_("e-Way Bill already exists for this document"))
	...
	if len(doc.company_gstin) < 15:
		vmraid.throw(_("You must be a registered supplier to generate e-Way Bill"))


def get_transport_details(data, doc):
	if doc.distance > 4000:
		vmraid.throw(_("Distance cannot be greater than 4000 kms"))
	...
	if doc.mode_of_transport == "Road":
		...
		if not doc.gst_vehicle_type:
			vmraid.throw(_("Vehicle Type is required if Mode of Transport is Road"))
		else:
			data.vehicleType = vehicle_types.get(doc.gst_vehicle_type)
	else:
		if not doc.lr_no or not doc.lr_date:
			vmraid.throw(_("Transport Receipt No and Date are mandatory for your chosen Mode of Transport"))

	if doc.lr_no:
		data.transDocNo = doc.lr_no

	if doc.lr_date:
		data.transDocDate = vmraid.utils.formatdate(doc.lr_date, "dd/mm/yyyy")
...
def validate_pincode(pincode, address):
	...
	if not pincode:
		vmraid.throw(_(pin_not_found.format(address)))

	pincode = pincode.replace(" ", "")
	if not pincode.isdigit() or len(pincode) != 6:
		vmraid.throw(_(incorrect_pin.format(address)))
	else:
		return int(pincode)
...
def validate_state_code(state_code, address):
	...
	if not state_code:
		vmraid.throw(_(no_state_code.format(address)))
	else:
...
def get_gst_accounts(
	...
	gst_accounts = vmraid._dict()
	gst_settings_accounts = vmraid.get_all(
		"GST Account",
		filters=filters,
		fields=["cgst_account", "sgst_account", "igst_account", "cess_account", "utgst_account"],
	)

	if not gst_settings_accounts and not vmraid.flags.in_test and not vmraid.flags.in_migrate:
		vmraid.throw(_("Please set GST Accounts in GST Settings"))
	...
	if doc.reverse_charge == "Y":
		...
		if base_gst_tax != base_reverse_charge_booked:
			...
			msg += _(
				...
				gst_document_link='<a href="https://docs.erpadda.com/docs/user/manual/en/regional/india/gst-setup">GST Documentation</a>'
			)
...
def get_regional_round_off_accounts(company, account_list):
	country = vmraid.get_cached_value("Company", company, "country")
	...
	if not vmraid.db.get_single_value("GST Settings", "round_off_gst_values"):
		return
...
def get_depreciation_amount(asset, depreciable_value, row):
	if row.depreciation_method in ("Straight Line", "Manual"):
		...
		if depreciable_value == asset.gross_purchase_amount:
			if row.finance_book and vmraid.db.get_value("Finance Book", row.finance_book, "for_income_tax"):
...
def set_item_tax_from_hsn_code(item):
	if not item.taxes and item.gst_hsn_code:
		hsn_doc = vmraid.get_doc("GST HSN Code", item.gst_hsn_code)
