erpadda.setup_einvoice_actions = (doctype) => {
	vmraid.ui.form.on(doctype, {
		async refresh(frm) {
			if (frm.doc.docstatus == 2) return;

			const res = await vmraid.call({
				method: 'erpadda.regional.india.e_invoice.utils.validate_eligibility',
				args: { doc: frm.doc }
			});
			...
			if (!irn && !__unsaved) {
				const action = () => {
					if (frm.doc.__unsaved) {
						vmraid.throw(__('Please save the document to generate IRN.'));
					}
					vmraid.call({
						method: 'erpadda.regional.india.e_invoice.utils.get_einvoice',
				...
				const action = () => {
					const d = new vmraid.ui.Dialog({
						...
						primary_action: function() {
							...
							vmraid.call({
								method: 'erpadda.regional.india.e_invoice.utils.cancel_irn',
								args: {
									doctype,
									docname: name,
									irn: irn,
				...
				const action = () => {
					const d = new vmraid.ui.Dialog({
						...
						primary_action: function() {
							const data = d.get_values();
							vmraid.call({
								method: 'erpadda.regional.india.e_invoice.utils.generate_eway_bill',
								args: {
									doctype,
									docname: name,
									irn,
					...
					d.fields_dict.transporter.df.onchange = function () {
						...
						if (transporter) {
							vmraid.db.get_value('Supplier', transporter, ['gst_transporter_id', 'supplier_name'])
								.then(({ message }) => {
					...
					d.fields_dict.driver.df.onchange = function () {
						...
						if (driver) {
							vmraid.db.get_value('Driver', driver, ['full_name'])
								.then(({ message }) => {
			...
			if (irn && ewaybill && !irn_cancelled && !eway_bill_cancelled) {
				const action = () => {
					...
					message += '<br><br>';
					message += __('You must first use the portal to cancel the e-way bill and then update the cancelled status in the ERPAdda system.');

					const dialog = vmraid.msgprint({
						...
						primary_action: {
							action: function() {
								vmraid.call({
									method: 'erpadda.regional.india.e_invoice.utils.cancel_eway_bill',
									args: { doctype, docname: name },
									freeze: true,
...
const request_irn_generation = (frm) => {
	vmraid.call({
		method: 'erpadda.regional.india.e_invoice.utils.generate_irn',
...
const show_einvoice_preview = (frm, einvoice) => {
	...
	einvoice["Irn"] = einvoice["AckNo"] = ''; einvoice["AckDt"] = vmraid.datetime.nowdate();
	...
	vmraid.call({
		...
		callback: function (r) {
			if (!r.exc) {
				...
				vmraid.dom.set_style(style, "custom-print-style");
				preview_dialog.show();
			}
		}
	});
