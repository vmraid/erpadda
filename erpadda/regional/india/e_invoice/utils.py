...
from vmraid.integrations.utils import make_get_request, make_post_request
...
from erpadda.regional.india.utils import get_gst_accounts, get_place_of_supply
...
def validate_eligibility(doc):
	...
	einvoicing_enabled = cint(vmraid.db.get_single_value("E Invoice Settings", "enable"))
	if not einvoicing_enabled:
		return False

	einvoicing_eligible_from = (
		vmraid.db.get_single_value("E Invoice Settings", "applicable_from") or "2021-04-01"
	)
	if getdate(doc.get("posting_date")) < getdate(einvoicing_eligible_from):
		return False

	invalid_company = not vmraid.db.get_value("E Invoice User", {"company": doc.get("company")})
...
def validate_einvoice_fields(doc):
	...
	if doc.docstatus == 0 and doc._action == "save":
		if doc.irn:
			vmraid.throw(_("You cannot edit the invoice after generating IRN"), title=_("Edit Not Allowed"))
		if len(doc.name) > 16:
		...
		vmraid.throw(_("You must generate IRN before submitting the document."), title=_("Missing IRN"))
...
def get_party_details(address_name, skip_gstin_validation=False):
	addr = vmraid.get_doc("Address", address_name)
	...
	party_address_details = vmraid._dict(
		dict(
...
def get_overseas_address_details(address_name):
	address_title, address_line1, address_line2, city = vmraid.db.get_value(
...
def get_invoice_value_details(invoice):
	invoice_value_details = vmraid._dict(dict())
...
def get_return_doc_reference(invoice):
	invoice_date = vmraid.db.get_value("Sales Invoice", invoice.return_against, "posting_date")
	return vmraid._dict(
...
def validate_mandatory_fields(invoice):
	...
	if not vmraid.db.get_value("Address", invoice.company_address, "gstin"):
		vmraid.throw(
			_(
	...
	if invoice.gst_category != "Overseas" and not vmraid.db.get_value(
...
def make_einvoice(invoice):
	...
	shipping_details = payment_details = prev_doc_details = eway_bill_details = vmraid._dict({})
	...
	# not yet implemented
	period_details = export_details = vmraid._dict({})
...
def log_error(data=None):
	...
	return vmraid.log_error(title=_("E Invoice Request Failed"), message=message)
...
class GSPConnector:
	...
	def set_invoice(self):
		...
		if self.doctype and self.docname:
			self.invoice = vmraid.get_cached_doc(self.doctype, self.docname)

	def set_credentials(self):
		self.e_invoice_settings = vmraid.get_cached_doc("E Invoice Settings")
	...
	def get_seller_gstin(self):
		gstin = vmraid.db.get_value("Address", self.invoice.company_address, "gstin")
		if not gstin:
			vmraid.throw(
	...
	def get_client_credentials(self):
		...
		return vmraid.conf.einvoice_client_id, vmraid.conf.einvoice_client_secret
	...
	def get_gstin_details(gstin):
		...
		if not hasattr(vmraid.local, "gstin_cache"):
			vmraid.local.gstin_cache = {}

		key = gstin
		gsp_connector = GSPConnector()
		details = gsp_connector.fetch_gstin_details(gstin)

		vmraid.local.gstin_cache[key] = details
		vmraid.cache().hset("gstin_cache", key, details)
		return details
			...
			if res.get("success"):
				...
				if irn_details:
					...
					raise RequestFailed(
						...
						Contact ERPAdda support to resolve the issue."
					)
	...
	def raise_error(self, raise_exception=False, errors=None):
		...
		if errors:
			vmraid.throw(errors, title=title, as_list=1)
		else:
...
def get_einvoice(doctype, docname):
	invoice = vmraid.get_doc(doctype, docname)
...
def cancel_eway_bill(doctype, docname):
	...
	vmraid.db.set_value(doctype, docname, "ewaybill", "")
	vmraid.db.set_value(doctype, docname, "eway_bill_cancelled", 1)
...
def schedule_bulk_generate_irn(docnames):
	...
	vmraid.publish_realtime(
		...
		{"user": vmraid.session.user, "failures": failures, "invoices": docnames},
	)
...
def show_bulk_action_failure_message(failures):
	for doc in failures:
		...
		vmraid.msgprint(message, title=_("Bulk E-Invoice Generation Complete"), indicator="red")
...
def enqueue_bulk_action(job, **kwargs):
	...
	vmraid.msgprint(msg, alert=1)


def check_scheduler_status():
	if is_scheduler_inactive() and not vmraid.flags.in_test:
		vmraid.throw(_("Scheduler is inactive. Cannot enqueue job."), title=_("Scheduler Inactive"))
