...
from erpadda.accounts.utils import get_fiscal_year, FiscalYearError
from vmraid.utils import today


def setup(company=None, patch=True):
	# Company independent fixtures should be called only once at the first company setup
	if patch or vmraid.db.count("Company", {"country": "India"}) <= 1:
...
def setup_company_independent_fixtures(patch=False):
	...
	vmraid.enqueue("erpadda.regional.india.setup.add_hsn_sac_codes", now=vmraid.flags.in_test)
...
def add_hsn_sac_codes():
	if vmraid.flags.in_test and vmraid.flags.created_hsn_codes:
		return
	...
	if vmraid.flags.in_test:
		vmraid.flags.created_hsn_codes = True


def create_hsn_codes(data, code_field):
	for d in data:
		hsn_code = vmraid.new_doc("GST HSN Code")
...
def add_print_formats():
	vmraid.reload_doc("regional", "print_format", "gst_tax_invoice")
	vmraid.reload_doc("selling", "print_format", "gst_pos_invoice")
	vmraid.reload_doc("accounts", "print_format", "GST E-Invoice")

	vmraid.db.set_value("Print Format", "GST POS Invoice", "disabled", 0)
	vmraid.db.set_value("Print Format", "GST Tax Invoice", "disabled", 0)
	vmraid.db.set_value("Print Format", "GST E-Invoice", "disabled", 0)


def make_property_setters(patch=False):
	# GST rules do not allow for an invoice no. bigger than 16 characters
	journal_entry_types = vmraid.get_meta("Journal Entry").get_options("voucher_type").split("\n") + [
		"Reversal Of ITC"
	]
	sales_invoice_series = ["SINV-.YY.-", "SRET-.YY.-", ""] + vmraid.get_meta(
		"Sales Invoice"
	).get_options("naming_series").split("\n")
	purchase_invoice_series = ["PINV-.YY.-", "PRET-.YY.-", ""] + vmraid.get_meta(
		"Purchase Invoice"
	).get_options("naming_series").split("\n")
...
def make_fixtures(company=None):
	docs = []
	company = company or vmraid.db.get_value("Global Defaults", None, "default_company")
...
def update_regional_tax_settings(country, company):
	...
	gst_settings = vmraid.get_single("GST Settings")
...
def set_tax_withholding_category(company):
	...
	abbr = vmraid.get_value("Company", company, "abbr")
	tds_account = vmraid.get_value("Account", "TDS Payable - {0}".format(abbr), "name")
	...
	for d in docs:
		if not vmraid.db.exists("Tax Withholding Category", d.get("name")):
			doc = vmraid.get_doc(d)
			...
			doc = vmraid.get_doc("Tax Withholding Category", d.get("name"), for_update=True)
...
def create_gratuity_rule():
	...
	if not vmraid.db.exists("Gratuity Rule", "Indian Standard Gratuity Rule"):
...
def update_accounts_settings_for_taxes():
	if vmraid.db.count("Company") == 1:
		vmraid.db.set_value("Accounts Settings", None, "add_taxes_from_item_tax_template", 0)
