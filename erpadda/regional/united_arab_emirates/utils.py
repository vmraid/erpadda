...
from vmraid.utils import flt, money_in_words, round_based_on_smallest_currency_fraction
...
def update_itemised_tax_data(doc):
	...
	for row in doc.items:
		...
		if row.item_tax_rate:
			item_tax_rate = vmraid.parse_json(row.item_tax_rate)
		...
		meta = vmraid.get_meta(row.doctype)
...
def get_tax_accounts(company):
	...
	tax_accounts_dict = vmraid._dict()
	tax_accounts_list = vmraid.get_all(
		"UAE VAT Account", filters={"parent": company}, fields=["Account"]
	)

	if not tax_accounts_list and not vmraid.flags.in_test:
		vmraid.throw(_('Please set Vat Accounts for Company: "{0}" in UAE VAT Settings').format(company))
...
def update_totals(vat_tax, base_vat_tax, doc):
	...
	doc.base_in_words = money_in_words(
		doc.base_grand_total, erpadda.get_company_currency(doc.company)
	)
