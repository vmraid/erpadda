...
from erpadda.payroll.doctype.gratuity_rule.gratuity_rule import get_gratuity_rule
...
def add_print_formats():
	vmraid.reload_doc("regional", "print_format", "detailed_tax_invoice")
	vmraid.reload_doc("regional", "print_format", "simplified_tax_invoice")
	vmraid.reload_doc("regional", "print_format", "tax_invoice")
...
def add_custom_roles_for_reports():
	...
	if not vmraid.db.get_value("Custom Role", dict(report="UAE VAT 201")):
		vmraid.get_doc(
			dict(
				doctype="Custom Role",
				report="UAE VAT 201",
...
def create_gratuity_rule():
	...
	if not vmraid.db.exists("Gratuity Rule", "Rule Under Limited Contract (UAE)"):
	...
	if not vmraid.db.exists("Gratuity Rule", "Rule Under Unlimited Contract on termination (UAE)"):
	...
	if not vmraid.db.exists("Gratuity Rule", "Rule Under Unlimited Contract on resignation (UAE)"):
