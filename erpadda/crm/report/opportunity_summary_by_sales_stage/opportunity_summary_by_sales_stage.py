...
class OpportunitySummaryBySalesStage(object):
	...
	def set_sales_stage_columns(self):
		self.sales_stage_list = vmraid.db.get_list("Sales Stage", pluck="name")
	...
	def get_data_query(self, based_on, data_based_on):
		if self.filters.get("data_based_on") == "Number":
			...
			self.query_result = vmraid.db.get_list(
				"Opportunity",
	...
	def get_formatted_data(self):
		self.formatted_data = vmraid._dict()
	...
	def set_formatted_data_based_on_sales_stage(self, based_on, sales_stage, count):
		self.formatted_data.setdefault(based_on, vmraid._dict()).setdefault(sales_stage, 0)
	...
	def get_default_currency(self):
		...
		return vmraid.db.get_value("Company", company, "default_currency")
