...
from erpadda.crm.report.sales_pipeline_analytics.sales_pipeline_analytics import execute


class TestSalesPipelineAnalytics(unittest.TestCase):
	@classmethod
	def setUpClass(self):
		vmraid.db.delete("Opportunity")
		create_company()
		create_customer()
		create_opportunity()
...
def create_company():
	doc = vmraid.db.exists("Company", "Best Test")
	if not doc:
		doc = vmraid.new_doc("Company")
...
def create_customer():
	doc = vmraid.db.exists("Customer", "_Test NC")
	if not doc:
		doc = vmraid.new_doc("Customer")
		doc.customer_name = "_Test NC"
		doc.insert()


def create_opportunity():
	doc = vmraid.db.exists({"doctype": "Opportunity", "party_name": "_Test NC"})
	if not doc:
		doc = vmraid.new_doc("Opportunity")
		doc.opportunity_from = "Customer"
		customer_name = vmraid.db.get_value("Customer", {"customer_name": "_Test NC"}, ["customer_name"])
