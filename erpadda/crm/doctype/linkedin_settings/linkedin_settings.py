...
class LinkedInSettings(Document):
		...
		params = urlencode(
			{
				...
				"redirect_uri": "{0}/api/method/erpadda.crm.doctype.linkedin_settings.linkedin_settings.callback?".format(
					vmraid.utils.get_url()
				),
	...
	def get_access_token(self, code):
		...
		response = vmraid.parse_json(response.content.decode())
	...
	def get_member_profile(self):
		...
		vmraid.local.response["location"] = get_url_to_form("LinkedIn Settings", "LinkedIn Settings")
	...
	def post(self, text, title, media=None):
		if not media:
			...
			if media_id:
				...
				vmraid.log_error("Failed to upload media.", "LinkedIn Upload Error")
	...
	def api_error(self, response):
		content = vmraid.parse_json(response.content.decode())

		if response.status_code == 401:
			self.db_set("session_status", "Expired")
			vmraid.db.commit()
			vmraid.throw(content["message"], title=_("LinkedIn Error - Unauthorized"))
		elif response.status_code == 403:
			vmraid.msgprint(_("You didn't have permission to access this API"))
			vmraid.throw(content["message"], title=_("LinkedIn Error - Access Denied"))
		else:
			vmraid.throw(response.reason, title=response.status_code)


@vmraid.whitelist(allow_guest=True)
def callback(code=None, error=None, error_description=None):
	if not error:
		linkedin_settings = vmraid.get_doc("LinkedIn Settings")
