...
class Contract(Document):
	def autoname(self):
		...
		if vmraid.db.exists("Contract", name):
			count = len(vmraid.get_all("Contract", filters={"name": ["like", "%{}%".format(name)]}))
	...
	def before_submit(self):
		self.signed_by_company = vmraid.session.user
...
def update_status_for_contracts():
	...
	for contract in contracts:
		...
		vmraid.db.set_value("Contract", contract.get("name"), "status", status)
