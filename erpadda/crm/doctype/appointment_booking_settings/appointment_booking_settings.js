vmraid.ui.form.on('Appointment Booking Settings', 'validate',check_times);
function check_times(frm) {
	$.each(frm.doc.availability_of_slots || [], function (i, d) {
		...
		if (from_time > to_time) {
			vmraid.throw(__('In row {0} of Appointment Booking Slots: "To Time" must be later than "From Time".', [i + 1]));
		}
	});
}
