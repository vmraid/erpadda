...
class TwitterSettings(Document):
	...
	def get_authorize_url(self):
		callback_url = (
			"{0}/api/method/erpadda.crm.doctype.twitter_settings.twitter_settings.callback?".format(
		...
		try:
			...
			vmraid.msgprint(_("Error! Failed to get request token."))
			vmraid.throw(
				_("Invalid {0} or {1}").format(vmraid.bold("Consumer Key"), vmraid.bold("Consumer Secret Key"))
			)
	...
	def get_access_token(self, oauth_token, oauth_verifier):
		...
		try:
			...
			vmraid.local.response["location"] = get_url_to_form("Twitter Settings", "Twitter Settings")
		except TweepError as e:
			vmraid.msgprint(_("Error! Failed to get access token."))
			vmraid.throw(_("Invalid Consumer Key or Consumer Secret Key"))
...
def callback(oauth_token=None, oauth_verifier=None):
	if oauth_token and oauth_verifier:
		twitter_settings = vmraid.get_single("Twitter Settings")
