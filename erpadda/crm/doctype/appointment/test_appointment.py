# Copyright (c) 2019, VMRaid and Contributors
# See license.txt

import datetime
import unittest

import vmraid


def create_test_lead():
	test_lead = vmraid.db.get_value("Lead", {"email_id": "test@example.com"})
	if test_lead:
		return vmraid.get_doc("Lead", test_lead)
	test_lead = vmraid.get_doc(
		{"doctype": "Lead", "lead_name": "Test Lead", "email_id": "test@example.com"}
	)
	test_lead.insert(ignore_permissions=True)
	return test_lead


def create_test_appointments():
	test_appointment = vmraid.get_doc(
		{
			"doctype": "Appointment",
			"email": "test@example.com",
			"status": "Open",
			"customer_name": "Test Lead",
			"customer_phone_number": "666",
			"customer_skype": "test",
			"customer_email": "test@example.com",
			"scheduled_time": datetime.datetime.now(),
		}
	)
	test_appointment.insert()
	return test_appointment


class TestAppointment(unittest.TestCase):
	test_appointment = test_lead = None

	def setUp(self):
		self.test_lead = create_test_lead()
		self.test_appointment = create_test_appointments()

	def test_calendar_event_created(self):
		cal_event = vmraid.get_doc("Event", self.test_appointment.calendar_event)
		self.assertEqual(cal_event.starts_on, self.test_appointment.scheduled_time)

	def test_lead_linked(self):
		lead = vmraid.get_doc("Lead", self.test_lead.name)
		self.assertIsNotNone(lead)
