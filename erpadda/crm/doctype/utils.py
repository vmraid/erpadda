...
def get_last_interaction(contact=None, lead=None):
	...
	if contact:
		...
		if query_condition:
			...
			last_communication = vmraid.db.sql(
				"""
				SELECT `name`, `content`
	...
	if lead:
		last_communication = vmraid.get_all(
			"Communication",
...
def get_scheduled_employees_for_popup(communication_medium):
	...
	now_time = vmraid.utils.nowtime()
	weekday = vmraid.utils.get_weekday()

	available_employee_groups = vmraid.get_all(
