...
class Opportunity(TransactionBase):
	...
	def validate(self):
		self._prev = vmraid._dict(
			{
				"contact_date": vmraid.db.get_value("Opportunity", self.name, "contact_date")
				if (not cint(self.get("__islocal")))
				else None,
				"contact_by": vmraid.db.get_value("Opportunity", self.name, "contact_by")
				if (not cint(self.get("__islocal")))
				else None,
			}
		)
	...
	def map_fields(self):
		for field in self.meta.fields:
			if not self.get(field.fieldname):
				try:
					value = vmraid.db.get_value(self.opportunity_from, self.party_name, field.fieldname)
					vmraid.db.set(self, field.fieldname, value)
				except Exception:
					continue
	...
	def make_new_lead_if_required(self):
		...
		if (not self.get("party_name")) and self.contact_email:
			...
			lead_name = vmraid.db.get_value("Lead", {"email_id": self.contact_email})
			if not lead_name:
	...
	def declare_enquiry_lost(self, lost_reasons_list, competitors, detailed_reason=None):
		if not self.has_active_quotation():
			...
			vmraid.throw(_("Cannot declare as lost, because Quotation has been made."))
	...
	def add_calendar_event(self, opts=None, force=False):
		if vmraid.db.get_single_value("CRM Settings", "create_event_on_next_contact_date_opportunity"):
			if not opts:
				opts = vmraid._dict()
	...
	def validate_item_details(self):
		...
		for d in self.items:
			...
			item = vmraid.db.get_value("Item", d.item_code, item_fields, as_dict=True)
...
def make_quotation(source_name, target_doc=None):
	def set_missing_values(source, target):
		from erpadda.controllers.accounts_controller import get_default_taxes_and_charges

		quotation = vmraid.get_doc(target)

		company_currency = vmraid.get_cached_value("Company", quotation.company, "default_currency")
...
def set_multiple_status(names, status):
	...
	for name in names:
		opp = vmraid.get_doc("Opportunity", name)
		opp.status = status
		opp.save()
...
def auto_close_opportunity():
	...
	auto_close_after_days = (
		vmraid.db.get_single_value("CRM Settings", "close_opportunity_after_days") or 15
	)

	opportunities = vmraid.db.sql(
	...
	for opportunity in opportunities:
		doc = vmraid.get_doc("Opportunity", opportunity.get("name"))
		doc.status = "Closed"
...
def make_opportunity_from_communication(communication, company, ignore_communication_links=False):
	from erpadda.crm.doctype.lead.lead import make_lead_from_communication
