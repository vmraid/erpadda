...
import vmraid
from vmraid.utils import now_datetime, random_string, today

from erpadda.crm.doctype.lead.lead import make_customer
from erpadda.crm.doctype.lead.test_lead import make_lead
from erpadda.crm.doctype.opportunity.opportunity import make_quotation
from erpadda.crm.utils import get_linked_communication_list

test_records = vmraid.get_test_records("Opportunity")
...
class TestOpportunity(unittest.TestCase):
	def test_opportunity_status(self):
		...
		doc = vmraid.get_doc("Opportunity", doc.name)
	...
	def test_make_new_lead_if_required(self):
		...
		self.assertEqual(
			vmraid.db.get_value("Lead", opp_doc.party_name, "email_id"), opp_doc.contact_email
		)
	...
	def test_carry_forward_of_email_and_comments(self):
		...
		opportunity_comment_count = vmraid.db.count(
		...
		quotation_comment_count = vmraid.db.count(
			"Comment",
			{
...
def make_opportunity_from_lead():
	...
	opp_doc = vmraid.get_doc(args).insert(ignore_permissions=True)
