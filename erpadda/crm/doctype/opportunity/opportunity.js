...
vmraid.ui.form.on("Opportunity", {
	...
	validate: function(frm) {
		if (frm.doc.status == "Lost" && !frm.doc.lost_reasons.length) {
			...
			vmraid.throw(__("Lost Reasons are required in case opportunity is Lost."));
		}
	},
...
vmraid.ui.form.on("Opportunity Item", {
	calculate: function(frm, cdt, cdn) {
		let row = vmraid.get_doc(cdt, cdn);
		vmraid.model.set_value(cdt, cdn, "amount", flt(row.qty) * flt(row.rate));
		vmraid.model.set_value(cdt, cdn, "base_rate", flt(frm.doc.conversion_rate) * flt(row.rate));
		vmraid.model.set_value(cdt, cdn, "base_amount", flt(frm.doc.conversion_rate) * flt(row.amount));
...
erpadda.crm.Opportunity = class Opportunity extends vmraid.ui.form.Controller {
	...
	make_customer() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.crm.doctype.opportunity.opportunity.make_customer",
			frm: cur_frm
		})
	}
};
