...
def get_children(parent=None, company=None, exclude_node=None):
	...
	for employee in employees:
		is_expandable = vmraid.db.count("Employee", filters={"reports_to": employee.get("id")})
...
def get_connections(employee):
	...
	nodes_to_expand = vmraid.get_list("Employee", filters=[["reports_to", "=", employee]])
	num_connections += len(nodes_to_expand)

	while nodes_to_expand:
		parent = nodes_to_expand.pop(0)
		descendants = vmraid.get_list("Employee", filters=[["reports_to", "=", parent.name]])
