...
from erpadda.hr.doctype.staffing_plan.staffing_plan import (
...
class JobOpening(WebsiteGenerator):
	...
	def validate(self):
		if not self.route:
			self.route = vmraid.scrub(self.job_title).replace("_", "-")
	...
	def validate_current_vacancies(self):
		if not self.staffing_plan:
			...
			planned_vacancies = vmraid.db.sql(
				"""
		...
		if self.staffing_plan and self.planned_vacancies:
			staffing_plan_company = vmraid.db.get_value("Staffing Plan", self.staffing_plan, "company")
			lft, rgt = vmraid.get_cached_value("Company", staffing_plan_company, ["lft", "rgt"])
