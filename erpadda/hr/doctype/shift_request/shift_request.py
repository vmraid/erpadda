...
from erpadda.hr.utils import share_doc_with_approver, validate_active_employee


class OverlappingShiftRequestError(vmraid.ValidationError):
	pass
...
class ShiftRequest(Document):
		...
		if self.status == "Approved":
			...
			vmraid.msgprint(
				_("Shift Assignment: {0} created for Employee: {1}").format(
					vmraid.bold(assignment_doc.name), vmraid.bold(self.employee)
				)
			)

	def on_cancel(self):
		shift_assignment_list = vmraid.get_list(
	...
	def validate_default_shift(self):
		...
		if self.shift_type == default_shift:
			vmraid.throw(
				_("You can not request for your Default Shift: {0}").format(vmraid.bold(self.shift_type))
			)
	...
	def get_overlapping_dates(self):
		...
		shift = vmraid.qb.DocType("Shift Request")
		query = (
	...
	def throw_overlap_error(self, shift_details):
		...
		vmraid.throw(msg, title=_("Overlapping Shift Requests"), exc=OverlappingShiftRequestError)
