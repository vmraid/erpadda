...
from erpadda.hr.doctype.shift_request.shift_request import OverlappingShiftRequestError
from erpadda.hr.doctype.shift_type.test_shift_type import setup_shift_type

test_dependencies = ["Shift Type"]


class TestShiftRequest(VMRaidTestCase):
	def setUp(self):
	...
	def test_make_shift_request(self):
		...
		department = vmraid.get_value("Employee", "_T-Employee-00001", "department")
		set_shift_approver(department)
		approver = vmraid.db.sql(
		...
		shift_assignment = vmraid.db.get_value(
			"Shift Assignment",
		...
		shift_assignment_docstatus = vmraid.db.get_value(
	...
	def test_shift_request_approver_perms(self):
		setup_shift_type(shift_type="Day Shift")
		employee = vmraid.get_doc("Employee", "_T-Employee-00001")
		...
		self.assertTrue(shift_request.name in vmraid.share.get_shared("Shift Request", user))

		# check shared doc revoked
		shift_request.reload()
		department = vmraid.get_value("Employee", "_T-Employee-00001", "department")
		set_shift_approver(department)
		department_approver = vmraid.db.sql(
			"""select approver from `tabDepartment Approver` where parent= %s and parentfield = 'shift_request_approver'""",
			(department),
		)[0][0]
		shift_request.approver = department_approver
		shift_request.save()
		self.assertTrue(shift_request.name not in vmraid.share.get_shared("Shift Request", user))
