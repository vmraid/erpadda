...
vmraid.ui.form.on('Interview', {
	onload: function (frm) {
		frm.events.set_job_applicant_query(frm);

		frm.set_query('interviewer', 'interview_details', function () {
			return {
				query: 'erpadda.hr.doctype.interview.interview.get_interviewer_list'
			};
		});
	},
	...
	refresh: function (frm) {
		if (frm.doc.docstatus != 2 && !frm.doc.__islocal) {
			...
			if ((allowed_interviewers.includes(vmraid.session.user))) {
				vmraid.db.get_value('Interview Feedback', {'interviewer': vmraid.session.user, 'interview': frm.doc.name, 'docstatus': 1}, 'name', (r) => {
	...
	show_feedback_dialog: function (frm, data) {
		...
		let d = new vmraid.ui.Dialog({
			...
			primary_action: function(values) {
				vmraid.call({
					method: 'erpadda.hr.doctype.interview.interview.create_interview_feedback',
					args: {
						data: values,
						interview_name: frm.doc.name,
						interviewer: vmraid.session.user,
	...
	interview_round: async function (frm) {
		...
		let round_data = (await vmraid.db.get_value('Interview Round', frm.doc.interview_round, 'designation')).message;
	...
	set_interview_details: function (frm) {
		vmraid.call({
			method: 'erpadda.hr.doctype.interview.interview.get_interviewers',
			args: {
				interview_round: frm.doc.interview_round
			},
	...
	job_applicant: function (frm) {
		if (!frm.doc.interview_round) {
			...
			frm.refresh();
			vmraid.throw(__('Select Interview Round First'));
		}
	...
	set_designation_and_job_opening: async function (frm) {
		...
		let job_applicant_data = (await vmraid.db.get_value(
