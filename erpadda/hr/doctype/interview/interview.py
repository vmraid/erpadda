...
from vmraid.utils import cstr, flt, get_datetime, get_link_to_form


class DuplicateInterviewRoundError(vmraid.ValidationError):
	pass
...
class Interview(Document):
	...
	def validate_duplicate_interview(self):
		duplicate_interview = vmraid.db.exists(
			"Interview",
		...
		if duplicate_interview:
			vmraid.throw(
				_(
					...
					vmraid.bold(get_link_to_form("Interview", duplicate_interview)),
					vmraid.bold(self.job_applicant),
				)
			)

	def validate_designation(self):
		applicant_designation = vmraid.db.get_value("Job Applicant", self.job_applicant, "designation")
		if self.designation:
			if self.designation != applicant_designation:
				vmraid.throw(
					_(
						...
						self.interview_round, vmraid.bold(self.designation), applicant_designation
					),
	...
	def validate_overlap(self):
		...
		if overlaps:
			...
			vmraid.throw(overlapping_details, title=_("Overlap"))
	...
	def reschedule_interview(self, scheduled_on, from_time, to_time):
		...
		vmraid.msgprint(_("Interview Rescheduled successfully"), indicator="green")


def get_recipients(name, for_feedback=0):
	interview = vmraid.get_doc("Interview", name)
	...
	if for_feedback:
		...
		recipients.append(vmraid.db.get_value("Job Applicant", interview.job_applicant, "email_id"))
...
def send_interview_reminder():
	reminder_settings = vmraid.db.get_value(
		"HR Settings",
		"HR Settings",
	...
	remind_before = cstr(vmraid.db.get_single_value("HR Settings", "remind_before")) or "01:00:00"
	...
	interview_template = vmraid.get_doc(
		"Email Template", reminder_settings.interview_reminder_template
	)

	for d in interviews:
		doc = vmraid.get_doc("Interview", d.name)
		context = doc.as_dict()
		message = vmraid.render_template(interview_template.response, context)
...
def send_daily_feedback_reminder():
	...
	interview_feedback_template = vmraid.get_doc(
	...
	for entry in interviews:
		...
		doc = vmraid.get_doc("Interview", entry.name)
		context = doc.as_dict()

		message = vmraid.render_template(interview_feedback_template.response, context)
...
def get_expected_skill_set(interview_round):
	return vmraid.get_all("Expected Skill Set", filters={"parent": interview_round}, fields=["skill"])
...
def create_interview_feedback(data, interview_name, interviewer, job_applicant):
	...
	if vmraid.session.user != interviewer:
		vmraid.throw(_("Only Interviewer Are allowed to submit Interview Feedback"))

	interview_feedback = vmraid.new_doc("Interview Feedback")
