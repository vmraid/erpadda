...
from vmraid.utils import add_days, getdate, nowtime

from erpadda.hr.doctype.designation.test_designation import create_designation
from erpadda.hr.doctype.interview.interview import DuplicateInterviewRoundError
from erpadda.hr.doctype.job_applicant.job_applicant import get_interview_details
...
class TestInterview(unittest.TestCase):
	...
	def test_notification_for_scheduling(self):
		from erpadda.hr.doctype.interview.interview import send_interview_reminder
	...
	def test_notification_for_feedback_submission(self):
		from erpadda.hr.doctype.interview.interview import send_daily_feedback_reminder
...
def create_interview_round(name, skill_set, interviewers=[], designation=None, save=True):
	...
	interview_round = vmraid.new_doc("Interview Round")
...
def create_skill_set(skill_set):
	for skill in skill_set:
		if not vmraid.db.exists("Skill", skill):
			doc = vmraid.new_doc("Skill")
			doc.skill_name = skill
			doc.save()


def create_interview_type(name="test_interview_type"):
	if vmraid.db.exists("Interview Type", name):
		return vmraid.get_doc("Interview Type", name).name
	else:
		doc = vmraid.new_doc("Interview Type")
		doc.name = name
