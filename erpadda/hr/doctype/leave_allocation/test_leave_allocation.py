...
from vmraid.utils import add_days, add_months, getdate, nowdate

import erpadda
from erpadda.hr.doctype.employee.test_employee import make_employee
from erpadda.hr.doctype.leave_allocation.leave_allocation import (
	BackDatedAllocationError,
	OverAllocationError,
)
from erpadda.hr.doctype.leave_ledger_entry.leave_ledger_entry import process_expired_allocation
from erpadda.hr.doctype.leave_type.test_leave_type import create_leave_type


class TestLeaveAllocation(VMRaidTestCase):
	def setUp(self):
		vmraid.db.delete("Leave Period")
		vmraid.db.delete("Leave Allocation")

		emp_id = make_employee("test_emp_leave_allocation@salary.com", company="_Test Company")
		self.employee = vmraid.get_doc("Employee", emp_id)
	...
	def test_overlapping_allocation(self):
		...
		vmraid.get_doc(leaves[0]).save()
		self.assertRaises(vmraid.ValidationError, vmraid.get_doc(leaves[1]).save)
	...
	def test_validation_for_over_allocation_based_on_leave_setup(self):
		vmraid.delete_doc_if_exists("Leave Period", "Test Allocation Period")
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		self.assertFalse(
			vmraid.db.exists("Leave Ledger Entry", {"transaction_name": leave_allocation.name})
		)
	...
	def test_validation_against_leave_application_after_submit(self):
		...
		self.assertRaises(vmraid.ValidationError, leave_allocation.submit)
...
def create_leave_allocation(**args):
	...
	employee = vmraid.get_doc("Employee", emp_id)
