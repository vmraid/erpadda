...
from vmraid.utils import add_days, date_diff, flt, formatdate, getdate

from erpadda.hr.doctype.leave_application.leave_application import get_approved_leaves_for_period
from erpadda.hr.doctype.leave_ledger_entry.leave_ledger_entry import (
	create_leave_ledger_entry,
	expire_allocation,
)
from erpadda.hr.utils import get_leave_period, set_employee_name
...
class BackDatedAllocationError(vmraid.ValidationError):
	pass


class OverAllocationError(vmraid.ValidationError):
	pass


class LessAllocationError(vmraid.ValidationError):
	pass


class ValueMultiplierError(vmraid.ValidationError):
	pass
...
class LeaveAllocation(Document):
	...
	def validate_leave_allocation_days(self):
		...
		max_leaves_allowed = flt(
			vmraid.db.get_value("Leave Type", self.leave_type, "max_leaves_allowed")
		)
	...
	def update_leave_policy_assignments_when_no_allocations_left(self):
		allocations = vmraid.db.get_list(
			"Leave Allocation",
	...
	def set_total_leaves_allocated(self):
		...
		if (
			...
			and not vmraid.db.get_value("Leave Type", self.leave_type, "is_earned_leave")
			and not vmraid.db.get_value("Leave Type", self.leave_type, "is_compensatory")
		):
			vmraid.throw(
	...
	def limit_carry_forward_based_on_max_allowed_leaves(self):
		max_leaves_allowed = vmraid.db.get_value("Leave Type", self.leave_type, "max_leaves_allowed")
	...
	def validate_total_leaves_allocated(self):
		...
		if date_difference < self.total_leaves_allocated:
			vmraid.throw(_("Total allocated leaves are more than days in the period"), OverAllocationError)

	def create_leave_ledger_entry(self, submit=True):
		if self.unused_leaves:
			expiry_days = vmraid.db.get_value(
...
def get_leave_allocation_for_period(
	...
	Allocation = vmraid.qb.DocType("Leave Allocation")
	return (
		vmraid.qb.from_(Allocation)
...
def get_carry_forwarded_leaves(employee, leave_type, date, carry_forward=None):
	...
	if carry_forward and previous_allocation:
		...
		if unused_leaves:
			max_carry_forwarded_leaves = vmraid.db.get_value(
...
def validate_carry_forward(leave_type):
	if not vmraid.db.get_value("Leave Type", leave_type, "is_carry_forward"):
		vmraid.throw(_("Leave Type {0} cannot be carry-forwarded").format(leave_type))
