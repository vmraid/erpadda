...
vmraid.ui.form.on("Leave Allocation", {
	onload: function(frm) {
		// Ignore cancellation of doctype on cancel all.
		frm.ignore_doctypes_on_cancel_all = ["Leave Ledger Entry"];

		if (!frm.doc.from_date) frm.set_value("from_date", vmraid.datetime.get_today());
	...
	refresh: function(frm) {
		if (frm.doc.docstatus === 1 && frm.doc.expired) {
			var valid_expiry = moment(vmraid.datetime.get_today()).isBetween(frm.doc.from_date, frm.doc.to_date);
			if (valid_expiry) {
		...
		if (frm.doc.leave_policy_assignment) {
			vmraid.db.get_value("Leave Type", frm.doc.leave_type, "is_earned_leave", (r) => {
	...
	expire_allocation: function(frm) {
		vmraid.call({
			method: 'erpadda.hr.doctype.leave_ledger_entry.leave_ledger_entry.expire_allocation',
			args: {
				'allocation': frm.doc,
				'expiry_date': vmraid.datetime.get_today()
			},
			freeze: true,
			callback: function(r) {
				if (!r.exc) {
					vmraid.msgprint(__("Allocation Expired!"));
				}
				frm.refresh();
			}
		});
	...
	leave_policy: function(frm) {
		if (frm.doc.leave_policy && frm.doc.leave_type) {
			vmraid.db.get_value("Leave Policy Detail", {
