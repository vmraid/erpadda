...
from vmraid.utils import cstr, get_datetime, get_link_to_form, get_time, getdate, now_datetime
...
class OverlappingShiftError(vmraid.ValidationError):
	pass
...
class ShiftAssignment(Document):
	...
	def get_overlapping_dates(self):
		...
		shift = vmraid.qb.DocType("Shift Assignment")
		query = (
			vmraid.qb.from_(shift)
	...
	def throw_overlap_error(self, shift_details):
		shift_details = vmraid._dict(shift_details)
		if shift_details.docstatus == 1 and shift_details.status == "Active":
			msg = _(
				"Employee {0} already has an active Shift {1}: {2} that overlaps within this period."
			).format(
				vmraid.bold(self.employee),
				vmraid.bold(shift_details.shift_type),
				get_link_to_form("Shift Assignment", shift_details.name),
			)
			vmraid.throw(msg, title=_("Overlapping Shifts"), exc=OverlappingShiftError)
...
def has_overlapping_timings(shift_1: str, shift_2: str) -> bool:
	...
	"""
	curr_shift = vmraid.db.get_value("Shift Type", shift_1, ["start_time", "end_time"], as_dict=True)
	overlapping_shift = vmraid.db.get_value(
...
def get_events(start, end, filters=None):
	...
	employee = vmraid.db.get_value(
		"Employee", {"user_id": vmraid.session.user}, ["name", "company"], as_dict=True
	)
	if employee:
...
def add_assignments(events, start, end, conditions=None):
	...
	records = vmraid.db.sql(query, {"start_date": start, "end_date": end}, as_dict=True)
	...
	for d in records:
		...
		while daily_event_start <= daily_event_end:
			start_timing = (
				vmraid.utils.get_datetime(daily_event_start) + shift_timing_map[d.shift_type]["start_time"]
			)
			end_timing = (
				vmraid.utils.get_datetime(daily_event_start) + shift_timing_map[d.shift_type]["end_time"]
			)
...
def get_shifts_for_date(employee: str, for_timestamp: datetime) -> List[Dict[str, str]]:
	...
	assignment = vmraid.qb.DocType("Shift Assignment")

	return (
		vmraid.qb.from_(assignment)
...
def get_employee_shift(
	...
	default_shift = vmraid.db.get_value("Employee", employee, "default_shift")
...
def is_holiday_date(employee: str, shift_details: Dict) -> bool:
	holiday_list_name = vmraid.db.get_value(
...
def get_shift_details(shift_type_name: str, for_timestamp: datetime = None) -> Dict:
	...
	shift_type = vmraid.get_doc("Shift Type", shift_type_name)
