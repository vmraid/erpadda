...
from vmraid.utils import add_days, cstr, date_diff, getdate
...
from erpadda.hr.utils import get_holiday_dates_for_employee
...
@vmraid.whitelist()
def get_template():
	if not vmraid.has_permission("Attendance", "create"):
		raise vmraid.PermissionError

	args = vmraid.local.form_dict

	if getdate(args.from_date) > getdate(args.to_date):
		vmraid.throw(_("To Date should be greater than From Date"))
	...
	try:
		...
		vmraid.respond_as_web_page("Holiday List Missing", html=e)
		return

	# write out response as a type csv
	vmraid.response["result"] = cstr(w.getvalue())
	vmraid.response["type"] = "csv"
	vmraid.response["doctype"] = "Attendance"


def add_header(w):
	status = ", ".join(
		(vmraid.get_meta("Attendance").get_field("status").options or "").strip().split("\n")
	)
	w.writerow(["Notes:"])
...
def get_naming_series():
	series = vmraid.get_meta("Attendance").get_field("naming_series").options.strip().split("\n")
	if not series:
		vmraid.throw(_("Please setup numbering series for Attendance via Setup > Numbering Series"))
	return series[0]
...
def upload():
	...
	rows = read_csv_content(vmraid.local.uploaded_file)
	if not rows:
		vmraid.throw(_("Please select a csv file"))
	vmraid.enqueue(import_attendances, rows=rows, now=True if len(rows) < 200 else False)
...
def import_attendances(rows):
	...
	from vmraid.utils.csvutils import check_record, import_doc

	for i, row in enumerate(rows):
		if not row:
			continue
		row_idx = i + 5
		d = vmraid._dict(zip(columns, row))

		d["doctype"] = "Attendance"
		if d.name:
			d["docstatus"] = vmraid.db.get_value("Attendance", d.name, "docstatus")

		try:
			check_record(d)
			ret.append(import_doc(d, "Attendance", 1, row_idx, submit=True))
			vmraid.publish_realtime("import_attendance", dict(progress=i, total=len(rows)))
	...
	vmraid.publish_realtime("import_attendance", dict(messages=ret, error=error))
