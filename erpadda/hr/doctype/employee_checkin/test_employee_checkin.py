...
from erpadda.hr.doctype.employee_checkin.employee_checkin import (
...
from erpadda.hr.doctype.shift_type.test_shift_type import make_shift_assignment, setup_shift_type
from erpadda.payroll.doctype.salary_slip.test_salary_slip import make_holiday_list


class TestEmployeeCheckin(VMRaidTestCase):
	def setUp(self):
	...
	def test_mark_attendance_and_link_log(self):
		...
		now_date = nowdate()
		vmraid.db.delete("Attendance", {"employee": employee})
		...
		attendance_count = vmraid.db.count(
			"Attendance",
	...
	def test_calculate_working_hours(self):
		...
		logs_type_1 = [vmraid._dict(x) for x in logs_type_1]
		logs_type_2 = [vmraid._dict(x) for x in logs_type_2]
	...
	def test_fetch_shift_based_on_default_shift(self):
		...
		date = getdate()
		vmraid.db.set_value("Employee", employee, "default_shift", default_shift.name)
