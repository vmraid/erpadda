...
from vmraid.utils import cint, get_datetime
...
class EmployeeCheckin(Document):
	...
	def validate_duplicate_log(self):
		...
		if doc:
			doc_link = vmraid.get_desk_link("Employee Checkin", doc)
			vmraid.throw(
...
def add_log_based_on_employee_field(
	...
	if not employee_field_value or not timestamp:
		vmraid.throw(_("'employee_field_value' and 'timestamp' are required."))

	employee = vmraid.db.get_values(
		"Employee",
	...
	doc = vmraid.new_doc("Employee Checkin")
...
def mark_attendance_and_link_log(
	...
	if attendance_status == "Skip":
		...
		vmraid.throw(_("{} is an invalid Attendance Status.").format(attendance_status))
