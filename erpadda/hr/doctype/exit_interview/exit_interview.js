...
vmraid.ui.form.on('Exit Interview', {
	refresh: function(frm) {
		if (!frm.doc.__islocal && !frm.doc.questionnaire_email_sent && vmraid.boot.user.can_write.includes('Exit Interview')) {
	...
	employee: function(frm) {
		vmraid.db.get_value('Employee', frm.doc.employee, 'relieving_date', (message) => {
	...
	send_exit_questionnaire: function(frm) {
		vmraid.call({
			method: 'erpadda.hr.doctype.exit_interview.exit_interview.send_exit_questionnaire',
			args: {
				'interviews': [frm.doc]
			},
			callback: function(r) {
