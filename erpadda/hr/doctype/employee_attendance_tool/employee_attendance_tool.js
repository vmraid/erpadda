vmraid.ui.form.on("Employee Attendance Tool", {
	refresh: function(frm) {
		frm.disable_save();
	},

	onload: function(frm) {
		frm.set_value("date", vmraid.datetime.get_today());
		erpadda.employee_attendance_tool.load_employees(frm);
	},
...
erpadda.employee_attendance_tool = {
	load_employees: function(frm) {
		if(frm.doc.date) {
			vmraid.call({
				method: "erpadda.hr.doctype.employee_attendance_tool.employee_attendance_tool.get_employees",
				args: {
					date: frm.doc.date,
				...
				callback: function(r) {
					if(r.message['unmarked'].length > 0) {
						...
						frm.EmployeeSelector = new erpadda.EmployeeSelector(frm, frm.employee_area, r.message['unmarked'])
					}
					else{
					...
					if(r.message['marked'].length > 0) {
						...
						frm.marked_employee = new erpadda.MarkedEmployee(frm, frm.marked_employee_area, r.message['marked'])
					}
					else{
...
erpadda.MarkedEmployee = class MarkedEmployee {
...
erpadda.EmployeeSelector = class EmployeeSelector {
	constructor(frm, wrapper, employee) {
		this.wrapper = wrapper;
		this.frm = frm;
		this.make(frm, employee);
	...
	make(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-mark-present")
			...
			.on("click", function() {
				...
				vmraid.call({
					method: "erpadda.hr.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_attendance",
					args:{
