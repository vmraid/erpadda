...
vmraid.ui.form.on('Appraisal', {
	...
	kra_template: function(frm) {
		...
		erpadda.utils.map_current_doc({
			method: "erpadda.hr.doctype.appraisal.appraisal.fetch_appraisal_template",
...
vmraid.ui.form.on('Appraisal Goal', {
	score: function(frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		if (flt(d.score) > 5) {
			vmraid.msgprint(__("Score must be less than or equal to 5"));
			d.score = 0;
