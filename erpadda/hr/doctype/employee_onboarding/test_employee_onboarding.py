...
from erpadda.hr.doctype.employee_onboarding.employee_onboarding import (
	IncompleteTaskError,
	make_employee,
)
...
def get_job_applicant():
	if vmraid.db.exists("Job Applicant", "test@researcher.com"):
		return vmraid.get_doc("Job Applicant", "test@researcher.com")
...
def get_job_offer(applicant_name):
	job_offer = vmraid.db.exists("Job Offer", {"job_applicant": applicant_name})
	if job_offer:
		return vmraid.get_doc("Job Offer", job_offer)
...
def create_employee_onboarding():
	...
	holiday_list = vmraid.get_doc("Holiday List", holiday_list)
