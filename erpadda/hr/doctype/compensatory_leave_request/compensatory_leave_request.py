...
from vmraid.utils import add_days, cint, date_diff, format_date, getdate
...
class CompensatoryLeaveRequest(Document):
	def validate(self):
		...
		if self.half_day:
			if not self.half_day_date:
				vmraid.throw(_("Half Day Date is mandatory"))
			if (
				not getdate(self.work_from_date) <= getdate(self.half_day_date) <= getdate(self.work_end_date)
			):
				vmraid.throw(_("Half Day Date should be in between Work From Date and Work End Date"))
		validate_overlap(self, self.work_from_date, self.work_end_date)
		self.validate_holidays()
		self.validate_attendance()
		if not self.leave_type:
			vmraid.throw(_("Leave Type is madatory"))
	...
	def validate_attendance(self):
		...
		if len(attendance) < date_diff(self.work_end_date, self.work_from_date) + 1:
			vmraid.throw(_("You are not present all day(s) between compensatory leave request days"))
		...
		if len(holidays) < date_diff(self.work_end_date, self.work_from_date) + 1:
			if date_diff(self.work_end_date, self.work_from_date):
				msg = _("The days between {0} to {1} are not valid holidays.").format(
					vmraid.bold(format_date(self.work_from_date)), vmraid.bold(format_date(self.work_end_date))
				)
			else:
				msg = _("{0} is not a holiday.").format(vmraid.bold(format_date(self.work_from_date)))
	...
	def on_cancel(self):
		if self.leave_allocation:
			...
			if self.half_day:
				date_difference -= 0.5
			leave_allocation = vmraid.get_doc("Leave Allocation", self.leave_allocation)
			if leave_allocation:
				leave_allocation.new_leaves_allocated -= date_difference
	...
	def get_existing_allocation_for_period(self, leave_period):
		...
		if leave_allocation:
			return vmraid.get_doc("Leave Allocation", leave_allocation[0].name)
		else:
			return False

	def create_leave_allocation(self, leave_period, date_difference):
		is_carry_forward = vmraid.db.get_value("Leave Type", self.leave_type, "is_carry_forward")
