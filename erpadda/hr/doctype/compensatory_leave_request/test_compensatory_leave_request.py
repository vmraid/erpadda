...
import vmraid
from vmraid.utils import add_days, add_months, today

from erpadda.hr.doctype.attendance_request.test_attendance_request import get_employee
from erpadda.hr.doctype.leave_application.leave_application import get_leave_balance_on
...
class TestCompensatoryLeaveRequest(unittest.TestCase):
	def setUp(self):
		vmraid.db.sql(""" delete from `tabCompensatory Leave Request`""")
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_ledger_entry = vmraid.get_all("Leave Ledger Entry", fields="*", filters=filters)
...
def get_compensatory_leave_request(employee, leave_date=today()):
	prev_comp_leave_req = vmraid.db.get_value(
	...
	if prev_comp_leave_req:
		return vmraid.get_doc("Compensatory Leave Request", prev_comp_leave_req)
...
def create_holiday_list():
	if vmraid.db.exists("Holiday List", "_Test Compensatory Leave"):
		return
