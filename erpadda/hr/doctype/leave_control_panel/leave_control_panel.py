...
from vmraid.utils import cint, comma_and, cstr, flt
...
class LeaveControlPanel(Document):
	...
	def validate_values(self):
		for f in ["from_date", "to_date", "leave_type", "no_of_days"]:
			if not self.get(f):
				vmraid.throw(_("{0} is required").format(self.meta.get_label(f)))
	...
	def allocate_leave(self):
		...
		if not employees:
			vmraid.throw(_("No employee found"))

		for d in self.get_employees():
			try:
				la = vmraid.new_doc("Leave Allocation")
				la.set("__islocal", 1)
				la.employee = cstr(d[0])
				la.employee_name = vmraid.db.get_value("Employee", cstr(d[0]), "employee_name")
