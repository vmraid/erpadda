...
vmraid.ui.form.on("Job Applicant", {
		...
		if (!frm.doc.__islocal) {
			if (frm.doc.__onload && frm.doc.__onload.job_offer) {
				...
				frm.add_custom_button(__("Job Offer"), function() {
					vmraid.set_route("Form", "Job Offer", frm.doc.__onload.job_offer);
				}, __("View"));
			} else {
	...
	make_dashboard: function(frm) {
		vmraid.call({
			method: "erpadda.hr.doctype.job_applicant.job_applicant.get_interview_details",
			args: {
				job_applicant: frm.doc.name
			},
			callback: function(r) {
				if (r.message) {
					...
					frm.dashboard.add_section(
						vmraid.render_template("job_applicant_dashboard", {
	...
	create_interview: function (frm, values) {
		vmraid.call({
			method: "erpadda.hr.doctype.job_applicant.job_applicant.create_interview",
			args: {
				doc: frm.doc,
