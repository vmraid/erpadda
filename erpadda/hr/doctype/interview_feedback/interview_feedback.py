...
from vmraid.utils import flt, get_link_to_form, getdate
...
class InterviewFeedback(Document):
		...
		if self.interviewer not in applicable_interviewers:
			vmraid.throw(
				_("{0} is not allowed to submit Interview Feedback for the Interview: {1}").format(
					vmraid.bold(self.interviewer), vmraid.bold(self.interview)
				)
			)

	def validate_interview_date(self):
		scheduled_date = vmraid.db.get_value("Interview", self.interview, "scheduled_on")

		if getdate() < getdate(scheduled_date) and self.docstatus == 1:
			vmraid.throw(
				_("{0} submission before {1} is not allowed").format(
					vmraid.bold("Interview Feedback"), vmraid.bold("Interview Scheduled Date")
				)
			)

	def validate_duplicate(self):
		duplicate_feedback = vmraid.db.exists(
			"Interview Feedback",
	...
	def update_interview_details(self):
		doc = vmraid.get_doc("Interview", self.interview)
...
def get_applicable_interviewers(interview):
	data = vmraid.get_all("Interview Detail", filters={"parent": interview}, fields=["interviewer"])
