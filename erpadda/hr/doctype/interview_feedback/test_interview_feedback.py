...
import vmraid
from vmraid.utils import add_days, flt, getdate

from erpadda.hr.doctype.interview.test_interview import (
...
class TestInterviewFeedback(unittest.TestCase):
	def test_validation_for_skill_set(self):
		...
		vmraid.set_user(interviewer)

		self.assertRaises(vmraid.ValidationError, interview_feedback.save)
	...
	def test_average_ratings_on_feedback_submission_and_cancellation(self):
		...
		avg_on_interview_detail = vmraid.db.get_value(
			"Interview Detail",
			{
