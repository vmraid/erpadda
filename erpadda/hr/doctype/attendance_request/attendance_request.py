...
from vmraid.utils import add_days, date_diff, getdate

from erpadda.hr.doctype.employee.employee import is_holiday
from erpadda.hr.utils import validate_active_employee, validate_dates
	...
	def validate(self):
		...
		if self.half_day:
			if not getdate(self.from_date) <= getdate(self.half_day_date) <= getdate(self.to_date):
				vmraid.throw(_("Half day date should be in between from date and to date"))

	def on_submit(self):
		self.create_attendance()

	def on_cancel(self):
		attendance_list = vmraid.get_list(
			"Attendance", {"employee": self.employee, "attendance_request": self.name}
		)
		if attendance_list:
			for attendance in attendance_list:
				attendance_obj = vmraid.get_doc("Attendance", attendance["name"])
	...
	def create_attendance(self):
		...
		for number in range(request_days):
			...
			if not skip_attendance:
				attendance = vmraid.new_doc("Attendance")
