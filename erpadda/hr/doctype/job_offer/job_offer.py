...
class JobOffer(Document):
	def onload(self):
		employee = vmraid.db.get_value("Employee", {"job_applicant": self.job_applicant}, "name") or ""
		self.set_onload("employee", employee)

	def validate(self):
		self.validate_vacancies()
		job_offer = vmraid.db.exists(
		...
		if job_offer and job_offer != self.name:
			vmraid.throw(
				_("Job Offer: {0} is already for Job Applicant: {1}").format(
					vmraid.bold(job_offer), vmraid.bold(self.job_applicant)
				)
			)

	def validate_vacancies(self):
		staffing_plan = get_staffing_plan_detail(self.designation, self.company, self.offer_date)
		check_vacancies = vmraid.get_single("HR Settings").check_vacancies
		if staffing_plan and check_vacancies:
			...
			if not staffing_plan.get("vacancies") or cint(staffing_plan.vacancies) - len(job_offers) <= 0:
				error_variable = "for " + vmraid.bold(self.designation)
				if staffing_plan.get("parent"):
					error_variable = vmraid.bold(get_link_to_form("Staffing Plan", staffing_plan.parent))

				vmraid.throw(_("There are no vacancies under staffing plan {0}").format(error_variable))
...
def update_job_applicant(status, job_applicant):
	if status in ("Accepted", "Rejected"):
		vmraid.set_value("Job Applicant", job_applicant, "status", status)


def get_staffing_plan_detail(designation, company, offer_date):
	detail = vmraid.db.sql(
		"""
	...
	return vmraid._dict(detail[0]) if (detail and detail[0].parent) else None


@vmraid.whitelist()
def make_employee(source_name, target_doc=None):
	def set_missing_values(source, target):
		target.personal_email, target.first_name = vmraid.db.get_value(
