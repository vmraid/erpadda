...
class HRSettings(Document):
	...
	def show_freq_change_warning(self, from_date, to_date):
		from_date = vmraid.bold(format_date(from_date))
		to_date = vmraid.bold(format_date(to_date))
		vmraid.msgprint(
			msg=vmraid._(
			...
			primary_action={
				"label": vmraid._("Yes, Proceed"),
				"client_action": "erpadda.proceed_save_with_reminders_frequency_change",
			},
			raise_exception=vmraid.ValidationError,
		)
