...
from vmraid.utils import cint, get_datetime, get_time, getdate
...
from erpadda.hr.doctype.shift_assignment.shift_assignment import (
	get_employee_shift,
	get_shift_details,
)
...
class ShiftType(Document):
	...
	def get_start_and_end_dates(self, employee):
		...
		"""
		date_of_joining, relieving_date, employee_creation = vmraid.db.get_value(
	...
	def get_assigned_employee(self, from_date=None, consider_default_shift=False):
		...
		assigned_employees = vmraid.get_all("Shift Assignment", filters=filters, pluck="employee")

		if consider_default_shift:
			filters = {"default_shift": self.name, "status": ["!=", "Inactive"]}
			default_shift_employees = vmraid.get_all("Employee", filters=filters, pluck="name")
...
def process_auto_attendance_for_all_shifts():
	shift_list = vmraid.get_all("Shift Type", "name", {"enable_auto_attendance": "1"}, as_list=True)
	for shift in shift_list:
		doc = vmraid.get_doc("Shift Type", shift[0])
