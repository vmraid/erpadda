...
from vmraid.utils import add_days, get_time, get_year_ending, get_year_start, getdate, now_datetime
...
class TestShiftType(VMRaidTestCase):
	def setUp(self):
		vmraid.db.delete("Shift Type")
		vmraid.db.delete("Shift Assignment")
		vmraid.db.delete("Employee Checkin")
	...
	def test_mark_attendance(self):
		from erpadda.hr.doctype.employee_checkin.test_employee_checkin import make_checkin
		...
		attendance = vmraid.db.get_value(
	...
	def test_working_hours_threshold_for_absent_and_half_day_2(self):
		...
		attendance = vmraid.db.get_value("Attendance", {"shift": shift_type.name}, "status")
	...
	def test_skip_marking_absent_on_a_holiday(self):
		...
		shift_type.process_auto_attendance()
		attendance = vmraid.db.get_value("Attendance", {"employee": employee}, "status")
...
def setup_shift_type(**args):
	...
	if not vmraid.db.exists("Holiday List", "Employee Checkin Test Holiday List"):
...
def make_shift_assignment(shift_type, employee, start_date, end_date=None):
	shift_assignment = vmraid.get_doc(
		{
