...
class TestFullandFinalStatement(unittest.TestCase):
	...
	def tearDown(self):
		vmraid.db.sql("Delete from `tabFull and Final Statement`")
		vmraid.db.sql("Delete from `tabAsset`")
		vmraid.db.sql("Delete from `tabAsset Movement`")
	...
	def test_check_bootstraped_data_asset_movement_and_jv_creation(self):
		...
		vmraid.db.set_value("Employee", employee, "relieving_date", add_days(today(), 30))
...
def create_full_and_final_statement(employee):
	fnf = vmraid.new_doc("Full and Final Statement")
