...
from vmraid.utils import flt, get_link_to_form, today
...
class FullandFinalStatement(Document):
	...
	def validate_settlement(self, component_type):
		for data in self.get(component_type, []):
			if data.status == "Unsettled":
				vmraid.throw(_("Settle all Payables and Receivables before submission"))

	def validate_asset(self):
		for data in self.assets_allocated:
			if data.status == "Owned":
				vmraid.throw(_("All allocated assets should be returned before submission"))
	...
	def get_assets_movement(self):
		asset_movements = vmraid.get_all(
			"Asset Movement Item",
			filters={"docstatus": 1},
				...
				data.append(
					{
						...
						"date": vmraid.db.get_value("Asset Movement", movement.parent, "transaction_date"),
						"status": "Owned",
					}
				)
		return data
...
def get_account_and_amount(ref_doctype, ref_document):
	...
	if ref_doctype == "Salary Slip":
		salary_details = vmraid.db.get_value(
			"Salary Slip", ref_document, ["payroll_entry", "net_pay"], as_dict=1
		)
		amount = salary_details.net_pay
		payable_account = (
			vmraid.db.get_value("Payroll Entry", salary_details.payroll_entry, "payroll_payable_account")
	...
	if ref_doctype == "Gratuity":
		payable_account, amount = vmraid.db.get_value(
