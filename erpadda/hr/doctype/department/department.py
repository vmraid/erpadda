...
class Department(NestedSet):
	...
	def before_rename(self, old, new, merge=False):
		...
		if not vmraid.get_cached_value("Company", self.company, "abbr") in new:
			new = get_abbreviated_name(new, self.company)

		return new

	def on_update(self):
		if not (vmraid.local.flags.ignore_update_nsm or vmraid.flags.in_setup_wizard):
...
def on_doctype_update():
	vmraid.db.add_index("Department", ["lft", "rgt"])


def get_abbreviated_name(name, company):
	abbr = vmraid.get_cached_value("Company", company, "abbr")
