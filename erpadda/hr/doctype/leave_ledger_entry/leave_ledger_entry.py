...
from vmraid.utils import DATE_FORMAT, flt, getdate, today


class LeaveLedgerEntry(Document):
	def validate(self):
		if getdate(self.from_date) > getdate(self.to_date):
			vmraid.throw(_("To date needs to be before from date"))

	def on_cancel(self):
		# allow cancellation of expiry leaves
		if self.is_expired:
			vmraid.db.set_value("Leave Allocation", self.transaction_name, "expired", 0)
		else:
			vmraid.throw(_("Only expired allocation can be cancelled"))
...
def validate_leave_allocation_against_leave_application(ledger):
	...
	leave_application_records = vmraid.db.sql_list(
		"""
...
def get_previous_expiry_ledger_entry(ledger):
	...
	creation_date = vmraid.db.get_value(
		"Leave Ledger Entry",
		filters={
...
def process_expired_allocation():
	...
	leave_type_records = vmraid.db.get_values(
	...
	expire_allocation = vmraid.db.sql(
		"""
		SELECT
...
def expire_allocation(allocation, expiry_date=None):
	...
	vmraid.db.set_value("Leave Allocation", allocation.name, "expired", 1)


def expire_carried_forward_allocation(allocation):
	"""Expires remaining leaves in the on carried forward allocation"""
	from erpadda.hr.doctype.leave_application.leave_application import get_leaves_for_period
