...
# test_records = vmraid.get_test_records('Daily Work Summary')
...
class TestDailyWorkSummary(unittest.TestCase):
	...
	def test_email_trigger_failed(self):
		hour = "00:00"
		if vmraid.utils.nowtime().split(":")[0] == "00":
			hour = "01:00"
	...
	def test_incoming(self):
		...
		daily_work_summary = vmraid.get_doc(
			"Daily Work Summary", vmraid.get_all("Daily Work Summary")[0].name
		)
	...
	def setup_and_prepare_test(self, hour=None):
		vmraid.db.sql("delete from `tabDaily Work Summary`")
		...
		vmraid.db.sql("delete from `tabDaily Work Summary Group`")

		self.users = vmraid.get_all(
			"User", fields=["email"], filters=dict(email=("!=", "test@example.com"))
		)
		self.setup_groups(hour)

		from erpadda.hr.doctype.daily_work_summary_group.daily_work_summary_group import trigger_emails

		trigger_emails()

		# check if emails are created

		self.emails = vmraid.db.sql(
	...
	def setup_groups(self, hour=None):
		...
		if not hour:
			hour = vmraid.utils.nowtime().split(":")[0]
			hour = hour + ":00"

		groups = vmraid.get_doc(
			dict(
