...
from vmraid.utils import global_date_format
...
class DailyWorkSummary(Document):
	...
	def get_message_details(self):
		...
		dws_group = vmraid.get_doc("Daily Work Summary Group", self.daily_work_summary_group)
		...
		for d in replies:
			...
			try:
				if original_image:
					file_name = vmraid.get_list("File", {"file_url": original_image})
		...
		did_not_reply = [
			(vmraid.db.get_value("User", {"email": email}, "full_name") or email) for email in did_not_reply
		]
...
def get_user_emails_from_group(group):
	...
	if isinstance(group_doc, str):
		group_doc = vmraid.get_doc("Daily Work Summary Group", group)
...
def get_users_email(doc):
	return [d.email for d in doc.users if vmraid.db.get_value("User", d.user, "enabled")]
