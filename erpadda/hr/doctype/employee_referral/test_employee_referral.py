...
from erpadda.hr.doctype.employee_referral.employee_referral import (
...
class TestEmployeeReferral(unittest.TestCase):
	def setUp(self):
		vmraid.db.sql("DELETE FROM `tabJob Applicant`")
		vmraid.db.sql("DELETE FROM `tabEmployee Referral`")
...
def create_employee_referral():
	emp_ref = vmraid.new_doc("Employee Referral")
