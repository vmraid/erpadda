...
vmraid.ui.form.on("Employee Referral", {
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.status === "Accepted") {
			vmraid.db.get_list("Additional Salary", {
				filters: {
	...
	create_job_applicant: function(frm) {
		vmraid.model.open_mapped_doc({
			method: "erpadda.hr.doctype.employee_referral.employee_referral.create_job_applicant",
			frm: frm
		});
	},
	...
	create_additional_salary: function(frm) {
		vmraid.call({
			method: "erpadda.hr.doctype.employee_referral.employee_referral.create_additional_salary",
			args: {
				doc: frm.doc
			},
			callback: function (r) {
