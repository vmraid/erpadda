...
def create_job_applicant(source_name, target_doc=None):
	emp_ref = vmraid.get_doc("Employee Referral", source_name)
	...
	job_applicant = vmraid.new_doc("Job Applicant")
...
def create_additional_salary(doc):
	...
	if not vmraid.db.exists("Additional Salary", {"ref_docname": doc.name}):
		additional_salary = vmraid.new_doc("Additional Salary")
		additional_salary.employee = doc.referrer
		additional_salary.company = vmraid.db.get_value("Employee", doc.referrer, "company")
