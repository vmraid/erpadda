vmraid.listview_settings['Leave Policy Assignment'] = {
	onload: function (list_view) {
		let me = this;
		list_view.page.add_inner_button(__("Bulk Leave Policy Assignment"), function () {
			me.dialog = new vmraid.ui.form.MultiSelectDialog({
				doctype: "Employee",
				target: cur_list,
				setters: {
					employee_name: '',
				...
				action(employees, data) {
					vmraid.call({
						method: 'erpadda.hr.doctype.leave_policy_assignment.leave_policy_assignment.create_assignment_for_multiple_employees',
						async: false,
						args: {
							employees: employees,
							data: data
	...
	set_effective_date: function () {
		if (cur_dialog.fields_dict.assignment_based_on.value === "Leave Period" && cur_dialog.fields_dict.leave_period.value) {
			vmraid.model.with_doc("Leave Period", cur_dialog.fields_dict.leave_period.value, function () {
				let from_date = vmraid.model.get_value("Leave Period", cur_dialog.fields_dict.leave_period.value, "from_date");
				let to_date = vmraid.model.get_value("Leave Period", cur_dialog.fields_dict.leave_period.value, "to_date");
