...
from vmraid.utils import date_diff, flt, formatdate, get_last_day, get_link_to_form, getdate
...
class LeavePolicyAssignment(Document):
	...
	def set_dates(self):
		if self.assignment_based_on == "Leave Period":
			self.effective_from, self.effective_to = vmraid.db.get_value(
				"Leave Period", self.leave_period, ["from_date", "to_date"]
			)
		elif self.assignment_based_on == "Joining Date":
			self.effective_from = vmraid.db.get_value("Employee", self.employee, "date_of_joining")

	def validate_policy_assignment_overlap(self):
		leave_policy_assignments = vmraid.get_all(
	...
	def warn_about_carry_forwarding(self):
		...
		leave_policy = vmraid.get_doc("Leave Policy", self.leave_policy)
		...
		for policy in leave_policy.leave_policy_details:
			...
			if not leave_type.is_carry_forward:
				...
				).format(vmraid.bold(get_link_to_form("Leave Type", leave_type.name)))
				vmraid.msgprint(msg, indicator="orange", alert=True)

	@vmraid.whitelist()
	def grant_leave_alloc_for_employee(self):
		if self.leaves_allocated:
			vmraid.throw(_("Leave already have been assigned for this Leave Policy Assignment"))
		else:
			leave_allocations = {}
	...
	def get_new_leaves(self, leave_type, new_leaves_allocated, leave_type_details, date_of_joining):
		...
		precision = get_field_precision(
			vmraid.get_meta("Leave Allocation").get_field("new_leaves_allocated")
		)
	...
	def get_leaves_for_passed_months(
		...
		from erpadda.hr.utils import get_monthly_earned_leave

		current_date = vmraid.flags.current_date or getdate()
...
def add_current_month_if_applicable(months_passed, date_of_joining, based_on_doj):
	date = getdate(vmraid.flags.current_date) or getdate()
...
def create_assignment_for_multiple_employees(employees, data):
	...
	for employee in employees:
		assignment = vmraid.new_doc("Leave Policy Assignment")
...
def get_leave_type_details():
	leave_type_details = vmraid._dict()
