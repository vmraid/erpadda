...
import vmraid
from vmraid.utils import add_days, add_months, get_first_day, get_last_day, getdate

from erpadda.hr.doctype.leave_application.test_leave_application import (
	get_employee,
	get_leave_period,
)
...
class TestLeavePolicyAssignment(unittest.TestCase):
	...
	def test_grant_leaves(self):
		...
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee.name], vmraid._dict(data)
		)
		self.assertEqual(
			vmraid.db.get_value("Leave Policy Assignment", leave_policy_assignments[0], "leaves_allocated"),
			1,
		)

		leave_allocation = vmraid.get_list(
			"Leave Allocation",
			filters={
		...
		leave_alloc_doc = vmraid.get_doc("Leave Allocation", leave_allocation)
	...
	def test_earned_leave_allocation(self):
		...
		vmraid.flags.current_date = add_days(get_last_day(getdate()), -1)
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee.name], vmraid._dict(data)
		)

		leaves_allocated = vmraid.db.get_value(
			"Leave Allocation",
			{"leave_policy_assignment": leave_policy_assignments[0]},
	...
	def test_earned_leave_alloc_for_passed_months_based_on_leave_period(self):
		...
		vmraid.flags.current_date = get_first_day(getdate())
		data = {
			"assignment_based_on": "Leave Period",
			"leave_policy": leave_policy.name,
			"leave_period": leave_period.name,
	...
	def test_earned_leave_alloc_for_passed_months_on_month_end_based_on_leave_period(self):
		...
		vmraid.flags.current_date = get_last_day(getdate())
	...
	def test_earned_leave_alloc_for_passed_months_with_cf_leaves_based_on_leave_period(self):
		...
		vmraid.flags.current_date = get_last_day(add_months(getdate(), -1))
		data = {
			"assignment_based_on": "Leave Period",
			"leave_policy": leave_policy.name,
			"leave_period": leave_period.name,
		...
		# if the daily job is not completed yet, there is another check present
		# to ensure leave is not already allocated to avoid duplication
		from erpadda.hr.utils import is_earned_leave_already_allocated

		vmraid.flags.current_date = get_last_day(getdate())

		allocation = vmraid.get_doc("Leave Allocation", details.name)
	...
	def test_earned_leave_alloc_for_passed_months_based_on_joining_date(self):
		...
		effective_from = vmraid.db.get_value(
	...
	def tearDown(self):
		vmraid.db.rollback()
		vmraid.db.set_value("Employee", self.employee.name, "date_of_joining", self.original_doj)
		vmraid.flags.current_date = None


def create_earned_leave_type(leave_type, based_on_doj=False):
	vmraid.delete_doc_if_exists("Leave Type", leave_type, force=1)
...
def create_leave_period(name, start_date=None):
	vmraid.delete_doc_if_exists("Leave Period", name, force=1)
	if not start_date:
