...
class TestEmployeeSeparation(unittest.TestCase):
	def test_employee_separation(self):
		...
		project = vmraid.get_doc("Project", separation.project)
	...
	def tearDown(self):
		for entry in vmraid.get_all("Employee Separation"):
			doc = vmraid.get_doc("Employee Separation", entry.name)
			if doc.docstatus == 1:
				doc.cancel()
			doc.delete()
...
def create_employee_separation():
	employee = vmraid.db.get_value("Employee", {"status": "Active", "company": "_Test Company"})
