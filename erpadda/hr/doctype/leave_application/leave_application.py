...
from vmraid.query_builder.functions import Max, Min, Sum
...
from erpadda.buying.doctype.supplier_scorecard.supplier_scorecard import daterange
...
from erpadda.hr.doctype.leave_ledger_entry.leave_ledger_entry import create_leave_ledger_entry
from erpadda.hr.utils import (
...
class LeaveDayBlockedError(vmraid.ValidationError):
	pass
...
class AttendanceAlreadyMarkedError(vmraid.ValidationError):
	pass


class NotAnOptionalHoliday(vmraid.ValidationError):
	pass


class InsufficientLeaveBalanceError(vmraid.ValidationError):
	pass


class LeaveAcrossAllocationsError(vmraid.ValidationError):
	pass
...
class LeaveApplication(Document):
	...
	def validate(self):
		...
		if vmraid.db.get_value("Leave Type", self.leave_type, "is_optional_leave"):
	...
	def on_update(self):
		if self.status == "Open" and self.docstatus < 1:
			...
			if vmraid.db.get_single_value("HR Settings", "send_leave_notification"):
	...
	def validate_applicable_after(self):
		if self.leave_type:
			leave_type = vmraid.get_doc("Leave Type", self.leave_type)
			if leave_type.applicable_after > 0:
				...
				if number_of_days >= 0:
					holidays = 0
					if not vmraid.db.get_value("Leave Type", self.leave_type, "include_holiday"):
	...
	def validate_dates(self):
		if vmraid.db.get_single_value("HR Settings", "restrict_backdated_leave_application"):
			if self.from_date and getdate(self.from_date) < getdate():
				allowed_role = vmraid.db.get_single_value(
				...
				if not allowed_role:
					vmraid.throw(
						_("Backdated Leave Application is restricted. Please set the {} in {}").format(
							vmraid.bold("Role Allowed to Create Backdated Leave Application"),
		...
		if (
			...
			vmraid.throw(_("Half Day Date should be between From Date and To Date"))
	...
	def validate_dates_across_allocation(self):
		if vmraid.db.get_value("Leave Type", self.leave_type, "allow_negative"):
			return
		...
		if not (alloc_on_from_date or alloc_on_to_date):
			vmraid.throw(_("Application period cannot be outside leave allocation period"))
	...
	def get_allocation_based_on_application_dates(self) -> Tuple[Dict, Dict]:
		...
		def _get_leave_allocation_record(date):
			LeaveAllocation = vmraid.qb.DocType("Leave Allocation")
			allocation = (
				vmraid.qb.from_(LeaveAllocation)
	...
	def validate_back_dated_application(self):
		future_allocation = vmraid.db.sql(
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			attendance_name = vmraid.db.exists(
			...
			if date in holiday_dates:
				if attendance_name:
					...
					attendance = vmraid.get_doc("Attendance", attendance_name)
					attendance.flags.ignore_permissions = True
					if attendance.docstatus == 1:
						attendance.cancel()
					vmraid.delete_doc("Attendance", attendance_name, force=1)
				continue
	...
	def cancel_attendance(self):
		if self.docstatus == 2:
			...
			for name in attendance:
				vmraid.db.set_value("Attendance", name, "docstatus", 2)

	def validate_salary_processed_days(self):
		if not vmraid.db.get_value("Leave Type", self.leave_type, "is_lwp"):
			return

		last_processed_pay_slip = vmraid.db.sql(
			"""
	...
	def show_block_day_warning(self):
		...
		if block_dates:
			vmraid.msgprint(_("Warning: Leave application contains following block dates") + ":")
			for d in block_dates:
				vmraid.msgprint(formatdate(d.block_date) + ": " + d.reason)
	...
	def validate_block_days(self):
		...
		if block_dates and self.status == "Approved":
			vmraid.throw(_("You are not authorized to approve leaves on Block Dates"), LeaveDayBlockedError)
	...
	def show_insufficient_balance_message(self, leave_balance_for_consumption: float) -> None:
		...
		if vmraid.db.get_value("Leave Type", self.leave_type, "allow_negative"):
			...
			vmraid.msgprint(msg, title=_("Warning"), indicator="orange")
		else:
			vmraid.throw(
				_("Insufficient leave balance for Leave Type {0}").format(vmraid.bold(self.leave_type)),
	...
	def get_total_leaves_on_half_day(self):
		leave_count_on_half_day_date = vmraid.db.sql(
	...
	def validate_max_days(self):
		max_days = vmraid.db.get_value("Leave Type", self.leave_type, "max_continuous_days_allowed")
		if max_days and self.total_leave_days > cint(max_days):
			vmraid.throw(_("Leave of type {0} cannot be longer than {1}").format(self.leave_type, max_days))
	...
	def validate_optional_leave(self):
		...
		if not leave_period:
			vmraid.throw(_("Cannot find active Leave Period"))
		optional_holiday_list = vmraid.db.get_value(
	...
	def notify_employee(self):
		...
		parent_doc = vmraid.get_doc("Leave Application", self.name)
		args = parent_doc.as_dict()

		template = vmraid.db.get_single_value("HR Settings", "leave_status_notification_template")
		if not template:
			vmraid.msgprint(_("Please set default template for Leave Status Notification in HR Settings."))
			return
		email_template = vmraid.get_doc("Email Template", template)
		message = vmraid.render_template(email_template.response, args)
	...
	def notify_leave_approver(self):
		if self.leave_approver:
			...
			template = vmraid.db.get_single_value("HR Settings", "leave_approval_notification_template")
			if not template:
				vmraid.msgprint(
		...
		if cint(self.follow_via_email):
			...
			if not isinstance(contact, list):
				if not args.notify == "employee":
					contact = vmraid.get_doc("User", contact).email or contact

			sender = dict()
			sender["email"] = vmraid.get_doc("User", vmraid.session.user).email
			...
			try:
				...
				vmraid.msgprint(_("Email sent to {0}").format(contact))
	...
	def create_leave_ledger_entry(self, submit=True):
		...
		lwp = vmraid.db.get_value("Leave Type", self.leave_type, "is_lwp")
		...
		if expiry_date:
			...
			if self.is_separate_ledger_entry_required(alloc_on_from_date, alloc_on_to_date):
				...
				raise_exception = False if vmraid.flags.in_patch else True
				args = dict(
...
def get_number_of_leave_days(
	...
	if not vmraid.db.get_value("Leave Type", leave_type, "include_holiday"):
...
def get_leave_details(employee, date):
	...
	for d in allocation_records:
		allocation = allocation_records.get(d, vmraid._dict())
	...
	lwp = vmraid.get_list("Leave Type", filters={"is_lwp": 1}, pluck="name")
...
def get_leave_balance_on(
	...
	allocation = allocation_records.get(leave_type, vmraid._dict())
...
def get_leave_allocation_records(employee, date, leave_type=None):
	...
	Ledger = vmraid.qb.DocType("Leave Ledger Entry")

	cf_leave_case = (
		vmraid.qb.terms.Case().when(Ledger.is_carry_forward == "1", Ledger.leaves).else_(0)
	)
	sum_cf_leaves = Sum(cf_leave_case).as_("cf_leaves")

	new_leaves_case = (
		vmraid.qb.terms.Case().when(Ledger.is_carry_forward == "0", Ledger.leaves).else_(0)
	)
	sum_new_leaves = Sum(new_leaves_case).as_("new_leaves")

	query = (
		vmraid.qb.from_(Ledger)
		.select(
			sum_cf_leaves,
			sum_new_leaves,
...
def get_remaining_leaves(
	...
	return vmraid._dict(leave_balance=leave_balance, leave_balance_for_consumption=remaining_leaves)
	...
	for leave_entry in leave_entries:
		...
		if inclusive_period and leave_entry.transaction_type == "Leave Encashment":
			...
			if leave_entry.leaves % 1:
				half_day = 1
				half_day_date = vmraid.db.get_value(
...
def is_lwp(leave_type):
	lwp = vmraid.db.sql("select is_lwp from `tabLeave Type` where name = %s", leave_type)
...
def get_events(start, end, filters=None):
	...
	employee = vmraid.db.get_value(
		"Employee", filters={"user_id": vmraid.session.user}, fieldname=["name", "company"], as_dict=True
	)
	...
	if "Employee" in vmraid.get_roles():
...
def add_department_leaves(events, start, end, employee, company):
	...
	# department leaves
	department_employees = vmraid.db.sql_list(
...
def add_leaves(events, start, end, filter_conditions=None):
	...
	if not cint(
		vmraid.db.get_value("HR Settings", None, "show_leaves_of_all_department_members_in_calendar")
	):
	...
	for d in vmraid.db.sql(query, {"start": start, "end": end}, as_dict=True):
		e = {
			"name": d.name,
...
def get_mandatory_approval(doctype):
	...
	if doctype == "Leave Application":
		mandatory = vmraid.db.get_single_value(
...
def get_approved_leaves_for_period(employee, leave_type, from_date, to_date):
	...
	leave_applications = vmraid.db.sql(
		query,
		{"from_date": from_date, "to_date": to_date, "employee": employee, "leave_type": leave_type},
		as_dict=1,
	)
...
def get_leave_approver(employee):
	leave_approver, department = vmraid.db.get_value(
		"Employee", employee, ["leave_approver", "department"]
	)

	if not leave_approver and department:
		leave_approver = vmraid.db.get_value(
			"Department Approver",
