...
import vmraid
from vmraid.permissions import clear_user_permissions_for_doctype
...
from erpadda.hr.doctype.holiday_list.test_holiday_list import set_holiday_list
from erpadda.hr.doctype.leave_allocation.test_leave_allocation import create_leave_allocation
from erpadda.hr.doctype.leave_application.leave_application import (
...
from erpadda.payroll.doctype.salary_slip.test_salary_slip import (
	make_holiday_list,
...
class TestLeaveApplication(unittest.TestCase):
	def setUp(self):
		...
		vmraid.db.delete("Attendance", {"employee": "_T-Employee-00001"})
		vmraid.db.set_value("Employee", "_T-Employee-00001", "holiday_list", "")

		from_date = get_year_start(getdate())
		to_date = get_year_ending(getdate())
		self.holiday_list = make_holiday_list(from_date=from_date, to_date=to_date)

		if not vmraid.db.exists("Leave Type", "_Test Leave Type"):
			vmraid.get_doc(
	...
	def get_application(self, doc):
		application = vmraid.copy_doc(doc)
	...
	def test_validate_application_across_allocations(self):
		...
		vmraid.delete_doc_if_exists("Leave Type", "Test Leave Validation", force=1)
		leave_type = vmraid.get_doc(
		...
		self.assertRaises(vmraid.ValidationError, leave_application.insert)
	...
	def test_insufficient_leave_balance_validation(self):
		...
		vmraid.db.set_value("Leave Type", "Test Leave Validation", "allow_negative", True)
		make_leave_application(
	...
	def test_attendance_for_include_holidays(self):
		...
		vmraid.delete_doc_if_exists("Leave Type", "Test Include Holidays", force=1)
		leave_type = vmraid.get_doc(
		...
		self.assertEqual(vmraid.db.count("Attendance", {"leave_application": leave_application.name}), 4)
	...
	def test_attendance_update_for_exclude_holidays(self):
		...
		vmraid.delete_doc_if_exists("Leave Type", "Test Do Not Include Holidays", force=1)
		leave_type = vmraid.get_doc(
			dict(
		...
		attendance_on_holiday = vmraid.get_doc(config)
		...
		attendance = vmraid.get_doc(config)
		...
		self.assertEqual(vmraid.db.count("Attendance", {"leave_application": leave_application.name}), 3)

		# attendance on holiday deleted
		self.assertFalse(vmraid.db.exists("Attendance", attendance_on_holiday.name))

		# attendance on non-holiday updated
		self.assertEqual(vmraid.db.get_value("Attendance", attendance.name, "status"), "On Leave")
	...
	def test_optional_leave(self):
		...
		if not vmraid.db.exists("Holiday List", holiday_list):
			vmraid.get_doc(
				dict(
		...
		vmraid.db.set_value("Leave Period", leave_period.name, "optional_holiday_list", holiday_list)
		leave_type = "Test Optional Type"
		if not vmraid.db.exists("Leave Type", leave_type):
			vmraid.get_doc(
	...
	def test_leaves_allowed(self):
		employee = get_employee()
		leave_period = get_leave_period()
		vmraid.delete_doc_if_exists("Leave Type", "Test Leave Type", force=1)
		leave_type = vmraid.get_doc(
	...
	def test_applicable_after(self):
		...
		date = add_days(nowdate(), -7)
		vmraid.db.set_value("Employee", employee.name, "date_of_joining", date)
		...
		vmraid.delete_doc_if_exists("Leave Type", "Test Leave Type 1", force=1)
		leave_type_1 = vmraid.get_doc(
		...
		vmraid.db.set_value("Employee", employee.name, "date_of_joining", "2010-01-01")
	...
	def test_earned_leaves_creation(self):
		...
		vmraid.delete_doc_if_exists("Leave Type", "Test Earned Leave Type", force=1)
		vmraid.get_doc(
			dict(
		...
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[employee.name], vmraid._dict(data)
		)

		from erpadda.hr.utils import allocate_earned_leaves
		...
		vmraid.db.set_value("Leave Type", leave_type, "max_leaves_allowed", 0)
		i = 0
		while i < 6:
			allocate_earned_leaves(ignore_duplicates=True)
			i += 1
	...
	def test_current_leave_on_submit(self):
		...
		if not vmraid.db.exists("Leave Type", leave_type):
			vmraid.get_doc(dict(leave_type_name=leave_type, doctype="Leave Type")).insert()

		allocation = vmraid.get_doc(
			dict(
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_application.cancel()
		self.assertFalse(
			vmraid.db.exists("Leave Ledger Entry", {"transaction_name": leave_application.name})
		)
	...
	def test_leave_approver_perms(self):
		...
		employee.save()
		self.assertTrue("Leave Approver" in vmraid.get_roles(user))
		...
		application.insert()
		self.assertTrue(application.name in vmraid.share.get_shared("Leave Application", user))

		# check shared doc revoked
		application.reload()
		application.leave_approver = "test@example.com"
		application.save()
		self.assertTrue(application.name not in vmraid.share.get_shared("Leave Application", user))
...
def set_leave_approver():
	employee = get_employee()
	dept_doc = vmraid.get_doc("Department", employee.department)
	dept_doc.append("leave_approvers", {"approver": "test@example.com"})
	dept_doc.save(ignore_permissions=True)


def get_leave_period():
	leave_period_name = vmraid.db.get_value("Leave Period", {"company": "_Test Company"})
	if leave_period_name:
		return vmraid.get_doc("Leave Period", leave_period_name)
	else:
