...
vmraid.ui.form.on('Expense Claim', {
	onload: function(frm) {
	...
	company: function(frm) {
		...
		for (var i = 0; i < expenses.length; i++) {
			...
			vmraid.call({
				method: "erpadda.hr.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
				args: {
...
vmraid.ui.form.on('Expense Claim Detail', {
	expense_type: function(frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		if (!frm.doc.company) {
			d.expense_type = "";
			vmraid.msgprint(__("Please set the Company"));
...
cur_frm.cscript.onload = function(doc) {
	if (doc.__islocal) {
		cur_frm.set_value("posting_date", vmraid.datetime.get_today());
...
cur_frm.cscript.refresh = function(doc) {
	...
	if(!doc.__islocal) {
		...
		if (doc.docstatus===1) {
			...
			if (cint(doc.total_amount_reimbursed) > 0 && vmraid.model.can_read(entry_doctype)) {
				cur_frm.add_custom_button(__('Bank Entries'), function() {
					...
					vmraid.set_route("List", entry_doctype);
				}, __("View"));
			}
			/* eslint-enable */
		}
...
cur_frm.cscript.set_help = function(doc) {
	cur_frm.set_intro("");
	if(doc.__islocal && !in_list(vmraid.user_roles, "HR User")) {
...
vmraid.ui.form.on("Expense Claim", {
	...
	onload: function(frm) {
		if (frm.doc.docstatus == 0) {
			return vmraid.call({
				method: "erpadda.hr.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function(r) {
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus===1 && !cint(frm.doc.is_paid) && cint(frm.doc.grand_total) > 0
				...
				&& vmraid.model.can_create("Payment Entry")) {
	...
	make_payment_entry: function(frm) {
		...
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "erpadda.hr.doctype.expense_claim.expense_claim.make_bank_entry";
		}
		return vmraid.call({
			method: method,
			args: {
	...
	employee_name: function(frm) {
		erpadda.expense_claim.set_title(frm);
	},
	...
	get_advances: function(frm) {
		vmraid.model.clear_table(frm.doc, "advances");
		if (frm.doc.employee) {
			return vmraid.call({
				method: "erpadda.hr.doctype.expense_claim.expense_claim.get_advances",
				args: {
					employee: frm.doc.employee
				},
				callback: function(r, rt) {
					...
					if(r.message) {
						$.each(r.message, function(i, d) {
							var row = vmraid.model.add_child(frm.doc, "Expense Claim Advance", "advances");
...
vmraid.ui.form.on("Expense Claim Detail", {
	amount: function(frm, cdt, cdn) {
		...
		vmraid.model.set_value(cdt, cdn, 'sanctioned_amount', child.amount);
	},
	...
	cost_center: function(frm, cdt, cdn) {
		erpadda.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "expenses", "cost_center");
	}
});

vmraid.ui.form.on("Expense Claim Advance", {
	employee_advance: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		if(!frm.doc.employee){
			vmraid.msgprint(__('Select an employee to get the employee advance.'));
			frm.doc.advances = [];
			refresh_field("advances");
		}
		else {
...
vmraid.ui.form.on("Expense Taxes and Charges", {
