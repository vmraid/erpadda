...
import vmraid
from vmraid.utils import flt, nowdate, random_string
...
from erpadda.hr.doctype.expense_claim.expense_claim import make_bank_entry
...
class TestExpenseClaim(unittest.TestCase):
	...
	def test_total_expense_claim_for_project(self):
		vmraid.db.sql("""delete from `tabTask`""")
		vmraid.db.sql("""delete from `tabProject`""")
		vmraid.db.sql("update `tabExpense Claim` set project = '', task = ''")

		project = vmraid.get_doc({"project_name": "_Test Project 1", "doctype": "Project"})
		project.save()
		...
		self.assertEqual(vmraid.db.get_value("Task", task_name, "total_expense_claim"), 200)
		self.assertEqual(vmraid.db.get_value("Project", project.name, "total_expense_claim"), 200)

		expense_claim2 = make_expense_claim(
			payable_account, 600, 500, company_name, "Travel Expenses - _TC3", project.name, task_name
		)

		self.assertEqual(vmraid.db.get_value("Task", task_name, "total_expense_claim"), 700)
		self.assertEqual(vmraid.db.get_value("Project", project.name, "total_expense_claim"), 700)
	...
	def test_expense_claim_status(self):
		...
		je = vmraid.get_doc(je_dict)
		je.posting_date = nowdate()
		je.cheque_no = random_string(5)
		je.cheque_date = nowdate()
		je.submit()

		expense_claim = vmraid.get_doc("Expense Claim", expense_claim.name)
	...
	def test_expense_claim_against_fully_paid_advances(self):
		from erpadda.hr.doctype.employee_advance.test_employee_advance import (
			get_advances_for_claim,
			make_employee_advance,
			make_payment_entry,
		)

		vmraid.db.delete("Employee Advance")
	...
	def test_expense_approver_perms(self):
		...
		expense_claim.save()
		self.assertTrue(expense_claim.name in vmraid.share.get_shared("Expense Claim", user))

		# check shared doc revoked
		expense_claim.reload()
		expense_claim.expense_approver = "test@example.com"
		expense_claim.save()
		self.assertTrue(expense_claim.name not in vmraid.share.get_shared("Expense Claim", user))
...
def get_payable_account(company):
	return vmraid.get_cached_value("Company", company, "default_payable_account")
...
def make_expense_claim(
	...
	currency, cost_center = vmraid.db.get_value(
	...
	expense_claim = vmraid.get_doc(expense_claim)
...
def get_outstanding_and_total_reimbursed_amounts(expense_claim):
	outstanding_amount = flt(
		vmraid.db.get_value("Expense Claim", expense_claim.name, "total_sanctioned_amount")
	) - flt(vmraid.db.get_value("Expense Claim", expense_claim.name, "total_amount_reimbursed"))
	total_amount_reimbursed = flt(
		vmraid.db.get_value("Expense Claim", expense_claim.name, "total_amount_reimbursed")
	)
