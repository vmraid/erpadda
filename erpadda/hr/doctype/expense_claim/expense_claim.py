...
from vmraid.utils import cstr, flt, get_link_to_form
...
from erpadda.hr.utils import set_employee_name, share_doc_with_approver, validate_active_employee


class InvalidExpenseApproverError(vmraid.ValidationError):
	pass


class ExpenseApproverIdentityError(vmraid.ValidationError):
	pass


class ExpenseClaim(AccountsController):
	def onload(self):
		self.get("__onload").make_payment_via_journal_entry = vmraid.db.get_single_value(
	...
	def validate(self):
		...
		if self.task and not self.project:
			self.project = vmraid.db.get_value("Task", self.task, "project")
	...
	def set_payable_account(self):
		if not self.payable_account and not self.is_paid:
			self.payable_account = vmraid.get_cached_value(
	...
	def on_submit(self):
		if self.approval_status == "Draft":
			vmraid.throw(_("""Approval Status must be 'Approved' or 'Rejected'"""))
	...
	def update_claimed_amount_in_employee_advance(self):
		for d in self.get("advances"):
			vmraid.get_doc("Employee Advance", d.employee_advance).update_claimed_amount()

	def update_task_and_project(self):
		if self.task:
			self.update_task()
		elif self.project:
			vmraid.get_doc("Project", self.project).update_project()
			...
			if not data.cost_center:
				vmraid.throw(
					_("Row {0}: {1} is required in the expenses table to book an expense claim.").format(
						data.idx, vmraid.bold("Cost Center")
					)
				)

		if self.is_paid:
			if not self.mode_of_payment:
				vmraid.throw(_("Mode of payment is required to make a payment").format(self.employee))
	...
	def update_task(self):
		task = vmraid.get_doc("Task", self.task)
	...
	def validate_advances(self):
		...
		if self.total_advance_amount:
			...
			if flt(self.total_advance_amount, precision) > flt(self.total_claimed_amount, precision):
				vmraid.throw(_("Total advance amount cannot be greater than total claimed amount"))

			if self.total_sanctioned_amount and flt(self.total_advance_amount, precision) > flt(
				self.total_sanctioned_amount, precision
			):
				vmraid.throw(_("Total advance amount cannot be greater than total sanctioned amount"))
...
def update_reimbursed_amount(doc, amount):
	...
	doc.set_status()
	vmraid.db.set_value("Expense Claim", doc.name, "status", doc.status)
...
def make_bank_entry(dt, dn):
	...
	expense_claim = vmraid.get_doc(dt, dn)
	...
	je.append(
		"accounts",
		{
			...
			"cost_center": erpadda.get_default_cost_center(expense_claim.company),
...
def get_expense_claim_account_and_cost_center(expense_claim_type, company):
	...
	cost_center = erpadda.get_default_cost_center(company)
	...
	if not account:
		vmraid.throw(
			_("Set the default account for the {0} {1}").format(
				vmraid.bold("Expense Claim Type"), get_link_to_form("Expense Claim Type", expense_claim_type)
			)
		)
...
def get_advances(employee, advance_id=None):
	...
	query = vmraid.qb.from_(advance).select(
		advance.name,
...
def get_expense_claim(
	...
	default_payable_account = vmraid.get_cached_value("Company", company, "default_payable_account")
	default_cost_center = vmraid.get_cached_value("Company", company, "cost_center")

	expense_claim = vmraid.new_doc("Expense Claim")
