...
class EmployeeAdvanceOverPayment(vmraid.ValidationError):
	pass
...
class EmployeeAdvance(Document):
	...
	def set_total_advance_paid(self):
		gle = vmraid.qb.DocType("GL Entry")
		...
		if flt(return_amount) > self.paid_amount - self.claimed_amount:
			vmraid.throw(_("Return amount cannot be greater unclaimed amount"))
	...
	def update_claimed_amount(self):
		...
		vmraid.db.set_value("Employee Advance", self.name, "claimed_amount", flt(claimed_amount))
		self.reload()
		self.set_status(update=True)
...
def get_pending_amount(employee, posting_date):
	employee_due_amount = vmraid.get_all(
		"Employee Advance",
...
def make_bank_entry(dt, dn):
	...
	if not payment_account:
		vmraid.throw(_("Please set a Default Cash Account in Company defaults"))

	advance_account_currency = vmraid.db.get_value("Account", doc.advance_account, "account_currency")
	...
	je.append(
		"accounts",
		{
			...
			"party_type": "Employee",
			"cost_center": erpadda.get_default_cost_center(doc.company),
			"party": doc.employee,
			"is_advance": "Yes",
		},
	)
...
def make_return_entry(
	...
	advance_account_currency = vmraid.db.get_value("Account", advance_account, "account_currency")
...
def get_voucher_type(mode_of_payment=None):
	...
	if mode_of_payment:
		mode_of_payment_type = vmraid.get_cached_value("Mode of Payment", mode_of_payment, "type")
