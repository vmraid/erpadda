...
class TestStaffingPlan(unittest.TestCase):
	def test_staffing_plan(self):
		_set_up()
		vmraid.db.set_value("Company", "_Test Company 3", "is_group", 1)
		if vmraid.db.exists("Staffing Plan", "Test"):
			return
		staffing_plan = vmraid.new_doc("Staffing Plan")
	...
	def test_staffing_plan_subsidiary_company(self):
		...
		if vmraid.db.exists("Staffing Plan", "Test 1"):
			return
		staffing_plan = vmraid.new_doc("Staffing Plan")
...
def make_company():
	if vmraid.db.exists("Company", "_Test Company 10"):
		return
