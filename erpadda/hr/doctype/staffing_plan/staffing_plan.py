...
class SubsidiaryCompanyError(vmraid.ValidationError):
	pass


class ParentCompanyError(vmraid.ValidationError):
	pass
...
class StaffingPlan(Document):
	...
	def validate_with_parent_plan(self, staffing_plan_detail):
		if not vmraid.get_cached_value("Company", self.company, "parent_company"):
		...
		parent_company = vmraid.db.get_value("Staffing Plan", parent_plan_details[0].name, "company")
		...
		if cint(staffing_plan_detail.vacancies) > cint(parent_plan_details[0].vacancies) or flt(
			...
			vmraid.throw(
				_(
					...
					vmraid.bold(staffing_plan_detail.designation),
		...
		lft, rgt = vmraid.get_cached_value("Company", parent_company, ["lft", "rgt"])
		all_sibling_details = vmraid.db.sql(
	...
	def validate_with_subsidiary_plans(self, staffing_plan_detail):
		...
		children_details = vmraid.db.sql(
			"""select sum(spd.vacancies) as vacancies,
			sum(spd.total_estimated_cost) as total_estimated_cost
			from `tabStaffing Plan Detail` spd join `tabStaffing Plan` sp on spd.parent=sp.name
			where spd.designation=%s and sp.docstatus=1
...
def get_designation_counts(designation, company):
	...
	employee_counts["employee_count"] = vmraid.db.get_value(
		"Employee",
		filters={"designation": designation, "status": "Active", "company": ("in", company_set)},
		fieldname=["count(name)"],
	)

	employee_counts["job_openings"] = vmraid.db.get_value(
		"Job Opening",
...
def get_active_staffing_plan_details(company, designation, from_date=None, to_date=None):
	...
	if not company or not designation:
		vmraid.throw(_("Please select Company and Designation"))

	staffing_plan = vmraid.db.sql(
		"""
	...
	if not staffing_plan:
		parent_company = vmraid.get_cached_value("Company", company, "parent_company")
		if parent_company:
