...
vmraid.ui.form.on('Staffing Plan', {
	setup: function(frm) {
...
vmraid.ui.form.on('Staffing Plan Detail', {
	...
	vacancies: function(frm, cdt, cdn) {
		...
		if(child.vacancies < child.current_openings) {
			vmraid.throw(__("Vacancies cannot be lower than the current openings"));
		}
		set_number_of_positions(frm, cdt, cdn);
	},
...
var set_number_of_positions = function(frm, cdt, cdn) {
	let child = locals[cdt][cdn];
	if (!child.designation) vmraid.throw(__("Please enter the designation"));
	vmraid.call({
		"method": "erpadda.hr.doctype.staffing_plan.staffing_plan.get_designation_counts",
		args: {
		...
		callback: function (data) {
			if(data.message){
				vmraid.model.set_value(cdt, cdn, 'current_count', data.message.employee_count);
				vmraid.model.set_value(cdt, cdn, 'current_openings', data.message.job_openings);
				...
				if (cint(child.number_of_positions) < total_positions){
					vmraid.model.set_value(cdt, cdn, 'number_of_positions', total_positions);
				}
			}
			else{ // No employees for this designation
				vmraid.model.set_value(cdt, cdn, 'current_count', 0);
				vmraid.model.set_value(cdt, cdn, 'current_openings', 0);
			}
		}
	});
...
var set_total_estimated_cost = function(frm, cdt, cdn) {
	...
	if(child.vacancies > 0 && child.estimated_cost_per_position) {
		vmraid.model.set_value(cdt, cdn, 'total_estimated_cost', child.vacancies * child.estimated_cost_per_position);
	}
	else {
		vmraid.model.set_value(cdt, cdn, 'total_estimated_cost', 0);
	}
