...
from vmraid.utils import add_days, add_months, comma_sep, getdate, today

from erpadda.hr.doctype.employee.employee import get_all_employee_emails, get_employee_email
...
def send_reminders_in_advance_weekly():
	to_send_in_advance = int(vmraid.db.get_single_value("HR Settings", "send_holiday_reminders"))
	frequency = vmraid.db.get_single_value("HR Settings", "frequency")
...
def send_advance_holiday_reminders(frequency):
	...
	employees = vmraid.db.get_all("Employee", filters={"status": "Active"}, pluck="name")
...
def send_birthday_reminders():
	...
	to_send = int(vmraid.db.get_single_value("HR Settings", "send_birthday_reminders"))
	if not to_send:
		return
...
def get_birthday_reminder_text_and_message(birthday_persons):
	if len(birthday_persons) == 1:
		...
		birthday_person_text = comma_sep(person_names, vmraid._("{0} & {1}"), False)
...
def get_employees_having_an_event_today(event_type):
	...
	employees_born_today = vmraid.db.multisql(
		{
			"mariadb": f"""
...
def send_work_anniversary_reminders():
	...
	to_send = int(vmraid.db.get_single_value("HR Settings", "send_work_anniversary_reminders"))
	if not to_send:
		return
...
def get_work_anniversary_reminder_text_and_message(anniversary_persons):
	if len(anniversary_persons) == 1:
		...
		# converts ["Jim", "Rim", "Dim"] to Jim, Rim & Dim
		anniversary_person = comma_sep(person_names_with_years, vmraid._("{0} & {1}"), False)
		persons_name = comma_sep(names, vmraid._("{0} & {1}"), False)
