...
import vmraid
from vmraid import _, scrub, throw
...
from vmraid.utils import add_years, cstr, getdate, today, validate_email_address
from vmraid.utils.nestedset import NestedSet

from erpadda.utilities.transaction_base import delete_events


class EmployeeUserDisabledError(vmraid.ValidationError):
	pass


class InactiveEmployeeStatusError(vmraid.ValidationError):
	pass
...
class Employee(NestedSet):
	...
	def autoname(self):
		naming_method = vmraid.db.get_value("HR Settings", None, "emp_created_by")
	...
	def validate(self):
		...
		if self.user_id:
			...
			existing_user_id = vmraid.db.get_value("Employee", self.name, "user_id")
			if existing_user_id:
	...
	def validate_user_details(self):
		if self.user_id:
			data = vmraid.db.get_value("User", self.user_id, ["enabled", "user_image"], as_dict=1)
	...
	def update_user_permissions(self):
		...
		employee_user_permission_exists = vmraid.db.exists(
	...
	def update_user(self):
		...
		user = vmraid.get_doc("User", self.user_id)
	...
	def update_approver_role(self):
		if self.leave_approver:
			user = vmraid.get_doc("User", self.leave_approver)
			user.flags.ignore_permissions = True
			user.add_roles("Leave Approver")

		if self.expense_approver:
			user = vmraid.get_doc("User", self.expense_approver)
			user.flags.ignore_permissions = True
	...
	def set_preferred_email(self):
		preferred_email_field = vmraid.scrub(self.prefered_contact_email)
	...
	def validate_status(self):
		if self.status == "Left":
			...
			if reports_to:
				link_to_employees = [
					vmraid.utils.get_link_to_form("Employee", employee.name, label=employee.employee_name)
	...
	def validate_for_enabled_user_id(self, enabled):
		...
		if enabled is None:
			vmraid.throw(_("User {0} does not exist").format(self.user_id))
		if enabled == 0:
			vmraid.throw(_("User {0} is disabled").format(self.user_id), EmployeeUserDisabledError)

	def validate_duplicate_user_id(self):
		employee = vmraid.db.sql_list(
	...
	def on_trash(self):
		...
		if vmraid.db.exists("Employee Transfer", {"new_employee_id": self.name, "docstatus": 1}):
			emp_transfer = vmraid.get_doc(
	...
	def validate_preferred_email(self):
		if self.prefered_contact_email and not self.get(scrub(self.prefered_contact_email)):
			vmraid.msgprint(_("Please enter {0}").format(self.prefered_contact_email))

	def validate_onboarding_process(self):
		employee_onboarding = vmraid.get_all(
			"Employee Onboarding",
			filters={
		...
		if employee_onboarding:
			doc = vmraid.get_doc("Employee Onboarding", employee_onboarding[0].name)
	...
	def reset_employee_emails_cache(self):
		...
		if cell_number != prev_number or self.get("user_id") != prev_doc.get("user_id"):
			vmraid.cache().hdel("employees_with_number", cell_number)
			vmraid.cache().hdel("employees_with_number", prev_number)
...
def get_retirement_date(date_of_birth=None):
	...
	if date_of_birth:
		try:
			retirement_age = int(vmraid.db.get_single_value("HR Settings", "retirement_age") or 60)
...
def validate_employee_role(doc, method):
	...
	if "Employee" in [d.role for d in doc.get("roles")]:
		if not vmraid.db.get_value("Employee", {"user_id": doc.name}):
			vmraid.msgprint(_("Please set User ID field in an Employee record to set Employee Role"))
...
def update_user_permissions(doc, method):
	...
	if "Employee" in [d.role for d in doc.get("roles")]:
		if not has_permission("User Permission", ptype="write", raise_exception=False):
			return
		employee = vmraid.get_doc("Employee", {"user_id": doc.name})
...
def get_holiday_list_for_employee(employee, raise_exception=True):
	if employee:
		holiday_list, company = vmraid.db.get_value("Employee", employee, ["holiday_list", "company"])
	else:
		holiday_list = ""
...
def is_holiday(
	...
	holidays = vmraid.get_all("Holiday", fields=["description"], filters=filters, pluck="description")
...
def deactivate_sales_person(status=None, employee=None):
	if status == "Left":
		sales_person = vmraid.db.get_value("Sales Person", {"Employee": employee})
		if sales_person:
			vmraid.db.set_value("Sales Person", sales_person, "enabled", 0)
...
def get_all_employee_emails(company):
	...
	for employee in employee_list:
		if not employee:
			continue
		user, company_email, personal_email = vmraid.db.get_value(
...
def get_children(doctype, parent=None, company=None, is_root=False, is_tree=False):
	...
	employees = vmraid.get_list(doctype, fields=fields, filters=filters, order_by="name")

	for employee in employees:
		is_expandable = vmraid.get_all(doctype, filters=[["reports_to", "=", employee.get("value")]])
...
def on_doctype_update():
	vmraid.db.add_index("Employee", ["lft", "rgt"])
