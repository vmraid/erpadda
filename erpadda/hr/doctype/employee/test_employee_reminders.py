...
import vmraid
from vmraid.utils import add_months, getdate

from erpadda.hr.doctype.employee.employee_reminders import send_holidays_reminder_in_advance
from erpadda.hr.doctype.employee.test_employee import make_employee
from erpadda.hr.doctype.hr_settings.hr_settings import set_proceed_with_frequency_change
from erpadda.hr.utils import get_holidays_for_employee


class TestEmployeeReminders(unittest.TestCase):
	@classmethod
	def setUpClass(cls):
		from erpadda.hr.doctype.holiday_list.test_holiday_list import make_holiday_list
		...
		test_employee_2 = vmraid.get_doc("Employee", test_employee_2)
	...
	def test_birthday_reminders(self):
		employee = vmraid.get_doc(
			"Employee", vmraid.db.sql_list("select name from tabEmployee limit 1")[0]
		)
		...
		from erpadda.hr.doctype.employee.employee_reminders import (
	...
	def test_work_anniversary_reminders(self):
		make_employee(
			...
			date_of_joining="1998" + vmraid.utils.nowdate()[4:],
	...
	def test_work_anniversary_reminder_not_sent_for_0_years(self):
		...
		from erpadda.hr.doctype.employee.employee_reminders import get_employees_having_an_event_today
	...
	def test_advance_holiday_reminders_monthly(self):
		from erpadda.hr.doctype.employee.employee_reminders import send_reminders_in_advance_monthly
		...
		recipients = vmraid.db.get_all("Email Queue Recipient", pluck="recipient")
	...
	def test_advance_holiday_reminders_weekly(self):
		from erpadda.hr.doctype.employee.employee_reminders import send_reminders_in_advance_weekly
