...
from vmraid.utils import cint, cstr, formatdate, get_datetime, get_link_to_form, getdate, nowdate

from erpadda.hr.doctype.shift_assignment.shift_assignment import has_overlapping_timings
from erpadda.hr.utils import get_holiday_dates_for_employee, validate_active_employee


class DuplicateAttendanceError(vmraid.ValidationError):
	pass


class OverlappingShiftAttendanceError(vmraid.ValidationError):
	pass
...
class Attendance(Document):
		...
		if duplicate:
			vmraid.throw(
				_("Attendance for employee {0} is already marked for the date {1}: {2}").format(
					...
					vmraid.bold(self.attendance_date),
	...
	def validate_employee_status(self):
		if vmraid.db.get_value("Employee", self.employee, "status") == "Inactive":
			vmraid.throw(_("Cannot mark attendance for an Inactive employee {0}").format(self.employee))
