vmraid.listview_settings['Attendance'] = {
			...
			let dialog = new vmraid.ui.Dialog({
				...
				primary_action(data) {
					if (cur_dialog.no_unmarked_days_left) {
						vmraid.msgprint(__("Attendance for the month of {0} , has already been marked for the Employee {1}",
							[dialog.fields_dict.month.value, dialog.fields_dict.employee.value]));
					} else {
						vmraid.confirm(__('Mark attendance as {0} for {1} on selected dates?', [data.status, data.month]), () => {
							vmraid.call({
								method: "erpadda.hr.doctype.attendance.attendance.mark_bulk_attendance",
								args: {
									data: data
								},
								callback: function (r) {
	...
	get_multi_select_options: function(employee, month, exclude_holidays) {
		return new Promise(resolve => {
			vmraid.call({
				method: 'erpadda.hr.doctype.attendance.attendance.get_unmarked_days',
				async: false,
				args: {
					employee: employee,
					month: month,
