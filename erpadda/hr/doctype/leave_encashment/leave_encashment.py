...
from erpadda.hr.doctype.leave_allocation.leave_allocation import get_unused_leaves
...
class LeaveEncashment(Document):
	...
	def before_submit(self):
		if self.encashment_amount <= 0:
			vmraid.throw(_("You can only submit Leave Encashment for a valid encashment amount"))
	...
	def on_submit(self):
		...
		additional_salary.company = vmraid.get_value("Employee", self.employee, "company")
		additional_salary.employee = self.employee
		additional_salary.currency = self.currency
		earning_component = vmraid.get_value("Leave Type", self.leave_type, "earning_component")
		if not earning_component:
			vmraid.throw(_("Please set Earning Component for Leave type: {0}.").format(self.leave_type))
		...
		vmraid.db.set_value(
			...
			vmraid.db.get_value("Leave Allocation", self.leave_allocation, "total_leaves_encashed")
	...
	def get_leave_details_for_encashment(self):
		...
		if not vmraid.db.get_value("Leave Type", self.leave_type, "allow_encashment"):
			vmraid.throw(_("Leave Type {0} is not encashable").format(self.leave_type))
		...
		encashable_days = self.leave_balance - vmraid.db.get_value(
			"Leave Type", self.leave_type, "encashment_threshold_days"
		)
		self.encashable_days = encashable_days if encashable_days > 0 else 0

		per_day_encashment = vmraid.db.get_value(
