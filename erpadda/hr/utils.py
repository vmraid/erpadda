...
import erpadda
from erpadda.hr.doctype.employee.employee import (
	InactiveEmployeeStatusError,
	get_holiday_list_for_employee,
)


class DuplicateDeclarationError(vmraid.ValidationError):
	pass
...
def update_employee_work_history(employee, details, date=None, cancel=False):
	...
	internal_work_history = {}
	for item in details:
		field = vmraid.get_meta("Employee").get_field(item.fieldname)
		if not field:
			continue
...
def delete_employee_work_history(details, employee, date):
	...
	if filters:
		vmraid.db.delete("Employee Internal Work History", filters)
...
def validate_duplicate_exemption_for_payroll_period(doctype, docname, payroll_period, employee):
	existing_record = vmraid.db.exists(
		doctype,
		{
...
def allocate_earned_leaves(ignore_duplicates=False):
	...
	for e_leave_type in e_leave_types:
		...
		for allocation in leave_allocations:
			...
			annual_allocation = vmraid.db.get_value(
				"Leave Policy Detail",
		...
		if ignore_duplicates or not is_earned_leave_already_allocated(allocation, annual_allocation):
			...
			if e_leave_type.based_on_date_of_joining:
				text = _("allocated {0} leave(s) via scheduler on {1} based on the date of joining").format(
					vmraid.bold(earned_leaves), vmraid.bold(formatdate(today_date))
				)
			else:
...
def is_earned_leave_already_allocated(allocation, annual_allocation):
	...
	date_of_joining = vmraid.db.get_value("Employee", allocation.employee, "date_of_joining")

	assignment = vmraid.get_doc("Leave Policy Assignment", allocation.leave_policy_assignment)
...
def get_sal_slip_total_benefit_given(employee, payroll_period, component=False):
	...
	sum_of_given_benefit = vmraid.db.sql(
		query,
		{
			"employee": employee,
...
def get_holidays_for_employee(
	...
	holidays = vmraid.get_all("Holiday", fields=["description", "holiday_date"], filters=filters)
...
def get_previous_claimed_amount(employee, payroll_period, non_pro_rata=False, component=False):
	...
	sum_of_claimed_amount = vmraid.db.sql(
		query,
		{
			"employee": employee,
			"start_date": payroll_period.start_date,
...
def share_doc_with_approver(doc, user):
	...
	if not vmraid.has_permission(doc=doc, ptype="submit", user=user):
		vmraid.share.add(doc.doctype, doc.name, user, submit=1, flags={"ignore_share_permission": True})

		vmraid.msgprint(
			_("Shared with the user {0} with {1} access").format(user, vmraid.bold("submit"), alert=True)
		)
...
def validate_active_employee(employee):
	if vmraid.db.get_value("Employee", employee, "status") == "Inactive":
		vmraid.throw(
