...
from erpadda.hr.doctype.leave_allocation.leave_allocation import get_previous_allocation
...
def execute(filters: Optional[Filters] = None) -> Tuple:
	if filters.to_date <= filters.from_date:
		vmraid.throw(_('"From Date" can not be greater than or equal to "To Date"'))
...
def get_data(filters: Filters) -> List:
	leave_types = vmraid.db.get_list("Leave Type", pluck="name", order_by="name")
	conditions = get_conditions(filters)

	user = vmraid.session.user
	department_approver_map = get_department_leave_approver_map(filters.get("department"))

	active_employees = vmraid.get_list(
		"Employee",
		filters=conditions,
	...
	for leave_type in leave_types:
		if len(active_employees) > 1:
			...
			row = vmraid._dict({"leave_type": leave_type})
		...
		for employee in active_employees:
			...
			if (
				...
				or ("HR Manager" in vmraid.get_roles(user))
			):
				if len(active_employees) > 1:
					row = vmraid._dict()
...
def get_department_leave_approver_map(department: Optional[str] = None):
	...
	department_list = vmraid.get_list(
		"Department",
		filters={"disabled": 0},
	...
	for key, group in groupby(employee_data, lambda x: x["employee_name"]):
		for grp in group:
			if grp.closing_balance:
				leaves.append(
					vmraid._dict({"leave_type": grp.leave_type, "closing_balance": grp.closing_balance})
				)
