...
from erpadda.hr.doctype.exit_interview.test_exit_interview import create_exit_interview
from erpadda.hr.doctype.full_and_final_statement.test_full_and_final_statement import (
	create_full_and_final_statement,
)
from erpadda.hr.report.employee_exits.employee_exits import execute
...
class TestEmployeeExits(unittest.TestCase):
	...
	def setUpClass(cls):
		create_company()
		vmraid.db.sql("delete from `tabEmployee` where company='Test Company'")
		vmraid.db.sql("delete from `tabFull and Final Statement` where company='Test Company'")
		vmraid.db.sql("delete from `tabExit Interview` where company='Test Company'")
	...
	def test_employee_exits_summary(self):
		...
		employee1 = vmraid.get_doc("Employee", self.emp1)
		employee2 = vmraid.get_doc("Employee", self.emp2)
		expected_data = [
			{
	...
	def test_pending_exit_interviews_summary(self):
		...
		employee4 = vmraid.get_doc("Employee", self.emp4)
		expected_data = [
			{
	...
	def test_pending_fnf_summary(self):
		...
		employee3 = vmraid.get_doc("Employee", self.emp3)
		employee4 = vmraid.get_doc("Employee", self.emp4)
		expected_data = [
			{
...
def create_company():
	if not vmraid.db.exists("Company", "Test Company"):
		vmraid.get_doc(
			{
				"doctype": "Company",
