...
from vmraid.query_builder.functions import Count, Extract, Sum
from vmraid.utils import cint, cstr, getdate
...
def execute(filters: Optional[Filters] = None) -> Tuple:
	...
	if not (filters.month and filters.year):
		vmraid.throw(_("Please select month and year."))

	attendance_map = get_attendance_map(filters)
	if not attendance_map:
		vmraid.msgprint(_("No attendance records found."), alert=True, indicator="orange")
		return [], [], None, None
		...
		columns.append(
			{
				...
				"fieldname": vmraid.scrub(filters.group_by),
				"fieldtype": "Link",
				"options": "Branch",
				"width": 120,
			}
...
def get_columns_for_leave_types() -> List[Dict]:
	leave_types = vmraid.db.get_all("Leave Type", pluck="name")
	types = []
	for entry in leave_types:
		types.append(
			{"label": entry, "fieldname": vmraid.scrub(entry), "fieldtype": "Float", "width": 120}
		)
...
def get_data(filters: Filters, attendance_map: Dict) -> List[Dict]:
	...
	if filters.group_by:
		group_by_column = vmraid.scrub(filters.group_by)
		...
		for value in group_by_param_values:
			...
			if records:
				data.append({group_by_column: vmraid.bold(value)})
				data.extend(records)
	else:
...
def get_attendance_map(filters: Filters) -> Dict:
	...
	"""
	Attendance = vmraid.qb.DocType("Attendance")
	query = (
		vmraid.qb.from_(Attendance)
		.select(
			Attendance.employee,
	...
	for d in attendance_list:
		attendance_map.setdefault(d.employee, vmraid._dict()).setdefault(d.shift, vmraid._dict())
...
def get_employee_related_details(group_by: str, company: str) -> Tuple[Dict, List]:
	...
	"""
	Employee = vmraid.qb.DocType("Employee")
	query = (
		vmraid.qb.from_(Employee)
		.select(
			Employee.name,
	...
	if group_by:
		for parameter, employees in groupby(employee_details, key=lambda d: d[group_by]):
			...
			emp_map.setdefault(parameter, vmraid._dict())
...
def get_holiday_map(filters: Filters) -> Dict[str, List[Dict]]:
	...
	holiday_lists = vmraid.db.get_all("Holiday List", pluck="name")
	default_holiday_list = vmraid.get_cached_value("Company", filters.company, "default_holiday_list")
	holiday_lists.append(default_holiday_list)

	holiday_map = vmraid._dict()
	Holiday = vmraid.qb.DocType("Holiday")
...
def get_attendance_summary_and_days(employee: str, filters: Filters) -> Tuple[Dict, List]:
	...
	absent_case = vmraid.qb.terms.Case().when(Attendance.status == "Absent", 1).else_(0)
	sum_absent = Sum(absent_case).as_("total_absent")

	leave_case = vmraid.qb.terms.Case().when(Attendance.status == "On Leave", 1).else_(0)
	sum_leave = Sum(leave_case).as_("total_leaves")

	half_day_case = vmraid.qb.terms.Case().when(Attendance.status == "Half Day", 0.5).else_(0)
...
def get_leave_summary(employee: str, filters: Filters) -> Dict[str, float]:
	...
	"""
	Attendance = vmraid.qb.DocType("Attendance")
	day_case = vmraid.qb.terms.Case().when(Attendance.status == "Half Day", 0.5).else_(1)
	...
	for d in leave_details:
		leave_type = vmraid.scrub(d.leave_type)
...
def get_entry_exits_summary(employee: str, filters: Filters) -> Dict[str, float]:
	...
	late_entry_case = vmraid.qb.terms.Case().when(Attendance.late_entry == "1", "1")
	count_late_entries = Count(late_entry_case).as_("total_late_entries")

	early_exit_case = vmraid.qb.terms.Case().when(Attendance.early_exit == "1", "1")
