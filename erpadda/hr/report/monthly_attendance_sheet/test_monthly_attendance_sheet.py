...
from vmraid.utils import get_year_ending, get_year_start, getdate, now_datetime
...
from erpadda.hr.doctype.leave_application.test_leave_application import make_allocation_record
from erpadda.hr.report.monthly_attendance_sheet.monthly_attendance_sheet import execute
...
class TestMonthlyAttendanceSheet(VMRaidTestCase):
	def setUp(self):
	...
	def test_attendance_with_group_by_filter(self):
		...
		department = vmraid.db.get_value("Employee", self.employee, "department")
	...
	def test_validations(self):
		...
		self.assertRaises(vmraid.ValidationError, execute_report_with_invalid_filters)
...
def execute_report_with_invalid_filters():
	filters = vmraid._dict({"company": "_Test Company", "group_by": "Department"})
