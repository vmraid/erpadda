...
def post_depreciation_entries(date=None):
	...
	if not cint(
		vmraid.db.get_value("Accounts Settings", None, "book_asset_depreciation_entry_automatically")
	):
		return
...
def make_depreciation_entry(asset_name, date=None):
	vmraid.has_permission("Journal Entry", throw=True)
	...
	depreciation_cost_center, depreciation_series = vmraid.get_cached_value(
...
def get_depreciation_accounts(asset):
	...
	if not accumulated_depreciation_account or not depreciation_expense_account:
		accounts = vmraid.get_cached_value(
...
def get_credit_and_debit_accounts(accumulated_depreciation_account, depreciation_expense_account):
	root_type = vmraid.get_value("Account", depreciation_expense_account, "root_type")
	...
	if root_type == "Expense":
		...
		vmraid.throw(_("Depreciation Expense Account should be an Income or Expense Account."))
...
def scrap_asset(asset_name):
	...
	if asset.docstatus != 1:
		vmraid.throw(_("Asset {0} must be submitted").format(asset.name))
	elif asset.status in ("Cancelled", "Sold", "Scrapped"):
		vmraid.throw(
			_("Asset {0} cannot be scrapped, as it is already {1}").format(asset.name, asset.status)
		)

	depreciation_series = vmraid.get_cached_value(
	...
	vmraid.db.set_value("Asset", asset_name, "disposal_date", today())
	vmraid.db.set_value("Asset", asset_name, "journal_entry_for_scrap", je.name)
	asset.set_status("Scrapped")

	vmraid.msgprint(_("Asset scrapped via Journal Entry {0}").format(je.name))
...
def restore_asset(asset_name):
	...
	vmraid.get_doc("Journal Entry", je).cancel()
...
def get_disposal_account_and_cost_center(company):
	disposal_account, depreciation_cost_center = vmraid.get_cached_value(
	...
	if not depreciation_cost_center:
		vmraid.throw(_("Please set 'Asset Depreciation Cost Center' in Company {0}").format(company))
