...
class Asset(AccountsController):
	...
	def validate_asset_and_reference(self):
		if self.purchase_invoice or self.purchase_receipt:
			...
			reference_doc = vmraid.get_doc(reference_doc, reference_name)
	...
	def validate_item(self):
		item = vmraid.get_cached_value(
			"Item", self.item_code, ["is_fixed_asset", "is_stock_item", "disabled"], as_dict=1
		)
		if not item:
			vmraid.throw(_("Item {0} does not exist").format(self.item_code))
		elif item.disabled:
			vmraid.throw(_("Item {0} has been disabled").format(self.item_code))
		elif not item.is_fixed_asset:
			vmraid.throw(_("Item {0} must be a Fixed Asset Item").format(self.item_code))
		elif item.is_stock_item:
			vmraid.throw(_("Item {0} must be a non-stock item").format(self.item_code))

	def validate_cost_center(self):
		if not self.cost_center:
			return

		cost_center_company = vmraid.db.get_value("Cost Center", self.cost_center, "company")
		if cost_center_company != self.company:
			vmraid.throw(
				_("Selected Cost Center {} doesn't belongs to {}").format(
					vmraid.bold(self.cost_center), vmraid.bold(self.company)
				),
	...
	def validate_in_use_date(self):
		if not self.available_for_use_date:
			vmraid.throw(_("Available for use date is required"))
	...
	def set_missing_values(self):
		if not self.asset_category:
			self.asset_category = vmraid.get_cached_value("Item", self.item_code, "asset_category")
	...
	def validate_asset_values(self):
		...
		if not flt(self.gross_purchase_amount):
			vmraid.throw(_("Gross Purchase Amount is mandatory"), vmraid.MandatoryError)
		...
		if is_cwip_accounting_enabled(self.asset_category):
			...
			if (
				...
				and not vmraid.db.get_value("Purchase Invoice", self.purchase_invoice, "update_stock")
			):
				vmraid.throw(
		...
		if not self.calculate_depreciation:
			...
			vmraid.throw(_("Enter depreciation details"))
		...
		if self.available_for_use_date and getdate(self.available_for_use_date) < getdate(
			...
			vmraid.throw(_("Available-for-use Date should be after purchase date"))
	...
	def validate_gross_and_purchase_amount(self):
		...
		if self.gross_purchase_amount and self.gross_purchase_amount != self.purchase_receipt_amount:
			...
			vmraid.throw(error_message, title=_("Invalid Gross Purchase Amount"))
	...
	def make_asset_movement(self):
		...
		if reference_docname:
			posting_date, posting_time = vmraid.db.get_value(
		...
		if not self.is_existing_asset:
			...
			if self.opening_accumulated_depreciation:
				if not self.number_of_depreciations_booked:
					vmraid.throw(_("Please set Number of Depreciations Booked"))
			else:
				self.number_of_depreciations_booked = 0
	...
	def validate_cancellation(self):
		...
		if self.status not in ("Submitted", "Partially Depreciated", "Fully Depreciated"):
			vmraid.throw(_("Asset cannot be cancelled, as it is already {0}").format(self.status))

	def cancel_movement_entries(self):
		movements = vmraid.db.sql(
		...
		for movement in movements:
			movement = vmraid.get_doc("Asset Movement", movement.get("name"))
			movement.cancel()

	def delete_depreciation_entries(self):
		for d in self.get("schedules"):
			if d.journal_entry:
				vmraid.get_doc("Journal Entry", d.journal_entry).cancel()
	...
	def get_default_finance_book_idx(self):
		if not self.get("default_finance_book") and self.company:
			self.default_finance_book = erpadda.get_default_finance_book(self.company)
	...
	def validate_make_gl_entry(self):
		...
		if asset_bought_with_invoice:
			...
			expense_booked = vmraid.db.sql(query, (purchase_document, fixed_asset_account), as_dict=1)
			if expense_booked:
				# if expense is already booked from invoice then do not make gl entries regardless of cwip enabled/disabled
				return False

			cwip_booked = vmraid.db.sql(query, (purchase_document, cwip_account), as_dict=1)
			if cwip_booked:
	...
	def get_purchase_document(self):
		asset_bought_with_invoice = self.purchase_invoice and vmraid.db.get_value(
		...
		if not fixed_asset_account:
			vmraid.throw(
				_("Set {0} in asset category {1} for company {2}").format(
					vmraid.bold("Fixed Asset Account"),
					vmraid.bold(self.asset_category),
					vmraid.bold(self.company),
				),
	...
	def get_depreciation_rate(self, args, on_validate=False):
		...
		float_precision = cint(vmraid.db.get_default("float_precision")) or 2
...
def update_maintenance_status():
	assets = vmraid.get_all("Asset", filters={"docstatus": 1, "maintenance_required": 1})

	for asset in assets:
		asset = vmraid.get_doc("Asset", asset.name)
		if vmraid.db.exists("Asset Repair", {"asset_name": asset.name, "repair_status": "Pending"}):
			asset.set_status("Out of Order")
		elif vmraid.db.exists(
...
def make_post_gl_entry():
	...
	asset_categories = vmraid.db.get_all("Asset Category", fields=["name", "enable_cwip_accounting"])

	for asset_category in asset_categories:
		if cint(asset_category.enable_cwip_accounting):
			assets = vmraid.db.sql_list(
			...
			for asset in assets:
				doc = vmraid.get_doc("Asset", asset)
				doc.make_gl_entries()


def get_asset_naming_series():
	meta = vmraid.get_meta("Asset")
...
def make_sales_invoice(asset, item_code, company, serial_no=None):
	...
	si.company = company
	si.currency = vmraid.get_cached_value("Company", company, "default_currency")
...
def create_asset_maintenance(asset, item_code, item_name, asset_category, company):
	asset_maintenance = vmraid.new_doc("Asset Maintenance")
...
def create_asset_repair(asset, asset_name):
	asset_repair = vmraid.new_doc("Asset Repair")
...
def create_asset_value_adjustment(asset, asset_category, company):
	asset_value_adjustment = vmraid.new_doc("Asset Value Adjustment")
...
def transfer_asset(args):
	...
	movement_entry = vmraid.new_doc("Asset Movement")
...
def get_item_details(item_code, asset_category):
	asset_category_doc = vmraid.get_doc("Asset Category", asset_category)
	books = []
...
def get_asset_account(account_name, asset=None, asset_category=None, company=None):
	...
	if not account:
		account = vmraid.get_cached_value("Company", company, account_name)
...
def make_journal_entry(asset_name):
	...
	depreciation_cost_center, depreciation_series = vmraid.db.get_value(
...
def make_asset_movement(assets, purpose=None):
	...
	if len(assets) == 0:
		vmraid.throw(_("Atleast one asset has to be selected."))

	asset_movement = vmraid.new_doc("Asset Movement")
	asset_movement.quantity = len(assets)
	for asset in assets:
		asset = vmraid.get_doc("Asset", asset.get("name"))
...
def is_cwip_accounting_enabled(asset_category):
	return cint(vmraid.db.get_value("Asset Category", asset_category, "enable_cwip_accounting"))
...
def split_asset(asset_name, split_qty):
	...
	if split_qty >= asset.asset_quantity:
		vmraid.throw(_("Split qty cannot be grater than or equal to asset qty"))
...
def create_new_asset_after_split(asset, split_qty):
	new_asset = vmraid.copy_doc(asset)
