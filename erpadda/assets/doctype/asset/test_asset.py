...
import vmraid
from vmraid.utils import add_days, add_months, cstr, flt, get_last_day, getdate, nowdate

from erpadda.accounts.doctype.purchase_invoice.test_purchase_invoice import make_purchase_invoice
from erpadda.assets.doctype.asset.asset import make_sales_invoice, split_asset
...
class AssetSetup(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		vmraid.db.sql("delete from `tabTax Rule`")
...
class TestAsset(AssetSetup):
	...
	def test_gross_purchase_amount_is_mandatory(self):
		...
		self.assertRaises(vmraid.MandatoryError, asset.save)
	...
	def test_pr_or_pi_mandatory_if_not_existing_asset(self):
		...
		self.assertRaises(vmraid.ValidationError, asset.save)
	...
	def test_validate_item(self):
		asset = create_asset(item_code="MacBook Pro", do_not_save=1)
		item = vmraid.get_doc("Item", "MacBook Pro")
	...
	def test_is_fixed_asset_set(self):
		...
		doc = vmraid.new_doc("Purchase Invoice")
	...
	def test_gle_made_by_asset_sale(self):
		...
		self.assertEqual(vmraid.db.get_value("Asset", asset.name, "status"), "Sold")
		...
		si.cancel()
		self.assertEqual(vmraid.db.get_value("Asset", asset.name, "status"), "Partially Depreciated")
	...
	def test_asset_splitting(self):
		...
		jv = vmraid.get_doc("Journal Entry", journal_entry)
	...
	def test_cwip_accounting(self):
		...
		asset = vmraid.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")

		asset_doc = vmraid.get_doc("Asset", asset)
	...
	def test_asset_cwip_toggling_cases(self):
		cwip = vmraid.db.get_value("Asset Category", "Computers", "enable_cwip_accounting")
		name = vmraid.db.get_value(
			"Asset Category Account", filters={"parent": "Computers"}, fieldname=["name"]
		)
		cwip_acc = "CWIP Account - _TC"

		vmraid.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 0)
		vmraid.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", "")
		vmraid.db.get_value("Company", "_Test Company", "capital_work_in_progress_account", "")
		...
		asset = vmraid.db.get_value("Asset", {"purchase_invoice": pi.name, "docstatus": 0}, "name")
		asset_doc = vmraid.get_doc("Asset", asset)
		...
		vmraid.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 1)
		vmraid.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		asset = vmraid.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")
		asset_doc = vmraid.get_doc("Asset", asset)
		asset_doc.available_for_use_date = nowdate()
		asset_doc.calculate_depreciation = 0
		...
		vmraid.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", cwip)
		vmraid.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		vmraid.db.get_value("Company", "_Test Company", "capital_work_in_progress_account", cwip_acc)
...
class TestDepreciationMethods(AssetSetup):
	...
	def test_discounted_wdv_depreciation_rate_for_indian_region(self):
		# set indian company
		company_flag = vmraid.flags.company
		vmraid.flags.company = "_Test Company"

		finance_book = vmraid.new_doc("Finance Book")
		...
		vmraid.flags.company = company_flag
...
class TestDepreciationBasics(AssetSetup):
	...
	def test_get_depreciation_amount(self):
		...
		from erpadda.assets.doctype.asset.asset import get_depreciation_amount
	...
	def test_number_of_depreciations(self):
		...
		self.assertRaises(vmraid.ValidationError, asset_2.save)
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_expense_account(self):
		...
		je = vmraid.get_doc("Journal Entry", asset.schedules[0].journal_entry)
		accounting_entries = [
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_income_account(self):
		...
		depr_expense_account = vmraid.get_doc("Account", "_Test Depreciations - _TC")
	...
	def test_depreciation_entry_cancellation(self):
		...
		vmraid.get_doc("Journal Entry", depr_entry).cancel()
	...
	def test_gle_made_by_depreciation_entries(self):
		...
		vmraid.db.set_value("Company", "_Test Company", "series_for_depreciation_entry", "DEPR-")
		post_depreciation_entries(date="2021-01-01")
		asset.load_from_db()
	...
	def test_asset_cost_center(self):
		...
		self.assertRaises(vmraid.ValidationError, asset.submit)
...
def create_asset_data():
	if not vmraid.db.exists("Asset Category", "Computers"):
		create_asset_category()

	if not vmraid.db.exists("Item", "Macbook Pro"):
...
def enable_cwip_accounting(asset_category, enable=1):
	vmraid.db.set_value("Asset Category", asset_category, "enable_cwip_accounting", enable)
