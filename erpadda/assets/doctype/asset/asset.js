...
vmraid.provide("erpadda.asset");
vmraid.provide("erpadda.accounts.dimensions");

vmraid.ui.form.on('Asset', {
	onload: function(frm) {
	...
	setup: function(frm) {
		frm.make_methods = {
			'Asset Movement': () => {
				vmraid.call({
				method: "erpadda.assets.doctype.asset.asset.make_asset_movement",
				freeze: true,
				args:{
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				query: "erpadda.controllers.queries.get_purchase_receipts",
				filters: { item_code: doc.item_code }
			}
		});
		frm.set_query("purchase_invoice", (doc) => {
			return {
				query: "erpadda.controllers.queries.get_purchase_invoices",
				filters: { item_code: doc.item_code }
			}
		});
	},
	...
	refresh: function(frm) {
		vmraid.ui.form.trigger("Asset", "is_existing_asset");
		...
		if (frm.doc.docstatus==1) {
			if (in_list(["Submitted", "Partially Depreciated", "Fully Depreciated"], frm.doc.status)) {
				frm.add_custom_button(__("Transfer Asset"), function() {
					erpadda.asset.transfer_asset(frm);
				}, __("Manage"));

				frm.add_custom_button(__("Scrap Asset"), function() {
					erpadda.asset.scrap_asset(frm);
				}, __("Manage"));
				...
				frm.add_custom_button(__("Restore Asset"), function() {
					erpadda.asset.restore_asset(frm);
				}, __("Manage"));
			}
	...
	make_journal_entry: function(frm) {
		vmraid.call({
			method: "erpadda.assets.doctype.asset.asset.make_journal_entry",
			args: {
	...
	setup_chart: function(frm) {
		...
		if(frm.doc.opening_accumulated_depreciation) {
			last_depreciation_date = vmraid.datetime.add_months(frm.doc.next_depreciation_date,
	...
	set_finance_book: function(frm) {
		vmraid.call({
			method: "erpadda.assets.doctype.asset.asset.get_item_details",
			args: {
	...
	opening_accumulated_depreciation: function(frm) {
		erpadda.asset.set_accumulated_depreciation(frm);
	},
	...
	make_sales_invoice: function(frm) {
		vmraid.call({
			...
			method: "erpadda.assets.doctype.asset.asset.make_sales_invoice",
			callback: function(r) {
				var doclist = vmraid.model.sync(r.message);
				vmraid.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_maintenance: function(frm) {
		vmraid.call({
			...
			method: "erpadda.assets.doctype.asset.asset.create_asset_maintenance",
			callback: function(r) {
				var doclist = vmraid.model.sync(r.message);
				vmraid.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	create_asset_repair: function(frm) {
		vmraid.call({
			...
			method: "erpadda.assets.doctype.asset.asset.create_asset_repair",
			callback: function(r) {
				var doclist = vmraid.model.sync(r.message);
				vmraid.set_route("Form", doclist[0].doctype, doclist[0].name);
			}
	...
	split_asset: function(frm) {
		...
		dialog.set_primary_action(__('Split'), function() {
			...
			vmraid.call({
				...
				method: "erpadda.assets.doctype.asset.asset.split_asset",
				callback: function(r) {
					let doclist = vmraid.model.sync(r.message);
					vmraid.set_route("Form", doclist[0].doctype, doclist[0].name);
				}
			});
	...
	create_asset_value_adjustment: function(frm) {
		vmraid.call({
			...
			method: "erpadda.assets.doctype.asset.asset.create_asset_value_adjustment",
			freeze: 1,
			callback: function(r) {
				var doclist = vmraid.model.sync(r.message);
				vmraid.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	purchase_receipt: (frm) => {
		...
		if (frm.doc.purchase_receipt) {
			if (frm.doc.item_code) {
				vmraid.db.get_doc('Purchase Receipt', frm.doc.purchase_receipt).then(pr_doc => {
	...
	purchase_invoice: (frm) => {
		...
		if (frm.doc.purchase_invoice) {
			if (frm.doc.item_code) {
				vmraid.db.get_doc('Purchase Invoice', frm.doc.purchase_invoice).then(pi_doc => {
	...
	set_values_from_purchase_doc: function(frm, doctype, purchase_doc) {
		...
		if (!item) {
			doctype_field = vmraid.scrub(doctype)
	...
	set_depreciation_rate: function(frm, row) {
		if (row.total_number_of_depreciations && row.frequency_of_depreciation
			...
			vmraid.call({
				...
				callback: function(r) {
					if (r.message) {
						vmraid.flags.dont_change_rate = true;
...
vmraid.ui.form.on('Asset Finance Book', {
	...
	rate_of_depreciation: function(frm, cdt, cdn) {
		if(!vmraid.flags.dont_change_rate) {
			vmraid.model.set_value(cdt, cdn, "expected_value_after_useful_life", 0);
		}

		vmraid.flags.dont_change_rate = false;
	},

	depreciation_start_date: function(frm, cdt, cdn) {
		const book = locals[cdt][cdn];
		if (frm.doc.available_for_use_date && book.depreciation_start_date == frm.doc.available_for_use_date) {
			vmraid.msgprint(__("Depreciation Posting Date should not be equal to Available for Use Date."));
...
vmraid.ui.form.on('Depreciation Schedule', {
	make_depreciation_entry: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (!row.journal_entry) {
			vmraid.call({
				method: "erpadda.assets.doctype.asset.depreciation.make_depreciation_entry",
				args: {
					"asset_name": frm.doc.name,
...
erpadda.asset.set_accumulated_depreciation = function(frm) {
...
erpadda.asset.scrap_asset = function(frm) {
	vmraid.confirm(__("Do you really want to scrap this asset?"), function () {
		vmraid.call({
			args: {
				"asset_name": frm.doc.name
			},
			method: "erpadda.assets.doctype.asset.depreciation.scrap_asset",
			callback: function(r) {
				cur_frm.reload_doc();
			}
		})
...
erpadda.asset.restore_asset = function(frm) {
	vmraid.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		vmraid.call({
			args: {
				"asset_name": frm.doc.name
			},
			method: "erpadda.assets.doctype.asset.depreciation.restore_asset",
			callback: function(r) {
				cur_frm.reload_doc();
			}
		})
...
erpadda.asset.transfer_asset = function() {
	vmraid.call({
		method: "erpadda.assets.doctype.asset.asset.make_asset_movement",
		freeze: true,
		args:{
