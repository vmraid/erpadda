...
from vmraid.utils import add_months, cint, flt, getdate, time_diff_in_hours

from erpadda.accounts.general_ledger import make_gl_entries
from erpadda.assets.doctype.asset.asset import get_asset_account
from erpadda.controllers.accounts_controller import AccountsController


class AssetRepair(AccountsController):
	def validate(self):
		self.asset_doc = vmraid.get_doc("Asset", self.asset)
		self.update_status()
	...
	def update_status(self):
		if self.repair_status == "Pending":
			vmraid.db.set_value("Asset", self.asset, "status", "Out of Order")
		else:
	...
	def before_submit(self):
		...
		if self.get("capitalize_repair_cost"):
			...
			if (
				vmraid.db.get_value("Asset", self.asset, "calculate_depreciation")
	...
	def check_repair_status(self):
		if self.repair_status == "Pending":
			vmraid.throw(_("Please update Repair Status."))
	...
	def increase_stock_quantity(self):
		stock_entry = vmraid.get_doc("Stock Entry", self.stock_entry)
	...
	def get_gl_entries(self):
		gl_entries = []
		repair_and_maintenance_account = vmraid.db.get_value(
		...
		expense_account = (
			vmraid.get_doc("Purchase Invoice", self.purchase_invoice).items[0].expense_account
		)
