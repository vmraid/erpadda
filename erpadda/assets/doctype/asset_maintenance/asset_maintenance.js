...
vmraid.ui.form.on('Asset Maintenance', {
	setup: (frm) => {
		frm.set_query("assign_to", "asset_maintenance_tasks", function(doc) {
			return {
				query: "erpadda.assets.doctype.asset_maintenance.asset_maintenance.get_team_members",
				filters: {
	...
	make_dashboard: (frm) => {
		if(!frm.is_new()) {
			vmraid.call({
				method: 'erpadda.assets.doctype.asset_maintenance.asset_maintenance.get_maintenance_log',
					...
					(r.message || []).forEach(function(d) {
						$(`<div class='row' style='margin-bottom: 10px;'>
							<div class='col-sm-3 small'>
								<a onclick="vmraid.set_route('List', 'Asset Maintenance Log',
...
vmraid.ui.form.on('Asset Maintenance Task', {
...
var get_next_due_date = function (frm, cdt, cdn) {
	...
	if (d.start_date && d.periodicity) {
		return vmraid.call({
			method: 'erpadda.assets.doctype.asset_maintenance.asset_maintenance.calculate_next_due_date',
			args: {
			...
			callback: function(r) {
				if (r.message) {
					vmraid.model.set_value(cdt, cdn, "next_due_date", r.message);
				}
				else {
					vmraid.model.set_value(cdt, cdn, "next_due_date", "");
				}
			}
		});
	}
