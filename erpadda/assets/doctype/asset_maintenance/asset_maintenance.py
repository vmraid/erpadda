...
from vmraid.utils import add_days, add_months, add_years, getdate, nowdate
...
class AssetMaintenance(Document):
	...
	def sync_maintenance_tasks(self):
		...
		asset_maintenance_logs = vmraid.get_all(
		...
		if asset_maintenance_logs:
			for asset_maintenance_log in asset_maintenance_logs:
				maintenance_log = vmraid.get_doc("Asset Maintenance Log", asset_maintenance_log.name)
				maintenance_log.db_set("maintenance_status", "Cancelled")


@vmraid.whitelist()
def assign_tasks(asset_maintenance_name, assign_to_member, maintenance_task, next_due_date):
	team_member = vmraid.db.get_value("User", assign_to_member, "email")
	args = {
...
def update_maintenance_log(asset_maintenance, item_code, item_name, task):
	asset_maintenance_log = vmraid.get_value(
		"Asset Maintenance Log",
		{
	...
	if not asset_maintenance_log:
		...
		maintenance_log = vmraid.get_doc("Asset Maintenance Log", asset_maintenance_log)
