...
import vmraid
from vmraid.utils import add_days, get_last_day, nowdate
...
class TestAssetMaintenance(unittest.TestCase):
	...
	def test_create_asset_maintenance(self):
		...
		asset_doc = vmraid.get_doc("Asset", asset_name)
		...
		if not vmraid.db.exists("Asset Maintenance", "Photocopier"):
			asset_maintenance = vmraid.get_doc(
				{
	...
	def test_create_asset_maintenance_log(self):
		if not vmraid.db.exists("Asset Maintenance Log", "Photocopier"):
			asset_maintenance_log = vmraid.get_doc(
				{
...
def create_maintenance_team():
	...
	if not vmraid.db.exists("Role", "Technician"):
		vmraid.get_doc({"doctype": "Role", "role_name": "Technician"}).insert()
	...
	if not vmraid.db.exists("Asset Maintenance Team", "Team Awesome"):
		vmraid.get_doc(
			{
