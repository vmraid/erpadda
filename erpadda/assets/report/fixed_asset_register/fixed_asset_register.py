...
def get_conditions(filters):
	...
	date_field = vmraid.scrub(filters.date_based_on or "Purchase Date")
...
def get_data(filters):
	...
	group_by = vmraid.scrub(filters.get("group_by"))

	if group_by == "asset_category":
		fields = ["asset_category", "gross_purchase_amount", "opening_accumulated_depreciation"]
		assets_record = vmraid.db.get_all("Asset", filters=conditions, fields=fields, group_by=group_by)
		...
		assets_record = vmraid.db.get_all("Asset", filters=conditions, fields=fields)
...
def prepare_chart_data(data, filters):
	labels_values_map = {}
	date_field = vmraid.scrub(filters.date_based_on)
	...
	for d in period_list:
		labels_values_map.setdefault(
			d.get("label"), vmraid._dict({"asset_value": 0, "depreciated_amount": 0})
		)
		...
		return [
			{
				...
				"fieldtype": "Link",
				"fieldname": vmraid.scrub(filters.get("group_by")),
