...
from erpadda.projects.report.employee_hours_utilization_based_on_timesheet.employee_hours_utilization_based_on_timesheet import (
	execute,
)
...
class TestEmployeeUtilization(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		vmraid.db.set_value("HR Settings", "HR Settings", "standard_working_hours", 9)

	@classmethod
	def create_test_timesheets(cls):
		timesheet1 = vmraid.new_doc("Timesheet")
		...
		timesheet2 = vmraid.new_doc("Timesheet")
	...
	def tearDownClass(cls):
		...
		vmraid.db.sql("DELETE FROM `tabTimesheet` WHERE company='_Test Company'")
		vmraid.db.sql(f"DELETE FROM `tabProject` WHERE name='{cls.test_project.name}'")
	...
	def test_utilization_report_for_single_employee(self):
		...
		emp1_data = vmraid.get_doc("Employee", self.test_emp1)
		expected_data = [
			{
				"employee": self.test_emp1,
	...
	def test_utilization_report_for_project(self):
		...
		emp2_data = vmraid.get_doc("Employee", self.test_emp2)
		expected_data = [
			{
