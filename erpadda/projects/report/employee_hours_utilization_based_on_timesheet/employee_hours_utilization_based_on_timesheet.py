...
class EmployeeHoursReport:
	...
	def validate_dates(self):
		...
		if self.day_span <= 0:
			vmraid.throw(_("From Date must come before To Date"))

	def validate_standard_working_hours(self):
		self.standard_working_hours = vmraid.db.get_single_value("HR Settings", "standard_working_hours")
		if not self.standard_working_hours:
			msg = _(
				...
				vmraid.bold("Standard Working Hours"),
				vmraid.utils.get_link_to_form("HR Settings", "HR Settings"),
			)
	...
	def generate_filtered_time_logs(self):
		...
		self.filtered_time_logs = vmraid.db.sql(
			"""
	...
	def generate_stats_by_employee(self):
		self.stats_by_employee = vmraid._dict()

		for emp, hours, is_billable, project in self.filtered_time_logs:
			self.stats_by_employee.setdefault(emp, vmraid._dict()).setdefault("billed_hours", 0.0)
	...
	def set_employee_department_and_name(self):
		for emp in self.stats_by_employee:
			emp_name = vmraid.db.get_value("Employee", emp, "employee_name")
			emp_dept = vmraid.db.get_value("Employee", emp, "department")
