...
from vmraid.desk.form.assign_to import clear, close_all_assignments
from vmraid.model.mapper import get_mapped_doc
from vmraid.utils import add_days, cstr, date_diff, flt, get_link_to_form, getdate, today
from vmraid.utils.nestedset import NestedSet


class CircularReferenceError(vmraid.ValidationError):
	pass


class EndDateCannotBeGreaterThanProjectEndDateError(vmraid.ValidationError):
	pass
...
class Task(NestedSet):
	...
	def get_customer_details(self):
		cust = vmraid.db.sql("select customer_name from `tabCustomer` where name=%s", self.customer)
		if cust:
	...
	def validate_dates(self):
		if (
			...
			vmraid.throw(
				_("{0} can not be greater than {1}").format(
					vmraid.bold("Expected Start Date"), vmraid.bold("Expected End Date")
				)
			)
		...
		if (
			...
			vmraid.throw(
				_("{0} can not be greater than {1}").format(
					vmraid.bold("Actual Start Date"), vmraid.bold("Actual End Date")
				)
			)

	def validate_parent_expected_end_date(self):
		if self.parent_task:
			parent_exp_end_date = vmraid.db.get_value("Task", self.parent_task, "exp_end_date")
	...
	def validate_parent_project_dates(self):
		if not self.project or vmraid.flags.in_test:
			return

		expected_end_date = vmraid.db.get_value("Project", self.project, "expected_end_date")
	...
	def validate_status(self):
		...
		if self.status != self.get_db_value("status") and self.status == "Completed":
			for d in self.depends_on:
				if vmraid.db.get_value("Task", d.task, "status") not in ("Completed", "Cancelled"):
					vmraid.throw(
						_(
							"Cannot complete task {0} as its dependant task {1} are not ccompleted / cancelled."
						).format(vmraid.bold(self.name), vmraid.bold(d.task))
					)
	...
	def validate_progress(self):
		if flt(self.progress or 0) > 100:
			vmraid.throw(_("Progress % for a task cannot be more than 100."))
	...
	def validate_parent_template_task(self):
		if self.parent_task:
			if not vmraid.db.get_value("Task", self.parent_task, "is_template"):
				parent_task_format = """<a href="#Form/Task/{0}">{0}</a>""".format(self.parent_task)
				vmraid.throw(_("Parent Task {0} is not a Template Task").format(parent_task_format))

	def validate_depends_on_tasks(self):
		if self.depends_on:
			for task in self.depends_on:
				if not vmraid.db.get_value("Task", task.task, "is_template"):
					dependent_task_format = """<a href="#Form/Task/{0}">{0}</a>""".format(task.task)
					vmraid.throw(_("Dependent Task {0} is not a Template Task").format(dependent_task_format))

	def validate_completed_on(self):
		if self.completed_on and getdate(self.completed_on) > getdate():
			vmraid.throw(_("Completed On cannot be greater than Today"))
	...
	def update_total_expense_claim(self):
		self.total_expense_claim = vmraid.db.sql(
	...
	def update_project(self):
		if self.project and not self.flags.from_project:
			vmraid.get_cached_doc("Project", self.project).update_project()
	...
	def check_recursion(self):
		...
		for d in check_list:
			...
			while len(task_list) > count:
				...
				for b in tasks:
					if b[0] == self.name:
						vmraid.throw(_("Circular Reference Error"), CircularReferenceError)
					if b[0]:
	...
	def reschedule_dependent_tasks(self):
		...
		if end_date:
			for task_name in vmraid.db.sql(
				"""
			...
			""",
				...
				task = vmraid.get_doc("Task", task_name.name)
				if (
					task.exp_start_date
	...
	def has_webform_permission(self):
		project_user = vmraid.db.get_value(
			"Project User", {"parent": self.project, "user": vmraid.session.user}, "user"
		)
		if project_user:
			return True
	...
	def populate_depends_on(self):
		if self.parent_task:
			parent = vmraid.get_doc("Task", self.parent_task)
...
def check_if_child_exists(name):
	child_tasks = vmraid.get_all("Task", filters={"parent_task": name})
	...
	for task in tasks:
		if task.status == "Pending Review":
			if getdate(task.review_date) > getdate(today()):
				continue
		vmraid.get_doc("Task", task.name).update_status()
...
def add_multiple_tasks(data, parent):
	...
	new_doc["project"] = vmraid.db.get_value("Task", {"name": parent}, "project") or ""

	for d in data:
		if not d.get("subject"):
			continue
		new_doc["subject"] = d.get("subject")
		new_task = vmraid.get_doc(new_doc)
		new_task.insert()


def on_doctype_update():
	vmraid.db.add_index("Task", ["lft", "rgt"])
