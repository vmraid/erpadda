...
from vmraid.utils import add_days, flt, get_datetime, get_time, get_url, nowtime, today

from erpadda.controllers.employee_boarding_controller import update_employee_boarding_status
from erpadda.controllers.queries import get_filters_cond
from erpadda.education.doctype.student_attendance.student_attendance import get_holiday_list
from erpadda.hr.doctype.daily_work_summary.daily_work_summary import get_users_email
from erpadda.hr.doctype.holiday_list.holiday_list import is_holiday


class Project(Document):
	def get_feed(self):
		return "{0}: {1}".format(_(self.status), vmraid.safe_decode(self.project_name))
	...
	def copy_from_template(self):
		...
		"""
		if self.project_template and not vmraid.db.get_all("Task", dict(project=self.name), limit=1):
			...
			template = vmraid.get_doc("Project Template", self.project_template)
			...
			for task in template.tasks:
				template_task_details = vmraid.get_doc("Task", task.task)
	...
	def dependency_mapping(self, template_tasks, project_tasks):
		for template_task in template_tasks:
			...
			project_task = vmraid.get_doc("Task", project_task.name)
	...
	def check_depends_on_value(self, template_task, project_task, project_tasks):
		if template_task.get("depends_on") and not project_task.get("depends_on"):
			for child_task in template_task.get("depends_on"):
				child_task_subject = vmraid.db.get_value("Task", child_task.task, "subject")
	...
	def check_for_parent_tasks(self, template_task, project_task, project_tasks):
		if template_task.get("parent_task") and not project_task.get("parent_task"):
			parent_task_subject = vmraid.db.get_value("Task", template_task.get("parent_task"), "subject")
			corresponding_project_task = list(
	...
	def after_insert(self):
		self.copy_from_template()
		if self.sales_order:
			vmraid.db.set_value("Sales Order", self.sales_order, "project", self.name)

	def on_trash(self):
		vmraid.db.set_value("Sales Order", {"project": self.name}, "project", "")
	...
	def update_percent_complete(self):
		...
		total = vmraid.db.count("Task", dict(project=self.name))
		...
		if not total:
			...
			if self.percent_complete_method == "Task Weight" and total > 0:
				weight_sum = vmraid.db.sql(
					"""select sum(task_weight) from tabTask where
					project=%s""",
					self.name,
				)[0][0]
				weighted_progress = vmraid.db.sql(
				...
				for row in weighted_progress:
					pct_complete += row["progress"] * vmraid.utils.safe_div(row["task_weight"], weight_sum)
	...
	def update_costing(self):
		...
		from_expense_claim = vmraid.db.sql(
			"""select
	...
	def update_purchase_costing(self):
		total_purchase_cost = vmraid.db.sql(
	...
	def update_sales_amount(self):
		total_sales_amount = vmraid.db.sql(
	...
	def update_billed_amount(self):
		total_billed_amount = vmraid.db.sql(
			"""select sum(base_net_total)
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			vmraid.db.set_value("Project", new_name, "copied_from", new_name)
...
def get_cost_center_name(project):
	return vmraid.db.get_value("Project", project, "cost_center")
...
def create_duplicate_project(prev_doc, project_name):
	...
	if project_name == prev_doc.get("name"):
		vmraid.throw(_("Use a name that is different from previous project name"))

	# change the copied doc name to new project name
	project = vmraid.copy_doc(prev_doc)
	...
	task_list = vmraid.get_all("Task", filters={"project": prev_doc.get("name")}, fields=["name"])

	# Create duplicate task for all the task
	for task in task_list:
		task = vmraid.get_doc("Task", task)
		new_task = vmraid.copy_doc(task)
...
def send_project_update_email_to_users(project):
	...
	incoming_email_account = vmraid.db.get_value(
...
def collect_project_status():
	for data in vmraid.get_all("Project Update", {"date": today(), "sent": 0}):
		replies = vmraid.get_all(
			"Communication",
		...
		for d in replies:
			doc = vmraid.get_doc("Project Update", data.name)
			user_data = vmraid.db.get_values(
			...
			doc.append(
				"users",
				{
					...
					"project_status": vmraid.utils.md_to_html(
...
def send_project_status_email_to_users():
	...
	for d in vmraid.get_all("Project Update", {"date": yesterday, "sent": 0}):
		doc = vmraid.get_doc("Project Update", d.name)

		project_doc = vmraid.get_doc("Project", doc.project)
...
def update_project_sales_billing():
	sales_update_frequency = vmraid.db.get_single_value("Selling Settings", "sales_update_frequency")
	if sales_update_frequency == "Each Transaction":
		return
	elif sales_update_frequency == "Monthly" and vmraid.utils.now_datetime().day != 1:
		return
	...
	project_map = {}
	for project_details in vmraid.db.sql(
		"""
		...
		project = project_map.setdefault(
			project_details.name, vmraid.get_doc("Project", project_details.name)
		)
...
def create_kanban_board_if_not_exists(project):
	from vmraid.desk.doctype.kanban_board.kanban_board import quick_kanban_board

	project = vmraid.get_doc("Project", project)
	if not vmraid.db.exists("Kanban Board", project.project_name):
...
def set_project_status(project, status):
	...
	if not status in ("Completed", "Cancelled"):
		vmraid.throw(_("Status must be Cancelled or Completed"))

	project = vmraid.get_doc("Project", project)
	vmraid.has_permission(doc=project, throw=True)

	for task in vmraid.get_all("Task", dict(project=project.name)):
		vmraid.db.set_value("Task", task.name, "status", status)
