...
import vmraid
from vmraid.utils import add_days, getdate, nowdate

from erpadda.projects.doctype.project_template.test_project_template import make_project_template
from erpadda.projects.doctype.task.test_task import create_task
from erpadda.selling.doctype.sales_order.sales_order import make_project as make_project_from_so
from erpadda.selling.doctype.sales_order.test_sales_order import make_sales_order

test_records = vmraid.get_test_records("Project")
...
class TestProject(unittest.TestCase):
	def test_project_with_template_having_no_parent_and_depend_tasks(self):
		...
		vmraid.db.sql(""" delete from tabTask where project = %s """, project_name)
		vmraid.delete_doc("Project", project_name)
	...
	def test_project_template_having_parent_child_tasks(self):
		...
		if vmraid.db.get_value("Project", {"project_name": project_name}, "name"):
			project_name = vmraid.db.get_value("Project", {"project_name": project_name}, "name")
...
def make_project(args):
	...
	if args.project_name and vmraid.db.exists("Project", {"project_name": args.project_name}):
		return vmraid.get_doc("Project", {"project_name": args.project_name})
...
def task_exists(subject):
	result = vmraid.db.get_list("Task", filters={"subject": subject}, fields=["name"])
	if not len(result):
		return False
	return vmraid.get_doc("Task", result[0].name)
