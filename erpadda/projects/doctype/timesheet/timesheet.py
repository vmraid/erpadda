...
from vmraid.utils import add_to_date, flt, get_datetime, getdate, time_diff_in_hours
...
class OverWorkLoggedError(vmraid.ValidationError):
	pass
...
class Timesheet(Document):
	...
	def validate_mandatory_fields(self):
		for data in self.time_logs:
			if not data.from_time and not data.to_time:
				vmraid.throw(_("Row {0}: From Time and To Time is mandatory.").format(data.idx))

			if not data.activity_type and self.employee:
				vmraid.throw(_("Row {0}: Activity Type is mandatory.").format(data.idx))

			if flt(data.hours) == 0.0:
				vmraid.throw(_("Row {0}: Hours value must be greater than zero.").format(data.idx))
	...
	def update_task_and_project(self):
		...
		for data in self.time_logs:
			if data.task and data.task not in tasks:
				task = vmraid.get_doc("Task", data.task)
				task.update_time_and_costing()
				task.save()
				tasks.append(data.task)

			elif data.project and data.project not in projects:
				vmraid.get_doc("Project", data.project).update_project()
	...
	def validate_overlap(self, data):
		settings = vmraid.get_single("Projects Settings")
		self.validate_overlap_for("user", data, self.user, settings.ignore_user_time_overlap)
		self.validate_overlap_for("employee", data, self.employee, settings.ignore_employee_time_overlap)

	def set_project(self, data):
		data.project = data.project or vmraid.db.get_value("Task", data.task, "project")
	...
	def get_overlap_for(self, fieldname, args, value):
		timesheet = vmraid.qb.DocType("Timesheet")
		timelog = vmraid.qb.DocType("Timesheet Detail")

		from_time = get_datetime(args.from_time)
		to_time = get_datetime(args.to_time)

		existing = (
			vmraid.qb.from_(timesheet)
			.join(timelog)
...
def get_projectwise_timesheet_data(project=None, parent=None, from_time=None, to_time=None):
	...
	return vmraid.db.sql(query, filters, as_dict=1)


@vmraid.whitelist()
def get_timesheet_detail_rate(timelog, currency):
	timelog_detail = vmraid.db.sql(
...
def make_sales_invoice(source_name, item_code=None, customer=None, currency=None):
	target = vmraid.new_doc("Sales Invoice")
	timesheet = vmraid.get_doc("Timesheet", source_name)

	if not timesheet.total_billable_hours:
		vmraid.throw(_("Invoice can't be made for zero billing hour"))

	if timesheet.total_billable_hours == timesheet.total_billed_hours:
		vmraid.throw(_("Invoice already created for all billing hours"))
...
def make_salary_slip(source_name, target_doc=None):
	target = vmraid.new_doc("Salary Slip")
...
def set_missing_values(time_sheet, target):
	doc = vmraid.get_doc("Timesheet", time_sheet)
...
def get_timesheets_list(
	...
	contact = vmraid.db.exists("Contact", {"user": user})
	if contact:
		# find customer
	...
	if customer:
		sales_invoices = [
			d.name for d in vmraid.get_all("Sales Invoice", filters={"customer": customer})
		] or [None]
		projects = [d.name for d in vmraid.get_all("Project", filters={"customer": customer})]
