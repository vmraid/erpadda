...
import vmraid
from vmraid.utils import add_months, add_to_date, now_datetime, nowdate
...
class TestTimesheet(unittest.TestCase):
	...
	def test_salary_slip_from_timesheet(self):
		...
		timesheet = vmraid.get_doc("Timesheet", timesheet.name)
	...
	def test_timesheet_billing_based_on_project(self):
		...
		ts = vmraid.get_doc("Timesheet", timesheet.name)
	...
	def test_timesheet_time_overlap(self):
		...
		timesheet = vmraid.new_doc("Timesheet")
		...
		self.assertRaises(vmraid.ValidationError, timesheet.save)
...
def make_salary_structure_for_timesheet(employee, company=None):
	...
	if not vmraid.db.get_value("Salary Structure Assignment", {"employee": employee, "docstatus": 1}):
		vmraid.db.set_value("Employee", employee, "date_of_joining", add_months(nowdate(), -5))
...
def update_activity_type(activity_type):
	activity_type = vmraid.get_doc("Activity Type", activity_type)
