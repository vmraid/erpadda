...
class OverlappingConditionError(vmraid.ValidationError):
	pass


class FromGreaterThanToError(vmraid.ValidationError):
	pass


class ManyBlankToValuesError(vmraid.ValidationError):
	pass
	...
	def add_shipping_rule_to_tax_table(self, doc, shipping_amount):
		...
		if self.shipping_rule_type == "Selling":
			...
			if not doc.meta.get_field("taxes").options == "Sales Taxes and Charges":
				vmraid.throw(_("Shipping rule only applicable for Selling"))
			...
			if not doc.meta.get_field("taxes").options == "Purchase Taxes and Charges":
				vmraid.throw(_("Shipping rule only applicable for Buying"))
