...
class TransactionExists(vmraid.ValidationError):
	pass
...
class PromotionalScheme(Document):
	def validate(self):
		if not self.selling and not self.buying:
			vmraid.throw(_("Either 'Selling' or 'Buying' must be selected"), title=_("Mandatory"))
		if not (self.price_discount_slabs or self.product_discount_slabs):
			vmraid.throw(_("Price or product discount slabs are required"))
	...
	def validate_applicable_for(self):
		if self.applicable_for:
			applicable_for = vmraid.scrub(self.applicable_for)

			if not self.get(applicable_for):
				msg = f"The field {vmraid.bold(self.applicable_for)} is required"
				vmraid.throw(_(msg))
	...
	def validate_pricing_rules(self):
		...
		docnames = vmraid.get_all("Pricing Rule", filters={"promotional_scheme": self.name})
		...
		if docnames and not transaction_exists:
			for docname in docnames:
				vmraid.delete_doc("Pricing Rule", docname.name)
	...
	def update_pricing_rules(self, pricing_rules):
		...
		for doc in docs:
			...
			if doc.get("__islocal"):
				...
				doc.save()
				vmraid.msgprint(_("Pricing Rule {0} is updated").format(doc.name))

		if count:
			vmraid.msgprint(_("New {0} pricing rules are created").format(count))

	def on_trash(self):
		for rule in vmraid.get_all("Pricing Rule", {"promotional_scheme": self.name}):
			vmraid.delete_doc("Pricing Rule", rule.name)


def raise_for_transaction_exists(name):
	msg = f"""You can't change the {vmraid.bold(_('Applicable For'))}
		because transactions are present against the Promotional Scheme {vmraid.bold(name)}. """
	msg += "Kindly disable this Promotional Scheme and create new for new Applicable For."

	vmraid.throw(_(msg), TransactionExists)
...
def _get_pricing_rules(doc, child_doc, discount_fields, rules=None):
	...
	applicable_for = vmraid.scrub(doc.get("applicable_for"))
...
def get_pricing_rule_docname(
	...
	docname = vmraid.get_all("Pricing Rule", fields=fields, filters=filters)
...
def prepare_pricing_rule(
	...
	if docname:
		pr = vmraid.get_doc("Pricing Rule", docname)
	else:
		pr = vmraid.new_doc("Pricing Rule")
...
def set_args(args, pr, doc, child_doc, discount_fields, child_doc_fields):
	...
	for field in ["items", "item_groups", "brands"]:
		...
		apply_on = vmraid.scrub(doc.get("apply_on"))
