...
from erpadda.accounts.doctype.promotional_scheme.promotional_scheme import TransactionExists
from erpadda.selling.doctype.sales_order.test_sales_order import make_sales_order


class TestPromotionalScheme(unittest.TestCase):
	def setUp(self):
		if vmraid.db.exists("Promotional Scheme", "_Test Scheme"):
			vmraid.delete_doc("Promotional Scheme", "_Test Scheme")

	def test_promotional_scheme(self):
		ps = make_promotional_scheme(applicable_for="Customer", customer="_Test Customer")
		price_rules = vmraid.get_all(
			"Pricing Rule",
			fields=["promotional_scheme_id", "name", "creation"],
			filters={"promotional_scheme": ps.name},
		)
		self.assertTrue(len(price_rules), 1)
		price_doc_details = vmraid.db.get_value(
		...
		vmraid.delete_doc("Promotional Scheme", ps.name)
		price_rules = vmraid.get_all(
			"Pricing Rule",
			fields=["promotional_scheme_id", "name"],
			filters={"promotional_scheme": ps.name},
	...
	def test_promotional_scheme_without_applicable_for(self):
		...
		price_rules = vmraid.get_all("Pricing Rule", filters={"promotional_scheme": ps.name})
...
def make_promotional_scheme(**args):
	...
	ps = vmraid.new_doc("Promotional Scheme")
	...
	if args.applicable_for:
		...
		ps.append(
			vmraid.scrub(args.applicable_for),
			{vmraid.scrub(args.applicable_for): args.get(vmraid.scrub(args.applicable_for))},
		)
