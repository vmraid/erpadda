...
erpadda.accounts.PaymentReconciliationController = class PaymentReconciliationController extends vmraid.ui.form.Controller {
	onload() {
		const default_company = vmraid.defaults.get_default('company');
		...
		this.frm.set_query('receivable_payable_account', () => {
			return {
				filters: {
					...
					"is_group": 0,
					"account_type": vmraid.boot.party_account_types[this.frm.doc.party_type]
				}
			};
		});
		...
		return this.frm.call({
			...
			callback: () => {
				if (!(this.frm.doc.payments.length || this.frm.doc.invoices.length)) {
					vmraid.throw({message: __("No Unreconciled Invoices and Payments found for this party and account")});
				} else if (!(this.frm.doc.invoices.length)) {
					vmraid.throw({message: __("No Outstanding Invoices found for this party")});
				} else if (!(this.frm.doc.payments.length)) {
					vmraid.throw({message: __("No Unreconciled Payments found for this party")});
				}
				this.frm.refresh();
			}
		});
		...
		if (show_dialog && show_dialog.length) {
			...
			const dialog = new vmraid.ui.Dialog({
				...
				primary_action: () => {
					...
					args.forEach(d => {
						vmraid.model.set_value("Payment Reconciliation Allocation", d.docname,
...
extend_cscript(cur_frm.cscript, new erpadda.accounts.PaymentReconciliationController({frm: cur_frm}));
