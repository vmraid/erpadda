...
from vmraid.utils import flt, getdate, nowdate, today

import erpadda
from erpadda.accounts.utils import get_outstanding_invoices, reconcile_against_document
from erpadda.controllers.accounts_controller import get_advance_payment_entries
...
class PaymentReconciliation(Document):
	...
	def get_jv_entries(self):
		...
		dr_or_cr = (
			...
			if erpadda.get_party_account_type(self.party_type) == "Receivable"
	...
	def get_dr_or_cr_notes(self):
		...
		""".format(
				...
				party_type_field=vmraid.scrub(self.party_type),
	...
	def check_mandatory_to_fetch(self):
		for fieldname in ["company", "party_type", "party", "receivable_payable_account"]:
			if not self.get(fieldname):
				vmraid.throw(_("Please select {0} first").format(self.meta.get_label(fieldname)))

	def validate_entries(self):
		if not self.get("invoices"):
			vmraid.throw(_("No records found in the Invoices table"))

		if not self.get("payments"):
			vmraid.throw(_("No records found in the Payments table"))

	def validate_allocation(self):
		unreconciled_invoices = vmraid._dict()
		...
		if not invoices_to_reconcile:
			vmraid.throw(_("No records found in Allocation table"))
	...
	def get_conditions(self, get_invoices=False, get_payments=False, get_return_invoices=False):
		...
		if get_invoices:
			condition += (
				" and posting_date >= {0}".format(vmraid.db.escape(self.from_invoice_date))
				if self.from_invoice_date
				else ""
			)
			condition += (
				" and posting_date <= {0}".format(vmraid.db.escape(self.to_invoice_date))
			...
			condition += (
				" and doc.posting_date >= {0}".format(vmraid.db.escape(self.from_payment_date))
				if self.from_payment_date
				else ""
			)
			condition += (
				" and doc.posting_date <= {0}".format(vmraid.db.escape(self.to_payment_date))
			...
			condition += (
				" and posting_date >= {0}".format(vmraid.db.escape(self.from_payment_date))
				if self.from_payment_date
				else ""
			)
			condition += (
				" and posting_date <= {0}".format(vmraid.db.escape(self.to_payment_date))
				if self.to_payment_date
				else ""
			)
