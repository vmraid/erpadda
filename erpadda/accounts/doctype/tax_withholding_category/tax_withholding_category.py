...
from vmraid.utils import cint, getdate
	...
	def validate_dates(self):
		...
		for d in self.get("rates"):
			if getdate(d.from_date) >= getdate(d.to_date):
				vmraid.throw(_("Row #{0}: From Date cannot be before To Date").format(d.idx))

			# validate overlapping of dates
			if last_date and getdate(d.to_date) < getdate(last_date):
				vmraid.throw(_("Row #{0}: Dates overlapping with other row").format(d.idx))

	def validate_accounts(self):
		existing_accounts = []
		for d in self.get("accounts"):
			if d.get("account") in existing_accounts:
				vmraid.throw(_("Account {0} added multiple times").format(vmraid.bold(d.get("account"))))
...
def get_party_details(inv):
	...
	if not party:
		vmraid.throw(_("Please select {0} first").format(party_type))
...
def get_party_tax_withholding_details(inv, tax_withholding_category=None):
	...
	has_pan_field = vmraid.get_meta(party_type).has_field("pan")
	...
	if not tax_withholding_category:
		...
		tax_withholding_details = vmraid.db.get_value(party_type, party, fields, as_dict=1)
	...
	if not pan_no and has_pan_field:
		pan_no = vmraid.db.get_value(party_type, party, "pan")

	# Get others suppliers with the same PAN No
	if pan_no:
		parties = vmraid.get_all(party_type, filters={"pan": pan_no}, pluck="name")
...
def get_tax_withholding_details(tax_withholding_category, posting_date, company):
	tax_withholding = vmraid.get_doc("Tax Withholding Category", tax_withholding_category)
...
def get_tax_withholding_rates(tax_withholding, posting_date):
	...
	vmraid.throw(_("No Tax Withholding data found for the current posting date."))
...
def get_lower_deduction_certificate(tax_details, pan_no):
	...
	if ldc_name:
		return vmraid.get_doc("Lower Deduction Certificate", ldc_name)
...
def get_invoice_vouchers(parties, tax_details, company, party_type="Supplier"):
	...
	filters = {
		"company": company,
		vmraid.scrub(party_type): ["in", parties],
	...
	invoices = vmraid.get_all(doctype, filters=filters, pluck="name") or [""]
...
def get_advance_vouchers(
	...
	return vmraid.get_all("GL Entry", filters=filters, distinct=1, pluck="voucher_no") or [""]
...
def get_taxes_deducted_on_advances_allocated(inv, tax_details):
	...
	if advances:
		pe = vmraid.qb.DocType("Payment Entry").as_("pe")
...
def get_deducted_tax(taxable_vouchers, tax_details):
	...
	entries = vmraid.db.get_all("GL Entry", filters, pluck=field)
	return sum(entries)
...
def get_tds_amount(ldc, parties, inv, tax_details, tax_deducted, vouchers):
	...
	supp_credit_amt = vmraid.db.get_value("Purchase Invoice", invoice_filters, field) or 0.0
	...
	if (threshold and inv.net_total >= threshold) or (
		...
		if (cumulative_threshold and supp_credit_amt >= cumulative_threshold) and cint(
			...
			net_total = vmraid.db.get_value("Purchase Invoice", invoice_filters, "sum(net_total)") or 0.0
			net_total += inv.net_total
...
def get_tds_amount_from_ldc(ldc, parties, pan_no, tax_details, posting_date, net_total):
	tds_amount = 0
	limit_consumed = vmraid.db.get_value(
		"Purchase Invoice",
...
def get_debit_note_amount(suppliers, from_date, to_date, company=None):
	...
	return vmraid.get_all("Purchase Invoice", filters, fields)[0].get("net_total") or 0.0
