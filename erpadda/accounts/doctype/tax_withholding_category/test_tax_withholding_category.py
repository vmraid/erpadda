...
class TestTaxWithholdingCategory(unittest.TestCase):
	...
	def test_single_threshold_tds(self):
		...
		gl_entries = vmraid.db.get_all("GL Entry", filters={"voucher_no": pi.name}, fields=["*"])
...
def cancel_invoices():
	purchase_invoices = vmraid.get_all(
		"Purchase Invoice",
		{
	...
	sales_invoices = vmraid.get_all(
		"Sales Invoice", {"customer": "Test TCS Customer", "docstatus": 1}, pluck="name"
	)

	for d in purchase_invoices:
		vmraid.get_doc("Purchase Invoice", d).cancel()

	for d in sales_invoices:
		vmraid.get_doc("Sales Invoice", d).cancel()


def create_purchase_invoice(**args):
	# return sales invoice doc object
	item = vmraid.db.get_value("Item", {"item_name": "TDS Item"}, "name")
...
def create_sales_invoice(**args):
	# return sales invoice doc object
	item = vmraid.db.get_value("Item", {"item_name": "TCS Item"}, "name")
...
def create_records():
	...
	for name in [
		...
		if vmraid.db.exists("Supplier", name):
			continue
	...
	for name in ["Test TCS Customer"]:
		if vmraid.db.exists("Customer", name):
			continue
	...
	# create item
	if not vmraid.db.exists("Item", "TDS Item"):
		vmraid.get_doc(
			{
				"doctype": "Item",
				"item_code": "TDS Item",
	...
	if not vmraid.db.exists("Item", "TCS Item"):
		vmraid.get_doc(
			{
				"doctype": "Item",
				"item_code": "TCS Item",
	...
	# create tds account
	if not vmraid.db.exists("Account", "TDS - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	# create tcs account
	if not vmraid.db.exists("Account", "TCS - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_tax_with_holding_category():
	...
	if not vmraid.db.exists("Tax Withholding Category", "Cumulative Threshold TDS"):
		vmraid.get_doc(
			{
	...
	if not vmraid.db.exists("Tax Withholding Category", "Cumulative Threshold TCS"):
		vmraid.get_doc(
			{
				"doctype": "Tax Withholding Category",
	...
	# Single thresold
	if not vmraid.db.exists("Tax Withholding Category", "Single Threshold TDS"):
		vmraid.get_doc(
			{
				"doctype": "Tax Withholding Category",
	...
	if not vmraid.db.exists("Tax Withholding Category", "New TDS Category"):
		vmraid.get_doc(
			{
				"doctype": "Tax Withholding Category",
	...
	if not vmraid.db.exists("Tax Withholding Category", "Test Service Category"):
		vmraid.get_doc(
			{
				"doctype": "Tax Withholding Category",
	...
	if not vmraid.db.exists("Tax Withholding Category", "Test Goods Category"):
		vmraid.get_doc(
			{
				"doctype": "Tax Withholding Category",
