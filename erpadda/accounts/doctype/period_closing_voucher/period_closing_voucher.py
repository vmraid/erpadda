...
class PeriodClosingVoucher(AccountsController):
	...
	def validate_account_head(self):
		closing_account_type = vmraid.db.get_value("Account", self.closing_account_head, "root_type")
		...
		if account_currency != company_currency:
			vmraid.throw(_("Currency of the Closing Account must be {0}").format(company_currency))

	def validate_posting_date(self):
		from erpadda.accounts.utils import get_fiscal_year, validate_fiscal_year
	...
	def get_pnl_gl_entry(self, pl_accounts):
		company_cost_center = vmraid.db.get_value("Company", self.company, "cost_center")
		gl_entries = []
