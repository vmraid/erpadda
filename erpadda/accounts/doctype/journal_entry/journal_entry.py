...
from vmraid.utils import cint, cstr, flt, fmt_money, formatdate, get_link_to_form, nowdate

import erpadda
from erpadda.accounts.deferred_revenue import get_deferred_booking_accounts
...
from erpadda.accounts.utils import (
...
class StockAccountInvalidTransaction(vmraid.ValidationError):
	pass
...
class JournalEntry(AccountsController):
	...
	def validate(self):
		...
		if not vmraid.flags.is_reverse_depr_entry:
	...
	def on_cancel(self):
		...
		from erpadda.payroll.doctype.salary_slip.salary_slip import unlink_ref_doc_from_salary_slip
	...
	def update_advance_paid(self):
		...
		for voucher_type, order_list in advance_paid.items():
			for voucher_no in list(set(order_list)):
				vmraid.get_doc(voucher_type, voucher_no).set_total_advance_paid()

	def update_status_for_full_and_final_statement(self):
		for entry in self.accounts:
			if entry.reference_type == "Full and Final Statement":
				if self.docstatus == 1:
					vmraid.db.set_value("Full and Final Statement", entry.reference_name, "status", "Paid")
				elif self.docstatus == 2:
					vmraid.db.set_value("Full and Final Statement", entry.reference_name, "status", "Unpaid")
	...
	def validate_inter_company_accounts(self):
		if (
			...
			doc = vmraid.get_doc("Journal Entry", self.inter_company_journal_entry_reference)
			account_currency = vmraid.get_cached_value("Company", self.company, "default_currency")
			previous_account_currency = vmraid.get_cached_value("Company", doc.company, "default_currency")
			if account_currency == previous_account_currency:
				if self.total_credit != doc.total_debit or self.total_debit != doc.total_credit:
					vmraid.throw(_("Total Credit/ Debit Amount should be same as linked Journal Entry"))
	...
	def apply_tax_withholding(self):
		from erpadda.accounts.report.general_ledger.general_ledger import get_account_type_map
		...
		if len(parties) > 1:
			vmraid.throw(_("Cannot apply TDS against multiple parties in one entry"))
	...
	def update_invoice_discounting(self):
		...
		for inv_disc in invoice_discounting_list:
			inv_disc_doc = vmraid.get_doc("Invoice Discounting", inv_disc)
			status = None
	...
	def unlink_advance_entry_reference(self):
		for d in self.get("accounts"):
			if d.is_advance == "Yes" and d.reference_type in ("Sales Invoice", "Purchase Invoice"):
				doc = vmraid.get_doc(d.reference_type, d.reference_name)
	...
	def unlink_asset_reference(self):
		for d in self.get("accounts"):
			if d.reference_type == "Asset" and d.reference_name:
				asset = vmraid.get_doc("Asset", d.reference_name)
	...
	def unlink_inter_company_jv(self):
		if (
			...
			vmraid.db.set_value("Journal Entry", self.name, "inter_company_journal_entry_reference", "")
	...
	def validate_party(self):
		for d in self.get("accounts"):
			account_type = vmraid.db.get_value("Account", d.account, "account_type")
			...
			if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
				...
				if d.is_advance == "Yes":
					if d.party_type == "Customer" and flt(d.debit) > 0:
						vmraid.throw(_("Row {0}: Advance against Customer must be credit").format(d.idx))
					elif d.party_type == "Supplier" and flt(d.credit) > 0:
						vmraid.throw(_("Row {0}: Advance against Supplier must be debit").format(d.idx))

	def validate_against_jv(self):
		for d in self.get("accounts"):
			if d.reference_type == "Journal Entry":
				account_root_type = vmraid.db.get_value("Account", d.account, "root_type")
				...
				if d.reference_name == self.name:
					vmraid.throw(_("You can not enter current voucher in 'Against Journal Entry' column"))

				against_entries = vmraid.db.sql(
	...
	def validate_orders(self):
		...
		for reference_name, total in self.reference_totals.items():
			...
			if reference_type in ("Sales Order", "Purchase Order"):
				order = vmraid.get_doc(reference_type, reference_name)

				if order.docstatus != 1:
					vmraid.throw(_("{0} {1} is not submitted").format(reference_type, reference_name))

				if flt(order.per_billed) >= 100:
					vmraid.throw(_("{0} {1} is fully billed").format(reference_type, reference_name))

				if cstr(order.status) == "Closed":
					vmraid.throw(_("{0} {1} is closed").format(reference_type, reference_name))
	...
	def set_against_account(self):
		...
		if self.voucher_type in ("Deferred Revenue", "Deferred Expense"):
			for d in self.get("accounts"):
				...
				d.against_account = vmraid.db.get_value(d.reference_type, d.reference_name, field)
		else:
			for d in self.get("accounts"):
				if flt(d.debit > 0):
	...
	def validate_debit_credit_amount(self):
		for d in self.get("accounts"):
			if not flt(d.debit) and not flt(d.credit):
				vmraid.throw(_("Row {0}: Both Debit and Credit values cannot be zero").format(d.idx))
	...
	def set_total_debit_credit(self):
		...
		for d in self.get("accounts"):
			if d.debit and d.credit:
				vmraid.throw(_("You cannot credit and debit same account at the same time"))
	...
	def validate_multi_currency(self):
		...
		if alternate_currency:
			if not self.multi_currency:
				vmraid.throw(_("Please check Multi Currency option to allow accounts with other currency"))
	...
	def set_exchange_rate(self):
		for d in self.get("accounts"):
			...
			if not d.exchange_rate:
				vmraid.throw(_("Row {0}: Exchange Rate is mandatory").format(d.idx))
	...
	def create_remarks(self):
		...
		if self.cheque_no:
			if self.cheque_date:
				...
				msgprint(_("Please enter Reference date"), raise_exception=vmraid.MandatoryError)
	...
	def set_print_format_fields(self):
		...
		if party_type and pay_to_recd_from:
			self.pay_to_recd_from = vmraid.db.get_value(
	...
	def validate_expense_claim(self):
		for d in self.accounts:
			if d.reference_type == "Expense Claim":
				sanctioned_amount, reimbursed_amount = vmraid.db.get_value(
	...
	def validate_credit_debit_note(self):
		if self.stock_entry:
			if vmraid.db.get_value("Stock Entry", self.stock_entry, "docstatus") != 1:
				vmraid.throw(_("Stock Entry {0} is not submitted").format(self.stock_entry))
	...
	def validate_empty_accounts_table(self):
		if not self.get("accounts"):
			vmraid.throw(_("Accounts table cannot be blank."))
...
def get_default_bank_cash_account(company, account_type=None, mode_of_payment=None, account=None):
	from erpadda.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
	...
	if not account:
		...
		if account_type == "Bank":
			account = vmraid.get_cached_value("Company", company, "default_bank_account")
			if not account:
				account_list = vmraid.get_all(
			...
			account = vmraid.get_cached_value("Company", company, "default_cash_account")
			if not account:
				account_list = vmraid.get_all(
	...
	if account:
		account_details = vmraid.db.get_value(
...
def get_payment_entry(ref_doc, args):
	cost_center = ref_doc.get("cost_center") or vmraid.get_cached_value(
	...
	party_row = je.append(
		"accounts",
		{
			...
			"account_type": vmraid.db.get_value("Account", args.get("party_account"), "account_type"),
...
def get_against_jv(doctype, txt, searchfield, start, page_len, filters):
	if not vmraid.db.has_column("Journal Entry", searchfield):
		return []
...
def get_outstanding(args):
	if not vmraid.has_permission("Account"):
		vmraid.msgprint(_("No Permission"), raise_exception=1)

	if isinstance(args, str):
		args = json.loads(args)

	company_currency = erpadda.get_company_currency(args.get("company"))

	if args.get("doctype") == "Journal Entry":
		condition = " and party=%(party)s" if args.get("party") else ""

		against_jv_amount = vmraid.db.sql(
			"""
...
def get_party_account_and_balance(company, party_type, party, cost_center=None):
	...
	return {
		...
		"account_currency": vmraid.db.get_value("Account", account, "account_currency"),
	}
...
def get_exchange_rate(
	...
	if not account_details:
		vmraid.throw(_("Please select correct account"))
	...
	if account_currency != company_currency:
		if reference_type in ("Sales Invoice", "Purchase Invoice") and reference_name:
			exchange_rate = vmraid.db.get_value(reference_type, reference_name, "conversion_rate")
...
def make_inter_company_journal_entry(name, voucher_type, company):
	journal_entry = vmraid.new_doc("Journal Entry")
