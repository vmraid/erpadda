...
from erpadda.accounts.doctype.journal_entry.journal_entry import StockAccountInvalidTransaction
from erpadda.exceptions import InvalidAccountCurrency


class TestJournalEntry(unittest.TestCase):
	def test_journal_entry_with_against_jv(self):
		jv_invoice = vmraid.copy_doc(test_records[2])
		base_jv = vmraid.copy_doc(test_records[0])
	...
	def test_jv_against_purchase_order(self):
		...
		base_jv = vmraid.copy_doc(test_records[1])
	...
	def jv_against_voucher_testcase(self, base_jv, test_voucher):
		...
		submitted_voucher = vmraid.get_doc(test_voucher.doctype, test_voucher.name)
	...
	def cancel_against_voucher_testcase(self, test_voucher):
		if test_voucher.doctype == "Journal Entry":
			...
			submitted_voucher = vmraid.get_doc(test_voucher.doctype, test_voucher.name)
			self.assertRaises(vmraid.LinkExistsError, submitted_voucher.cancel)
	...
	def test_inter_company_jv(self):
		vmraid.db.set_value("Account", "Sales Expenses - _TC", "inter_company_account", 1)
		vmraid.db.set_value("Account", "Buildings - _TC", "inter_company_account", 1)
		vmraid.db.set_value("Account", "Sales Expenses - _TC1", "inter_company_account", 1)
		vmraid.db.set_value("Account", "Buildings - _TC1", "inter_company_account", 1)
		jv = make_journal_entry(
			"Sales Expenses - _TC",
			"Buildings - _TC",
			100,
	...
	def test_jv_with_project(self):
		...
		if not vmraid.db.exists("Project", {"project_name": "Journal Entry Project"}):
			...
			project_name = vmraid.get_value("Project", {"project_name": "_Test Project"})
...
test_records = vmraid.get_test_records("Journal Entry")
