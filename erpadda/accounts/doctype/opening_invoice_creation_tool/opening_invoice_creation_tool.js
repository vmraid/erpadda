...
vmraid.ui.form.on('Opening Invoice Creation Tool', {
	setup: function(frm) {
		...
		vmraid.realtime.on('opening_invoice_creation_progress', data => {
			if (!frm.doc.import_in_progress) {
				frm.dashboard.reset();
				frm.doc.import_in_progress = true;
			}
			if (data.user != vmraid.session.user) return;
			if (data.count == data.total) {
				setTimeout((title) => {
					...
					vmraid.msgprint(__("Opening {0} Invoice created", [frm.doc.invoice_type]));
				}, 1500, data.title);
				return;
			}
			...
			return frm.call({
				...
				callback: function(r) {
					if (r.message.length == 1) {
						vmraid.msgprint(__("{0} Invoice created successfully.", [frm.doc.invoice_type]));
					} else if (r.message.length < 50) {
						vmraid.msgprint(__("{0} Invoices created successfully.", [frm.doc.invoice_type]));
					}
				}
			});
		});
	...
	company: function(frm) {
		if (frm.doc.company) {
			...
			vmraid.call({
				method: 'erpadda.accounts.doctype.opening_invoice_creation_tool.opening_invoice_creation_tool.get_temporary_opening_account',
				args: {
	...
	make_dashboard: function(frm) {
		...
		if(!$.isEmptyObject(opening_invoices_summary)) {
			let section = frm.dashboard.add_section(
				vmraid.render_template('opening_invoice_creation_tool_dashboard', {
...
vmraid.ui.form.on('Opening Invoice Creation Tool Item', {
	invoices_add: (frm) => {
		frm.trigger('update_invoice_table');
	}
});
