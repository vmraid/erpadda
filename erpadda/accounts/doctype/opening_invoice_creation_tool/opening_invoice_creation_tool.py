...
class OpeningInvoiceCreationTool(Document):
	...
	def get_opening_invoice_summary(self):
		...
		companies = vmraid.get_all("Company", fields=["name as company", "default_currency as currency"])
		if not companies:
			return None, None
	...
	def validate_company(self):
		if not self.company:
			vmraid.throw(_("Please select the Company"))
	...
	def validate_mandatory_invoice_fields(self, row):
		if not vmraid.db.exists(row.party_type, row.party):
			if self.create_missing_party:
				self.add_party(row.party_type, row.party)
			else:
				vmraid.throw(
					_("Row #{}: {} {} does not exist.").format(
						row.idx, vmraid.bold(row.party_type), vmraid.bold(row.party)
					)
				)
		...
		for d in ("Party", "Outstanding Amount", "Temporary Opening Account"):
			if not row.get(scrub(d)):
				vmraid.throw(mandatory_error_msg.format(row.idx, d, self.invoice_type))
	...
	def add_party(self, party_type, party):
		party_doc = vmraid.new_doc(party_type)
		if party_type == "Customer":
			party_doc.customer_name = party
		else:
			supplier_group = vmraid.db.get_single_value("Buying Settings", "supplier_group")
			if not supplier_group:
				vmraid.throw(_("Please Set Supplier Group in Buying Settings."))
	...
	def get_invoice_dict(self, row=None):
		def get_item_dict():
			cost_center = row.get("cost_center") or vmraid.get_cached_value(
				"Company", self.company, "cost_center"
			)
			if not cost_center:
				vmraid.throw(
					_("Please set the Default Cost Center in {0} company.").format(vmraid.bold(self.company))
				)

			income_expense_account_field = (
				"income_account" if row.party_type == "Customer" else "expense_account"
			)
			default_uom = vmraid.db.get_single_value("Stock Settings", "stock_uom") or _("Nos")
			rate = flt(row.outstanding_amount) / flt(row.qty)

			item_dict = vmraid._dict(
				{
					"uom": default_uom,
					"rate": rate or 0.0,
					"qty": row.qty,
		...
		invoice = vmraid._dict(
			{
				...
				vmraid.scrub(row.party_type): row.party,
				"is_pos": 0,
				"doctype": "Sales Invoice" if self.invoice_type == "Sales" else "Purchase Invoice",
				"update_stock": 0,  # important: https://github.com/vmraid/erpadda/pull/23559
...
def start_import(invoices):
	...
	for idx, d in enumerate(invoices):
		try:
			...
			vmraid.log_error(title="Error while creating Opening Invoice", message=message)
			vmraid.db.commit()
	if errors:
		vmraid.msgprint(
...
def get_temporary_opening_account(company=None):
	...
	accounts = vmraid.get_all("Account", filters={"company": company, "account_type": "Temporary"})
	if not accounts:
		vmraid.throw(_("Please add a Temporary Opening account in Chart of Accounts"))
