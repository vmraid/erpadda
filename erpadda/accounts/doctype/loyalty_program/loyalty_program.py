...
def get_loyalty_details(
	...
	condition = ""
	if company:
		condition = " and company=%s " % vmraid.db.escape(company)
	if not include_expired_entry:
		condition += " and expiry_date>='%s' " % expiry_date

	loyalty_point_details = vmraid.db.sql(
...
def get_loyalty_program_details_with_points(
	...
	loyalty_program = vmraid.get_doc("Loyalty Program", loyalty_program)
	lp_details.update(
...
def get_loyalty_program_details(
	...
	lp_details = vmraid._dict()

	if not loyalty_program:
		loyalty_program = vmraid.db.get_value("Customer", customer, "loyalty_program")

		if not loyalty_program and not silent:
			vmraid.throw(_("Customer isn't enrolled in any Loyalty Program"))
		elif silent and not loyalty_program:
			return vmraid._dict({"loyalty_programs": None})

	if not company:
		company = vmraid.db.get_default("company") or vmraid.get_all("Company")[0].name
...
def get_redeemption_factor(loyalty_program=None, customer=None):
	...
	if not loyalty_program:
		customer_loyalty_program = vmraid.db.get_value("Customer", customer, "loyalty_program")
		loyalty_program = customer_loyalty_program
	if loyalty_program:
		return vmraid.db.get_value("Loyalty Program", loyalty_program, "conversion_factor")
	else:
		vmraid.throw(_("Customer isn't enrolled in any Loyalty Program"))
...
def validate_loyalty_points(ref_doc, points_to_redeem):
	...
	if hasattr(ref_doc, "loyalty_program") and ref_doc.loyalty_program:
		...
		loyalty_program = vmraid.db.get_value("Customer", ref_doc.customer, ["loyalty_program"])

	if (
		loyalty_program
		and vmraid.db.get_value("Loyalty Program", loyalty_program, ["company"]) != ref_doc.company
	):
		vmraid.throw(_("The Loyalty Program isn't valid for the selected company"))
	...
	if loyalty_program and points_to_redeem:
		...
		if points_to_redeem > loyalty_program_details.loyalty_points:
			vmraid.throw(_("You don't have enought Loyalty Points to redeem"))

		loyalty_amount = flt(points_to_redeem * loyalty_program_details.conversion_factor)

		if loyalty_amount > ref_doc.grand_total:
			vmraid.throw(_("You can't redeem Loyalty Points having more value than the Grand Total."))
