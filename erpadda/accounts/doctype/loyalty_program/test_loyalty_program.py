...
import vmraid
from vmraid.utils import cint, flt, getdate, today

from erpadda.accounts.doctype.loyalty_program.loyalty_program import (
	get_loyalty_program_details_with_points,
)
from erpadda.accounts.party import get_dashboard_info
...
class TestLoyaltyProgram(unittest.TestCase):
	...
	def test_loyalty_points_earned_single_tier(self):
		...
		customer = vmraid.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
	...
	def test_loyalty_points_earned_multiple_tier(self):
		...
		customer = vmraid.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
		customer.loyalty_program = vmraid.get_doc(
	...
	def test_cancel_sales_invoice(self):
		...
		si.cancel()
		lpe = vmraid.db.exists("Loyalty Point Entry", lpe.name)
	...
	def test_sales_invoice_return(self):
		...
		si_original = vmraid.get_doc("Sales Invoice", lpe_original.invoice)

		earned_points = get_points_earned(si_original)
		lpe_after_return = vmraid.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
		...
		for d in [si_return, si_original]:
			try:
				d.cancel()
			except vmraid.TimestampMismatchError:
				vmraid.get_doc("Sales Invoice", d.name).cancel()

	def test_loyalty_points_for_dashboard(self):
		doc = vmraid.get_doc("Customer", "Test Loyalty Customer")
	...
	return vmraid.get_doc(
		{
			...
			"customer": vmraid.get_doc("Customer", {"customer_name": "Test Loyalty Customer"}).name,
			...
			"items": [
				{
					...
					"item_code": vmraid.get_doc("Item", {"item_name": "Loyal Item"}).name,
					"qty": qty,
					"rate": 10000,
...
def create_records():
	...
	if not vmraid.db.exists("Account", "Loyalty - _TC"):
		vmraid.get_doc(
			{
				"doctype": "Account",
	...
	if not vmraid.db.exists("Loyalty Program", "Test Single Loyalty"):
		vmraid.get_doc(
			{
	...
	if not vmraid.db.exists("Customer", "Test Loyalty Customer"):
		vmraid.get_doc(
			{
	...
	if not vmraid.db.exists("Loyalty Program", "Test Multiple Loyalty"):
		vmraid.get_doc(
			{
				"doctype": "Loyalty Program",
	...
	# create an item
	if not vmraid.db.exists("Item", "Loyal Item"):
		vmraid.get_doc(
			{
				"doctype": "Item",
