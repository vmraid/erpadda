...
def validate_columns(data):
	if not data:
		vmraid.throw(_("No data found. Seems like you uploaded a blank file"))
...
def validate_company(company):
	parent_company, allow_account_creation_against_child_company = vmraid.db.get_value(
		"Company", {"name": company}, ["parent_company", "allow_account_creation_against_child_company"]
	)

	if parent_company and (not allow_account_creation_against_child_company):
		msg = _("{} is a child company.").format(vmraid.bold(company)) + " "
		msg += _("Please import accounts against parent company or enable {} in company master.").format(
			vmraid.bold("Allow Account Creation Against Child Company")
		)
		vmraid.throw(msg, title=_("Wrong Company"))

	if vmraid.db.get_all("GL Entry", {"company": company}, "name", limit=1):
		return False
...
def get_file(file_name):
	file_doc = vmraid.get_doc("File", {"file_url": file_name})
...
def generate_data_from_csv(file_doc, as_dict=False):
	...
	with open(file_path, "r") as in_file:
		...
		for row in csv_reader:
			if as_dict:
				data.append({vmraid.scrub(header): row[index] for index, header in enumerate(headers)})
			else:
				if not row[1]:
					row[1] = row[0]
					row[3] = row[2]
...
def build_forest(data):
	...
	if error_messages:
		vmraid.throw("<br>".join(error_messages))
...
def build_response_as_excel(writer):
	...
	vmraid.response["filename"] = "coa_importer_template.xlsx"
...
def download_template(file_type, template_type):
	...
	if file_type == "CSV":
		# download csv file
		vmraid.response["result"] = cstr(writer.getvalue())
		vmraid.response["type"] = "csv"
		vmraid.response["doctype"] = "Chart of Accounts Importer"
	else:
...
def validate_accounts(file_doc, extension):
	...
	for account in accounts:
		...
		if "parent_account" not in account:
			...
			vmraid.throw(msg, title=_("Parent Account Missing"))
...
def validate_missing_roots(roots):
	...
	if missing:
		vmraid.throw(_("Please add Root Account for - {0}").format(" , ".join(missing)))
...
def unset_existing_data(company):
	...
	vmraid.db.set_value("Company", company, update_values, update_values)
...
def set_default_accounts(company):
	...
	company.update(
		{
			"default_receivable_account": vmraid.db.get_value(
				"Account", {"company": company.name, "account_type": "Receivable", "is_group": 0}
			),
			"default_payable_account": vmraid.db.get_value(
