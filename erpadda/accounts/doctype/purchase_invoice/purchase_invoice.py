...
from vmraid.utils import cint, cstr, flt, formatdate, get_link_to_form, getdate, nowdate
...
from erpadda.accounts.general_ledger import (
	get_round_off_account_and_cost_center,
	make_gl_entries,
	make_reverse_gl_entries,
	merge_similar_entries,
)
from erpadda.accounts.party import get_due_date, get_party_account
from erpadda.accounts.utils import get_account_currency, get_fiscal_year
from erpadda.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
from erpadda.assets.doctype.asset_category.asset_category import get_asset_category_account
from erpadda.buying.utils import check_on_hold_or_closed_status
from erpadda.controllers.accounts_controller import validate_account_head
from erpadda.controllers.buying_controller import BuyingController
from erpadda.stock import get_warehouse_account_map
from erpadda.stock.doctype.purchase_receipt.purchase_receipt import (
	get_item_account_wise_additional_cost,
	update_billed_amount_based_on_po,
)


class WarehouseMissingError(vmraid.ValidationError):
	pass
...
class PurchaseInvoice(BuyingController):
	...
	def onload(self):
		...
		supplier_tds = vmraid.db.get_value("Supplier", self.supplier, "tax_withholding_category")
	...
	def validate_release_date(self):
		if self.release_date and getdate(nowdate()) >= getdate(self.release_date):
			vmraid.throw(_("Release date must be in the future"))

	def validate_cash(self):
		if not self.cash_bank_account and flt(self.paid_amount):
			vmraid.throw(_("Cash or Bank Account is mandatory for making payment entry"))
	...
	def set_missing_values(self, for_validate=False):
		...
		tds_category = vmraid.db.get_value("Supplier", self.supplier, "tax_withholding_category")
	...
	def check_conversion_rate(self):
		default_currency = erpadda.get_company_currency(self.company)
	...
	def validate_credit_to_acc(self):
		...
		if self.supplier and account.account_type != "Payable":
			vmraid.throw(
				...
				).format(vmraid.bold("Credit To"), vmraid.bold(self.credit_to)),
				title=_("Invalid Account"),
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(vmraid.db.get_single_value("Buying Settings", "maintain_same_rate")) and not self.is_return
		):
	...
	def validate_item_code(self):
		for d in self.get("items"):
			if not d.item_code:
				vmraid.msgprint(_("Item Code required at Row No {0}").format(d.idx), raise_exception=True)
	...
	def set_expense_account(self, for_validate=False):
		...
		for item in self.get("items"):
			...
			if item.item_code:
				asset_category = vmraid.get_cached_value("Item", item.item_code, "asset_category")
			...
			if (
				...
				and (
					not item.po_detail
					or not vmraid.db.get_value("Purchase Order Item", item.po_detail, "delivered_by_supplier")
				)
			):
				...
				if self.update_stock and item.warehouse and (not item.from_warehouse):
					if (
						...
						msg = _(
							...
							item.idx,
							vmraid.bold(warehouse_account[item.warehouse]["account"]),
							vmraid.bold(item.expense_account),
							vmraid.bold(item.warehouse),
						)
						vmraid.msgprint(msg, title=_("Expense Head Changed"))
					item.expense_account = warehouse_account[item.warehouse]["account"]
				else:
					# check if 'Stock Received But Not Billed' account is credited in Purchase receipt or not
					if item.purchase_receipt:
						negative_expense_booked_in_pr = vmraid.db.sql(
						...
						if negative_expense_booked_in_pr:
							if (
								...
								msg = _(
									...
									item.idx, vmraid.bold(stock_not_billed_account), vmraid.bold(item.purchase_receipt)
								)
								vmraid.msgprint(msg, title=_("Expense Head Changed"))
						...
						if (
							...
							msg = _(
								...
								item.idx, vmraid.bold(stock_not_billed_account), vmraid.bold(item.item_code)
							)
							msg += "<br>"
							msg += _(
	...
	def po_required(self):
		if vmraid.db.get_value("Buying Settings", None, "po_required") == "Yes":
			...
			for d in self.get("items"):
				if not d.purchase_order:
					msg = _("Purchase Order Required for item {}").format(vmraid.bold(d.item_code))
					msg += "<br><br>"
					msg += _("To submit the invoice without purchase order please set {0} as {1} in {2}").format(
						vmraid.bold(_("Purchase Order Required")),
						vmraid.bold("No"),
	...
	def pr_required(self):
		stock_items = self.get_stock_items()
		if vmraid.db.get_value("Buying Settings", None, "pr_required") == "Yes":
			...
			for d in self.get("items"):
				if not d.purchase_receipt and d.item_code in stock_items:
					msg = _("Purchase Receipt Required for item {}").format(vmraid.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase receipt please set {0} as {1} in {2}"
					).format(
						vmraid.bold(_("Purchase Receipt Required")),
						vmraid.bold("No"),
						get_link_to_form("Buying Settings", "Buying Settings", "Buying Settings"),
					)
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if d.purchase_order:
				submitted = vmraid.db.sql(
					"select name from `tabPurchase Order` where docstatus = 1 and name = %s", d.purchase_order
				)
				if not submitted:
					vmraid.throw(_("Purchase Order {0} is not submitted").format(d.purchase_order))
			if d.purchase_receipt:
				submitted = vmraid.db.sql(
					"select name from `tabPurchase Receipt` where docstatus = 1 and name = %s", d.purchase_receipt
				)
				if not submitted:
					vmraid.throw(_("Purchase Receipt {0} is not submitted").format(d.purchase_receipt))
	...
	def on_submit(self):
		...
		if self.update_stock == 1:
			...
			from erpadda.stock.doctype.serial_no.serial_no import update_serial_nos_after_submit
	...
	def get_gl_entries(self, warehouse_account=None):
		self.auto_accounting_for_stock = erpadda.is_perpetual_inventory_enabled(self.company)
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if self.update_stock:
			stock_ledger_entries = vmraid.get_all(
				"Stock Ledger Entry",
				...
				if self.update_stock and self.auto_accounting_for_stock and item.item_code in stock_items:
					...
					if flt(item.rm_supp_cost):
						...
						if not supplier_warehouse_account:
							vmraid.throw(_("Please set account in Warehouse {0}").format(self.supplier_warehouse))
						gl_entries.append(
							self.get_gl_dict(
								{
					...
					if provisional_accounting_for_non_stock_items:
						if item.purchase_receipt:
							provisional_account = vmraid.db.get_value(
							...
							if not purchase_receipt_doc:
								purchase_receipt_doc = vmraid.get_doc("Purchase Receipt", item.purchase_receipt)
								purchase_receipt_doc_map[item.purchase_receipt] = purchase_receipt_doc

							# Post reverse entry for Stock-Received-But-Not-Billed if it is booked in Purchase Receipt
							expense_booked_in_pr = vmraid.db.get_value(
								"GL Entry",
								{
									"is_cancelled": 0,
					...
					if self.update_stock and item.landed_cost_voucher_amount:
						...
						assets = vmraid.db.get_all(
							"Asset", filters={"purchase_invoice": self.name, "item_code": item.item_code}
						)
						for asset in assets:
							vmraid.db.set_value("Asset", asset.name, "gross_purchase_amount", flt(item.valuation_rate))
							vmraid.db.set_value(
	...
	def get_asset_gl_entry(self, gl_entries):
		...
		for item in self.get("items"):
			if item.is_fixed_asset:
				...
				item_exp_acc_type = vmraid.db.get_value("Account", item.expense_account, "account_type")
				...
				if not self.update_stock:
					...
					if item.item_tax_amount and not cint(erpadda.is_perpetual_inventory_enabled(self.company)):
						asset_eiiav_currency = get_account_currency(eiiav_account)
						gl_entries.append(
							self.get_gl_dict(
								{
	...
	def update_project(self):
		...
		for d in self.items:
			if d.project and d.project not in project_list:
				project = vmraid.get_doc("Project", d.project)
	...
	def validate_supplier_invoice(self):
		if self.bill_date:
			if getdate(self.bill_date) > getdate(self.posting_date):
				vmraid.throw(_("Supplier Invoice Date cannot be greater than Posting Date"))

		if self.bill_no:
			if cint(vmraid.db.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
				...
				if pi:
					pi = pi[0][0]
					vmraid.throw(_("Supplier Invoice No exists in Purchase Invoice {0}").format(pi))
	...
	def update_billing_status_in_pr(self, update_modified=True):
		...
		for pr in set(updated_pr):
			from erpadda.stock.doctype.purchase_receipt.purchase_receipt import update_billing_percentage

			pr_doc = vmraid.get_doc("Purchase Receipt", pr)
	...
	def set_tax_withholding(self):
		...
		if self.apply_tds and not self.get("tax_withholding_category"):
			self.tax_withholding_category = vmraid.db.get_value(
	...
	def update_advance_tax_references(self, cancel=0):
		for tax in self.get("advance_tax"):
			at = vmraid.qb.DocType("Advance Taxes and Charges").as_("at")

			if cancel:
				vmraid.qb.update(at).set(
...
def get_purchase_document_details(doc):
	...
	exchange_rate_map = vmraid._dict(
		vmraid.get_all(
			parent_doctype,
...
def change_release_date(name, release_date=None):
	if vmraid.db.exists("Purchase Invoice", name):
		pi = vmraid.get_doc("Purchase Invoice", name)
...
def on_doctype_update():
	vmraid.db.add_index("Purchase Invoice", ["supplier", "is_return", "return_against"])
