...
erpadda.accounts.PurchaseInvoice = class PurchaseInvoice extends erpadda.buying.BuyingController {
	setup(doc) {
	...
	refresh(doc) {
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			vmraid.model.with_doc("Supplier", me.frm.doc.supplier, function() {
				var supplier = vmraid.model.get_doc("Supplier", me.frm.doc.supplier);
	...
	unblock_invoice() {
		const me = this;
		vmraid.call({
			'method': 'erpadda.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice',
	...
	can_change_release_date(date) {
		const diff = vmraid.datetime.get_diff(date, vmraid.datetime.nowdate());
		if (diff < 0) {
			vmraid.throw(__('New release date should be in the future'));
			return false;
		} else {
			return true;
		}
	...
	make_comment_dialog_and_block_invoice(){
		...
		this.dialog.set_primary_action(__('Save'), function() {
			...
			vmraid.call({
				'method': 'erpadda.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice',
				'args': {
	...
	set_release_date(data) {
		return vmraid.call({
			'method': 'erpadda.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date',
			'args': data,
	...
	supplier() {
		...
		erpadda.utils.get_party_details(this.frm, "erpadda.accounts.party.get_party_details",
			{
	...
	make_inter_company_invoice(frm) {
		vmraid.model.open_mapped_doc({
			method: "erpadda.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm
		});
	}
	...
	on_submit() {
		$.each(this.frm.doc["items"] || [], function(i, row) {
			if(row.purchase_receipt) vmraid.model.clear_doc("Purchase Receipt", row.purchase_receipt)
		})
	}

	make_debit_note() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: cur_frm
		})
	}
};

cur_frm.script_manager.make(erpadda.accounts.PurchaseInvoice);
	...
	add_custom_buttons: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.per_received > 0) {
			frm.add_custom_button(__('Purchase Receipt'), () => {
				...
				vmraid.set_route("List", "Purchase Receipt", "List")
			}, __('View'));
		}
	},
