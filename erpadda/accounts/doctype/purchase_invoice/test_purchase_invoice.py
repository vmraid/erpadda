...
import vmraid
from vmraid.utils import add_days, cint, flt, getdate, nowdate, today
...
from erpadda.buying.doctype.purchase_order.purchase_order import get_mapped_purchase_invoice
...
from erpadda.controllers.accounts_controller import get_payment_terms
...
from erpadda.stock.doctype.stock_entry.test_stock_entry import get_qty_after_transaction
...
class TestPurchaseInvoice(unittest.TestCase):
	...
	def test_gl_entries_without_perpetual_inventory(self):
		...
		pi = vmraid.copy_doc(test_records[0])
		self.assertTrue(not cint(erpadda.is_perpetual_inventory_enabled(pi.company)))
		pi.insert()
		pi.submit()
	...
	def test_gl_entries_with_perpetual_inventory(self):
		...
		self.assertTrue(cint(erpadda.is_perpetual_inventory_enabled(pi.company)), 1)

		self.check_gle_for_pi(pi.name)

	def test_terms_added_after_save(self):
		pi = vmraid.copy_doc(test_records[1])
		pi.insert()
	...
	def test_payment_entry_unlink_against_purchase_invoice(self):
		...
		pi_doc = vmraid.get_doc("Purchase Invoice", pi_doc.name)
		pi_doc.load_from_db()
		self.assertTrue(pi_doc.status, "Paid")

		self.assertRaises(vmraid.LinkExistsError, pi_doc.cancel)
		unlink_payment_on_cancel_of_invoice()
	...
	def test_purchase_invoice_for_blocked_supplier(self):
		...
		self.assertRaises(vmraid.ValidationError, make_purchase_invoice)
	...
	def test_purchase_invoice_blocked_invoice_must_be_in_future(self):
		...
		self.assertRaises(vmraid.ValidationError, pi.save)
		pi.release_date = ""
		pi.save()
	...
	def test_purchase_invoice_temporary_blocked(self):
		...
		self.assertRaises(vmraid.ValidationError, pe.save)
	...
	def test_purchase_invoice_change_naming_series(self):
		...
		self.assertRaises(vmraid.CannotChangeConstantError, pi.save)
	...
	def test_purchase_invoice_with_subcontracted_item(self):
		wrapper = vmraid.copy_doc(test_records[0])
	...
	def test_purchase_invoice_with_advance(self):
		...
		jv = vmraid.copy_doc(jv_test_records[1])
		jv.insert()
		jv.submit()
	...
	def test_total_purchase_cost_for_project(self):
		if not vmraid.db.exists("Project", {"project_name": "_Test Project for Purchase"}):
			project = make_project({"project_name": "_Test Project for Purchase"})
		else:
			project = vmraid.get_doc("Project", {"project_name": "_Test Project for Purchase"})

		existing_purchase_cost = vmraid.db.sql(
		...
		self.assertEqual(
			vmraid.db.get_value("Project", project.name, "total_purchase_cost"),
		...
		pi.cancel()
		self.assertEqual(
			vmraid.db.get_value("Project", project.name, "total_purchase_cost"), existing_purchase_cost
		)
	...
	def test_purchase_invoice_update_stock_gl_entry_with_perpetual_inventory(self):
		...
		pi = make_purchase_invoice(
			update_stock=1,
			posting_date=vmraid.utils.nowdate(),
			posting_time=vmraid.utils.nowtime(),
	...
	def test_auto_batch(self):
		item_code = vmraid.db.get_value("Item", {"has_batch_no": 1, "create_new_batch": 1}, "name")
		...
		self.assertTrue(vmraid.db.get_value("Batch", {"item": item_code, "reference_name": pi.name}))

	def test_update_stock_and_purchase_return(self):
		actual_qty_0 = get_qty_after_transaction()

		pi = make_purchase_invoice(
			update_stock=1, posting_date=vmraid.utils.nowdate(), posting_time=vmraid.utils.nowtime()
		)
	...
	def test_subcontracting_via_purchase_invoice(self):
		from erpadda.buying.doctype.purchase_order.test_purchase_order import update_backflush_based_on
	...
	def test_rejected_serial_no(self):
		...
		self.assertEqual(
			vmraid.db.get_value("Serial No", pi.get("items")[0].serial_no, "warehouse"),
			pi.get("items")[0].warehouse,
		)

		self.assertEqual(
			vmraid.db.get_value("Serial No", pi.get("items")[0].rejected_serial_no, "warehouse"),
	...
	def test_duplicate_due_date_in_terms(self):
		...
		self.assertRaises(vmraid.ValidationError, pi.insert)

	def test_debit_note(self):
		from erpadda.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry
		from erpadda.accounts.doctype.sales_invoice.test_sales_invoice import get_outstanding_amount
	...
	def test_gain_loss_with_advance_entry(self):
		...
		original_account = vmraid.db.get_value("Company", "_Test Company", "exchange_gain_loss_account")
		vmraid.db.set_value(
		...
		vmraid.db.set_value("Company", "_Test Company", "exchange_gain_loss_account", original_account)
...
def update_tax_witholding_category(company, account):
	...
	if not vmraid.db.get_value(
		...
		tds_category = vmraid.get_doc("Tax Withholding Category", "TDS - 194 - Dividends - Individual")
...
def make_purchase_invoice(**args):
	pi = vmraid.new_doc("Purchase Invoice")
...
test_records = vmraid.get_test_records("Purchase Invoice")
