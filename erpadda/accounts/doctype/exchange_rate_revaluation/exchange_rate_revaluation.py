...
import erpadda
from erpadda.accounts.doctype.journal_entry.journal_entry import get_balance_on
...
class ExchangeRateRevaluation(Document):
	...
	def validate_mandatory(self):
		if not (self.company and self.posting_date):
			vmraid.throw(_("Please select Company and Posting Date to getting entries"))
	...
	def check_journal_entry_condition(self):
		total_debit = vmraid.db.get_value(
	...
	def get_accounts_data(self, account=None):
		...
		precision = get_field_precision(
			vmraid.get_meta("Exchange Rate Revaluation Account").get_field("new_balance_in_base_currency"),
			company_currency,
		)
	...
	def make_jv_entry(self):
		...
		unrealized_exchange_gain_loss_account = vmraid.get_cached_value(
...
def get_account_details(account, company, posting_date, party_type=None, party=None):
	account_currency, account_type = vmraid.db.get_value(
		"Account", account, ["account_currency", "account_type"]
	)
	if account_type in ["Receivable", "Payable"] and not (party_type and party):
		vmraid.throw(_("Party Type and Party is mandatory for {0} account").format(account_type))
