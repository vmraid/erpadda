...
class TestGLEntry(unittest.TestCase):
	def test_round_off_entry(self):
		vmraid.db.set_value("Company", "_Test Company", "round_off_account", "_Test Write Off - _TC")
		vmraid.db.set_value(
	...
	def test_rename_entries(self):
		...
		rename_gle_sle_docs()
		naming_series = parse_naming_series(parts=vmraid.get_meta("GL Entry").autoname.split(".")[:-1])
		...
		old_naming_series_current_value = vmraid.db.sql(
			"SELECT current from tabSeries where name = %s", naming_series
		)[0][0]

		rename_gle_sle_docs()

		new_gl_entries = vmraid.get_all(
			"GL Entry",
			fields=["name", "to_rename"],
			filters={"voucher_type": "Journal Entry", "voucher_no": je.name},
			order_by="creation",
		...
		new_naming_series_current_value = vmraid.db.sql(
			"SELECT current from tabSeries where name = %s", naming_series
		)[0][0]
