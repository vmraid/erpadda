...
import vmraid
from vmraid import ValidationError, _, scrub, throw
from vmraid.utils import cint, comma_or, flt, getdate, nowdate

import erpadda
from erpadda.accounts.doctype.bank_account.bank_account import (
	get_bank_account_details,
	get_party_bank_account,
)
from erpadda.accounts.doctype.invoice_discounting.invoice_discounting import (
...
from erpadda.accounts.general_ledger import make_gl_entries, process_gl_map
from erpadda.accounts.party import get_party_account
from erpadda.accounts.utils import get_account_currency, get_balance_on, get_outstanding_invoices
from erpadda.controllers.accounts_controller import (
	AccountsController,
	get_supplier_block_status,
	validate_taxes_and_charges,
)
from erpadda.hr.doctype.expense_claim.expense_claim import update_reimbursed_amount
...
class PaymentEntry(AccountsController):
	...
	def on_submit(self):
		if self.difference_amount:
			vmraid.throw(_("Difference Amount must be zero"))
	...
	def set_payment_req_status(self):
		from erpadda.accounts.doctype.payment_request.payment_request import update_payment_req_status
	...
	def delink_advance_entry_references(self):
		for reference in self.references:
			if reference.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				doc = vmraid.get_doc(reference.reference_doctype, reference.reference_name)
	...
	def set_missing_values(self):
		if self.payment_type == "Internal Transfer":
			...
			if not self.party_type:
				vmraid.throw(_("Party Type is mandatory"))

			if not self.party:
				vmraid.throw(_("Party is mandatory"))

			_party_name = (
				"title" if self.party_type in ("Student", "Shareholder") else self.party_type.lower() + "_name"
			)
			self.party_name = vmraid.db.get_value(self.party_type, self.party, _party_name)
	...
	def validate_payment_type(self):
		if self.payment_type not in ("Receive", "Pay", "Internal Transfer"):
			vmraid.throw(_("Payment Type must be one of Receive, Pay and Internal Transfer"))

	def validate_party_details(self):
		if self.party:
			if not vmraid.db.exists(self.party_type, self.party):
				vmraid.throw(_("Invalid {0}: {1}").format(self.party_type, self.party))

			if self.party_account and self.party_type in ("Customer", "Supplier"):
				self.validate_account_type(
					self.party_account, [erpadda.get_party_account_type(self.party_type)]
				)
	...
	def validate_account_type(self, account, account_types):
		account_type = vmraid.db.get_value("Account", account, "account_type")
		# if account_type not in account_types:
		# 	vmraid.throw(_("Account Type for {0} must be {1}").format(account, comma_or(account_types)))
	...
	def validate_mandatory(self):
		for field in ("paid_amount", "received_amount", "source_exchange_rate", "target_exchange_rate"):
			if not self.get(field):
				vmraid.throw(_("{0} is mandatory").format(self.meta.get_label(field)))
	...
	def validate_reference_documents(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not vmraid.db.exists(d.reference_doctype, d.reference_name):
					vmraid.throw(_("{0} {1} does not exist").format(d.reference_doctype, d.reference_name))
				else:
					ref_doc = vmraid.get_doc(d.reference_doctype, d.reference_name)
					...
					if ref_doc.docstatus != 1:
						vmraid.throw(_("{0} {1} must be submitted").format(d.reference_doctype, d.reference_name))
	...
	def validate_paid_invoices(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype in ("Sales Invoice", "Purchase Invoice", "Fees"):
				outstanding_amount, is_return = vmraid.get_cached_value(
		...
		for k, v in no_oustanding_refs.items():
			vmraid.msgprint(
				_(
					...
					_(k),
					vmraid.bold(", ".join(d.reference_name for d in v)),
					vmraid.bold(_("negative outstanding amount")),
				)
				+ "<br><br>"
	...
	def update_payment_schedule(self, cancel=0):
		...
		for ref in self.get("references"):
			if ref.payment_term and ref.reference_name:
				...
				if not invoice_paid_amount_map.get(key):
					payment_schedule = vmraid.get_all(
						"Payment Schedule",
		...
		for idx, (key, allocated_amount) in enumerate(invoice_payment_amount_map.items(), 1):
			if not invoice_paid_amount_map.get(key):
				vmraid.throw(_("Payment term {0} not used in {1}").format(key[0], key[1]))
	...
	def validate_received_amount(self):
		if self.paid_from_account_currency == self.paid_to_account_currency:
			if self.paid_amount < self.received_amount:
				vmraid.throw(_("Received Amount cannot be greater than Paid Amount"))
	...
	def set_title(self):
		if vmraid.flags.in_import and self.title:
	...
	def validate_transaction_reference(self):
		...
		bank_account_type = vmraid.db.get_value("Account", bank_account, "account_type")

		if bank_account_type == "Bank":
			if not self.reference_no or not self.reference_date:
				vmraid.throw(_("Reference No and Reference Date is mandatory for Bank transaction"))
	...
	def add_party_gl_entries(self, gl_entries):
		if self.party_account:
			...
			dr_or_cr = (
				"credit" if erpadda.get_party_account_type(self.party_type) == "Receivable" else "debit"
			)

			for d in self.get("references"):
				cost_center = self.cost_center
				if d.reference_doctype == "Sales Invoice" and not cost_center:
					cost_center = vmraid.db.get_value(d.reference_doctype, d.reference_name, "cost_center")
	...
	def add_tax_gl_entries(self, gl_entries):
		for d in self.get("taxes"):
			...
			if account_currency != self.company_currency:
				vmraid.throw(_("Currency for {0} must be {1}").format(d.account_head, self.company_currency))
	...
	def add_deductions_gl_entries(self, gl_entries):
		for d in self.get("deductions"):
			if d.amount:
				...
				if account_currency != self.company_currency:
					vmraid.throw(_("Currency for {0} must be {1}").format(d.account, self.company_currency))
		...
		if self.payment_type in ("Receive", "Pay") and self.party:
			for d in self.get("references"):
				if d.allocated_amount and d.reference_doctype in (
					...
					vmraid.get_doc(d.reference_doctype, d.reference_name).set_total_advance_paid()

	def update_expense_claim(self):
		if self.payment_type in ("Pay") and self.party:
			for d in self.get("references"):
				if d.reference_doctype == "Expense Claim" and d.reference_name:
					doc = vmraid.get_doc("Expense Claim", d.reference_name)
					if self.docstatus == 2:
...
def get_outstanding_reference_documents(args):
	...
	company_currency = vmraid.get_cached_value("Company", args.get("company"), "default_currency")

	# Get positive outstanding sales /purchase invoices/ Fees
	condition = ""
	if args.get("voucher_type") and args.get("voucher_no"):
		condition = " and voucher_type={0} and voucher_no={1}".format(
			vmraid.db.escape(args["voucher_type"]), vmraid.db.escape(args["voucher_no"])
		)
	...
	if args.get("company"):
		condition += " and company = {0}".format(vmraid.db.escape(args.get("company")))
	...
	for d in outstanding_invoices:
		...
		if party_account_currency != company_currency:
			if d.voucher_type in ("Sales Invoice", "Purchase Invoice", "Expense Claim"):
				d["exchange_rate"] = vmraid.db.get_value(d.voucher_type, d.voucher_no, "conversion_rate")
			elif d.voucher_type == "Journal Entry":
				d["exchange_rate"] = get_exchange_rate(
					party_account_currency, company_currency, d.posting_date
				)
		if d.voucher_type in ("Purchase Invoice"):
			d["bill_no"] = vmraid.db.get_value(d.voucher_type, d.voucher_no, "bill_no")
	...
	if not data:
		vmraid.msgprint(
			...
			).format(_(args.get("party_type")).lower(), vmraid.bold(args.get("party")))
		)
...
def split_invoices_based_on_payment_terms(outstanding_invoices):
	...
	for idx, d in enumerate(outstanding_invoices):
		if d.voucher_type in ["Sales Invoice", "Purchase Invoice"]:
			payment_term_template = vmraid.db.get_value(
				d.voucher_type, d.voucher_no, "payment_terms_template"
			)
			if payment_term_template:
				allocate_payment_based_on_payment_terms = vmraid.db.get_value(
...
def get_orders_to_be_billed(
	...
	# Add cost center condition
	if voucher_type:
		doc = vmraid.get_doc({"doctype": voucher_type})
		condition = ""
...
def get_party_details(company, party_type, party, date, cost_center=None):
	...
	if not vmraid.db.exists(party_type, party):
		vmraid.throw(_("Invalid {0}: {1}").format(party_type, party))
	...
	party_name = vmraid.db.get_value(party_type, party, _party_name)
...
def get_account_details(account, date, cost_center=None):
	vmraid.has_permission("Payment Entry", throw=True)

	# to check if the passed account is accessible under reference doctype Payment Entry
	account_list = vmraid.get_list(
	...
	if not account_list:
		vmraid.throw(_("Account: {0} is not permitted under Payment Entry").format(account))
...
def get_company_defaults(company):
	...
	ret = vmraid.get_cached_value("Company", company, fields, as_dict=1)
...
def get_reference_details(reference_doctype, reference_name, party_account_currency):
	...
	company_currency = ref_doc.get("company_currency") or erpadda.get_company_currency(
		ref_doc.company
	)
...
def get_payment_entry(dt, dn, party_amount=None, bank_account=None, bank_amount=None):
	...
	if dt in ("Sales Order", "Purchase Order") and flt(doc.per_billed, 2) > 0:
		vmraid.throw(_("Can only make payment against unbilled {0}").format(dt))
	...
	pe = vmraid.new_doc("Payment Entry")
	...
	if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
		vmraid.msgprint(_("{0} is on hold till {1}").format(doc.name, doc.release_date))
	else:
		if doc.doctype in ("Sales Invoice", "Purchase Invoice") and vmraid.get_value(
	...
	if party_account and bank:
		...
		if discount_amount:
			pe.set_gain_or_loss(
				account_details={
					"account": vmraid.get_cached_value("Company", pe.company, "default_discount_account"),
					"cost_center": pe.cost_center
					or vmraid.get_cached_value("Company", pe.company, "cost_center"),
...
def apply_early_payment_discount(paid_amount, received_amount, doc):
	...
	if doc.doctype in eligible_for_payments and has_payment_schedule:
		...
		if total_discount:
			money = vmraid.utils.fmt_money(total_discount, currency=doc.get("currency"))
			vmraid.msgprint(_("Discount of {} applied as per Payment Term").format(money), alert=1)
