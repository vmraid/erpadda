...
from erpadda.accounts.doctype.sales_invoice.test_sales_invoice import (
	create_sales_invoice,
	create_sales_invoice_against_cost_center,
)
from erpadda.hr.doctype.expense_claim.test_expense_claim import make_expense_claim
...
class TestPaymentEntry(unittest.TestCase):
	def test_payment_entry_against_order(self):
		...
		so_advance_paid = vmraid.db.get_value("Sales Order", so.name, "advance_paid")
	...
	def test_payment_entry_against_si_usd_to_usd(self):
		...
		outstanding_amount = flt(vmraid.db.get_value("Sales Invoice", si.name, "outstanding_amount"))
	...
	def test_payment_entry_against_pi(self):
		...
		outstanding_amount = flt(vmraid.db.get_value("Sales Invoice", pi.name, "outstanding_amount"))
		self.assertEqual(outstanding_amount, 0)
	...
	def test_payment_against_sales_invoice_to_check_status(self):
		...
		outstanding_amount, status = vmraid.db.get_value(
	...
	def test_payment_entry_against_payment_terms_with_discount(self):
		...
		vmraid.db.set_value("Company", si.company, "default_discount_account", "Write Off - _TC")
	...
	def test_payment_entry_against_ec(self):
		...
		payable = vmraid.get_cached_value("Company", "_Test Company", "default_payable_account")
		...
		outstanding_amount = flt(
			vmraid.db.get_value("Expense Claim", ec.name, "total_sanctioned_amount")
		) - flt(vmraid.db.get_value("Expense Claim", ec.name, "total_amount_reimbursed"))
		self.assertEqual(outstanding_amount, 0)
	...
	def test_payment_entry_retrieves_last_exchange_rate(self):
		from erpadda.setup.doctype.currency_exchange.test_currency_exchange import (
			save_new_records,
			test_records,
		)
	...
	def test_payment_against_negative_sales_invoice(self):
		pe1 = vmraid.new_doc("Payment Entry")
		...
		si1_outstanding = vmraid.db.get_value("Sales Invoice", si1.name, "outstanding_amount")
		...
		outstanding_amount = flt(vmraid.db.get_value("Sales Invoice", si1.name, "outstanding_amount"))
		self.assertEqual(outstanding_amount, 0)
...
def create_payment_entry(**args):
	payment_entry = vmraid.new_doc("Payment Entry")
...
def create_payment_terms_template():
	...
	if not vmraid.db.exists("Payment Terms Template", "Test Receivable Template"):
		payment_term_template = vmraid.get_doc(
			{
...
def create_payment_terms_template_with_discount():
	...
	if not vmraid.db.exists("Payment Terms Template", "Test Discount Template"):
		payment_term_template = vmraid.get_doc(
			{
				"doctype": "Payment Terms Template",
...
def create_payment_term(name):
	if not vmraid.db.exists("Payment Term", name):
		vmraid.get_doc({"doctype": "Payment Term", "payment_term_name": name}).insert()
