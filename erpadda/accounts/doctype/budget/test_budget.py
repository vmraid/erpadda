...
import vmraid
from vmraid.utils import now_datetime, nowdate

from erpadda.accounts.doctype.budget.budget import BudgetError, get_actual_expense
...
class TestBudget(unittest.TestCase):
	def test_monthly_budget_crossed_ignore(self):
		...
		self.assertTrue(
			vmraid.db.get_value("GL Entry", {"voucher_type": "Journal Entry", "voucher_no": jv.name})
		)
	...
	def test_exception_approver_role(self):
		...
		vmraid.db.set_value("Company", budget.company, "exception_budget_approver_role", "Accounts User")

		jv.submit()
		self.assertEqual(vmraid.db.get_value("Journal Entry", jv.name, "docstatus"), 1)
		jv.cancel()

		vmraid.db.set_value("Company", budget.company, "exception_budget_approver_role", "")
	...
	def test_monthly_budget_crossed_for_mr(self):
		...
		vmraid.db.set_value("Budget", budget.name, "fiscal_year", fiscal_year)
	...
	def test_monthly_budget_crossed_stop2(self):
		...
		project = vmraid.get_value("Project", {"project_name": "_Test Project"})
	...
	def test_monthly_budget_against_parent_group_cost_center(self):
		...
		if not vmraid.db.exists("Cost Center", cost_center):
			vmraid.get_doc(
				{
					"doctype": "Cost Center",
...
def set_total_expense_zero(posting_date, budget_against_field=None, budget_against_CC=None):
	if budget_against_field == "project":
		budget_against = vmraid.db.get_value("Project", {"project_name": "_Test Project"})
	else:
...
def make_budget(**args):
	...
	if budget_against == "Project":
		...
		budget_list = vmraid.get_all("Budget", fields=["name"], filters={"name": ("like", project_name)})
	else:
		cost_center_name = "{0}%".format(cost_center or "_Test Cost Center - _TC/" + fiscal_year)
		budget_list = vmraid.get_all(
			"Budget", fields=["name"], filters={"name": ("like", cost_center_name)}
		)
	for d in budget_list:
		vmraid.db.sql("delete from `tabBudget` where name = %(name)s", d)
		vmraid.db.sql("delete from `tabBudget Account` where parent = %(name)s", d)

	budget = vmraid.new_doc("Budget")

	if budget_against == "Project":
		budget.project = vmraid.get_value("Project", {"project_name": "_Test Project"})
	else:
		budget.cost_center = cost_center or "_Test Cost Center - _TC"

	monthly_distribution = vmraid.get_doc("Monthly Distribution", "_Test Distribution")
