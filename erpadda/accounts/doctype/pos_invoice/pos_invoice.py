...
from vmraid.utils import cint, flt, get_link_to_form, getdate, nowdate

from erpadda.accounts.doctype.loyalty_program.loyalty_program import validate_loyalty_points
from erpadda.accounts.doctype.payment_request.payment_request import make_payment_request
...
from erpadda.stock.doctype.batch.batch import get_batch_qty, get_pos_reserved_batch_qty
...
class POSInvoice(SalesInvoice):
	...
	def validate(self):
		if not cint(self.is_pos):
			vmraid.throw(
				_("POS Invoice should have {} field checked.").format(vmraid.bold("Include Payment"))
			)
	...
	def on_submit(self):
		...
		if not self.is_return and self.loyalty_program:
			...
			against_psi_doc = vmraid.get_doc("POS Invoice", self.return_against)
	...
	def before_cancel(self):
		if (
			...
			and vmraid.db.get_value("Sales Invoice", self.consolidated_invoice, "docstatus") == 1
		):
		...
		for pay in self.payments:
			if pay.type == "Phone" and pay.amount >= 0:
				...
				if paid_amt and pay.amount != paid_amt:
					return vmraid.throw(_("Payment related to {0} is not completed").format(pay.mode_of_payment))
	...
	def validate_pos_reserved_serial_nos(self, item):
		...
		bold_invalid_serial_nos = vmraid.bold(", ".join(invalid_serial_nos))
	...
	def validate_pos_reserved_batch_qty(self, item):
		...
		bold_item_name = vmraid.bold(item.item_name)
		bold_extra_batch_qty_needed = vmraid.bold(
			abs(available_batch_qty - reserved_batch_qty - item.qty)
		)
		bold_invalid_batch_no = vmraid.bold(item.batch_no)
	...
	def validate_delivered_serial_nos(self, item):
		...
		if delivered_serial_nos:
			bold_delivered_serial_nos = vmraid.bold(", ".join(delivered_serial_nos))
			vmraid.throw(
				_(
	...
	def validate_invalid_serial_nos(self, item):
		...
		for serial_no in serial_nos:
			if not vmraid.db.exists("Serial No", serial_no):
		...
		msg = _("Row #{}: Following Serial numbers for item {} are <b>Invalid</b>: {}").format(
			item.idx, vmraid.bold(item.get("item_code")), vmraid.bold(invalid_serials)
		)
		if invalid_serials:
			error_msg.append(msg)

		if error_msg:
			vmraid.throw(error_msg, title=_("Invalid Item"), as_list=True)

	def validate_stock_availablility(self):
		if self.is_return:
			return

		if self.docstatus.is_draft() and not vmraid.db.get_value(
		...
		for d in self.get("items"):
			is_service_item = not (vmraid.db.get_value("Item", d.get("item_code"), "is_stock_item"))
			if is_service_item:
				return
			if d.serial_no:
	...
	def validate_serialised_or_batched_item(self):
		...
		for d in self.get("items"):
			...
			msg = ""
			item_code = vmraid.bold(d.item_code)
			...
			if serialized and batched and (no_batch_selected or no_serial_selected):
				...
				msg = _("Row #{}: You must select {} serial numbers for item {}.").format(
					d.idx, vmraid.bold(cint(d.qty)), item_code
				)
		...
		for d in self.get("items"):
			if d.get("qty") > 0:
				vmraid.throw(
					...
					).format(d.idx, vmraid.bold(d.item_code)),
					title=_("Invalid Item"),
				)
			if d.get("serial_no"):
				serial_nos = get_serial_nos(d.serial_no)
				for sr in serial_nos:
					serial_no_exists = vmraid.db.sql(
						"""
						SELECT name
					...
					if not serial_no_exists:
						bold_return_against = vmraid.bold(self.return_against)
						bold_serial_no = vmraid.bold(sr)
						vmraid.throw(
							_(
	...
	def validate_change_account(self):
		if (
			...
			and vmraid.db.get_value("Account", self.account_for_change_amount, "company") != self.company
		):
			vmraid.throw(
	...
	def validate_change_amount(self):
		...
		if flt(self.change_amount) and not self.account_for_change_amount:
			vmraid.msgprint(_("Please enter Account for Change Amount"), raise_exception=1)
	...
	def validate_loyalty_transaction(self):
		if self.redeem_loyalty_points and (
			...
			expense_account, cost_center = vmraid.db.get_value(
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.pos_profile:
			...
			if not pos_profile:
				vmraid.throw(_("No POS Profile found. Please create a New POS Profile first"))
			self.pos_profile = pos_profile.get("name")

		profile = {}
		if self.pos_profile:
			profile = vmraid.get_doc("POS Profile", self.pos_profile)
		...
		if profile:
			...
			if self.customer:
				customer_price_list, customer_group, customer_currency = vmraid.db.get_value(
					"Customer", self.customer, ["default_price_list", "customer_group", "default_currency"]
				)
				customer_group_price_list = vmraid.db.get_value(
			...
			for item in self.get("items"):
				if item.get("item_code"):
					profile_details = get_pos_profile_item_details(
						profile.get("company"), vmraid._dict(item.as_dict()), profile
					)
	...
	def reset_mode_of_payments(self):
		if self.pos_profile:
			pos_profile = vmraid.get_cached_doc("POS Profile", self.pos_profile)
	...
	def create_payment_request(self):
		for pay in self.payments:
			if pay.type == "Phone":
				if pay.amount <= 0:
					vmraid.throw(_("Payment amount cannot be less than or equal to 0"))

				if not self.contact_mobile:
					vmraid.throw(_("Please enter the phone number first"))
	...
	def get_new_payment_request(self, mop):
		payment_gateway_account = vmraid.db.get_value(
	...
	def get_existing_payment_request(self, pay):
		...
		pr = vmraid.db.get_value("Payment Request", filters=filters)
		if pr:
			return vmraid.get_doc("Payment Request", pr)


@vmraid.whitelist()
def get_stock_availability(item_code, warehouse):
	if vmraid.db.get_value("Item", item_code, "is_stock_item"):
		is_stock_item = True
		...
		is_stock_item = False
		if vmraid.db.exists("Product Bundle", item_code):
...
def get_bundle_availability(bundle_item_code, warehouse):
	product_bundle = vmraid.get_doc("Product Bundle", bundle_item_code)
...
def make_merge_log(invoices):
	...
	if len(invoices) == 0:
		vmraid.throw(_("Atleast one invoice has to be selected."))

	merge_log = vmraid.new_doc("POS Invoice Merge Log")
	merge_log.posting_date = getdate(nowdate())
	for inv in invoices:
		inv_data = vmraid.db.get_values(
