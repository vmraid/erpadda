...
from erpadda.accounts.doctype.subscription.subscription import get_prorata_factor

test_dependencies = ("UOM", "Item Group", "Item")


def create_plan():
	if not vmraid.db.exists("Subscription Plan", "_Test Plan Name"):
		plan = vmraid.new_doc("Subscription Plan")
	...
	if not vmraid.db.exists("Subscription Plan", "_Test Plan Name 2"):
		plan = vmraid.new_doc("Subscription Plan")
	...
	if not vmraid.db.exists("Subscription Plan", "_Test Plan Name 3"):
		plan = vmraid.new_doc("Subscription Plan")
	...
	if not vmraid.db.exists("Subscription Plan", "_Test Plan Name 4"):
		plan = vmraid.new_doc("Subscription Plan")
	...
	if not vmraid.db.exists("Subscription Plan", "_Test Plan Multicurrency"):
		plan = vmraid.new_doc("Subscription Plan")
...
def create_parties():
	if not vmraid.db.exists("Supplier", "_Test Supplier"):
		supplier = vmraid.new_doc("Supplier")
		supplier.supplier_name = "_Test Supplier"
		supplier.supplier_group = "All Supplier Groups"
		supplier.insert()

	if not vmraid.db.exists("Customer", "_Test Subscription Customer"):
...
class TestSubscription(unittest.TestCase):
	...
	def test_create_subscription_with_trial_with_correct_period(self):
		subscription = vmraid.new_doc("Subscription")
	...
	def test_create_subscription_trial_with_wrong_dates(self):
		...
		self.assertRaises(vmraid.ValidationError, subscription.save)
		subscription.delete()
	...
	def test_subscription_cancel_after_grace_period(self):
		settings = vmraid.get_single("Subscription Settings")
	...
	def test_restart_active_subscription(self):
		...
		self.assertRaises(vmraid.ValidationError, subscription.restart_subscription)
	...
	def test_multicurrency_subscription(self):
		...
		currency = vmraid.db.get_value("Sales Invoice", subscription.invoices[0].invoice, "currency")
