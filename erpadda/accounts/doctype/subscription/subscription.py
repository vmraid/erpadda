...
class Subscription(Document):
	...
	def validate_plans_billing_cycle(billing_cycle_data):
		...
		if billing_cycle_data and len(billing_cycle_data) != 1:
			vmraid.throw(_("You can only have Plans with the same billing cycle in a Subscription"))
	...
	def set_status_grace_period(self):
		...
		"""
		subscription_settings = vmraid.get_single("Subscription Settings")
	...
	def get_current_invoice(self):
		...
		if len(self.invoices):
			...
			if vmraid.db.exists(doctype, current.get("invoice")):
				doc = vmraid.get_doc(doctype, current.get("invoice"))
				return doc
			else:
				vmraid.throw(_("Invoice {0} no longer exists").format(current.get("invoice")))
	...
	def validate(self):
		...
		self.cost_center = erpadda.get_default_cost_center(self.get("company"))
	...
	def validate_trial_period(self):
		...
		if self.trial_period_start and self.trial_period_end:
			if getdate(self.trial_period_end) < getdate(self.trial_period_start):
				vmraid.throw(_("Trial Period End Date Cannot be before Trial Period Start Date"))

		if self.trial_period_start and not self.trial_period_end:
			vmraid.throw(_("Both Trial Period Start Date and Trial Period End Date must be set"))

		if self.trial_period_start and getdate(self.trial_period_start) > getdate(self.start_date):
			vmraid.throw(_("Trial Period Start date cannot be after Subscription Start Date"))
	...
	def validate_to_follow_calendar_months(self):
		if self.follow_calendar_months:
			...
			if not self.end_date:
				vmraid.throw(_("Subscription End Date is mandatory to follow calendar months"))
	...
	def create_invoice(self, prorate):
		...
		invoice = vmraid.new_doc(doctype)
		...
		if doctype == "Sales Invoice":
			...
			if vmraid.db.get_value("Supplier", self.party, "tax_withholding_category"):
				invoice.apply_tds = 1
	...
	def get_items_from_plans(self, plans, prorate=0):
		...
		party = self.party
		for plan in plans:
			plan_doc = vmraid.get_doc("Subscription Plan", plan.plan)
			...
			deferred = vmraid.db.get_value("Item", item_code, deferred_field)
	...
	def process_for_active(self):
		...
		if not self.is_current_invoice_generated(
			...
			prorate = vmraid.db.get_single_value("Subscription Settings", "prorate")
	...
	def process_for_past_due_date(self):
		...
		current_invoice = self.get_current_invoice()
		if not current_invoice:
			vmraid.throw(_("Current invoice {0} is missing").format(current_invoice.invoice))
		else:
	...
	def has_outstanding_invoice(self):
		...
		outstanding_invoices = vmraid.get_all(
	...
	def cancel_subscription(self):
		...
		if self.status != "Cancelled":
			...
			to_prorate = vmraid.db.get_single_value("Subscription Settings", "prorate")
			self.status = "Cancelled"
			self.cancelation_date = nowdate()
	...
	def restart_subscription(self):
		...
		if self.status == "Cancelled":
			...
			vmraid.throw(_("You cannot restart a Subscription that is not cancelled."))
...
def get_all_subscriptions():
	...
	"""
	return vmraid.db.get_all("Subscription", {"status": ("!=", "Cancelled")})
...
def process(data):
	...
	if data:
		try:
			subscription = vmraid.get_doc("Subscription", data["name"])
...
def cancel_subscription(name):
	...
	"""
	subscription = vmraid.get_doc("Subscription", name)
