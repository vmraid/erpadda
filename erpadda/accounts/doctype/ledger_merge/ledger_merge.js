...
vmraid.ui.form.on('Ledger Merge', {
	setup: function(frm) {
		vmraid.realtime.on('ledger_merge_refresh', ({ ledger_merge }) => {
			if (ledger_merge !== frm.doc.name) return;
			vmraid.model.clear_doc(frm.doc.doctype, frm.doc.name);
			vmraid.model.with_doc(frm.doc.doctype, frm.doc.name).then(() => {
				frm.refresh();
			});
		});

		vmraid.realtime.on('ledger_merge_progress', data => {
		...
		frm.set_query("account", function(doc) {
			if (!doc.company) vmraid.throw(__('Please set Company'));
			if (!doc.root_type) vmraid.throw(__('Please set Root Type'));
			return {
				filters: {
		...
		frm.set_query('account', 'merge_accounts', function(doc) {
			if (!doc.company) vmraid.throw(__('Please set Company'));
			if (!doc.root_type) vmraid.throw(__('Please set Root Type'));
			if (!doc.account) vmraid.throw(__('Please set Account'));
			let acc = [doc.account];
...
vmraid.ui.form.on('Ledger Merge Accounts', {
