...
import erpadda
from erpadda.accounts.deferred_revenue import validate_service_stop_date
from erpadda.accounts.doctype.loyalty_program.loyalty_program import (
	get_loyalty_program_details_with_points,
	validate_loyalty_points,
)
from erpadda.accounts.doctype.tax_withholding_category.tax_withholding_category import (
	get_party_tax_withholding_details,
)
from erpadda.accounts.general_ledger import get_round_off_account_and_cost_center
from erpadda.accounts.party import get_due_date, get_party_account, get_party_details
from erpadda.accounts.utils import get_account_currency
from erpadda.assets.doctype.asset.depreciation import (
...
from erpadda.controllers.selling_controller import SellingController
from erpadda.projects.doctype.timesheet.timesheet import get_projectwise_timesheet_data
from erpadda.setup.doctype.company.company import update_company_current_month_sales
from erpadda.stock.doctype.batch.batch import set_batch_nos
from erpadda.stock.doctype.delivery_note.delivery_note import update_billed_amount_based_on_so
from erpadda.stock.doctype.serial_no.serial_no import (
...
class SalesInvoice(SellingController):
	...
	def validate(self):
		...
		if self.redeem_loyalty_points:
			lp = vmraid.get_doc("Loyalty Program", self.loyalty_program)
	...
	def validate_fixed_asset(self):
		for d in self.get("items"):
			if d.is_fixed_asset and d.meta.get_field("asset") and d.asset:
				asset = vmraid.get_doc("Asset", d.asset)
				if self.doctype == "Sales Invoice" and self.docstatus == 1:
					if self.update_stock:
						vmraid.throw(_("'Update Stock' cannot be checked for fixed asset sale"))
	...
	def validate_item_cost_centers(self):
		for item in self.items:
			cost_center_company = vmraid.get_cached_value("Cost Center", item.cost_center, "company")
			if cost_center_company != self.company:
				vmraid.throw(
					_("Row #{0}: Cost Center {1} does not belong to company {2}").format(
						vmraid.bold(item.idx), vmraid.bold(item.cost_center), vmraid.bold(self.company)
					)
				)
	...
	def on_submit(self):
		...
		if not self.auto_repeat:
			vmraid.get_doc("Authorization Control").validate_approving_authority(
		...
		if (
			vmraid.db.get_single_value("Selling Settings", "sales_update_frequency") == "Each Transaction"
		):
		...
		if not self.is_return and not self.is_consolidated and self.loyalty_program:
			...
			against_si_doc = vmraid.get_doc("Sales Invoice", self.return_against)
	...
	def validate_pos_return(self):
		...
		if self.is_pos and self.is_return:
			...
			if total_amount_in_payments < invoice_total:
				vmraid.throw(_("Total payments amount can't be greater than {}").format(-invoice_total))

	def validate_pos_paid_amount(self):
		if len(self.payments) == 0 and self.is_pos:
			vmraid.throw(_("At least one mode of payment is required for POS invoice."))
	...
	def check_credit_limit(self):
		from erpadda.selling.doctype.customer.customer import check_credit_limit

		validate_against_credit_limit = False
		bypass_credit_limit_check_at_sales_order = vmraid.db.get_value(
	...
	def unlink_sales_invoice_from_timesheets(self):
		for row in self.timesheets:
			timesheet = vmraid.get_doc("Timesheet", row.time_sheet)
	...
	def set_missing_values(self, for_validate=False):
		...
		if not self.debit_to:
			...
			self.party_account_currency = vmraid.db.get_value(
		...
		if not print_format and not cint(vmraid.db.get_value("Print Format", "POS Invoice", "disabled")):
	...
	def update_time_sheet(self, sales_invoice):
		for d in self.timesheets:
			if d.time_sheet:
				timesheet = vmraid.get_doc("Timesheet", d.time_sheet)
	...
	def validate_time_sheets_are_submitted(self):
		for data in self.timesheets:
			if data.time_sheet:
				status = vmraid.db.get_value("Timesheet", data.time_sheet, "status")
				if status not in ["Submitted", "Payslip"]:
					vmraid.throw(_("Timesheet {0} is already completed or cancelled").format(data.time_sheet))
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.account_for_change_amount:
			self.account_for_change_amount = vmraid.get_cached_value(
				"Company", self.company, "default_cash_account"
			)

		from erpadda.stock.get_item_details import get_pos_profile, get_pos_profile_item_details
		...
		pos = {}
		if self.pos_profile:
			pos = vmraid.get_doc("POS Profile", self.pos_profile)
		...
		if pos:
			...
			if self.customer:
				customer_price_list, customer_group = vmraid.get_value(
					"Customer", self.customer, ["default_price_list", "customer_group"]
				)
				customer_group_price_list = vmraid.get_value(
			...
			for item in self.get("items"):
				if item.get("item_code"):
					profile_details = get_pos_profile_item_details(
						pos, vmraid._dict(item.as_dict()), pos, update_data=True
					)
			...
			if self.tc_name and not self.terms:
				self.terms = vmraid.db.get_value("Terms and Conditions", self.tc_name, "terms")
	...
	def get_company_abbr(self):
		return vmraid.db.sql("select abbr from tabCompany where name=%s", self.company)[0][0]
	...
	def validate_debit_to_acc(self):
		...
		account = vmraid.get_cached_value(
			"Account", self.debit_to, ["account_type", "report_type", "account_currency"], as_dict=True
		)

		if not account:
			vmraid.throw(_("Debit To is required"), title=_("Account Missing"))

		if account.report_type != "Balance Sheet":
			msg = (
				_("Please ensure {} account is a Balance Sheet account.").format(vmraid.bold("Debit To")) + " "
			)
			msg += _(
				"You can change the parent account to a Balance Sheet account or select a different account."
			)
			vmraid.throw(msg, title=_("Invalid Account"))

		if self.customer and account.account_type != "Receivable":
			msg = (
				_("Please ensure {} account {} is a Receivable account.").format(
					vmraid.bold("Debit To"), vmraid.bold(self.debit_to)
				)
				+ " "
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(vmraid.db.get_single_value("Selling Settings", "maintain_same_sales_rate"))
	...
	def so_dn_required(self):
		...
		for key, value in prev_doc_field_map.items():
			if vmraid.db.get_single_value("Selling Settings", value[0]) == "Yes":

				if vmraid.get_value("Customer", self.customer, value[0]):
					continue
	...
	def validate_pos(self):
		if self.is_return:
			...
			if flt(self.paid_amount) + flt(self.write_off_amount) - flt(invoice_total) > 1.0 / (
				...
				vmraid.throw(_("Paid amount + Write Off Amount can not be greater than Grand Total"))
	...
	def validate_warehouse(self):
		...
		for d in self.get_item_list():
			if (
				not d.warehouse
				and d.item_code
				and vmraid.get_cached_value("Item", d.item_code, "is_stock_item")
			):
				vmraid.throw(_("Warehouse required for stock Item {0}").format(d.item_code))
	...
	def validate_write_off_account(self):
		if flt(self.write_off_amount) and not self.write_off_account:
			self.write_off_account = vmraid.get_cached_value("Company", self.company, "write_off_account")
	...
	def validate_dropship_item(self):
		for item in self.items:
			if item.sales_order:
				if vmraid.db.get_value("Sales Order Item", item.so_detail, "delivered_by_supplier"):
					vmraid.throw(_("Could not update stock, invoice contains drop shipping item."))
	...
	def update_packing_list(self):
		if cint(self.update_stock) == 1:
			from erpadda.stock.doctype.packed_item.packed_item import make_packing_list
	...
	def set_billing_hours_and_amount(self):
		if not self.project:
			for timesheet in self.timesheets:
				ts_doc = vmraid.get_doc("Timesheet", timesheet.time_sheet)
	...
	def get_warehouse(self):
		user_pos_profile = vmraid.db.sql(
			"""select name, warehouse from `tabPOS Profile`
			where ifnull(user,'') = %s and company = %s""",
			(vmraid.session["user"], self.company),
		)
		warehouse = user_pos_profile[0][1] if user_pos_profile else None

		if not warehouse:
			global_pos_profile = vmraid.db.sql(
				"""select name, warehouse from `tabPOS Profile`
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if d.sales_order and vmraid.db.get_value("Sales Order", d.sales_order, "docstatus") != 1:
				vmraid.throw(_("Sales Order {0} is not submitted").format(d.sales_order))

			if d.delivery_note and vmraid.db.get_value("Delivery Note", d.delivery_note, "docstatus") != 1:
				throw(_("Delivery Note {0} is not submitted").format(d.delivery_note))

	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from erpadda.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries

		auto_accounting_for_stock = erpadda.is_perpetual_inventory_enabled(self.company)
		if not gl_entries:
		...
		if gl_entries:
			...
			if update_outstanding == "No":
				from erpadda.accounts.doctype.gl_entry.gl_entry import update_outstanding_amt
	...
	def get_gl_entries(self, warehouse_account=None):
		from erpadda.accounts.general_ledger import merge_similar_entries
	...
	def make_tax_gl_entries(self, gl_entries):
		enable_discount_accounting = cint(
			vmraid.db.get_single_value("Accounts Settings", "enable_discount_accounting")
		)
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if cint(self.update_stock) and erpadda.is_perpetual_inventory_enabled(self.company):
			gl_entries += super(SalesInvoice, self).get_gl_entries()

	def get_asset(self, item):
		if item.get("asset"):
			asset = vmraid.get_doc("Asset", item.asset)
		else:
			vmraid.throw(
	...
	def modify_depreciation_schedule_for_asset_repairs(self, asset):
		asset_repairs = vmraid.get_all(
			"Asset Repair", filters={"asset": asset.name}, fields=["name", "increase_in_asset_life"]
		)

		for repair in asset_repairs:
			if repair.increase_in_asset_life:
				asset_repair = vmraid.get_doc("Asset Repair", repair.name)
				asset_repair.modify_depreciation_schedule()
				asset.prepare_depreciation_data()

	def reverse_depreciation_entry_made_after_sale(self, asset):
		from erpadda.accounts.doctype.journal_entry.journal_entry import make_reverse_journal_entry
		...
		for schedule in asset.get("schedules"):
			...
			if schedule.schedule_date == posting_date_of_original_invoice:
				if not self.sale_was_made_on_original_schedule_date(
					...
					vmraid.flags.is_reverse_depr_entry = True
					reverse_journal_entry.submit()

					vmraid.flags.is_reverse_depr_entry = False
					asset.flags.ignore_validate_update_after_submit = True
	...
	def get_posting_date_of_sales_invoice(self):
		return vmraid.db.get_value("Sales Invoice", self.return_against, "posting_date")
	...
	def make_pos_gl_entries(self, gl_entries):
		if cint(self.is_pos):
			...
			skip_change_gl_entries = not cint(
				vmraid.db.get_single_value("Accounts Settings", "post_change_gl_entries")
			)
	...
	def make_gle_for_change_amount(self, gl_entries):
		if self.change_amount:
			if self.account_for_change_amount:
				...
				vmraid.throw(_("Select change amount account"), title=_("Mandatory Field"))

	def make_write_off_gl_entry(self, gl_entries):
		# write off entries, applicable if only pos
		if self.write_off_account and flt(self.write_off_amount, self.precision("write_off_amount")):
			write_off_account_currency = get_account_currency(self.write_off_account)
			default_cost_center = vmraid.get_cached_value("Company", self.company, "cost_center")
	...
	def update_billing_status_in_dn(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			vmraid.get_doc("Delivery Note", dn).update_billing_percentage(update_modified=update_modified)
	...
	def update_serial_no(self, in_cancel=False):
		...
		for item in self.items:
			...
			for serial_no in get_serial_nos(item.serial_no):
				if serial_no and vmraid.db.get_value("Serial No", serial_no, "item_code") == item.item_code:
					vmraid.db.set_value("Serial No", serial_no, "sales_invoice", invoice)
	...
	def validate_serial_against_delivery_note(self):
		...
		for item in self.items:
			...
			serial_nos = vmraid.db.get_value("Delivery Note Item", item.dn_detail, "serial_no") or ""
			...
			if serial_no_diff:
				dn_link = vmraid.utils.get_link_to_form("Delivery Note", item.delivery_note)
				serial_no_msg = ", ".join(vmraid.bold(d) for d in serial_no_diff)
				...
				vmraid.throw(msg=msg, title=_("Serial Nos Mismatch"))
	...
	def update_project(self):
		if self.project:
			project = vmraid.get_doc("Project", self.project)
	...
	def verify_payment_amount_is_positive(self):
		for entry in self.payments:
			if entry.amount < 0:
				vmraid.throw(_("Row #{0} (Payment Table): Amount must be positive").format(entry.idx))

	def verify_payment_amount_is_negative(self):
		for entry in self.payments:
			if entry.amount > 0:
				vmraid.throw(_("Row #{0} (Payment Table): Amount must be negative").format(entry.idx))
	...
	def delete_loyalty_point_entry(self):
		...
		if against_lp_entry:
			...
			vmraid.db.sql("""delete from `tabLoyalty Point Entry` where invoice=%s""", (self.name))
			# Set loyalty program
			self.set_loyalty_program_tier()
	...
	def set_loyalty_program_tier(self):
		...
		vmraid.db.set_value("Customer", self.customer, "loyalty_program_tier", lp_details.tier_name)

	def get_returned_amount(self):
		from vmraid.query_builder.functions import Coalesce, Sum

		doc = vmraid.qb.DocType(self.doctype)
		returned_amount = (
			vmraid.qb.from_(doc)
	...
	def apply_loyalty_points(self):
		from erpadda.accounts.doctype.loyalty_point_entry.loyalty_point_entry import (
...
def get_discounting_status(sales_invoice):
	...
	invoice_discounting_list = vmraid.db.sql(
		"""
		select status
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
	...
	if inter_company_reference:
		doc = vmraid.get_doc(ref_doc, inter_company_reference)
		ref_party = doc.supplier if doctype in ["Sales Invoice", "Sales Order"] else doc.customer
		if not vmraid.db.get_value(partytype, {"represents_company": doc.company}, "name") == party:
			vmraid.throw(_("Invalid {0} for Inter Company Transaction.").format(partytype))
		if not vmraid.get_cached_value(ref_partytype, ref_party, "represents_company") == company:
			vmraid.throw(_("Invalid Company for Inter Company Transaction."))

	elif vmraid.db.get_value(partytype, {"name": party, internal: 1}, "name") == party:
...
def update_linked_doc(doctype, name, inter_company_reference):
	...
	if inter_company_reference:
		vmraid.db.set_value(doctype, inter_company_reference, ref_field, name)
...
def unlink_inter_company_doc(doctype, name, inter_company_reference):
	...
	if inter_company_reference:
		vmraid.db.set_value(doctype, name, ref_field, "")
		vmraid.db.set_value(ref_doc, inter_company_reference, ref_field, "")


def get_list_context(context=None):
	from erpadda.controllers.website_list_for_contact import get_list_context
...
def make_sales_return(source_name, target_doc=None):
	from erpadda.controllers.sales_and_purchase_return import make_return_doc
...
def get_inter_company_details(doc, doctype):
	if doctype in ["Sales Invoice", "Sales Order", "Delivery Note"]:
		...
		company = vmraid.get_cached_value("Supplier", doc.supplier, "represents_company")
...
def validate_inter_company_transaction(doc, doctype):
	...
	valid_price_list = vmraid.db.get_value(
		"Price List", {"name": price_list, "buying": 1, "selling": 1}
	)
	if not valid_price_list and not doc.is_internal_transfer():
		vmraid.throw(_("Selected Price List should have buying and selling fields checked."))

	party = details.get("party")
	if not party:
		partytype = "Supplier" if doctype in ["Sales Invoice", "Sales Order"] else "Customer"
		vmraid.throw(_("No {0} found for Inter Company Transactions.").format(partytype))

	company = details.get("company")
	default_currency = vmraid.get_cached_value("Company", company, "default_currency")
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Sales Invoice", "Sales Order"]:
		source_doc = vmraid.get_doc(doctype, source_name)
	...
	def update_details(source_doc, target_doc, source_parent):
		...
		if target_doc.doctype in ["Purchase Invoice", "Purchase Order"]:
			currency = vmraid.db.get_value("Supplier", details.get("party"), "default_currency")
			...
			currency = vmraid.db.get_value("Customer", details.get("party"), "default_currency")
			target_doc.company = details.get("company")
...
def get_delivery_note_details(internal_reference):
	si_item_details = vmraid.get_all(
...
def get_pd_details(doctype, sd_detail_map, sd_detail_field):
	...
	pd_item_details = vmraid.get_all(
		doctype,
...
def get_loyalty_programs(customer):
	...
	from erpadda.selling.doctype.customer.customer import get_loyalty_programs
	...
	if len(lp_details) == 1:
		vmraid.db.set(customer, "loyalty_program", lp_details[0])
		return lp_details
	else:
		return lp_details
...
def on_doctype_update():
	vmraid.db.add_index("Sales Invoice", ["customer", "is_return", "return_against"])


@vmraid.whitelist()
def create_invoice_discounting(source_name, target_doc=None):
	invoice = vmraid.get_doc("Sales Invoice", source_name)
	invoice_discounting = vmraid.new_doc("Invoice Discounting")
...
def update_multi_mode_option(doc, pos_profile):
	...
	if invalid_modes:
		...
		vmraid.throw(msg.format(", ".join(invalid_modes)), title=_("Missing Account"))
...
def create_dunning(source_name, target_doc=None):
	...
	from erpadda.accounts.doctype.dunning.dunning import (
...
def check_if_return_invoice_linked_with_payment_entry(self):
	...
	if not vmraid.db.get_single_value(
	...
	payment_entries = vmraid.db.sql_list(
		"""
		SELECT
			t1.name
		FROM
	...
	if payment_entries:
		for payment in payment_entries:
			payment_entry = vmraid.get_doc("Payment Entry", payment)
