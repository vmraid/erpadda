...
erpadda.accounts.SalesInvoiceController = class SalesInvoiceController extends erpadda.selling.SellingController {
	setup(doc) {
	...
	company() {
		...
		if (this.frm.doc.company) {
			vmraid.call({
				method:
					"erpadda.accounts.party.get_party_account",
				args: {
					party_type: 'Customer',
	...
	onload() {
		...
		erpadda.queries.setup_warehouse_query(this.frm);
	...
	refresh(doc, dt, dn) {
		...
		if(doc.docstatus==1 && !doc.is_return) {
			...
			if (doc.outstanding_amount>0) {
				...
				if (doc.due_date < vmraid.datetime.get_today()) {
			...
			if(!doc.auto_repeat) {
				cur_frm.add_custom_button(__('Subscription'), function() {
					erpadda.utils.make_subscription(doc.doctype, doc.name)
				}, __('Create'))
			}
		}
	...
	make_maintenance_schedule() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: cur_frm
		})
	}
	...
	on_submit(doc, dt, dn) {
		...
		if (vmraid.get_route()[0] != 'Form') {
			return
		}

		doc.items.forEach((row) => {
			if(row.delivery_note) vmraid.model.clear_doc("Delivery Note", row.delivery_note)
		});
	}
		...
		this.$sales_order_btn = this.frm.add_custom_button(__('Sales Order'),
			function() {
				erpadda.utils.map_current_doc({
					method: "erpadda.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(__('Quotation'),
			function() {
				erpadda.utils.map_current_doc({
					method: "erpadda.selling.doctype.quotation.quotation.make_sales_invoice",
	...
	delivery_note_btn() {
		...
		this.$delivery_note_btn = this.frm.add_custom_button(__('Delivery Note'),
			function() {
				erpadda.utils.map_current_doc({
					method: "erpadda.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
					...
					get_query: function() {
						...
						return {
							query: "erpadda.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters
						};
					}
				});
	...
	customer() {
		...
		if(this.frm.doc.customer) {
			vmraid.call({
				"method": "erpadda.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				"args": {
	...
	make_inter_company_invoice() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm
		});
	}
	...
	make_sales_return() {
		vmraid.model.open_mapped_doc({
			method: "erpadda.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: cur_frm
		})
	}
	...
	asset(frm, cdt, cdn) {
		...
		if(row.asset) {
			vmraid.call({
				method: erpadda.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
					"company": frm.doc.company
				},
				callback: function(r, rt) {
					vmraid.model.set_value(cdt, cdn, "income_account", r.message[0]);
					vmraid.model.set_value(cdt, cdn, "cost_center", r.message[1]);
				}
			})
		}
	}
				...
				return this.frm.call({
					...
					callback: function(r) {
						if(!r.exc) {
							...
							vmraid.model.set_default_values(me.frm.doc);
							me.set_dynamic_labels();
	...
	currency() {
		...
		if (this.frm.doc.timesheets) {
			this.frm.doc.timesheets.forEach((d) => {
				let row = vmraid.get_doc(d.doctype, d.name)
...
extend_cscript(cur_frm.cscript, new erpadda.accounts.SalesInvoiceController({frm: cur_frm}));

cur_frm.cscript['Make Delivery Note'] = function() {
	vmraid.model.open_mapped_doc({
		method: "erpadda.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm
	})
}
...
cur_frm.set_query("income_account", "items", function(doc) {
	return{
		query: "erpadda.controllers.queries.get_income_account",
...
cur_frm.cscript.income_account = function(doc, cdt, cdn) {
	erpadda.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
}

cur_frm.cscript.expense_account = function(doc, cdt, cdn) {
	erpadda.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
}

cur_frm.cscript.cost_center = function(doc, cdt, cdn) {
	erpadda.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
}
...
vmraid.ui.form.on('Sales Invoice', {
	setup: function(frm){
		...
		frm.fields_dict["timesheets"].grid.get_field("time_sheet").get_query = function(doc, cdt, cdn){
			return{
				query: "erpadda.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.fields_dict['items'].grid.get_field('expense_account').get_query = function(doc) {
			if (erpadda.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
						'report_type': 'Profit and Loss',
						'company': doc.company,
		...
		frm.set_query('pos_profile', function(doc) {
			if(!doc.company) {
				vmraid.throw(_('Please set Company'));
			}

			return {
				query: 'erpadda.accounts.doctype.pos_profile.pos_profile.pos_profile_query',
				filters: {
					company: doc.company
				}
			};
	...
	hide_fields: function(frm) {
		...
		if(cint(doc.is_pos) == 1) {
			...
			for (var i in parent_fields) {
				var docfield = vmraid.meta.docfield_map[doc.doctype][parent_fields[i]];
				if(!docfield.hidden) unhide_field(parent_fields[i]);
			}
		}

		// India related fields
		if (vmraid.boot.sysdefaults.country == 'India') unhide_field(['c_form_applicable', 'c_form_no']);
	...
	patient: function(frm) {
		if (vmraid.boot.active_domains.includes("Healthcare")){
			if(frm.doc.patient){
				vmraid.call({
					method: "vmraid.client.get_value",
					args:{
	...
	async get_timesheet_data(frm, kwargs) {
		return vmraid.call({
			method: "erpadda.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
			args: kwargs
		}).then(r => {
	...
	create_invoice_discounting: function(frm) {
		vmraid.model.open_mapped_doc({
			method: "erpadda.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: frm
		});
	},
	...
	create_dunning: function(frm) {
		vmraid.model.open_mapped_doc({
			method: "erpadda.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: frm
		});
	}
});

vmraid.ui.form.on("Sales Invoice Timesheet", {
...
var set_timesheet_detail_rate = function(cdt, cdn, currency, timelog) {
	vmraid.call({
		method: "erpadda.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency
		},
		callback: function(r) {
			if (!r.exc && r.message) {
				vmraid.model.set_value(cdt, cdn, 'billing_amount', r.message);
			}
		}
	});
}
