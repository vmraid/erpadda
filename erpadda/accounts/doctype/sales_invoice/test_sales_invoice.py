...
from vmraid.utils import add_days, flt, getdate, nowdate

import erpadda
from erpadda.accounts.doctype.account.test_account import create_account, get_inventory_account
from erpadda.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from erpadda.accounts.doctype.purchase_invoice.purchase_invoice import WarehouseMissingError
from erpadda.accounts.doctype.purchase_invoice.test_purchase_invoice import (
	unlink_payment_on_cancel_of_invoice,
)
from erpadda.accounts.doctype.sales_invoice.sales_invoice import make_inter_company_transaction
from erpadda.accounts.utils import PaymentEntryUnlinkError
from erpadda.assets.doctype.asset.depreciation import post_depreciation_entries
from erpadda.assets.doctype.asset.test_asset import create_asset, create_asset_data
from erpadda.controllers.accounts_controller import update_invoice_status
from erpadda.controllers.taxes_and_totals import get_itemised_tax_breakup_data
from erpadda.exceptions import InvalidAccountCurrency, InvalidCurrency
from erpadda.regional.india.utils import get_ewb_data
from erpadda.stock.doctype.delivery_note.delivery_note import make_sales_invoice
...
from erpadda.stock.doctype.serial_no.serial_no import SerialNoWarehouseError
from erpadda.stock.doctype.stock_entry.test_stock_entry import (
	get_qty_after_transaction,
	make_stock_entry,
)
from erpadda.stock.utils import get_incoming_rate


class TestSalesInvoice(unittest.TestCase):
	def make(self):
		w = vmraid.copy_doc(test_records[0])
		w.is_pos = 0
		w.insert()
		w.submit()
		return w
	...
	def test_timestamp_change(self):
		...
		w2 = vmraid.get_doc(w.doctype, w.name)
		...
		time.sleep(1)
		self.assertRaises(vmraid.TimestampMismatchError, w2.save)

	def test_sales_invoice_change_naming_series(self):
		si = vmraid.copy_doc(test_records[2])
		si.insert()
		si.naming_series = "TEST-"

		self.assertRaises(vmraid.CannotChangeConstantError, si.save)

		si = vmraid.copy_doc(test_records[1])
		si.insert()
		si.naming_series = "TEST-"
	...
	def test_payment_entry_unlink_against_invoice(self):
		...
		si = vmraid.copy_doc(test_records[0])
		si.is_pos = 0
		si.insert()
		si.submit()
		...
		unlink_payment_on_cancel_of_invoice(0)
		si = vmraid.get_doc("Sales Invoice", si.name)
		self.assertRaises(vmraid.LinkExistsError, si.cancel)
		unlink_payment_on_cancel_of_invoice()
	...
	def test_sales_invoice_discount_amount(self):
		si = vmraid.copy_doc(test_records[3])
	...
	def test_discount_amount_gl_entry(self):
		vmraid.db.set_value("Company", "_Test Company", "round_off_account", "Round Off - _TC")
		si = vmraid.copy_doc(test_records[3])
		si.discount_amount = 104.94
		si.append(
			"taxes",
	...
	def test_inclusive_rate_validations(self):
		...
		self.assertRaises(vmraid.ValidationError, si.insert)
	...
	def test_payment(self):
		...
		from erpadda.accounts.doctype.journal_entry.test_journal_entry import (
			test_records as jv_test_records,
		)

		jv = vmraid.get_doc(vmraid.copy_doc(jv_test_records[0]))
		jv.get("accounts")[0].reference_type = w.doctype
		jv.get("accounts")[0].reference_name = w.name
		jv.insert()
		jv.submit()

		self.assertEqual(vmraid.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 162.0)
		...
		jv.cancel()
		self.assertEqual(vmraid.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 562.0)
	...
	def test_pos_returns_with_repayment(self):
		from erpadda.accounts.doctype.sales_invoice.sales_invoice import make_sales_return
	...
	def test_pos_with_no_gl_entry_for_change_amount(self):
		vmraid.db.set_value("Accounts Settings", None, "post_change_gl_entries", 0)
		...
		vmraid.db.set_value("Accounts Settings", None, "post_change_gl_entries", 1)
	...
	def _insert_purchase_receipt(self):
		from erpadda.stock.doctype.purchase_receipt.test_purchase_receipt import (
			test_records as pr_test_records,
		)

		pr = vmraid.copy_doc(pr_test_records[0])
		pr.naming_series = "_T-Purchase Receipt-"
		pr.insert()
		pr.submit()

	def _insert_delivery_note(self):
		from erpadda.stock.doctype.delivery_note.test_delivery_note import (
			test_records as dn_test_records,
		)

		dn = vmraid.copy_doc(dn_test_records[0])
	...
	def test_sales_invoice_with_advance(self):
		...
		jv = vmraid.copy_doc(jv_test_records[0])
		jv.insert()
		jv.submit()
	...
	def test_serialized(self):
		...
		from erpadda.stock.doctype.stock_entry.test_stock_entry import make_serialized_item
		...
		self.assertFalse(vmraid.db.get_value("Serial No", serial_nos[0], "warehouse"))
		self.assertEqual(
			vmraid.db.get_value("Serial No", serial_nos[0], "delivery_document_no"), si.name
		)
	...
	def test_serialized_cancel(self):
		...
		self.assertEqual(
			vmraid.db.get_value("Serial No", serial_nos[0], "warehouse"), "_Test Warehouse - _TC"
		)
		self.assertFalse(vmraid.db.get_value("Serial No", serial_nos[0], "delivery_document_no"))
		self.assertFalse(vmraid.db.get_value("Serial No", serial_nos[0], "sales_invoice"))

	def test_serialize_status(self):
		serial_no = vmraid.get_doc(
			{
				"doctype": "Serial No",
	...
	def test_return_sales_invoice(self):
		...
		incoming_rate, stock_value_difference = vmraid.db.get_value(
			"Stock Ledger Entry",
		...
		# Check gl entry
		gle_warehouse_amount = vmraid.db.get_value(
			"GL Entry",
		...
		party_credited = vmraid.db.get_value(
			"GL Entry",
			{
		...
		self.assertEqual(vmraid.db.get_value("Sales Invoice", si.name, "outstanding_amount"), 1500)
	...
	def test_gle_made_when_asset_is_returned(self):
		...
		disposal_account = vmraid.get_cached_value("Company", "_Test Company", "disposal_account")

		# Asset value is 100,000 but it was sold for 90,000, so there should be a loss of 10,000
		loss_for_si = vmraid.get_all(
			"GL Entry",
			filters={"voucher_no": si.name, "account": disposal_account},
			fields=["credit", "debit"],
		)[0]

		loss_for_return_si = vmraid.get_all(
			"GL Entry",
	...
	def test_incoming_rate_for_stand_alone_credit_note(self):
		...
		incoming_rate = vmraid.db.get_value(
			"Stock Ledger Entry", {"voucher_no": return_si.name}, "incoming_rate"
		)
		debit_amount = vmraid.db.get_value(
	...
	def test_outstanding_amount_after_advance_jv_cancelation(self):
		...
		jv = vmraid.get_doc("Journal Entry", jv.name)
		jv.cancel()
	...
	def test_outstanding_amount_after_advance_payment_entry_cancelation(self):
		...
		# added to avoid Document has been modified exception
		pe = vmraid.get_doc("Payment Entry", pe.name)
		pe.cancel()
	...
	def test_multiple_uom_in_selling(self):
		...
		item_price = vmraid.new_doc("Item Price")
	...
	def test_item_wise_tax_breakup_india(self):
		...
		vmraid.flags.country = None

	def test_item_wise_tax_breakup_outside_india(self):
		vmraid.flags.country = "United States"
	...
	def test_company_monthly_sales(self):
		existing_current_month_sales = vmraid.get_cached_value(
			"Company", "_Test Company", "total_monthly_sales"
		)

		si = create_sales_invoice()
		current_month_sales = vmraid.get_cached_value("Company", "_Test Company", "total_monthly_sales")
	...
	def test_sales_invoice_with_shipping_rule(self):
		from erpadda.accounts.doctype.shipping_rule.test_shipping_rule import create_shipping_rule
	...
	def test_credit_note(self):
		...
		si_doc = vmraid.get_doc("Sales Invoice", si.name)
		self.assertEqual(si_doc.outstanding_amount, 0)

	def test_sales_invoice_with_cost_center(self):
		from erpadda.accounts.doctype.cost_center.test_cost_center import create_cost_center
	...
	def test_sales_invoice_with_project_link(self):
		from erpadda.projects.doctype.project.test_project import make_project
	...
	def test_deferred_revenue_missing_account(self):
		...
		self.assertRaises(vmraid.ValidationError, si.save)
	...
	def test_fixed_deferred_revenue(self):
		...
		acc_settings = vmraid.get_doc("Accounts Settings", "Accounts Settings")
	...
	def test_inter_company_transaction(self):
		from erpadda.selling.doctype.customer.test_customer import create_internal_customer
		...
		if not vmraid.db.exists("Supplier", "_Test Internal Supplier"):
			supplier = vmraid.get_doc(
				{
	...
	def test_inter_company_transaction_without_default_warehouse(self):
		...
		# setup
		old_negative_stock = vmraid.db.get_single_value("Stock Settings", "allow_negative_stock")
		vmraid.db.set_value("Stock Settings", None, "allow_negative_stock", 1)

		old_perpetual_inventory = erpadda.is_perpetual_inventory_enabled("_Test Company 1")
		vmraid.local.enable_perpetual_inventory["_Test Company 1"] = 1
		...
		if not vmraid.db.exists("Customer", "_Test Internal Customer"):
		...
		# tear down
		vmraid.local.enable_perpetual_inventory["_Test Company 1"] = old_perpetual_inventory
		vmraid.db.set_value("Stock Settings", None, "allow_negative_stock", old_negative_stock)
	...
	def test_einvoice_submission_without_irn(self):
		# init
		einvoice_settings = vmraid.get_doc("E Invoice Settings")
		...
		vmraid.flags.country = country

	def test_einvoice_json(self):
		from erpadda.regional.india.e_invoice.utils import make_einvoice, validate_totals
	...
	def test_sales_invoice_against_supplier(self):
		from erpadda.accounts.doctype.opening_invoice_creation_tool.test_opening_invoice_creation_tool import (
			make_customer,
		)
		from erpadda.accounts.doctype.party_link.party_link import create_party_link
		from erpadda.buying.doctype.supplier.test_supplier import create_supplier
		...
		vmraid.db.set_value("Accounts Settings", None, "enable_common_party_accounting", 1)
		...
		party_link.delete()
		vmraid.db.set_value("Accounts Settings", None, "enable_common_party_accounting", 0)
	...
	def test_sales_commission(self):
		...
		vmraid.db.set_value("Item", si.get("items")[0].item_code, "grant_commission", 1)
		vmraid.db.set_value("Item", si.get("items")[1].item_code, "grant_commission", 0)
	...
	def test_sales_invoice_submission_post_account_freezing_date(self):
		vmraid.db.set_value("Accounts Settings", None, "acc_frozen_upto", add_days(getdate(), 1))
		si = create_sales_invoice(do_not_save=True)
	...
	def test_over_billing_case_against_delivery_note(self):
		...
		over_billing_allowance = vmraid.db.get_single_value(
			"Accounts Settings", "over_billing_allowance"
		)
		vmraid.db.set_value("Accounts Settings", None, "over_billing_allowance", 0)
		...
		item_copy = vmraid.copy_doc(si.items[0])
		si.append("items", item_copy)
		si.save()

		with self.assertRaises(vmraid.ValidationError) as err:
			si.submit()

		self.assertTrue("cannot overbill" in str(err.exception).lower())

		vmraid.db.set_value("Accounts Settings", None, "over_billing_allowance", over_billing_allowance)
	...
	def test_multi_currency_deferred_revenue_via_journal_entry(self):
		...
		acc_settings = vmraid.get_single("Accounts Settings")
		...
		vmraid.db.set_value("Accounts Settings", None, "acc_frozen_upto", getdate("2019-01-31"))
		...
		gl_entries = gl_entries = vmraid.db.sql(
	...
	def test_gain_loss_with_advance_entry(self):
		...
		unlink_enabled = vmraid.db.get_value(
...
def make_test_address_for_ewaybill():
	if not vmraid.db.exists("Address", "_Test Address for Eway bill-Billing"):
	...
	if not vmraid.db.exists("Address", "_Test Customer-Address for Eway bill-Billing"):
		address = vmraid.get_doc(
			{
				"address_line1": "_Test Address Line 1",
				"address_line2": "_Test Address Line 2",
	...
	if not vmraid.db.exists("Address", "_Test Customer-Address for Eway bill-Shipping"):
		address = vmraid.get_doc(
			{
				"address_line1": "_Test Address Line 1",
				"address_line2": "_Test Address Line 2",
	...
	if not vmraid.db.exists("Address", "_Test Dispatch-Address for Eway bill-Shipping"):
		address = vmraid.get_doc(
			{
...
def make_test_transporter_for_ewaybill():
	if not vmraid.db.exists("Supplier", "_Test Transporter"):
		vmraid.get_doc(
			{
				"doctype": "Supplier",
...
def create_sales_invoice(**args):
	si = vmraid.new_doc("Sales Invoice")
...
test_records = vmraid.get_test_records("Sales Invoice")
...
def create_internal_supplier(supplier_name, represents_company, allowed_to_interact_with):
	if not vmraid.db.exists("Supplier", supplier_name):
		supplier = vmraid.get_doc(
			{
				"supplier_group": "_Test Supplier Group",
		...
		supplier_name = vmraid.db.exists("Supplier", supplier_name)
