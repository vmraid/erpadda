...
from erpadda.accounts.doctype.pos_profile.pos_profile import get_child_nodes
from erpadda.stock.get_item_details import get_pos_profile
...
class TestPOSProfile(unittest.TestCase):
	def test_pos_profile(self):
		...
		if pos_profile:
			doc = vmraid.get_doc("POS Profile", pos_profile.get("name"))
...
def make_pos_profile(**args):
	vmraid.db.sql("delete from `tabPOS Payment Method`")
	vmraid.db.sql("delete from `tabPOS Profile`")

	args = vmraid._dict(args)

	pos_profile = vmraid.get_doc(
		{
			...
			"customer_group": vmraid.db.get_value("Customer Group", {"is_group": 0}, "name"),
	...
	if not vmraid.db.get_value("Mode of Payment Account", {"company": company, "parent": "Cash"}):
	...
	if not vmraid.db.exists("POS Profile", args.name or "_Test POS Profile"):
		pos_profile.insert()
