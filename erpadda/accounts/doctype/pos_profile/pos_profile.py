...
from vmraid.utils import get_link_to_form, now
...
class POSProfile(Document):
	...
	def validate_all_link_fields(self):
		...
		for link_dt, dn_list in accounts.items():
			for link_dn in dn_list:
				if link_dn and not vmraid.db.exists(
					{"doctype": link_dt, "company": self.company, "name": link_dn}
				):
					vmraid.throw(_("{0} does not belong to Company {1}").format(link_dn, self.company))
	...
	def validate_payment_methods(self):
		if not self.payments:
			vmraid.throw(_("Payment methods are mandatory. Please add at least one payment method."))

		default_mode = [d.default for d in self.payments if d.default]
		if not default_mode:
			vmraid.throw(_("Please select a default mode of payment"))

		if len(default_mode) > 1:
			vmraid.throw(_("You can only select one mode of payment as default"))
	...
	def set_defaults(self, include_current_pos=True):
		vmraid.defaults.clear_default("is_pos")
		...
		pos_view_users = vmraid.db.sql_list(
			"""select pfu.user
		...
		for user in pos_view_users:
			if user:
				vmraid.defaults.set_user_default("is_pos", 1, user)
			else:
				vmraid.defaults.set_global_default("is_pos", 1)


def get_item_groups(pos_profile):
	item_groups = []
	pos_profile = vmraid.get_cached_doc("POS Profile", pos_profile)

	if pos_profile.get("item_groups"):
		# Get items based on the item groups defined in the POS profile
		for data in pos_profile.get("item_groups"):
			item_groups.extend(
				["%s" % vmraid.db.escape(d.name) for d in get_child_nodes("Item Group", data.item_group)]
			)
...
def get_child_nodes(group_type, root):
	lft, rgt = vmraid.db.get_value(group_type, root, ["lft", "rgt"])
...
def pos_profile_query(doctype, txt, searchfield, start, page_len, filters):
	...
	company = filters.get("company") or vmraid.defaults.get_user_default("company")
