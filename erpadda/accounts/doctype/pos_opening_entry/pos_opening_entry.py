...
class POSOpeningEntry(StatusUpdater):
	...
	def validate_pos_profile_and_cashier(self):
		if self.company != vmraid.db.get_value("POS Profile", self.pos_profile, "company"):
			vmraid.throw(
				_("POS Profile {} does not belongs to company {}").format(self.pos_profile, self.company)
			)

		if not cint(vmraid.db.get_value("User", self.user, "enabled")):
			vmraid.throw(_("User {} is disabled. Please select valid user/cashier").format(self.user))
