...
class TestPricingRule(unittest.TestCase):
	...
	def test_pricing_rule_for_discount(self):
		from vmraid import MandatoryError
		...
		vmraid.get_doc(test_record.copy()).insert()
		...
		prule = vmraid.get_doc(test_record.copy())
		prule.priority = 1
		...
		vmraid.db.sql("update `tabPricing Rule` set priority=NULL where campaign='_Test Campaign'")
		from erpadda.accounts.doctype.pricing_rule.utils import MultiplePricingRuleConflict
	...
	def test_pricing_rule_for_variants(self):
		...
		if not vmraid.db.exists("Item", "Test Variant PRT"):
			vmraid.get_doc(
				{
					"doctype": "Item",
	...
	def test_pricing_rule_for_stock_qty(self):
		...
		if not vmraid.db.get_value("UOM Conversion Detail", {"parent": "_Test Item", "uom": "box"}):
		...
		so.submit()
		so = vmraid.get_doc("Sales Order", so.name)
	...
	def test_pricing_rule_with_margin_and_discount(self):
		vmraid.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule")
		make_pricing_rule(
	...
	def test_cumulative_pricing_rule(self):
		vmraid.delete_doc_if_exists("Pricing Rule", "_Test Cumulative Pricing Rule")
		test_record = {
			"doctype": "Pricing Rule",
			...
			"company": "_Test Company",
			"valid_from": vmraid.utils.nowdate(),
			"valid_upto": vmraid.utils.nowdate(),
		}
		vmraid.get_doc(test_record.copy()).insert()
		...
		args = vmraid._dict(
			{
				...
				"customer": "_Test Customer",
				"name": None,
				"transaction_date": vmraid.utils.nowdate(),
			}
		)
		details = get_item_details(args)
	...
	def test_multiple_pricing_rules(self):
		...
		vmraid.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule 1")
		vmraid.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule 2")
	...
	def test_item_price_with_pricing_rule(self):
		...
		rule = vmraid.get_doc(pricing_rule_record)
		rule.insert()
		...
		si.delete()
		rule.delete()
		vmraid.get_doc("Item Price", {"item_code": "Water Flask"}).delete()
		item.delete()
	...
	def test_pricing_rule_for_different_currency(self):
		...
		rule1 = vmraid.get_doc(pricing_rule_record)
		rule1.currency = "USD"
	...
	def test_multiple_pricing_rules_with_min_qty(self):
		...
		vmraid.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule with Min Qty - 1")
		vmraid.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule with Min Qty - 2")
...
def setup_pricing_rule_data():
	if not vmraid.db.exists("Campaign", "_Test Campaign"):
		vmraid.get_doc(
...
def delete_existing_pricing_rules():
	for doctype in [
		...
		vmraid.db.sql("delete from `tab{0}`".format(doctype))
