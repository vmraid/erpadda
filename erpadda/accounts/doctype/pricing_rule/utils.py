...
from vmraid.utils import cint, flt, fmt_money, get_link_to_form, getdate, today

from erpadda.setup.doctype.item_group.item_group import get_child_item_groups
from erpadda.stock.doctype.warehouse.warehouse import get_child_warehouses
from erpadda.stock.get_item_details import get_conversion_factor


class MultiplePricingRuleConflict(vmraid.ValidationError):
	pass
...
def get_pricing_rules(args, doc=None):
	...
	if not vmraid.db.exists("Pricing Rule", {"disable": 0, args.transaction_type: 1}):
		return
	...
	if doc:
		for pricing_rule in pricing_rules:
			if pricing_rule.condition:
				try:
					if vmraid.safe_eval(pricing_rule.condition, None, doc.as_dict()):
...
def _get_pricing_rules(apply_on, args, values):
	apply_on_field = vmraid.scrub(apply_on)
	...
	if apply_on_field in ["item_code", "brand"]:
		...
		if apply_on_field == "item_code":
			if "variant_of" not in args:
				args.variant_of = vmraid.get_cached_value("Item", args.item_code, "variant_of")
...
def _get_tree_conditions(args, parenttype, table, allow_blank=True):
	field = vmraid.scrub(parenttype)
	condition = ""
	if args.get(field):
		if not vmraid.flags.tree_conditions:
			vmraid.flags.tree_conditions = {}
		key = (parenttype, args.get(field))
		if key in vmraid.flags.tree_conditions:
			return vmraid.flags.tree_conditions[key]

		try:
			lft, rgt = vmraid.db.get_value(parenttype, args.get(field), ["lft", "rgt"])
		except TypeError:
			vmraid.throw(_("Invalid {0}").format(args.get(field)))

		parent_groups = vmraid.db.sql_list(
		...
		if parenttype in ["Customer Group", "Item Group", "Territory"]:
			parent_field = "parent_{0}".format(vmraid.scrub(parenttype))
			root_name = vmraid.db.get_list(
				parenttype,
		...
		if parent_groups:
			...
			condition = "ifnull({table}.{field}, '') in ({parent_groups})".format(
				table=table, field=field, parent_groups=", ".join(vmraid.db.escape(d) for d in parent_groups)
			)

			vmraid.flags.tree_conditions[key] = condition
	return condition
...
def filter_pricing_rules(args, pricing_rules, doc=None):
	...
	if pricing_rules:
		...
		if pricing_rules[0].apply_rule_on_other:
			field = vmraid.scrub(pricing_rules[0].apply_rule_on_other)

			if field and pricing_rules[0].get("other_" + field) != args.get(field):
				return

		pr_doc = vmraid.get_cached_doc("Pricing Rule", pricing_rules[0].name)
		...
		if pricing_rules[0].mixed_conditions and doc:
			...
			items = [args.get(vmraid.scrub(pr_doc.get("apply_on")))]
...
def get_qty_and_rate_for_mixed_conditions(doc, pr_doc, args):
	...
	apply_on = vmraid.scrub(pr_doc.get("apply_on"))
...
def get_qty_and_rate_for_other_item(doc, pr_doc, pricing_rules):
	...
	for row in doc.items:
		if row.get(vmraid.scrub(pr_doc.apply_rule_on_other)) in items:
...
def get_qty_amount_data_for_cumulative(pr_doc, doc, items=None):
	...
	date_field = (
		"transaction_date" if vmraid.get_meta(doctype).has_field("transaction_date") else "posting_date"
	)
			...
			if d.price_or_product_discount == "Price":
				...
				for field in ["additional_discount_percentage", "discount_amount"]:
					...
					if (
						...
						vmraid.msgprint(_("User has not applied rule on the invoice {0}").format(doc.name))
					else:
						if not d.coupon_code_based:
							doc.set(field, d.get(pr_field))
						elif doc.get("coupon_code"):
							# coupon code based pricing rule
							coupon_code_pricing_rule = vmraid.db.get_value(
				...
				item_details = vmraid._dict({"parenttype": doc.doctype, "free_item_data": []})
...
def get_product_discount_rule(pricing_rule, item_details, args=None, doc=None):
	...
	item_data = vmraid.get_cached_value(
...
def get_pricing_rule_items(pr_doc):
	...
	if pr_doc.apply_rule_on_other:
		apply_on = vmraid.scrub(pr_doc.apply_rule_on_other)
...
def validate_coupon_code(coupon_name):
	coupon = vmraid.get_doc("Coupon Code", coupon_name)

	if coupon.valid_from:
		if coupon.valid_from > getdate(today()):
			vmraid.throw(_("Sorry, this coupon code's validity has not started"))
	elif coupon.valid_upto:
		if coupon.valid_upto < getdate(today()):
			vmraid.throw(_("Sorry, this coupon code's validity has expired"))
	elif coupon.used >= coupon.maximum_use:
		vmraid.throw(_("Sorry, this coupon code is no longer valid"))
