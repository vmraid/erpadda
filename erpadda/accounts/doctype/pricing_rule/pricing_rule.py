...
from vmraid.utils import cint, flt, getdate
...
class PricingRule(Document):
	...
	def validate_duplicate_apply_on(self):
		if self.apply_on != "Transaction":
			...
			values = [d.get(vmraid.scrub(self.apply_on)) for d in self.get(field) if field]
			if len(values) != len(set(values)):
				vmraid.throw(_("Duplicate {0} found in the table").format(self.apply_on))

	def validate_mandatory(self):
		for apply_on, field in apply_on_dict.items():
			if self.apply_on == apply_on and len(self.get(field) or []) < 1:
				throw(_("{0} is not added in the table").format(apply_on), vmraid.MandatoryError)

		tocheck = vmraid.scrub(self.get("applicable_for", ""))
		if tocheck and not self.get(tocheck):
			throw(_("{0} is required").format(self.meta.get_label(tocheck)), vmraid.MandatoryError)

		if self.apply_rule_on_other:
			o_field = "other_" + vmraid.scrub(self.apply_rule_on_other)
			if not self.get(o_field) and o_field in other_fields:
				vmraid.throw(
					_("For the 'Apply Rule On Other' condition the field {0} is mandatory").format(
						vmraid.bold(self.apply_rule_on_other)
					)
				)

		if self.price_or_product_discount == "Price" and not self.rate_or_discount:
			throw(_("Rate or Discount is required for the price discount."), vmraid.MandatoryError)

		if self.apply_discount_on_rate:
			if not self.priority:
				throw(
					_("As the field {0} is enabled, the field {1} is mandatory.").format(
						vmraid.bold("Apply Discount on Discounted Rate"), vmraid.bold("Priority")
					)
				)
	...
	def cleanup_fields_value(self):
		for logic_field in ["apply_on", "applicable_for", "rate_or_discount"]:
			fieldname = vmraid.scrub(self.get(logic_field) or "")
		...
		apply_rule_on_other = vmraid.scrub(self.apply_rule_on_other or "")
	...
	def validate_max_discount(self):
		if self.rate_or_discount == "Discount Percentage" and self.get("items"):
			for d in self.items:
				max_discount = vmraid.get_cached_value("Item", d.item_code, "max_discount")
				if max_discount and flt(self.discount_percentage) > flt(max_discount):
					throw(_("Max discount allowed for item: {0} is {1}%").format(d.item_code, max_discount))

	def validate_price_list_with_currency(self):
		if self.currency and self.for_price_list:
			price_list_currency = vmraid.db.get_value("Price List", self.for_price_list, "currency", True)
	...
	def validate_dates(self):
		if self.is_cumulative and not (self.valid_from and self.valid_upto):
			vmraid.throw(_("Valid from and valid upto fields are mandatory for the cumulative"))

		if self.valid_from and self.valid_upto and getdate(self.valid_from) > getdate(self.valid_upto):
			vmraid.throw(_("Valid from date must be less than valid upto date"))
	...
	def validate_condition(self):
		if (
			...
			vmraid.throw(_("Invalid condition expression"))
...
def get_serial_no_for_item(args):
	from erpadda.stock.get_item_details import get_serial_no
...
def get_pricing_rule_for_item(args, price_list_rate=0, doc=None, for_validate=False):
	...
	if args.ignore_pricing_rule or not args.item_code:
		if vmraid.db.exists(args.doctype, args.name) and args.get("pricing_rules"):
	...
	if pricing_rules:
		...
		for pricing_rule in pricing_rules:
			...
			if isinstance(pricing_rule, str):
				pricing_rule = vmraid.get_cached_doc("Pricing Rule", pricing_rule)
				...
				item_details.update(
					{
						...
						"apply_rule_on": (
							vmraid.scrub(pricing_rule.apply_rule_on_other)
							if pricing_rule.apply_rule_on_other
							else vmraid.scrub(pricing_rule.get("apply_on"))
						),
					}
				)
		...
		item_details.pricing_rules = vmraid.as_json([d.pricing_rule for d in rules])
...
def update_args_for_pricing_rule(args):
	if not (args.item_group and args.brand):
		item = vmraid.get_cached_value("Item", args.item_code, ("item_group", "brand"))
		if not item:
			return
		...
		if not args.item_group:
			vmraid.throw(_("Item Group not mentioned in item master for item {0}").format(args.item_code))

	if args.transaction_type == "selling":
		if args.customer and not (args.customer_group and args.territory):

			if args.quotation_to and args.quotation_to != "Customer":
				customer = vmraid._dict()
			else:
				customer = vmraid.get_cached_value("Customer", args.customer, ["customer_group", "territory"])
		...
		args.supplier_group = vmraid.get_cached_value("Supplier", args.supplier, "supplier_group")
...
def remove_pricing_rule_for_item(pricing_rules, item_details, item_code=None, rate=None):
	...
	for d in get_applied_pricing_rules(pricing_rules):
		if not d or not vmraid.db.exists("Pricing Rule", d):
			continue
		pricing_rule = vmraid.get_cached_doc("Pricing Rule", d)
...
def make_pricing_rule(doctype, docname):
	doc = vmraid.new_doc("Pricing Rule")
	doc.applicable_for = doctype
	doc.set(vmraid.scrub(doctype), docname)
...
def get_item_uoms(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters.get("apply_on") != "Item Code":
		field = vmraid.scrub(filters.get("apply_on"))
		items = [d.name for d in vmraid.db.get_all("Item", filters={field: filters.get("value")})]
