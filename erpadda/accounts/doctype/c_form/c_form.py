...
class CForm(Document):
		...
		for d in self.get("invoices"):
			if d.invoice_no:
				...
				if inv and inv[0][0] != "Yes":
					vmraid.throw(_("C-form is not applicable for Invoice: {0}").format(d.invoice_no))
	...
	def before_cancel(self):
		...
		vmraid.db.sql("""update `tabSales Invoice` set c_form_no=null where c_form_no=%s""", self.name)
	...
	def set_cform_in_sales_invoices(self):
		...
		if inv:
			...
			vmraid.throw(_("Please enter atleast 1 invoice in the table"))

	def set_total_invoiced_amount(self):
		total = sum(flt(d.grand_total) for d in self.get("invoices"))
		vmraid.db.set(self, "total_invoiced_amount", total)

	@vmraid.whitelist()
	def get_invoice_details(self, invoice_no):
		"""Pull details from invoices for referrence"""
		if invoice_no:
			inv = vmraid.db.get_value(
				"Sales Invoice",
				invoice_no,
