...
vmraid.ui.form.on('Account', {
	...
	add_toolbar_buttons: function(frm) {
		...
		if (frm.doc.is_group == 1) {
			...
			&& vmraid.boot.user.can_read.indexOf("GL Entry") !== -1) {
	...
	merge_account: function(frm) {
		var d = new vmraid.ui.Dialog({
			...
			primary_action: function() {
				...
				vmraid.call({
					method: "erpadda.accounts.doctype.account.account.merge_account",
					args: {
						old: frm.doc.name,
						new: data.name,
					...
					callback: function(r) {
						if(!r.exc) {
							if(r.message) {
								vmraid.set_route("Form", "Account", r.message);
							}
							d.hide();
						}
					}
	...
	update_account_number: function(frm) {
		var d = new vmraid.ui.Dialog({
			...
			primary_action: function() {
				...
				vmraid.call({
					method: "erpadda.accounts.doctype.account.account.update_account_number",
					args: {
