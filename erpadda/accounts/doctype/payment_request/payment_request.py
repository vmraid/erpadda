...
from vmraid.utils import flt, get_url, nowdate
from vmraid.utils.background_jobs import enqueue

from erpadda.accounts.doctype.payment_entry.payment_entry import (
	get_company_defaults,
	get_payment_entry,
)
from erpadda.accounts.doctype.subscription_plan.subscription_plan import get_plan_rate
from erpadda.accounts.party import get_party_account, get_party_bank_account
from erpadda.accounts.utils import get_account_currency
from erpadda.erpadda_integrations.stripe_integration import create_stripe_subscription
...
class PaymentRequest(Document):
	...
	def validate_reference_document(self):
		if not self.reference_doctype or not self.reference_name:
			vmraid.throw(_("To create a Payment Request reference document is required"))
	...
	def validate_payment_request_amount(self):
		...
		if existing_payment_request_amount:
			ref_doc = vmraid.get_doc(self.reference_doctype, self.reference_name)
	...
	def validate_currency(self):
		ref_doc = vmraid.get_doc(self.reference_doctype, self.reference_name)
		if self.payment_account and ref_doc.currency != vmraid.db.get_value(
			"Account", self.payment_account, "account_currency"
		):
			vmraid.throw(_("Transaction currency must be same as Payment Gateway currency"))

	def validate_subscription_details(self):
		if self.is_a_subscription:
			amount = 0
			for subscription_plan in self.subscription_plans:
				payment_gateway = vmraid.db.get_value(
	...
	def get_request_amount(self):
		data_of_completed_requests = vmraid.get_all(
			"Integration Request",
			filters={
	...
	def get_payment_url(self):
		if self.reference_doctype != "Fees":
			...
			data.update({"company": vmraid.defaults.get_defaults().company})
		...
		return controller.get_payment_url(
			**{
				...
				"reference_doctype": "Payment Request",
				"reference_docname": self.name,
				"payer_email": self.email_to or vmraid.session.user,
				"payer_name": vmraid.safe_encode(data.customer_name),
				"order_id": self.name,
				"currency": self.currency,
			}
		)
	...
	def get_message(self):
		...
		if self.message:
			return vmraid.render_template(self.message, context)
	...
	def check_if_payment_entry_exists(self):
		if self.status == "Paid":
			if vmraid.get_all(
				...
				vmraid.throw(_("Payment Entry already exists"), title=_("Error"))
	...
	def on_payment_authorized(self, status=None):
		...
		shopping_cart_settings = vmraid.get_doc("E Commerce Settings")
		...
		if status in ["Authorized", "Completed"]:
			...
			if (
				...
				and hasattr(vmraid.local, "session")
				and vmraid.local.session.user != "Guest"
...
def make_payment_request(**args):
	...
	ref_doc = vmraid.get_doc(args.dt, args.dn)
	gateway_account = get_gateway_details(args) or vmraid._dict()
	...
	if args.order_type == "Shopping Cart":
		existing_payment_request = vmraid.db.get_value(
			"Payment Request",
	...
	if existing_payment_request:
		...
		pr = vmraid.get_doc("Payment Request", existing_payment_request)
	else:
		if args.order_type != "Shopping Cart":
		...
		pr = vmraid.new_doc("Payment Request")
		pr.update(
			{
	...
	if args.order_type == "Shopping Cart":
		...
		vmraid.local.response["location"] = pr.get_payment_url()
...
def get_amount(ref_doc, payment_account=None):
	...
	if grand_total > 0:
		...
		vmraid.throw(_("Payment Entry is already created"))
...
def get_existing_payment_request_amount(ref_dt, ref_dn):
	...
	"""
	existing_payment_request_amount = vmraid.db.sql(
		"""
...
def get_gateway_details(args):  # nosemgrep
	...
	if args.order_type == "Shopping Cart":
		payment_gateway_account = vmraid.get_doc("E Commerce Settings").payment_gateway_account
...
def get_print_format_list(ref_doctype):
	...
	print_format_list.extend(
		[p.name for p in vmraid.get_all("Print Format", filters={"doc_type": ref_doctype})]
	)
...
def resend_payment_email(docname):
	return vmraid.get_doc("Payment Request", docname).send_email()


@vmraid.whitelist()
def make_payment_entry(docname):
	doc = vmraid.get_doc("Payment Request", docname)
	return doc.create_payment_entry(submit=False).as_dict()


def update_payment_req_status(doc, method):
	from erpadda.accounts.doctype.payment_entry.payment_entry import get_reference_details

	for ref in doc.references:
		payment_request_name = vmraid.db.get_value(
			"Payment Request",
			{
		...
		if payment_request_name:
			...
			pay_req_doc = vmraid.get_doc("Payment Request", payment_request_name)
...
def get_subscription_details(reference_doctype, reference_name):
	if reference_doctype == "Sales Invoice":
		...
		for subscription in subscriptions:
			plans = vmraid.get_doc("Subscription", subscription.sub_name).plans
			for plan in plans:
...
def validate_payment(doc, method=None):
	if doc.reference_doctype != "Payment Request" or (
		vmraid.db.get_value(doc.reference_doctype, doc.reference_docname, "status") != "Paid"
	):
		return
