...
class TestPaymentRequest(unittest.TestCase):
	def setUp(self):
		if not vmraid.db.get_value("Payment Gateway", payment_gateway["gateway"], "name"):
			vmraid.get_doc(payment_gateway).insert(ignore_permissions=True)
		...
		for method in payment_method:
			if not vmraid.db.get_value(
				...
				vmraid.get_doc(method).insert(ignore_permissions=True)
	...
	def test_payment_request_linkings(self):
		...
		pr = make_payment_request(
			dt="Sales Order",
			dn=so_inr.name,
			recipient_id="saurabh@erpadda.com",
	...
	def test_payment_entry(self):
		...
		vmraid.db.set_value("Company", "_Test Company", "write_off_account", "_Test Write Off - _TC")
		vmraid.db.set_value("Company", "_Test Company", "cost_center", "_Test Cost Center - _TC")
		...
		so_inr = vmraid.get_doc("Sales Order", so_inr.name)
	...
	def test_multiple_payment_entries_against_sales_order(self):
		...
		pr1 = make_payment_request(
			dt="Sales Order", dn=so.name, recipient_id="nabin@erpadda.com", return_doc=1
		)
		pr1.grand_total = 200
		pr1.submit()
		...
		pr2.grand_total = 900
		self.assertRaises(vmraid.ValidationError, pr2.save)
