...
from vmraid.utils import add_days, add_months, format_date, getdate, today
from vmraid.utils.jinja import validate_template
from vmraid.utils.pdf import get_pdf
from vmraid.www.printview import get_print_style
...
from erpadda.accounts.report.general_ledger.general_ledger import execute as get_soa
...
class ProcessStatementOfAccounts(Document):
	def validate(self):
		...
		if not self.customers:
			vmraid.throw(_("Customers not selected."))
...
def get_report_pdf(doc, consolidated=True):
	...
	template_path = (
		"erpadda/accounts/doctype/process_statement_of_accounts/process_statement_of_accounts.html"
	)

	for entry in doc.customers:
		if doc.include_ageing:
			ageing_filters = vmraid._dict(
				{
					"company": doc.company,
		...
		tax_id = vmraid.get_doc("Customer", entry.customer).tax_id
		...
		html = vmraid.render_template(
			template_path,
			{
				...
				"terms_and_conditions": vmraid.db.get_value(
...
def get_customers_based_on_territory_or_customer_group(customer_collection, collection_name):
	...
	collection = vmraid.get_doc(customer_collection, collection_name)
	selected = [
		customer.name
		for customer in vmraid.get_list(
			customer_collection,
...
def get_customers_based_on_sales_person(sales_person):
	lft, rgt = vmraid.db.get_value("Sales Person", sales_person, ["lft", "rgt"])
	...
	sales_person_records = vmraid._dict()
	for d in records:
...
def get_recipients_and_cc(customer, doc):
	...
	if doc.cc_to != "":
		try:
			cc = [vmraid.get_value("User", doc.cc_to, "email")]
		except Exception:
			pass
...
def fetch_customers(customer_collection, collection_name, primary_mandatory):
	...
	if customer_collection == "Sales Person":
		...
		if not bool(customers):
			vmraid.throw(_("No Customers found with selected options."))
	else:
		if customer_collection == "Sales Partner":
			customers = vmraid.get_list(
				"Customer",
				fields=["name", "email_id"],
...
def get_customer_emails(customer_name, primary_mandatory, billing_and_primary=True):
	...
	if len(billing_email) == 0 or (billing_email[0][0] is None):
		if billing_and_primary:
			vmraid.throw(_("No billing email found for customer: {0}").format(customer_name))
		else:
			return ""

	if billing_and_primary:
		primary_email = vmraid.get_value("Customer", customer_name, "email_id")
		if primary_email is None and int(primary_mandatory):
			vmraid.throw(_("No primary email found for customer: {0}").format(customer_name))
...
def download_statements(document_name):
	doc = vmraid.get_doc("Process Statement Of Accounts", document_name)
	report = get_report_pdf(doc)
	if report:
		vmraid.local.response.filename = doc.name + ".pdf"
		vmraid.local.response.filecontent = report
...
def send_emails(document_name, from_scheduler=False):
	...
	if report:
		for customer, report_pdf in report.items():
			...
			subject = vmraid.render_template(doc.subject, context)
			message = vmraid.render_template(doc.body, context)
		...
		if doc.enable_auto_email and from_scheduler:
			...
			doc.add_comment(
				"Comment", "Emails sent on: " + vmraid.utils.format_datetime(vmraid.utils.now())
			)
