...
from vmraid.model.mapper import map_child_doc, map_doc
...
class POSInvoiceMergeLog(Document):
	...
	def validate_pos_invoice_status(self):
		for d in self.pos_invoices:
			status, docstatus, is_return, return_against = vmraid.db.get_value(
				"POS Invoice", d.pos_invoice, ["status", "docstatus", "is_return", "return_against"]
			)

			bold_pos_invoice = vmraid.bold(d.pos_invoice)
			bold_status = vmraid.bold(status)
			if docstatus != 1:
				vmraid.throw(_("Row #{}: POS Invoice {} is not submitted yet").format(d.idx, bold_pos_invoice))
			...
			if (
				...
				bold_return_against = vmraid.bold(return_against)
				return_against_status = vmraid.db.get_value("POS Invoice", return_against, "status")
				if return_against_status != "Consolidated":
					# if return entry is not getting merged in the current pos closing and if it is not consolidated
					bold_unconsolidated = vmraid.bold("not Consolidated")
	...
	def on_submit(self):
		pos_invoice_docs = [
			vmraid.get_cached_doc("POS Invoice", d.pos_invoice) for d in self.pos_invoices
		]
	...
	def get_new_sales_invoice(self):
		sales_invoice = vmraid.new_doc("Sales Invoice")
	...
	def cancel_linked_invoices(self):
		for si_name in [self.consolidated_invoice, self.consolidated_credit_note]:
			if not si_name:
				continue
			si = vmraid.get_doc("Sales Invoice", si_name)
...
def get_all_unconsolidated_invoices():
	...
	pos_invoices = vmraid.db.get_all(
		"POS Invoice",
		filters=filters,
		fields=[
			"name as pos_invoice",
...
def consolidate_pos_invoices(pos_invoices=None, closing_entry=None):
	...
	if vmraid.flags.in_test and not invoices:
...
def split_invoices(invoices):
	...
	special_invoices = []
	pos_return_docs = [
		vmraid.get_cached_doc("POS Invoice", d.pos_invoice)
		for d in invoices
	...
	for pos_invoice in pos_return_docs:
		for item in pos_invoice.items:
			...
			return_against_is_consolidated = (
				vmraid.db.get_value("POS Invoice", pos_invoice.return_against, "status", cache=True)
				== "Consolidated"
			)
...
def create_merge_logs(invoice_by_customer, closing_entry=None):
	try:
		...
		vmraid.db.rollback()
		message_log = vmraid.message_log.pop() if vmraid.message_log else str(e)
		...
		vmraid.db.commit()
		vmraid.publish_realtime("closing_process_complete", {"user": vmraid.session.user})


def cancel_merge_logs(merge_logs, closing_entry=None):
	try:
		for log in merge_logs:
			merge_log = vmraid.get_doc("POS Invoice Merge Log", log)
