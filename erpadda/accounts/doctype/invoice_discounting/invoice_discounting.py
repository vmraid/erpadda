...
class InvoiceDiscounting(AccountsController):
	...
	def validate_mandatory(self):
		if self.docstatus == 1 and not (self.loan_start_date and self.loan_period):
			vmraid.throw(_("Loan Start Date and Loan Period are mandatory to save the Invoice Discounting"))

	def validate_invoices(self):
		discounted_invoices = [
			record.sales_invoice
			for record in vmraid.get_all(
			...
			if record.sales_invoice in discounted_invoices:
				vmraid.throw(
					_("Row({0}): {1} is already discounted in {2}").format(
						record.idx, vmraid.bold(record.sales_invoice), vmraid.bold(record.parent)
					)
				)

			actual_outstanding = vmraid.db.get_value(
			...
			if record.outstanding_amount > actual_outstanding:
				vmraid.throw(
					_(
						...
						record.idx, vmraid.bold(actual_outstanding), vmraid.bold(record.sales_invoice)
					)
				)
	...
	def set_status(self, status=None, cancel=0):
		if status:
			...
			for d in self.invoices:
				vmraid.get_doc("Sales Invoice", d.sales_invoice).set_status(update=True, update_modified=False)
		else:
			self.status = "Draft"
			if self.docstatus == 1:
	...
	def update_sales_invoice(self):
		for d in self.invoices:
			if self.docstatus == 1:
				...
				discounted_invoice = vmraid.db.exists(
					{"doctype": "Discounted Invoice", "sales_invoice": d.sales_invoice, "docstatus": 1}
				)
				is_discounted = 1 if discounted_invoice else 0
			vmraid.db.set_value("Sales Invoice", d.sales_invoice, "is_discounted", is_discounted)
	...
	def make_gl_entries(self):
		...
		for d in self.invoices:
			inv = vmraid.db.get_value("Sales Invoice", d.sales_invoice, invoice_fields, as_dict=1)

			if d.outstanding_amount:
				outstanding_in_company_currency = flt(
					d.outstanding_amount * inv.conversion_rate, d.precision("outstanding_amount")
				)
				ar_credit_account_currency = vmraid.get_cached_value(
	...
	def create_disbursement_entry(self):
		...
		je.append(
			"accounts",
			{
				...
				"cost_center": erpadda.get_default_cost_center(self.company),
			},
		)
	...
	def close_loan(self):
		...
		if getdate(self.loan_end_date) > getdate(nowdate()):
			for d in self.invoices:
				outstanding_amount = vmraid.db.get_value(
...
def get_party_account_based_on_invoice_discounting(sales_invoice):
	party_account = None
	invoice_discounting = vmraid.db.sql(
		"""
