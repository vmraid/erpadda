...
from erpadda.accounts.report.sales_register.sales_register import get_mode_of_payments
from erpadda.selling.report.item_wise_sales_history.item_wise_sales_history import (
...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		for tax in tax_columns:
			...
			row.update(
				{
					vmraid.scrub(tax + " Rate"): item_tax.get("tax_rate", 0),
					vmraid.scrub(tax + " Amount"): item_tax.get("tax_amount", 0),
				}
			)
...
def get_group_by_conditions(filters, doctype):
	if filters.get("group_by") == "Invoice":
		...
		return "ORDER BY `tab{0} Item`.{1}".format(doctype, vmraid.scrub(filters.get("group_by")))
	elif filters.get("group_by") in ("Customer", "Customer Group", "Territory", "Supplier"):
		return "ORDER BY `tab{0}`.{1}".format(doctype, vmraid.scrub(filters.get("group_by")))
...
def get_delivery_notes_against_sales_order(item_list):
	so_dn_map = vmraid._dict()
	so_item_rows = list(set([d.so_detail for d in item_list]))

	if so_item_rows:
		delivery_notes = vmraid.db.sql(
			"""
...
def get_tax_accounts(
	...
	tax_amount_precision = (
		get_field_precision(
			vmraid.get_meta(tax_doctype).get_field("tax_amount"), currency=company_currency
		)
		or 2
	)
				...
				for item_code, tax_data in item_wise_tax_detail.items():
					...
					for d in item_row_map.get(parent, {}).get(item_code, []):
						...
						if item_tax_amount:
							...
							itemised_tax.setdefault(d.name, {})[description] = vmraid._dict(
	...
	for desc in tax_columns:
		columns.append(
			{
				"label": _(desc + " Rate"),
				"fieldname": vmraid.scrub(desc + " Rate"),
				"fieldtype": "Float",
				"width": 100,
			}
		)
		...
		columns.append(
			{
				"label": _(desc + " Amount"),
				"fieldname": vmraid.scrub(desc + " Amount"),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 100,
			}
...
def get_display_value(filters, group_by_field, item):
	if filters.get("group_by") == "Item":
		...
		party = vmraid.scrub(filters.get("group_by"))
...
def get_group_by_and_display_fields(filters):
	if filters.get("group_by") == "Item":
		...
		group_by_field = vmraid.scrub(filters.get("group_by"))
		subtotal_display_field = "item_code"
...
def add_sub_total_row(item, total_row_map, group_by_value, tax_columns):
	...
	for tax in tax_columns:
		total_row.setdefault(vmraid.scrub(tax + " Amount"), 0.0)
		total_row[vmraid.scrub(tax + " Amount")] += flt(item[vmraid.scrub(tax + " Amount")])
