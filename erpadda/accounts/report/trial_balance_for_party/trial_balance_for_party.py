...
def get_data(filters, show_party_name):
	if filters.get("party_type") in ("Customer", "Supplier", "Employee", "Member"):
		party_name_field = "{0}_name".format(vmraid.scrub(filters.get("party_type")))
...
def get_opening_balances(filters):
	...
	if filters.get("account"):
		account_filter = "and account = %s" % (vmraid.db.escape(filters.get("account")))
...
def get_balances_within_period(filters):
	...
	balances_within_period = vmraid._dict()
	for d in gle:
...
def is_party_name_visible(filters):
	...
	if filters.get("party_type") in ["Customer", "Supplier"]:
		if filters.get("party_type") == "Customer":
			party_naming_by = vmraid.db.get_single_value("Selling Settings", "cust_master_name")
		else:
			party_naming_by = vmraid.db.get_single_value("Buying Settings", "supp_master_name")
