...
def get_accounts_data(based_on, company):
	if based_on == "cost_center":
		...
		return vmraid.get_all("Project", fields=["name"], filters={"company": company}, order_by="name")
	else:
		filters = {}
		doctype = vmraid.unscrub(based_on)
		has_company = vmraid.db.has_column(doctype, "company")

		if has_company:
			filters.update({"company": company})

		return vmraid.get_all(doctype, fields=["name"], filters=filters, order_by="name")
