...
from vmraid.utils import add_days, add_months, cint, cstr, flt, formatdate, get_first_day, getdate
...
def get_period_list(
	...
	for i in range(cint(math.ceil(months / months_to_add))):
		period = vmraid._dict({"from_date": start_date})
...
def validate_fiscal_year(fiscal_year, from_fiscal_year, to_fiscal_year):
	if not fiscal_year.get("year_start_date") or not fiscal_year.get("year_end_date"):
		vmraid.throw(_("Start Year and End Year are mandatory"))

	if getdate(fiscal_year.get("year_end_date")) < getdate(fiscal_year.get("year_start_date")):
		vmraid.throw(_("End Year cannot be before Start Year"))


def validate_dates(from_date, to_date):
	if not from_date or not to_date:
		vmraid.throw(_("From Date and To Date are mandatory"))

	if to_date < from_date:
		vmraid.throw(_("To Date cannot be less than From Date"))
...
def set_gl_entries_by_account(
	...
	if accounts:
		additional_conditions += " and account in ({})".format(
			", ".join(vmraid.db.escape(d) for d in accounts)
		)
		...
		if filters.get("include_default_book_entries"):
			gl_filters["company_fb"] = vmraid.db.get_value("Company", company, "default_finance_book")
...
def get_cost_centers_with_children(cost_centers):
	...
	for d in cost_centers:
		if vmraid.db.exists("Cost Center", d):
			lft, rgt = vmraid.db.get_value("Cost Center", d, ["lft", "rgt"])
			children = vmraid.get_all("Cost Center", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
			all_cost_centers += [c.name for c in children]
		else:
			vmraid.throw(_("Cost Center: {0} does not exist").format(d))
