...
def execute(filters=None):
	...
	filters.currency = vmraid.get_cached_value("Company", filters.company, "default_currency")

	gross_profit_data = GrossProfitGenerator(filters)

	data = []

	group_wise_columns = vmraid._dict(
		{
			"invoice": [
				"invoice_or_item",
				"customer",
...
class GrossProfitGenerator(object):
	def __init__(self, filters=None):
		...
		self.filters = vmraid._dict(filters)
	...
	def process(self):
		...
		self.currency_precision = cint(vmraid.db.get_default("currency_precision")) or 3
		self.float_precision = cint(vmraid.db.get_default("float_precision")) or 2
		...
		for row in reversed(self.si_list):
			...
			product_bundles = []
			if row.update_stock:
				product_bundles = self.product_bundles.get(row.parenttype, {}).get(row.parent, vmraid._dict())
			elif row.dn_detail:
				product_bundles = self.product_bundles.get("Delivery Note", {}).get(
					row.delivery_note, vmraid._dict()
				)
	...
	def get_returned_invoice_items(self):
		returned_invoices = vmraid.db.sql(
			"""
			select
		...
		self.returned_invoices = vmraid._dict()
		for inv in returned_invoices:
			self.returned_invoices.setdefault(inv.return_against, vmraid._dict()).setdefault(
				inv.item_code, []
			).append(inv)
	...
	def get_last_purchase_rate(self, item_code, row):
		purchase_invoice = vmraid.qb.DocType("Purchase Invoice")
		purchase_invoice_item = vmraid.qb.DocType("Purchase Invoice Item")

		query = (
			vmraid.qb.from_(purchase_invoice_item)
		...
		query.orderby(purchase_invoice.posting_date, order=vmraid.qb.desc)
		query.limit(1)
		...
		for index, row in enumerate(self.si_list):
			if parents_index < len(parents) and row.parent == parents[parents_index]:
				...
				if not row.indent:
					...
					if vmraid.db.exists("Product Bundle", row.item_code):
		...
		return vmraid._dict(
			{
				...
				"base_net_amount": vmraid.db.get_value("Sales Invoice", row.parent, "base_net_total"),
			}
		)
	...
	def load_product_bundle(self):
		...
		for d in vmraid.db.sql(
			...
			self.product_bundles.setdefault(d.parenttype, vmraid._dict()).setdefault(
				d.parent, vmraid._dict()
			).setdefault(d.parent_item, []).append(d)

	def load_non_stock_items(self):
		self.non_stock_items = vmraid.db.sql_list(
