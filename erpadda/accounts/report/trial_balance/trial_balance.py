...
from vmraid.utils import cstr, flt, formatdate, getdate
...
def validate_filters(filters):
	if not filters.fiscal_year:
		vmraid.throw(_("Fiscal Year {0} is required").format(filters.fiscal_year))

	fiscal_year = vmraid.db.get_value(
		"Fiscal Year", filters.fiscal_year, ["year_start_date", "year_end_date"], as_dict=True
	)
	if not fiscal_year:
		vmraid.throw(_("Fiscal Year {0} does not exist").format(filters.fiscal_year))
	else:
...
def get_data(filters):
	...
	company_currency = filters.presentation_currency or erpadda.get_company_currency(filters.company)
...
def get_rootwise_opening_balances(filters, report_type):
	...
	if filters.cost_center:
		lft, rgt = vmraid.db.get_value("Cost Center", filters.cost_center, ["lft", "rgt"])
	...
	query_filters = {
		...
		"company_fb": vmraid.db.get_value("Company", filters.company, "default_finance_book"),
	}
