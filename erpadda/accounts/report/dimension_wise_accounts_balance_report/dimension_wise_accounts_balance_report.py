...
from erpadda.accounts.report.trial_balance.trial_balance import validate_filters
...
def get_data(filters, dimension_list):
	company_currency = erpadda.get_company_currency(filters.company)
	...
	min_lft, max_rgt = vmraid.db.sql(
		"""select min(lft), max(rgt) from `tabAccount`
		where company=%s""",
		(filters.company),
	)[0]

	account = vmraid.db.sql_list(
	...
	format_gl_entries(
		gl_entries_by_account, accounts_by_name, dimension_list, vmraid.scrub(filters.get("dimension"))
	)
...
def set_gl_entries_by_account(dimension_list, filters, account, gl_entries_by_account):
	...
	if account:
		condition += " and account in ({})".format(", ".join([vmraid.db.escape(d) for d in account]))
	...
	if filters.get("include_default_book_entries"):
		gl_filters["company_fb"] = vmraid.db.get_value(
	...
	gl_entries = vmraid.db.sql(
		...
		order by account, posting_date""".format(
			dimension=vmraid.scrub(filters.get("dimension")), condition=condition
		),
		gl_filters,
		as_dict=True,
	)  # nosec
		...
		for entry in entries:
			...
			for dimension in dimension_list:
				if dimension == entry.get(dimension_type):
					d[vmraid.scrub(dimension)] = (
						d.get(vmraid.scrub(dimension), 0.0) + flt(entry.debit) - flt(entry.credit)
					)
...
def prepare_data(accounts, filters, company_currency, dimension_list):
	...
	for d in accounts:
		...
		for dimension in dimension_list:
			row[vmraid.scrub(dimension)] = flt(d.get(vmraid.scrub(dimension), 0.0), 3)

			if abs(row[vmraid.scrub(dimension)]) >= 0.005:
				# ignore zero values
				has_value = True
				total += flt(d.get(vmraid.scrub(dimension), 0.0), 3)
...
def accumulate_values_into_parents(accounts, accounts_by_name, dimension_list):
	...
	for d in reversed(accounts):
		if d.parent_account:
			for dimension in dimension_list:
				accounts_by_name[d.parent_account][vmraid.scrub(dimension)] = accounts_by_name[
					d.parent_account
				].get(vmraid.scrub(dimension), 0.0) + d.get(vmraid.scrub(dimension), 0.0)


def get_condition(dimension):
	conditions = []

	conditions.append("{0} in %(dimensions)s".format(vmraid.scrub(dimension)))

	return " and {}".format(" and ".join(conditions)) if conditions else ""


def get_dimensions(filters):
	meta = vmraid.get_meta(filters.get("dimension"), cached=False)
	query_filters = {}

	if meta.has_field("company"):
		query_filters = {"company": filters.get("company")}

	return vmraid.get_all(filters.get("dimension"), filters=query_filters, pluck="name")
...
def get_columns(dimension_list):
	...
	for dimension in dimension_list:
		columns.append(
			{
				"fieldname": vmraid.scrub(dimension),
				"label": dimension,
				"fieldtype": "Currency",
				"options": "currency",
				"width": 150,
