...
import erpadda
from erpadda.accounts.report.balance_sheet.balance_sheet import (
	get_chart_data,
...
from erpadda.accounts.report.cash_flow.cash_flow import (
	add_total_row_account,
	get_account_type_based_gl_data,
	get_cash_flow_accounts,
)
from erpadda.accounts.report.cash_flow.cash_flow import get_report_summary as get_cash_flow_summary
from erpadda.accounts.report.financial_statements import (
	filter_out_zero_value_rows,
	get_fiscal_year_data,
	sort_accounts,
)
from erpadda.accounts.report.profit_and_loss_statement.profit_and_loss_statement import (
...
from erpadda.accounts.report.utils import convert, convert_to_presentation_currency
...
def execute(filters=None):
	...
	if filters.get("report") == "Balance Sheet":
		...
		if cint(vmraid.db.get_single_value("Accounts Settings", "use_custom_cash_flow")):
			from erpadda.accounts.report.cash_flow.custom_cash_flow import execute as execute_custom
...
def get_company_currency(filters=None):
	return filters.get("presentation_currency") or vmraid.get_cached_value(
			...
			if d:
				...
				for company in companies:
					...
					if (
						...
						parent_company_currency = erpadda.get_company_currency(d.company)
						child_company_currency = erpadda.get_company_currency(entry.company)
...
def get_subsidiary_companies(company):
	lft, rgt = vmraid.get_cached_value("Company", company, ["lft", "rgt"])
...
def set_gl_entries_by_account(
	...
	company_lft, company_rgt = vmraid.get_cached_value(
		"Company", filters.get("company"), ["lft", "rgt"]
	)

	additional_conditions = get_additional_conditions(from_date, ignore_closing_entries, filters)
	companies = vmraid.db.sql(
	...
	for d in companies:
		gl_entries = vmraid.db.sql(
			...
			),
			{
				...
				"company_fb": vmraid.db.get_value("Company", d.name, "default_finance_book"),
			},
			as_dict=True,
		)
