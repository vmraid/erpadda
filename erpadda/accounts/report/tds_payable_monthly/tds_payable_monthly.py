...
def get_result(filters, tds_docs, tds_accounts, tax_category_map):
	...
	for name, details in gle_map.items():
		...
		if tds_deducted:
			row = {
				"pan"
				if vmraid.db.has_column("Supplier", "pan")
...
def get_supplier_pan_map():
	supplier_map = vmraid._dict()
	suppliers = vmraid.db.get_all(
...
def get_columns(filters):
	pan = "pan" if vmraid.db.has_column("Supplier", "pan") else "tax_id"
	columns = [
		{"label": _(vmraid.unscrub(pan)), "fieldname": pan, "fieldtype": "Data", "width": 90},
		{
			"label": _("Supplier"),
			"options": "Supplier",
			"fieldname": "supplier",
...
def get_tds_docs(filters):
	...
	tax_category_map = {}
	or_filters = {}
	bank_accounts = vmraid.get_all("Account", {"is_group": 0, "account_type": "Bank"}, pluck="name")

	tds_accounts = vmraid.get_all(
