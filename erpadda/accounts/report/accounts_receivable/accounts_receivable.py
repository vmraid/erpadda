...
from vmraid.utils import cint, cstr, flt, getdate, nowdate

from erpadda.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimension_with_children,
)
from erpadda.accounts.utils import get_currency_precision
...
class ReceivablePayableReport(object):
	...
	def run(self, args):
		...
		self.set_defaults()
		self.party_naming_by = vmraid.db.get_value(
	...
	def set_defaults(self):
		if not self.filters.get("company"):
			self.filters.company = vmraid.db.get_single_value("Global Defaults", "default_company")
		self.company_currency = vmraid.get_cached_value(
	...
	def init_voucher_balance(self):
		...
		for gle in self.gl_entries:
			...
			if not key in self.voucher_balance:
				self.voucher_balance[key] = vmraid._dict(
	...
	def build_delivery_note_map(self):
		if self.invoices and self.filters.show_delivery_notes:
			self.delivery_notes = vmraid._dict()

			# delivery note link inside sales invoice
			si_against_dn = vmraid.db.sql(
				"""
	...
	def get_invoice_details(self):
		self.invoice_details = vmraid._dict()
		if self.party_type == "Customer":
			si_list = vmraid.db.sql(
				"""
	...
	def get_payment_terms(self, row):
		...
		payment_terms_details = vmraid.db.sql(
			"""
			select
		...
		for d in payment_terms_details:
			term = vmraid._dict(original_row)
		...
		for key in ("paid", "credit_note"):
			if row[key] > 0:
				if not additional_row:
					additional_row = vmraid._dict(row)
	...
	def get_future_payments(self):
		if self.filters.show_future_payments:
			self.future_payments = vmraid._dict()
	...
	def get_return_entries(self):
		...
		self.return_entries = vmraid._dict(
			vmraid.get_all(doctype, filters, ["name", "return_against"], as_list=1)
		)
	...
	def get_sales_invoices_or_customers_based_on_sales_person(self):
		if self.filters.get("sales_person"):
			lft, rgt = vmraid.db.get_value("Sales Person", self.filters.get("sales_person"), ["lft", "rgt"])
			...
			self.sales_person_records = vmraid._dict()
			for d in records:
	...
	def get_cost_center_conditions(self, conditions):
		lft, rgt = vmraid.db.get_value("Cost Center", self.filters.cost_center, ["lft", "rgt"])
		cost_center_list = [
			center.name
			for center in vmraid.get_list("Cost Center", filters={"lft": (">=", lft), "rgt": ("<=", rgt)})
		]
	...
	def get_hierarchical_filters(self, doctype, key):
		lft, rgt = vmraid.db.get_value(doctype, self.filters.get(key), ["lft", "rgt"])
	...
	def get_party_details(self, party):
		if not party in self.party_details:
			if self.party_type == "Customer":
				self.party_details[party] = vmraid.db.get_value(
					"Customer",
					party,
	...
	def get_chart_data(self):
		...
		for row in self.data:
			...
			if not cint(row.bold):
				...
				precision = cint(vmraid.db.get_default("float_precision")) or 2
