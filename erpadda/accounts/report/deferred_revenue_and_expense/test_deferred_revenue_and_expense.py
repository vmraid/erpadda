...
from erpadda.accounts.report.deferred_revenue_and_expense.deferred_revenue_and_expense import (
...
class TestDeferredRevenueAndExpense(unittest.TestCase):
	...
	def test_deferred_expense(self):
		...
		pi = make_purchase_invoice(
			...
			is_return=False,
			update_stock=False,
			posting_date=vmraid.utils.datetime.date(2021, 5, 1),
			parent_cost_center="Main - _CD",
			cost_center="Main - _CD",
			do_not_save=True,
			rate=300,
...
def create_company():
	company = vmraid.db.exists("Company", "_Test Company DR")
	if not company:
