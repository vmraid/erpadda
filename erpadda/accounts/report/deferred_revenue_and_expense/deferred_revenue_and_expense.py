...
import vmraid
from vmraid import _, qb
from vmraid.query_builder import Column, functions
from vmraid.utils import add_days, date_diff, flt, get_first_day, get_last_day, rounded

from erpadda.accounts.report.financial_statements import get_period_list
...
class Deferred_Item(object):
	...
	def report_data(self):
		...
		"""
		ret_data = vmraid._dict({"name": self.item_name})
	...
	def make_dummy_gle(self, name, date, amount):
		"""
		return - vmraid._dict() of a dummy gle entry
		"""
		entry = vmraid._dict(
	...
	def calculate_item_revenue_expense_for_period(self):
		...
		for period in self.period_list:
			...
			self.period_total.append(
				vmraid._dict({"key": period.key, "total": period_sum, "actual": actual})
			)
...
class Deferred_Invoice(object):
	def __init__(self, invoice, items, filters, period_list):
		...
		items - list : vmraid._dict() with item details. Refer Deferred_Item for required fields
		"""
		self.name = invoice
	...
	def calculate_invoice_revenue_expense_for_period(self):
		...
		for period in self.period_list:
			self.period_total.append(vmraid._dict({"key": period.key, "total": 0, "actual": 0}))
	...
	def report_data(self):
		...
		"""
		ret_data = []
		inv_total = vmraid._dict({"name": self.name})
...
class Deferred_Revenue_and_Expense_Report(object):
	def __init__(self, filters=None):
		...
		if not filters:
			fiscal_year = vmraid.get_doc("Fiscal Year", vmraid.defaults.get_user_default("fiscal_year"))
	...
	def generate_report_data(self):
		...
		if ret is not []:
			if self.filters.type == "Revenue":
				total_row = vmraid._dict({"name": "Total Deferred Income"})
			elif self.filters.type == "Expense":
				total_row = vmraid._dict({"name": "Total Deferred Expense"})
