...
from vmraid.utils import add_to_date, flt, get_date_str

from erpadda.accounts.report.financial_statements import get_columns, get_data, get_period_list
...
def get_accounts_in_mappers(mapping_names):
	cfm = vmraid.qb.DocType("Cash Flow Mapping")
	cfma = vmraid.qb.DocType("Cash Flow Mapping Accounts")
	result = (
		vmraid.qb.select(
			cfma.name,
			cfm.label,
...
def setup_mappers(mappers):
	...
	for mapping in mappers:
		...
		doc = vmraid.get_doc("Cash Flow Mapper", mapping["name"])
...
def _get_account_type_based_data(
	...
	from erpadda.accounts.report.cash_flow.cash_flow import get_start_date

	company = filters.company
	data = {}
	total = 0
	GLEntry = vmraid.qb.DocType("GL Entry")
	Account = vmraid.qb.DocType("Account")

	for period in period_list:
		start_date = get_start_date(period, accumulated_values, company)

		account_subquery = (
			vmraid.qb.from_(Account)
