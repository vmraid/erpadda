...
def get_account_type_based_gl_data(company, start_date, end_date, account_type, filters=None):
	...
	if filters.include_default_book_entries:
		company_fb = vmraid.db.get_value("Company", company, "default_finance_book")
		cond = """ AND (finance_book in (%s, %s, '') OR finance_book IS NULL)
			""" % (
			vmraid.db.escape(filters.finance_book),
			vmraid.db.escape(company_fb),
		)
	else:
		cond = " AND (finance_book in (%s, '') OR finance_book IS NULL)" % (
			vmraid.db.escape(cstr(filters.finance_book))
		)

	gl_sum = vmraid.db.sql_list(
		"""
