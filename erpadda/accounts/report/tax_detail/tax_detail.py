...
def run_report(report_name, data):
	...
	report_config = json.loads(vmraid.get_doc("Report", report_name).json)
		...
		for component_name, component in section.items():
			...
			if component["type"] == "section":
				if component_name == section_name:
					vmraid.throw(_("A report component cannot refer to its parent section") + ": " + section_name)
				try:
...
def filter_match(value, string):
	...
	try:
		...
		return vmraid.safe_eval(f"{value} {operator} {num}")
	except ValueError:
		if operator == "<":
			return True
		return False
...
def get_custom_reports(name=None):
	...
	if name:
		filters["name"] = name
	reports = vmraid.get_list("Report", filters=filters, fields=["name", "json"], as_list=False)
...
def save_custom_report(reference_report, report_name, data):
	if reference_report != "Tax Detail":
		vmraid.throw(_("The wrong report is referenced."))
	if report_name == "Tax Detail":
		vmraid.throw(_("The parent report cannot be overwritten."))
	...
	try:
		newdoc = vmraid.get_doc(doc)
		newdoc.insert()
		vmraid.msgprint(_("Report created successfully"))
	except vmraid.exceptions.DuplicateEntryError:
		dbdoc = vmraid.get_doc("Report", report_name)
		dbdoc.update(doc)
		dbdoc.save()
