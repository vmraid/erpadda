...
vmraid.query_reports["Tax Detail"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			label: __("From Date"),
			fieldtype: "Date",
			default: vmraid.datetime.month_start(vmraid.datetime.get_today()),
			reqd: 1,
			width: "60px"
		},
		{
			...
			label: __("To Date"),
			fieldtype: "Date",
			default: vmraid.datetime.month_end(vmraid.datetime.get_today()),
			reqd: 1,
			width: "60px"
		},
		{
			...
			fieldtype: "Read Only",
			default: vmraid.query_report.report_name,
			hidden: 1,
			reqd: 1
		},
		{
...
function hide_filters(report) {
	report.page.page_form[0].querySelectorAll('.form-group.vmraid-control').forEach(function setHidden(field) {
...
erpadda.TaxDetail = class TaxDetail {
	constructor() {
		this.patch();
		this.load_report();
	}
	...
	refresh_report() {
		...
		// already run within vmraid.run_serially
		this.loading = true;
		this.super.refresh_report.apply(this.qr);
		if (this.qr.report_name !== 'Tax Detail') {
			vmraid.call({
				method: 'erpadda.accounts.report.tax_detail.tax_detail.get_custom_reports',
	...
	save_report() {
		...
		if (this.qr.report_name !== 'Tax Detail') {
			vmraid.call({
				method:'erpadda.accounts.report.tax_detail.tax_detail.save_custom_report',
				args: {
	...
	check_datatable() {
		if (!this.qr.datatable) {
			vmraid.throw(__('Please change the date range to load data first'));
		}
	}
	set_section(name) {
	...
	create_controls() {
		...
		controls['delete_section'] = this.qr.page.add_field({
			...
			click: () => {
				...
				if (cur_section) {
					vmraid.confirm(__('Are you sure you want to delete section') + ' ' + cur_section + '?',
			...
			click: () => {
				...
				if (section_name) {
					...
					if (component_type === 'filter') {
						...
						vmraid.throw(__('Please select the Component Type first'));
					}
				} else {
					vmraid.throw(__('Please select the Section first'));
				}
			}
		});
		controls['delete_component'] = this.qr.page.add_field({
			...
			click: () => {
				...
				if (component) {
					vmraid.confirm(__('Are you sure you want to delete component') + ' ' + component + '?',
...
if (!window.taxdetail) {
	window.taxdetail = new erpadda.TaxDetail();
}
	...
	const dialog = new vmraid.ui.Dialog({
		...
		primary_action: function new_report_pa(values) {
			vmraid.call({
				...
				vmraid.set_route('query-report', values.report_name);
			});
			dialog.hide();
		}
	});
