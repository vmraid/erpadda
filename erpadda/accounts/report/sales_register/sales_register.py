...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	company_currency = vmraid.get_cached_value("Company", filters.get("company"), "default_currency")
	...
	for inv in invoice_list:
		...
		for income_acc in income_accounts:
			...
			row.update({vmraid.scrub(income_acc): income_amount})

		# Add amount in unrealized account
		for account in unrealized_profit_loss_accounts:
			row.update(
				{vmraid.scrub(account + "_unrealized"): flt(internal_invoice_map.get((inv.name, account)))}
			)
		...
		for tax_acc in tax_accounts:
			if tax_acc not in income_accounts:
				tax_amount_precision = (
					get_field_precision(
						vmraid.get_meta("Sales Taxes and Charges").get_field("tax_amount"), currency=company_currency
					)
					or 2
				)
				tax_amount = flt(invoice_tax_map.get(inv.name, {}).get(tax_acc), tax_amount_precision)
				total_tax += tax_amount
				row.update({vmraid.scrub(tax_acc): tax_amount})
...
def get_columns(invoice_list, additional_table_columns):
	...
	if invoice_list:
		income_accounts = vmraid.db.sql_list(
		...
		tax_accounts = vmraid.db.sql_list(
		...
		unrealized_profit_loss_accounts = vmraid.db.sql_list(
	...
	for account in income_accounts:
		income_columns.append(
			{
				"label": account,
				"fieldname": vmraid.scrub(account),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
	...
	for account in unrealized_profit_loss_accounts:
		unrealized_profit_loss_account_columns.append(
			{
				"label": account,
				"fieldname": vmraid.scrub(account + "_unrealized"),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
...
def get_invoice_income_map(invoice_list):
	...
	for d in income_details:
		invoice_income_map.setdefault(d.parent, vmraid._dict()).setdefault(d.income_account, [])
...
def get_internal_invoice_map(invoice_list):
	unrealized_amount_details = vmraid.db.sql(
...
def get_invoice_tax_map(invoice_list, invoice_income_map, income_accounts):
	...
	for d in tax_details:
		if d.account_head in income_accounts:
			...
			invoice_tax_map.setdefault(d.parent, vmraid._dict()).setdefault(d.account_head, [])
...
def get_invoice_so_dn_map(invoice_list):
	...
	for d in si_items:
		if d.sales_order:
			invoice_so_dn_map.setdefault(d.parent, vmraid._dict()).setdefault("sales_order", []).append(
				d.sales_order
			)
		...
		if d.delivery_note:
			...
			delivery_note_list = vmraid.db.sql_list(
		...
		if delivery_note_list:
			invoice_so_dn_map.setdefault(d.parent, vmraid._dict()).setdefault(
...
def get_invoice_cc_wh_map(invoice_list):
	...
	for d in si_items:
		if d.cost_center:
			invoice_cc_wh_map.setdefault(d.parent, vmraid._dict()).setdefault("cost_center", []).append(
				d.cost_center
			)

		if d.warehouse:
			invoice_cc_wh_map.setdefault(d.parent, vmraid._dict()).setdefault("warehouse", []).append(
				d.warehouse
			)
