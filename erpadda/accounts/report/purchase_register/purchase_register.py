...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	company_currency = vmraid.get_cached_value("Company", filters.company, "default_currency")
...
def get_columns(invoice_list, additional_table_columns):
	...
	if invoice_list:
		expense_accounts = vmraid.db.sql_list(
...
def get_invoice_expense_map(invoice_list):
	...
	for d in expense_details:
		invoice_expense_map.setdefault(d.parent, vmraid._dict()).setdefault(d.expense_account, [])
...
def get_invoice_po_pr_map(invoice_list):
	...
	for d in pi_items:
		if d.purchase_order:
			invoice_po_pr_map.setdefault(d.parent, vmraid._dict()).setdefault("purchase_order", []).append(
				d.purchase_order
			)
		...
		if pr_list:
			invoice_po_pr_map.setdefault(d.parent, vmraid._dict()).setdefault("purchase_receipt", pr_list)

		if d.project:
			invoice_po_pr_map.setdefault(d.parent, vmraid._dict()).setdefault("project", []).append(
				d.project
			)
