...
class PartyLedgerSummaryReport(object):
	...
	def get_data(self):
		company_currency = vmraid.get_cached_value(
			"Company", self.filters.get("company"), "default_currency"
		)
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Customer" else "debit"

		self.party_data = vmraid._dict({})
	...
	def prepare_conditions(self):
		...
		if self.filters.party_type == "Customer":
			...
			if self.filters.get("territory"):
				lft, rgt = vmraid.db.get_value("Territory", self.filters.get("territory"), ["lft", "rgt"])
	...
	def get_party_adjustment_amounts(self):
		...
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Customer" else "debit"
		round_off_account = vmraid.get_cached_value("Company", self.filters.company, "round_off_account")
		...
		for voucher_gl_entries in adjustment_voucher_entries.values():
			...
			for gle in voucher_gl_entries:
				...
				elif vmraid.get_cached_value("Account", gle.account, "account_type") == income_or_expense:
