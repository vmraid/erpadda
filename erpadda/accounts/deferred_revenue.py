	...
	for item in doc.items:
		...
		if item.service_stop_date:
			if date_diff(item.service_stop_date, item.service_start_date) < 0:
				vmraid.throw(_("Service Stop Date cannot be before Service Start Date"))

			if date_diff(item.service_stop_date, item.service_end_date) > 0:
				vmraid.throw(_("Service Stop Date cannot be after Service End Date"))
		...
		if (
			...
			vmraid.throw(_("Cannot change Service Stop Date for item in row {0}").format(item.idx))
...
def build_conditions(process_type, account, company):
	...
	if account:
		...
		conditions += f"AND p.company = {vmraid.db.escape(company)}"
...
def convert_deferred_expense_to_expense(
	...
	for invoice in invoices:
		doc = vmraid.get_doc("Purchase Invoice", invoice)
		book_deferred_income_or_expense(doc, deferred_process, end_date)

	if vmraid.flags.deferred_accounting_error:
...
def convert_deferred_revenue_to_income(
	...
	for invoice in invoices:
		doc = vmraid.get_doc("Sales Invoice", invoice)
		book_deferred_income_or_expense(doc, deferred_process, end_date)
...
def get_already_booked_amount(doc, item):
	...
	gl_entries_details = vmraid.db.sql(
		"""
	...
	journal_entry_details = vmraid.db.sql(
		"""
...
def book_deferred_income_or_expense(doc, deferred_process, posting_date=None):
	...
	accounts_frozen_upto = vmraid.get_cached_value("Accounts Settings", "None", "acc_frozen_upto")
	...
	via_journal_entry = cint(
		vmraid.db.get_singles_value("Accounts Settings", "book_deferred_entries_via_journal_entry")
	)
	submit_journal_entry = cint(
		vmraid.db.get_singles_value("Accounts Settings", "submit_journal_entries")
	)
	book_deferred_entries_based_on = vmraid.db.get_singles_value(
	...
	if gl_entries:
		try:
			...
			if vmraid.flags.in_test:
				...
				vmraid.flags.deferred_accounting_error = True
...
def get_deferred_booking_accounts(doctype, voucher_detail_no, dr_or_cr):
	...
	if doctype == "Sales Invoice":
		credit_account, debit_account = vmraid.db.get_value(
			"Sales Invoice Item",
