...
from vmraid.utils import cint, cstr, flt, formatdate, get_number_format_info, getdate, now, nowdate

import erpadda

# imported to enable erpadda.accounts.utils.get_account_currency
from erpadda.accounts.doctype.account.account import get_account_currency  # noqa
from erpadda.stock import get_warehouse_account_map
from erpadda.stock.utils import get_stock_value_on


class FiscalYearError(vmraid.ValidationError):
	pass


class PaymentEntryUnlinkError(vmraid.ValidationError):
	pass
...
def get_fiscal_years(
	...
	fiscal_years = vmraid.cache().hget("fiscal_years", company) or []

	if not fiscal_years:
		# if year start date is 2012-04-01, year end date should be 2013-03-31 (hence subdate)
		cond = ""
		if fiscal_year:
			cond += " and fy.name = {0}".format(vmraid.db.escape(fiscal_year))
		if company:
			cond += """
		...
		vmraid.cache().hset("fiscal_years", company, fiscal_years)
	...
	if company:
		error_msg = _("""{0} for {1}""").format(error_msg, vmraid.bold(company))

	if verbose == 1:
		vmraid.msgprint(error_msg)
...
def get_balance_on(
	...
	if not account and vmraid.form_dict.get("account"):
		account = vmraid.form_dict.get("account")
	if not date and vmraid.form_dict.get("date"):
		date = vmraid.form_dict.get("date")
	if not party_type and vmraid.form_dict.get("party_type"):
		party_type = vmraid.form_dict.get("party_type")
	if not party and vmraid.form_dict.get("party"):
		party = vmraid.form_dict.get("party")
	if not cost_center and vmraid.form_dict.get("cost_center"):
		cost_center = vmraid.form_dict.get("cost_center")

	cond = ["is_cancelled=0"]
	if date:
		cond.append("posting_date <= %s" % vmraid.db.escape(cstr(date)))
	else:
	...
	if cost_center and report_type == "Profit and Loss":
		cc = vmraid.get_doc("Cost Center", cost_center)
		if cc.is_group:
			cond.append(
				""" exists (
			...
			cond.append("""gle.cost_center = %s """ % (vmraid.db.escape(cost_center, percent=False),))

	if account:

		if not (vmraid.flags.ignore_account_permission or ignore_account_permission):
		...
		if acc.is_group:
			...
			if acc.account_currency == vmraid.get_cached_value("Company", acc.company, "default_currency"):
				in_account_currency = False
		else:
			cond.append("""gle.account = %s """ % (vmraid.db.escape(account, percent=False),))

	if party_type and party:
		cond.append(
			"""gle.party_type = %s and gle.party = %s """
			% (vmraid.db.escape(party_type), vmraid.db.escape(party, percent=False))
		)

	if company:
		cond.append("""gle.company = %s """ % (vmraid.db.escape(company, percent=False)))
...
def get_count_on(account, fieldname, date):
	...
	if account:
		...
		if not vmraid.flags.ignore_account_permission:
			acc.check_permission("read")
...
def add_cc(args=None):
	...
	if args.parent_cost_center == args.company:
		args.parent_cost_center = "{0} - {1}".format(
			args.parent_cost_center, vmraid.get_cached_value("Company", args.company, "abbr")
		)
...
def reconcile_against_document(args):
	...
	for key, entries in reconciled_entries.items():
		...
		doc = vmraid.get_doc(voucher_type, voucher_no)
		vmraid.flags.ignore_party_validation = True
		...
		doc = vmraid.get_doc(entry.voucher_type, entry.voucher_no)
		doc.make_gl_entries(cancel=0, adv_adj=1)
		vmraid.flags.ignore_party_validation = False
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		party_account_field = (
			"paid_from" if erpadda.get_party_account_type(args.party_type) == "Receivable" else "paid_to"
		)
...
def validate_allocated_amount(args):
	precision = args.get("precision") or vmraid.db.get_single_value(
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
	...
	new_row.update((vmraid.copy_doc(jv_detail)).as_dict())
...
def update_reference_in_payment_entry(d, payment_entry, do_not_save=False):
	...
	if d.difference_amount and d.difference_account:
		account_details = {
			...
			or vmraid.get_cached_value("Company", payment_entry.company, "cost_center"),
		}
...
def unlink_ref_doc_from_payment_entries(ref_doc):
	...
	vmraid.db.sql(
		...
		(now(), vmraid.session.user, ref_doc.doctype, ref_doc.name),
	)
...
def remove_ref_doc_link_from_jv(ref_type, ref_no):
	...
	if linked_jv:
		vmraid.db.sql(
			...
			and docstatus < 2""",
			(now(), vmraid.session.user, ref_type, ref_no),
		)

		vmraid.msgprint(_("Journal Entries {0} are un-linked").format("\n".join(linked_jv)))


def remove_ref_doc_link_from_pe(ref_type, ref_no):
	linked_pe = vmraid.db.sql_list(
	...
	if linked_pe:
		...
		for pe in linked_pe:
			try:
				pe_doc = vmraid.get_doc("Payment Entry", pe)
				pe_doc.set_amounts()
				...
				vmraid.throw(msg, exc=PaymentEntryUnlinkError, title=_("Payment Unlink Error"))
		...
		vmraid.msgprint(_("Payment Entries {0} are un-linked").format("\n".join(linked_pe)))


@vmraid.whitelist()
def get_company_default(company, fieldname, ignore_validation=False):
	value = vmraid.get_cached_value("Company", company, fieldname)

	if not ignore_validation and not value:
		throw(
			_("Please set default {0} in Company {1}").format(
				vmraid.get_meta("Company").get_label(fieldname), company
			)
		)
...
def get_stock_rbnb_difference(posting_date, company):
	stock_items = vmraid.db.sql_list(
		"""select distinct item_code
		from `tabStock Ledger Entry` where company=%s""",
		company,
	)

	pr_valuation_amount = vmraid.db.sql(
		"""
	...
	pi_valuation_amount = vmraid.db.sql(
		"""
	...
	stock_rbnb_account = "Stock Received But Not Billed - " + vmraid.get_cached_value(
...
def get_outstanding_invoices(party_type, party, account, condition=None, filters=None):
	...
	precision = vmraid.get_precision("Sales Invoice", "outstanding_amount") or 2

	if account:
		root_type, account_type = vmraid.get_cached_value(
			"Account", account, ["root_type", "account_type"]
		)
		party_account_type = "Receivable" if root_type == "Asset" else "Payable"
		party_account_type = account_type or party_account_type
	else:
		party_account_type = erpadda.get_party_account_type(party_type)
...
def get_account_name(
	...
	return vmraid.db.get_value(
		"Account",
		{
			...
			"account_currency": account_currency or vmraid.defaults.get_defaults().currency,
			"company": company or vmraid.defaults.get_defaults().company,
		},
		"name",
	)
...
def get_companies():
	...
	return [d.name for d in vmraid.get_list("Company", fields=["name"], order_by="name")]


@vmraid.whitelist()
def get_children(doctype, parent, company, is_root=False):
	from erpadda.accounts.report.financial_statements import sort_accounts
	...
	acc = vmraid.get_list(doctype, fields=fields, filters=filters)
...
def create_payment_gateway_account(gateway, payment_channel="Email"):
	from erpadda.setup.setup_wizard.operations.install_fixtures import create_bank_account
	...
	bank_account = vmraid.db.get_value(
		"Account",
	...
	if not bank_account:
		vmraid.msgprint(_("Payment Gateway Account not created, please create one manually."))
		return
...
def update_cost_center(docname, cost_center_name, cost_center_number, company, merge):
	...
	if cost_center_number:
		vmraid.db.set_value("Cost Center", docname, "cost_center_number", cost_center_number.strip())
	else:
		vmraid.db.set_value("Cost Center", docname, "cost_center_number", "")

	vmraid.db.set_value("Cost Center", docname, "cost_center_name", cost_center_name.strip())

	new_name = get_autoname_with_number(cost_center_number, cost_center_name, docname, company)
	if docname != new_name:
		vmraid.rename_doc("Cost Center", docname, new_name, force=1, merge=merge)
		return new_name
...
def validate_field_number(doctype_name, docname, number_value, company, field_name):
	...
	if number_value:
		...
		doctype_with_same_number = vmraid.db.get_value(doctype_name, filters)
...
def get_autoname_with_number(number_value, doc_title, name, company):
	...
	if name:
		...
		abbr = vmraid.get_cached_value("Company", company, ["abbr"], as_dict=True)
...
def get_coa(doctype, parent, is_root, chart=None):
	from erpadda.accounts.doctype.account.chart_of_accounts.chart_of_accounts import (
		build_tree_from_json,
	)

	# add chart to flags to retrieve when called from expand all function
	chart = chart if chart else vmraid.flags.chart
	vmraid.flags.chart = chart
...
def repost_gle_for_stock_vouchers(
	...
	precision = get_field_precision(vmraid.get_meta("GL Entry").get_field("debit")) or 2

	gle = get_voucherwise_gl_entries(stock_vouchers, posting_date)
	for voucher_type, voucher_no in stock_vouchers:
		existing_gle = gle.get((voucher_type, voucher_no), [])
		voucher_obj = vmraid.get_cached_doc(voucher_type, voucher_no)
...
def get_future_stock_vouchers(
	...
	future_stock_vouchers = vmraid.db.sql(
...
def get_stock_and_account_balance(account=None, posting_date=None, company=None):
	...
	precision = vmraid.get_precision("Journal Entry Account", "debit_in_account_currency")
...
def check_and_delete_linked_reports(report):
	...
	icons = vmraid.get_all("Desktop Icon", fields=["name"], filters={"_report": report})
	if icons:
		for icon in icons:
			vmraid.delete_doc("Desktop Icon", icon)
