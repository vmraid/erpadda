...
import vmraid
from vmraid.tests.utils import VMRaidTestCase, change_settings, timeout
from vmraid.utils import add_days, add_months, cint, flt, now, today

from erpadda.manufacturing.doctype.job_card.job_card import JobCardCancelError
from erpadda.manufacturing.doctype.production_plan.test_production_plan import make_bom
from erpadda.manufacturing.doctype.work_order.work_order import (
	CapacityError,
	ItemHasVariantError,
	OverProductionError,
...
from erpadda.stock.doctype.item.test_item import create_item, make_item
from erpadda.stock.doctype.stock_entry import test_stock_entry
from erpadda.stock.doctype.warehouse.test_warehouse import create_warehouse
from erpadda.stock.utils import get_bin


class TestWorkOrder(VMRaidTestCase):
	def setUp(self):
	...
	def check_planned_qty(self):
		...
		# from stores to wip
		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 4))
		...
		# from wip to fg
		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 4))
		s.insert()
		s.submit()

		self.assertEqual(vmraid.db.get_value("Work Order", wo_order.name, "produced_qty"), 4)

		planned2 = vmraid.db.get_value(
			"Bin", {"item_code": "_Test FG Item", "warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty"
		)
	...
	def test_over_production(self):
		...
		s = vmraid.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 7))
		s.insert()
	...
	def test_reserved_qty_for_partial_completion(self):
		...
		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
		s.submit()
	...
	def test_production_item(self):
		...
		vmraid.db.set_value("Item", "_Test FG Item", "end_of_life", "2000-1-1")

		self.assertRaises(vmraid.ValidationError, wo_order.save)

		vmraid.db.set_value("Item", "_Test FG Item", "end_of_life", None)
		vmraid.db.set_value("Item", "_Test FG Item", "disabled", 1)

		self.assertRaises(vmraid.ValidationError, wo_order.save)

		vmraid.db.set_value("Item", "_Test FG Item", "disabled", 0)
	...
	def test_reserved_qty_for_production_on_stock_entry(self):
		...
		s = vmraid.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 2))
		...
		s = vmraid.get_doc(make_stock_entry(self.wo_order.name, "Manufacture", 2))
	...
	def test_backflush_qty_for_overpduction_manufacture(self):
		...
		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)

		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)
		...
		s1 = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 50))
		s1.submit()
		...
		for ste in cancel_stock_entry:
			doc = vmraid.get_doc("Stock Entry", ste)
			doc.cancel()
	...
	def test_reserved_qty_for_stopped_production(self):
		...
		s = vmraid.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 1))
	...
	def test_scrap_material_qty(self):
		...
		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 2))
		for d in s.get("items"):
			d.s_warehouse = "Stores - _TC"
		s.insert()
		s.submit()

		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		s.insert()
		s.submit()

		wo_order_details = vmraid.db.get_value(
	...
	def test_allow_overproduction(self):
		...
		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 3))
		s.insert()
		self.assertRaises(StockOverProductionError, s.submit)
	...
	def test_work_order_with_non_stock_item(self):
		...
		if not vmraid.db.get_value("Item Price", {"item_code": "_Test FG Non Stock Item"}):
			vmraid.get_doc(
				{
					"doctype": "Item Price",
		...
		if not vmraid.db.get_value("BOM", {"item": fg_item}):
		...
		se = vmraid.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		se.insert()
		se.submit()

		ste = vmraid.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		ste.insert()
	...
	def test_job_card(self):
		stock_entries = []
		bom = vmraid.get_doc("BOM", {"docstatus": 1, "with_operations": 1, "company": "_Test Company"})
		...
		ste = vmraid.get_doc(make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		ste.submit()
		stock_entries.append(ste)

		job_cards = vmraid.get_all(
			"Job Card", filters={"work_order": work_order.name}, order_by="creation asc"
		)
		self.assertEqual(len(job_cards), len(bom.operations))

		for i, job_card in enumerate(job_cards):
			doc = vmraid.get_doc("Job Card", job_card)
			doc.time_logs[0].completed_qty = 1
			doc.submit()

		ste1 = vmraid.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		ste1.submit()
	...
	def test_capcity_planning(self):
		...
		if data:
			...
			vmraid.db.set_value("Manufacturing Settings", None, {"capacity_planning_for_days": 30})
	...
	def test_work_order_with_non_transfer_item(self):
		...
		wo = make_wo_order_test_record(production_item=fg_item)
		ste = vmraid.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		ste.insert()
		ste.submit()
		self.assertEqual(len(ste.items), 1)
		ste1 = vmraid.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
	...
	def test_batch_size_for_fg_item(self):
		...
		vmraid.db.set_value("Manufacturing Settings", None, "make_serial_no_batch_from_work_order", 0)
		...
		vmraid.db.set_value("Manufacturing Settings", None, "make_serial_no_batch_from_work_order", 1)
		ste1 = vmraid.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
		...
		ste1 = vmraid.get_doc(make_stock_entry(work_order.name, "Manufacture", 30))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
	...
	def test_partial_material_consumption(self):
		vmraid.db.set_value("Manufacturing Settings", None, "material_consumption", 1)
		...
		ste1 = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		ste1.submit()
		ste_cancel_list.append(ste1)

		ste3 = vmraid.get_doc(make_stock_entry(wo_order.name, "Material Consumption for Manufacture", 2))
		...
		vmraid.db.set_value("Manufacturing Settings", None, "material_consumption", 0)
	...
	def test_extra_material_transfer(self):
		...
		ste3 = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		...
		ste2 = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		...
		vmraid.db.set_value("Manufacturing Settings", None, "backflush_raw_materials_based_on", "BOM")
	...
	def test_make_stock_entry_for_customer_provided_item(self):
		...
		if not vmraid.db.exists("BOM", {"item": finished_item}):
		...
		ste = vmraid.get_doc(make_stock_entry(wo.name, purpose="Material Transfer for Manufacture"))
		ste.insert()
	...
	def test_valuation_rate_missing_on_make_stock_entry(self):
		...
		if not vmraid.db.get_value("BOM", {"item": item_name}):
		...
		stock_entry = vmraid.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture"))
		self.assertRaises(vmraid.ValidationError, stock_entry.save)

	def test_wo_completion_with_pl_bom(self):
		from erpadda.manufacturing.doctype.bom.test_bom import (
		...
		if not vmraid.db.exists("BOM", bom_no):
		...
		se = vmraid.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", qty))
		se.get("items")[0].s_warehouse = "Stores - _TC"
		se.insert()
		se.submit()

		se = vmraid.get_doc(make_stock_entry(wo.name, "Manufacture", qty))
		se.insert()
		se.submit()
		...
		self.assertEqual(vmraid.db.get_value("Work Order", wo.name, "produced_qty"), qty)
		self.assertEqual(vmraid.db.get_value("Work Order", wo.name, "process_loss_qty"), total_pl_qty)
	...
	def test_job_card_scrap_item(self):
		...
		if not vmraid.db.get_value("BOM", {"item": item}):
			bom = make_bom(
		...
		job_card = vmraid.db.get_value("Job Card", {"work_order": wo_order.name}, "name")
		update_job_card(job_card)

		stock_entry = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
		...
		job_card = vmraid.db.get_value("Job Card", {"work_order": wo_order.name, "docstatus": 0}, "name")
		update_job_card(job_card, 10)
	...
	def test_close_work_order(self):
		...
		job_cards = vmraid.db.get_value("Job Card", {"work_order": wo_order.name}, "name")

		if len(job_cards) == len(bom.operations):
			for jc in job_cards:
				job_card_doc = vmraid.get_doc("Job Card", jc)
				job_card_doc.append(
					"time_logs",
	...
	def test_partial_manufacture_entries(self):
		...
		sm = vmraid.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 100))
		...
		s = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 90))
		...
		s1 = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
		...
		s2 = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
	...
	def test_auto_batch_creation(self):
		from erpadda.manufacturing.doctype.bom.test_bom import create_nested_bom

		fg_item = vmraid.generate_hash(length=20)
		child_item = vmraid.generate_hash(length=20)

		bom_tree = {fg_item: {child_item: {}}}

		create_nested_bom(bom_tree, prefix="")

		item = vmraid.get_doc("Item", fg_item)
		item.has_batch_no = 1
	...
	def test_manufacture_entry_mapped_idx_with_exploded_bom(self):
		...
		ste_manu = vmraid.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
	...
	def test_work_order_multiple_material_transfer(self):
		...
		final_transfer_entry = vmraid.get_doc(  # transfer last RM with For Quantity = 0
...
def update_job_card(job_card, jc_qty=None):
	employee = vmraid.db.get_value("Employee", {"status": "Active"}, "name")
	job_card_doc = vmraid.get_doc("Job Card", job_card)
	job_card_doc.set(
		"scrap_items",
		[
...
def allow_overproduction(fieldname, percentage):
	doc = vmraid.get_doc("Manufacturing Settings")
...
test_records = vmraid.get_test_records("Work Order")
