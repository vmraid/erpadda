...
from erpadda.manufacturing.doctype.bom.bom import (
	get_bom_item_rate,
	get_bom_items_as_dict,
	validate_bom_no,
)
from erpadda.manufacturing.doctype.manufacturing_settings.manufacturing_settings import (
	get_mins_between_operations,
)
from erpadda.stock.doctype.batch.batch import make_batch
from erpadda.stock.doctype.item.item import get_item_defaults, validate_end_of_life
...
from erpadda.stock.utils import get_bin, get_latest_stock_qty, validate_warehouse_company
from erpadda.utilities.transaction_base import validate_uom_is_integer


class OverProductionError(vmraid.ValidationError):
	pass


class CapacityError(vmraid.ValidationError):
	pass


class StockOverProductionError(vmraid.ValidationError):
	pass


class OperationTooLongError(vmraid.ValidationError):
	pass


class ItemHasVariantError(vmraid.ValidationError):
	pass


class SerialNoQtyError(vmraid.ValidationError):
	pass


class WorkOrder(Document):
	def onload(self):
		ms = vmraid.get_doc("Manufacturing Settings")
	...
	def validate_sales_order(self):
		if self.sales_order:
			...
			if len(so):
				...
				vmraid.throw(_("Sales Order {0} is not valid").format(self.sales_order))

	def check_sales_order_on_hold_or_close(self):
		status = vmraid.db.get_value("Sales Order", self.sales_order, "status")
		if status in ("Closed", "On Hold"):
			vmraid.throw(_("Sales Order {0} is {1}").format(self.sales_order, status))

	def set_default_warehouse(self):
		if not self.wip_warehouse:
			self.wip_warehouse = vmraid.db.get_single_value(
				"Manufacturing Settings", "default_wip_warehouse"
			)
		if not self.fg_warehouse:
			self.fg_warehouse = vmraid.db.get_single_value("Manufacturing Settings", "default_fg_warehouse")
	...
	def validate_work_order_against_so(self):
		# already ordered qty
		ordered_qty_against_so = vmraid.db.sql(
	...
	def update_production_plan_status(self):
		production_plan = vmraid.get_doc("Production Plan", self.production_plan)
		produced_qty = 0
		if self.production_plan_item:
			total_qty = vmraid.get_all(
				"Work Order",
	...
	def on_submit(self):
		if not self.wip_warehouse and not self.skip_transfer:
			vmraid.throw(_("Work-in-Progress Warehouse is required before Submit"))
		if not self.fg_warehouse:
			vmraid.throw(_("For Warehouse is required before Submit"))

		if self.production_plan and vmraid.db.exists(
	...
	def create_serial_no_batch_no(self):
		...
		if not cint(
			vmraid.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")
		):
			return
	...
	def create_batch_for_finished_good(self):
		...
		batch_auto_creation = vmraid.get_cached_value("Item", self.production_item, "create_new_batch")
		if not batch_auto_creation:
			vmraid.msgprint(
				_("Batch not created for item {} since it does not have a batch series.").format(
					vmraid.bold(self.production_item)
				),
				alert=True,
				indicator="orange",
			)
	...
	def delete_auto_created_batch_and_serial_no(self):
		for row in vmraid.get_all("Serial No", filters={"work_order": self.name}):
			vmraid.delete_doc("Serial No", row.name)
			self.db_set("serial_no", "")

		for row in vmraid.get_all("Batch", filters={"reference_name": self.name}):
			vmraid.delete_doc("Batch", row.name)

	def make_serial_nos(self, args):
		self.serial_no = clean_serial_no_string(self.serial_no)
		serial_no_series = vmraid.get_cached_value("Item", self.production_item, "serial_no_series")
		if serial_no_series:
	...
	def create_job_card(self):
		manufacturing_settings_doc = vmraid.get_doc("Manufacturing Settings")
	...
	def set_operation_start_end_time(self, idx, row):
		...
		if row.planned_start_time == row.planned_end_time:
			vmraid.throw(_("Capacity Planning Error, planned start time can not be same as end time"))

	def validate_cancel(self):
		if self.status == "Stopped":
			vmraid.throw(_("Stopped Work Order cannot be cancelled, Unstop it first to cancel"))

		# Check whether any stock entry exists against this Work Order
		stock_entry = vmraid.db.sql(
			"""select name from `tabStock Entry`
		...
		if stock_entry:
			vmraid.throw(
				_("Cannot cancel because submitted Stock Entry {0} exists").format(
					vmraid.utils.get_link_to_form("Stock Entry", stock_entry[0][0])
				)
			)
	...
	def update_planned_qty(self):
		...
		if self.material_request:
			mr_obj = vmraid.get_doc("Material Request", self.material_request)
	...
	def update_ordered_qty(self):
		if (
			...
			qty = vmraid.get_value("Production Plan Item", self.production_plan_item, "ordered_qty") or 0.0
			...
			vmraid.db.set_value("Production Plan Item", self.production_plan_item, "ordered_qty", qty)

			doc = vmraid.get_doc("Production Plan", self.production_plan)
			doc.set_status()
	...
	def update_work_order_qty_in_so(self):
		...
		if self.product_bundle_item:
			total_bundle_qty = vmraid.db.sql(
				""" select sum(qty) from
				`tabProduct Bundle Item` where parent = %s""",
				(vmraid.db.escape(self.product_bundle_item)),
			)[0][0]
	...
	def update_work_order_qty_in_combined_so(self):
		...
		prod_plan = vmraid.get_doc("Production Plan", self.production_plan)
		item_reference = vmraid.get_value(
	...
	def update_completed_qty_in_material_request(self):
		if self.material_request:
			vmraid.get_doc("Material Request", self.material_request).update_completed_qty(
	...
	def set_work_order_operations(self):
		...
		self.set("operations", [])
		if not self.bom_no or not vmraid.get_cached_value("BOM", self.bom_no, "with_operations"):
			return

		operations = []

		if self.use_multi_level_bom:
			bom_tree = vmraid.get_doc("BOM", self.bom_no).get_tree_representation()
		...
		bom_qty = vmraid.get_cached_value("BOM", self.bom_no, "quantity")
	...
	def update_operation_status(self):
		...
		for d in self.get("operations"):
			if not d.completed_qty:
				...
				vmraid.throw(_("Completed Qty cannot be greater than 'Qty to Manufacture'"))
	...
	def delete_job_card(self):
		for d in vmraid.get_all("Job Card", ["name"], {"work_order": self.name}):
			vmraid.delete_doc("Job Card", d.name)

	def validate_production_item(self):
		if vmraid.get_cached_value("Item", self.production_item, "has_variants"):
			vmraid.throw(_("Work Order cannot be raised against a Item Template"), ItemHasVariantError)
	...
	def validate_qty(self):
		if not self.qty > 0:
			vmraid.throw(_("Quantity to Manufacture must be greater than 0."))
	...
	def validate_operation_time(self):
		for d in self.operations:
			if not d.time_in_mins > 0:
				vmraid.throw(_("Operation Time must be greater than 0 for Operation {0}").format(d.operation))
	...
	def update_batch_produced_qty(self, stock_entry_doc):
		...
		for row in stock_entry_doc.items:
			if row.batch_no and (row.is_finished_item or row.is_scrap_item):
				...
				vmraid.db.set_value("Batch", row.batch_no, "produced_qty", flt(qty))
...
def get_bom_operations(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.get_all("BOM Operation", filters=filters, fields=["operation"], as_list=1)
...
def get_item_details(item, project=None, skip_bom_info=False):
	...
	res["bom_no"] = vmraid.db.get_value("BOM", filters=filters)

	if not res["bom_no"]:
		variant_of = vmraid.db.get_value("Item", item, "variant_of")

		if variant_of:
			res["bom_no"] = vmraid.db.get_value("BOM", filters={"item": variant_of, "is_default": 1})
	...
	if not res["bom_no"]:
		if project:
			...
			vmraid.throw(_("Default BOM for {0} not found").format(item))

	bom_data = vmraid.db.get_value(
		"BOM",
		res["bom_no"],
...
def make_work_order(bom_no, item, qty=0, project=None, variant_items=None):
	if not vmraid.has_permission("Work Order", "write"):
		vmraid.throw(_("Not permitted"), vmraid.PermissionError)

	item_details = get_item_details(item, project)

	wo_doc = vmraid.new_doc("Work Order")
...
def add_variant_item(variant_items, wo_doc, bom_no, table_name="items"):
	...
	for item in variant_items:
		...
		bom_doc = vmraid.get_cached_doc("BOM", bom_no)
...
def set_work_order_ops(name):
	po = vmraid.get_doc("Work Order", name)
...
def make_stock_entry(work_order_id, purpose, qty=None):
	work_order = vmraid.get_doc("Work Order", work_order_id)
	...
	if work_order.bom_no:
		stock_entry.inspection_required = vmraid.db.get_value(
...
def get_default_warehouse():
	doc = vmraid.get_cached_doc("Manufacturing Settings")
...
def stop_unstop(work_order, status):
	...
	pro_order = vmraid.get_doc("Work Order", work_order)

	if pro_order.status == "Closed":
		vmraid.throw(_("Closed Work Order can not be stopped or Re-opened"))

	pro_order.update_status(status)
	pro_order.update_planned_qty()
	vmraid.msgprint(_("Work Order has been {0}").format(status))
...
def split_qty_based_on_batch_size(wo_doc, row, qty):
	if not cint(
		vmraid.db.get_value("Operation", row.operation, "create_job_card_based_on_batch_size")
	):
...
def create_pick_list(source_name, target_doc=None, for_qty=None):
	...
	max_finished_goods_qty = vmraid.db.get_value("Work Order", source_name, "qty")
	...
	def update_item_quantity(source, target, source_parent):
		...
		if qty:
			...
			target.uom = vmraid.get_value("Item", source.item_code, "stock_uom")
