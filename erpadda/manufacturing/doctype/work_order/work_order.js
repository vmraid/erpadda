...
vmraid.ui.form.on("Work Order", {
	setup: function(frm) {
		...
		frm.set_query("bom_no", function() {
			if (frm.doc.production_item) {
				...
				vmraid.msgprint(__("Please enter Production Item first"));
			}
		});
		...
		frm.set_query("operation", "required_items", function() {
			return {
				query: "erpadda.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
	...
	onload: function(frm) {
		...
		if(frm.doc.__islocal) {
			...
			erpadda.work_order.set_default_warehouse(frm);
		}
	},
	...
	refresh: function(frm) {
		...
		erpadda.work_order.set_custom_buttons(frm);
		frm.set_intro("");
	...
	make_job_card: function(frm) {
		...
		const dialog = vmraid.prompt({fieldname: 'operations', fieldtype: 'Table', label: __('Operations'),
			fields: [
				{
					fieldtype: 'Link',
					fieldname: 'operation',
			...
			vmraid.call({
				method: "erpadda.manufacturing.doctype.work_order.work_order.make_job_card",
				freeze: true,
				args: {
	...
	production_item: function(frm) {
		if (frm.doc.production_item) {
			vmraid.call({
				method: "erpadda.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function(r) {
					if(r.message) {
						...
						erpadda.in_production_item_onchange = true;
						...
						erpadda.in_production_item_onchange = false;
					}
				}
			});
		}
	...
	project: function(frm) {
		if(!erpadda.in_production_item_onchange && !frm.doc.bom_no) {
	...
	set_sales_order: function(frm) {
		if(frm.doc.production_item) {
			vmraid.call({
				method: "erpadda.manufacturing.doctype.work_order.work_order.query_sales_order",
	...
	additional_operating_cost: function(frm) {
		erpadda.work_order.calculate_cost(frm.doc);
		erpadda.work_order.calculate_total_cost(frm);
	},
});

vmraid.ui.form.on("Work Order Item", {
	source_warehouse: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if(!row.item_code) {
			vmraid.throw(__("Please set the Item Code first"));
			...
			vmraid.call({
				...
				callback: function (r) {
					vmraid.model.set_value(row.doctype, row.name,
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (row.item_code) {
			vmraid.call({
				method: "erpadda.stock.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company
				},
				callback: function(r) {
					if (r.message) {
						vmraid.model.set_value(cdt, cdn, {
							"required_qty": 1,
...
vmraid.ui.form.on("Work Order Operation", {
...
erpadda.work_order = {
	set_custom_buttons: function(frm) {
		...
		if (doc.docstatus === 1 && doc.status != "Closed") {
			frm.add_custom_button(__('Close'), function() {
				vmraid.confirm(__("Once the Work Order is Closed. It can't be resumed."),
					() => {
						erpadda.work_order.change_work_order_status(frm, "Closed");
					}
				);
			}, __("Status"));
			...
			if (doc.status != 'Stopped' && doc.status != 'Completed') {
				frm.add_custom_button(__('Stop'), function() {
					erpadda.work_order.change_work_order_status(frm, "Stopped");
				}, __("Status"));
			} else if (doc.status == 'Stopped') {
				frm.add_custom_button(__('Re-open'), function() {
					erpadda.work_order.change_work_order_status(frm, "Resumed");
				}, __("Status"));
			}
			...
			if (show_start_btn) {
				...
				if (pending_to_transfer && frm.doc.status != 'Stopped') {
					...
					frm.add_custom_button(__('Create Pick List'), function() {
						erpadda.work_order.create_pick_list(frm);
					});
					var start_btn = frm.add_custom_button(__('Start'), function() {
						erpadda.work_order.make_se(frm, 'Material Transfer for Manufacture');
					});
			...
			if(!frm.doc.skip_transfer){
				...
				&& frm.doc.status != 'Stopped') {
					...
					if (frm.doc.__onload && frm.doc.__onload.material_consumption == 1) {
						...
						if (counter > 0) {
							var consumption_btn = frm.add_custom_button(__('Material Consumption'), function() {
								...
								erpadda.work_order.make_consumption_se(frm, backflush_raw_materials_based_on);
							});
					...
					var finish_btn = frm.add_custom_button(__('Finish'), function() {
						erpadda.work_order.make_se(frm, 'Manufacture');
					});
	...
	calculate_cost: function(doc) {
		if (doc.operations){
			...
			for(var i=0;i<op.length;i++) {
				...
				vmraid.model.set_value('Work Order Operation', op[i].name,
	...
	set_default_warehouse: function(frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			vmraid.call({
				method: "erpadda.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				callback: function(r) {
					if (!r.exe) {
	...
	show_prompt_for_qty_input: function(frm, purpose) {
		...
		return new Promise((resolve, reject) => {
			vmraid.prompt({
				...
				if (data.qty > max) {
					vmraid.msgprint(__('Quantity must not be more than {0}', [max]));
					reject();
				}
	...
	make_se: function(frm, purpose) {
		this.show_prompt_for_qty_input(frm, purpose)
			.then(data => {
				return vmraid.xcall('erpadda.manufacturing.doctype.work_order.work_order.make_stock_entry', {
					'work_order_id': frm.doc.name,
					'purpose': purpose,
					'qty': data.qty
				});
			}).then(stock_entry => {
				vmraid.model.sync(stock_entry);
				vmraid.set_route('Form', stock_entry.doctype, stock_entry.name);
			});
	...
	create_pick_list: function(frm, purpose='Material Transfer for Manufacture') {
		this.show_prompt_for_qty_input(frm, purpose)
			.then(data => {
				return vmraid.xcall('erpadda.manufacturing.doctype.work_order.work_order.create_pick_list', {
					'source_name': frm.doc.name,
					'for_qty': data.qty
				});
			}).then(pick_list => {
				vmraid.model.sync(pick_list);
				vmraid.set_route('Form', pick_list.doctype, pick_list.name);
			});
	},
	...
	make_consumption_se: function(frm, backflush_raw_materials_based_on) {
		...
		vmraid.call({
			method:"erpadda.manufacturing.doctype.work_order.work_order.make_stock_entry",
			args: {
				"work_order_id": frm.doc.name,
	...
	change_work_order_status: function(frm, status) {
		...
		vmraid.call({
			method: `erpadda.manufacturing.doctype.work_order.work_order.${method_name}`,
			freeze: true,
