...
from vmraid.utils import cint, cstr, flt, today
from vmraid.website.website_generator import WebsiteGenerator

import erpadda
from erpadda.setup.utils import get_exchange_rate
from erpadda.stock.doctype.item.item import get_item_details
from erpadda.stock.get_item_details import get_conversion_factor, get_price_list_rate
...
class BOMTree:
	...
	def __create_tree(self):
		bom = vmraid.get_cached_doc("BOM", self.name)
...
class BOM(WebsiteGenerator):
	...
	def autoname(self):
		...
		if vmraid.db.exists("BOM", name):
			conflicting_bom = vmraid.get_doc("BOM", name)

			if conflicting_bom.item != self.item:
				msg = _("A BOM with name {0} already exists for item {1}.").format(
					vmraid.bold(name), vmraid.bold(conflicting_bom.item)
				)
	...
	def validate(self):
		self.route = vmraid.scrub(self.name).replace("_", "-")

		if not self.company:
			vmraid.throw(_("Please select a Company first."), title=_("Mandatory"))
	...
	def on_update(self):
		vmraid.cache().hdel("bom_children", self.name)
	...
	def on_cancel(self):
		vmraid.db.set(self, "is_active", 0)
		vmraid.db.set(self, "is_default", 0)
	...
	def get_item_det(self, item_code):
		...
		if not item:
			vmraid.throw(_("Item: {0} does not exist in the system").format(item_code))
	...
	def get_bom_material_detail(self, args=None):
		...
		if not args:
			args = vmraid.form_dict.get("args")
	...
	def validate_bom_currency(self, item):
		if (
			item.get("bom_no")
			and vmraid.db.get_value("BOM", item.get("bom_no"), "currency") != self.currency
		):
			vmraid.throw(
	...
	def update_cost(self, update_parent=True, from_child_bom=False, update_hour_rate=True, save=True):
		...
		if self.total_cost != existing_bom_cost and update_parent:
			parent_boms = vmraid.db.sql_list(
			...
			for bom in parent_boms:
				vmraid.get_doc("BOM", bom).update_cost(from_child_bom=True)

		if not from_child_bom:
			vmraid.msgprint(_("Cost Updated"), alert=True)
	...
	def manage_default_bom(self):
		...
		if self.is_default and self.is_active:
			...
			item = vmraid.get_doc("Item", self.item)
			if item.default_bom != self.name:
				vmraid.db.set_value("Item", self.item, "default_bom", self.name)
		elif (
			not vmraid.db.exists(dict(doctype="BOM", docstatus=1, item=self.item, is_default=1))
			and self.is_active
		):
			vmraid.db.set(self, "is_default", 1)
		else:
			vmraid.db.set(self, "is_default", 0)
			item = vmraid.get_doc("Item", self.item)
			if item.default_bom == self.name:
				vmraid.db.set_value("Item", self.item, "default_bom", None)
	...
	def validate_main_item(self):
		...
		if not item:
			vmraid.throw(_("Item {0} does not exist in the system or has expired").format(self.item))
		else:
			ret = vmraid.db.get_value("Item", self.item, ["description", "stock_uom", "item_name"])
			self.description = ret[0]
			self.uom = ret[1]
			self.item_name = ret[2]

		if not self.quantity:
			vmraid.throw(_("Quantity should be greater than 0"))

	def validate_currency(self):
		if self.rm_cost_as_per == "Price List":
			price_list_currency = vmraid.db.get_value("Price List", self.buying_price_list, "currency")
	...
	def validate_materials(self):
		...
		if not self.get("items"):
			vmraid.throw(_("Raw Materials cannot be blank."))
		...
		for m in self.get("items"):
			...
			if flt(m.qty) <= 0:
				vmraid.throw(_("Quantity required for Item {0} in row {1}").format(m.item_code, m.idx))
			check_list.append(m)
	...
	def check_recursion(self, bom_list=None):
		...
		def _throw_error(bom_name):
			vmraid.throw(_("BOM recursion: {0} cannot be parent or child of {0}").format(bom_name))
	...
	def traverse_tree(self, bom_list=None):
		def _get_children(bom_no):
			children = vmraid.cache().hget("bom_children", bom_no)
			if children is None:
				children = vmraid.db.sql_list(
					"""SELECT `bom_no` FROM `tabBOM Item`
					WHERE `parent`=%s AND `bom_no`!='' AND `parenttype`='BOM'""",
					bom_no,
				)
				vmraid.cache().hset("bom_children", bom_no, children)
			return children
	...
	def update_rate_and_time(self, row, update_hour_rate=False):
		if not row.hour_rate or update_hour_rate:
			hour_rate = flt(vmraid.get_cached_value("Workstation", row.workstation, "hour_rate"))
	...
	def company_currency(self):
		return erpadda.get_company_currency(self.company)
	...
	def add_exploded_items(self, save=True):
		...
		if save:
			vmraid.db.sql("""delete from `tabBOM Explosion Item` where parent=%s""", self.name)
	...
	def validate_bom_links(self):
		if not self.is_active:
			...
			if act_pbom and act_pbom[0][0]:
				vmraid.throw(_("Cannot deactivate or cancel BOM as it is linked with other BOMs"))
	...
	def validate_operations(self):
		if self.with_operations and not self.get("operations") and self.docstatus == 1:
			vmraid.throw(_("Operations cannot be left blank"))

		if self.with_operations:
			for d in self.operations:
				if not d.description:
					d.description = vmraid.db.get_value("Operation", d.operation, "description")
	...
	def validate_scrap_items(self):
		for item in self.scrap_items:
			...
			if item.item_code == self.item and not item.is_process_loss:
				...
				).format(vmraid.bold(item.item_code))
			...
			must_be_whole_number = vmraid.get_value("UOM", item.stock_uom, "must_be_whole_number")
			if item.is_process_loss and must_be_whole_number:
				msg = _(
					"Item: {0} with Stock UOM: {1} cannot be a Scrap/Loss Item as {1} is a whole UOM."
				).format(vmraid.bold(item.item_code), vmraid.bold(item.stock_uom))
...
def get_bom_item_rate(args, bom_doc):
	if bom_doc.rm_cost_as_per == "Valuation Rate":
		...
		rate = (
			...
			or flt(vmraid.db.get_value("Item", args["item_code"], "last_purchase_rate"))
		) * (args.get("conversion_factor") or 1)
	elif bom_doc.rm_cost_as_per == "Price List":
		if not bom_doc.buying_price_list:
			vmraid.throw(_("Please select Price List"))
		bom_args = vmraid._dict(
			{
				"doctype": "BOM",
		...
		item_doc = vmraid.get_cached_doc("Item", args.get("item_code"))
...
def get_valuation_rate(args):
	...
	if valuation_rate <= 0:
		last_valuation_rate = vmraid.db.sql(
	...
	if not valuation_rate:
		valuation_rate = vmraid.db.get_value("Item", args["item_code"], "valuation_rate")
...
def get_bom_items_as_dict(
	...
	if cint(fetch_exploded):
		...
		items = vmraid.db.sql(query, {"qty": qty, "bom": bom, "company": company}, as_dict=True)
	else:
		query = query.format(
			table="BOM Item",
			where_conditions="",
	...
	for item, item_details in item_dict.items():
		for d in [
			...
			company_in_record = vmraid.db.get_value(d[0], item_details.get(d[1]), "company")
			if not item_details.get(d[1]) or (company_in_record and company != company_in_record):
				item_dict[item][d[1]] = vmraid.get_cached_value("Company", company, d[2]) if d[2] else None
...
def validate_bom_no(item, bom_no):
	...
	if not bom.is_active:
		vmraid.throw(_("BOM {0} must be active").format(bom_no))
	if bom.docstatus != 1:
		if not getattr(vmraid.flags, "in_test", False):
			vmraid.throw(_("BOM {0} must be submitted").format(bom_no))
	if item:
		...
		if (
			...
			or bom.item.lower() == cstr(vmraid.db.get_value("Item", item, "variant_of")).lower()
		):
			rm_item_exists = True
		if not rm_item_exists:
			vmraid.throw(_("BOM {0} does not belong to Item {1}").format(bom_no, item))


@vmraid.whitelist()
def get_children(parent=None, is_root=False, **filters):
	if not parent or parent == "BOM":
		vmraid.msgprint(_("Please select a BOM"))
		return

	if parent:
		vmraid.form_dict.parent = parent

	if vmraid.form_dict.parent:
		bom_doc = vmraid.get_cached_doc("BOM", vmraid.form_dict.parent)
		vmraid.has_permission("BOM", doc=bom_doc, throw=True)

		bom_items = vmraid.get_all(
			"BOM Item",
			fields=["item_code", "bom_no as value", "stock_qty"],
			filters=[["parent", "=", vmraid.form_dict.parent]],
			order_by="idx",
		)

		item_names = tuple(d.get("item_code") for d in bom_items)

		items = vmraid.get_list(
			"Item",
		...
		for bom_item in bom_items:
			...
			bom_item.image = vmraid.db.escape(bom_item.image)
...
def add_additional_cost(stock_entry, work_order):
	...
	expenses_included_in_valuation = vmraid.get_cached_value(
...
def add_non_stock_items_cost(stock_entry, work_order, expense_account):
	bom = vmraid.get_doc("BOM", work_order.bom_no)
	...
	non_stock_items = vmraid.get_all(
		"Item",
		fields="name",
...
def add_operations_cost(stock_entry, work_order=None, expense_account=None):
	from erpadda.stock.doctype.stock_entry.stock_entry import get_operating_cost_per_unit
...
def get_bom_diff(bom1, bom2):
	from vmraid.model import table_fields

	if bom1 == bom2:
		vmraid.throw(
			_("BOM 1 {0} and BOM 2 {1} should not be same").format(vmraid.bold(bom1), vmraid.bold(bom2))
		)

	doc1 = vmraid.get_doc("BOM", bom1)
	doc2 = vmraid.get_doc("BOM", bom2)
...
def item_query(doctype, txt, searchfield, start, page_len, filters):
	meta = vmraid.get_meta("Item", cached=True)
	...
	if filters and filters.get("item_code"):
		has_variants = vmraid.get_cached_value("Item", filters.get("item_code"), "has_variants")
		if not has_variants:
...
def make_variant_bom(source_name, bom_no, item, variant_items, target_doc=None):
	from erpadda.manufacturing.doctype.work_order.work_order import add_variant_item
