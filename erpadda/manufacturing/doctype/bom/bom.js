...
vmraid.provide("erpadda.bom");

vmraid.ui.form.on("BOM", {
	setup: function(frm) {
		...
		frm.set_query("item", function() {
			return {
				query: "erpadda.manufacturing.doctype.bom.bom.item_query",
				filters: {
					"is_stock_item": 1
				}
			};
	...
	refresh: function(frm) {
		...
		if (frm.doc.has_variants) {
			...
			frm.$wrapper.find(".variants-intro").on("click", () => {
				vmraid.set_route("List", "Item", {"variant_of": frm.doc.item});
			});
		}
	},
	...
	make_work_order: function(frm) {
		frm.events.setup_variant_prompt(frm, "Work Order", (frm, item, data, variant_items) => {
			vmraid.call({
				method: "erpadda.manufacturing.doctype.work_order.work_order.make_work_order",
				args: {
					bom_no: frm.doc.name,
					item: item,
					qty: data.qty || 0.0,
	...
	make_variant_bom: function(frm) {
		frm.events.setup_variant_prompt(frm, "Variant BOM", (frm, item, data, variant_items) => {
			vmraid.call({
				method: "erpadda.manufacturing.doctype.bom.bom.make_variant_bom",
				args: {
	...
	setup_variant_prompt: function(frm, title, callback, skip_qty_field) {
				...
				fields: [
					...
					},
					{
						...
						get_query: function(data) {
							if (!data.item_code) {
								vmraid.throw(__("Select template item"));
							}
		...
		let dialog = vmraid.prompt(fields, data => {
			let item = data.item || frm.doc.item;
			let variant_items = data.items || [];

			variant_items.forEach(d => {
				if (!d.variant_item_code) {
					vmraid.throw(__("Select variant item code for the template item {0}", [d.item_code]));
				}
			})
	...
	make_quality_inspection: function(frm) {
		vmraid.model.open_mapped_doc({
			method: "erpadda.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm
		})
	},
		...
		if (frm.doc.routing) {
			vmraid.call({
				...
				callback: function(r) {
					if (!r.exc) {
						frm.refresh_fields();
						erpadda.bom.calculate_op_cost(frm.doc);
						erpadda.bom.calculate_total(frm.doc);
					}
				}
			});
		}
...
erpadda.bom.BomController = class BomController extends erpadda.TransactionController {
	conversion_rate(doc) {
		if(this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			erpadda.bom.update_cost(doc);
		}
	}
...
extend_cscript(cur_frm.cscript, new erpadda.bom.BomController({frm: cur_frm}));

cur_frm.cscript.hour_rate = function(doc) {
	erpadda.bom.calculate_op_cost(doc);
	erpadda.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return vmraid.call({
			...
			callback: function(r) {
				...
				erpadda.bom.calculate_rm_cost(doc);
				erpadda.bom.calculate_scrap_materials_cost(doc);
				erpadda.bom.calculate_total(doc);
			},
			freeze: true
		});
...
cur_frm.cscript.rate = function(doc, cdt, cdn) {
	...
	if (d.bom_no) {
		vmraid.msgprint(__("You cannot change the rate if BOM is mentioned against any Item."));
		get_bom_material_detail(doc, cdt, cdn, scrap_items);
	} else {
		erpadda.bom.calculate_rm_cost(doc);
		erpadda.bom.calculate_scrap_materials_cost(doc);
...
erpadda.bom.update_cost = function(doc) {
	erpadda.bom.calculate_op_cost(doc);
	erpadda.bom.calculate_rm_cost(doc);
	erpadda.bom.calculate_scrap_materials_cost(doc);
	erpadda.bom.calculate_total(doc);
...
erpadda.bom.calculate_op_cost = function(doc) {
	...
	for(var i=0;i<op.length;i++) {
		...
		vmraid.model.set_value('BOM Operation',op[i].name, "operating_cost", operating_cost);
		vmraid.model.set_value('BOM Operation',op[i].name, "base_operating_cost", base_operating_cost);
...
// rm : raw material
erpadda.bom.calculate_rm_cost = function(doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
	...
	for(var i=0;i<rm.length;i++) {
		...
		vmraid.model.set_value('BOM Item', rm[i].name, 'base_rate',
			flt(rm[i].rate) * flt(doc.conversion_rate));
		vmraid.model.set_value('BOM Item', rm[i].name, 'amount', amount);
		vmraid.model.set_value('BOM Item', rm[i].name, 'base_amount', base_amount);
		vmraid.model.set_value('BOM Item', rm[i].name,
...
// sm : scrap material
erpadda.bom.calculate_scrap_materials_cost = function(doc) {
	...
	for(var i=0;i<sm.length;i++) {
		...
		vmraid.model.set_value('BOM Scrap Item',sm[i].name, 'base_rate', base_rate);
		vmraid.model.set_value('BOM Scrap Item',sm[i].name, 'amount', amount);
		vmraid.model.set_value('BOM Scrap Item',sm[i].name, 'base_amount', base_amount);
...
erpadda.bom.calculate_total = function(doc) {
...
vmraid.ui.form.on("BOM Operation", "operation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
vmraid.ui.form.on("BOM Operation", "workstation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	vmraid.call({
		...
		callback: function (data) {
			vmraid.model.set_value(d.doctype, d.name, "base_hour_rate", data.message.hour_rate);
			vmraid.model.set_value(d.doctype, d.name, "hour_rate",
...
vmraid.ui.form.on("BOM Item", "qty", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	d.stock_qty = d.qty * d.conversion_factor;
	refresh_field("stock_qty", d.name, d.parentfield);
});

vmraid.ui.form.on("BOM Item", "item_code", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	vmraid.db.get_value('Item', {name: d.item_code}, 'allow_alternative_item', (r) => {
		d.allow_alternative_item = r.allow_alternative_item
	})
	refresh_field("allow_alternative_item", d.name, d.parentfield);
});

vmraid.ui.form.on("BOM Item", "sourced_by_supplier", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
vmraid.ui.form.on("BOM Item", "rate", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.sourced_by_supplier) {
		d.rate = 0;
		refresh_field("rate", d.name, d.parentfield);
...
vmraid.ui.form.on("BOM Operation", "operations_remove", function(frm) {
	erpadda.bom.calculate_op_cost(frm.doc);
	erpadda.bom.calculate_total(frm.doc);
});

vmraid.ui.form.on("BOM Item", "items_remove", function(frm) {
	erpadda.bom.calculate_rm_cost(frm.doc);
	erpadda.bom.calculate_total(frm.doc);
});
...
vmraid.ui.form.on("BOM Scrap Item", {
