...
from erpadda.manufacturing.doctype.bom.bom import item_query, make_variant_bom
...
from erpadda.tests.test_subcontracting import set_backflush_based_on

test_records = vmraid.get_test_records("BOM")
test_dependencies = ["Item", "Quality Inspection Template"]


class TestBOM(VMRaidTestCase):
	...
	def test_get_items_list(self):
		from erpadda.manufacturing.doctype.bom.bom import get_bom_items

		self.assertEqual(len(get_bom_items(bom=get_default_bom(), company="_Test Company")), 3)

	def test_default_bom(self):
		def _get_default_bom_in_item():
			return cstr(vmraid.db.get_value("Item", "_Test FG Item 2", "default_bom"))

		bom = vmraid.get_doc("BOM", {"item": "_Test FG Item 2", "is_default": 1})
	...
	def test_bom_cost(self):
		bom = vmraid.copy_doc(test_records[2])
		bom.insert()
	...
	def test_bom_cost_multi_uom_multi_currency_based_on_price_list(self):
		vmraid.db.set_value("Price List", "_Test Price List", "price_not_uom_dependent", 1)
	...
	def test_subcontractor_sourced_item(self):
		...
		if not vmraid.db.exists("Item", "Test Extra Item 3"):
	...
	def test_bom_recursion_transitive(self):
		...
		bom1 = vmraid.new_doc("BOM")
		bom1.item = item1
		bom1.append("items", vmraid._dict(item_code=item2))
		bom1.save()
		bom1.submit()

		bom2 = vmraid.new_doc("BOM")
		bom2.item = item2
		bom2.append("items", vmraid._dict(item_code=item1))
		...
		bom1.cancel()
		vmraid.delete_doc("BOM", bom1.name, ignore_missing=True, force=True)
		vmraid.delete_doc("BOM", bom2.name, ignore_missing=True, force=True)

	def test_bom_with_process_loss_item(self):
		fg_item_non_whole, fg_item_whole, bom_item = create_process_loss_bom_items()

		if not vmraid.db.exists("BOM", f"BOM-{fg_item_non_whole.item_code}-001"):
		...
		self.assertRaises(vmraid.ValidationError, bom_doc.submit)
	...
	def test_exclude_exploded_items_from_bom(self):
		...
		new_bom = vmraid.copy_doc(vmraid.get_doc("BOM", bom_no))
	...
	def test_valid_transfer_defaults(self):
		bom_with_op = vmraid.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "with_operations": 1, "is_active": 1}
		)
		bom = vmraid.copy_doc(vmraid.get_doc("BOM", bom_with_op), ignore_no_copy=False)
		...
		bom.reload()
		bom.transfer_material_against = None
		with self.assertRaises(vmraid.ValidationError):
			bom.save()
		bom.reload()
	...
	def test_bom_versioning(self):
		bom_tree = {vmraid.generate_hash(length=10): {vmraid.generate_hash(length=10): {}}}
		...
		version = vmraid.copy_doc(amendment)
		version.docstatus = 0
	...
	def test_clear_inpection_quality(self):
		...
		bom = vmraid.copy_doc(test_records[2], ignore_no_copy=True)
		bom.docstatus = 0
		bom.is_default = 0
	...
	def test_bom_pricing_based_on_lpp(self):
		...
		parent = vmraid.generate_hash(length=10)
		child = vmraid.generate_hash(length=10)
...
def get_default_bom(item_code="_Test FG Item 2"):
	return vmraid.db.get_value("BOM", {"item": item_code, "is_active": 1, "is_default": 1})
...
def create_nested_bom(tree, prefix="_Test bom "):
	...
	def create_items(bom_tree):
		for item_code, subtree in bom_tree.items():
			...
			if not vmraid.db.exists("Item", bom_item_code):
				vmraid.get_doc(doctype="Item", item_code=bom_item_code, item_group="_Test Item Group").insert()
	...
	for item in order_of_creating_bom:
		...
		if child_items:
			...
			bom = vmraid.get_doc(doctype="BOM", item=bom_item_code)
...
def reset_item_valuation_rate(item_code, warehouse_list=None, qty=None, rate=None):
	...
	if not warehouse_list:
		warehouse_list = vmraid.db.sql_list(
			"""
...
def create_process_loss_bom_item(item_tuple):
	...
	if vmraid.db.exists("Item", item_code) is None:
