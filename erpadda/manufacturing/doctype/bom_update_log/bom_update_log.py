...
class BOMMissingError(vmraid.ValidationError):
	pass
...
class BOMUpdateLog(Document):
	...
	def validate_bom_items(self):
		current_bom_item = vmraid.db.get_value("BOM", self.current_bom, "item")
		new_bom_item = vmraid.db.get_value("BOM", self.new_bom, "item")
	...
	def on_submit(self):
		...
		if self.update_type == "Replace BOM":
			...
			vmraid.enqueue(
				method="erpadda.manufacturing.doctype.bom_update_log.bom_update_log.run_bom_job",
				doc=self,
				boms=boms,
				timeout=40000,
			)
...
def replace_bom(boms: Dict) -> None:
	...
	for bom in parent_boms:
		bom_obj = vmraid.get_doc("BOM", bom)
...
def update_new_bom_in_bom_items(unit_cost: float, current_bom: str, new_bom: str) -> None:
	bom_item = vmraid.qb.DocType("BOM Item")
	(
		vmraid.qb.update(bom_item)
...
def get_parent_boms(new_bom: str, bom_list: Optional[List] = None) -> List:
	...
	for d in parents:
		if new_bom == d.parent:
			vmraid.throw(_("BOM recursion: {0} cannot be child of {1}").format(new_bom, d.parent))
...
def get_new_bom_unit_cost(new_bom: str) -> float:
	...
	new_bom_unitcost = (
		vmraid.qb.from_(bom).select(bom.total_cost / bom.quantity).where(bom.name == new_bom).run()
	)
...
def run_bom_job(
	...
	try:
		...
		vmraid.db.rollback()
		error_log = vmraid.log_error(message=vmraid.get_traceback(), title=_("BOM Update Tool Error"))
