...
from erpadda.manufacturing.doctype.bom_update_log.bom_update_log import (
	BOMMissingError,
	run_bom_job,
)
from erpadda.manufacturing.doctype.bom_update_tool.bom_update_tool import enqueue_replace_bom

test_records = vmraid.get_test_records("BOM")


class TestBOMUpdateLog(VMRaidTestCase):
	...
	def tearDown(self):
		...
		if self._testMethodName == "test_bom_update_log_completion":
			...
			vmraid.db.delete("BOM Update Log")
	...
	def test_bom_update_log_validate(self):
		...
		with self.assertRaises(vmraid.ValidationError):
			enqueue_replace_bom(boms=vmraid._dict(current_bom=self.boms.new_bom, new_bom=self.boms.new_bom))

		with self.assertRaises(vmraid.ValidationError):
			enqueue_replace_bom(boms=vmraid._dict(current_bom=self.boms.new_bom, new_bom="Dummy BOM"))
