...
from vmraid.utils import add_to_date, flt, now_datetime, nowdate

from erpadda.controllers.item_variant import create_variant
...
class TestProductionPlan(VMRaidTestCase):
	def setUp(self):
		for item in [
			...
			sr = vmraid.db.get_value(
				"Stock Reconciliation Item", {"item_code": item, "docstatus": 1}, "parent"
			)
			if sr:
				sr_doc = vmraid.get_doc("Stock Reconciliation", sr)
				sr_doc.cancel()
	...
	def test_production_plan_mr_creation(self):
		...
		material_requests = vmraid.get_all(
		...
		for name in material_requests:
			mr = vmraid.get_doc("Material Request", name[0])
			if mr.docstatus != 0:
				mr.cancel()

		for name in work_orders:
			mr = vmraid.delete_doc("Work Order", name[0])

		pln = vmraid.get_doc("Production Plan", pln.name)
		pln.cancel()
	...
	def test_production_plan_start_date(self):
		...
		for wo in work_orders:
			vmraid.delete_doc("Work Order", wo.name)
	...
	def test_production_plan_sales_orders(self):
		...
		pln = vmraid.new_doc("Production Plan")
		...
		work_order = vmraid.db.get_value(
			"Work Order",
			{"sales_order": sales_order, "production_plan": pln.name, "sales_order_item": sales_order_item},
			"name",
		)

		wo_doc = vmraid.get_doc("Work Order", work_order)
		wo_doc.update(
			{"wip_warehouse": "Work In Progress - _TC", "fg_warehouse": "Finished Goods - _TC"}
		)
		wo_doc.submit()

		so_wo_qty = vmraid.db.get_value("Sales Order Item", sales_order_item, "work_order_qty")
	...
	def test_production_plan_combine_items(self):
		...
		for so_item in so_items:
			so_wo_qty = vmraid.db.get_value("Sales Order Item", so_item, "work_order_qty")
	...
	def test_production_plan_combine_subassembly(self):
		...
		vmraid.db.set_value("BOM Item", parent_bom_2.items[0].name, "bom_no", subassembly_bom)
		...
		plan.append(
			...
			{  # Add Green-Car to Prod Plan
				...
				"item_code": "Green-Car",
				"bom_no": vmraid.db.get_value("Item", "Green-Car", "default_bom"),
				"planned_qty": 1,
	...
	def test_pp_to_mr_customer_provided(self):
		...
		material_request = vmraid.db.get_value(
			"Material Request Item",
			{"production_plan": production_plan.name, "item_code": "CUST-0987"},
			"parent",
		)
		mr = vmraid.get_doc("Material Request", material_request)
	...
	def test_production_plan_with_multi_level_bom(self):
		...
		if not vmraid.db.get_value("BOM", {"item": "Test BOM 3"}):
			make_bom(item="Test BOM 3", raw_materials=["Test RM BOM 1"], rm_qty=3)

		if not vmraid.db.get_value("BOM", {"item": "Test BOM 2"}):
			make_bom(item="Test BOM 2", raw_materials=["Test BOM 3"], rm_qty=3)

		if not vmraid.db.get_value("BOM", {"item": "Test BOM 1"}):
		...
		pln.append(
			...
			{
				"item_code": item_code,
				"bom_no": vmraid.db.get_value("BOM", {"item": "Test BOM 1"}),
				"planned_qty": 3,
			},
		)
	...
	def test_get_sales_order_with_variant(self):
		...
		if not vmraid.db.exists("Item", {"item_code": "PIV"}):
		...
		if not vmraid.db.exists("BOM", {"item": "PIV"}):
			parent_bom = make_bom(item="PIV", raw_materials=[rm_item.item_code])
		else:
			parent_bom = vmraid.get_doc("BOM", {"item": "PIV"})

		if not vmraid.db.exists("Item", {"item_code": "PIV-RED"}):
			variant = create_variant("PIV", {"Colour": "Red"})
			variant.save()
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		else:
			variant = vmraid.get_doc("Item", "PIV-RED")
		if not vmraid.db.exists("BOM", {"item": "PIV-RED"}):
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		...
		so.cancel()
		vmraid.delete_doc("Sales Order", so.name)
		variant_bom.cancel()
		vmraid.delete_doc("BOM", variant_bom.name)
	...
	def test_subassmebly_sorting(self):
		...
		plan.append(
			"po_items",
			{
				"use_multi_level_bom": 1,
				"item_code": items[1],
				"bom_no": vmraid.db.get_value("Item", items[1], "default_bom"),
				"planned_qty": 1,
				"planned_start_date": now_datetime(),
			},
		)
	...
	def test_multiple_work_order_for_production_plan_item(self):
		...
		def create_work_order(item, pln, qty):
			...
			for key, item in items_data.items():
				...
				wo_doc = vmraid.get_doc("Work Order", wo_name)
				wo_doc.update(
					{"wip_warehouse": "Work In Progress - _TC", "fg_warehouse": "Finished Goods - _TC"}
				)
				wo_doc.submit()
		...
		wo1 = vmraid.get_doc("Work Order", wo_list[0])
		wo1.cancel()
		pln.reload()
		self.assertEqual(pln.po_items[0].ordered_qty, 2)

		# Cancel 2nd Work Order
		wo2 = vmraid.get_doc("Work Order", wo_list[1])
		wo2.cancel()
		pln.reload()
	...
	def test_production_plan_pending_qty_with_sales_order(self):
		...
		se = vmraid.get_doc(make_se_from_wo(wo.name, "Manufacture", 1))
		se.submit()
	...
	def test_qty_based_status(self):
		pp = vmraid.new_doc("Production Plan")
		pp.po_items = [vmraid._dict(planned_qty=5, produce_qty=4)]
		self.assertFalse(pp.all_items_completed())

		pp.po_items = [
			vmraid._dict(planned_qty=5, produce_qty=10),
			vmraid._dict(planned_qty=5, produce_qty=4),
		]
		self.assertFalse(pp.all_items_completed())
	...
	def test_production_plan_planned_qty(self):
		...
		pln.make_work_order()
		work_order = vmraid.db.get_value("Work Order", {"production_plan": pln.name}, "name")
		wo_doc = vmraid.get_doc("Work Order", work_order)
		wo_doc.update(
			{"wip_warehouse": "Work In Progress - _TC", "fg_warehouse": "Finished Goods - _TC"}
		)
	...
	def test_temporary_name_relinking(self):
		...
		for _ in range(10):
			po_item = pp.append(
				"po_items",
				{
					...
					"temporary_name": vmraid.generate_hash(length=10),
				},
			)
		...
		pp.append("sub_assembly_items", {"production_plan_item": vmraid.generate_hash(length=16)})
		pp._rename_temporary_references()
	...
	if not args.get("sales_order"):
		pln.append(
			"po_items",
			{
				...
				"bom_no": vmraid.db.get_value("Item", args.item_code, "default_bom"),
