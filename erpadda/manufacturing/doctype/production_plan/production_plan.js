...
vmraid.ui.form.on('Production Plan', {
	...
	setup: function(frm) {
		...
		frm.fields_dict['po_items'].grid.get_field('bom_no').get_query = function(doc, cdt, cdn) {
			...
			} else vmraid.msgprint(__("Please enter Item first"));
		}
		...
		if (frm.doc.docstatus === 1) {
			...
			if (frm.doc.status !== "Completed") {
				frm.add_custom_button(__("Work Order Tree"), ()=> {
					vmraid.set_route('Tree', 'Work Order', {production_plan: frm.doc.name});
				}, __('View'));

				frm.add_custom_button(__("Production Plan Summary"), ()=> {
					vmraid.set_route('query-report', 'Production Plan Summary', {production_plan: frm.doc.name});
				}, __('View'));
		...
		const projected_qty_formula = ` <table class="table table-bordered" style="background-color: var(--scrollbar-track-color);">
			<tr><td style="padding-left:25px">
				...
				<h3 style="text-decoration: underline;">
					<a href = "https://erpadda.com/docs/user/manual/en/stock/projected-quantity">
	...
	make_material_request: function(frm) {
		...
		vmraid.confirm(__("Do you want to submit the material request"),
			function() {
	...
	get_items_for_mr: function(frm) {
		if (!frm.doc.for_warehouse) {
			vmraid.throw(__("To make material requests, 'Make Material Request for Warehouse' field is mandatory"));
		}
	...
	get_items_for_material_requests: function(frm, warehouses) {
		...
		vmraid.call({
			method: "erpadda.manufacturing.doctype.production_plan.production_plan.get_items_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
	...
	download_materials_required: function(frm) {
		...
		vmraid.prompt(fields, (row) => {
			let get_template_url = 'erpadda.manufacturing.doctype.production_plan.production_plan.download_raw_materials';
...
vmraid.ui.form.on("Production Plan Item", {
	item_code: function(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.item_code) {
			vmraid.call({
				method: "erpadda.manufacturing.doctype.production_plan.production_plan.get_item_data",
				args: {
					item_code: row.item_code
				},
				callback: function(r) {
					for (let key in r.message) {
						vmraid.model.set_value(cdt, cdn, key, r.message[key]);
					}
				}
			});
		}
...
vmraid.ui.form.on("Material Request Plan Item", {
	warehouse: function(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.warehouse && row.item_code && frm.doc.company) {
			vmraid.call({
				method: "erpadda.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row,
					company: frm.doc.company,
				...
				callback: function(r) {
					...
					vmraid.model.set_value(cdt, cdn, 'projected_qty', projected_qty);
					vmraid.model.set_value(cdt, cdn, 'actual_qty', actual_qty);
				}
			})
		}
	}
...
vmraid.ui.form.on("Production Plan Sales Order", {
	sales_order(frm, cdt, cdn) {
		...
		vmraid.call({
			method: "erpadda.manufacturing.doctype.production_plan.production_plan.get_so_details",
			args: { sales_order },
			callback(r) {
				const {transaction_date, customer, grand_total} = r.message;
				vmraid.model.set_value(cdt, cdn, 'sales_order_date', transaction_date);
				vmraid.model.set_value(cdt, cdn, 'customer', customer);
				vmraid.model.set_value(cdt, cdn, 'grand_total', grand_total);
			}
		});
	}
});
