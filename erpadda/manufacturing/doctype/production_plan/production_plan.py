...
from erpadda.manufacturing.doctype.bom.bom import get_children as get_bom_children
from erpadda.manufacturing.doctype.bom.bom import validate_bom_no
from erpadda.manufacturing.doctype.work_order.work_order import get_item_details
...
class ProductionPlan(Document):
	...
	def validate_data(self):
		for d in self.get("po_items"):
			if not d.bom_no:
				vmraid.throw(_("Please select BOM for Item in Row {0}").format(d.idx))
			else:
				validate_bom_no(d.item_code, d.bom_no)

			if not flt(d.planned_qty):
				vmraid.throw(_("Please enter Planned Qty for Item {0} at row {1}").format(d.item_code, d.idx))
	...
	def get_open_sales_orders(self):
		...
		if open_so:
			...
			vmraid.msgprint(_("Sales orders are not available for production"))
	...
	def get_bom_item(self):
		...
		bom_item = None
		has_bom = vmraid.db.exists({"doctype": "BOM", "item": self.item_code, "docstatus": 1})
		if not has_bom:
			template_item = vmraid.db.get_value("Item", self.item_code, ["variant_of"])
			bom_item = (
				"bom.item = {0}".format(vmraid.db.escape(template_item)) if template_item else bom_item
			)
		return bom_item
	...
	def get_so_items(self):
		...
		if not self.get("sales_orders") or not self.get_so_mr_list("sales_order", "sales_orders"):
			vmraid.throw(_("Please fill the Sales Orders table"), title=_("Sales Orders Required"))

		so_list = self.get_so_mr_list("sales_order", "sales_orders")

		item_condition = ""
		bom_item = "bom.item = so_item.item_code"
		if self.item_code and vmraid.db.exists("Item", self.item_code):
			bom_item = self.get_bom_item() or bom_item
			item_condition = " and so_item.item_code = {0}".format(vmraid.db.escape(self.item_code))
	...
	def get_mr_items(self):
		...
		item_condition = ""
		if self.item_code:
			item_condition = " and mr_item.item_code ={0}".format(vmraid.db.escape(self.item_code))
	...
	def delete_draft_work_order(self):
		for d in vmraid.get_all(
			...
			vmraid.delete_doc("Work Order", d.name)
	...
	def get_production_items(self):
		...
		for d in self.po_items:
			...
			if not item_details["project"] and d.sales_order:
				item_details["project"] = vmraid.get_cached_value("Sales Order", d.sales_order, "project")
	...
	def make_work_order(self):
		from erpadda.manufacturing.doctype.work_order.work_order import get_default_warehouse
	...
	def show_list_created_message(self, doctype, doc_list=None):
		...
		vmraid.flags.mute_messages = False
		if doc_list:
			doc_list = [get_link_to_form(doctype, p) for p in doc_list]
			msgprint(_("{0} created").format(comma_and(doc_list)))

	def create_work_order(self, item):
		from erpadda.manufacturing.doctype.work_order.work_order import OverProductionError

		wo = vmraid.new_doc("Work Order")
		wo.update(item)
	...
	def make_material_request(self):
		...
		for item in self.mr_items:
			item_doc = vmraid.get_cached_doc("Item", item.item_code)
			...
			if not key in material_request_map:
				...
				material_request_map[key] = vmraid.new_doc("Material Request")
			...
			material_request.append(
				"items",
				{
					...
					"project": vmraid.db.get_value("Sales Order", item.sales_order, "project")
					if item.sales_order
					else None,
				},
			)
...
def download_raw_materials(doc, warehouses=None):
	...
	doc.warehouse = None
	vmraid.flags.show_qty_in_stock_uom = 1
...
def get_material_request_items(
	...
	if row["purchase_uom"] != row["stock_uom"]:
		if not (row["conversion_factor"] or vmraid.flags.show_qty_in_stock_uom):
			vmraid.throw(
	...
	if vmraid.db.get_value("UOM", row["purchase_uom"], "must_be_whole_number"):
...
def get_bin_details(row, company, for_warehouse=None, all_warehouse=False):
	if isinstance(row, str):
		row = vmraid._dict(json.loads(row))

	company = vmraid.db.escape(company)
	...
	if warehouse:
		lft, rgt = vmraid.db.get_value("Warehouse", warehouse, ["lft", "rgt"])
...
def get_warehouse_list(warehouses):
	...
	for row in warehouses:
		child_warehouses = vmraid.db.get_descendants("Warehouse", row.get("warehouse"))
		if child_warehouses:
...
def get_items_for_material_requests(doc, warehouses=None, get_parent_warehouse_data=None):
	...
	for data in po_items:
		...
		if data.get("bom") or data.get("bom_no"):
			...
			if not planned_qty:
				vmraid.throw(_("For row {0}: Enter Planned Qty").format(data.get("idx")))
		...
		for item_code, details in item_details.items():
			so_item_details.setdefault(sales_order, vmraid._dict())
	...
	if not mr_items:
		to_enable = vmraid.bold(_("Ignore Existing Projected Quantity"))
		warehouse = vmraid.bold(doc.get("for_warehouse"))
		message = (
			_(
		...
		vmraid.msgprint(message, title=_("Note"))

	return mr_items


def get_materials_from_other_locations(item, warehouses, new_mr_items, company):
	from erpadda.stock.doctype.pick_list.pick_list import get_available_item_locations
	...
	if required_qty:
		stock_uom, purchase_uom = vmraid.db.get_value(
		...
		if purchase_uom != stock_uom and purchase_uom == item["uom"]:
			...
			if not (conversion_factor or vmraid.flags.show_qty_in_stock_uom):
				vmraid.throw(
					_("UOM Conversion factor ({0} -> {1}) not found for item: {2}").format(
		...
		if vmraid.db.get_value("UOM", purchase_uom, "must_be_whole_number"):
			required_qty = ceil(required_qty)
...
def get_sub_assembly_items(bom_no, bom_data, to_produce_qty, indent=0):
	...
	for d in data:
		if d.expandable:
			parent_item_code = vmraid.get_cached_value("BOM", bom_no, "item")
