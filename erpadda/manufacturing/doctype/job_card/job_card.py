...
class OperationMismatchError(vmraid.ValidationError):
	pass


class OperationSequenceError(vmraid.ValidationError):
	pass


class JobCardCancelError(vmraid.ValidationError):
	pass


class JobCard(Document):
	def onload(self):
		excess_transfer = vmraid.db.get_single_value(
		...
		if self.get("time_logs"):
			for d in self.get("time_logs"):
				if d.to_time and get_datetime(d.from_time) > get_datetime(d.to_time):
					vmraid.throw(_("Row {0}: From time must be less than to time").format(d.idx))
	...
	def get_overlap_for(self, args, check_next_available_slot=False):
		...
		if self.workstation:
			production_capacity = (
				vmraid.get_cached_value("Workstation", self.workstation, "production_capacity") or 1
			)
	...
	def schedule_time_logs(self, row):
		...
		while row.remaining_time_in_mins > 0:
			args = vmraid._dict({"from_time": row.planned_start_time, "to_time": row.planned_end_time})
	...
	def check_workstation_time(self, row):
		workstation_doc = vmraid.get_cached_doc("Workstation", self.workstation)
		if not workstation_doc.working_hours or cint(
			vmraid.db.get_single_value("Manufacturing Settings", "allow_overtime")
		):
	...
	def get_required_items(self):
		...
		doc = vmraid.get_doc("Work Order", self.get("work_order"))
			...
			if self.get("operation") == d.operation:
				self.append(
					"items",
					{
						...
						"uom": vmraid.db.get_value("Item", d.item_code, "stock_uom"),
	...
	def validate_job_card(self):
		if (
			self.work_order
			and vmraid.get_cached_value("Work Order", self.work_order, "status") == "Stopped"
		):
			vmraid.throw(
	...
	def validate_produced_quantity(self, for_quantity, wo):
		...
		if wo.produced_qty > for_quantity:
			first_part_msg = _(
				...
				vmraid.bold(_("Job Card")), vmraid.bold(self.name), vmraid.bold(self.production_item)
			)

			second_part_msg = _(
				"Kindly cancel the Manufacturing Entries first against the work order {0}."
			).format(vmraid.bold(get_link_to_form("Work Order", self.work_order)))
	...
	def set_transferred_qty_in_job_card(self, ste_doc):
		for row in ste_doc.items:
			...
			vmraid.db.set_value("Job Card Item", row.job_card_item, "transferred_qty", flt(qty))
	...
	def set_transferred_qty(self, update_status=False):
		...
		qty = 0
		if self.work_order:
			doc = vmraid.get_doc("Work Order", self.work_order)
	...
	def validate_operation_id(self):
		if (
			...
			and self.operation
			and self.work_order
			and vmraid.get_cached_value("Work Order Operation", self.operation_row_number, "name")
			!= self.operation_id
		):
	...
	def validate_work_order(self):
		if self.is_work_order_closed():
			vmraid.throw(_("You can't make any changes to Job Card since Work Order is closed."))

	def is_work_order_closed(self):
		if self.work_order:
			status = vmraid.get_value("Work Order", self.work_order)
...
def make_time_log(args):
	...
	doc = vmraid.get_doc("Job Card", args.job_card_id)
...
def get_operations(doctype, txt, searchfield, start, page_len, filters):
	if not filters.get("work_order"):
		vmraid.msgprint(_("Please select a Work Order first."))
		return []
...
def make_stock_entry(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		wo_allows_alternate_item = vmraid.db.get_value(
			"Work Order", target.work_order, "allow_alternative_item"
		)
		for item in target.items:
			item.allow_alternative_item = int(
				wo_allows_alternate_item
				and vmraid.get_cached_value("Item", item.item_code, "allow_alternative_item")
			)
