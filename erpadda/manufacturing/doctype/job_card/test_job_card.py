...
from erpadda.manufacturing.doctype.job_card.job_card import OperationMismatchError, OverlapError
from erpadda.manufacturing.doctype.job_card.job_card import (
...
class TestJobCard(VMRaidTestCase):
	def setUp(self):
	...
	def test_job_card_with_different_work_station(self):
		...
		if job_card:
			workstation = vmraid.db.get_value(
			...
			completed_qty = vmraid.db.get_value(
	...
	def test_job_card_overlap(self):
		...
		jc1_name = vmraid.db.get_value("Job Card", {"work_order": self.work_order.name})
		jc2_name = vmraid.db.get_value("Job Card", {"work_order": wo2.name})

		jc1 = vmraid.get_doc("Job Card", jc1_name)
		jc2 = vmraid.get_doc("Job Card", jc2_name)
	...
	def test_job_card_multiple_materials_transfer(self):
		...
		job_card_name = vmraid.db.get_value("Job Card", {"work_order": self.work_order.name})
		job_card = vmraid.get_doc("Job Card", job_card_name)
	...
	def test_job_card_material_transfer_correctness(self):
		...
		job_card_name = vmraid.db.get_value(
...
def create_bom_with_multiple_operations():
	...
	test_record = vmraid.get_test_records("BOM")[2]
	bom_doc = vmraid.get_doc(test_record)
