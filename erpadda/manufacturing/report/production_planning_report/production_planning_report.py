...
class ProductionPlanReport(object):
	...
	def prepare_other_conditions(self, filters, doctype):
		...
		if self.filters.company:
			filters += " and `tab%s`.company = %s" % (doctype, vmraid.db.escape(self.filters.company))
	...
	def get_raw_materials(self):
		...
		if self.filters.based_on == "Work Order":
			...
			for d in self.orders:
				bom_no = d.bom_no or vmraid.get_cached_value("Item", d.production_item, "default_bom")
	...
	def get_purchase_details(self):
		...
		purchased_items = vmraid.get_all(
			"Purchase Order Item",
